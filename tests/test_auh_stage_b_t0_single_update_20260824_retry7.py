...
TEMPLATE = DOC / "stage_b_t0_single_update_retry7_authority_addendum.template.json"
ACTIVE = DOC / "stage_b_t0_single_update_retry7_authority_addendum.json"
LAUNCHER = (
    REPO
    / "methods"
    / "bernini_action_editing"
    / "scripts"
    / "auh_stage_b_t0_single_update_20260824_retry7.sh"
)
...
class StageBT0SingleUpdateStaticContractTest(unittest.TestCase):
    ...
    def test_template_is_explicitly_inert_until_final_hash_lock(self) -> None:
        ...
        self.assertEqual(
            ...
            {
                "methods/bernini_action_editing/score_g1_joint_action_repr_admission_v1.py": "04cbb9093fc6e59b9db996fed05ba8aa121652f81bfbca6fd0b63f0c3802c041",
                "methods/bernini_action_editing/evaluate_g1_action_repr_selectivity_v1.py": "43008c560637954dc7384d35b230082edd771c02be1431a7a80a705afeea7509",
                "methods/bernini_action_editing/materialize_g1_flow_control_cohort_v1.py": "2fc195596af388ead9e936555938acf75586036250fc292b6f24f8debe58100b",
                "methods/bernini_action_editing/materialize_g1_middle_control_cohort_v1.py": "ad7e69d058570195ab790eb7500bfedf19ac7f80637dc52913e2a91a439d9d0c",
            },
        )

    def test_active_retry7_authority_when_present_is_create_once_and_source_pinned(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_stage_b_t0_retry7",
        )
        self.assertEqual(
            active["runtime_paths"]["fresh_stage_root_name"],
            "stage_b_t0_retry7",
        )
        self_relative = "tests/test_auh_stage_b_t0_single_update_20260824_retry7.py"
    ...
    def test_g1_replay_threads_and_strict_legacy_contract_are_closed(self) -> None:
        self.assertEqual(
            self.authority["g1_replay_thread_contract"],
            {
        ...
        self.assertEqual(
            self.authority["runtime_execution_contract"],
            {
        ...
        for token in (
            ...
            "ACTION_REPR_T0_TORCH_NUM_THREADS=1",
            "ACTION_REPR_T0_TORCH_NUM_INTEROP_THREADS=1",
            "ACTION_REPR_T0_STRICT_LEGACY_REPLAY=true",
            "strict_legacy_diagnostics_bitwise_replay",
            "complete_transitive_g1_verifier_hash_pins_required",
        ...
        for relative in (
            ...
            self.assertIn(f"require_source_pin methods/bernini_action_editing/{relative}", self.script)
    ...
    def test_launcher_parses_and_rejects_draft_or_unpinned_authority(self) -> None:
        ...
        self.assertIn('source_root="$experiment_root/source_stage_b_t0_retry7"', self.script)
        self.assertIn('stage_root="$experiment_root/stage_b_t0_retry7"', self.script)
        ...
        if runner_path.exists():
            self.assertIn("O_EXCL", runner_path.read_text(encoding="utf-8"))
    ...
    def test_retry7_permanent_preoptimizer_claim_contract_is_closed(self) -> None:
        ...
        self.assertEqual(
            ...
            str(Path(expected_output).parent / ".single_update.retry7.attempt_claim.json"),
        )
        self.assertTrue(output["attempt_claim_permanent_on_failure"])
        self.assertTrue(output["existing_claim_requires_new_revision"])
        self.assertEqual(output["next_revision_after_claim_failure"], "retry8")
        ...
        self.assertIn("use retry8", status)
    ...
    def _run_unclaimed_launcher_contract(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            claim = root / ".single_update.retry7.attempt_claim.json"
    ...
    def test_launcher_rejects_claimed_incomplete_symlink_and_output_without_claim(self) -> None:
        ...
        for marker in ("file", "symlink"):
            with self.subTest(marker=marker):
                ...
                self.assertIn("use retry8", claimed.stderr)
    ...
    def test_retry6_sealed_bytes_remain_unchanged(self) -> None:
        expected = {
            "methods/bernini_action_editing/train_action_repr_target_t0_canary_retry6_v1.py": "326b87e1b462baead05ca53f5bf578e50ecdcb1d72c7e55e2c9fce5e660292a6",
            "methods/bernini_action_editing/scripts/auh_stage_b_t0_single_update_20260824_retry6.sh": "17446af768535deaa84d1cf09c37324eee2e14a90cd8aeba8fa295d90ca583a7",
            "methods/bernini_action_editing/tests/test_train_action_repr_target_t0_canary_retry6_v1.py": "30978b223065a51acd57f0beff2c3830654c84ed54e994c9e4f9337442780cf2",
            "tests/test_auh_stage_b_t0_single_update_20260824_retry6.py": "be8a19e0b6b6825392df247b211e03138d504132d721b8040a3a463791e54c24",
            "md/action_editing/20260824_reward/stage_b_t0_single_update_retry6_authority_addendum.template.json": "a3b3fca04f1ce26a203fbef07358fa8e42f890334a88eda180f1cd847c078f42",
            "md/action_editing/20260824_reward/stage_b_t0_single_update_retry6_authority_addendum.json": "3f9ec056cfbd49c25561fb9bd5e218c53dcb044eab440060a2d27b3ee2471274",
        }
        for relative, digest in expected.items():
            with self.subTest(relative=relative):
                self.assertEqual(
