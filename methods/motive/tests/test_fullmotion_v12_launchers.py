...
SMOKE = (
    REPO_ROOT
    / "tmp"
    / "launch_fullmotion_v12_smoke_20260801T180000Z.sh"
)
SUPERVISOR = REPO_ROOT / "tmp" / "launch_wait_fullmotion_v12_job116234.sh"
SNAPSHOT_TEST_RUNNER = REPO_ROOT / "tmp" / "run_fullmotion_v12_snapshot_tests.sh"

RUN_ID = "fullmotion128_v12_20260801T180000Z"
SNAPSHOT_NAME = "goku-full-motion128-source-v12-20260801T180000Z"
TREE_PLACEHOLDER = "__V12_TREE_SHA__"
RELEASE_PLACEHOLDER = "__V12_RELEASE_CHALLENGE__"
SMOKE_INPUT_SHA = (
...
class FullMotionV12LauncherTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_fixed_names_placeholders_hashes_and_job_ids(self) -> None:
        ...
        self.assertIn("release_id=goku-full-motion128-v12-release", self.supervisor)
    ...
    def test_placeholders_are_explicitly_fail_closed(self) -> None:
        self.assertIn(
            'unbound_tree_sha_sentinel=__V12_"TREE"_SHA__', self.smoke
        )
        self.assertIn(
        ...
        self.assertIn(
            'unbound_tree_sha_sentinel=__V12_"TREE"_SHA__',
            self.supervisor,
        )
        self.assertIn(
            'unbound_release_challenge_sentinel=__V12_"RELEASE"_CHALLENGE__',
            self.supervisor,
        )
        self.assertIn(
    ...
    def test_snapshot_receipt_gates_precede_smoke_pid_claim_and_launch(self) -> None:
        smoke_initial = self.smoke.index(
            "sealed v12 snapshot-test receipt preflight failed"
        )
        smoke_job_lookup = self.smoke.index('job_record="$(scontrol show job')
        smoke_final = self.smoke.index(
            "sealed v12 snapshot-test receipt changed before smoke launch"
        )
        ...
        supervisor_final = self.supervisor.index(
            "sealed v12 snapshot-test receipt changed before allocation claim"
        )
        claim = self.supervisor.index(
            'atomic_publish_lines "${holder_claim}"'
        )
        pipeline_exec = self.supervisor.index("exec nohup env")
    ...
    def test_all_run_scoped_outputs_and_release_id_are_v12(self) -> None:
        output_markers = (
            "qwen_smoke8_v12",
...
"""
    ...
    def test_v6_gate_is_sealed_before_supervisor_claim(self) -> None:
        ...
        gate_validation = self.supervisor.index(
            "strict v12 smoke-gate preflight failed"
        )
    ...
    def test_supervisor_parent_child_launch_handshake_is_fail_closed(self) -> None:
        for marker in (
            "pipeline_v12_supervisor.pid",
            "pipeline_v12_launch_cancel_request.txt",
            "pipeline_v12_launch_token.txt",
