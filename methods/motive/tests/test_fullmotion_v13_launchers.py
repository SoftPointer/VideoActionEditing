...
SMOKE = (
    REPO_ROOT
    / "tmp"
    / "launch_fullmotion_v13_smoke_20260801T195500Z.sh"
)
SUPERVISOR = (
    REPO_ROOT
    / "tmp"
...
SNAPSHOT_TEST_RUNNER = REPO_ROOT / "tmp" / "run_fullmotion_v13_snapshot_tests.sh"

RUN_ID = "fullmotion128_v13_20260801T195500Z"
SNAPSHOT_NAME = "goku-full-motion128-source-v13-20260801T195500Z"
TREE_PLACEHOLDER = "__V13_TREE_SHA__"
RELEASE_PLACEHOLDER = "__V13_RELEASE_CHALLENGE__"
SMOKE_INPUT_SHA = (
...
class FullMotionV13LauncherTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_fixed_names_placeholders_hashes_and_job_ids(self) -> None:
        ...
        for text in (self.smoke, self.supervisor):
            self.assertIn(
                "control_dir=${base}/allocations/fullmotion128_g8_20260801T105000Z",
                text,
            )
        ...
        self.assertIn("release_id=goku-full-motion128-v13-release", self.supervisor)
    ...
    def test_placeholders_are_explicitly_fail_closed(self) -> None:
        self.assertIn(
            'unbound_tree_sha_sentinel=__V13_"TREE"_SHA__', self.smoke
        )
        self.assertIn(
        ...
        self.assertIn(
            'unbound_tree_sha_sentinel=__V13_"TREE"_SHA__',
            self.supervisor,
        )
        self.assertIn(
            'unbound_release_challenge_sentinel=__V13_"RELEASE"_CHALLENGE__',
            self.supervisor,
        )
        self.assertIn(
    ...
    def test_snapshot_receipt_gates_precede_smoke_pid_claim_and_launch(self) -> None:
        smoke_initial = self.smoke.index(
            "sealed v13 snapshot-test receipt preflight failed"
        )
        smoke_job_lookup = self.smoke.index('job_record="$(scontrol show job')
        smoke_final = self.smoke.index(
            "sealed v13 snapshot-test receipt changed before smoke launch"
        )
        ...
        supervisor_final = self.supervisor.index(
            "sealed v13 snapshot-test receipt changed before allocation claim"
        )
    ...
    def test_all_run_scoped_outputs_and_release_id_are_v13(self) -> None:
        output_markers = (
            "qwen_smoke8_v13",
            "qwen_smoke8_gate_v13.json",
            "qwen_full768_v13",
            "final_pool_v13",
            "wan_generation_v13",
            "postcheck_v13",
            "exact128_v13",
            "finalize_release_watcher_v13",
            "pipeline_v13",
...
"""
    ...
    def test_v6_gate_is_sealed_before_supervisor_claim(self) -> None:
        ...
        gate_validation = self.supervisor.index(
            "strict v13 smoke-gate preflight failed"
        )
    ...
    def test_supervisor_parent_child_launch_handshake_is_fail_closed(self) -> None:
        for marker in (
            "pipeline_v13_supervisor.pid",
            "pipeline_v13_launch_cancel_request.txt",
            "pipeline_v13_launch_token.txt",
