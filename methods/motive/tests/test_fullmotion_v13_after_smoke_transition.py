...
TRANSITION = (
    REPO_ROOT / "tmp" / "run_fullmotion_v13_after_smoke_transition.sh"
)
SUPERVISOR = (
    REPO_ROOT
    / "tmp"
    / "launch_wait_fullmotion_v13_job116234_envclosed_v2.sh"
)
...
TRANSITION_SCREENRC = (
    REPO_ROOT / "tmp" / "fullmotion_v13_after_smoke_transition.screenrc"
)
...
EXPECTED_HASHES = {
    SUPERVISOR: "55f9940ec0176c348e5ad53639a262b7ab0932303aaa7016569476fa97c31ab2",
    SIGNER: "d11981e3f3ad06d7b0ee32b9ad05aa62221fe0d6e43c10a1e37197c3d9f03127",
    WATCHER: "a969c5cc20720b42d9f3e2a9c0b624b858e64a89a1334c03b8ca51c39cdf27a5",
}


class FullMotionV13AfterSmokeTransitionTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_bash_syntax_and_private_mode(self) -> None:
        ...
        self.assertIn("fullmotion_v13_after_smoke_transition.log", screenrc)
        self.assertIn("logfile flush 1", screenrc)
        self.assertIn(
            "/usr/bin/env -i ... /bin/bash --noprofile --norc", self.script
        )
        self.assertIn(
            "fullmotion_v13_after_smoke_transition.launchd.out", self.script
        )
        self.assertIn(
            "fullmotion_v13_after_smoke_transition.launchd.err", self.script
        )
    ...
    def test_complete_transition_has_atomic_local_lock(self) -> None:
        ...
        for marker in (
            ".fullmotion_v13_after_smoke_transition.lock",
    ...
    def test_terminal_failure_and_invalid_gate_never_continue(self) -> None:
        ...
        self.assertIn(
            'fail "v13 smoke gate is terminal but unsafe or invalid',
            self.script,
        )

    def test_signer_launchd_job_is_clean_fresh_exact_and_verified(self) -> None:
        for marker in (
            "signer_label=com.siriuschu.fullmotion.v13.signer",
            ...
            "fullmotion_v13_signer.launchd.out",
            "fullmotion_v13_signer.launchd.err",
...
set -Eeuo pipefail
mode=$1
signer_service=gui/501/com.siriuschu.fullmotion.v13.signer
local_signer_watcher=/workspace/tmp/run_fullmotion_v13_signer_watch.sh
signer_stdout=/workspace/tmp/fullmotion_v13_signer.launchd.out
signer_stderr=/workspace/tmp/fullmotion_v13_signer.launchd.err
watcher_sha={'a' * 64}
...
"""
    ...
    def test_success_is_sealed_then_enters_inert_non_relaunching_state(self) -> None:
        ...
        for marker in (
            "fullmotion_v13_after_smoke_transition_terminal_envclosed_v2.txt",
            "motive-fullmotion-v13-after-smoke-transition-terminal-v2",
    ...
    def test_embedded_gate_validator_accepts_only_canonical_bound_pass(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            qwen_root = root_path / "qwen_smoke8_v13"
            qwen_root.mkdir()
            gate_path = root_path / "qwen_smoke8_gate_v13.json"
