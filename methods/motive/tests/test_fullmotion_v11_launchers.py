...
SMOKE = (
    REPO_ROOT
    / "tmp"
    / "launch_fullmotion_v11_smoke_20260801T164500Z.sh"
)
SUPERVISOR = REPO_ROOT / "tmp" / "launch_wait_fullmotion_v11_job116234.sh"

RUN_ID = "fullmotion128_v11_20260801T164500Z"
SNAPSHOT_NAME = "goku-full-motion128-source-v11-20260801T164500Z"
TREE_PLACEHOLDER = "__V11_TREE_SHA__"
RELEASE_PLACEHOLDER = "__V11_RELEASE_CHALLENGE__"
SMOKE_INPUT_SHA = (
...
class FullMotionV11LauncherTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_launchers_have_valid_bash_syntax(self) -> None:
        for script in (SMOKE, SUPERVISOR):
    ...
    def test_fixed_names_placeholders_hashes_and_job_ids(self) -> None:
        ...
        self.assertIn("release_id=goku-full-motion128-v11-release", self.supervisor)
    ...
    def test_placeholders_are_explicitly_fail_closed(self) -> None:
        self.assertIn(
            'unbound_tree_sha_sentinel=__V11_"TREE"_SHA__', self.smoke
        )
        self.assertIn(
        ...
        self.assertIn(
            'unbound_tree_sha_sentinel=__V11_"TREE"_SHA__',
            self.supervisor,
        )
        self.assertIn(
            'unbound_release_challenge_sentinel=__V11_"RELEASE"_CHALLENGE__',
            self.supervisor,
        )
        self.assertIn(
    ...
    def test_global_placeholder_binding_does_not_poison_sentinels(self) -> None:
        ...
        bound_smoke = self.smoke.replace(TREE_PLACEHOLDER, tree_sha)
        bound_supervisor = self.supervisor.replace(
            TREE_PLACEHOLDER, tree_sha
        ).replace(RELEASE_PLACEHOLDER, release_challenge)
    ...
    def test_v6_gate_is_sealed_before_supervisor_claim(self) -> None:
        ...
        gate_validation = self.supervisor.index(
            "strict v11 smoke-gate preflight failed"
        )
    ...
    def test_supervisor_parent_child_launch_handshake_is_fail_closed(self) -> None:
        for marker in (
            "pipeline_v11_supervisor.pid",
            "pipeline_v11_launch_cancel_request.txt",
            "pipeline_v11_launch_token.txt",
