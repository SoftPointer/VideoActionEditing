#!/usr/bin/env python3
"""Render forward/noop-only shards from the frozen prospective fit manifest.

This runtime deliberately excludes the failed independently prompted reverse,
incomplete, camera, appearance, and wrong-owner branches.  Forward is a frozen
Bernini self-generated action anchor; noop is the exact registered source.
Neither output is admitted as a training target by this program.
"""
...
SCHEMA_VERSION = "bernini-prospective-forward-anchor-shard-v1"
RELEASE_BRANCHES = ("noop", "forward")
AUTHORITY = {
    "fit_generation_only": True,
...
class ForwardAnchorShardError(RuntimeError):
    """Raised before a partial or authority-expanding shard is accepted."""
...
def _released_rows(
    ...
    if (
        ...
        or any(row["analysis_split"] != "fit" for row in rows)
        or len(rows) != 2 * len(cells)
    ):
        raise ForwardAnchorShardError("forward/noop fit-cell closure differs")
    branch_order = {branch: index for index, branch in enumerate(RELEASE_BRANCHES)}
    return sorted(
        rows,
        key=lambda row: (
            row["source_id"], row["seed"], branch_order[row["branch"]]
        ),
    )
...
def _receipt(
    *, manifest_sha256: str, manifest_digest: str,
    ...
    unsigned = {
        ...
        "status": "forward_noop_shard_complete_decoded_review_pending",
...
def command_run(args: argparse.Namespace) -> int:
    ...
    rows = _released_rows(manifest, cells)
    ...
    if any(entries_root.iterdir()):
        raise ForwardAnchorShardError("entries root must be fresh")
    ...
    base._write_create_only(output_root / "forward-anchor-shard.receipt.json", result)
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    output_root = base._plain_directory(args.output_root, label="output root")
    receipts = [
        base._verify_entry(row, output_root, args.expected_manifest_sha256)
        for row in rows
    ]
    expected = _receipt(
        manifest_sha256=args.expected_manifest_sha256,
    ...
    observed = base._read_object(
        base._plain_file(
            output_root / "forward-anchor-shard.receipt.json",
            label="forward anchor shard receipt",
        ),
        label="forward anchor shard receipt",
    )
    if observed != expected:
        raise ForwardAnchorShardError("forward anchor shard receipt differs")
...
if __name__ == "__main__":
    ...
    except (ForwardAnchorShardError, base.FactorialBranchRunError) as error:
        print(f"[forward-anchor-shard] ERROR: {error}", file=sys.stderr)
        raise SystemExit(2)
