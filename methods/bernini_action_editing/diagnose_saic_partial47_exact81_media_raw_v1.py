#!/usr/bin/env python3
"""CPU-only exact81 camera/technical/temporal diagnostics for sealed SAIC r4.

The only accepted bank index is the already sealed exact47 source-bound input
...
PINNED_LOCAL_SOURCES = {
    ...
    "diagnose_saic_partial47_source_bound_dinov2_raw_v1.py":
...
import diagnose_saic_partial47_source_bound_dinov2_raw_v1 as frozen_input  # noqa: E402
import saic_exact81_media_diagnostics_v1 as diagnostics  # noqa: E402


SCHEMA_VERSION = "bernini-saic-partial47-exact81-media-raw-v1"
...
EXPECTED_CANDIDATE_BOUND_SOURCE_COUNT = 7
...
def load_and_validate_input_manifest(
    ...
    if expected != EXPECTED_INPUT_MANIFEST_SHA256:
        raise Exact47MediaRawError("only the sealed r4 exact47 input manifest is accepted")
    try:
    ...
    if raw_sha != EXPECTED_INPUT_MANIFEST_SHA256:
        raise Exact47MediaRawError("sealed r4 exact47 input manifest SHA-256 differs")
    if (
        ...
        raise Exact47MediaRawError("sealed r4 exact47 bank contract differs")
    ...
    if type(rows) is not list or len(rows) != EXPECTED_CANDIDATE_COUNT:
        raise Exact47MediaRawError("sealed bank must contain exactly 47 attempts")
    ...
    if len(receipt_paths) != EXPECTED_CANDIDATE_COUNT or receipt_paths != sealed_receipt_paths:
        raise Exact47MediaRawError("attempt root is not the sealed exact47 receipt set")
    ...
    if (
        ...
        raise Exact47MediaRawError("exact47 candidate/MP4/source coverage differs")
...
def run_full(args: Any) -> int:
    ...
    if (
        ...
        raise Exact47MediaRawError("full aggregate exact47/runtime coverage differs")
