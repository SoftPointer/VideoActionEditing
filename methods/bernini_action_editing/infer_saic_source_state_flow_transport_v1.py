#!/usr/bin/env python3
"""Frozen Bernini exact-81 SAIC source-state flow-transport runner.
...
The registered pilot arms are:

``T0``
    Text-only T2V APG fields, one candidate, ANC off.

``T1``
    Text-only T2V APG fields, five early candidates, source-similarity
    aggregation, and ANC on.

``I0``
    R2V APG fields conditioned on one independently VAE-encoded source RGB
    frame 0, one candidate, ANC off.

``IAVG``
    The same source-I0 field with five candidates for the first three cells,
    uniform aggregation, and ANC on.

``I1``
    The IAVG bank with source-similarity softmax (temperature 0.01).

``I1A``
    I1 plus an exact latent phase-zero source anchor after every Euler cell.

All arms start the *outer clean edit state* from one source-only VAE latent
materialized once and sealed before any arm runs.  This runner never
re-encodes the complete source video; T0 and I0 consume byte-identical source
coordinates.  T0 gives the frozen generator no visual condition.  I0/IAVG/I1/I1A
give both the source-caption and target-caption field queries the exact same
single frame-0 reference.  No full source-video tokens enter either registered
native field.  The only external semantic inputs are an exact 81-frame source
video, a full source-content caption, and a full target-content caption.  No
target video, mask, pose, flow, track, trajectory, motion donor, or oracle
edited frame is accepted.
...
SCHEMA_VERSION = "bernini-saic-source-state-flow-transport-inference-v1"
METHOD = "frozen-bernini-source-state-flow-transport"
...
def resolve_sealed_forward_cell(
    ...
    *,
    ...
    if branch != "forward":
        raise SAICInferenceError("SSFT v1 pilot accepts only branch=forward")
...
def validate_cli(args: argparse.Namespace) -> ArmSpec:
    ...
    if args.branch != "forward":
        raise SAICInferenceError("SSFT v1 is fixed to the forward branch")
    ...
    if source_clean.parent != output.parent:
        raise SAICInferenceError(
            "sealed source-clean latent must be a sibling of the arm output"
        )
...
def build_reference_encoder_receipt(
    *,
    ...
    payload = {
        ...
        "operation": "source_rgb_frame0_slice_then_independent_wan_vae_encode",
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "sealed_inputs": {
            **dict(sealed_assets),
            ...
            "accepted_external_derived_inputs": [
                "sealed_source_clean_coordinate_derived_only_from_source_video"
            ],
        ...
        "transport": {
            ...
            "reference_is_source_rgb_frame0": spec.uses_reference_frame0,
            "reference_is_available_at_inference": spec.uses_reference_frame0,
            "reference_independently_vae_encoded": spec.uses_reference_frame0,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    source_pixels = (
        source_tensor.to(device=device, dtype=torch.float32)
        if spec.uses_reference_frame0
        else None
    )
    with torch.inference_mode():
        reference_frame0 = (
            _vae_encode(
                vae,
                # I0 remains an independently encoded source RGB frame.  The
                # complete 81-frame source-clean coordinate is never encoded
                # inline by this runner.
                source_pixels[:, :, 0:1, :, :].contiguous(),
            )
            .float()
            .contiguous()
            if spec.uses_reference_frame0
    ...
    if spec.uses_reference_frame0 and (
        reference_frame0 is None
        or tuple(reference_frame0.shape) != expected_reference_shape
        or reference_frame0.dtype != torch.float32
    ):
        raise SAICInferenceError("independent source RGB frame-0 latent differs")
    ...
    reference_broadcast = (
        native_canary._broadcast_condition_from_rank_zero(
            reference_frame0,
            label="ssft_source_rgb_frame0_reference",
            world_size=distributed.world_size,
        )
        if reference_frame0 is not None
        else None
    )
    ...
    reference_identity = (
        native_canary._all_rank_tensor_identity(
            reference_frame0,
            label="ssft_source_rgb_frame0_reference",
            world_size=distributed.world_size,
        )
    ...
    model.to("cpu")
    del adapter, noise_bank, rollout, source_clean, reference_frame0
