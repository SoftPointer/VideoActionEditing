#!/usr/bin/env python3
"""Render the future-use v4-D decoded-residual EPMC video canary.
...
creating output until the completed v4-D authority and gate materializer are
...
RECEIPT_SCHEMA = "bernini-v4d-epmc-temporal-gate-video-canary-v1"
RECEIPT_STATUS = "V4D_EPMC_TEMPORAL_GATE_VIDEO_CANARY_COMPLETE_DIAGNOSTIC_ONLY"
PIN_PLACEHOLDER = "TO_BE_PINNED"
RELEASE_SEALED = False

# These are intentionally unresolved.  They may be filled only after the v4-D
...
class V4DEPMCVideoCanaryError(RuntimeError):
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise V4DEPMCVideoCanaryError("value is not canonical JSON") from error
...
def _required_sha256(value: Any, *, label: str) -> str:
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise V4DEPMCVideoCanaryError(f"{label} must be a lowercase SHA-256")
    return value
...
def _require_release_sealed() -> Any:
    ...
    if (
        ...
        raise V4DEPMCVideoCanaryError(
            "UNSEALED v4-D EPMC video canary: gate authorities are TO_BE_PINNED"
        )

    # Deliberately deferred until the local release surface is fully pinned.
    from methods.bernini_action_editing import (
        materialize_v4d_epmc_gate_state_v1 as gate_materializer,
    )
    ...
    if (
        ...
        raise V4DEPMCVideoCanaryError(
            "sealed v4-D gate materializer source/schema binding differs"
        )
...
def _runtime_modules() -> tuple[Any, Any, Any, Any, Any]:
    ...
    if (
        ...
        raise V4DEPMCVideoCanaryError("inherited v4-B canary core source differs")
...
def _plain_gate_state(path_value: str | Path) -> Path:
    ...
    if not path.is_absolute():
        raise V4DEPMCVideoCanaryError("gate-state must be an absolute path")
    try:
        info = path.lstat()
    except OSError as error:
        raise V4DEPMCVideoCanaryError("cannot stat gate-state") from error
    if (
...
def _reject_duplicate_keys(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V4DEPMCVideoCanaryError(f"duplicate JSON key: {key!r}")
        result[key] = value
    return result


def _reject_nonfinite(value: str) -> None:
    raise V4DEPMCVideoCanaryError(f"non-finite JSON number: {value}")
...
def _positive_zero(value: Any, *, label: str) -> None:
    ...
    if int(torch.count_nonzero(flat).item()) != 0:
        raise V4DEPMCVideoCanaryError(f"{label} must be exact zero")
    if int(torch.count_nonzero(flat.view(torch.uint8)).item()) != 0:
        raise V4DEPMCVideoCanaryError(f"{label} must be byte-exact positive zero")
...
def _code_from_json(value: Mapping[str, Any], *, name: str) -> Any:
    ...
    if not isinstance(value, Mapping):
        raise V4DEPMCVideoCanaryError(f"{name} gate payload must be an object")
    try:
        ...
        raise V4DEPMCVideoCanaryError(f"{name} gate tensors differ") from error
    phase = phase.contiguous()
    block = block.contiguous()
    try:
        code = epmc.MotionCode(phase, block)
    except epmc.PrivilegedMotionCodeContractError as error:
        raise V4DEPMCVideoCanaryError(str(error)) from error
    if (
        ...
        raise V4DEPMCVideoCanaryError(f"{name} gate tensor digest differs")
...
def load_gate_state(
    ...
    try:
        ...
        with os.fdopen(descriptor, "rb") as handle:
            ...
            if (
                ...
                raise V4DEPMCVideoCanaryError("gate-state inode seal differs")
    ...
    except V4DEPMCVideoCanaryError:
        raise
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V4DEPMCVideoCanaryError("gate-state is not strict ASCII JSON") from error
    finally:
        if descriptor is not None:
            os.close(descriptor)
    if type(payload) is not dict:
        raise V4DEPMCVideoCanaryError("gate-state must contain one object")
    ...
    v4d_receipt = payload.get("v4d_receipt")
    ...
    if (
        ...
        or payload.get("v4d_aggregate_gate_verified_true") is not True
        or not isinstance(implementation, Mapping)
        or implementation.get("gate_materializer_sha256")
        != EXPECTED_GATE_MATERIALIZER_SHA256
        or implementation.get("implementation_sha256")
        != gate_materializer.EXPECTED_V4D_IMPLEMENTATION_SHA256
        or not isinstance(v4d_receipt, Mapping)
        or v4d_receipt.get("file_sha256")
        != gate_materializer.EXPECTED_V4D_RECEIPT_FILE_SHA256
        or v4d_receipt.get("receipt_digest")
        != gate_materializer.EXPECTED_V4D_RECEIPT_SELF_DIGEST
        or v4d_receipt.get("decoded_temporal_codec_development_gate") is not True
        ...
        or decoded_contract.get("sole_code_shape") != [4, 96]
        ...
        raise V4DEPMCVideoCanaryError(
            "gate-state does not carry the closed v4-D privileged diagnostic scope"
        )
    ...
    try:
        ...
        raise V4DEPMCVideoCanaryError("temporal calibration tensors differ") from error
    ...
    for name, indices in (
        ...
        if not torch.equal(codes[name].phase_gates, expected):
            raise V4DEPMCVideoCanaryError(f"{name} is not the frozen phase permutation")
        actual = torch.sort(codes[name].phase_gates[:, 1:], dim=1).values
        if not torch.equal(reference, actual):
            raise V4DEPMCVideoCanaryError(f"{name} changed the phase multiset")
    ...
    if int(torch.count_nonzero(codes["correct"].phase_gates[:, 1:]).item()) == 0:
        raise V4DEPMCVideoCanaryError("correct temporal gate degenerated to all zero")
...
def build_video_receipt(
    *,
    ...
    if (
        ...
        raise V4DEPMCVideoCanaryError("video canary runtime closure differs")
    payload: dict[str, Any] = {
        "schema_version": RECEIPT_SCHEMA,
        "status": RECEIPT_STATUS,
        "method": "v4d-decoded-residual-epmc-temporal-head-gating",
        ...
        "verified_claims": {
            "v4d_aggregate_gate_true_before_render": True,
    ...
    if not all(payload["verified_claims"].values()):
        raise V4DEPMCVideoCanaryError("one or more video canary invariants failed")
...
def _runner_argv(args: argparse.Namespace) -> list[str]:
    ...
    return [value.replace("v4b_epmc", "v4d_epmc") for value in values]
