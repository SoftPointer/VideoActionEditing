#!/usr/bin/env python3
"""Build/audit the deterministic no-diagnostic 136141 exact8 release."""
...
SCHEMA_VERSION = "bernini-full30-action-confirmation8-136141-release-v1"
...
RESOURCE_SPECIALIZED = (
    "tools/reserve4_fixed_generation_sp4_136141_confirmation8_specialized_v1.py"
)
...
R10_R13_PARITY_EVIDENCE: Mapping[str, Any] = {
    ...
    "fit_r13_release": {
        "archive_sha256": (
            "4e012228b6d7ee07c25604412ad697f620c4c2535544a69aa4bbaf2bd7c4ac10"
        ),
        "manifest_file_sha256": (
            "480fe08198cf80b8441dad8e9048b30d25a922b0112a17f0bd4ee3016c4c9a4a"
        ),
        "manifest_digest": (
            "494569e481d0007f919f7f750cd6db8ce35e207cfdf43fb139a33768a0e8f9a2"
        ),
        "content_closure_sha1": "22866d63163558efb56bd44ff1b6b153e50cf219",
    },
}
...
_NEW_FILES = (
    "full30_action_confirmation8_136141_plan_v1.py",
    "full30_action_confirmation8_136141_generator_v1.py",
    "full30_action_confirmation8_136141_controller_v1.py",
)
_TOOLS = (
    "tools/build_full30_action_confirmation8_136141_release_v1.py",
...
FILES_AND_MODES: Mapping[str, int] = {
    **{path: 0o444 for path in _RUNTIME_FILES + _NEW_FILES + _TOOLS},
    "scripts/auh_full30_action_confirmation8_136141_world4_v1.sh": 0o555,
    "scripts/auh_generic_action_data_prep_rank_exec_v1.sh": 0o555,
}
COMPONENT_FILES: Mapping[str, str] = {
    "plan_sha256": "full30_action_confirmation8_136141_plan_v1.py",
    "generator_sha256": "full30_action_confirmation8_136141_generator_v1.py",
    "controller_sha256": "full30_action_confirmation8_136141_controller_v1.py",
    ...
    "resource_136141_specialization_sha256": RESOURCE_SPECIALIZED,
}
ENTRYPOINTS = (
    "full30_action_confirmation8_136141_controller_v1.py",
    "scripts/auh_full30_action_confirmation8_136141_world4_v1.sh",
...
class Confirmation8ReleaseError(RuntimeError):
    pass


def fail(message: str) -> NoReturn:
    raise Confirmation8ReleaseError(message)
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise Confirmation8ReleaseError(
...
def _stable_plain_bytes(path: Path) -> bytes:
    ...
    try:
        ...
        raise Confirmation8ReleaseError("release input is unavailable") from error
...
def specialize_resource_bytes(raw: bytes) -> bytes:
    if (
        ...
        or raw.count(b"136309") != 0
        or raw.count(b"136140") != 0
    ):
        fail("fit-r13 resource specialization preimage differs")
    # The fit-r13 resource is already the exact 136141 holder implementation.
    # A dedicated release member is materialized byte-identically so the
    # confirmation release has its own path without mutating the audited code.
    specialized = bytes(raw)
    if (
        len(specialized) != len(raw)
        or hashlib.sha256(specialized).hexdigest() != RESOURCE_SPECIALIZED_SHA256
        or specialized.count(b"136141") != 7
        or specialized.count(b"136309") != 0
        or specialized != raw
    ):
        fail("136141 resource specialization postimage differs")
    return specialized
...
def build_manifest(
    ...
    unsigned = {
        ...
        "release_scope": (
            "BOX-EXP-009-confirmation-action-anchor-exact8-136141-no-diagnostic"
        ),
        "resource_specialization": {
            ...
            "confirmation_holder_job": 136141,
            "replacement_count": 0,
            "identity_specialization_required": True,
            "preimage_equals_postimage_required": True,
        ...
        "authority": {
            ...
            "purpose": (
                "materialize the fixed confirmation action anchors required by "
                "the pre-optimizer full30 action-learning authority"
            ),
            "scientific_target": (
                "test Frozen action versus matched incomplete endpoints on four "
                "held-out confirmation seed cells under identical Gaussian pairs"
            ),
            "learning_target": (
                "data authority for later all-30-block action learning; this run "
                "learns no parameter and restores no source trajectory"
            ),
            "numeric_target": (
                "8/8 exact81 at exact40; 4/4 identical-Gaussian pairs; 8/8 "
                "independent full81 passes; 4/4 same-state gates; 0 diagnostics; "
                "0 optimizer updates"
            ),
            ...
            "core_validation": (
                "independent full81 pair review plus official same-state PsiOut "
                "threshold and fail-closed resource gates"
            ),
            ...
            "confirmation_seed_cells": [
                ["0c6915018a5f4d9b", 2026080822, "sp4-a", "head-turn-smile"],
                ["33322eb8ec1e4703", 2026080823, "sp4-b", "peace-wave"],
                ["0c6915018a5f4d9b", 2026080922, "sp4-a", "head-turn-smile"],
                ["33322eb8ec1e4703", 2026080923, "sp4-b", "peace-wave"],
            ],
            "action_and_incomplete_each_require_independent_full81_pass": True,
            "same_state_controls_source": (
...
def verify_archive(raw: bytes, manifest: Mapping[str, Any]) -> None:
    try:
        ...
        raise Confirmation8ReleaseError("archive is invalid") from error
...
def validate_manifest(value: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        or specialization.get("replacement_count") != 0
        or specialization.get("identity_specialization_required") is not True
        or specialization.get("preimage_equals_postimage_required") is not True
        or specialization.get("source_sha256")
        != specialization.get("specialized_sha256")
        or authority.get("formal_candidate_count") != 8
        or authority.get("formal_branch_order") != ["action", "incomplete"]
        or authority.get("diagnostic_task_count") != 0
        or authority.get("diagnostic_generation_allowed") is not False
        or authority.get("inference_steps_per_clip") != 40
        or authority.get("optimizer_steps") != 0
        or authority.get("optimizer_authorized") is not False
        or topology.get("holder")
        != {"job_id": 136141, "node": "auh7-1b-gpu-299"}
        or topology.get("diagnostic_model_invocation_count") != 0
        or topology.get("diagnostic_generation_allowed") is not False
        or topology.get("host_memory_request_gib") != 60
        or topology.get("host_sampled_current_safe_ceiling_gib") != 56
        or topology.get("host_cgroup_sample_interval_ns") != 10_000_000
        or topology.get("physical_safetensors_safe_open_required") is not True
        or topology.get("r10_r13_mp4_gaussian_clean_latent_parity_required")
        is not True
    ):
...
def audit(
    ...
    try:
        ...
        raise Confirmation8ReleaseError("release manifest is invalid JSON") from error
