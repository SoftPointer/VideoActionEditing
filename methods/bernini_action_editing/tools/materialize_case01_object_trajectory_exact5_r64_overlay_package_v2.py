#!/usr/bin/env python3
"""Seal the r5f-v3 source overlay or materialize its fresh exact-five package.
...
snapshot publication receipt are replayed.  A separate receipt-gated exact5
overlay contributes only the new composite adapter, evaluator, runner,
launcher, and this materializer.  No byte is read from the burned canary-v1
package.  The output is a new receipt-gated release25/exact26 HOLD package.
"""
...
TARGET_ROOT = (
    EXPERIMENTS / "bernini_case01_object_trajectory_exact5_r64_canary_v2"
)
...
RANK_CACHE_ROOT = Path(
    "/tmp/bernini-case01-object-trajectory-exact5-r64-job143808-node292-r2-rank-cache"
)
OVERLAY_ROOT = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r5f_v3_source_overlay_5_20260822_r1"
)
...
PACKAGE_PUBLICATION_SCHEMA = (
    "case01-object-trajectory-exact5-r64-package-publication-v3"
)
OVERLAY_SCHEMA = "case01-object-trajectory-exact5-r5f-v3-source-overlay-v1"
OVERLAY_RECEIPT_SCHEMA = OVERLAY_SCHEMA + "-receipt"
OVERLAY_PUBLICATION_PROTOCOL = (
    "fresh_precreated_exact_tree_then_held_files_sealed_and_sibling_O_EXCL_receipt"
)
OVERLAY_UNSEALED_FILE_MODE = 0o644
OVERLAY_UNSEALED_DIRECTORY_MODE = 0o755
PUBLICATION_RESERVATION_MODE = 0o600
PUBLICATION_RECEIPT_MODE = 0o400

FINAL_RUNNER_SHA256 = "02207e64a129444b26adf8bd92307102c4a91e85d2a029fa60030a7e9e6f45c8"
FINAL_EVAL_SHA256 = "cfdfc5fec04243265b6c122649fed9144d89510d17184a77782c0ec0ddc5ed8a"
FINAL_WRAPPER_SHA256 = "b30bba5c9cd233d412ffd88d8413311e9ffbb79d3ddf69aaf6eb2ee96183b489"
FINAL_LAUNCHER_SHA256 = "0073a3b549bdbecc49866471c43f6882e4e494876312a7082560b0cc29e0f913"
BASE_RUNNER_V1_SHA256 = (
...
if len(BASE_RELEASE_FILES) != 25:
    raise RuntimeError("base release25 closure differs")

REPLACED_BASE_RELEASE_FILES = frozenset({
    "methods/bernini_action_editing/full644_exploratory_matched_infer_adapter_auh_r5f.py",
    "methods/bernini_action_editing/case01_object_trajectory_exact5_runner_v1.py",
    "methods/bernini_action_editing/case01_object_trajectory_exact5_eval_v1.py",
    "methods/bernini_action_editing/case01_object_trajectory_exact5_spooled_launcher_auh_v1.py",
...
OVERLAY_RELEASE_FILES = {
    "methods/bernini_action_editing/infer_case01_object_trajectory_oracle_auh_r5f_v3.py":
        FINAL_WRAPPER_SHA256,
    "methods/bernini_action_editing/case01_object_trajectory_exact5_runner_v3.py":
        FINAL_RUNNER_SHA256,
    "methods/bernini_action_editing/case01_object_trajectory_exact5_eval_v3.py":
        FINAL_EVAL_SHA256,
    "methods/bernini_action_editing/case01_object_trajectory_exact5_spooled_launcher_auh_v3.py":
        FINAL_LAUNCHER_SHA256,
}
RELEASE_FILES = {
    relative: pin for relative, pin in BASE_RELEASE_FILES.items()
    if relative not in REPLACED_BASE_RELEASE_FILES
}
RELEASE_FILES.update(OVERLAY_RELEASE_FILES)
if (
    len(REPLACED_BASE_RELEASE_FILES) != 4
    or not REPLACED_BASE_RELEASE_FILES <= set(BASE_RELEASE_FILES)
    or len(OVERLAY_RELEASE_FILES) != 4
    or len(RELEASE_FILES) != 25
):
    raise RuntimeError("fresh release25 overlay decomposition differs")
...
LEGACY_REUSED_PATHS = frozenset(tuple(BASE_RELEASE_FILES)[:18])
FORMAL_REVIEW_TEST = {
...
if (
    ...
    or len(set(BASE_RELEASE_FILES) | set(DIAGNOSTIC_FILES)
           | set(SNAPSHOT_AUTHORITY_FILES)
           | {SNAPSHOT_MATERIALIZER_RELATIVE}) != 33
):
...
def require_overlay_source_pins() -> None:
...
def require_final_pins() -> None:
    require_overlay_source_pins()


def validate_overlay_publication_pins(
    receipt_sha256: str, receipt_size: int, receipt_digest: str,
    root_identity: Sequence[int],
) -> list[int]:
    blocked: list[str] = []
    if SHA_RE.fullmatch(receipt_sha256) is None:
        blocked.append("overlay_receipt_sha256")
    if type(receipt_size) is not int or receipt_size <= 0:
        blocked.append("overlay_receipt_size")
    if SHA_RE.fullmatch(receipt_digest) is None:
        blocked.append("overlay_receipt_digest")
    if (
        not isinstance(root_identity, (list, tuple))
        or len(root_identity) != 11
        or any(type(value) is not int for value in root_identity)
    ):
        blocked.append("overlay_root_identity")
    if blocked:
        raise HoldPackageError(
            "HOLD: overlay publication pins blocked: " + ",".join(blocked)
        )
    return list(root_identity)
...
def _normalize_overlay_specs(
    specs: Mapping[str, Mapping[str, Any] | Sequence[Any]],
) -> dict[str, tuple[str, int]]:
    expected_relatives = set(OVERLAY_RELEASE_FILES) | {
        OVERLAY_MATERIALIZER_RELATIVE
    }
    if set(specs) != expected_relatives or len(specs) != 5:
        raise HoldPackageError("overlay exact5 source-name closure differs")
    result: dict[str, tuple[str, int]] = {}
    for relative in sorted(specs):
        value = specs[relative]
        if isinstance(value, Mapping):
            if set(value) != {"sha256", "size"}:
                raise HoldPackageError(f"overlay source spec differs: {relative}")
            digest = value.get("sha256"); size = value.get("size")
        elif (
        ...
        path = Path(relative)
        if (
            path.is_absolute() or path.parts == () or ".." in path.parts
            or "." in path.parts or str(path) != relative
            or SHA_RE.fullmatch(digest if isinstance(digest, str) else "") is None
            or type(size) is not int or size <= 0
        ):
            raise HoldPackageError(f"overlay source spec differs: {relative}")
        expected_pin = OVERLAY_RELEASE_FILES.get(relative)
        if expected_pin is not None and digest != expected_pin:
            raise HoldPackageError(f"overlay source pin differs: {relative}")
        result[relative] = (digest, size)
    return result


def _walk_exact_tree_unsealed(
    root: Path, expected_files: set[str], *, owner_uid: int, owner_gid: int,
) -> tuple[dict[str, os.stat_result], dict[str, os.stat_result]]:
    actual_files: dict[str, os.stat_result] = {}
    actual_directories: dict[str, os.stat_result] = {}
    pending = [(root, ".")]
    while pending:
        ...
        if (
            ...
            or info.st_gid != owner_gid or directory.resolve(strict=True) != directory
            or stat.S_IMODE(info.st_mode) != OVERLAY_UNSEALED_DIRECTORY_MODE
        ):
            raise HoldPackageError(f"unsealed overlay directory differs: {relative}")
        actual_directories[relative] = info
        with os.scandir(directory) as entries:
            for entry in entries:
                child_relative = (
                ...
                if stat.S_ISDIR(child.st_mode):
                    ...
                    and child.st_uid == owner_uid and child.st_gid == owner_gid
                    and stat.S_IMODE(child.st_mode) == OVERLAY_UNSEALED_FILE_MODE
                ):
                    actual_files[child_relative] = child
                else:
                    raise HoldPackageError(
                        f"unsealed overlay special/shared entry refused: {child_relative}"
                    )
    if (
        set(actual_files) != expected_files
        or set(actual_directories) != expected_directories(expected_files)
    ):
        raise HoldPackageError("unsealed overlay physical exact-tree differs")
...
def _write_all(descriptor: int, raw: bytes) -> None:
    ...
    while offset < len(raw):
        ...
        if written <= 0:
            raise HoldPackageError("overlay receipt write was incomplete")
        offset += written


def seal_overlay_source_root(
    root: Path, receipt: Path,
    specs: Mapping[str, Mapping[str, Any] | Sequence[Any]],
) -> dict[str, Any]:
    """Seal one fresh precreated exact5 overlay and its sibling 0400 receipt."""
    require_overlay_source_pins()
    if root != OVERLAY_ROOT or receipt != OVERLAY_RECEIPT_PATH:
        raise HoldPackageError("overlay seal path is not the configured fresh root")
    if receipt.parent != root.parent or os.path.lexists(receipt):
        raise HoldPackageError("overlay sibling receipt is not fresh")
    normalized = _normalize_overlay_specs(specs)
    parent = root.parent
    require_real_directory_chain(parent, label="overlay parent")
    root_named = os.lstat(root)
    if (
        not stat.S_ISDIR(root_named.st_mode) or root.resolve(strict=True) != root
        or root_named.st_uid != os.geteuid() or root_named.st_gid != os.getegid()
        or stat.S_IMODE(root_named.st_mode) != OVERLAY_UNSEALED_DIRECTORY_MODE
    ):
        raise HoldPackageError("overlay precreated root authority differs")
    _walk_exact_tree_unsealed(
        root, set(normalized), owner_uid=root_named.st_uid,
        owner_gid=root_named.st_gid,
    )
    held_files: dict[str, tuple[int, bytes, os.stat_result]] = {}
    held_directories: dict[str, int] = {}
    ...
    try:
        ...
        for relative in sorted(expected_directories(set(normalized))):
            directory = root if relative == "." else root / relative
            held_directories[relative] = os.open(
                directory, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0)
                | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0),
            )
        for relative, (digest, size) in normalized.items():
            path = root / relative
            descriptor = os.open(
                path, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0)
                | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_NONBLOCK", 0),
            ...
            if (
                ...
                or _identity(before) != _identity(after)
                or before.st_uid != root_named.st_uid
                or before.st_gid != root_named.st_gid
                or before.st_size != size or raw != second
                or hashlib.sha256(raw).hexdigest() != digest
            ):
                os.close(descriptor)
                raise HoldPackageError(f"overlay held source differs: {relative}")
            held_files[relative] = (descriptor, raw, before)
        executing = Path(__file__).resolve(strict=True)
        executing_raw = stable(executing, None)
        materializer_fd = held_files[OVERLAY_MATERIALIZER_RELATIVE][0]
        if (
            executing_raw != held_files[OVERLAY_MATERIALIZER_RELATIVE][1]
            or _identity(os.lstat(executing))
            != _identity(os.fstat(materializer_fd))
        ):
            raise HoldPackageError("executing overlay materializer bytes differ")
        # File descriptors remain held while the exact tree is sealed.
        for descriptor, _raw, _before in held_files.values():
            os.fchmod(descriptor, 0o444); os.fsync(descriptor)
        for relative in sorted(
            held_directories, key=lambda value: value.count("/"), reverse=True,
        ):
            os.fchmod(held_directories[relative], 0o555)
        for descriptor in held_directories.values():
            os.fsync(descriptor)
        file_rows: list[dict[str, Any]] = []
        for relative in sorted(held_files):
            descriptor, raw, _before = held_files[relative]
            after = os.fstat(descriptor); named = os.lstat(root / relative)
            if (
                _identity(after) != _identity(named)
                or stat.S_IMODE(after.st_mode) != 0o444
                or after.st_nlink != 1
                or raw != _read_fd(descriptor, after.st_size)
            ):
                raise HoldPackageError(f"sealed overlay source differs: {relative}")
            file_rows.append({
                "path": relative,
                "sha256": hashlib.sha256(raw).hexdigest(),
                "size": len(raw), "mode": 0o444,
                "identity": list(_identity(after)),
                "provenance": "fresh_scp_then_held_exact5_seal",
            })
        directory_rows: list[dict[str, Any]] = []
        for relative in sorted(held_directories):
            info = os.fstat(held_directories[relative])
            named_path = root if relative == "." else root / relative
            if (
                _identity(info) != _identity(os.lstat(named_path))
                or stat.S_IMODE(info.st_mode) != 0o555
            ):
                raise HoldPackageError(f"sealed overlay directory differs: {relative}")
            directory_rows.append({"path": relative, "identity": list(_identity(info))})
        root_identity = list(_identity(os.fstat(held_directories["."])))
        if _identity(os.fstat(parent_fd)) != _identity(parent_before):
            raise HoldPackageError("held overlay parent changed before receipt")
        receipt_fd = os.open(
            receipt.name, os.O_RDWR | os.O_CREAT | os.O_EXCL
            | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0),
            0o600, dir_fd=parent_fd,
        )
        receipt_created = os.fstat(receipt_fd)
        if (
            not stat.S_ISREG(receipt_created.st_mode)
            or receipt_created.st_nlink != 1
            or stat.S_IMODE(receipt_created.st_mode) != 0o600
        ):
            raise HoldPackageError("overlay receipt reservation differs")
        value: dict[str, Any] = {
            "schema_version": OVERLAY_RECEIPT_SCHEMA,
            "status": "PUBLISHED_RECEIPT_GATED",
            "launch_allowed": False,
            "target_root": str(root), "receipt_path": str(receipt),
            "source_file_count": 5, "file_mode": 0o444,
            "directory_mode": 0o555, "receipt_mode": 0o400,
            "files": file_rows, "directories": directory_rows,
            "root_identity": root_identity,
            "materializer_relative": OVERLAY_MATERIALIZER_RELATIVE,
            "publication_protocol": OVERLAY_PUBLICATION_PROTOCOL,
            "precreated_by_scp": True, "exact_tree": True,
            "held_source_descriptors_through_receipt_reservation": True,
            "receipt_is_consumption_gate": True,
            "retry_allowed": False,
            "receipt_inode_anchor": _inode_anchor(receipt_created),
        }
        value["receipt_digest"] = object_sha(value)
        raw = canonical(value) + b"\n"
        _write_all(receipt_fd, raw)
        os.fsync(receipt_fd); os.fchmod(receipt_fd, 0o400); os.fsync(receipt_fd)
        os.fsync(parent_fd)
        terminal = os.fstat(receipt_fd)
        named_receipt = os.lstat(receipt)
        if (
            _identity(terminal) != _identity(named_receipt)
            or stat.S_IMODE(terminal.st_mode) != 0o400
            or _read_fd(receipt_fd, terminal.st_size) != raw
        ):
            raise HoldPackageError("sealed overlay receipt terminal differs")
        for row in file_rows:
            descriptor = held_files[row["path"]][0]
            opened = os.fstat(descriptor); named = os.lstat(root / row["path"])
            if (
                _identity(opened) != tuple(row["identity"])
                or _identity(named) != tuple(row["identity"])
                or _read_fd(descriptor, opened.st_size)
                != held_files[row["path"]][1]
            ):
                raise HoldPackageError(
                    f"overlay source changed across receipt seal: {row['path']}"
                )
        for row in directory_rows:
            descriptor = held_directories[row["path"]]
            named_path = root if row["path"] == "." else root / row["path"]
            if (
                _identity(os.fstat(descriptor)) != tuple(row["identity"])
                or _identity(os.lstat(named_path)) != tuple(row["identity"])
            ):
                raise HoldPackageError(
                    f"overlay directory changed across receipt seal: {row['path']}"
                )
        return value
    finally:
...
def preflight_overlay(
    root: Path, receipt_path: Path, *, materializer_sha256: str,
    receipt_sha256: str, receipt_size: int, receipt_digest: str,
    root_identity: Sequence[int],
) -> tuple[dict[str, bytes], dict[str, Any]]:
    """Consume the immutable sibling receipt before naming the overlay root."""
    require_final_pins()
    if root != OVERLAY_ROOT or receipt_path != OVERLAY_RECEIPT_PATH:
        raise HoldPackageError("overlay preflight path differs")
    if SHA_RE.fullmatch(materializer_sha256) is None:
        raise HoldPackageError("overlay materializer SHA pin is incomplete")
    expected_root_identity = validate_overlay_publication_pins(
        receipt_sha256, receipt_size, receipt_digest, root_identity,
    )
    receipt_raw = stable(
        receipt_path, receipt_sha256, receipt_size,
        expected_mode=0o400,
    )
    value = strict_json(receipt_raw, label="overlay publication receipt")
    ...
    expected_specs = {
        **{relative: (pin, None) for relative, pin in OVERLAY_RELEASE_FILES.items()},
        OVERLAY_MATERIALIZER_RELATIVE: (materializer_sha256, None),
    }
    rows = value.get("files"); directories = value.get("directories")
    root_info = os.lstat(root)
    receipt_info = os.lstat(receipt_path)
    if (
        set(value) != fields or value.get("schema_version") != OVERLAY_RECEIPT_SCHEMA
        or value.get("status") != "PUBLISHED_RECEIPT_GATED"
        or value.get("launch_allowed") is not False
        or value.get("target_root") != str(root)
        or value.get("receipt_path") != str(receipt_path)
        or value.get("source_file_count") != 5
        or value.get("file_mode") != 0o444
        or value.get("directory_mode") != 0o555
        or value.get("receipt_mode") != 0o400
        or value.get("materializer_relative") != OVERLAY_MATERIALIZER_RELATIVE
        ...
        or claimed != object_sha(unsigned) or claimed != receipt_digest
        or value.get("root_identity") != expected_root_identity
        or value.get("root_identity") != list(_identity(root_info))
        or value.get("receipt_inode_anchor") != _inode_anchor(receipt_info)
        or type(rows) is not list or len(rows) != 5
        or type(directories) is not list
    ):
        raise HoldPackageError("overlay publication receipt closure differs")
    validate_exact_tree(
        root, expected_files=set(expected_specs), file_mode=0o444,
        directory_mode=0o555, expected_uid=root_info.st_uid,
        expected_gid=root_info.st_gid,
    )
    expected_directory_names = sorted(expected_directories(set(expected_specs)))
    if [row.get("path") if type(row) is dict else None for row in directories] != expected_directory_names:
        raise HoldPackageError("overlay directory receipt ordering differs")
    for row in directories:
        relative = row["path"]
        path = root if relative == "." else root / relative
        if (
            set(row) != {"path", "identity"}
            or row.get("identity") != list(_identity(os.lstat(path)))
        ):
            raise HoldPackageError(f"overlay directory receipt differs: {relative}")
    if [row.get("path") if type(row) is dict else None for row in rows] != sorted(expected_specs):
        raise HoldPackageError("overlay source receipt ordering differs")
    raw_by_relative: dict[str, bytes] = {}
    for row in rows:
        relative = row["path"]; expected_sha, _ = expected_specs[relative]
        if (
            ...
            or row.get("identity") != list(_identity(os.lstat(root / relative)))
            or row.get("provenance") != "fresh_scp_then_held_exact5_seal"
        ):
            raise HoldPackageError(f"overlay source receipt differs: {relative}")
        raw_by_relative[relative] = stable(
            root / relative, expected_sha, row["size"], expected_mode=0o444,
        )
    executing_raw = stable(Path(__file__).resolve(strict=True), materializer_sha256)
    if executing_raw != raw_by_relative[OVERLAY_MATERIALIZER_RELATIVE]:
        raise HoldPackageError("executing overlay materializer differs from receipt leaf")
    if stable(
        receipt_path, receipt_sha256, receipt_size,
        expected_mode=0o400,
    ) != receipt_raw or list(_identity(os.lstat(root))) != expected_root_identity:
        raise HoldPackageError("overlay post-replay differs")
    return raw_by_relative, {
        "root": str(root), "root_identity": expected_root_identity,
        "receipt": {
            "path": str(receipt_path), "sha256": receipt_sha256,
            "size": receipt_size, "receipt_digest": claimed,
        },
        "source_file_count": 5,
        "materializer": {
            "path": str(root / OVERLAY_MATERIALIZER_RELATIVE),
            "sha256": materializer_sha256,
            "size": len(raw_by_relative[OVERLAY_MATERIALIZER_RELATIVE]),
        },
        "files": rows,
    }
...
def snapshot_expected_files(materializer_sha256: str) -> dict[str, str]:
    ...
    result = {
        **BASE_RELEASE_FILES, **DIAGNOSTIC_FILES, **SNAPSHOT_AUTHORITY_FILES,
        SNAPSHOT_MATERIALIZER_RELATIVE: materializer_sha256,
    }
    if len(result) != 33:
...
def preflight_snapshot(
    ...
    base_materializer_raw = stable(
        source_root / SNAPSHOT_MATERIALIZER_RELATIVE, materializer_sha256,
    )
    if base_materializer_raw != raw_by_relative[SNAPSHOT_MATERIALIZER_RELATIVE]:
        raise HoldPackageError("snapshot materializer leaf replay differs")
    fresh_rows = [
...
def _runtime_identities_from_preflight(
    ...
    internal = {
        "runner": method_prefix + "case01_object_trajectory_exact5_runner_v3.py",
        "legacy_exact5_runner": method_prefix + "case01_source_bone_exact5_runner_v1.py",
        "object_eval": method_prefix + "case01_object_trajectory_exact5_eval_v3.py",
        ...
        "adapter": method_prefix + "infer_case01_object_trajectory_oracle_auh_r5f_v3.py",
        "object_wrapper_inner": method_prefix + "infer_case01_object_trajectory_oracle_v1.py",
    ...
    for role in launcher.IDENTITY_ROLES:
        ...
        result[role] = row
    if tuple(result) != launcher.IDENTITY_ROLES or len(result) != 26:
        raise HoldPackageError("exact26 runtime ordering differs")
    return result
...
def materialize(
    root: Path, source_root: Path, overlay_root: Path,
    overlay_receipt_path: Path, job_id: str, node: str, *,
    snapshot_manifest_sha256: str, snapshot_materializer_sha256: str,
    overlay_materializer_sha256: str,
    overlay_receipt_sha256: str, overlay_receipt_size: int,
    overlay_receipt_digest: str, overlay_root_identity: Sequence[int],
) -> dict[str, Any]:
    require_final_pins()
    expected_overlay_root_identity = validate_overlay_publication_pins(
        overlay_receipt_sha256, overlay_receipt_size,
        overlay_receipt_digest, overlay_root_identity,
    )
    if (
        root != TARGET_ROOT or source_root != SOURCE_SNAPSHOT_ROOT
        or overlay_root != OVERLAY_ROOT
        or overlay_receipt_path != OVERLAY_RECEIPT_PATH
        or snapshot_materializer_sha256 != BASE_MATERIALIZER_SHA256
    ):
        raise HoldPackageError("package/source/overlay authority path differs")
    ...
    # Both sibling receipts and both physical trees are consumed before the
    # first package publication reservation or shadow mkdir.
    snapshot_bytes, snapshot_evidence = preflight_snapshot(
        source_root, manifest_sha256=snapshot_manifest_sha256,
        materializer_sha256=snapshot_materializer_sha256,
    )
    overlay_bytes, overlay_evidence = preflight_overlay(
        overlay_root, overlay_receipt_path,
        materializer_sha256=overlay_materializer_sha256,
        receipt_sha256=overlay_receipt_sha256,
        receipt_size=overlay_receipt_size,
        receipt_digest=overlay_receipt_digest,
        root_identity=expected_overlay_root_identity,
    )
    if replay_staging_receipt_authority(
    ...
    release_bytes = {
        relative: (
            ...
            if relative in OVERLAY_RELEASE_FILES else snapshot_bytes[relative]
        )
    ...
    launcher_source = load_module_bytes(
        overlay_root / launcher_relative, release_bytes[launcher_relative],
        "_case01_hold_launcher_preflight",
    )
    eval_relative = (
        "methods/bernini_action_editing/"
        "case01_object_trajectory_exact5_eval_v3.py"
    )
    eval_module = load_module_bytes(
        overlay_root / eval_relative, release_bytes[eval_relative],
    ...
    for relative in ("release", "authority/conditions", "plan", "launch",
                     "diagnostics", "evidence", "outputs/media_v3", "final",
                     "logs", "runtime"):
    ...
    producer = {
        ...
        "inference_wrapper_path": str(
            method / "infer_case01_object_trajectory_oracle_auh_r5f_v3.py"
        ),
        "inference_wrapper_sha256": FINAL_WRAPPER_SHA256,
        "inference_wrapper_size": len(release_bytes[composite_relative]),
        "object_wrapper_inner_path": str(
            method / "infer_case01_object_trajectory_oracle_v1.py"
        ),
        "object_wrapper_inner_sha256": hashlib.sha256(
            release_bytes[inner_relative]
        ).hexdigest(),
        "object_wrapper_inner_size": len(release_bytes[inner_relative]),
    ...
    plan = eval_module.build_plan(
        ...
        producer=producer, output_root=root / "outputs/media_v3", launch_allowed=False)
    ...
    for task, task_id in zip(plan["tasks"], eval_module.TASK_IDS):
        video = publish_root / "outputs/media_v3" / f"{task_id}.mp4"
        task["output"] = {
    ...
    if (
        ...
        or any(
            not task["output"]["video_path"].startswith(
                str(publish_root / "outputs/media_v3") + os.sep
            )
    ...
    launch_input = {
        ...
        "output_report": str(publish_root / "final/object_trajectory_exact5_report_v3.json"),
        "runner_attestation": str(publish_root / "final/object_trajectory_exact5_runner_attestation_v3.json"),
        "model_root": str(MODEL_ROOT), "bernini_root": str(BERNINI_ROOT),
        "veomni_root": str(VEOMNI_ROOT),
        "authority_root": str(publish_root / "runtime/model-authority-v3"),
        "rank_cache_root": str(RANK_CACHE_ROOT),
    }
    launcher.validate_input(
        launch_input, reopen=False, plan_override=plan,
    )
    input_relative = "launch/root_launch_input_HOLD_v3.json"
    input_raw = canonical(launch_input) + b"\n"
    input_path = record(input_relative, input_raw, 0o444)
    payload_relative = "launch/root_launch_payload_HOLD_v3.sh"
    receipt_relative = "launch/root_launch_receipt_HOLD_v3.json"
    ...
    release_rows = [
        {
            ...
            "sha256": hashlib.sha256(release_bytes[relative]).hexdigest(),
            "size": len(release_bytes[relative]),
            "provenance": (
                "receipt_gated_exact5_overlay"
                if relative in OVERLAY_RELEASE_FILES
                else "receipt_gated_exact35_snapshot"
            ),
        }
        for relative in sorted(RELEASE_FILES)
    ]
    production_identities = launch_input["identities"]
    crosslink = {
        "adapter": production_identities["adapter"],
        "object_wrapper_inner": production_identities["object_wrapper_inner"],
        "producer_adapter": {
            "path": producer["inference_wrapper_path"],
            "sha256": producer["inference_wrapper_sha256"],
            "size": producer["inference_wrapper_size"],
        },
        "producer_object_wrapper_inner": {
            "path": producer["object_wrapper_inner_path"],
            "sha256": producer["object_wrapper_inner_sha256"],
            "size": producer["object_wrapper_inner_size"],
        },
        "distinct_paths": (
            production_identities["adapter"]["path"]
            != production_identities["object_wrapper_inner"]["path"]
        ),
        "outer_calls_pinned_inner_contract": True,
    }
    if (
        crosslink["adapter"] != crosslink["producer_adapter"]
        or crosslink["object_wrapper_inner"]
        != crosslink["producer_object_wrapper_inner"]
        or crosslink["distinct_paths"] is not True
    ):
        raise HoldPackageError("inner/outer producer identity crosslink differs")
    report: dict[str, Any] = {
        "schema_version": "case01-object-trajectory-exact5-r64-hold-materialization-v3",
        ...
        "source_overlay": overlay_evidence,
        "source_provenance": {
            "base_release_leaf_count": 21,
            "overlay_release_leaf_count": 4,
        ...
        "uncooperative_same_uid_race_out_of_scope": True,
        "retry_allowed": False,
        "release_file_count": 25, "production_identity_count": 26,
        "release": {
            "files": release_rows,
            "manifest_digest": object_sha(release_rows),
        },
        "production": {
            "identity_roles": list(launcher.IDENTITY_ROLES),
            "identities": production_identities,
            "identity_set_digest": object_sha(production_identities),
    ...
    report_relative = "authority/package_materialization_receipt_v3.json"
    ...
    validate_shadow_package(
        ...
        extra_directories={
            "evidence", "outputs", "outputs/media_v3", "final", "logs",
            "runtime",
        },
    )
    ...
    overlay_bytes_after, overlay_evidence_after = preflight_overlay(
        overlay_root, overlay_receipt_path,
        materializer_sha256=overlay_materializer_sha256,
        receipt_sha256=overlay_receipt_sha256,
        receipt_size=overlay_receipt_size,
    ...
    if (
        overlay_bytes_after != overlay_bytes
        or overlay_evidence_after != overlay_evidence
    ):
        raise HoldPackageError("pre-publish overlay receipt/tree replay differs")
    ...
    publication_value = {
        ...
        "source_overlay_receipt_sha256": overlay_evidence["receipt"]["sha256"],
        "source_overlay_receipt_digest": overlay_evidence["receipt"]["receipt_digest"],
        "source_overlay_root_identity": overlay_evidence["root_identity"],
        "publication_protocol": STAGING_PUBLICATION_PROTOCOL,
        "rename_noreplace": False,
        "cooperative_writer_exclusion": True,
        "target_absent_rechecked_before_rename": True,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    seal = commands.add_parser("seal-overlay")
    seal.add_argument("--root", default=str(OVERLAY_ROOT))
    seal.add_argument("--receipt", default=str(OVERLAY_RECEIPT_PATH))
    seal.add_argument(
        "--spec", action="append", required=True,
        help="exactly five relative=sha256:size source specifications",
    )
    package = commands.add_parser("materialize")
    package.add_argument("--root", default=str(TARGET_ROOT))
    package.add_argument("--source-root", default=str(SOURCE_SNAPSHOT_ROOT))
    package.add_argument("--overlay-root", default=str(OVERLAY_ROOT))
    package.add_argument(
        "--overlay-receipt", default=str(OVERLAY_RECEIPT_PATH),
    )
    package.add_argument("--job-id", required=True)
    package.add_argument("--node", required=True)
    package.add_argument("--snapshot-manifest-sha256", required=True)
    package.add_argument(
        "--snapshot-materializer-sha256", default=BASE_MATERIALIZER_SHA256,
    )
    package.add_argument("--overlay-materializer-sha256", required=True)
    package.add_argument("--overlay-receipt-sha256", required=True)
    package.add_argument("--overlay-receipt-size", required=True, type=int)
    package.add_argument("--overlay-receipt-digest", required=True)
    package.add_argument("--overlay-root-identity-json", required=True)
    return parser


def _parse_overlay_spec_arguments(values: Sequence[str]) -> dict[str, tuple[str, int]]:
    result: dict[str, tuple[str, int]] = {}
    for value in values:
        if type(value) is not str or "=" not in value:
            raise HoldPackageError("overlay --spec syntax differs")
        relative, encoded = value.split("=", 1)
        if ":" not in encoded:
            raise HoldPackageError("overlay --spec syntax differs")
        digest, size_text = encoded.rsplit(":", 1)
        if relative in result:
            raise HoldPackageError("duplicate overlay --spec path")
        try:
            size = int(size_text, 10)
        except ValueError as error:
            raise HoldPackageError("overlay --spec size differs") from error
        result[relative] = (digest, size)
    return _normalize_overlay_specs(result)


def _parse_root_identity_argument(raw: str) -> list[int]:
    try:
        value = json.loads(raw)
    except (TypeError, ValueError, json.JSONDecodeError) as error:
        raise HoldPackageError("overlay root identity argv is invalid JSON") from error
    if canonical(value).decode("utf-8") != raw:
        raise HoldPackageError("overlay root identity argv is noncanonical")
    return validate_overlay_publication_pins(
        "0" * 64, 1, "0" * 64, value,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "seal-overlay":
            report = seal_overlay_source_root(
                Path(args.root), Path(args.receipt),
                _parse_overlay_spec_arguments(args.spec),
            )
        else:
            report = materialize(
                Path(args.root), Path(args.source_root),
                Path(args.overlay_root), Path(args.overlay_receipt),
                args.job_id, args.node,
                snapshot_manifest_sha256=args.snapshot_manifest_sha256,
                snapshot_materializer_sha256=args.snapshot_materializer_sha256,
                overlay_materializer_sha256=args.overlay_materializer_sha256,
                overlay_receipt_sha256=args.overlay_receipt_sha256,
                overlay_receipt_size=args.overlay_receipt_size,
                overlay_receipt_digest=args.overlay_receipt_digest,
                overlay_root_identity=_parse_root_identity_argument(
                    args.overlay_root_identity_json
                ),
            )
