#!/usr/bin/env python3
"""Exactly-once submit the explicit 1-to-0 shared-TCPStore env canary."""
...
def die(message: str) -> None:
    raise SystemExit(f"submit-saic-r6-shared-store-env-canary: {message}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--formal-launcher", required=True)
    parser.add_argument("--formal-launcher-sha256", required=True)
...
def main() -> int:
    ...
    launcher = exact_file(args.launcher, args.launcher_sha256, "canary launcher")
    formal_launcher = exact_file(
        args.formal_launcher, args.formal_launcher_sha256, "formal launcher"
    )
    ...
    provisional = {
        "schema_version": "saic-r6-shared-store-env-override-submission-v1",
        "status": "reserved_before_sbatch_not_submission_success",
        "submission_success": False,
        "job_success": None,
        "launcher_sha256": args.launcher_sha256,
        "formal_launcher_sha256": args.formal_launcher_sha256,
    }
    ...
    exports = {
        ...
        "SAIC_R6_ENV_CANARY_FORMAL_LAUNCHER": str(formal_launcher),
        "SAIC_R6_ENV_CANARY_FORMAL_LAUNCHER_SHA256": args.formal_launcher_sha256,
        "SAIC_R6_ENV_CANARY_PYTHON": str(python_bin),
        "SAIC_R6_ENV_CANARY_OUTPUT_PARENT": str(output_parent),
    }
    ...
    core = {
        ...
        "submitted_job": {
            ...
            "stdout_sha256": hashlib.sha256(completed.stdout.encode("ascii")).hexdigest(),
            "stderr_sha256": hashlib.sha256(completed.stderr.encode("ascii")).hexdigest(),
        },
        "request": {
            "job_name": "saic-r6-env-cny",
            "partition": "faculty",
            "qos": "bgqos",
            "nodes": 1,
            "ntasks": 1,
        ...
        "inputs": {
            ...
            "launcher_sha256": args.launcher_sha256,
            "formal_launcher": str(formal_launcher),
            "formal_launcher_sha256": args.formal_launcher_sha256,
        },
        "outputs": {
