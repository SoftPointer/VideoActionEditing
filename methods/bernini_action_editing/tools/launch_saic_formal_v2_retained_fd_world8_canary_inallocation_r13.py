...
STEP_RECEIPT_SCHEMA = "saic-formal-v2-retained-fd-world8-inallocation-r13-step-launch-v1"
PROVISIONAL_SCHEMA = STEP_RECEIPT_SCHEMA + "-reservation"
STEM = "saic-formal-v2-retained-fd-world8-canary-inallocation-r13"
ROOT = Path(
...
RECEIPT = OUTPUT_PARENT / "inallocation-r13-step-launch-receipt.json"
CLIENT_RECEIPT = OUTPUT_PARENT / "inallocation-r13-srun-client-receipt.json"
LAUNCHER = INPUTS / "launch_saic_formal_v2_retained_fd_world8_canary_inallocation_r13.py"
WRAPPER = INPUTS / "auh_canary_saic_formal_v2_retained_fd_world8_inallocation_r13.sh"
PAYLOAD = INPUTS / "auh_canary_saic_formal_v2_retained_fd_world8_payload_inallocation_r13.sh"
...
POSTFLIGHT = POSTFLIGHT_ROOT / "postflight_saic_formal_v2_retained_fd_world8_canary_inallocation_r13.py"
...
EXPECTED_WRAPPER_SHA256 = "a5c191379128830bb877536a42e7271a2ade2a1c82b344846d4b5dc33da2ec79"
EXPECTED_PAYLOAD_SHA256 = "36438ff41f3f56786cbcaad992759aa05d2a1db338a6cfc83e2d2b5c9c580951"
EXPECTED_POSTFLIGHT_SHA256 = "391dddb246474b3b260a1a0010e570244c46a40c17361d802ed4a2c1f5c2109b"
...
def die(message: str) -> None:
    raise SystemExit(f"launch-saic-fv2-fd-world8-inallocation-r13: {message}")
...
STDIN_LOADER = (
    ...
    "exec(compile(raw,'inallocation-r13-step-bootstrap','exec'),{'__name__':'__main__'})"
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        != "saic-formal-v2-retained-fd-world8-inallocation-r13-release-manifest-v1"
    ...
    command = [
        ...
        "--open-mode=truncate",
        f"--output={LOG_DIR}/saic-fv2-fd-w8-inalloc-r13-%J.out",
        f"--error={LOG_DIR}/saic-fv2-fd-w8-inalloc-r13-%J.err",
    ...
    try:
        ...
        old_after = os.fstat(receipt_fd)
        public = os.stat(
            RECEIPT.name, dir_fd=output_dir_fd, follow_symlinks=False,
        )
        ...
        if (
            (old_after.st_dev, old_after.st_ino) != (before.st_dev, before.st_ino)
            or (public.st_dev, public.st_ino) != (before.st_dev, before.st_ino)
            or stat.S_IMODE(old_after.st_mode) != 0o444
            or stat.S_IMODE(public.st_mode) != 0o444
            or old_after.st_nlink != 1 or public.st_nlink != 1
        ...
        # A compute node rewrote the reserved inode through another NFS client.
        # The login-side open file description may retain stale cached content,
        # so it is used only for identity/mode checks and is never parsed.
        os.close(receipt_fd)
        receipt_fd = None
        fresh_receipt_fd = os.open(
            RECEIPT.name, os.O_RDONLY | os.O_NOFOLLOW, dir_fd=output_dir_fd,
        )
        try:
            fresh_before = os.fstat(fresh_receipt_fd)
            fresh_public_before = os.stat(
                RECEIPT.name, dir_fd=output_dir_fd, follow_symlinks=False,
            )
            if (
                not stat.S_ISREG(fresh_before.st_mode)
                or fresh_before.st_nlink != 1
                or fresh_before.st_uid != os.getuid()
                or stat.S_IMODE(fresh_before.st_mode) != 0o444
                or not stat.S_ISREG(fresh_public_before.st_mode)
                or fresh_public_before.st_uid != os.getuid()
                or (fresh_before.st_dev, fresh_before.st_ino)
                != (before.st_dev, before.st_ino)
                or (fresh_public_before.st_dev, fresh_public_before.st_ino)
                != (before.st_dev, before.st_ino)
                or stat.S_IMODE(fresh_public_before.st_mode) != 0o444
                or fresh_public_before.st_nlink != 1
                or fresh_before.st_size != fresh_public_before.st_size
                or fresh_before.st_size <= 0
            ):
                die("freshly reopened step receipt identity differs")
            chunks: list[bytes] = []
            while True:
                chunk = os.read(fresh_receipt_fd, 65536)
                if not chunk:
                    break
                chunks.append(chunk)
            final_raw = b"".join(chunks)
            fresh_after = os.fstat(fresh_receipt_fd)
            fresh_public_after = os.stat(
                RECEIPT.name, dir_fd=output_dir_fd, follow_symlinks=False,
            )
            receipt_identity = lambda info: (
                info.st_dev, info.st_ino, info.st_size, info.st_mtime_ns,
                stat.S_IMODE(info.st_mode), info.st_nlink, info.st_uid,
            )
            if (
                receipt_identity(fresh_before) != receipt_identity(fresh_after)
                or fresh_after.st_size != len(final_raw)
                or (fresh_after.st_dev, fresh_after.st_ino)
                != (fresh_public_after.st_dev, fresh_public_after.st_ino)
                or receipt_identity(fresh_public_before)
                != receipt_identity(fresh_public_after)
            ):
                die("freshly reopened step receipt changed during read")
        finally:
            os.close(fresh_receipt_fd)
        ...
        client_core = {
            "schema_version": (
                "saic-formal-v2-retained-fd-world8-inallocation-r13-srun-client-v1"
            ),
