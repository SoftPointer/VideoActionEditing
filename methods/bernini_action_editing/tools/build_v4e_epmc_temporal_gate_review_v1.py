#!/usr/bin/env python3
"""Build the future-use detached v4-E/EPMC temporal-gate HTML review.
...
creating output until the v4-E canary runtime is fully pinned.
"""
...
SCHEMA = "bernini-v4e-epmc-temporal-gate-review-packet-v1"
...
class V4EEPMCReviewError(RuntimeError):
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise V4EEPMCReviewError("value is not canonical JSON") from error
...
def _required_sha256(value: Any, *, label: str) -> str:
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise V4EEPMCReviewError(f"{label} must be a lowercase SHA-256")
    return value
...
def _require_release_sealed() -> Any:
    ...
    if (
        ...
        raise V4EEPMCReviewError(
            "UNSEALED v4-E EPMC review builder: runtime pins are TO_BE_PINNED"
        )
    from methods.bernini_action_editing import (
        infer_v4e_epmc_temporal_gate_canary_v1 as runtime,
    )
    ...
    if (
        ...
        raise V4EEPMCReviewError("sealed v4-E runtime source/schema ABI differs")
    return runtime
...
def _plain_file(
    ...
    *,
    ...
    if not path.is_absolute():
        raise V4EEPMCReviewError(f"{label} must be an absolute path")
    try:
        info = path.lstat()
    except OSError as error:
        raise V4EEPMCReviewError(f"cannot stat {label}") from error
    if (
        stat.S_ISLNK(info.st_mode)
        or not stat.S_ISREG(info.st_mode)
        or info.st_nlink != 1
    ):
        raise V4EEPMCReviewError(f"{label} must be a plain nlink1 file")
    if maximum_bytes is not None and not 0 < info.st_size <= maximum_bytes:
        raise V4EEPMCReviewError(f"{label} size exceeds its bound")
...
def _reject_duplicate_keys(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V4EEPMCReviewError(f"duplicate JSON key: {key!r}")
        result[key] = value
    return result


def _reject_nonfinite(value: str) -> None:
    raise V4EEPMCReviewError(f"non-finite JSON number: {value}")
...
def _strict_receipt(
    ...
    except V4EEPMCReviewError:
        raise
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V4EEPMCReviewError(f"seed {seed} receipt is not ASCII JSON") from error
    finally:
        if descriptor is not None:
            os.close(descriptor)
    if type(value) is not dict:
        raise V4EEPMCReviewError(f"seed {seed} receipt must be an object")
    ...
    if (
        ...
        or value.get("v4e_aggregate_gate_verified_true") is not True
        ...
        or gate_state.get("v4e_aggregate_gate_verified_true") is not True
        ...
        or claims.get("aggregate_fold_receipt_checkpoint_strong_join_before_render")
        is not True
        ...
        raise V4EEPMCReviewError(f"seed {seed} receipt closure differs")
    outputs = value.get("outputs")
    if not isinstance(outputs, Mapping) or set(outputs) != set(ARMS):
        raise V4EEPMCReviewError(f"seed {seed} output manifest differs")
    return value
...
def _copy_sealed(source: Path, destination: Path, *, expected_sha256: str) -> None:
    ...
    if destination.exists() or destination.is_symlink():
        raise V4EEPMCReviewError(f"refusing to overwrite {destination}")
    if _file_sha256(source) != expected_sha256:
        raise V4EEPMCReviewError(f"source hash differs for {source}")
    ...
    if (
        ...
        raise V4EEPMCReviewError("sealed review copy differs")
    ...
    for seed in SEEDS:
        ...
        cards = [
            ...
            _video_card(
                ...
                "v4-E decoded-residual temporal order",
                "candidate",
            ),
            _video_card(
                "Reverse",
...
<title>v4-E → EPMC temporal-gating diagnostic</title>
<style>
...
<h1>v4-E → real Bernini EPMC temporal gating</h1>
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise V4EEPMCReviewError("source/detached-anchor authority pin differs")
    if (
        hashlib.sha256(args.instruction.encode("utf-8")).hexdigest()
        != EXPECTED_INSTRUCTION_SHA256
    ):
        raise V4EEPMCReviewError("instruction bytes differ")
    ...
    if (
        ...
        raise V4EEPMCReviewError("source/detached-anchor SHA256 differs")
    ...
    for seed, run_dir in run_dirs.items():
        if not run_dir.is_absolute() or run_dir.is_symlink() or not run_dir.is_dir():
            raise V4EEPMCReviewError(f"seed {seed} run directory differs")
        ...
        if _file_sha256(receipt_path) != receipt_hashes[seed]:
            raise V4EEPMCReviewError(f"seed {seed} receipt file SHA256 differs")
        ...
        for arm in ARMS:
            ...
            if not isinstance(manifest, Mapping):
                raise V4EEPMCReviewError(f"seed {seed} {arm} manifest differs")
            ...
            if (
                ...
                raise V4EEPMCReviewError(f"seed {seed} {arm} output join differs")
    ...
    if not output.is_absolute() or output.exists() or output.is_symlink():
        raise V4EEPMCReviewError("output-dir must be a fresh absolute directory")
    ...
    if stat.S_IMODE(index.stat().st_mode) != 0o444 or index.stat().st_nlink != 1:
        raise V4EEPMCReviewError("index seal differs")
    ...
    packet: dict[str, Any] = {
        ...
        "aggregate_fold_receipt_checkpoint_strong_join_verified_by_runtime": True,
    ...
    if (
        ...
        raise V4EEPMCReviewError("packet seal/readback differs")
    for directory in (
        media / "seed2028",
        media / "seed2029",
        media,
