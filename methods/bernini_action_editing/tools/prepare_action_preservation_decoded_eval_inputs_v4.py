#!/usr/bin/env python3
"""Build the r7-specific exact15-r3 decoded-eval input authorities.
...
SCHEMA = "bernini-action-preservation-decoded-eval-r7-input-authority-v4"
...
class Exact15R3InputPreparationError(RuntimeError):
    """An r7/exact15-r3 authority or create-only publication differs."""
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise Exact15R3InputPreparationError(
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise Exact15R3InputPreparationError(message)
...
def _strict_json(raw: bytes, *, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Exact15R3InputPreparationError(f"cannot decode {label}") from error
...
def build_authority_receipt(
    *, release_dir: Path, model_manifest_path: Path,
    ...
    try:
        ...
        raise Exact15R3InputPreparationError(str(error)) from error
    ...
    _require(
        ...
        "runtime/builder exact15-r3 trust closure differs",
    )
    ...
    _require(
        ...
        "forbidden ptrace authorization exists in exact15-r3",
    )
    value: dict[str, Any] = {
        ...
        "release_component_pin_set_digest": object_sha256(
            [
                {
                    "path": relative,
                    "sha256": builder.EXPECTED_COMPONENTS[relative][0],
                    "size": builder.EXPECTED_COMPONENTS[relative][1],
                    "mode": builder.EXPECTED_COMPONENTS[relative][2],
                }
                for relative in builder.MEMBER_ORDER
            ]
        ),
        "training_authority": {
        ...
        "gpu_used": False,
        "scientific_promotion_authorized": False,
        "status": "R7_EXACT15_R3_INPUT_AUTHORITY_PREPARED_NOT_DEPLOYED",
    }
...
__all__ = [
    "Exact15R3InputPreparationError",
    "R7_EXACT32_ROWS_DIGEST",
    "R7_TRAINING_COMPLETE_SHA256",
    "SCHEMA",
    "SOURCE_PREPROCESSING_SCHEMA",
