#!/usr/bin/env python3
"""Build the sealed source snapshot for the case01 exact-five R64 canary (v2).

Historical production bytes are read exclusively from the retained r5f
snapshot.  Its source leaves are required to be exactly 0644; this is distinct
from the newly produced target snapshot, whose leaves are exactly 0444.  Only the new exact5 evaluator, wrapper, launcher, package
...
FAILED_BUILDER_RELATIVE = (
    "methods/bernini_action_editing/tools/"
    "build_case01_source_bone_exact5_source_snapshot_v1.py"
)
FAILED_BUILDER_SHA256 = (
    "906db79519f8689f4ec3a2ceee626f788cd8d9f032178ba2b61346a5108d9a69"
)
FAILED_BUILDER_SIZE = 19690
AUDIT_RELATIVE = (
...
def stable_file(
    ...
    *,
    expected_mode: int,
    expected_uid: int = 2012,
    expected_gid: int = 2000,
) -> tuple[bytes, int]:
    if (
    ...
    if (
        ...
        or ident(before) != ident(named) or before.st_uid != expected_uid
        or before.st_gid != expected_gid or mode != expected_mode
...
def _exact_staging_closure() -> None:
    expected = set(NEW_STAGED_FILES) | {
        BUILDER_RELATIVE, FAILED_BUILDER_RELATIVE,
    }
    if len(expected) != 9:
        raise Exact5SnapshotError("staging expected-file count differs")
    actual: set[str] = set()
    actual_directories = {"."}
    ...
    # Every staging leaf is exactly 0644/uid2012/gid2000/nlink1.  In
    # particular, do not replace this with a generic "not group/world
    # writable" test: 0444 and 0600 are both authority drift here.
    for relative, digest in NEW_STAGED_FILES.items():
        stable_file(
            STAGING_ROOT / relative, digest, expected_mode=0o644,
        )
    failed_raw, _ = stable_file(
        STAGING_ROOT / FAILED_BUILDER_RELATIVE, FAILED_BUILDER_SHA256,
        expected_mode=0o644,
    )
    if len(failed_raw) != FAILED_BUILDER_SIZE:
        raise Exact5SnapshotError("failed-lineage builder size differs")
    stable_file(
        STAGING_ROOT / BUILDER_RELATIVE, None, expected_mode=0o644,
    )
...
def build_snapshot() -> dict[str, Any]:
    ...
    for relative, digest in OLD_REUSED_FILES.items():
        raw, _ = stable_file(
            OLD_R5F_SNAPSHOT / relative, digest, expected_mode=0o644,
        )
    ...
    for relative, (raw, _) in sources.items():
        replay, _ = stable_file(
            TARGET_ROOT / relative, hashlib.sha256(raw).hexdigest(),
            expected_mode=0o444,
        )
        if replay != raw:
    ...
    stable_file(
        ...
        hashlib.sha256(manifest_raw).hexdigest(), expected_mode=0o444,
    )
    return manifest
