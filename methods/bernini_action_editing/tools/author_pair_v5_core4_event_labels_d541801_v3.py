...
This d541801/v3 closure accepts only the exact frozen scorer source that made
the completed 40-receipt bank.  A later scorer with compatible-looking APIs is
not an authority for this calibration bridge.
...
TEMPLATE_SCHEMA = (
    "bernini-pair-v5-core4-v2-event-label-authoring-template-d541801-v3"
)
...
def _scorer_runtime() -> Any:
    """Load only the exact Torch-bearing d541801/v3 bank verifier."""
    ...
    try:
        source_path = _plain_file(
            getattr(runtime, "__file__", ""), label="frozen d541801 scorer source"
        )
    except (TypeError, PairV5Core4LabelAuthoringError) as error:
        raise PairV5Core4LabelAuthoringError(
            "frozen d541801 scorer source path is unavailable"
        ) from error
    if (
        file_sha256(source_path) != REQUIRED_SCORER_SOURCE_SHA256
        or getattr(runtime, "SCORE_RECEIPT_SCHEMA", None)
        != REQUIRED_SCORE_RECEIPT_SCHEMA
        or getattr(runtime, "GROUP_RECEIPT_SCHEMA", None)
        != REQUIRED_GROUP_RECEIPT_SCHEMA
    ):
        raise PairV5Core4LabelAuthoringError(
            "loaded scorer is not the exact frozen d541801/v3 authority"
        )
    return runtime
