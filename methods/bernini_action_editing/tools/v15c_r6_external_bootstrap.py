#!/usr/bin/env python3
"""External, stdlib-only trust bootstrap for the v15c-r6 observer package.
...
LAUNCHER_RELATIVE_PATH = (
    ...
    "auh_launch_e00_source_sam2_proposal_role_probe_v15c_r6_sealed.sh"
)
...
class BootstrapV15CR6Error(RuntimeError):
    """The externally authenticated release closure differs."""


BootstrapV15CR5Error = BootstrapV15CR6Error
...
def require_sha(value: Any, label: str) -> str:
    if not isinstance(value, str) or SHA_RE.fullmatch(value) is None:
        raise BootstrapV15CR5Error(f"{label} is not lowercase SHA256")
    return value
...
def _regular_lstat(path: Path, label: str) -> os.stat_result:
    try:
        ...
        raise BootstrapV15CR5Error(f"{label} is absent") from error
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise BootstrapV15CR5Error(f"{label} is not one regular non-symlink file")
    return info
...
def file_sha256(path: Path) -> str:
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode):
            raise BootstrapV15CR5Error("opened release member is not regular")
        while True:
        ...
        if (
            ...
            raise BootstrapV15CR5Error("release member changed while hashing")
    finally:
        os.close(descriptor)
...
def descriptor_sha256(descriptor: int) -> str:
    ...
    try:
        ...
        before = os.fstat(descriptor)
        if not stat.S_ISREG(before.st_mode):
            raise BootstrapV15CR5Error("trusted bootstrap descriptor is not regular")
        ...
        raise BootstrapV15CR5Error("trusted bootstrap descriptor differs") from error
    if (
        before.st_dev,
        before.st_ino,
        before.st_size,
        ...
        raise BootstrapV15CR5Error("trusted bootstrap descriptor changed")
    return digest.hexdigest()


def descriptor_samefile(descriptor: int, path: Path) -> bool:
    """Compare an authenticated descriptor with one canonical pathname."""
    try:
        descriptor_info = os.fstat(descriptor)
        path_info = _regular_lstat(path, str(path))
    except OSError as error:
        raise BootstrapV15CR5Error("descriptor/path identity differs") from error
...
def open_beneath(root: Path, relative: str) -> int:
    ...
    if not stat.S_ISREG(os.fstat(result).st_mode):
        os.close(result)
        raise BootstrapV15CR5Error("secure-open release member is not regular")
    return result
...
def descriptor_bytes(descriptor: int) -> bytes:
    try:
        ...
        raise BootstrapV15CR5Error("release descriptor read differs") from error
    if (
        before.st_dev,
        before.st_ino,
        before.st_size,
        ...
        raise BootstrapV15CR5Error("release descriptor changed while reading")
...
def read_json(path: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise BootstrapV15CR5Error("release manifest JSON differs") from error
    if type(value) is not dict:
        raise BootstrapV15CR5Error("release manifest is not one object")
    return value


def _relative_path(value: Any, label: str) -> str:
    if not isinstance(value, str) or not value or "\\" in value:
        raise BootstrapV15CR5Error(f"{label} path differs")
...
def _path_beneath(root: Path, relative: str, *, final_regular: bool) -> Path:
    ...
    for index, part in enumerate(parts):
        ...
        try:
            ...
            raise BootstrapV15CR5Error("release closure path is absent") from error
        if stat.S_ISLNK(info.st_mode):
            raise BootstrapV15CR5Error("release closure contains a symlink")
        if index + 1 < len(parts):
            if not stat.S_ISDIR(info.st_mode):
                raise BootstrapV15CR5Error("release closure parent is not a directory")
        elif final_regular and not stat.S_ISREG(info.st_mode):
            raise BootstrapV15CR5Error("release closure member is not regular")
    return current


def _verify_manifest_semantics(manifest: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    if set(manifest) != set(RELEASE_KEYS):
        raise BootstrapV15CR5Error("release exact keys differ")
    payload = dict(manifest)
    claimed = require_sha(payload.pop("release_sha256", None), "release self-hash")
    if claimed != object_sha256(payload):
        raise BootstrapV15CR5Error("release self-hash differs")
    ...
    if paths != sorted(paths) or len(set(paths)) != EXPECTED_CORE_MEMBER_COUNT:
        raise BootstrapV15CR5Error("release member registry differs")
    if LAUNCHER_RELATIVE_PATH not in paths or not any(
        path.endswith("finalize_source_sam2_proposal_role_probe_v15c_r6.py")
        for path in paths
    ):
        raise BootstrapV15CR5Error("launcher/finalizer are outside release closure")
...
def verify_release_source(
    ...
    if not source_root.is_dir() or source_root.is_symlink():
        raise BootstrapV15CR5Error("source root differs")
    ...
    if release_path.absolute() != expected_release.absolute():
        raise BootstrapV15CR5Error("release manifest placement differs")
    ...
    if hashlib.sha256(release_bytes).hexdigest() != expected:
        raise BootstrapV15CR5Error("external release hash differs")
    try:
    ...
    for row in members:
        ...
        if info.st_size != row["size"] or digest != row["sha256"]:
            raise BootstrapV15CR5Error("release member bytes differ")
    return manifest
...
def verify_snapshot(
    ...
    *,
    ...
    if snapshot.is_symlink() or not snapshot.is_dir():
        raise BootstrapV15CR5Error("snapshot root differs")
    ...
    for current_text, directory_names, file_names in os.walk(snapshot, topdown=True, followlinks=False):
        ...
        for name in directory_names:
            ...
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
                raise BootstrapV15CR5Error("snapshot contains a non-directory or symlink")
            relative = path.relative_to(snapshot).as_posix()
            if name == "__pycache__":
                raise BootstrapV15CR5Error("snapshot contains __pycache__")
    ...
    if observed_files != files_expected or observed_directories != directories_expected:
        raise BootstrapV15CR5Error("snapshot exact tree differs")
    expected_file_rows = {
    ...
    for relative in sorted(files_expected):
        ...
        if (
            ...
            raise BootstrapV15CR5Error("snapshot member bytes/mode/link-count differ")
    ...
    for relative in directories_expected:
        ...
        if stat.S_IMODE(path.lstat().st_mode) != expected_directory_mode:
            raise BootstrapV15CR5Error("snapshot directory mode differs")
    reopened = read_json(release_path)
    if reopened != manifest:
        raise BootstrapV15CR5Error("snapshot manifest differs")
    return observed
...
def snapshot_observation(
    ...
    *,
    ...
    """Record either the live construction state or the live sealed state."""
    files = verify_snapshot(
        snapshot, manifest, expected_release_sha256, sealed=sealed
    )
    directories = {"."}
    for current_text, directory_names, _ in os.walk(
        ...
        current = Path(current_text)
        for name in directory_names:
            directories.add((current / name).relative_to(snapshot).as_posix())
    if len(files) != EXPECTED_SNAPSHOT_FILE_COUNT:
        raise BootstrapV15CR5Error("snapshot file count differs")
    return {
        "observation_scope": (
...
def _copy_one(
    ...
    try:
        ...
        if not stat.S_ISREG(source_before.st_mode) or source_before.st_size != expected_size:
            raise BootstrapV15CR5Error("copy source identity differs")
        ...
        try:
            while True:
                ...
                while view:
                    ...
                    if written <= 0:
                        raise BootstrapV15CR5Error("snapshot short write")
                    view = view[written:]
        ...
        if (
            ...
            raise BootstrapV15CR5Error("copy source changed during read")
    finally:
        os.close(source_fd)
    if copied != expected_size or digest.hexdigest() != expected_sha:
        raise BootstrapV15CR5Error("copied member bytes differ")
...
def materialize_snapshot(
    ...
    if snapshot.exists() or snapshot.is_symlink():
        raise BootstrapV15CR5Error("snapshot destination is not fresh")
    ...
    construction = snapshot_observation(
    ...
    sealed = snapshot_observation(
        snapshot, manifest, expected_release_sha256, sealed=True
    )
...
def publish_noreplace(path: Path, payload: bytes) -> None:
    ...
    if path.exists() or path.is_symlink() or temporary.exists() or temporary.is_symlink():
        raise BootstrapV15CR5Error("bootstrap receipt destination is not fresh")
    descriptor = os.open(
        temporary,
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise BootstrapV15CR5Error("bootstrap receipt short write")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
    ...
    if path.read_bytes() != payload:
        raise BootstrapV15CR5Error("bootstrap receipt reopen differs")


def _clean_environment(python_bin: Path, expected_job: str) -> dict[str, str]:
    environment = {
        "HOME": "/nonexistent/v15c-r6",
        "LANG": "C",
        "LC_ALL": "C",
    ...
    for key in (
        ...
        if value is not None:
            if SAFE_GPU_ENV_RE.fullmatch(value) is None:
                raise BootstrapV15CR5Error(f"unsafe inherited {key}")
...
def _exact_keys(value: Mapping[str, Any], keys: Sequence[str], label: str) -> None:
    if set(value) != set(keys):
        raise BootstrapV15CR5Error(f"{label} exact keys differ")
...
def bootstrap_and_exec(args: argparse.Namespace) -> None:
    ...
    if (
        ...
        or not descriptor_samefile(args.trusted_bootstrap_fd, bootstrap_path)
    ):
        raise BootstrapV15CR5Error("external bootstrap bytes differ")
    if (
        args.trusted_python_fd != 8
        or descriptor_sha256(args.trusted_python_fd) != expected_python_sha
        or file_sha256(python_bin) != expected_python_sha
        or not descriptor_samefile(args.trusted_python_fd, python_bin)
    ):
        raise BootstrapV15CR5Error("Python bytes differ")
    if (
        sys.executable != str(python_bin)
        or not os.path.samefile(sys.executable, python_bin)
        or file_sha256(Path(sys.executable)) != expected_python_sha
    ):
        raise BootstrapV15CR5Error(
            "live Python argv0/sys.executable authority differs"
        )
    if args.expected_job != "143808" or args.expected_node != "auh7-1b-gpu-292":
        raise BootstrapV15CR5Error("execution constants differ")
    if os.environ.get("SLURM_JOB_ID") != args.expected_job:
        raise BootstrapV15CR5Error("Slurm job authority differs")
    node = os.uname().nodename.split(".", 1)[0]
    if node != args.expected_node:
        raise BootstrapV15CR5Error("node authority differs")
    ...
    if not RUN_NAME_RE.fullmatch(run_root.name):
        raise BootstrapV15CR5Error("run id differs")
    ...
    if run_root.parent.absolute() != run_parent or run_root.exists() or run_root.is_symlink():
        raise BootstrapV15CR5Error("run root is not a fresh direct child")
    run_root.mkdir(mode=0o700)
    snapshot = run_root / "sealed_code_snapshot"
    snapshot_observations = materialize_snapshot(
        source_root,
        release_path,
        snapshot,
        manifest,
    ...
    receipt: dict[str, Any] = {
        ...
        "python": {
            ...
            "startup_flags": ["-I", "-S", "-B"],
            "trusted_fd": args.trusted_python_fd,
            "trusted_fd_sha256": descriptor_sha256(args.trusted_python_fd),
        ...
        "release": {
            ...
            "core_member_count": len(manifest["members"]),
            "snapshot_file_count": EXPECTED_SNAPSHOT_FILE_COUNT,
        },
        "snapshot": {
            "root": str(snapshot),
            "construction_phase": snapshot_observations["construction_phase"],
            "sealed_phase": snapshot_observations["sealed_phase"],
        },
        "execution": {
    ...
    if (
        launcher_info.st_nlink != 1
        or descriptor_sha256(launcher_fd) != next(
            row["sha256"]
            for row in manifest["members"]
        ...
        raise BootstrapV15CR5Error("launcher changed before descriptor exec")
