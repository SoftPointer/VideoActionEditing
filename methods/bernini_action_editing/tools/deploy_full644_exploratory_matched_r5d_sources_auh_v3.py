...
SCHEMA = "full644-exploratory-matched-r5d-source-deploy-auh-v3"
...
def publish_final(parent_fd,name,absolute,raw,digest):
    flags=os.O_RDWR|os.O_CREAT|os.O_EXCL|getattr(os,"O_NOFOLLOW",0)|getattr(os,"O_CLOEXEC",0)
    descriptor=os.open(name,flags,0,dir_fd=parent_fd)
    try:
        offset=0
        while offset<len(raw):
            count=os.write(descriptor,raw[offset:])
            if count<=0:
                fail("final target write made no progress")
            offset+=count
        os.fsync(descriptor)
        if pread(descriptor,len(raw))!=raw:
            fail("final target replay differs before commit")
        info=os.fstat(descriptor)
        named=os.stat(name,dir_fd=parent_fd,follow_symlinks=False)
        absolute_info=os.lstat(absolute)
        if not os.path.isabs(absolute) or os.path.normpath(absolute)!=absolute or os.path.realpath(absolute)!=absolute or ident(info)!=ident(named) or ident(info)!=ident(absolute_info) or not stat.S_ISREG(info.st_mode) or stat.S_IMODE(info.st_mode)!=0 or info.st_uid!=EXPECTED_UID or info.st_gid!=EXPECTED_GID or info.st_nlink!=1 or info.st_size!=len(raw) or hashlib.sha256(pread(descriptor,len(raw))).hexdigest()!=digest:
            fail("final target precommit identity differs")
        os.fchmod(descriptor,0o644)
        os.fsync(descriptor)
        info=os.fstat(descriptor)
        named=os.stat(name,dir_fd=parent_fd,follow_symlinks=False)
        absolute_info=os.lstat(absolute)
        replay=pread(descriptor,len(raw))
        if ident(info)!=ident(named) or ident(info)!=ident(absolute_info) or stat.S_IMODE(info.st_mode)!=0o644 or info.st_uid!=EXPECTED_UID or info.st_gid!=EXPECTED_GID or info.st_nlink!=1 or info.st_size!=len(raw) or replay!=raw or hashlib.sha256(replay).hexdigest()!=digest:
            fail("final target commit identity differs")
    finally:
        os.close(descriptor)
...
try:
    ...
    try:
        ...
        for relative,digest,size,parent_fd,name,absolute in targets:
            require_absent_at(parent_fd,name)
            publish_final(parent_fd,name,absolute,captured[relative],digest)
            os.fsync(parent_fd)
...
def authorization_token(
    ...
    return hashlib.sha256(
        b"r5d-source-deploy-explicit-authorization-v3\0"
...
compile(source,"<r5d-source-deploy-v3>","exec",dont_inherit=True)
