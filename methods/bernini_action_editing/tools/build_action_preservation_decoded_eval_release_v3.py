#!/usr/bin/env python3
"""Create/audit the pinned exact15-r2 decoded-eval release.
...
This successor supplies a disjoint generation, exact 15-member closure,
producer-exact r7 training-audit parsing, and independent literal
SHA/size/mode pins.  It never changes an earlier release directory.
"""
...
EXPECTED_COMPONENTS: Mapping[str, tuple[str, int, int]] = {
    ...
    "action_preservation_decoded_eval_bridge_v1.py": (
        "4081fe4c3cda50b7cb6c7be2e540baf06d50ce7e8b1fd9cbb10a8a85bda3fc85",
        92838,
        0o444,
    ),
...
MEMBER_ORDER = tuple(sorted(EXPECTED_COMPONENTS))
...
def _engine() -> ModuleType:
    if (
        MEMBER_ORDER != tuple(sorted(EXPECTED_COMPONENTS))
        or list(MEMBER_ORDER) != sorted(MEMBER_ORDER)
        or len(MEMBER_ORDER) != len(set(MEMBER_ORDER))
    ):
        raise Exact15ReleaseBuildError(
            "exact15 member order is not the unique ASCII lexical order"
        )
...
def _result(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    row["successor_builder"] = "retained-fd-model-authority-producer-audit-v3"
    return row


def build(release_dir: Path) -> dict[str, Any]:
    del release_dir
    raise Exact15ReleaseBuildError(OBSOLETE_REASON)
