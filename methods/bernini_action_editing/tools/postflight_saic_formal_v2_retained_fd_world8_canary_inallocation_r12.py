...
STEP_RECEIPT = OUTPUT_PARENT / "inallocation-r12-step-launch-receipt.json"
CLIENT_RECEIPT = OUTPUT_PARENT / "inallocation-r12-srun-client-receipt.json"
ADMISSION = OUTPUT_PARENT / "inallocation-r12-step-admission.json"
...
def die(message: str) -> None:
    raise SystemExit(f"postflight-saic-fv2-fd-world8-inallocation-r12: {message}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    evidence, evidence_raw = sealed(
        evidence_path,
        "saic-formal-v2-retained-fd-world8-inallocation-r12-operational-evidence-v1",
    ...
    if (
        ...
        or accounting["JobName"] != "saic-fv2-fd-w8-inalloc-r12"
    ...
    if (
        ...
        or expected_command.count("--job-name=saic-fv2-fd-w8-inalloc-r12") != 1
    ...
    stdout_log = LOG_DIR / f"saic-fv2-fd-w8-inalloc-r12-{job_step_id}.out"
    stderr_log = LOG_DIR / f"saic-fv2-fd-w8-inalloc-r12-{job_step_id}.err"
    sentinel = f"SAIC_FV2_FD_WORLD8_INALLOCATION_R12_PASS {job_step_id}\n".encode(
        "ascii"
    )
    ...
    core = {
        "schema_version": (
            "saic-formal-v2-retained-fd-world8-inallocation-r12-step-admission-v1"
        ),
