#!/usr/bin/env python3
"""Build the future-use detached v4-D/EPMC temporal-gate HTML review.
...
creating output until the v4-D canary runtime is fully pinned.
"""
...
SCHEMA = "bernini-v4d-epmc-temporal-gate-review-packet-v1"
...
class V4DEPMCReviewError(RuntimeError):
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise V4DEPMCReviewError("value is not canonical JSON") from error
...
def _required_sha256(value: Any, *, label: str) -> str:
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise V4DEPMCReviewError(f"{label} must be a lowercase SHA-256")
    return value
...
def _require_release_sealed() -> Any:
    ...
    if (
        ...
        raise V4DEPMCReviewError(
            "UNSEALED v4-D EPMC review builder: runtime pins are TO_BE_PINNED"
        )
    from methods.bernini_action_editing import (
        infer_v4d_epmc_temporal_gate_canary_v1 as runtime,
    )
    ...
    if (
        ...
        raise V4DEPMCReviewError("sealed v4-D runtime source/schema ABI differs")
    return runtime
...
def _plain_file(
    ...
    *,
    ...
    if not path.is_absolute():
        raise V4DEPMCReviewError(f"{label} must be an absolute path")
    try:
        info = path.lstat()
    except OSError as error:
        raise V4DEPMCReviewError(f"cannot stat {label}") from error
    if (
        stat.S_ISLNK(info.st_mode)
        or not stat.S_ISREG(info.st_mode)
        or info.st_nlink != 1
    ):
        raise V4DEPMCReviewError(f"{label} must be a plain nlink1 file")
    if maximum_bytes is not None and not 0 < info.st_size <= maximum_bytes:
        raise V4DEPMCReviewError(f"{label} size exceeds its bound")
...
def _reject_duplicate_keys(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V4DEPMCReviewError(f"duplicate JSON key: {key!r}")
        result[key] = value
    return result


def _reject_nonfinite(value: str) -> None:
    raise V4DEPMCReviewError(f"non-finite JSON number: {value}")
...
def _strict_receipt(path: Path, *, seed: int) -> dict[str, Any]:
    ...
    except V4DEPMCReviewError:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V4DEPMCReviewError(f"seed {seed} receipt is not ASCII JSON") from error
    if type(value) is not dict:
        raise V4DEPMCReviewError(f"seed {seed} receipt must be an object")
    ...
    if (
        ...
        or value.get("v4d_aggregate_gate_verified_true") is not True
        ...
        raise V4DEPMCReviewError(f"seed {seed} receipt closure differs")
    outputs = value.get("outputs")
    if not isinstance(outputs, Mapping) or set(outputs) != set(ARMS):
        raise V4DEPMCReviewError(f"seed {seed} output manifest differs")
    return value
...
def _copy_sealed(source: Path, destination: Path, *, expected_sha256: str) -> None:
    ...
    if destination.exists() or destination.is_symlink():
        raise V4DEPMCReviewError(f"refusing to overwrite {destination}")
    if _file_sha256(source) != expected_sha256:
        raise V4DEPMCReviewError(f"source hash differs for {source}")
    ...
    if (
        ...
        raise V4DEPMCReviewError("sealed review copy differs")
    ...
    for seed in SEEDS:
        ...
        cards = [
            ...
            _video_card(
                ...
                "v4-D decoded-residual temporal order",
                "candidate",
            ),
            _video_card(
                "Reverse",
...
<title>v4-D → EPMC temporal-gating diagnostic</title>
<style>
...
<h1>v4-D → real Bernini EPMC temporal gating</h1>
<div class="warning danger"><strong>PRIVILEGED OOF TEMPORAL-GATING DIAGNOSTIC ONLY.</strong> 这不是 action representation、renderer、画质或 video-editing qualification，也不是 source+instruction-only 推理。Anchor RGB 只作为 detached HTML reference，绝未进入 Bernini；但 gate 由该 OOF anchor 的 V-JEPA feature 生成，因此仍是 privileged side information。</div>
<div class="warning">v4-D 使用 <code>R=C(D(E(C(anchor))))-C(D(0))</code> 的 decoded residual，经 fold-1 model-fit-original-only p95 缩放、clamp 与 32→20 linear/align_corners 插值。16×12 block/head gates 与 phase0 均为正零，所以 EPMC effective gate 为 0.5×profile；再乘 outer CPMR 0.10 后，实际 projected-motion residual 系数仅为 <strong>0.05×profile</strong>。B0/Zero 只保证 final latent byte-exact，不声明 MP4 container byte parity。</div>
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise V4DEPMCReviewError("source/detached-anchor authority pin differs")
    if (
        hashlib.sha256(args.instruction.encode("utf-8")).hexdigest()
        != EXPECTED_INSTRUCTION_SHA256
    ):
        raise V4DEPMCReviewError("instruction bytes differ")
    ...
    if (
        ...
        raise V4DEPMCReviewError("source/detached-anchor SHA256 differs")
    ...
    for seed, run_dir in run_dirs.items():
        if not run_dir.is_absolute() or run_dir.is_symlink() or not run_dir.is_dir():
            raise V4DEPMCReviewError(f"seed {seed} run directory differs")
        ...
        if _file_sha256(receipt_path) != receipt_hashes[seed]:
            raise V4DEPMCReviewError(f"seed {seed} receipt file SHA256 differs")
        ...
        for arm in ARMS:
            ...
            if not isinstance(manifest, Mapping):
                raise V4DEPMCReviewError(f"seed {seed} {arm} manifest differs")
            ...
            if (
                ...
                raise V4DEPMCReviewError(f"seed {seed} {arm} output join differs")
    ...
    if not output.is_absolute() or output.exists() or output.is_symlink():
        raise V4DEPMCReviewError("output-dir must be a fresh absolute directory")
    ...
    if stat.S_IMODE(index.stat().st_mode) != 0o444 or index.stat().st_nlink != 1:
        raise V4DEPMCReviewError("index seal differs")
    ...
    if (
        ...
        raise V4DEPMCReviewError("packet seal/readback differs")
    for directory in (
        media / "seed2028",
        media / "seed2029",
        media,
