#!/usr/bin/env python3
"""Frozen remote-only bootstrap for CPU-controller stage/recovery.

This source is an independent held-stdin authority.  It validates the pinned
single-controller payload, performs either the exclusive-receipt-reserved stage
...
# Stable structural allowlist only.  Exact SHA-256/size comes from the
# controller-authenticated manifest and is rebound to the held payload bytes.
# Keeping the mutable final controller hash out of this bootstrap makes the
# authority graph one-way: final CPU controller -> deployer -> this verifier.
...
class SourceStageError(RuntimeError):
    """The receipt-reserved CPU-controller deployment contract differs."""


# Compatibility name for the deployer-focused hostile suite.
ControllerDeployBootstrapError = SourceStageError
...
def _validate_specs(value: Any) -> tuple[dict[str, Any], ...]:
    if type(value) not in (list, tuple):
        raise SourceStageError("controller source rows differ")
    if any(type(row) is not dict for row in value):
        raise SourceStageError("controller source row type differs")
    ...
    if (
        ...
        raise SourceStageError("controller source ordering differs")
    for row in rows:
        if (
...
def _manifest_value(specifications: Any) -> dict[str, Any]:
    ...
    if blocked_sources(rows):
        raise SourceStageError(
            "controller payload pins differ: "
...
def _parse_payload(
    ...
    controller_raw = captured[CONTROLLER_BASENAME]
    try:
        source = controller_raw.decode("utf-8", "strict")
        for optimize in (0, 2):
            compile(
                source, str(REMOTE_CONTROLLER_PATH), "exec",
                dont_inherit=True, optimize=optimize,
            )
    except (UnicodeError, SyntaxError, ValueError, TypeError) as error:
        raise SourceStageError("held CPU controller source differs") from error
...
def _receipt_value(
    *,
    ...
    if operation == STAGE_OPERATION:
        ...
        observation = {
            "kind": "recovered_existing_exact_controller_current_inode",
            "root_identity": list(target_identity),
            "held_inode_continuity": True,
...
def _write_file_at(
    ...
    *,
    ...
    created_anchor: tuple[int, ...] | None = None
    try:
        created = os.fstat(descriptor)
        created_anchor = _inode_anchor(created)
        creation_anchors[relative] = created_anchor
        ...
        if created_anchor is not None:
            try:
                ...
                if (
                    _inode_anchor(opened) != created_anchor
                    or _inode_anchor(named) != created_anchor
                    or _identity(opened) != _identity(named)
                ):
                    raise SourceStageError(
                        "failed shadow source replacement preserved: "
                        + relative
                    )
                ...
                try:
                    ...
                    if (
                        _inode_anchor(after_unlink) != created_anchor
                        or after_unlink.st_nlink != 0
                    ):
                        raise SourceStageError(
                            "failed shadow source unlink identity differs: "
                            + relative
                        )
                else:
                    raise SourceStageError(
                        "failed shadow source removal was not exact: "
                        + relative
                    )
                if creation_anchors.get(relative) != created_anchor:
                    raise SourceStageError(
                        "failed shadow source creation authority differs: "
                        + relative
                    )
                del creation_anchors[relative]
                os.fsync(parent_fd)
...
class HeldReceipt:
    ...
    def replay(
        ...
        value = _strict_json(first, label="controller deployment receipt")
        if (
            _identity(opened) != self.identity
...
def _capture_existing_tree_held(
    ...
    """Capture and replay the exact existing controller tree via held dirfds."""
    rows = _validate_specs(specifications)
    expected_files = {row["relative"]: row for row in rows}
    expected_dirs = set(expected_directories(rows))
...
def _open_existing_receipt(parent_fd: int, name: str) -> HeldReceipt:
    ...
    try:
        ...
        _strict_json(first, label="existing controller deployment receipt")
