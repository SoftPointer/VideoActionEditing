#!/usr/bin/env python3
"""External, stdlib-only trust bootstrap for the v15c-r5 observer package.
...
LAUNCHER_RELATIVE_PATH = (
    ...
    "auh_launch_e00_source_sam2_proposal_role_probe_v15c_r5_sealed.sh"
)
...
class BootstrapV15CR5Error(RuntimeError):
...
def _verify_manifest_semantics(manifest: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    ...
    if LAUNCHER_RELATIVE_PATH not in paths or not any(
        path.endswith("finalize_source_sam2_proposal_role_probe_v15c_r5.py")
        for path in paths
    ):
