#!/usr/bin/env python3
"""Materialize one fresh job/node-bound r5d case00 package without Slurm.

The production package has a 17-file local source closure.  Independently,
the r5d launcher authenticates 16 execution identities: eight package-local
...
r5c launcher is one of the other package-local sources because the r5d
...
TARGETS = {
    ("143811", "auh7-1b-gpu-306"): BASE
    / "bernini_full644_exploratory_matched_eval_auh_r5d_job143811_node306_"
      "case00_847b91a2_c91de7eb_85ccc17b_r1",
    ("143812", "auh7-1b-gpu-293"): BASE
    / "bernini_full644_exploratory_matched_eval_auh_r5d_job143812_node293_"
      "case00_847b91a2_c91de7eb_85ccc17b_r1",
    ("143808", "auh7-1b-gpu-315"): BASE
    / "bernini_full644_exploratory_matched_eval_auh_r5d_job143808_node315_"
      "case00_847b91a2_c91de7eb_85ccc17b_r1",
}
VACE_PYTHON = Path(
...
SELECTED_TASK_IDS = ("shared8-00-base", "shared8-00-full644")
...
class R5DMaterializationError(RuntimeError):
    """The fresh r5d package materialization contract differs."""
...
def stable_file(
    ...
    if (
        ...
        raise R5DMaterializationError(f"noncanonical stable path: {path}")
    descriptor = os.open(
        path,
        os.O_RDONLY
...
def create_file(path: Path, raw: bytes, mode: int) -> None:
    ...
    try:
        ...
        while offset < len(raw):
            ...
            if count <= 0:
                raise R5DMaterializationError("fresh file write made no progress")
            offset += count
        os.fsync(descriptor)
        before = os.fstat(descriptor)
        named = path.lstat()
        ...
        if (
            ...
            raise R5DMaterializationError("fresh file staging replay differs")
...
def ensure_ready_pins() -> None:
    for relative, digest in DIAGNOSTIC_SOURCE_PINS.items():
        if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            raise R5DMaterializationError(f"diagnostic source pin is not frozen: {relative}")
...
def launch_input(root: Path, job_id: str, node: str, plan: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "full644-exploratory-matched-root-launch-input-auh-r5d",
        ...
        "adapter": str(method_root / "full644_exploratory_matched_infer_adapter_auh_r5d.py"),
        ...
        "plan": str(plan),
        "output_report": str(root / "final/case00_canary_report_auh_r5d.json"),
        "runner_attestation": str(root / "final/case00_canary_runner_attestation_auh_r5d.json"),
        ...
        "rank_cache_root": str(root / "runtime/rank-cache"),
...
def shell_quote(value: str) -> str:
    if "\x00" in value:
        raise R5DMaterializationError("shell literal contains NUL")
...
def build_static_payload(
    *, root: Path, job_id: str, node: str, source: Path, source_sha256: str,
    ...
    lines = [
        ...
        f"readonly R5D_PYTHON={shell_quote(str(VACE_PYTHON))}",
        f"readonly R5D_SOURCE={shell_quote(str(source))}",
        'exec {R5D_PYTHON_FD}<"$R5D_PYTHON"',
        'exec {R5D_SOURCE_FD}<"$R5D_SOURCE"',
        '[[ "$R5D_PYTHON_FD" =~ ^[0-9]+$ && "$R5D_SOURCE_FD" =~ ^[0-9]+$ ]] || exit 99',
        "exec -c \"/proc/self/fd/$R5D_PYTHON_FD\" -I -S -B -c "
        + shell_quote(CAPTURED_PROBE_BOOTSTRAP)
        + ' "$R5D_PYTHON_FD" "$R5D_SOURCE_FD" '
...
def _build_plan(root: Path, release_dir: Path) -> tuple[Path, bytes, dict[str, Any]]:
    ...
    if result != 0:
        raise R5DMaterializationError("v2 compatibility builder failed")
    ...
    if (
        ...
        raise R5DMaterializationError("v2 plan replay differs")
...
def _materialize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if key not in TARGETS:
        raise R5DMaterializationError("unsupported r5d binding target")
    if os.geteuid() != 2012 or os.getegid() != 2000:
        raise R5DMaterializationError("materializer owner authority differs")
    source_root = Path(args.source_root)
    if not source_root.is_absolute() or os.path.normpath(str(source_root)) != str(source_root):
        raise R5DMaterializationError("source root path differs")
    root = TARGETS[key]
    if root.exists() or root.is_symlink():
        raise R5DMaterializationError(f"fresh r5d root exists: {root}")
    mkdir_fresh(root)
    ...
    launcher = load_module(
        "full644_exploratory_matched_spooled_launcher_auh_r5d_materializer",
        release_dir / "full644_exploratory_matched_spooled_launcher_auh_r5d.py",
        RELEASE_FILES["methods/bernini_action_editing/full644_exploratory_matched_spooled_launcher_auh_r5d.py"],
    )
    ...
    if (
        ...
        raise R5DMaterializationError("r5d production receipt replay differs")
    ...
    if json.loads(bootstrap_receipt_raw) != bootstrap_receipt:
        raise R5DMaterializationError("root bootstrap materialization replay differs")
    ...
    if set(expected_modes) != set(expected_sha256):
        raise R5DMaterializationError("package expected file closure differs")
    all_entries = list(root.rglob("*"))
    if any(
        not (path.is_file() or path.is_dir() or path.is_symlink())
        for path in all_entries
    ):
        raise R5DMaterializationError("package contains a special filesystem entry")
    actual_files = {
        str(path.relative_to(root)) for path in root.rglob("*")
        if path.is_file() or path.is_symlink()
    }
    if actual_files != set(expected_modes):
        raise R5DMaterializationError("package physical file closure differs")
    ...
    for relative in sorted(expected_modes):
        ...
        if info.st_uid != 2012 or info.st_gid != 2000:
            raise R5DMaterializationError(f"package artifact owner differs: {relative}")
        artifacts[relative] = {
    ...
    if actual_directories != expected_directories:
        raise R5DMaterializationError("package directory closure differs")
    ...
    for relative in sorted(expected_directories):
        ...
        if (
            ...
            raise R5DMaterializationError(f"package directory differs: {relative}")
    ...
    for relative in ("evidence", "outputs/media", "final", "runtime"):
        if list((root / relative).iterdir()):
            raise R5DMaterializationError(f"fresh result root is not empty: {relative}")
    if b"exact28" in plan_raw.lower() or b"auh_r5b" in plan_raw.lower() or b"auh_r5c" in plan_raw.lower():
        raise R5DMaterializationError("burned plan family entered r5d package")

    report: dict[str, Any] = {
        "schema_version": "full644-exploratory-matched-r5d-case00-materialization-v1",
...
def main(argv: Sequence[str] | None = None) -> int:
    if (
        ...
        raise R5DMaterializationError("isolated materializer startup differs")
