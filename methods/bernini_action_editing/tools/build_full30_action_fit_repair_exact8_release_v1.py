#!/usr/bin/env python3
"""Build/audit the deterministic BOX-EXP-011 fit repair exact8 release."""
...
SCHEMA_VERSION = "bernini-full30-action-fit-repair-exact8-release-v1"
...
PLAN_MEMBER = "full30_action_fit_repair_exact8_plan_v1.py"
...
FILES_AND_MODES: Mapping[str, int] = {
    **{
        path: 0o444
        for path in _RUNTIME_FILES + _PLAN_AND_CONTROL_FILES + _TOOLS
    },
    "scripts/auh_full30_action_fit_repair_exact8_136140_world4_v1.sh": 0o555,
    "scripts/auh_generic_action_data_prep_rank_exec_v1.sh": 0o555,
}
COMPONENT_FILES: Mapping[str, str] = {
    "prompt_plan_sha256": PLAN_MEMBER,
    "generator_sha256": "full30_action_fit_repair_exact8_generator_v1.py",
    "controller_sha256": "full30_action_fit_repair_exact8_controller_v1.py",
...
class FitRepairExact8ReleaseError(RuntimeError):
    """Raised before an ambiguous or mutable release can pass."""


def fail(message: str) -> NoReturn:
    raise FitRepairExact8ReleaseError(message)
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise FitRepairExact8ReleaseError(
...
def _stable_plain_bytes(path: Path) -> bytes:
    ...
    try:
        ...
        raise FitRepairExact8ReleaseError("release input is unavailable") from error
...
def _authority() -> Mapping[str, Any]:
    return {
        ...
        "purpose": "repair fit action/incomplete authoring after locked blind failure",
        ...
        "selected_cells": [
            ["arms-raised-to-both-hands-firmly-on-hips", "00435ad621c44fac", 2026080821, "sp4-a"],
            ["left-fist-to-flat-forward-palm-down", "71ba57892bd043df", 2026080824, "sp4-b"],
            ["arms-raised-to-both-hands-firmly-on-hips", "00435ad621c44fac", 2026080921, "sp4-a"],
            ["left-fist-to-flat-forward-palm-down", "71ba57892bd043df", 2026080924, "sp4-b"],
        ],
        "locked_blind_authority": {
...
def _topology() -> Mapping[str, Any]:
    return {
        ...
        "holder_choice": (
            "prefer currently free 136140 to avoid waiting behind confirmation; "
            "static release binds no live numbered child"
        ),
...
def build_manifest(
    ...
    unsigned = {
        ...
        "release_scope": "BOX-EXP-011-fit-action-incomplete-repair-exact8-only",
...
def verify_archive(raw: bytes, manifest: Mapping[str, Any]) -> None:
    try:
        ...
        raise FitRepairExact8ReleaseError("archive is invalid") from error
...
def validate_manifest(value: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        or specialization.get("repair_holder_job") != 136140
        or specialization.get("static_release_binds_live_child") is not False
