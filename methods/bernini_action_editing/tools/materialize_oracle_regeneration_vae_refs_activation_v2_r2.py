...
SCHEMA_VERSION = "bernini-oracle-regeneration-activation-v2-vae-authoring-run-r2"
WORLD_SIZE = 4
...
def _verify_inner_runtime_closure() -> Mapping[str, str]:
    observed: dict[str, str] = {}
    for relative, expected in INNER_RUNTIME_CLOSURE.items():
        requested = METHOD_ROOT / relative
        ...
        if (
            ...
            raise VaeReferenceMaterializationError(
                f"inner runtime closure differs: {relative}"
            )
        observed[relative] = expected
    return observed


def _verify_decode_import_closure(*, tools_package: Any, materialize_vae: Any) -> Mapping[str, Any]:
    expected_tools = (METHOD_ROOT / "tools").resolve(strict=True)
    tools_init = (expected_tools / "__init__.py").resolve(strict=True)
    materialize_path = (expected_tools / "materialize_vae.py").resolve(strict=True)
    builder_path = (expected_tools / "build_renderer_dataset.py").resolve(strict=True)
    if (
        Path(str(getattr(tools_package, "__file__", ""))).resolve(strict=True)
        != tools_init
        or [Path(str(item)).resolve(strict=True) for item in tools_package.__path__]
        != [expected_tools]
        or Path(str(getattr(materialize_vae, "__file__", ""))).resolve(strict=True)
        != materialize_path
        or Path(
            str(getattr(materialize_vae.raw_builder, "__file__", ""))
        ).resolve(strict=True)
        != builder_path
        or materialize_vae.raw_builder is not sys.modules.get(
            "tools.build_renderer_dataset"
        )
        or "_omnivideo2_strict_action_preview_materializer" in sys.modules
    ):
        raise VaeReferenceMaterializationError("decode import closure differs")
    return {
        "tools_init_path": str(tools_init),
        "tools_init_sha256": _sha256_file(tools_init),
        "materialize_vae_path": str(materialize_path),
        "materialize_vae_sha256": _sha256_file(materialize_path),
        "build_renderer_dataset_path": str(builder_path),
        "build_renderer_dataset_sha256": _sha256_file(builder_path),
        "tools_package_search_path": [str(expected_tools)],
...
def _write_receipt(path: Path, value: Mapping[str, Any]) -> None:
    """Create the final name directly; never replace or hard-link it.

    Lustre may expose a stale link count immediately after link+unlink.  The r2
    writer therefore uses one O_EXCL-created inode at the final name and keeps
    that descriptor through write, fchmod, fsync, and byte verification.  Any
    partial final file is deliberately preserved fail-closed: the same
    output directory cannot be silently retried or overwritten.
    """
    ...
    try:
        ...
        while offset < len(payload):
            try:
                ...
                continue
            if written <= 0:
                raise VaeReferenceMaterializationError("receipt write made no progress")
            offset += written
        os.fchmod(descriptor, 0o444)
        os.fsync(descriptor)
        created = os.fstat(descriptor)
        os.lseek(descriptor, 0, os.SEEK_SET)
        same_fd_chunks: list[bytes] = []
        while True:
            chunk = os.read(descriptor, 1024 * 1024)
            if not chunk:
                break
            same_fd_chunks.append(chunk)
        same_fd_after = os.fstat(descriptor)
        if (
            ...
            or b"".join(same_fd_chunks) != payload
            or (created.st_dev, created.st_ino, created.st_size)
            != (same_fd_after.st_dev, same_fd_after.st_ino, same_fd_after.st_size)
        ):
            raise VaeReferenceMaterializationError(
                "created receipt descriptor identity differs"
            )
        os.close(descriptor)
        descriptor = None
        ...
        if (
            ...
            or (before.st_dev, before.st_ino) != created_identity
...
def main(argv: list[str] | None = None) -> int:
    ...
    inner_runtime_closure = _verify_inner_runtime_closure()
    ...
    decode_import_closure = _verify_decode_import_closure(
    ...
    if (
        _verify_inner_runtime_closure() != inner_runtime_closure
        or _verify_decode_import_closure(
            tools_package=tools_package,
            materialize_vae=materialize_vae,
        )
        != decode_import_closure
    ):
        raise VaeReferenceMaterializationError(
            "inner/decode import closure changed during run"
        )
    ...
    if distributed.rank == 0:
        ...
        run_receipt = {
            ...
            "inner_runtime_closure": inner_runtime_closure,
            "decode_import_closure": decode_import_closure,
