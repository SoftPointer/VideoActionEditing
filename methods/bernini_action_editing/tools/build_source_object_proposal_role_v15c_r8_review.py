#!/usr/bin/env python3
"""Build a synchronized, reject-only audit page for v15c-r8 LOCAL candidates."""
...
SCHEMA = "bernini-source-object-proposal-role-overlay-v15c-r6"
MEDIA_SCHEMA = "bernini-source-object-proposal-role-media-validation-v15c-r6"
POSTFLIGHT_SCHEMA = "bernini-source-sam2-proposal-role-postflight-v15c-r8-local"
...
def _html(result: Mapping[str, Any], media: Mapping[str, Mapping[str, str]]) -> str:
    ...
    for key, title in (
        ...
        cards.append(
            ...
            f'<img loading="lazy" src="{html.escape(media[key]["contact_sheet"])}" alt="frames 0 20 40 60 80">'
            "</section>"
        )
...
def main() -> int:
    ...
    if (
        ...
        != "LOCAL_SCHEMA_POSTFLIGHT_PASS_REMOTE_OBSERVER_UNVERIFIED"
    ...
    source_copy = output / MEDIA_CONTRACT["source"]["video"]
    shutil.copy2(source, source_copy)
    media = {}
    source_sheet = output / MEDIA_CONTRACT["source"]["contact_sheet"]
    ...
    media["source"] = dict(MEDIA_CONTRACT["source"])
    all_ids = proposal_ids
    all_path = output / MEDIA_CONTRACT["all"]["video"]
    _render_overlay(
        frames=frames,
        root=track_path.parent,
    ...
    all_sheet = output / MEDIA_CONTRACT["all"]["contact_sheet"]
    _render_contact_sheet(
        frames=frames,
        root=track_path.parent,
        proposal_ids=all_ids,
    ...
    media["all"] = dict(MEDIA_CONTRACT["all"])
    for role in core.ROLE_NAMES:
        candidate = result["assignments"][role]
        role_path = output / MEDIA_CONTRACT[role]["video"]
        _render_overlay(
            frames=frames,
            root=track_path.parent,
        ...
        role_sheet = output / MEDIA_CONTRACT[role]["contact_sheet"]
        _render_contact_sheet(
            frames=frames,
            root=track_path.parent,
            proposal_ids=[] if candidate is None else [candidate],
        ...
        media[role] = dict(MEDIA_CONTRACT[role])
    media_validation = {}
