...
STEP_RECEIPT = OUTPUT_PARENT / "inallocation-step-launch-receipt.json"
CLIENT_RECEIPT = OUTPUT_PARENT / "inallocation-srun-client-receipt.json"
ADMISSION = OUTPUT_PARENT / "inallocation-step-admission.json"
...
def die(message: str) -> None:
    raise SystemExit(f"postflight-saic-fv2-fd-world8-inallocation-r11: {message}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        or accounting["JobName"] != "saic-fv2-fd-w8-inalloc-r11"
    ...
    if (
        ...
        or expected_command.count("--job-name=saic-fv2-fd-w8-inalloc-r11") != 1
    ...
    stdout_log = LOG_DIR / f"saic-fv2-fd-w8-inalloc-{job_step_id}.out"
    stderr_log = LOG_DIR / f"saic-fv2-fd-w8-inalloc-{job_step_id}.err"
    sentinel = f"SAIC_FV2_FD_WORLD8_INALLOCATION_R11_PASS {job_step_id}\n".encode(
        "ascii"
    )
    ...
    core = {
        "schema_version": (
            "saic-formal-v2-retained-fd-world8-inallocation-step-admission-v1"
        ),
