...
SCHEMA_VERSION = "bernini-oracle-regeneration-activation-v2-prompt-authoring-run-r2"
WORLD_SIZE = 4
...
def _write_receipt(path: Path, value: Mapping[str, Any]) -> None:
    """Create one final receipt inode directly with O_EXCL.

    This avoids Lustre's transient nlink visibility after link+unlink while
    retaining strict no-replace behavior.  A failed or crashed partial final
    name is preserved and therefore makes any later retry fail closed.
    """
    ...
    try:
        ...
        while offset < len(payload):
            try:
                ...
                continue
            if written <= 0:
                raise PromptMaterializationError("receipt write made no progress")
            offset += written
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if _verify_inner_runtime_closure() != inner_runtime_closure:
        raise PromptMaterializationError(
            "inner runtime closure changed during prompt materialization"
        )
