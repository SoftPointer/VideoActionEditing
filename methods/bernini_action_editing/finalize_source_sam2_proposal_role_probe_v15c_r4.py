#!/usr/bin/env python3
"""Independent release/runtime/media verifier and no-replace finalizer for v15c-r4."""
...
COMPLETE_SCHEMA = "bernini-source-object-proposal-role-v15c-r4-complete"
RELEASE_TAG = "v15c-r4"
...
class FinalizeV15CR4Error(RuntimeError):
...
def require_sha(value: Any, label: str) -> str:
    if not isinstance(value, str) or SHA_RE.fullmatch(value) is None:
        raise FinalizeV15CR4Error(f"{label} is not lowercase SHA256")
    return value


def require_exact_keys(value: Any, keys: Sequence[str] | set[str], label: str) -> None:
    if type(value) is not dict or set(value) != set(keys):
        raise FinalizeV15CR4Error(f"{label} exact keys differ")
...
def _regular(path: Path, label: str) -> os.stat_result:
    try:
        ...
        raise FinalizeV15CR4Error(f"{label} is absent") from error
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise FinalizeV15CR4Error(f"{label} is not one regular non-symlink file")
    return info
...
def file_sha256(path: Path) -> str:
    ...
    try:
        ...
        if (
            ...
            raise FinalizeV15CR4Error("file changed while hashing")
    finally:
        os.close(descriptor)
...
def read_json(path: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise FinalizeV15CR4Error("JSON input differs") from error
    if type(value) is not dict:
        raise FinalizeV15CR4Error("JSON input is not one object")
    return value
...
def verify_self_hash(value: Mapping[str, Any], field: str = "receipt_sha256") -> None:
    ...
    if claimed != object_sha256(payload):
        raise FinalizeV15CR4Error(f"{field} self-hash differs")


def _relative(value: Any, label: str) -> str:
    if not isinstance(value, str) or not value or "\\" in value:
        raise FinalizeV15CR4Error(f"{label} path differs")
...
def _verify_manifest(manifest: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    ...
    if (
        ...
        raise FinalizeV15CR4Error("release semantics differ")
    paths: list[str] = []
...
def verify_release(
    ...
    if root.is_symlink() or not root.is_dir():
        raise FinalizeV15CR4Error("release root differs")
    expected_path = root / RELEASE_RELATIVE_PATH
    if release_path.absolute() != expected_path.absolute():
        raise FinalizeV15CR4Error("release placement differs")
    if file_sha256(expected_path) != expected:
        raise FinalizeV15CR4Error("release file hash differs")
    ...
    for row in members:
        ...
        if info.st_size != row["size"] or file_sha256(path) != row["sha256"]:
            raise FinalizeV15CR4Error("release member bytes differ")
    return manifest
    ...
    for current_text, directory_names, file_names in os.walk(root, topdown=True, followlinks=False):
        ...
        for name in directory_names:
            ...
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
                raise FinalizeV15CR4Error("tree contains a symlink/non-directory")
            if name == "__pycache__":
                raise FinalizeV15CR4Error("tree contains __pycache__")
            directories.add(relative)
        for name in file_names:
            path = current / name
            info = path.lstat()
            relative = path.relative_to(root).as_posix()
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode) or name.endswith(".pyc"):
                raise FinalizeV15CR4Error("tree contains symlink/non-file/pyc")
            files.add(relative)
...
def verify_snapshot(
    ...
    if files != expected_files or directories != expected_directories:
        raise FinalizeV15CR4Error("snapshot exact tree differs")
    expected_rows = {
    ...
    for relative in sorted(expected_files):
        ...
        if (
            ...
            raise FinalizeV15CR4Error("snapshot member bytes/mode differ")
    ...
    for relative in expected_directories:
        ...
        if stat.S_IMODE(path.lstat().st_mode) != 0o500:
            raise FinalizeV15CR4Error("snapshot directory mode differs")
    return observed
...
def verify_bootstrap_receipt(
    ...
    *,
    ...
    if receipt_path.absolute() != run_root / "external_bootstrap_receipt.json":
        raise FinalizeV15CR4Error("bootstrap receipt placement differs")
    ...
    require_exact_keys(python, {"path", "sha256", "size", "startup_flags"}, "Python")
    require_exact_keys(
        release,
    ...
    try:
        ...
        raise FinalizeV15CR4Error("external bootstrap placement differs") from error
    ...
    if (
        ...
        raise FinalizeV15CR4Error("bootstrap receipt semantics differ")
    return receipt


def verify_inputs(paths: Mapping[str, Path]) -> Mapping[str, Mapping[str, Any]]:
    if set(paths) != set(EXPECTED_INPUT_HASHES):
        raise FinalizeV15CR4Error("input registry differs")
    ...
    for label in EXPECTED_INPUT_HASHES:
        ...
        if digest != EXPECTED_INPUT_HASHES[label]:
            raise FinalizeV15CR4Error(f"{label} pin differs")
...
def verify_runtime(
    *,
    ...
    if (
        ...
        raise FinalizeV15CR4Error("runtime placement/mode differs")
    ...
    if require_live_authority and (
        ...
        or os.environ.get("V15C_R4_EXTERNAL_BOOTSTRAP") != "1"
    ):
        raise FinalizeV15CR4Error("live execution authority differs")
...
def _probe_video(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise FinalizeV15CR4Error("cv2 is unavailable for independent media replay") from error
    capture = cv2.VideoCapture(str(path))
    if not capture.isOpened():
        raise FinalizeV15CR4Error("video reopen failed")
    ...
    try:
        while True:
            ...
            if getattr(frame, "shape", None) is None or tuple(frame.shape[:2]) != (1056, 704):
                raise FinalizeV15CR4Error("decoded video frame geometry differs")
            frame_count += 1
    finally:
        capture.release()
    if frame_count != 81 or abs(fps - 25.0) > 1.0e-6 or width != 704 or height != 1056:
        raise FinalizeV15CR4Error("decoded video contract differs")
...
def _probe_sheet(path: Path) -> None:
    try:
        ...
        raise FinalizeV15CR4Error("cv2 is unavailable for contact-sheet replay") from error
    image = cv2.imread(str(path), cv2.IMREAD_COLOR)
    if getattr(image, "shape", None) is None or tuple(image.shape) != (528, 1760, 3):
        raise FinalizeV15CR4Error("contact-sheet reopen differs")
...
def verify_review_bundle(run_root: Path, *, probe_media: bool = True) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise FinalizeV15CR4Error("overlay semantics differ")
    ...
    if observed_files != expected_actual_files:
        raise FinalizeV15CR4Error("review exact file tree differs")
    for relative in sorted(EXPECTED_REVIEW_FILES):
        ...
        if (
            ...
            raise FinalizeV15CR4Error("overlay listed bytes differ")
    ...
    if (
        ...
        raise FinalizeV15CR4Error("overlay input binding differs")
    ...
    if (
        ...
        raise FinalizeV15CR4Error("media validation closure differs")
    ...
    for key in VIDEO_KEYS:
        ...
        if (
            ...
            raise FinalizeV15CR4Error("media video receipt differs")
        if probe_media:
            replay = _probe_video(video_path)
            if replay != {key_name: row[key_name] for key_name in ("frame_count", "fps", "width", "height")}:
                raise FinalizeV15CR4Error("media video independent replay differs")
    ...
    try:
        ...
        raise FinalizeV15CR4Error("review index reopen differs") from error
    if (
        ...
        raise FinalizeV15CR4Error("review index authority differs")
    return {
...
def verify_observer_outputs(run_root: Path) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise FinalizeV15CR4Error("observer output authority differs")
...
def publish_noreplace(destination: Path, payload: bytes) -> None:
    ...
    if destination.exists() or destination.is_symlink() or temporary.exists() or temporary.is_symlink():
        raise FinalizeV15CR4Error("COMPLETE destination is not fresh")
    descriptor = os.open(
        temporary,
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise FinalizeV15CR4Error("COMPLETE short write")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
    ...
    if destination.read_bytes() != payload:
        raise FinalizeV15CR4Error("COMPLETE reopen differs")
...
def write_complete(args: argparse.Namespace) -> None:
    ...
    if (
        ...
        raise FinalizeV15CR4Error("final execution authority differs")
    ...
    if _output_manifest(run_root) != outputs_before:
        raise FinalizeV15CR4Error("outputs changed before COMPLETE publication")
    destination = run_root / "COMPLETE.manifest.json"
    publish_noreplace(destination, payload)
    if _output_manifest(run_root) != outputs_before:
        raise FinalizeV15CR4Error("outputs changed during COMPLETE publication")
...
def verify_complete(run_root: Path) -> None:
    ...
    if (
        ...
        raise FinalizeV15CR4Error("COMPLETE authority differs")
    require_exact_keys(
        execution,
    ...
    if execution != {
        ...
        raise FinalizeV15CR4Error("COMPLETE execution differs")
    ...
    if (
        ...
        raise FinalizeV15CR4Error("COMPLETE byte replay differs")
    ...
    if (
        ...
        raise FinalizeV15CR4Error("COMPLETE bootstrap binding differs")
    input_paths = {key: Path(row["path"]) for key, row in complete["inputs"].items()}
    if complete["inputs"] != verify_inputs(input_paths):
        raise FinalizeV15CR4Error("COMPLETE input replay differs")
    if complete["observer_outputs"] != verify_observer_outputs(run_root):
        raise FinalizeV15CR4Error("COMPLETE observer output replay differs")
    if complete["review"] != verify_review_bundle(run_root, probe_media=True):
        raise FinalizeV15CR4Error("COMPLETE review replay differs")
...
def main() -> int:
    ...
    if args.command == "verify-release":
        ...
        raise FinalizeV15CR4Error("unknown command")
    return 0
