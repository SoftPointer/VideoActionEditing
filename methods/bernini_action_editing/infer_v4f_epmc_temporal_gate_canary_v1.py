#!/usr/bin/env python3
"""Render the future-use v4-F decoded-residual EPMC video canary.
...
creating output until the completed v4-F authority and gate materializer are
...
The upstream v4-F gate covers only transform families exposed during model
...
RECEIPT_SCHEMA = "bernini-v4f-epmc-temporal-gate-video-canary-v1"
RECEIPT_STATUS = "V4F_EPMC_TEMPORAL_GATE_VIDEO_CANARY_COMPLETE_DIAGNOSTIC_ONLY"
PIN_PLACEHOLDER = "TO_BE_PINNED"
RELEASE_SEALED = False

# These are intentionally unresolved.  They may be filled only after the v4-F
...
class V4FEPMCVideoCanaryError(RuntimeError):
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise V4FEPMCVideoCanaryError("value is not canonical JSON") from error
...
def _required_sha256(value: Any, *, label: str) -> str:
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise V4FEPMCVideoCanaryError(f"{label} must be a lowercase SHA-256")
    return value
...
def _require_release_sealed() -> Any:
    ...
    if (
        ...
        raise V4FEPMCVideoCanaryError(
            "UNSEALED v4-F EPMC video canary: gate authorities are TO_BE_PINNED"
        )

    # Deliberately deferred until the local release surface is fully pinned.
    from methods.bernini_action_editing import (
        materialize_v4f_epmc_gate_state_v1 as gate_materializer,
    )
    ...
    if (
        ...
        raise V4FEPMCVideoCanaryError(
            "sealed v4-F gate materializer source/schema binding differs"
        )
...
def _runtime_modules() -> tuple[Any, Any, Any, Any, Any]:
    ...
    if (
        ...
        raise V4FEPMCVideoCanaryError("inherited v4-B canary core source differs")
...
def _plain_gate_state(path_value: str | Path) -> Path:
    ...
    if not path.is_absolute():
        raise V4FEPMCVideoCanaryError("gate-state must be an absolute path")
    try:
        info = path.lstat()
    except OSError as error:
        raise V4FEPMCVideoCanaryError("cannot stat gate-state") from error
    if (
...
def _reject_duplicate_keys(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V4FEPMCVideoCanaryError(f"duplicate JSON key: {key!r}")
        result[key] = value
    return result


def _reject_nonfinite(value: str) -> None:
    raise V4FEPMCVideoCanaryError(f"non-finite JSON number: {value}")
...
class GateStateBundle:
    ...
    def audit_receipt(self) -> dict[str, Any]:
        ...
        return {
            ...
            "fold1_selected_rho": self.payload["fold_checkpoint"]["selected_rho"],
...
def _positive_zero(value: Any, *, label: str) -> None:
    ...
    if int(torch.count_nonzero(flat).item()) != 0:
        raise V4FEPMCVideoCanaryError(f"{label} must be exact zero")
    if int(torch.count_nonzero(flat.view(torch.uint8)).item()) != 0:
        raise V4FEPMCVideoCanaryError(f"{label} must be byte-exact positive zero")
...
def _code_from_json(value: Mapping[str, Any], *, name: str) -> Any:
    ...
    if not isinstance(value, Mapping):
        raise V4FEPMCVideoCanaryError(f"{name} gate payload must be an object")
    try:
        ...
        raise V4FEPMCVideoCanaryError(f"{name} gate tensors differ") from error
    phase = phase.contiguous()
    block = block.contiguous()
    try:
        code = epmc.MotionCode(phase, block)
    except epmc.PrivilegedMotionCodeContractError as error:
        raise V4FEPMCVideoCanaryError(str(error)) from error
    if (
        ...
        raise V4FEPMCVideoCanaryError(f"{name} gate tensor digest differs")
...
def load_gate_state(
    ...
    if expected_sha256 != EXPECTED_GATE_STATE_FILE_SHA256:
        raise V4FEPMCVideoCanaryError("gate-state CLI SHA differs from release pin")
    ...
    try:
        ...
        with os.fdopen(descriptor, "rb") as handle:
            ...
            if (
                ...
                raise V4FEPMCVideoCanaryError("gate-state inode seal differs")
    ...
    except V4FEPMCVideoCanaryError:
        raise
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V4FEPMCVideoCanaryError("gate-state is not strict ASCII JSON") from error
    finally:
        if descriptor is not None:
            os.close(descriptor)
    if type(payload) is not dict:
        raise V4FEPMCVideoCanaryError("gate-state must contain one object")
    ...
    v4f_receipt = payload.get("v4f_receipt")
    fold_receipt = payload.get("fold_receipt")
    fold_preselection = payload.get("fold_preselection_checkpoint")
    ...
    if (
        ...
        or payload.get("v4f_aggregate_gate_verified_true") is not True
        or payload.get("v4f_exact5_all_fold_inner_gates_passed") is not True
        or not isinstance(implementation, Mapping)
        or implementation.get("gate_materializer_sha256")
        != EXPECTED_GATE_MATERIALIZER_SHA256
        or implementation.get("implementation_sha256")
        != gate_materializer.EXPECTED_V4F_IMPLEMENTATION_SHA256
        or not isinstance(v4f_receipt, Mapping)
        or v4f_receipt.get("file_sha256")
        != gate_materializer.EXPECTED_V4F_RECEIPT_FILE_SHA256
        or v4f_receipt.get("receipt_digest")
        != gate_materializer.EXPECTED_V4F_RECEIPT_SELF_DIGEST
        or v4f_receipt.get("exposed_five_view_codec_development_gate") is not True
        or v4f_receipt.get("known_exposed_transform_families_only") is not True
        or v4f_receipt.get("unseen_hostile_transform_gate") is not False
        or v4f_receipt.get("unseen_hostile_transform_gate_evaluated") is not False
        or v4f_receipt.get("exact5_all_fold_inner_gates_passed") is not True
        ...
        or not isinstance(fold_preselection, Mapping)
        or fold_preselection.get("file_sha256")
        != gate_materializer.EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256
        or fold_preselection.get("metadata_digest")
        != gate_materializer.EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_METADATA_DIGEST
        or fold_preselection.get("model_state_sha256")
        != gate_materializer.EXPECTED_FOLD1_PRESELECTION_MODEL_STATE_SHA256
        or fold_preselection.get("preselection_base_state_sha256")
        != gate_materializer.EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
        or fold_preselection.get("checkpoint_role")
        != "preselection_fixed_step1200"
        or fold_preselection.get("selected_step") != 1200
        or fold_preselection.get("deployment_rho") != 1.0
        or fold_preselection.get("aggregate_artifact_join_exact") is not True
        or fold_preselection.get("fold_receipt_artifact_join_exact") is not True
        or fold_preselection.get("selected_checkpoint_base_state_join_exact")
        is not True
        ...
        or fold_checkpoint.get("checkpoint_role") != "selected_fold_local_rho"
        or fold_checkpoint.get("selected_step") != 1200
        or fold_checkpoint.get("selected_rho")
        != gate_materializer.EXPECTED_FOLD1_SELECTED_RHO
        or fold_checkpoint.get("deployment_rho")
        != gate_materializer.EXPECTED_FOLD1_SELECTED_RHO
        or fold_checkpoint.get("preselection_checkpoint_file_sha256")
        != gate_materializer.EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256
        or fold_checkpoint.get("preselection_base_state_sha256")
        != gate_materializer.EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
        or fold_checkpoint.get("selected_rho_state_buffer_verified") is not True
        or fold_checkpoint.get("selected_preselection_base_state_join_exact")
        is not True
        ...
        or decoded_contract.get("exact_trainable_parameter_count") != 79040
        or decoded_contract.get("selected_fold_local_rho")
        != gate_materializer.EXPECTED_FOLD1_SELECTED_RHO
        or decoded_contract.get(
            "selected_rho_scales_encoder_delta_and_decoder_residual"
        ) is not True
        or decoded_contract.get("trained_base_excludes_only_residual_gate_rho")
        is not True
        ...
        raise V4FEPMCVideoCanaryError(
            "gate-state does not carry the closed v4-F privileged diagnostic scope"
        )
    ...
    try:
        ...
        raise V4FEPMCVideoCanaryError("temporal calibration tensors differ") from error
    ...
    for name, indices in (
        ...
        if not torch.equal(codes[name].phase_gates, expected):
            raise V4FEPMCVideoCanaryError(f"{name} is not the frozen phase permutation")
        actual = torch.sort(codes[name].phase_gates[:, 1:], dim=1).values
        if not torch.equal(reference, actual):
            raise V4FEPMCVideoCanaryError(f"{name} changed the phase multiset")
    ...
    if int(torch.count_nonzero(codes["correct"].phase_gates[:, 1:]).item()) == 0:
        raise V4FEPMCVideoCanaryError("correct temporal gate degenerated to all zero")
...
def build_video_receipt(
    *,
    ...
    gate_preselection = gate_bundle.payload.get("fold_preselection_checkpoint")
    ...
    if (
        ...
        or gate_bundle.payload.get("v4f_aggregate_gate_verified_true") is not True
        or gate_bundle.payload.get("v4f_exact5_all_fold_inner_gates_passed")
        is not True
        ...
        or gate_checkpoint.get("checkpoint_role") != "selected_fold_local_rho"
        or gate_checkpoint.get("selected_rho_state_buffer_verified") is not True
        or gate_checkpoint.get("selected_preselection_base_state_join_exact")
        is not True
        or not isinstance(gate_preselection, Mapping)
        or gate_preselection.get("checkpoint_role")
        != "preselection_fixed_step1200"
        or gate_preselection.get("selected_checkpoint_base_state_join_exact")
        is not True
        or gate_checkpoint.get("preselection_base_state_sha256")
        != gate_preselection.get("preselection_base_state_sha256")
        or gate_checkpoint.get("preselection_checkpoint_file_sha256")
        != gate_preselection.get("file_sha256")
        or not isinstance(gate_fold_receipt, Mapping)
        or gate_fold_receipt.get("aggregate_binding_exact") is not True
    ):
        raise V4FEPMCVideoCanaryError("video canary runtime closure differs")
    payload: dict[str, Any] = {
        "schema_version": RECEIPT_SCHEMA,
        "status": RECEIPT_STATUS,
        "method": "v4f-global-codec-decoded-residual-epmc-temporal-head-gating",
        ...
        "v4f_aggregate_gate_verified_true": True,
        "v4f_exact5_all_fold_inner_gates_passed": True,
        "fold1_selected_rho": gate_checkpoint["selected_rho"],
        "source": {
        ...
        "verified_claims": {
            "v4f_aggregate_gate_true_before_render": True,
            "v4f_exact5_all_fold_inner_gates_passed_before_render": True,
            "aggregate_fold_receipt_preselection_selected_checkpoint_strong_join_before_render": True,
    ...
    if not all(payload["verified_claims"].values()):
        raise V4FEPMCVideoCanaryError("one or more video canary invariants failed")
...
def _runner_argv(args: argparse.Namespace) -> list[str]:
    ...
    return [value.replace("v4b_epmc", "v4f_epmc") for value in values]
