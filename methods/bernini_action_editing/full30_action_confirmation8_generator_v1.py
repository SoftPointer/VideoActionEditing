#!/usr/bin/env python3
"""Generate and audit BOX-EXP-009 confirmation action-anchor exact8 media.

The formal lane is exactly four confirmation seed cells times the ordered
``action``/``incomplete`` pair.  The optional diagnostic lane is physically
separate and cannot satisfy, replace, or block the formal exact8 closure.
...
r10 tensor/MP4 parity are inherited from the frozen fit-r13 resource contract.
This module accepts only its release-built 136309 specialization; the fit-r13
preimage is never modified.
"""
...
import full30_action_confirmation8_plan_v1 as plan_contract  # noqa: E402
...
RESOURCE_SPECIALIZED_BASENAME = (
    "reserve4_fixed_generation_sp4_136309_specialized_v1.py"
)
RESOURCE_SPECIALIZED_SIZE = 166_064
HOLDER_JOB = "136309"
HOLDER_NODE = "auh7-1b-gpu-280"
SHARD_SCHEMA = "bernini-full30-action-confirmation8-shard-v1"
AUDIT_SCHEMA = "bernini-full30-action-confirmation8-audit-v1"
GAP_SCHEMA = "bernini-full30-action-confirmation8-gap-v1"
...
def load_resource_contract(value: str | Path) -> ModuleType:
    """Load only the release-built 136309 postimage of frozen fit-r13."""

    path = plain_file(value, "136309 resource contract")
    raw = path.read_bytes()
    require(
        ...
        and raw.count(b"136309") == 7,
        "136309 resource specialization identity differs",
    )
    module_name = "_bernini_full30_confirmation8_resource_daa6cb2c"
...
def _lane_tasks(plan: Mapping[str, Any], lane: str) -> list[Mapping[str, Any]]:
    require(lane in {"admission", "diagnostic"}, "generation lane differs")
    key = "admission_tasks" if lane == "admission" else "diagnostic_tasks"
    tasks = plan.get(key)
    require(isinstance(tasks, list), f"{lane} task lane is absent")
    expected_count = 8 if lane == "admission" else 32
    require(len(tasks) == expected_count, f"{lane} task count differs")
    return tasks
...
def run_shard(args: argparse.Namespace) -> int:
    ...
    lane_tasks = _lane_tasks(plan, args.lane)
    matching = [
        task
    ...
    expected_count = 2 if args.lane == "admission" else 8
    expected_branches = (
        plan_contract.ADMISSION_BRANCH_ORDER
        if args.lane == "admission"
        else plan_contract.DIAGNOSTIC_BRANCH_ORDER
    )
    require(
        len(matching) == expected_count
        and [task["semantic_branch"] for task in matching]
        == list(expected_branches),
        f"sealed {args.lane} shard scope differs",
    )
    ...
    for task in matching:
        ...
        try:
            ...
            environment = resource._candidate_environment(
                ...
                scratch=scratch,
                cache_token=(
                    f"confirmation8-{args.lane}-{args.seed_slot}-"
    ...
    if args.lane == "admission":
        ...
        unsigned["counts_toward_confirmation_exact8"] = False
        unsigned["diagnostic_generation_is_admission_prerequisite"] = False
        unsigned["diagnostic_generation_can_enter_q_a_min_or_optimizer"] = False
    receipt = {**unsigned, "receipt_digest": object_sha256(unsigned)}
    write_create_only(
        output / f"confirmation8-{args.lane}-shard-receipt-v1.json", receipt
    )
    return 0
...
def audit_exact8(
    *,
    ...
    diagnostic_ids = {
        task["candidate_id"] for task in plan["diagnostic_tasks"]
    }
    ...
    diagnostic_leak = sorted(set(observed_paths) & diagnostic_ids)
    ...
    gap_unsigned = {
        ...
        "diagnostic_candidate_leak_ids": diagnostic_leak,
    ...
    unsigned = {
        ...
        "optimizer_created": False,
        "optimizer_authorized": False,
        "optional_diagnostic_generation_observed_or_required": False,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--lane", choices=("admission", "diagnostic"), required=True)
