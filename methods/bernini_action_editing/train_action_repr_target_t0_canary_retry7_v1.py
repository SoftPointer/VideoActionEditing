...
SCHEMA_VERSION = "bernini-action-repr-target-t0-one-step-retry7-receipt-v1"
STEP_SCHEMA_VERSION = "bernini-action-repr-target-t0-retry7-adapter-state-receipt-v1"
METHOD = "bernini-action-repr-target-t0-one-step-retry7-canary-v1"
ATTEMPT_CLAIM_SCHEMA_VERSION = (
    "bernini-action-repr-target-t0-retry7-preoptimizer-attempt-claim-v1"
)
ATTEMPT_CLAIM_MARKER_NAME = ".single_update.retry7.attempt_claim.json"
EXPECTED_CANONICAL_OUTPUT_PATH = (
    "/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/"
    "VideoEdit_experiments/action_repr_target_selfgen_middle_g1_20260824_v2/"
    "stage_b_t0_retry7/target_t0/0be6494dfac3/single_update"
)
EXPECTED_ATTEMPT_CLAIM_PATH = (
    "/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/"
    "VideoEdit_experiments/action_repr_target_selfgen_middle_g1_20260824_v2/"
    "stage_b_t0_retry7/target_t0/0be6494dfac3/"
    ".single_update.retry7.attempt_claim.json"
)
...
G1_REPLAY_THREAD_CONTRACT = {
    "torch_intraop_threads": 1,
    "torch_interop_threads": 1,
    "strict_legacy_diagnostics_bitwise_replay": True,
    "must_apply_before_g1_authority_replay": True,
}
RUNTIME_EXECUTION_CONTRACT = {
    "torch_num_threads_exact": 1,
    "torch_num_interop_threads_exact": 1,
    "omp_num_threads_exact": 1,
    "mkl_num_threads_exact": 1,
    "openblas_num_threads_exact": 1,
    "numexpr_num_threads_exact": 1,
    "strict_legacy_replay_required": True,
    "strict_legacy_replay_before_claim": True,
    "complete_transitive_g1_verifier_hash_pins_required": True,
}
_STRICT_THREAD_ENVIRONMENT = {
    "OMP_NUM_THREADS": "1",
    "MKL_NUM_THREADS": "1",
...
def lock_strict_g1_replay_runtime(
    *, environ: Optional[Mapping[str, str]] = None
) -> Mapping[str, Any]:
    """Pin legacy G1 reductions before any authority replay or mutable action.

    The immutable v2.7 flow receipts were produced with one PyTorch CPU
    reduction thread and intentionally replay their floating diagnostics
    bit-for-bit.  PyTorch parallel reductions can differ by one final binary64
    bit, so retry7 makes the producer execution state an explicit prerequisite
    instead of weakening receipt equality.
    """

    environment = os.environ if environ is None else environ
    for name, expected in _STRICT_THREAD_ENVIRONMENT.items():
        if environment.get(name) != expected:
            fail(f"retry7 strict G1 replay requires {name}=1")
    ...
    try:
        ...
        raise TargetT0CanaryError(
            "retry7 could not pin PyTorch threads before strict G1 replay"
        ) from error
    intraop = int(torch.get_num_threads())
    interop = int(torch.get_num_interop_threads())
    if intraop != 1 or interop != 1:
        fail("retry7 strict G1 replay PyTorch thread state differs")
    return {
        ...
        "strict_legacy_replay": True,
        "applied_before_authority": True,
    }
...
def _canonical_attempt_output(
    output: Path | str, *, require_fresh: bool = True
) -> Path:
    candidate = Path(output).expanduser().absolute()
    if candidate.is_symlink():
        fail("retry7 attempt output may not be a symlink")
    if require_fresh and candidate.exists():
        fail("retry7 attempt output must be fresh before the permanent claim")
    try:
        parent = candidate.parent.resolve(strict=True)
    except (FileNotFoundError, OSError) as error:
        raise TargetT0CanaryError(
            "retry7 attempt-claim parent must already be one real directory"
        ) from error
    if not parent.is_dir() or parent.is_symlink() or candidate.name in {"", ".", ".."}:
        fail("retry7 attempt-claim output path is not canonical and safe")
    return parent / candidate.name


def _expected_preoptimizer_attempt_claim(
    *,
    output: Path | str,
    marker_path: Path | str,
    authority_sha256: str,
    source_hash_pins_digest: str,
    slurm_visible_devices: Mapping[str, Any],
    world_size: int,
    ...
    env = os.environ if environ is None else environ
    g2a_world4.require_sha256(authority_sha256, label="retry7 claim authority")
    g2a_world4.require_sha256(
        source_hash_pins_digest, label="retry7 claim source hash pins"
    )
    mapping = dict(validate_slurm_visible_device_receipt(slurm_visible_devices))
    ...
    if (
        ...
        fail("retry7 claim requires the numeric Slurm job id")
    if world_size != 4 or sequence_parallel_size != 4:
        fail("retry7 claim requires WORLD4/SP4")
    canonical_output = _canonical_attempt_output(
        output, require_fresh=require_fresh_output
    )
    marker = Path(marker_path).expanduser().absolute()
    if (
        marker != canonical_output.parent / ATTEMPT_CLAIM_MARKER_NAME
        or marker.name != ATTEMPT_CLAIM_MARKER_NAME
    ):
        fail("retry7 authority marker path/output binding differs")
    row: dict[str, Any] = {
        "schema_version": ATTEMPT_CLAIM_SCHEMA_VERSION,
        "authorization_addendum_sha256": authority_sha256,
        "source_hash_pins_digest": source_hash_pins_digest,
        "case_id": FIXED_CASE_ID,
        "canonical_output_path": str(canonical_output),
        "canonical_marker_path": str(marker),
        "slurm_job_id": job_id,
        "world_size": world_size,
        "sequence_parallel_size": sequence_parallel_size,
        "slurm_rocr_visible_devices": mapping,
    }
    row["claim_digest"] = object_sha256(row)
    return row


def validate_preoptimizer_attempt_claim_object(value: Any) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        fail("retry7 preoptimizer attempt claim must be an object")
    row = dict(value)
    declared = row.pop("claim_digest", None)
    g2a_world4.require_sha256(declared, label="retry7 attempt claim object")
    if object_sha256(row) != declared:
        fail("retry7 preoptimizer attempt claim object digest differs")
    if set(row) != {
        "schema_version",
        "authorization_addendum_sha256",
        "source_hash_pins_digest",
        "case_id",
        ...
        fail("retry7 preoptimizer attempt claim field closure differs")
    output = row.get("canonical_output_path")
    marker = row.get("canonical_marker_path")
    job_id = row.get("slurm_job_id")
    if (
        row.get("schema_version") != ATTEMPT_CLAIM_SCHEMA_VERSION
        or row.get("case_id") != FIXED_CASE_ID
        or row.get("world_size") != 4
        or row.get("sequence_parallel_size") != 4
        or not isinstance(output, str)
        or not Path(output).is_absolute()
        or Path(output).name in {"", ".", ".."}
        or not isinstance(marker, str)
        or not Path(marker).is_absolute()
        or Path(marker) != Path(output).parent / ATTEMPT_CLAIM_MARKER_NAME
        or not isinstance(job_id, str)
        or not job_id
        or not job_id.isascii()
        or not job_id.isdigit()
    ):
        fail("retry7 preoptimizer attempt claim identity differs")
    g2a_world4.require_sha256(
        row.get("authorization_addendum_sha256"), label="retry7 claim authority"
    )
    g2a_world4.require_sha256(
        row.get("source_hash_pins_digest"), label="retry7 claim source pins"
    )
    validate_slurm_visible_device_receipt(row.get("slurm_rocr_visible_devices"))
    return value


def replay_preoptimizer_attempt_claim(
    marker: Path | str,
    *,
    expected_claim: Optional[Mapping[str, Any]] = None,
) -> Mapping[str, Any]:
    path = Path(marker).expanduser().absolute()
    if path.name != ATTEMPT_CLAIM_MARKER_NAME:
        fail("retry7 attempt claim marker name differs")
    try:
        before = path.lstat()
    except (FileNotFoundError, OSError) as error:
        raise TargetT0CanaryError("retry7 attempt claim is missing") from error
    if (
        ...
        fail("retry7 attempt claim type, mode, or link count differs")
    nofollow = getattr(os, "O_NOFOLLOW", None)
    if nofollow is None:
        fail("retry7 attempt claim requires O_NOFOLLOW support")
    descriptor = os.open(path, os.O_RDONLY | nofollow)
    try:
        opened = os.fstat(descriptor)
        if (opened.st_dev, opened.st_ino) != (before.st_dev, before.st_ino):
            fail("retry7 attempt claim changed while opening")
        chunks = []
        total = 0
        while True:
            ...
            total += len(chunk)
            if total > 65536:
                fail("retry7 attempt claim is unexpectedly large")
            chunks.append(chunk)
    finally:
        os.close(descriptor)
    ...
    try:
        ...
        raise TargetT0CanaryError(
            "retry7 attempt claim is partial or not canonical JSON"
        ) from error
    if canonical_json_bytes(value) != payload:
        fail("retry7 attempt claim bytes are not canonical JSON")
    validate_preoptimizer_attempt_claim_object(value)
    if expected_claim is not None and canonical_json_bytes(value) != canonical_json_bytes(
        expected_claim
    ):
        fail("retry7 attempt claim does not match this authorized execution")
    return {
        "path": str(path),
        "file_sha256": hashlib.sha256(payload).hexdigest(),
        "object_digest": value["claim_digest"],
        "claim": value,
    }


def create_preoptimizer_attempt_claim(
    *,
    output: Path | str,
    marker_path: Path | str,
    authority_sha256: str,
    ...
    expected = _expected_preoptimizer_attempt_claim(
        output=output,
        marker_path=marker_path,
        authority_sha256=authority_sha256,
        source_hash_pins_digest=source_hash_pins_digest,
        slurm_visible_devices=slurm_visible_devices,
        world_size=world_size,
        sequence_parallel_size=sequence_parallel_size,
        environ=environ,
    )
    marker = Path(expected["canonical_marker_path"])
    nofollow = getattr(os, "O_NOFOLLOW", None)
    if nofollow is None:
        fail("retry7 attempt claim requires O_NOFOLLOW support")
    flags = os.O_CREAT | os.O_EXCL | os.O_WRONLY | nofollow
    canonical_output = Path(expected["canonical_output_path"])
    if canonical_output.exists() or canonical_output.is_symlink():
        fail("retry7 output ceased to be fresh immediately before atomic claim")
    try:
        descriptor = os.open(marker, flags, 0o600)
    except FileExistsError as error:
        raise TargetT0CanaryError(
            "retry7 permanent attempt claim already exists; use retry8"
        ) from error
    except OSError as error:
        raise TargetT0CanaryError(
            "retry7 permanent attempt claim could not be created"
        ) from error
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                fail("retry7 permanent attempt claim write made no progress")
            offset += written
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
    directory_flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0)
    directory_descriptor = os.open(marker.parent, directory_flags)
    try:
        os.fsync(directory_descriptor)
    finally:
        os.close(directory_descriptor)
    return replay_preoptimizer_attempt_claim(marker, expected_claim=expected)


def acquire_preoptimizer_attempt_claim_world4(
    *,
    output: Path | str,
    marker_path: Path | str,
    authority_sha256: str,
    ...
    if rank == 0:
        try:
            evidence = create_preoptimizer_attempt_claim(
                output=output,
                marker_path=marker_path,
                authority_sha256=authority_sha256,
                source_hash_pins_digest=source_hash_pins_digest,
    ...
    if not isinstance(result, Mapping) or set(result) != {
        ...
        fail("retry7 rank0 attempt-claim broadcast closure differs")
    if result.get("ok") is not True:
        fail(f"retry7 rank0 attempt claim failed permanently: {result.get('error')}")
    evidence = replay_preoptimizer_attempt_claim(marker, expected_claim=expected)
    if (
        evidence["file_sha256"] != result.get("file_sha256")
        or evidence["object_digest"] != result.get("object_digest")
    ):
        fail("retry7 attempt claim differs from the rank0 broadcast")
    return evidence


def validate_preoptimizer_attempt_claim_evidence(
    value: Any,
    *,
    expected_output: Optional[Path | str] = None,
) -> Mapping[str, Any]:
    if not isinstance(value, Mapping) or set(value) != {
        "path",
        "file_sha256",
        ...
        fail("retry7 attempt-claim evidence closure differs")
    path = value.get("path")
    claim = value.get("claim")
    if not isinstance(path, str) or not Path(path).is_absolute():
        fail("retry7 attempt-claim evidence path is not absolute")
    if Path(path).name != ATTEMPT_CLAIM_MARKER_NAME:
        fail("retry7 attempt-claim evidence marker name differs")
    g2a_world4.require_sha256(
        value.get("file_sha256"), label="retry7 attempt claim file"
    )
    g2a_world4.require_sha256(
        value.get("object_digest"), label="retry7 attempt claim object"
    )
    validate_preoptimizer_attempt_claim_object(claim)
    if value.get("object_digest") != claim.get("claim_digest"):
        fail("retry7 attempt-claim evidence object digest binding differs")
    claim_output = Path(claim["canonical_output_path"])
    if (
        Path(path) != Path(claim["canonical_marker_path"])
        or Path(path) != claim_output.parent / ATTEMPT_CLAIM_MARKER_NAME
    ):
        fail("retry7 attempt-claim evidence path/output binding differs")
    if expected_output is not None:
        try:
            output = Path(expected_output).expanduser().resolve(strict=True)
        except (FileNotFoundError, OSError) as error:
            raise TargetT0CanaryError(
                "published retry7 output is unavailable for claim replay"
            ) from error
        if output != claim_output:
            fail("published retry7 output differs from its permanent attempt claim")
    return value
...
class PreoptimizerAuthority:
    ...
    g1_replay_runtime: Optional[Mapping[str, Any]] = None
...
EXPECTED_SOURCE_PIN_PATHS = frozenset(
    {
        ...
        "methods/bernini_action_editing/scripts/auh_stage_b_t0_single_update_20260824_retry7.sh",
        "methods/bernini_action_editing/tests/test_train_action_repr_target_t0_canary_retry7_v1.py",
        "tests/test_auh_stage_b_t0_single_update_20260824_retry7.py",
    }
)


def _validate_retry7_source_hash_pins(value: Any) -> str:
    if not isinstance(value, Mapping) or set(value) != EXPECTED_SOURCE_PIN_PATHS:
    ...
    for relative, expected_sha in value.items():
        ...
        if not path.is_file() or path.is_symlink():
            fail(f"Stage-B source pin is not one regular file: {relative}")
        if file_sha256(path.resolve()) != expected_sha:
            fail(f"Stage-B source hash pin differs: {relative}")
    return object_sha256(dict(value))


def prevalidate_authorization_replay_contract(value: Any) -> Mapping[str, Any]:
    """Authenticate retry7 replay code/runtime before loading G1 siblings."""
    ...
    if (
        ...
        != "action_repr_target_selfgen_middle_g1_20260824_v2"
        or row.get("document_role")
        != "create_once_stage_b_target_t0_single_update_retry7_authority"
        or not isinstance(activation, Mapping)
        or activation.get("state") != "ACTIVE_CREATE_ONCE_AUTHORITY"
        or activation.get("create_once") is not True
        or activation.get("all_placeholders_replaced") is not True
        or activation.get("template_itself_authorizes_optimizer_creation") is not False
        or row.get("g1_replay_thread_contract") != G1_REPLAY_THREAD_CONTRACT
        or row.get("runtime_execution_contract") != RUNTIME_EXECUTION_CONTRACT
    ):
        fail("retry7 pre-G1 replay authority contract differs")
    pins_digest = _validate_retry7_source_hash_pins(row.get("source_hash_pins"))
    if row.get("source_hash_pins_digest") != pins_digest:
        fail("Stage-B source hash pins canonical digest differs")
    return row
...
def validate_authorization_addendum(
    ...
    *,
    ...
    source_hash_pins_digest = _validate_retry7_source_hash_pins(pins)
    if row.get("source_hash_pins_digest") != source_hash_pins_digest:
        fail("Stage-B source hash pins canonical digest differs")
    ...
    if (
        ...
        or distributed.get("rank0_atomic_preoptimizer_attempt_claim") is not True
        or distributed.get("rank0_claim_outcome_broadcast_before_peer_replay") is not True
        or distributed.get("all_ranks_replay_attempt_claim_from_disk") is not True
        or distributed.get("claim_creation_before_cache_model_adapter_optimizer") is not True
        ...
        or runtime_paths.get("fresh_source_root_name") != "source_stage_b_t0_retry7"
        or runtime_paths.get("fresh_stage_root_name") != "stage_b_t0_retry7"
        or runtime_paths.get("fresh_log_root_name") != "logs/stage_b_t0_retry7"
        ...
        or output_contract.get("atomic_preoptimizer_attempt_claim_required") is not True
        or output_contract.get("attempt_claim_marker_name") != ATTEMPT_CLAIM_MARKER_NAME
        or output_contract.get("canonical_output_path") != EXPECTED_CANONICAL_OUTPUT_PATH
        or output_contract.get("attempt_claim_marker_path") != EXPECTED_ATTEMPT_CLAIM_PATH
        or str(Path(output).expanduser().absolute()) != EXPECTED_CANONICAL_OUTPUT_PATH
        or output_contract.get("attempt_claim_permanent_on_failure") is not True
        or output_contract.get("existing_claim_requires_new_revision") is not True
        or output_contract.get("next_revision_after_claim_failure") != "retry8"
        or output_contract.get("claim_path_file_sha_and_object_digest_receipted") is not True
...
def authorize_preoptimizer_inputs(
    *,
    ...
    checkpoint: Path | str,
    output: Path | str,
    prelocked_g1_replay_runtime: Optional[Mapping[str, Any]] = None,
) -> PreoptimizerAuthority:
    """Close every data/receipt gate.  This function cannot create an optimizer."""

    g1_replay_runtime = lock_strict_g1_replay_runtime()
    if (
        prelocked_g1_replay_runtime is not None
        and canonical_json_bytes(prelocked_g1_replay_runtime)
        != canonical_json_bytes(g1_replay_runtime)
    ):
        fail("retry7 prelocked G1 replay runtime changed before authorization")
    output_path = Path(output).expanduser().absolute()
    if str(output_path) != EXPECTED_CANONICAL_OUTPUT_PATH:
        fail("T0 output CLI path differs from the exact retry7 authority")
    ...
    # This precedes all dynamically loaded G1 verifier siblings.  In
    # particular, flow/middle replay code cannot execute unless its exact
    # transitive source closure and one-thread legacy runtime are authorized.
    prevalidate_authorization_replay_contract(authorization)
    ...
    return PreoptimizerAuthority(
        ...
        g1_replay_runtime=g1_replay_runtime,
    )
    ...
    *,
    ...
    for step, state in ((0, result.step0_state), (1, result.step1_state)):
        ...
        payload = save_safetensors(
            dict(state),
            metadata={
                "schema_version": "bernini-action-repr-target-t0-retry7-adapter-state-v1",
    ...
    receipt: dict[str, Any] = {
            ...
            "upstream_authority": {
                ...
                "source_hash_pins_digest": authority.authorization[
                    "source_hash_pins_digest"
                ],
...
def validate_t0_receipt(value: Any) -> Mapping[str, Any]:
    ...
    g1_replay_runtime = (
        runtime.get("g1_replay_runtime") if isinstance(runtime, Mapping) else None
    )
    if g1_replay_runtime != {
        "torch_num_threads": 1,
        ...
        fail("T0 strict legacy G1 replay runtime receipt differs")
    attempt_claim = (
        runtime.get("preoptimizer_attempt_claim")
        if isinstance(runtime, Mapping)
        else None
    )
    validate_preoptimizer_attempt_claim_evidence(attempt_claim)
    claim = attempt_claim["claim"]
    if (
        claim["authorization_addendum_sha256"]
        != authority["authorization_addendum_sha256"]
        or claim["source_hash_pins_digest"] != authority["source_hash_pins_digest"]
        or claim["case_id"] != row.get("case_id")
        or claim["world_size"] != runtime.get("world_size")
        or claim["sequence_parallel_size"] != runtime.get("ulysses_size")
        or canonical_json_bytes(claim["slurm_rocr_visible_devices"])
        != canonical_json_bytes(slurm_rocr)
    ):
        fail("retry7 attempt claim/receipt authority or runtime binding differs")
    if (
...
def validate_published_t0_output(output: Path | str) -> Mapping[str, Any]:
    ...
    claim_evidence = validate_preoptimizer_attempt_claim_evidence(
        receipt["runtime"]["preoptimizer_attempt_claim"],
        expected_output=root,
    )
    replayed_claim = replay_preoptimizer_attempt_claim(
        claim_evidence["path"], expected_claim=claim_evidence["claim"]
    )
    if canonical_json_bytes(replayed_claim) != canonical_json_bytes(claim_evidence):
        fail("published retry7 attempt claim file/evidence binding differs")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    authorization = authorize_preoptimizer_inputs(
        ...
        output=args.output,
        prelocked_g1_replay_runtime=g1_replay_runtime,
    )
    ...
    attempt_claim = acquire_preoptimizer_attempt_claim_world4(
        output=args.output,
        marker_path=authorization.authorization["output_contract"][
            "attempt_claim_marker_path"
        ],
        authority_sha256=authorization.authorization_sha256,
        source_hash_pins_digest=authorization.authorization[
            "source_hash_pins_digest"
        ],
        slurm_visible_devices=slurm_visible_devices,
        world_size=contract.world_size,
        sequence_parallel_size=contract.ulysses_size,
        rank=contract.rank,
        broadcast_object_list=dist.broadcast_object_list,
    )
    _consensus(attempt_claim, label="permanent retry7 preoptimizer attempt claim")
    ...
    runtime = {
        ...
        "backend": backend,
        "slurm_rocr_visible_devices": slurm_visible_devices,
        "preoptimizer_attempt_claim": attempt_claim,
        "g1_replay_runtime": dict(authorization.g1_replay_runtime),
    ...
    source_lock = {
        ...
        Path(g2a_world4._g1_admission_module().__file__).name: file_sha256(
            Path(g2a_world4._g1_admission_module().__file__).resolve()
        ),
        Path(g2a_world4._g1_evaluator_module().__file__).name: file_sha256(
            Path(g2a_world4._g1_evaluator_module().__file__).resolve()
        ),
        Path(g2a_world4._flow_cohort_module().__file__).name: file_sha256(
            Path(g2a_world4._flow_cohort_module().__file__).resolve()
        ),
        Path(g2a_world4._middle_cohort_module().__file__).name: file_sha256(
            Path(g2a_world4._middle_cohort_module().__file__).resolve()
        ),
...
__all__ = [
    ...
    "acquire_preoptimizer_attempt_claim_world4",
    "create_preoptimizer_attempt_claim",
    "load_fixed_fit_case",
    "lock_strict_g1_replay_runtime",
    "main",
    "publish_create_only_result",
    "prevalidate_authorization_replay_contract",
    "reject_forbidden_media_fields",
    "replay_preoptimizer_attempt_claim",
    "resolve_fixed_projection",
    "run_one_step_optimizer_canary",
    "validate_authorization_addendum",
    "validate_published_t0_output",
    "validate_preoptimizer_attempt_claim_evidence",
    "validate_preoptimizer_attempt_claim_object",
    "validate_receipt",
