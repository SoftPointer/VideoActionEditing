#!/usr/bin/env python3
"""Generate and audit the BOX-EXP-011 fit action/incomplete repair exact8 media.

Exactly four comparator cells are rendered.  Each cell contains the ordered
``action``/``incomplete`` pair and must reuse one official Gaussian tensor.
There is no diagnostic lane or diagnostic CLI entrypoint.
"""
...
SHARD_SCHEMA = "bernini-full30-action-fit-repair-exact8-shard-v1"
AUDIT_SCHEMA = "bernini-full30-action-fit-repair-exact8-audit-v1"
GAP_SCHEMA = "bernini-full30-action-fit-repair-exact8-gap-v1"
...
class FitRepairExact8GenerationError(RuntimeError):
    """Raised before partial or widened fit_repair output can pass."""


def fail(message: str) -> NoReturn:
    raise FitRepairExact8GenerationError(message)
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise FitRepairExact8GenerationError(
...
def plain_file(value: str | Path, label: str) -> Path:
    ...
    try:
        ...
        raise FitRepairExact8GenerationError(f"{label} is unavailable") from error
    require(
        resolved == path
...
def load_resource_contract(value: str | Path) -> ModuleType:
    """Load only the release-built 136140 postimage of frozen fit-r13."""

    path = plain_file(value, "136140 resource contract")
    raw = path.read_bytes()
    require(
        ...
        "136140 resource specialization identity differs",
    )
    module_name = "_bernini_full30_fit_repair_resource_aa2f5c01"
...
def _runtime_binding(
    ...
    try:
        ...
        raise FitRepairExact8GenerationError(str(error)) from error
    require(
...
def validate_resource_smoke(
    ...
    require(
        ...
        "fit-r13 physical safe_open/r10-r13 smoke binding differs",
    )
    return receipt
...
def _gaussian_pair_proof(
    ...
    require(
        len(tasks) == len(receipts) == 2
        and [task["semantic_branch"] for task in tasks]
        == list(plan_contract.ADMISSION_BRANCH_ORDER),
        "comparator cell is not one ordered action/incomplete pair",
    )
    gaussians = [
...
def run_shard(args: argparse.Namespace) -> int:
    ...
    matching = [
        task for task in plan["admission_tasks"] if task["group_id"] == args.group_id
    ]
    require(
        len(matching) == 4
        and [task["semantic_branch"] for task in matching]
        == ["action", "incomplete", "action", "incomplete"],
        "sealed fit_repair exact4 shard scope differs",
    )
    ...
    receipts_by_cell: dict[str, list[tuple[Mapping[str, Any], Mapping[str, Any]]]] = {}
    for task in matching:
        ...
        try:
            ...
            environment = resource._candidate_environment(
                ...
                scratch=scratch,
                cache_token=(
                    f"fit-repair-exact8-{args.group_id}-"
                    f"{task['calibration_group_id']}-{task['semantic_branch']}"
                ),
            )
            ...
            if isinstance(error, FitRepairExact8GenerationError):
                raise
            raise FitRepairExact8GenerationError(str(error)) from error
        ...
        receipts_by_cell.setdefault(task["calibration_group_id"], []).append(
            (task, receipt)
        )
        receipt_rows.append(
            {
    ...
    gaussian_proofs = [
        ...
        for rows in receipts_by_cell.values()
    ]
    require(len(gaussian_proofs) == 2, "exact4 shard cell closure differs")
    unsigned = {
        ...
        "resource_compile_smoke": {
            ...
            "physical_safe_open_and_r10_r13_parity_passed": True,
        },
        "counts_toward_fit_repair_exact8": True,
    ...
    write_create_only(output / "fit-repair-exact8-shard-receipt-v1.json", receipt)
    return 0
...
def audit_exact8(
    *, plan_path: str | Path, expected_plan_sha256: str,
    ...
    require(len(generation_roots) == 2, "fit repair exact8 requires exactly two roots")
    ...
    receipt_rows: list[dict[str, Any]] = []
    by_cell: dict[str, list[tuple[Mapping[str, Any], Mapping[str, Any]]]] = {}
    for task in expected_tasks:
        receipt_path = observed_paths[task["candidate_id"]]
        receipt = _validate_candidate_receipt(resource, task, receipt_path)
        by_cell.setdefault(task["calibration_group_id"], []).append((task, receipt))
        receipt_rows.append(
            {
                "candidate_id": task["candidate_id"],
                "calibration_group_id": task["calibration_group_id"],
    ...
    require(len(gaussian_proofs) == 4, "formal exact8 comparator-cell closure differs")
    unsigned = {
