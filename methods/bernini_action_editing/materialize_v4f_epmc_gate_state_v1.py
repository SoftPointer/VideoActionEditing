#!/usr/bin/env python3
"""Materialize one fail-closed v4-F decoded-residual EPMC gate state.

This future-use bridge is intentionally unsealed.  Until the v4-F aggregate,
fold-1 receipt, implementation, preselection checkpoint, selected checkpoint,
selected rho, and trained-base placeholders are all
...
qualification.  The v4-F gate covers only the five transform families exposed
...
SCHEMA = "semantic-anchor-v4f-epmc-temporal-gate-state-v1"
STATUS = "V4F_EPMC_TEMPORAL_GATE_STATE_COMPLETE_DIAGNOSTIC_ONLY"
PIN_PLACEHOLDER = "TO_BE_PINNED"
RELEASE_SEALED = False

# These values deliberately prevent use until the completed v4-F result and
# its exact selected fold-1 checkpoint have been independently audited.
EXPECTED_V4F_RECEIPT_SCHEMA = PIN_PLACEHOLDER
EXPECTED_V4F_RECEIPT_STATUS = PIN_PLACEHOLDER
EXPECTED_V4F_FOLD_RECEIPT_SCHEMA = PIN_PLACEHOLDER
EXPECTED_V4F_FOLD_RECEIPT_STATUS = PIN_PLACEHOLDER
EXPECTED_V4F_CHECKPOINT_SCHEMA = PIN_PLACEHOLDER
EXPECTED_V4F_IMPLEMENTATION_SHA256 = PIN_PLACEHOLDER
EXPECTED_V4F_RECEIPT_FILE_SHA256 = PIN_PLACEHOLDER
EXPECTED_V4F_RECEIPT_SELF_DIGEST = PIN_PLACEHOLDER
EXPECTED_FOLD1_RECEIPT_SHA256 = PIN_PLACEHOLDER
EXPECTED_FOLD1_RECEIPT_SELF_DIGEST = PIN_PLACEHOLDER
EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256 = PIN_PLACEHOLDER
EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_METADATA_DIGEST = PIN_PLACEHOLDER
EXPECTED_FOLD1_PRESELECTION_MODEL_STATE_SHA256 = PIN_PLACEHOLDER
EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256 = PIN_PLACEHOLDER
...
EXPECTED_FOLD1_SELECTED_RHO: Any = PIN_PLACEHOLDER
...
class V4FEPMCGateStateError(RuntimeError):
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise V4FEPMCGateStateError("value is not canonical JSON") from error
...
def _tensor_sha256(value: torch.Tensor) -> str:
    ...
    if not isinstance(value, torch.Tensor) or value.device.type == "meta":
        raise V4FEPMCGateStateError("digest input must be a non-meta tensor")
...
def _required_sha256(value: Any, *, label: str) -> str:
    if type(value) is not str or _SHA256.fullmatch(value) is None:
        raise V4FEPMCGateStateError(f"{label} must be a lowercase SHA-256")
    return value


def _release_pin_values() -> dict[str, Any]:
    return {
        "v4f_receipt_schema": EXPECTED_V4F_RECEIPT_SCHEMA,
        "v4f_receipt_status": EXPECTED_V4F_RECEIPT_STATUS,
        "v4f_fold_receipt_schema": EXPECTED_V4F_FOLD_RECEIPT_SCHEMA,
        "v4f_fold_receipt_status": EXPECTED_V4F_FOLD_RECEIPT_STATUS,
        "v4f_checkpoint_schema": EXPECTED_V4F_CHECKPOINT_SCHEMA,
        "v4f_implementation_sha256": EXPECTED_V4F_IMPLEMENTATION_SHA256,
        "v4f_receipt_file_sha256": EXPECTED_V4F_RECEIPT_FILE_SHA256,
        "v4f_receipt_self_digest": EXPECTED_V4F_RECEIPT_SELF_DIGEST,
        "fold1_receipt_sha256": EXPECTED_FOLD1_RECEIPT_SHA256,
        "fold1_receipt_self_digest": EXPECTED_FOLD1_RECEIPT_SELF_DIGEST,
        "fold1_preselection_checkpoint_sha256": (
            EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256
        ),
        "fold1_preselection_checkpoint_metadata_digest": (
            EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_METADATA_DIGEST
        ),
        "fold1_preselection_model_state_sha256": (
            EXPECTED_FOLD1_PRESELECTION_MODEL_STATE_SHA256
        ),
        "fold1_preselection_base_state_sha256": (
        ...
        "fold1_selected_rho": EXPECTED_FOLD1_SELECTED_RHO,
    }


def _require_release_sealed() -> None:
    """Fail before parsing paths, importing v4-F, or creating outputs."""

    values = _release_pin_values()
    sha_names = {
        "v4f_implementation_sha256",
        "v4f_receipt_file_sha256",
        "v4f_receipt_self_digest",
    ...
    if (
        ...
        or type(EXPECTED_FOLD1_SELECTED_RHO) is not float
        or EXPECTED_FOLD1_SELECTED_RHO not in RHO_GRID
    ):
        raise V4FEPMCGateStateError(
            "UNSEALED v4-F EPMC bridge: all v4-F pins are TO_BE_PINNED"
        )
...
def _plain_absolute_file(
    ...
    if not path.is_absolute():
        raise V4FEPMCGateStateError(f"{label} must be an absolute path")
    try:
        info = path.lstat()
    except OSError as error:
        raise V4FEPMCGateStateError(f"cannot stat {label}") from error
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise V4FEPMCGateStateError(f"{label} must be a plain regular file")
    if not 0 < info.st_size <= maximum_bytes:
        raise V4FEPMCGateStateError(f"{label} size is outside its frozen bound")
    if info.st_nlink != 1 or stat.S_IMODE(info.st_mode) != 0o444:
        raise V4FEPMCGateStateError(f"{label} must be mode0444/nlink1")
...
def _reject_duplicate_keys(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V4FEPMCGateStateError(f"duplicate JSON key: {key!r}")
        result[key] = value
    return result


def _reject_nonfinite(value: str) -> None:
    raise V4FEPMCGateStateError(f"non-finite JSON number: {value}")
...
def _strict_json(
    ...
    *,
    ...
    try:
        ...
        with os.fdopen(descriptor, "rb") as handle:
            ...
            if (
                ...
                raise V4FEPMCGateStateError(f"{label} inode seal differs")
    ...
    except V4FEPMCGateStateError:
        raise
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V4FEPMCGateStateError(f"{label} is not strict ASCII JSON") from error
    finally:
        if descriptor is not None:
            os.close(descriptor)
    if type(value) is not dict:
        raise V4FEPMCGateStateError(f"{label} must contain one JSON object")
    return value
...
def _verify_self_digest(value: Mapping[str, Any], *, label: str) -> str:
    ...
    if _object_sha256(unsigned) != digest:
        raise V4FEPMCGateStateError(f"{label} self-digest differs")
    return digest
...
def _positive_zero(value: torch.Tensor, *, label: str) -> None:
    ...
    if int(torch.count_nonzero(flat).item()) != 0:
        raise V4FEPMCGateStateError(f"{label} must be exact zero")
...
def _current_source_binding(v4f: Any, v4c: Any) -> dict[str, str]:
    paths = {
        "implementation": Path(v4f.__file__).resolve(strict=True),
    ...
    if (
        result["implementation_sha256"] != EXPECTED_V4F_IMPLEMENTATION_SHA256
        ...
        raise V4FEPMCGateStateError("v4-F bridge source binding differs")
    return result
...
def _ordered_iids(
    ...
    if (
        ...
        raise V4FEPMCGateStateError(f"{label} ordered IID closure differs")
    return list(value)
...
def _fold_one_contract(receipt: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise V4FEPMCGateStateError("v4-F receipt lacks exact-five folds")
    ...
    if len(matches) != 1 or not isinstance(matches[0], Mapping):
        raise V4FEPMCGateStateError("v4-F fold-1 receipt is not unique")
    fold = matches[0]
    ...
    if not isinstance(training, Mapping):
        raise V4FEPMCGateStateError("fold-1 training audit is absent")
    ...
    if (
        ...
        raise V4FEPMCGateStateError("fold-1 fit/inner/OOF partition differs")
    return fold
...
def _scope_is_fail_closed(scope: Any, *, aggregate: bool) -> bool:
    ...
    return (
        ...
        and scope.get("inner_fold_local_gate_passed") is True
        and scope.get("aggregate_gate_evaluated") is False
    )


def validate_v4f_receipt_gate(
    receipt: Mapping[str, Any], *, expected_feature_receipt_sha256: str
) -> tuple[Mapping[str, Any], Mapping[str, Any], Mapping[str, Any], Mapping[str, Any]]:
    """Return fold-1 selected/preselection/fold records after exact5 PASS."""

    digest = _verify_self_digest(receipt, label="v4-F receipt")
    ...
    preselected = receipt.get("preselection_fold_checkpoint_artifacts")
    ...
    if (
        receipt.get("schema_version") != EXPECTED_V4F_RECEIPT_SCHEMA
        or receipt.get("status") != EXPECTED_V4F_RECEIPT_STATUS
        or digest != EXPECTED_V4F_RECEIPT_SELF_DIGEST
        ...
        or comparator.get("v4e_burned_oof_informed_v4f_homotopy_choice")
        is not True
        or comparator.get("one_predeclared_fold_local_selection_algorithm")
        is not True
        or comparator.get("rho_candidate_count") != len(RHO_GRID)
        or comparator.get("single_candidate") is not False
        ...
        or training_contract.get("fixed_preselection_step") != FIXED_SELECTED_STEP
        or training_contract.get("checkpoint_winner_selection_performed") is not False
        or training_contract.get("training_rho") != TRAINING_RHO
        or training_contract.get("rho_grid") != list(RHO_GRID)
        or training_contract.get("rho0_exact_comparator_only_not_selectable")
        is not True
        or training_contract.get("rho_candidate_count") != len(RHO_GRID)
        or training_contract.get("single_candidate") is not False
        or training_contract.get("each_outer_fold_selected_rho_independently")
        is not True
        or training_contract.get("cross_fold_inner_aggregation_or_global_rho_selection")
        is not False
        or training_contract.get("oof_selection") is not False
        or type(training_contract.get("selected_steps_by_fold")) is not list
        or training_contract.get("selected_steps_by_fold")
        != [FIXED_SELECTED_STEP] * 5
        or type(training_contract.get("selected_rhos_by_fold")) is not list
        or len(training_contract.get("selected_rhos_by_fold")) != 5
        or any(
            type(rho) is not float or rho not in RHO_GRID
            for rho in training_contract.get("selected_rhos_by_fold")
        )
        ...
        or not isinstance(preselected, Mapping)
        or preselected.get("count") != 5
        or preselected.get("all_reverified_by_cpu_aggregate") is not True
        or preselected.get(
            "all_selected_artifacts_join_same_fold_preselection_base_state"
        ) is not True
        or type(folds) is not list
        or len(folds) != 5
        or [fold.get("fold_index") for fold in folds if isinstance(fold, Mapping)]
        != list(range(5))
        or any(
            not isinstance(fold, Mapping)
            or fold.get("fold_status") != EXPECTED_V4F_FOLD_RECEIPT_STATUS
            or fold.get("rho_selection", {}).get("inner_pass") is not True
            or fold.get("selected_rho") not in RHO_GRID
            or fold.get("rho_selection", {}).get("selected_rho")
            != fold.get("selected_rho")
            or fold.get("training", {}).get("selected_step")
            != FIXED_SELECTED_STEP
            or fold.get("selected_checkpoint_artifact") is None
            or fold.get("preselection_checkpoint_artifact") is None
            for fold in folds
        )
        ...
        raise V4FEPMCGateStateError(
            "v4-F aggregate exposed-five-view gate is not a closed true gate"
        )
    ...
    if (
        ...
        raise V4FEPMCGateStateError("v4-F OOF evidence is not exact644")
    selected_rho_by_fold = {
        int(fold["fold_index"]): float(fold["selected_rho"])
        for fold in folds
    }
    if any(
        row.get("residual_gate_rho")
        != selected_rho_by_fold.get(row.get("outer_fold"))
        or row.get("rho0_exact_clip_pca_alias_used") is not False
        for row in evidence
    ):
        raise V4FEPMCGateStateError(
            "v4-F exact644 evidence does not bind each fold-selected rho"
        )
    ...
    if (
        ...
        raise V4FEPMCGateStateError("preregistered IID is not fold-1 OOF")
    ...
    if (
        ...
        raise V4FEPMCGateStateError("aggregate fold-1 OOF evidence join differs")
    ...
    if (
        ...
        raise V4FEPMCGateStateError("v4-F selected checkpoint manifest differs")
    for artifact in artifacts:
        if (
            ...
            or artifact.get("caller_model_reloaded_from_sealed_artifact_before_next_stage")
            is not True
            ...
            or artifact.get("checkpoint_role") != "selected_fold_local_rho"
            or artifact.get("selected_step") != FIXED_SELECTED_STEP
            or artifact.get("deployment_rho") not in RHO_GRID
            or artifact.get("preselection_checkpoint_file_sha256") is None
        ):
            raise V4FEPMCGateStateError("v4-F checkpoint artifact audit differs")
    fold1 = [row for row in artifacts if row.get("outer_fold") == EXPECTED_OUTER_FOLD]
    if len(fold1) != 1:
        raise V4FEPMCGateStateError("v4-F fold-1 checkpoint join is not unique")
    preselection_artifacts = preselected.get("artifacts")
    pair_joins = preselected.get("distinct_checkpoint_pair_joins")
    if (
        type(preselection_artifacts) is not list
        or len(preselection_artifacts) != 5
        or preselected.get("artifacts_manifest_sha256")
        != _object_sha256(preselection_artifacts)
        or type(pair_joins) is not list
        or len(pair_joins) != 5
        or pair_joins
        != [fold.get("preselection_selected_checkpoint_pair_join") for fold in folds]
    ):
        raise V4FEPMCGateStateError("v4-F preselection manifest differs")
    for artifact in preselection_artifacts:
        if (
            not isinstance(artifact, Mapping)
            or artifact.get("mode_octal") != "0444"
            or artifact.get("nlink") != 1
            or artifact.get("checkpoint_role") != "preselection_fixed_step1200"
            or artifact.get("selected_step") != FIXED_SELECTED_STEP
            or artifact.get("deployment_rho") != TRAINING_RHO
            or artifact.get("preselection_checkpoint_file_sha256") is not None
            or artifact.get("selected_training_audit_state_join_verified") is not True
            or artifact.get("fresh_reload_strict_state_verified") is not True
            or artifact.get("fresh_reload_output_bit_exact") is not True
            or artifact.get("caller_model_reloaded_from_sealed_artifact_before_next_stage")
            ...
            raise V4FEPMCGateStateError(
                "v4-F preselection checkpoint artifact audit differs"
            )
    fold1_preselection = [
        row for row in preselection_artifacts
        if row.get("outer_fold") == EXPECTED_OUTER_FOLD
    ]
    if (
        len(fold1_preselection) != 1
        or fold1[0].get("deployment_rho") != EXPECTED_FOLD1_SELECTED_RHO
        or fold1[0].get("preselection_checkpoint_file_sha256")
        != fold1_preselection[0].get("file_sha256")
        or fold1[0].get("preselection_base_state_sha256")
        != fold1_preselection[0].get("preselection_base_state_sha256")
        or fold1[0].get("preselection_base_state_sha256")
        != EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
    ):
        raise V4FEPMCGateStateError(
            "v4-F fold-1 selected/preselection/rho join differs"
        )
    ...
    if (
        ...
        raise V4FEPMCGateStateError("v4-F fold receipt bindings differ")
    fold1_bindings = [
    ...
    if (
        ...
        raise V4FEPMCGateStateError("v4-F fold-1 receipt strong binding differs")
    return (
        dict(fold1[0]),
        dict(fold1_preselection[0]),
        dict(fold_contract),
        dict(fold1_bindings[0]),
    )


def validate_v4f_fold_receipt(
    receipt: Mapping[str, Any], *, aggregate_fold: Mapping[str, Any],
    aggregate_artifact: Mapping[str, Any],
    aggregate_preselection_artifact: Mapping[str, Any],
    aggregate_binding: Mapping[str, Any], receipt_path: Path,
    preselection_path: Path, checkpoint_path: Path,
) -> None:
    """Join fold-1 receipt and both checkpoint roles to the aggregate."""

    digest = _verify_self_digest(receipt, label="v4-F fold-1 receipt")
    implementation = receipt.get("implementation")
    fold = receipt.get("fold")
    artifact = fold.get("selected_checkpoint_artifact") if isinstance(fold, Mapping) else None
    preselection = (
        fold.get("preselection_checkpoint_artifact")
        if isinstance(fold, Mapping) else None
    )
    rho_selection = fold.get("rho_selection") if isinstance(fold, Mapping) else None
    ...
    if (
        receipt.get("schema_version") != EXPECTED_V4F_FOLD_RECEIPT_SCHEMA
        or receipt.get("status") != EXPECTED_V4F_FOLD_RECEIPT_STATUS
        ...
        or preselection != aggregate_preselection_artifact
        or not isinstance(rho_selection, Mapping)
        or rho_selection.get("inner_pass") is not True
        or rho_selection.get("rho_candidate_count") != len(RHO_GRID)
        or rho_selection.get("rho0_selectable") is not False
        or rho_selection.get("selected_rho") != EXPECTED_FOLD1_SELECTED_RHO
        or fold.get("selected_rho") != EXPECTED_FOLD1_SELECTED_RHO
        or fold.get("fold_status") != EXPECTED_V4F_FOLD_RECEIPT_STATUS
        or fold.get("training", {}).get("selected_step") != FIXED_SELECTED_STEP
        or fold.get("preselection_selected_checkpoint_pair_join") is None
        ...
        or str(preselection_path) != preselection.get("path")
        or checkpoint_path == preselection_path
        or (
            checkpoint_path.stat().st_dev,
            checkpoint_path.stat().st_ino,
        ) == (
            preselection_path.stat().st_dev,
            preselection_path.stat().st_ino,
        )
        or artifact.get("checkpoint_role") != "selected_fold_local_rho"
        or preselection.get("checkpoint_role") != "preselection_fixed_step1200"
        or artifact.get("deployment_rho") != EXPECTED_FOLD1_SELECTED_RHO
        or preselection.get("deployment_rho") != TRAINING_RHO
        or artifact.get("preselection_checkpoint_file_sha256")
        != preselection.get("file_sha256")
        or artifact.get("preselection_base_state_sha256")
        != preselection.get("preselection_base_state_sha256")
        or artifact.get("preselection_base_state_sha256")
        != EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
    ):
        raise V4FEPMCGateStateError(
            "fold-1 receipt/aggregate/preselection/selected join differs"
        )
    ...
    if (
        ...
        or any(
            row.get("residual_gate_rho") != EXPECTED_FOLD1_SELECTED_RHO
            or row.get("rho0_exact_clip_pca_alias_used") is not False
            for row in evidence
        )
        ...
        raise V4FEPMCGateStateError("fold-1 OOF evidence closure differs")
...
def _load_checkpoint(
    ...
    *,
    ...
    expected_preselection_file_sha256: Optional[str],
    ...
    try:
        ...
        with os.fdopen(descriptor, "rb") as handle:
            ...
            if (
                ...
                or artifact.get("deployment_rho") != expected_rho
                or artifact.get("preselection_base_state_sha256")
                != expected_base_state_sha256
                or artifact.get("preselection_checkpoint_file_sha256")
                != expected_preselection_file_sha256
            ):
                raise V4FEPMCGateStateError(
            ...
            if (
                actual_sha != expected_file_sha256
    ...
    if type(loaded) is not dict or set(loaded) != {"metadata", "state_dict"}:
        raise V4FEPMCGateStateError("fold-1 checkpoint envelope differs")
    metadata, state = loaded["metadata"], loaded["state_dict"]
    if not isinstance(metadata, Mapping) or type(state) is not dict:
        raise V4FEPMCGateStateError("fold-1 checkpoint payload types differ")
    ...
    metadata_implementation = metadata.get("implementation")
    if (
        _object_sha256(unsigned) != declared_digest
        or declared_digest != expected_metadata_digest
        or metadata.get("schema_version") != EXPECTED_V4F_CHECKPOINT_SCHEMA
        or metadata.get("outer_fold") != EXPECTED_OUTER_FOLD
        or metadata.get("checkpoint_role") != expected_role
        or metadata.get("selected_step") != FIXED_SELECTED_STEP
        or metadata.get("deployment_rho") != expected_rho
        or metadata.get("deployment_rho_fp32_exact_power_of_two") is not True
        or metadata.get("preselection_base_state_sha256")
        != expected_base_state_sha256
        or not isinstance(metadata_implementation, Mapping)
        or metadata_implementation.get("implementation_sha256")
        != EXPECTED_V4F_IMPLEMENTATION_SHA256
        ...
        or metadata.get("model_state_sha256") != v4f._state_sha(state)
        ...
        or artifact.get("preselection_checkpoint_file_sha256")
        != expected_preselection_file_sha256
        or artifact.get("preselection_checkpoint_binding_sha256")
        != metadata.get("preselection_checkpoint_binding_sha256")
        or (
            expected_role == "preselection_fixed_step1200"
            and metadata.get("preselection_checkpoint_binding") is not None
        )
        or (
            expected_role == "preselection_fixed_step1200"
            and metadata.get("preselection_checkpoint_binding_sha256") is not None
        )
        or (
            expected_role == "selected_fold_local_rho"
            and (
                not isinstance(metadata.get("preselection_checkpoint_binding"), Mapping)
                or metadata.get("preselection_checkpoint_binding", {}).get("file_sha256")
                != expected_preselection_file_sha256
                or metadata.get("preselection_checkpoint_binding", {}).get(
                    "preselection_base_state_sha256"
                ) != expected_base_state_sha256
                or metadata.get("preselection_checkpoint_binding_sha256")
                != _object_sha256(metadata.get("preselection_checkpoint_binding"))
            )
        )
    ):
        raise V4FEPMCGateStateError("fold-1 checkpoint metadata/state join differs")
...
def _model_from_state(
    ...
    *,
    ...
    _load_tensor_runtime()
    required = {"clip_mean", "clip_basis", "fit_only_rms", "residual_gate_rho"}
    if not required.issubset(state):
        raise V4FEPMCGateStateError("fold-1 state lacks frozen analytic buffers")
    basis = metadata.get("basis")
    if not isinstance(basis, Mapping) or (
        basis.get("clip_mean_sha256") != v4f._tensor_sha(state["clip_mean"])
        or basis.get("clip_basis_sha256")
        != v4f._tensor_sha(state["clip_basis"])
        or basis.get("fit_only_global_rms_sha256")
        != v4f._tensor_sha(state["fit_only_rms"])
        ...
        raise V4FEPMCGateStateError("checkpoint analytic-basis hashes differ")
    fitted = v4f.ClipPCAFit(
    ...
    model = v4f.VJepa2GlobalCodec(
        fitted, state["fit_only_rms"].detach().clone()
    )
    if sum(parameter.numel() for parameter in model.parameters()) != 79040:
        raise V4FEPMCGateStateError("v4-F exact parameter closure differs")
    model.load_state_dict(state, strict=True)
    model.eval().requires_grad_(False)
    if (
        v4f._state_sha(model.state_dict()) != metadata.get("model_state_sha256")
        or v4f._base_state_sha(model.state_dict())
        != EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
    ):
        raise V4FEPMCGateStateError("strictly reloaded model state digest differs")
    return model
...
def decoded_residual(
    ...
    if (
        ...
        raise V4FEPMCGateStateError("decoded-residual input geometry differs")
    with torch.no_grad():
        decoded = model(centered_values)
    if tuple(decoded.shape) != tuple(centered_values.shape):
        raise V4FEPMCGateStateError("decoded codec output geometry differs")
    residual = (decoded - zero_decode).contiguous()
    if not bool(torch.isfinite(residual).all()):
        raise V4FEPMCGateStateError("decoded residual contains NaN or infinity")
    return residual
...
def residual_rms_profile(residual: torch.Tensor) -> torch.Tensor:
    ...
    if residual.ndim != 3 or tuple(residual.shape[1:]) != (
        ...
        raise V4FEPMCGateStateError("decoded residual must be [N,32,1024]")
    profile = residual.square().mean(dim=2).sqrt().contiguous()
    if not bool(torch.isfinite(profile).all()):
        raise V4FEPMCGateStateError("decoded-residual RMS is non-finite")
    return profile
...
def scaled_profile_32_to_20(
    ...
    if held_rms.shape[0] != 1 or fit_rms.shape[0] < 1:
        raise V4FEPMCGateStateError("held/fit residual population differs")
    p95 = torch.quantile(
    ...
    if not bool(torch.isfinite(p95).all()) or float(p95) <= 0.0:
        raise V4FEPMCGateStateError("fit-only decoded-residual p95 is non-positive")
    ...
    if (
        ...
        raise V4FEPMCGateStateError("scaled temporal gate geometry/range differs")
...
def build_motion_codes(profile20: torch.Tensor) -> dict[str, epmc.MotionCode]:
    ...
    if profile20.dtype != torch.float32 or tuple(profile20.shape) != (1, 20):
        raise V4FEPMCGateStateError("correct temporal profile must be FP32 [1,20]")
    phase = torch.cat(
    ...
    if int(torch.count_nonzero(correct.phase_gates[:, 1:]).item()) == 0:
        raise V4FEPMCGateStateError("correct temporal gate degenerated to all zero")
    codes = {
    ...
    if tuple(codes) != ARM_ORDER:
        raise V4FEPMCGateStateError("derived arm order differs")
    ...
    for name in ("reverse", "shuffle"):
        ...
        if not torch.equal(reference, actual):
            raise V4FEPMCGateStateError(f"{name} changed the phase multiset")
...
def _write_json_create_only(path: Path, value: Mapping[str, Any]) -> str:
    if not path.is_absolute() or not path.parent.is_dir() or path.exists():
        raise V4FEPMCGateStateError("output must be a fresh absolute JSON child")
    payload = json.dumps(
        value,
        sort_keys=True,
        indent=2,
    ...
    if (
        ...
        raise V4FEPMCGateStateError("gate-state seal/readback differs")
    return digest
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--v4f-receipt", required=True)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source_binding = _current_source_binding(v4f, v4c)
    if type(args.batch_size) is not int or not 1 <= args.batch_size <= 128:
        raise V4FEPMCGateStateError("batch-size must be an integer in [1,128]")
    ...
    receipt = _strict_json(
        receipt_path,
        label="v4-F receipt",
        expected_sha256=EXPECTED_V4F_RECEIPT_FILE_SHA256,
        maximum_bytes=_MAX_RECEIPT_BYTES,
    )
    artifact, preselection_artifact, fold_contract, fold_binding = (
        validate_v4f_receipt_gate(
    ...
    preselection_path = _plain_absolute_file(
        args.fold1_preselection_checkpoint,
        label="fold-1 preselection checkpoint",
        maximum_bytes=_MAX_CHECKPOINT_BYTES,
    )
    validate_v4f_fold_receipt(
        fold_receipt,
        aggregate_fold=fold_contract,
        aggregate_artifact=artifact,
        aggregate_preselection_artifact=preselection_artifact,
        aggregate_binding=fold_binding,
        receipt_path=fold_receipt_path,
        preselection_path=preselection_path,
        checkpoint_path=checkpoint_path,
    )
    preselection_metadata, preselection_state = _load_checkpoint(
        preselection_path,
        artifact=preselection_artifact,
        v4f=v4f,
        expected_file_sha256=EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256,
        ...
        expected_role="preselection_fixed_step1200",
        expected_rho=TRAINING_RHO,
        expected_base_state_sha256=EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256,
        expected_preselection_file_sha256=None,
    )
    metadata, state = _load_checkpoint(
        checkpoint_path,
        artifact=artifact,
        v4f=v4f,
        expected_file_sha256=EXPECTED_FOLD1_CHECKPOINT_SHA256,
        expected_metadata_digest=EXPECTED_FOLD1_CHECKPOINT_METADATA_DIGEST,
        expected_model_state_sha256=EXPECTED_FOLD1_MODEL_STATE_SHA256,
        expected_role="selected_fold_local_rho",
        expected_rho=EXPECTED_FOLD1_SELECTED_RHO,
        expected_base_state_sha256=EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256,
        expected_preselection_file_sha256=(
            EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256
        ),
    )
    selected_preselection_binding = metadata.get(
        "preselection_checkpoint_binding"
    )
    if (
        v4f._base_state_sha(preselection_state)
        != EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
        or v4f._base_state_sha(state)
        != EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
        or preselection_metadata.get("model_state_sha256")
        != EXPECTED_FOLD1_PRESELECTION_MODEL_STATE_SHA256
        or not isinstance(selected_preselection_binding, Mapping)
        or selected_preselection_binding.get("path") != str(preselection_path)
        or selected_preselection_binding.get("file_sha256")
        != EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256
        or selected_preselection_binding.get("size_bytes")
        != preselection_artifact.get("size_bytes")
        or selected_preselection_binding.get("metadata_digest")
        != preselection_metadata.get("metadata_digest")
        or selected_preselection_binding.get("model_state_sha256")
        != preselection_metadata.get("model_state_sha256")
        or selected_preselection_binding.get("preselection_base_state_sha256")
        != EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
    ):
        raise V4FEPMCGateStateError(
            "fold-1 selected state does not join sealed preselection trained base"
        )
    model = _model_from_state(metadata, state, v4f=v4f)
    ...
    if (
        ...
        raise V4FEPMCGateStateError("v4-C exact644 feature authority differs")
    ...
    if (
        ...
        raise V4FEPMCGateStateError("fold-1 feature/split/checkpoint join differs")
    ...
    if tuple(zero_decode.shape) != (1, TIME_STEPS, FEATURE_DIM):
        raise V4FEPMCGateStateError("C(D(0)) geometry differs")
    ...
    payload: dict[str, Any] = {
        ...
        "v4f_receipt": {
            ...
            "file_sha256": EXPECTED_V4F_RECEIPT_FILE_SHA256,
            ...
            "exact5_all_fold_inner_gates_passed": True,
        },
        "fold_receipt": {
        ...
        "fold_preselection_checkpoint": {
            "path": str(preselection_path),
            "file_sha256": EXPECTED_FOLD1_PRESELECTION_CHECKPOINT_SHA256,
            "metadata_digest": preselection_metadata["metadata_digest"],
            "model_state_sha256": preselection_metadata["model_state_sha256"],
            "preselection_base_state_sha256": (
                EXPECTED_FOLD1_PRESELECTION_BASE_STATE_SHA256
            ),
            ...
            "selected_checkpoint_base_state_join_exact": True,
        ...
        "fold_checkpoint": {
            ...
            "selected_rho": EXPECTED_FOLD1_SELECTED_RHO,
        ...
        "decoded_residual_contract": {
            ...
            "selected_fold_local_rho": EXPECTED_FOLD1_SELECTED_RHO,
            "selected_rho_scales_encoder_delta_and_decoder_residual": True,
            "trained_base_excludes_only_residual_gate_rho": True,
    ...
    if _current_source_binding(v4f, v4c) != source_binding:
        raise V4FEPMCGateStateError("consumer source binding changed during execution")
    ...
    if (
        ...
        raise V4FEPMCGateStateError("fresh gate-state semantic readback differs")
    return {
