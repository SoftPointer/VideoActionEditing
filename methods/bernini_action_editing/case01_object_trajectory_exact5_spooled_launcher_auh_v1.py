#!/usr/bin/env python3
"""Create-only HOLD launcher materializer for the case01 trajectory exact-five.
...
SCHEMA = "case01-object-trajectory-exact5-hold-launch-release-auh-v1"
INPUT_SCHEMA = "case01-object-trajectory-exact5-hold-launch-input-auh-v1"
RECEIPT_SCHEMA = "case01-object-trajectory-exact5-hold-launch-receipt-auh-v1"
...
if len(IDENTITY_ROLES) != 25 or len(set(IDENTITY_ROLES)) != 25:
    raise RuntimeError("object trajectory exact25 role closure differs")

METHOD_ROLE_BASENAMES = {
    "runner": "case01_object_trajectory_exact5_runner_v1.py",
    "legacy_exact5_runner": "case01_source_bone_exact5_runner_v1.py",
    "object_eval": "case01_object_trajectory_exact5_eval_v1.py",
    ...
    "adapter": "infer_case01_object_trajectory_oracle_v1.py",
...
if type(spec) is not dict or set(spec)!={"schema_version","campaign_mode","launch_allowed","identities","captured_runner","launch_input","result_path"} or canonical(spec)!=spec_raw or digest(spec)!=release_digest or spec.get("schema_version")!="case01-object-trajectory-exact5-root-bootstrap-diagnostic-v3" or spec.get("campaign_mode")!="case01-object-trajectory-exact5-r64-engineering-oracle" or spec.get("launch_allowed") is not False: raise RuntimeError("root diagnostic release differs")
identities=spec.get("identities")
if type(identities) is not dict or set(identities)!=set(roles) or len(identities)!=25 or len({row.get("path") for row in identities.values() if type(row) is dict})!=25: raise RuntimeError("exact25 identity closure differs")
...
if captured_runner.get("sha256")!="0d73fdaa4a4f1817f572eea471661850098ffe5aa54f54a9927c37a7e3f2a872" or captured_runner.get("size")!=21596 or captured_runner.get("path") in {row["path"] for row in identities.values()} or captured_runner==identities["runner"]: raise RuntimeError("captured runner authority differs/overlaps production exact25")
...
producer_roles={"legacy_infer_alias":("infer_lora_path","infer_lora_sha256","infer_lora_size"),"adapter":("inference_wrapper_path","inference_wrapper_sha256","inference_wrapper_size"),"trajectory_projection":("trajectory_projection_module_path","trajectory_projection_module_sha256","trajectory_projection_module_size"),"trajectory_scaffold_module":("trajectory_scaffold_module_path","trajectory_scaffold_module_sha256","trajectory_scaffold_module_size"),"ffprobe":("ffprobe_path","ffprobe_sha256","ffprobe_size")}
...
entry={"schema_version":"case01-object-trajectory-exact5-captured-root-entry-v3","release_digest":release_digest,"identity_roles":list(roles),"identity_set_digest":digest(identities),"launch_input_sha256":spec["launch_input"]["sha256"],"production_runner":identities["runner"],"captured_runner":captured_runner,"captured_runner_identity":list(captured_runner_identity),"plan_sha256":identities["plan"]["sha256"],"task_ids":tasks,"arm_order":arms,"all_exact25_named_identities_replayed":True,"captured_runner_outside_exact25":True,"captured_runner_bytes_compiled":True,"publication_performed":False}
