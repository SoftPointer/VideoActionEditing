#!/usr/bin/env python3
"""R3 per-rank fixed-RNG wrapper for one immutable E00 legacy arm."""
...
SCHEMA = "bernini-e00-legacy-fixed-rng-r3-receipt-v3"
...
class E00R3WrapperError(RuntimeError):
    pass
...
def run_with_fixed_initial_rng(
    ...
    *,
    ...
    if isinstance(cpu_seed, bool) or not isinstance(cpu_seed, int) or cpu_seed < 0:
        raise E00R3WrapperError("CPU seed must be a non-negative integer")
    if cuda_device is None:
        if cuda_seed is not None:
            raise E00R3WrapperError("CPU-only execution cannot accept a CUDA seed")
    elif isinstance(cuda_seed, bool) or not isinstance(cuda_seed, int) or cuda_seed < 0:
        raise E00R3WrapperError("CUDA execution requires a non-negative CUDA seed")
    ...
    fork_proof = {
        ...
        "after": caller_after,
        "cpu_state_restored": caller_before["cpu_sha256"] == caller_after["cpu_sha256"],
        "owned_cuda_state_restored": caller_before["cuda_sha256"] == caller_after["cuda_sha256"],
    }
    fixed_proof = {
        "enabled": True,
        "scheme": "explicit_rank_owned_cpu_cuda_manual_seed_r3",
        "scope": "inside_fork_rng_immediately_before_entire_legacy_inference_entrypoint",
        "cpu_seed": cpu_seed,
        "cuda_seed": cuda_seed,
        "seeded_initial": seeded_initial,
        "terminal_before_restore": terminal_before_restore,
    }
    if not fork_proof["cpu_state_restored"] or not fork_proof["owned_cuda_state_restored"]:
        raise E00R3WrapperError("fork_rng did not restore caller RNG bytes")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if file_sha256(Path(legacy_audit.__file__).resolve()) != EXPECTED_LEGACY_AUDIT_SHA256:
        raise E00R3WrapperError("sealed legacy audit wrapper bytes differ")

    protocol_validator = importlib.import_module("validate_e00_three_vessel_clean_diag_r3")
    ...
    if protocol.get("revision_tag") != REVISION_TAG:
        raise E00R3WrapperError("R3 protocol revision differs")
    ...
    if world_size != 4 or rank not in range(4) or local_rank not in range(4):
        raise E00R3WrapperError("R3 production diagnostic requires exactly four ranks")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 4:
        raise E00R3WrapperError("R3 production diagnostic requires four visible GPUs")
    torch.cuda.set_device(local_rank)
    seed_rows = [row for row in protocol["fixed_initial_rng"]["per_rank"] if row.get("rank") == rank]
    if len(seed_rows) != 1:
        raise E00R3WrapperError("rank seed lookup differs")
    ...
    if not callable(getattr(entrypoint, "main", None)):
        raise E00R3WrapperError("legacy entrypoint has no callable main")
    route_mode = {
    ...
    if return_code not in (None, 0):
        raise E00R3WrapperError(f"legacy entrypoint returned {return_code!r}")
    if [(row["master_seed"], row["step"], row["candidate"]) for row in noise_rows] != [
        (2027, step, 0) for step in range(40)
    ]:
        raise E00R3WrapperError("runtime keyed-noise call closure differs")
    ...
    if route_audit["call_count"] != expected_route_calls:
        raise E00R3WrapperError("pure-QK route call closure differs")
    if not latent_audit or latent_audit.get("finite") is not True:
        raise E00R3WrapperError("predecode latent audit is absent or non-finite")
    native_proof = legacy_audit._native_adapter_proof(contract["output"]) if rank == 0 else None
    if rank == 0 and native_proof is None:
        raise E00R3WrapperError("rank-zero native receipt is absent")
