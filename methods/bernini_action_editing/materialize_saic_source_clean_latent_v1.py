#!/usr/bin/env python3
"""Create one sealed Bernini clean-source latent for shared T0/I0 use.
...
row's source video, and calls the pinned Bernini ``_vae_encode`` exactly once
on the complete source tensor.  It never accepts an instruction, event bank,
target video, reference frame, adapter, mask, pose, flow, or motion donor.
...
train.  The purpose is solely to let otherwise independent T0 and I0 runner
processes consume byte-identical outer clean source state.
"""
...
RECEIPT_SCHEMA = "bernini-saic-source-clean-latent-receipt-v1"
ARTIFACT_SCHEMA = "bernini-saic-source-clean-latent-artifact-v1"
METHOD = "frozen-bernini-source-clean-latent-materializer"
TENSOR_KEY = "source_clean_latent"
FRAME_COUNT = 81
...
class SourceCleanLatentMaterializationError(RuntimeError):
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise SourceCleanLatentMaterializationError(
...
def validate_cli(args: argparse.Namespace) -> None:
    ...
    if (
        ...
        raise SourceCleanLatentMaterializationError("row_id is unsafe")
...
def load_sealed_source_row(
    ...
    *,
    ...
    try:
        ...
        raise SourceCleanLatentMaterializationError(str(error)) from error
...
def prepare_private_exact_source(
    ...
    *,
    ...
    try:
        ...
        raise SourceCleanLatentMaterializationError("PyTorch is required") from error
...
def validate_method_provenance(args: argparse.Namespace) -> dict[str, Any]:
    ...
    except SourceCleanLatentMaterializationError:
        raise
...
def validate_encoder_callable(
    ...
    *,
    ...
    if not callable(encoder):
        raise SourceCleanLatentMaterializationError("_vae_encode is not callable")
    if (
...
def encode_full_source_once(
    vae: Any,
    source_pixels: Any,
    *,
    encoder: Callable[[Any, Any], Any],
) -> tuple[Any, dict[str, Any]]:
    """Invoke the official encoder once and reject mutation or shape drift."""
    ...
    with torch.inference_mode():
        invocation_count += 1
        latent = encoder(vae, source_pixels)
    after_sha256 = tensor_raw_sha256(source_pixels)
    if invocation_count != 1:
        raise SourceCleanLatentMaterializationError(
            "full-source _vae_encode invocation count differs"
        )
    ...
    if (
        ...
        or tuple(int(item) for item in latent.shape[:3]) != (1, 16, 21)
    ):
        raise SourceCleanLatentMaterializationError(
            "source clean latent geometry/dtype/autograd state differs"
        )
    latent = latent.detach().contiguous()
    if not bool(torch.isfinite(latent).all().item()):
        raise SourceCleanLatentMaterializationError(
            "source clean latent contains non-finite values"
        )
    return latent, {
...
def _write_owned_bytes(
    ...
    *,
    ...
    temporary = final_path.with_name(
        f".{final_path.name}.source-clean-tmp-{transaction_token}"
    )
...
def _publish_source_clean_latent_owned(
    latent: Any,
    path: Path,
    *,
    transaction_token: str,
    ...
    if (
        ...
        raise SourceCleanLatentMaterializationError(
            "published latent must be detached FP32 [1,16,21,H,W]"
        )
    ...
    owned = _write_owned_bytes(
        ...
        role="source-clean latent",
    )
    try:
        os.chmod(path, 0o444)
        if stat.S_IMODE(path.stat().st_mode) != 0o444:
            raise SourceCleanLatentMaterializationError(
                "source-clean latent is not mode 0444"
            )
        with safe_open(str(path), framework="pt", device="cpu") as opened:
            if list(opened.keys()) != [TENSOR_KEY]:
                raise SourceCleanLatentMaterializationError(
                    "source-clean safetensors key differs"
                )
        ...
        if (
            ...
            raise SourceCleanLatentMaterializationError(
                "source-clean safetensors reopen differs"
            )
        ...
        if any(sealed_owned[key] != owned[key] for key in identity_keys):
            raise SourceCleanLatentMaterializationError(
                "source-clean identity changed while sealing"
            )
        public = {
            "schema_version": ARTIFACT_SCHEMA,
            "path": str(path),
...
def _publish_receipt_owned(
    ...
    *,
    ...
    owned = _write_owned_bytes(
        ...
        payload,
        transaction_token=transaction_token,
        role="source-clean receipt",
    )
    try:
        os.chmod(path, 0o444)
        if stat.S_IMODE(path.stat().st_mode) != 0o444:
            raise SourceCleanLatentMaterializationError(
                "source-clean receipt is not mode 0444"
            )
        raw = path.read_bytes()
        reopened = json.loads(raw.decode("ascii"))
        if raw != canonical_json_bytes(reopened) + b"\n":
            raise SourceCleanLatentMaterializationError(
                "source-clean receipt bytes are not canonical"
            )
        ...
        if (
            ...
            raise SourceCleanLatentMaterializationError(
                "source-clean receipt digest differs"
            )
...
def reopen_published_bundle(
    ...
    *,
    ...
    try:
        ...
        raise SourceCleanLatentMaterializationError(
            "cannot terminal-reopen source-clean latent"
        ) from error
    ...
    try:
        ...
        raise SourceCleanLatentMaterializationError(
            "cannot terminal-reopen source-clean receipt"
        ) from error
...
def publish_materialization_bundle(
    ...
    *,
    ...
    try:
        artifact_owned, artifact_public = _publish_source_clean_latent_owned(
...
def materialize_source_clean_latent(args: argparse.Namespace) -> dict[str, Any]:
    ...
    latent, encoding_state = encode_full_source_once(
        vae, source_pixels, encoder=_vae_encode
    )
    encoding = {**callable_identity, **encoding_state}
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    result = materialize_source_clean_latent(args)
...
__all__ = [
    ...
    "METHOD",
    "RECEIPT_SCHEMA",
    "SourceCleanLatentMaterializationError",
    "artifact_identity",
    "build_parser",
    "build_receipt",
    "canonical_json_bytes",
    "encode_full_source_once",
    "file_sha256",
    "load_sealed_source_row",
    "main",
    "materialize_source_clean_latent",
    "object_sha256",
