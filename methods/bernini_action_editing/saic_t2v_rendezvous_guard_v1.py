#!/usr/bin/env python3
"""Fail-closed dynamic rendezvous lifecycle for the SAIC full60 top-up.
...
RANK_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-rank-v1"
CLAIM_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-port-claim-v1"
DECISION_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-admission-v1"
COLLISION_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-port-collision-v1"
COMPLETION_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-completion-v1"
JOB_AUDIT_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-job-audit-v1"
PLAN_SCHEMA_VERSION = "saic-t2v-topup-rendezvous-dynamic-plan-v1"
...
def read_ready_bytes(path: Path, *, label: str) -> bytes:
    ...
    try:
        ...
        if (
            ...
            raise SAICT2VRendezvousGuardError(f"{label} ready inode differs")
        chunks = []
    ...
    if identity(before) != identity(after) or (leaf.st_dev, leaf.st_ino) != (
        before.st_dev,
        before.st_ino,
    ):
...
def exact_directory(
    ...
    *,
    ...
    if expected_identity is not None and (info.st_dev, info.st_ino) != _parse_identity(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            raise SAICT2VRendezvousGuardError(f"{label} inode differs")
        mode = stat.S_IMODE(info.st_mode)
        if mode == 0o600:
            time.sleep(0.02)
            continue
        if mode != 0o444:
            raise SAICT2VRendezvousGuardError(f"{label} readiness mode differs")
        return load_sealed(
            path, schema_version=schema_version, exact_fields=exact_fields
        )
...
def _validate_generic_admission(
    claim: Mapping[str, Any], *, require_rank_packets: bool
) -> dict[str, Any]:
    ...
    *,
    ...
    try:
        ...
        existing_admission = _validate_generic_admission(
            existing, require_rank_packets=True
        )
        collision = seal(
            {
                ...
                **identity_core(args),
                ...
                "existing_claim_sha256": ready_file_sha256(
    ...
    if rank == 0:
        ...
        while time.monotonic() < deadline:
            ...
            if claim_info is not None:
                claim_mode = stat.S_IMODE(claim_info.st_mode)
                if claim_mode == 0o600:
                    time.sleep(0.02)
                    continue
                candidate = load_sealed(
                    claim_path, schema_version=CLAIM_SCHEMA_VERSION, exact_fields=CLAIM_FIELDS
                )
                _validate_claim(candidate, port=port)
                if any(candidate.get(key) != value for key, value in identity_core(args).items()):
                    ...
                    if (
                        any(collision.get(key) != value for key, value in identity_core(args).items())
                        or collision.get("existing_claim_receipt_digest")
                        != candidate["receipt_digest"]
                    ):
                        raise SAICT2VRendezvousGuardError("worker collision evidence differs")
                    return COLLISION_EXIT_STATUS
                claim = candidate
                break
            time.sleep(0.02)
        if claim is None:
            raise SAICT2VRendezvousGuardError("timed out waiting for rank-zero port claim")
...
def admit_collision(args: argparse.Namespace) -> int:
    ...
    existing_admission = _validate_generic_admission(claim, require_rank_packets=True)
    if (
        ...
        != ready_file_sha256(claim_path, label="collision port claim")
...
def assemble(args: argparse.Namespace) -> int:
    ...
    packet_paths = sorted(lifecycle.glob("rank-*.json"), key=lambda item: item.name)
    if [path.name for path in packet_paths] != [f"rank-{rank}.json" for rank in range(WORLD_SIZE)]:
        raise SAICT2VRendezvousGuardError("successful lifecycle rank coverage differs")
    packets = [
        load_sealed(path, schema_version=RANK_SCHEMA_VERSION, exact_fields=RANK_FIELDS)
    ...
    attempt_path = exact_plain_file(
        candidate_output / ATTEMPT_RECEIPT_BASENAME,
        label="scientific attempt receipt",
        mode=0o444,
    )
    attempt = json.loads(
        read_ready_bytes(attempt_path, label="scientific attempt receipt").decode("ascii")
    )
    ...
    completion = seal(
        {
            ...
            **identity_core(args),
            ...
            "permanent_job_local_port_claim_sha256": ready_file_sha256(
            ...
            "attempt_receipt_sha256": ready_file_sha256(
                attempt_path, label="scientific attempt receipt"
            ),
...
def _validate_completion(
    ...
    if (
        ...
        != ready_file_sha256(claim_path, label="audited port claim")
    ...
    decision = _validate_generic_admission(claim, require_rank_packets=True)
    if (
    ...
    attempt_path = exact_plain_file(
        Path(str(completion.get("attempt_receipt_path"))),
        label="completed scientific attempt receipt",
        mode=0o444,
    )
    ...
    attempt = json.loads(
        read_ready_bytes(attempt_path, label="audited attempt receipt").decode("ascii")
    )
    unsigned_attempt = dict(attempt)
    attempt_digest = unsigned_attempt.pop("receipt_digest", None)
    if (
        ...
        != ready_file_sha256(attempt_path, label="audited attempt receipt")
...
def _validate_collision_for_job_audit(
    ...
    prior_admission = _validate_generic_admission(claim, require_rank_packets=True)
    if (
        ...
        or collision.get("existing_claim_receipt_digest") != claim["receipt_digest"]
        or collision.get("existing_claim_sha256")
        != ready_file_sha256(claim_path, label="audited collision claim")
        or collision.get("existing_admission_receipt_path")
        != claim["admission_receipt_path"]
        or collision.get("existing_admission_receipt_digest")
