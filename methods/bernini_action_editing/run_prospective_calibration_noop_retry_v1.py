#!/usr/bin/env python3
"""Render a same-seed noop for an admitted calibration forward retry.

The forward retry audit may admit a seed outside the preregistered pair.  A
factorial calibration cell must not compare that forward against a noop from a
different seed, so this runner changes only the seed of the frozen calibration
noop row.  It grants no representation, threshold, or optimizer authority.
"""
...
SCHEMA_VERSION = "bernini-prospective-calibration-noop-retry-v1"
AUTHORITY = {
    "calibration_same_seed_noop_generation_only": True,
...
class CalibrationNoopRetryError(RuntimeError):
    """Raised before an invalid same-seed noop can be rendered or accepted."""
...
def retry_row(
    ...
    if type(source_id) is not str or len(source_id) != 16:
        raise CalibrationNoopRetryError("source ID syntax differs")
    if type(retry_seed) is not int or not 0 <= retry_seed < 2**63:
        raise CalibrationNoopRetryError("retry seed differs")
    rows = [
        dict(row)
    ...
    if len(rows) != 2:
        raise CalibrationNoopRetryError(
            "source does not have two registered calibration noop rows"
        )
    registered_seeds = sorted(row["seed"] for row in rows)
    if retry_seed in registered_seeds:
        raise CalibrationNoopRetryError("retry seed aliases a registered seed")
    invariant = (
    ...
    if any(row[key] != template[key] for row in rows[1:] for key in invariant):
        raise CalibrationNoopRetryError(
            "registered calibration noop rows differ beyond seed"
        )
    template["seed"] = retry_seed
    template["entry_id"] = f"{source_id}-cal-retry-s{retry_seed}-noop"
...
def retry_receipt(
    *, manifest_sha256: str, manifest_digest: str, row: Mapping[str, Any],
    ...
    unsigned = {
        ...
        "status": "calibration_same_seed_noop_rendered_decoded_review_pending",
...
def command_run(args: argparse.Namespace) -> int:
    ...
    if any(entries_root.iterdir()):
        raise CalibrationNoopRetryError("entries root must be fresh")
    ...
    base._write_create_only(output_root / "calibration-noop-retry.receipt.json", value)
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    observed = base._read_object(
        base._plain_file(
            output_root / "calibration-noop-retry.receipt.json",
            label="calibration noop retry receipt",
        ),
        label="calibration noop retry receipt",
    )
    if observed != expected:
        raise CalibrationNoopRetryError("calibration noop retry receipt differs")
...
if __name__ == "__main__":
    ...
    except (CalibrationNoopRetryError, base.FactorialBranchRunError) as error:
        print(f"[calibration-noop-retry] ERROR: {error}", file=sys.stderr)
        raise SystemExit(2)
