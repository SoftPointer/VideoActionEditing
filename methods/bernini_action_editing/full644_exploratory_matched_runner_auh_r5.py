#!/usr/bin/env python3
"""AUH-env-bound case00 canary runner for the full644 matched evaluation.
...
SCHEMA = "full644-exploratory-matched-runner-attestation-auh-r5"
TASK_SCHEMA = "full644-exploratory-matched-runner-task-auh-r5"
FAILURE_SCHEMA = "full644-exploratory-matched-runner-failure-auh-r5"
...
def build_torchrun_argv(
    *,
    ...
    torch_local_agent_source_sha256: str,
    torch_dynamic_rendezvous_source: str,
    torch_dynamic_rendezvous_source_sha256: str,
    torch_multiprocessing_api_source: str,
    torch_multiprocessing_api_source_sha256: str,
    ...
    def execute_one(self, task: Mapping[str, Any], index: int) -> dict[str, Any]:
        ...
        try:
            ...
            argv = build_torchrun_argv(
                ...
                torch_local_agent_source=self.args.torch_local_agent_source,
                torch_local_agent_source_sha256=(
                    self.args.torch_local_agent_source_sha256
                ),
                torch_dynamic_rendezvous_source=(
                    self.args.torch_dynamic_rendezvous_source
                ),
                torch_dynamic_rendezvous_source_sha256=(
                    self.args.torch_dynamic_rendezvous_source_sha256
                ),
                torch_multiprocessing_api_source=(
                    self.args.torch_multiprocessing_api_source
                ),
                torch_multiprocessing_api_source_sha256=(
                    self.args.torch_multiprocessing_api_source_sha256
                ),
...
def verify_case00_canary_pair(
    ...
    report: dict[str, Any] = {
        "schema_version": (
            "full644-exploratory-matched-auh-r5-case00-canary-report-v1"
        ),
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    selected_task_ids = tuple(task["task_id"] for task in tasks)
    # Final artifacts must not overlap even the fourteen deliberately
    # unselected plan leaves in canary mode.
