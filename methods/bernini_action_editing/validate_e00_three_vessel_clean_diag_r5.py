#!/usr/bin/env python3
"""R5 protocol, phase capability, gate, and current-artifact validator."""
...
import validate_e00_three_vessel_clean_diag_r4 as r4


SCHEMA = "bernini-e00-clean-diagnostic-r5-protocol-v5"
REVISION_TAG = "E00_DFIX2_CLEAN_DIAG_R5_ACYCLIC_BOOTSTRAP_20260821"
RNG_SCHEMA = "bernini-e00-legacy-fixed-rng-r5-receipt-v5"
ARM_AUDIT_SCHEMA = "bernini-e00-clean-diagnostic-r5-arm-audit-v5"
AB_GATE_SCHEMA = "bernini-e00-clean-diagnostic-r5-ab-current-bit-exact-gate-v5"
FINAL_SCHEMA = "bernini-e00-clean-diagnostic-r5-final-current-artifact-audit-v5"
CAPABILITY_SCHEMA = "bernini-e00-clean-diagnostic-r5-bridge-capability-v5"
PHASE_A_MARKER_SCHEMA = "bernini-e00-clean-diagnostic-r5-phase-a-stopped-marker-v5"
BOOTSTRAP_SCHEMA = "bernini-e00-clean-diagnostic-r5-bootstrap-root-v5"
METHOD_ROOT = Path(__file__).resolve().parent
REPO_ROOT = METHOD_ROOT.parents[1]
DEFAULT_PROTOCOL = METHOD_ROOT / "assets/e00_three_vessel_clean_diag_r5_protocol_20260821.json"
ARM_ROLES = r4.ARM_ROLES
EXPECTED_SEEDS = copy.deepcopy(r4.EXPECTED_SEEDS)
REQUIRED_EQUALITIES = list(r4.REQUIRED_EQUALITIES)
REQUIRED_CURRENT_BINDINGS = list(r4.REQUIRED_CURRENT_BINDINGS)
ROOT_PINNED_FILES = [
...
BOOTSTRAP_CONSUMERS = [
    "methods/bernini_action_editing/scripts/auh_e00_three_vessel_clean_diag_r5_bridge.sh",
    "methods/bernini_action_editing/scripts/auh_launch_e00_three_vessel_clean_diag_r5_phase_a_only_node292.sh",
    "methods/bernini_action_editing/scripts/auh_launch_e00_three_vessel_clean_diag_r5_phase_bc_node292.sh",
]
HEX = set("0123456789abcdef")


class E00R5Error(RuntimeError):
    pass


def _fail(message: str) -> None:
    raise E00R5Error(message)
...
def _load(path: Path, label: str) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise E00R5Error(f"{label} is unreadable") from error
...
def validate_protocol(protocol: Mapping[str, Any]) -> dict[str, Any]:
    for path, expected in (
        ...
        ("bootstrap_contract.schema_version", BOOTSTRAP_SCHEMA),
        ("bootstrap_contract.package_relative_path", "methods/bernini_action_editing/assets/e00_three_vessel_clean_diag_r5_BOOTSTRAP_ROOT.json"),
        ("bootstrap_contract.launcher_and_bridge_must_hardcode_root_sha256", True),
        ("bootstrap_contract.root_does_not_pin_consumers", True),
        ("bootstrap_contract.authority_does_not_pin_bootstrap_files_or_consumers", True),
        ("bootstrap_contract.root_pinned_files", ROOT_PINNED_FILES),
        ("bootstrap_contract.bootstrap_consumers", BOOTSTRAP_CONSUMERS),
        ("phase_a_placeholder_contract.classification", "content_static_lossy_decode_reencode_placeholder"),
        ("phase_a_placeholder_contract.source_authority", "source_video_frame_zero_visual_content"),
        ("phase_a_placeholder_contract.motion", "static_81_frames_at_25_fps"),
        ...
        ("phase_a_placeholder_contract.purpose", "legacy parser placeholder only; no self-generated anchor pixels"),
        ...
        ("package_integrity_contract.bootstrap_root_required_before_authority_or_verifier_import", True),
    ...
    if protocol.get("two_phase_state_machine") != expected_states:
        _fail("R5 phase state machine differs")
    ...
    base_value = r4.r3.legacy.load_spec(base_path)
    ...
    for phase, schema in (
        ("phase_a", "bernini-e00-clean-diagnostic-r5-phase-a-authorization-v5"),
        ("phase_bc", "bernini-e00-clean-diagnostic-r5-phase-bc-authorization-v5"),
    ):
...
def load_protocol(path: Path | str = DEFAULT_PROTOCOL) -> Mapping[str, Any]:
    protocol = _load(Path(path), "R5 protocol")
    validate_protocol(protocol)
    return protocol


@contextmanager
def _r5_semantic_core() -> Iterator[None]:
    replacements = {
        "REVISION_TAG": REVISION_TAG,
    ...
    before = {name: getattr(r4, name) for name in replacements}
    try:
...
def _authorization_common(
    ...
    _plain_file(package_manifest_path, "R5 package manifest")
    for path, expected in (
...
def validate_phase_a_marker_current(
    ...
    marker = _load(marker_path, "R5 phase A marker")
    protocol_id = protocol_identity(protocol, protocol_path)
    for path, expected in (
        ...
        ("decoded_bit_exact_source_video_claimed", False),
    ):
    ...
    if phase_a_auth.get("schema_version") != "bernini-e00-clean-diagnostic-r5-phase-a-authorization-v5" or phase_a_auth.get("bridge_capability_token_sha256") != token_sha:
...
def validate_bridge_capability(
    *, phase: str, arm_role: str, protocol: Mapping[str, Any], protocol_path: Path,
    ...
    if phase == "A":
        ...
        for path, expected in (
            ("schema_version", "bernini-e00-clean-diagnostic-r5-phase-bc-authorization-v5"),
...
def _write_new_json(path: Path, value: Mapping[str, Any]) -> None:
    if path.exists() or path.is_symlink():
        _fail(f"refusing to overwrite R5 output: {path}")
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(path.name + ".tmp")
    if temporary.exists() or temporary.is_symlink():
        _fail(f"R5 temporary output already exists: {temporary}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "protocol":
        ...
        r4.r3.legacy._probe_video(Path(args.video)); _write_new_json(Path(args.audit_output), value)
