#!/usr/bin/env python3
"""CPU-only runner used to attest the r5d captured-source root bootstrap.
...
case00 Base-then-R64 canary argv and captured-entry authority, records the AUH Slurm
...
SCHEMA = "full644-exploratory-matched-r5d-root-bootstrap-cpu-probe-v1"
...
TASK_IDS = ("shared8-00-base", "shared8-00-full644")
SLURM_FIELDS = (
    "SLURM_JOB_ID",
    "SLURM_STEP_ID",
    "SLURM_GPUS_ON_NODE",
...
def validate_campaign_contract(campaign: Any, selected: Sequence[Any]) -> None:
    if campaign != CAMPAIGN or tuple(selected) != TASK_IDS:
        raise R5DRootBootstrapProbeError("case00 canary selection differs")
...
def _publish_receipt(path: Path, value: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        sentinel = f"R5D_ROOT_BOOTSTRAP_CPU_PROBE_PASS {value['receipt_digest']}\n".encode("ascii")
