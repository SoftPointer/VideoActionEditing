#!/usr/bin/env python3
"""Frozen native RV2V same-process P0/P1/P2 prompt-pairing canary.
...
SCHEMA_VERSION = "mev840-native-rv2v-paired-prompt-matrix-v1"
METHOD = "frozen-bernini-native-rv2v-paired-prompt-matrix"
AUTHORITY_SCHEMA = "mev840-native-rv2v-same-process-prompt-matrix-v2"
...
MECHANICAL_SLURM_JOB_ID = "147873"
MECHANICAL_SLURM_NODE = "auh7-1b-gpu-284"
MECHANICAL_CGROUP_LIMIT_BYTES = 64 * 1024**3
...
def load_prompt_matrix_authority(
    ...
    *,
    ...
    if runtime_authority != {
        ...
        "mechanical_slurm": {
            "job_id": MECHANICAL_SLURM_JOB_ID,
            "node": MECHANICAL_SLURM_NODE,
            "world_size": ULYSSES_SIZE,
        },
        "nearest_finite_cgroup_limit_bytes": MECHANICAL_CGROUP_LIMIT_BYTES,
    }:
...
def validate_cli(args: argparse.Namespace) -> dict[str, Any]:
    if args.num_inference_steps not in (2, NUM_INFERENCE_STEPS):
        raise NativeIdentityCanaryError("paired runner permits only 2 or 40 UniPC steps")
    if bool(args.skip_video_decode) and args.num_inference_steps != 2:
        raise NativeIdentityCanaryError("skip-video-decode is restricted to the 2-step mechanical canary")
    if args.num_inference_steps == 2 and (
        args.seed != 2028 or not bool(args.skip_video_decode)
    ):
        raise NativeIdentityCanaryError(
            "2-step mechanical canary requires seed 2028 and skip-video-decode"
        )
...
def _linux_cgroup_memory_evidence(
    *, baseline: Optional[Mapping[str, Any]] = None
    ...
    are cumulative.  A mechanical run therefore seals an early per-rank
    ...
    if effective["memory_max"] != MECHANICAL_CGROUP_LIMIT_BYTES:
...
def _mechanical_slurm_context(
    args: argparse.Namespace, authority_bundle: Mapping[str, Any]
) -> Optional[dict[str, Any]]:
    """Bind the two-step receipt to its authorized holder job and exact step."""

    if args.num_inference_steps != 2:
        return None
    mode = authority_bundle["authority"].get("execution_modes", {}).get(
        "mechanical_canary"
    )
    if mode != {
        "seed": 2028,
        ...
        "formal_launch_authorized_by_canary": False,
    }:
        raise NativeIdentityCanaryError("mechanical Slurm authority differs")
    ...
    if (
        ...
        or node != MECHANICAL_SLURM_NODE
        or world_size_text != str(ULYSSES_SIZE)
    ):
        raise NativeIdentityCanaryError("mechanical Slurm job/node/step/WORLD4 differs")
    return {
        "job_id": job_id,
        "step_id": step_id,
...
def _save_paired_outputs(
    *,
    ...
    outputs: dict[str, Any] = {}
    for cell, latent in generated.items():
        outputs[cell] = {
            "path": None,
            "sha256": None,
            "video_decode_skipped": True,
            "normalized_clean_latent": _save_normalized_clean_latent_atomically(
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "execution_mode": {
            ...
            "mechanical_canary": args.num_inference_steps == 2,
            "video_decode_skipped": bool(args.skip_video_decode),
            "scientific_candidate": args.num_inference_steps == 40,
        },
        "interpretation": {
            "purpose": "paired_native_rv2v_prompt_ablation_with_one_materialized_source_condition",
            "training_performed": False,
            "mechanical_canary_proves_video_quality": False,
            "mechanical_canary_proves_action_gain": False,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    local_slurm_context = _mechanical_slurm_context(args, authority_bundle)
    ...
    device = torch.device("cuda", distributed.local_rank)
    slurm_context = None
    if local_slurm_context is not None:
        slurm_rows = _all_rank_object(
            local_slurm_context,
            world_size=distributed.world_size,
            label="mechanical_slurm_context",
        )
        if any(row != slurm_rows[0] for row in slurm_rows[1:]):
            raise NativeIdentityCanaryError(
                "mechanical Slurm binding differs across WORLD4"
            )
    ...
    if distributed.rank == 0:
        del initial_noise_captures, generated
    dist.destroy_process_group()
    return 0
