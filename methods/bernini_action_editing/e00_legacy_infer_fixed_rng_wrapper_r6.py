#!/usr/bin/env python3
"""R6 fixed-RNG wrapper for the old-QK white-leakage diagnostic."""
...
SCHEMA = "bernini-e00-legacy-fixed-rng-r6-receipt-v6"
...
class E00R6WrapperError(RuntimeError):
    pass
...
@contextmanager
def capture_sampler_initial_latent() -> Iterator[dict[str, Any]]:
    """Bind the source-owned latent from which the sampler initializes edit state."""
    ...
    def captured(*args: Any, **kwargs: Any) -> Any:
        ...
        if source_latent is None:
            raise E00R6WrapperError("sampler source-owned initial latent is absent")
        identity = source_audit.tensor_identity(
            source_latent, label="R6 sampler source-owned initial edit latent"
        )
        audit.update({
            "definition": "edit_packed_is_initialized_from_source_latent_before_step0_keyed_noise",
            "source_owned_initial_edit_latent": identity,
        })
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if file_sha256(Path(legacy_audit.__file__).resolve()) != EXPECTED_LEGACY_AUDIT_SHA256:
        raise E00R6WrapperError("sealed legacy audit wrapper bytes differ")
    protocol_validator = importlib.import_module("validate_e00_three_vessel_clean_diag_r6")
    ...
    if protocol.get("revision_tag") != REVISION_TAG:
        raise E00R6WrapperError("R6 protocol revision differs")
    ...
    if world_size != 4 or rank not in range(4) or local_rank not in range(4):
        raise E00R6WrapperError("R6 diagnostic requires exactly four ranks")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 4:
        raise E00R6WrapperError("R6 diagnostic requires four visible GPUs")
    torch.cuda.set_device(local_rank)
    seed_rows = [row for row in protocol["fixed_initial_rng"]["per_rank"] if row.get("rank") == rank]
    if len(seed_rows) != 1:
        raise E00R6WrapperError("R6 rank seed lookup differs")
    ...
    if not callable(getattr(entrypoint, "main", None)):
        raise E00R6WrapperError("legacy entrypoint has no callable main")
    route_mode = {
    ...
    with legacy_audit.capture_keyed_noise_rows() as noise_rows, capture_sampler_initial_latent() as initial_latent_audit, legacy_audit.audit_route_application(
        mode=route_mode
        ...
        return_code, fork_proof, fixed_proof = run_with_fixed_initial_rng(
            torch, lambda: entrypoint.main(legacy_argv), cuda_device=local_rank,
            cpu_seed=int(seed_row["cpu_seed"]), cuda_seed=int(seed_row["cuda_seed"]),
        )
    if return_code not in (None, 0):
        raise E00R6WrapperError(f"legacy entrypoint returned {return_code!r}")
    if [(row["master_seed"], row["step"], row["candidate"]) for row in noise_rows] != [(2027, step, 0) for step in range(40)]:
        raise E00R6WrapperError("runtime keyed-noise call closure differs")
    ...
    if route_audit["call_count"] != expected_route_calls:
        raise E00R6WrapperError("pure-QK route call closure differs")
    if not latent_audit or latent_audit.get("finite") is not True:
        raise E00R6WrapperError("predecode latent audit is absent or non-finite")
    native_proof = legacy_audit._native_adapter_proof(contract["output"]) if rank == 0 else None
    if rank == 0 and native_proof is None:
        raise E00R6WrapperError("rank-zero native receipt is absent")
    is_a = args.wrapper_arm_role == "pure_noobserver_output_routeoff"
    receipt: Mapping[str, Any] = {
        ...
        "diagnostic_scope": {
            ...
            "only_intended_b_c_difference": "final_pure_qk_route_application",
        },
        "fork_rng": fork_proof,
        ...
        "legacy_abi": {
            ...
            "self_generated_anchor_video_read": not is_a,
            "required_anchor_path_is_source_placeholder": False,
            "required_anchor_path_is_source_frame0_static_placeholder": is_a,
            "phase_a_placeholder_classification": "content_static_lossy_decode_reencode_placeholder" if is_a else None,
            ...
            "honest_control_name": (
                "pure no-observer route-off; content-static lossy frame-0 placeholder, not bit-exact"
                if is_a else "observer-matched output-route-off K0; old-QK diagnostic"
                if args.wrapper_arm_role == "observer_matched_output_routeoff"
                else "old pure-QK temporal route-on; white-leakage diagnostic, not a property-preservation fix"
            ),
        },
        "runtime_noise": {"scheme": "sha256_keyed_cpu_torch_generator_v1", "master_seed": 2027, "rows": noise_rows},
        "sampler_initial_latent": initial_latent_audit,
