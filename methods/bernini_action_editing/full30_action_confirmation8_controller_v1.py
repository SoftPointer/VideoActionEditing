#!/usr/bin/env python3
"""Fail-closed controller for BOX-EXP-009 confirmation action-anchor exact8.
...
and the official same-state materializer threshold gate.  Generated diagnostic
branches are explicitly outside review admission, q, a_min, and optimization.
"""
...
CONTROLLER_PLAN_SCHEMA = "bernini-full30-action-confirmation8-controller-plan-v1"
REVIEW_SCHEMA = "bernini-full30-action-confirmation8-review-admission-v1"
MATERIALIZER_GATE_SCHEMA = (
    "bernini-full30-action-confirmation8-materializer-gate-v1"
)
TERMINAL_HOST_GATE_SCHEMA = (
    "bernini-full30-action-confirmation8-terminal-host-memory-gate-v1"
)
COMPLETION_SCHEMA = "bernini-full30-action-confirmation8-completion-v1"
HOLDER_JOB = "136309"
...
def validate_release_tree(
    *, method_root: str | Path, manifest_path: str | Path,
    ...
    require(
        ...
        "release-bound 136309 resource specialization does not replay",
    )
    return manifest
...
def _controller_plan_value(
    *,
    ...
    unsigned = {
        ...
        "purpose": "confirmation action-anchor exact8 pre-optimizer audit",
        "learning_target": "N/A",
        "numeric_target": "N/A",
        "dataset": "confirmation_action_anchor_exact8",
        "training_steps": 0,
        "baseline": "frozen Bernini T2V action-anchor generation",
        "core_validation": (
            "action and incomplete each pass all 81 frames; official same-state "
            "materializer controls pass thresholds"
        ),
        "release": {
        ...
        "holder": {"job_id": 136309, "node": HOLDER_NODE},
...
def validate_runtime_environment(
    ...
    require(
        ...
        "runtime is not the retained 136309/gpu280 numbered child",
    )
    forbidden = {
        "OPTIMIZER_STATE",
        "TRAINING_STEP",
...
def validate_review_admission(
    ...
    diagnostic_ids = {
        task["candidate_id"] for task in exact8_plan["diagnostic_tasks"]
    }
    rows = value.get("candidate_reviews")
    require(
        value.get("plan_digest") == exact8_plan["plan_digest"]
        and value.get("review_population") == "confirmation_action_anchor_exact8"
    ...
    require(
        not ({row["candidate_id"] for row in rows} & diagnostic_ids)
        and value.get("generated_diagnostic_review_required") is False
        and value.get("generated_diagnostic_admitted") is False,
        "diagnostic candidate leaked into review admission",
    )
...
def validate_materializer_gate(
    ...
    require(
        ...
        and value.get("generated_diagnostic_used_by_q") is False
        and value.get("generated_diagnostic_used_by_a_min") is False
        and value.get("generated_diagnostic_used_by_optimizer") is False
...
def validate_exact8_audit(
    ...
    require(
        ...
        and value.get("optional_diagnostic_generation_observed_or_required") is False
...
def seal_terminal_host_gate(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    unsigned = {
        ...
        "legacy_fit_r13_start_field_name_does_not_authorize_formal40": True,
        "optimizer_authorized": False,
    }
...
def seal_completion(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    unsigned = {
        ...
        "dataset": "confirmation_action_anchor_exact8",
        "formal_candidate_count": 8,
        "generated_diagnostic_required_or_admitted": False,
