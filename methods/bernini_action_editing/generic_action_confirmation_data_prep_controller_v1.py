#!/usr/bin/env python3
"""Fail-closed 136309 controller for sealed reserve4 confirmation media only.

The only runnable profile renders the frozen confirmation split: two registered seed
...
source/tokenizer setup or native sampling begins.  A frozen fit-r13 resource
contract supplies the 10 ms cgroup monitor and physical ``safe_open`` tensor
identity audit, while the confirmation runner supplies only the sealed
confirmation tasks.  Outputs are pending-external-review authoring media.  The
shipped ``train_lora.py`` is an inference import dependency; only the controller
and launcher are allowed entrypoints, and those entrypoints forbid review
certification, Phi extraction, P/O manifests, optimizers, and training.
"""
...
import build_generic_action_confirmation_release_v1 as release  # noqa: E402
import reserve4_confirmation_generation_sp4_v1 as reserve4_runner  # noqa: E402


RESOURCE_SPECIALIZATION_SCHEMA = (
    "bernini-confirmation40-fit-r13-resource-specialization-v1"
)
RESOURCE_SPECIALIZATION_RECEIPT_SCHEMA = (
    "bernini-confirmation40-fit-r13-resource-specialization-receipt-v1"
)
RESOURCE_SOURCE_MEMBER = "tools/reserve4_fixed_generation_sp4_v1.py"
RESOURCE_SOURCE_SHA256 = (
    "be722e4020040ba446f290f07378e870e2d3c1a4228ec997c3447770fcb53d5d"
)
RESOURCE_SOURCE_SIZE = 166_064
RESOURCE_SOURCE_HOLDER_JOB = "136141"
RESOURCE_CONFIRMATION_HOLDER_JOB = "136309"
RESOURCE_SPECIALIZED_SHA256 = (
    "daa6cb2c128b689129d40fd0597e6e95ca5f3bd9fc5132abc70e0bcbe3cfb4af"
)
RESOURCE_SPECIALIZED_MODULE_NAME = (
    "_bernini_confirmation40_fit_r13_resource_136309_daa6cb2c"
)
RESOURCE_EXPECTED_AST_STRING_SITES = (
    (318, 24, "136141"),
    (654, 41, "136141"),
    (1198, 41, "136141"),
    (1472, 24, "136141"),
...
RESOURCE_SPECIALIZATION_AUTHORITY = {
    "schema_version": RESOURCE_SPECIALIZATION_SCHEMA,
    "source_member": RESOURCE_SOURCE_MEMBER,
    "source_sha256": RESOURCE_SOURCE_SHA256,
    "source_size": RESOURCE_SOURCE_SIZE,
    "source_holder_job": RESOURCE_SOURCE_HOLDER_JOB,
    "confirmation_holder_job": RESOURCE_CONFIRMATION_HOLDER_JOB,
    "confirmation_holder_node": "auh7-1b-gpu-280",
    "replacement_count": len(RESOURCE_EXPECTED_AST_STRING_SITES),
    "ast_string_sites": [list(row) for row in RESOURCE_EXPECTED_AST_STRING_SITES],
    "specialized_sha256": RESOURCE_SPECIALIZED_SHA256,
    "specialized_size": RESOURCE_SOURCE_SIZE,
    "specialized_module_name": RESOURCE_SPECIALIZED_MODULE_NAME,
    "same_length_required": True,
    "old_holder_token_zero_residual_required": True,
    "preloaded_source_module_rejected": True,
    "only_holder_job_token_substitution_authorized": True,
    "holder_node_is_enforced_by_outer_confirmation_controller": True,
    "all_non_holder_bytes_identical_required": True,
    "memory_limit_gib_preserved": 60,
    "sampled_safe_ceiling_gib_preserved": 56,
    "sample_interval_ns_preserved": 10_000_000,
    "maximum_sample_gap_ns_preserved": 100_000_000,
    "rank_gpu_memory_limit_gib_preserved": 52,
    "t5_rank_gpu_residency_contract_preserved": True,
    "physical_safe_open_contract_preserved": True,
    "r10_tensor_parity_contract_preserved": True,
}


class ConfirmationResourceSpecializationError(RuntimeError):
    """Raised before any unpinned fit-r13 resource postimage executes."""


def _stable_resource_source_bytes(path: Path) -> bytes:
    if not path.is_absolute() or path.is_symlink():
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource source must be absolute and non-symlink"
        )
    try:
        resolved = path.resolve(strict=True)
    except OSError as error:
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource source is unavailable"
        ) from error
    if resolved != path:
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource source canonical path differs"
        )
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1:
            raise ConfirmationResourceSpecializationError(
                "fit-r13 resource source must be one plain single-link file"
            )
        chunks = bytearray()
        while True:
        ...
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource source cannot be opened stably"
        ) from error
    finally:
    ...
    if identity != (after.st_dev, after.st_ino, after.st_size, after.st_mtime_ns):
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource source changed while being read"
        )
    raw = bytes(chunks)
    if (
        len(raw) != before.st_size
        or len(raw) != RESOURCE_SOURCE_SIZE
        or hashlib.sha256(raw).hexdigest() != RESOURCE_SOURCE_SHA256
    ):
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource source size/SHA-256 differs"
        )
    return raw


def _specialize_resource_contract_bytes(raw: bytes) -> bytes:
    source_token = RESOURCE_SOURCE_HOLDER_JOB.encode("ascii")
    confirmation_token = RESOURCE_CONFIRMATION_HOLDER_JOB.encode("ascii")
    if (
        len(raw) != RESOURCE_SOURCE_SIZE
        or hashlib.sha256(raw).hexdigest() != RESOURCE_SOURCE_SHA256
        or raw.count(source_token) != len(RESOURCE_EXPECTED_AST_STRING_SITES)
        or raw.count(confirmation_token) != 0
    ):
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource specialization preimage differs"
        )
    try:
        source_tree = ast.parse(raw.decode("utf-8"), filename=RESOURCE_SOURCE_MEMBER)
    except (SyntaxError, UnicodeError) as error:
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource specialization preimage AST differs"
        ) from error
    sites = sorted(
        ...
        and RESOURCE_SOURCE_HOLDER_JOB in node.value
    )
    if sites != list(RESOURCE_EXPECTED_AST_STRING_SITES):
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource specialization AST sites differ"
        )
    specialized = raw.replace(source_token, confirmation_token)
    differing_byte_count = sum(left != right for left, right in zip(raw, specialized))
    expected_differing_byte_count = len(RESOURCE_EXPECTED_AST_STRING_SITES) * sum(
        left != right for left, right in zip(source_token, confirmation_token)
    )
    if (
        len(specialized) != len(raw)
        or specialized.count(source_token) != 0
        or specialized.count(confirmation_token)
        != len(RESOURCE_EXPECTED_AST_STRING_SITES)
        or differing_byte_count != expected_differing_byte_count
        or hashlib.sha256(specialized).hexdigest()
        != RESOURCE_SPECIALIZED_SHA256
        or specialized.replace(confirmation_token, source_token) != raw
    ):
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource specialization postimage differs"
        )
    try:
        specialized_tree = ast.parse(
            specialized.decode("utf-8"), filename=RESOURCE_SOURCE_MEMBER
        )
    except (SyntaxError, UnicodeError) as error:
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource specialization postimage AST differs"
        ) from error
    if any(
        isinstance(node, ast.Constant)
        and isinstance(node.value, str)
        and RESOURCE_SOURCE_HOLDER_JOB in node.value
        for node in ast.walk(specialized_tree)
    ):
        raise ConfirmationResourceSpecializationError(
            "fit-r13 resource specialization retained the old holder token"
        )
    return specialized


def _reject_preloaded_resource_source(source_path: Path) -> None:
    forbidden_names = {
        "reserve4_fixed_generation_sp4_v1",
        "methods.bernini_action_editing.tools.reserve4_fixed_generation_sp4_v1",
        RESOURCE_SPECIALIZED_MODULE_NAME,
    }
    for name, module in tuple(sys.modules.items()):
        if name in forbidden_names:
            raise ConfirmationResourceSpecializationError(
                f"preloaded fit-r13 resource module is forbidden: {name}"
            )
        module_file = getattr(module, "__file__", None)
        if not isinstance(module_file, str) or not module_file:
            continue
        try:
            if Path(module_file).resolve(strict=True) == source_path:
                raise ConfirmationResourceSpecializationError(
                    "preloaded fit-r13 resource source path is forbidden"
                )
        except OSError:
            continue


def _load_specialized_resource_contract(
    source_path: Optional[Path] = None,
) -> types.ModuleType:
    path = (
        METHOD_ROOT / RESOURCE_SOURCE_MEMBER
        if source_path is None
        else source_path
    )
    raw = _stable_resource_source_bytes(path)
    _reject_preloaded_resource_source(path)
    specialized = _specialize_resource_contract_bytes(raw)
    module = types.ModuleType(RESOURCE_SPECIALIZED_MODULE_NAME)
    module.__file__ = str(path)
    module.__package__ = None
    module.__confirmation_source_sha256__ = RESOURCE_SOURCE_SHA256
    module.__confirmation_specialized_sha256__ = RESOURCE_SPECIALIZED_SHA256
    module.__confirmation_specialization_authority__ = dict(
        RESOURCE_SPECIALIZATION_AUTHORITY
    )
    exec(
        compile(specialized, str(path), "exec", dont_inherit=True),
        module.__dict__,
    )
    required_exports = {
        ...
        "HOST_MEMORY_SAMPLE_INTERVAL_NS",
        ...
        "run_host_cgroup_memory_monitor",
        "write_host_cgroup_memory_gate_receipt",
        "load_host_cgroup_memory_gate",
        "_native_smoke_tensor_evidence",
        "load_r10_tensor_parity_authority",
        "_validate_r10_smoke_tensor_parity",
    }
    if (
        module.__name__ != RESOURCE_SPECIALIZED_MODULE_NAME
        or module.__file__ != str(path)
        or module.__confirmation_source_sha256__ != RESOURCE_SOURCE_SHA256
        or module.__confirmation_specialized_sha256__
        != RESOURCE_SPECIALIZED_SHA256
        or module.__confirmation_specialization_authority__
        != RESOURCE_SPECIALIZATION_AUTHORITY
        or not required_exports.issubset(module.__dict__)
        or module.HOST_MEMORY_LIMIT_GIB != 60
        or module.HOST_MEMORY_LIMIT_BYTES != 60 * 1024**3
        or module.HOST_MEMORY_SAFE_CEILING_GIB != 56
        or module.HOST_MEMORY_SAFE_CEILING_BYTES != 56 * 1024**3
        or module.HOST_MEMORY_SAMPLE_INTERVAL_NS != 10_000_000
        or module.HOST_MEMORY_MAX_SAMPLE_GAP_NS != 100_000_000
        or module.T2V_GPU_MEMORY_LIMIT_GIB != 52
    ):
        raise ConfirmationResourceSpecializationError(
            "executed fit-r13 resource specialization identity/constants differ"
        )
    return module


resource_contract = _load_specialized_resource_contract()


PLAN_SCHEMA = "bernini-generic-action-confirmation40-generation-136309-plan-v4"
COMPLETION_SCHEMA = "bernini-generic-action-confirmation40-generation-136309-completion-v4"
...
BINDING_RECEIPT_SCHEMA = (
    "bernini-generic-action-confirmation40-shard-physical-binding-v4"
)
GPU_ADMISSION_RECEIPT_SCHEMA = (
    "bernini-generic-action-confirmation40-gpu-host-admission-receipt-v4"
)
GENERATION_CLOSURE_SCHEMA = (
    "bernini-generic-action-confirmation40-generation-closure-receipt-v4"
)
PHYSICAL_SMOKE_RECEIPT_SCHEMA = (
    "bernini-generic-action-confirmation40-physical-smoke-receipt-v4"
)
HOLDER_JOB = 136309
HOLDER_NODE = "auh7-1b-gpu-280"
WORLD_SIZE = 4
HOST_MEMORY_LIMIT_GIB = resource_contract.HOST_MEMORY_LIMIT_GIB
HOST_MEMORY_LIMIT_BYTES = resource_contract.HOST_MEMORY_LIMIT_BYTES
HOST_MEMORY_SAFE_CEILING_GIB = resource_contract.HOST_MEMORY_SAFE_CEILING_GIB
HOST_MEMORY_SAFE_CEILING_BYTES = resource_contract.HOST_MEMORY_SAFE_CEILING_BYTES
HOST_MEMORY_SAMPLE_INTERVAL_NS = resource_contract.HOST_MEMORY_SAMPLE_INTERVAL_NS
HOST_MEMORY_MAX_SAMPLE_GAP_NS = resource_contract.HOST_MEMORY_MAX_SAMPLE_GAP_NS
T2V_GPU_MEMORY_LIMIT_GIB = 52
T2V_GPU_MEMORY_LIMIT_BYTES = T2V_GPU_MEMORY_LIMIT_GIB * 1024**3
HOST_CGROUP_MEMORY_GATE_SCHEMA = resource_contract.HOST_CGROUP_MEMORY_GATE_SCHEMA
LAUNCH_CONFIRMATION = "launch-approved-generic-action-confirmation40-generation-136309"
...
CONFIRMATION_SCOPE = {
    ...
    "authoring_registry_raw_sha256": reserve4_runner.AUTHORING_REGISTRY_SHA256,
    "reserve4_selection_raw_sha256": reserve4_runner.RESERVE4_SELECTION_SHA256,
    "seed1_spec_raw_sha256": SEED1_SPEC_SHA256,
    "seed2_spec_raw_sha256": SEED2_SPEC_SHA256,
    "cells": [dict(row) for row in reserve4_runner.CONFIRMATION_CELL_REGISTRY],
    "branch_order": list(reserve4_runner.bank_contract.MACE_BRANCH_ORDER),
    "candidate_count": 40,
...
TOPOLOGY_CONTRACT = {
    ...
    "fit_r13_resource_specialization": RESOURCE_SPECIALIZATION_AUTHORITY,
...
AUTHORITY_CONTRACT = {
    ...
    "generated_media_role": (
        "pending-external-blind-review-confirmation-authoring-media-only"
    ),
...
def _resource_specialization_receipt_value() -> Mapping[str, Any]:
    source_path = _plain_file(
        METHOD_ROOT / RESOURCE_SOURCE_MEMBER,
        label="fit-r13 resource specialization source",
    )
    raw = _stable_resource_source_bytes(source_path)
    specialized = _specialize_resource_contract_bytes(raw)
    if (
        resource_contract.__name__ != RESOURCE_SPECIALIZED_MODULE_NAME
        or resource_contract.__file__ != str(source_path)
        or resource_contract.__confirmation_source_sha256__
        != RESOURCE_SOURCE_SHA256
        or resource_contract.__confirmation_specialized_sha256__
        != RESOURCE_SPECIALIZED_SHA256
        or resource_contract.__confirmation_specialization_authority__
        != RESOURCE_SPECIALIZATION_AUTHORITY
    ):
        fail("active fit-r13 resource specialization module binding differs")
    unsigned = {
        "schema_version": RESOURCE_SPECIALIZATION_RECEIPT_SCHEMA,
        "source": {
            "path": str(source_path),
            "member": RESOURCE_SOURCE_MEMBER,
            "file_sha256": hashlib.sha256(raw).hexdigest(),
            "size": len(raw),
        },
        "specialized_postimage": {
            "module_name": resource_contract.__name__,
            "module_file": resource_contract.__file__,
            "sha256": hashlib.sha256(specialized).hexdigest(),
            "size": len(specialized),
            "executed_from_pinned_in_memory_postimage": True,
            "registered_in_sys_modules": False,
        },
        "authority": RESOURCE_SPECIALIZATION_AUTHORITY,
    }
    return {**unsigned, "receipt_digest": object_sha256(unsigned)}


def write_resource_specialization_receipt(
    run_root: Path,
) -> tuple[Mapping[str, Any], Path, str]:
    value = _resource_specialization_receipt_value()
    path = run_root / "resource-contract-specialization-receipt.json"
    observed = _write_create_only(path, value)
    return value, path, observed


def load_resource_specialization_receipt(
    path: Path, expected_sha256: str
) -> Mapping[str, Any]:
    source = _plain_file(path, label="resource specialization receipt")
    if file_sha256(source) != _digest(
        expected_sha256, label="resource specialization receipt"
    ):
        fail("resource specialization receipt SHA-256 differs")
    value = _load_json(source, label="resource specialization receipt")
    expected = _resource_specialization_receipt_value()
    if value != expected:
        fail("resource specialization receipt identity/postimage differs")
    return value
...
def validate_release_tree(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    if (
        manifest.get("external_evidence") != R10_PARITY_EVIDENCE
        or manifest.get("topology", {}).get("fit_r13_resource_specialization")
        != RESOURCE_SPECIALIZATION_AUTHORITY
        or manifest.get("component_pins", {}).get("resource_contract_sha256")
        != RESOURCE_SOURCE_SHA256
    ):
    ...
    return {
        ...
        "resource_specialization_authority": dict(
            RESOURCE_SPECIALIZATION_AUTHORITY
        ),
...
def build_upstream_plan(
    ...
    _run_checked(
        [
            closure["python_bin"],
            "-B",
            str(method_root / "tools/reserve4_confirmation_generation_sp4_v1.py"),
            "build-plan",
            "--seed1-spec",
            str(seed1),
            "--seed2-spec",
        ...
        label="reserve4 sealed confirmation generation plan",
    )
    path = output / "reserve4-fixed-generation-plan-v2.json"
    observed = file_sha256(path)
    value = validate_upstream_confirmation_plan(path, observed)
    return path, observed, value


def validate_upstream_confirmation_plan(
    path: Path, expected_sha256: str
) -> Mapping[str, Any]:
    """Replay the generic seal, then require the exact four confirmation shards."""

    source = _plain_file(path, label="reserve4 confirmation generation plan")
    expected = _digest(expected_sha256, label="reserve4 confirmation generation plan")
    if file_sha256(source) != expected:
        fail("reserve4 confirmation generation plan SHA-256 differs")
    try:
    ...
    if replayed_path != source or observed != expected:
        fail("reserve4 confirmation generation plan replay binding differs")
    tasks = value.get("tasks")
    if (
        value.get("schema_version") != reserve4_runner.PLAN_SCHEMA
        or value.get("analysis_split") != "confirmation"
        ...
        fail("reserve4 confirmation generation plan authority differs")
    if any(
        ...
        fail("confirmation rows are not on their sealed XGMI4 groups")
    expected_shards = []
    for seed_slot in ("seed1", "seed2"):
        for group_id, visible_gpus in (
            ...
            if (
                ...
                fail("confirmation shard is not one complete ordered ten-branch cell")
            candidate_ids = [row["candidate_id"] for row in rows]
            expected_shards.append(
                {
                    "shard_id": f"{seed_slot}-{group_id}-confirmation",
                    "seed_slot": seed_slot,
                    "group_id": group_id,
    ...
    if value.get("shards") != expected_shards:
        fail("confirmation plan must close four exact serial shards")
    if [row["candidate_id"] for row in tasks] != [
        candidate_id
        for shard in expected_shards
        for candidate_id in shard["candidate_ids"]
    ]:
        fail("confirmation task order differs from sealed shard order")
    ...
    if (
        ...
        fail("confirmation plan cell semantics differ")
    return value
...
def build_controller_plan(
    ...
    specialization, specialization_path, specialization_sha = (
        write_resource_specialization_receipt(run_root)
    )
    unsigned = {
        "schema_version": PLAN_SCHEMA,
        "plan_id": "generic-action-confirmation40-generation-136309-r4",
        "phase": "generation",
        "analysis_split": "confirmation",
        "holder": {"job_id": HOLDER_JOB, "node": HOLDER_NODE},
        ...
        "resource_contract_specialization": {
            "path": str(specialization_path),
            "file_sha256": specialization_sha,
            "receipt_digest": specialization["receipt_digest"],
            "source_sha256": RESOURCE_SOURCE_SHA256,
            "specialized_sha256": RESOURCE_SPECIALIZED_SHA256,
        },
...
def validate_controller_plan(path: Path, expected_sha256: str) -> Mapping[str, Any]:
    ...
    if (
        ...
        or value.get("confirmation_scope") != CONFIRMATION_SCOPE
        ...
        or value["release"].get("resource_specialization_authority")
        != RESOURCE_SPECIALIZATION_AUTHORITY
    ):
        fail("controller plan contract differs")
    specialization = value.get("resource_contract_specialization")
    if (
        not isinstance(specialization, Mapping)
        or set(specialization)
        != {
            "path", "file_sha256", "receipt_digest", "source_sha256",
            "specialized_sha256",
        }
        or specialization.get("source_sha256") != RESOURCE_SOURCE_SHA256
        or specialization.get("specialized_sha256")
        != RESOURCE_SPECIALIZED_SHA256
    ):
        fail("controller resource specialization reference differs")
    specialization_path = _plain_file(
        str(specialization.get("path")),
        label="controller resource specialization receipt",
    )
    if specialization_path.parent != source.parent:
        fail("controller resource specialization receipt escaped the run root")
    specialization_value = load_resource_specialization_receipt(
        specialization_path, str(specialization.get("file_sha256"))
    )
    if specialization_value.get("receipt_digest") != specialization.get(
        "receipt_digest"
    ):
        fail("controller resource specialization receipt digest differs")
    ...
    replayed = validate_upstream_confirmation_plan(
...
def validate_launch_environment(path: Path, expected_sha256: str) -> Mapping[str, Any]:
    ...
    expected = {
        ...
        "GADP_RESOURCE_CONTRACT_SHA256": pins["resource_contract_sha256"],
        "GADP_RESOURCE_SPECIALIZED_SHA256": RESOURCE_SPECIALIZED_SHA256,
...
def _launch(
    ...
    launcher = _plain_file(
        method_root / "scripts/auh_generic_action_confirmation_data_prep_136309_world4_v1.sh",
        label="confirmation generation launcher",
    )
    pins = closure["component_pins"]
    r10 = plan["r10_parity_evidence"]
    environment = {
    ...
    try:
        ...
        raise
    if return_code != 0:
        fail(f"confirmation generation launcher exited {return_code}")
...
def validate_generation_output_closure(
    *,
    ...
    plan = validate_upstream_confirmation_plan(plan_path, expected_plan_sha256)
    resolved_plan = _plain_file(plan_path, label="reserve4 confirmation generation plan")
...
def seal_compile_host_memory_gate(output: Path) -> Mapping[str, Any]:
    """Seal the r13 host gate after the disposable confirmation smoke."""

    try:
        value, _ = resource_contract.write_host_cgroup_memory_gate_receipt(
            output,
            measurement_phase="compile_smoke_before_formal40",
            formal_candidate_count_at_gate=0,
        )
    except resource_contract.Reserve4GenerationError as error:
        raise GenericActionDataPrepError(str(error)) from error
    return value


def _load_physical_smoke_receipt(
    path: Path, expected_sha256: str
) -> Mapping[str, Any]:
    source = _plain_file(path, label="physical smoke receipt")
    if file_sha256(source) != _digest(
        expected_sha256, label="physical smoke receipt"
    ):
        fail("physical smoke receipt SHA-256 differs")
    value = _load_canonical_receipt(source, label="physical smoke receipt")
    unsigned = dict(value)
    declared = unsigned.pop("receipt_digest", None)
    if (
        set(value)
        != {
            "schema_version", "controller_plan", "confirmation_plan",
            "resource_contract_specialization", "candidate_id",
            "pair_receipt", "native_receipt", "resource_lifecycle",
            "world4_gpu_peak", "tensor_parity_evidence",
            "r10_tensor_parity_authority", "physical_safe_open_performed",
            "exact_single_tensor_key_and_metadata_required",
            "safetensors_container_sha256_cross_process_equivalence_required",
            "formal_candidate_count_at_gate", "disposable_output_retained",
            "generated_media_role", "training_performed",
            "optimizer_authorized", "receipt_digest",
        }
        or value.get("schema_version") != PHYSICAL_SMOKE_RECEIPT_SCHEMA
        or declared != object_sha256(unsigned)
        or value.get("physical_safe_open_performed") is not True
        or value.get("exact_single_tensor_key_and_metadata_required") is not True
        or value.get(
            "safetensors_container_sha256_cross_process_equivalence_required"
        )
        is not False
        or value.get("formal_candidate_count_at_gate") != 0
        or value.get("disposable_output_retained") is not False
        or value.get("generated_media_role")
        != "disposable-runtime-parity-smoke-not-confirmation-evidence"
        or value.get("training_performed") is not False
        or value.get("optimizer_authorized") is not False
    ):
        fail("physical smoke receipt schema/authority differs")
    controller_plan = value.get("controller_plan")
    if (
        not isinstance(controller_plan, Mapping)
        or set(controller_plan) != {"path", "file_sha256", "plan_digest"}
        or not isinstance(controller_plan.get("path"), str)
        or not Path(str(controller_plan["path"])).is_absolute()
        or SHA256_RE.fullmatch(str(controller_plan.get("file_sha256"))) is None
        or SHA256_RE.fullmatch(str(controller_plan.get("plan_digest"))) is None
    ):
        fail("physical smoke controller-plan binding differs")
    controller_plan_path = _plain_file(
        str(controller_plan["path"]), label="physical smoke controller plan"
    )
    replayed_controller = validate_controller_plan(
        controller_plan_path, str(controller_plan["file_sha256"])
    )
    if replayed_controller.get("plan_digest") != controller_plan.get(
        "plan_digest"
    ):
        fail("physical smoke controller-plan digest differs")
    specialization = value.get("resource_contract_specialization")
    if (
        not isinstance(specialization, Mapping)
        or dict(specialization)
        != replayed_controller.get("resource_contract_specialization")
    ):
        fail("physical smoke resource specialization binding differs")
    plan = value.get("confirmation_plan")
    if (
        not isinstance(plan, Mapping)
        or set(plan) != {"path", "file_sha256", "plan_digest"}
        or not isinstance(plan.get("path"), str)
        or not Path(str(plan["path"])).is_absolute()
        or SHA256_RE.fullmatch(str(plan.get("file_sha256"))) is None
        or SHA256_RE.fullmatch(str(plan.get("plan_digest"))) is None
    ):
        fail("physical smoke confirmation-plan binding differs")
    for label in ("pair_receipt", "native_receipt"):
        reference = value.get(label)
        if (
            not isinstance(reference, Mapping)
            or set(reference) != {"path", "file_sha256", "receipt_digest"}
            or not isinstance(reference.get("path"), str)
            or not Path(str(reference["path"])).is_absolute()
            or SHA256_RE.fullmatch(str(reference.get("file_sha256"))) is None
            or SHA256_RE.fullmatch(str(reference.get("receipt_digest"))) is None
        ):
            fail(f"physical smoke {label} binding differs")
    evidence = value.get("tensor_parity_evidence")
    authority = value.get("r10_tensor_parity_authority")
    if not isinstance(evidence, Mapping) or not isinstance(authority, Mapping):
        fail("physical smoke tensor/r10 evidence is absent")
    try:
        resource_contract._validate_r10_smoke_tensor_parity(  # type: ignore[attr-defined]
            evidence, authority
        )
    except resource_contract.Reserve4GenerationError as error:
        raise GenericActionDataPrepError(str(error)) from error
    reopened = evidence.get("physical_safetensors_reopen")
    if (
        not isinstance(reopened, Mapping)
        or reopened.get("performed") is not True
        or [row.get("name") for row in reopened.get("artifacts", [])]
        != ["official_initial_gaussian", "predecode_clean_latent"]
    ):
        fail("physical smoke did not reopen both exact tensors")
    peak = value.get("world4_gpu_peak")
    if (
        not isinstance(peak, Mapping)
        or peak.get("rank_count") != 4
        or peak.get("limit_gib") != T2V_GPU_MEMORY_LIMIT_GIB
        or peak.get("all_rank_peak_reserved_strictly_below_limit") is not True
        or type(peak.get("maximum_peak_reserved_bytes")) is not int
        or int(peak["maximum_peak_reserved_bytes"]) >= T2V_GPU_MEMORY_LIMIT_BYTES
    ):
        fail("physical smoke WORLD4 GPU peak gate differs")
    return value


def seal_physical_smoke_receipt(
    *,
    plan_path: Path,
    expected_plan_sha256: str,
    candidate_output: Path,
    r10_compile_smoke_receipt: Path,
    expected_r10_compile_smoke_receipt_sha256: str,
    r10_generation_log: Path,
    expected_r10_generation_log_sha256: str,
    output: Path,
) -> Mapping[str, Any]:
    """Physically reopen disposable smoke tensors before its root is deleted."""

    plan = validate_upstream_confirmation_plan(
        plan_path, expected_plan_sha256
    )
    candidate_root = _plain_directory(
        candidate_output, label="disposable smoke candidate output"
    )
    task = plan["tasks"][0]
    if (
        candidate_root.name != task["candidate_id"]
        or task.get("analysis_split") != "confirmation"
        or task.get("seed_slot") != "seed1"
        or task.get("group_id") != "sp4-a"
    ):
        fail("physical smoke is not the sealed first confirmation task")
    pair_path = _plain_file(
        candidate_root / "pair-v5-t2v-calibration-receipt.json",
        label="disposable smoke pair receipt",
    )
    try:
        pair_receipt, _ = reserve4_runner._validate_candidate_receipt(  # type: ignore[attr-defined]
            task, pair_path
        )
    except reserve4_runner.Reserve4GenerationError as error:
        raise GenericActionDataPrepError(str(error)) from error
    native_path = _plain_file(
        pair_receipt["native_receipt_path"], label="disposable smoke native receipt"
    )
    if file_sha256(native_path) != pair_receipt["native_receipt_sha256"]:
        fail("disposable smoke native receipt SHA-256 differs")
    native_receipt = _load_json(native_path, label="disposable smoke native receipt")
    run_root = output.parent.parent
    controller_plan_path = _plain_file(
        run_root / "controller-plan.json",
        label="physical smoke controller plan",
    )
    controller_plan_sha = file_sha256(controller_plan_path)
    controller_plan = validate_controller_plan(
        controller_plan_path, controller_plan_sha
    )
    try:
        tensor_evidence = resource_contract._native_smoke_tensor_evidence(  # type: ignore[attr-defined]
            native_receipt
        )
        r10_authority = resource_contract.load_r10_tensor_parity_authority(
            r10_compile_smoke_receipt,
            expected_r10_compile_smoke_receipt_sha256,
            r10_generation_log,
            expected_r10_generation_log_sha256,
        )
        resource_contract._validate_r10_smoke_tensor_parity(  # type: ignore[attr-defined]
            tensor_evidence, r10_authority
        )
        resource_contract.assert_live_host_cgroup_memory_monitor()
    except resource_contract.Reserve4GenerationError as error:
        raise GenericActionDataPrepError(str(error)) from error
    try:
        ...
        raise GenericActionDataPrepError(
            "physical smoke resource-lifecycle validation failed"
        ) from error
    residency = lifecycle.get("world4_t2v_text_encoder_gpu_residency_gate")
    rows = residency.get("rank_evidence") if isinstance(residency, Mapping) else None
    if (
        not isinstance(rows, list)
        or len(rows) != 4
        or residency.get("all_rank_peak_reserved_within_52_gib") is not True
        or any(type(row.get("gpu_peak_reserved_bytes")) is not int for row in rows)
    ):
        fail("physical smoke WORLD4 GPU residency evidence differs")
    maximum = max(int(row["gpu_peak_reserved_bytes"]) for row in rows)
    if maximum >= T2V_GPU_MEMORY_LIMIT_BYTES:
        fail("physical smoke WORLD4 rank peak reserved reached 52 GiB")
    unsigned = {
        "schema_version": PHYSICAL_SMOKE_RECEIPT_SCHEMA,
        "controller_plan": {
        ...
        "confirmation_plan": {
            "path": str(_plain_file(plan_path, label="confirmation plan")),
            "file_sha256": expected_plan_sha256,
            "plan_digest": plan["plan_digest"],
        },
        "resource_contract_specialization": dict(
            controller_plan["resource_contract_specialization"]
        ),
        "candidate_id": task["candidate_id"],
        "pair_receipt": {
            "path": str(pair_path),
            "file_sha256": file_sha256(pair_path),
            "receipt_digest": pair_receipt["receipt_digest"],
        },
        "native_receipt": {
            "path": str(native_path),
            "file_sha256": file_sha256(native_path),
            "receipt_digest": pair_receipt["native_receipt_digest"],
        },
        "resource_lifecycle": lifecycle,
        "world4_gpu_peak": {
            "rank_count": 4,
            "limit_gib": T2V_GPU_MEMORY_LIMIT_GIB,
            "limit_bytes": T2V_GPU_MEMORY_LIMIT_BYTES,
            "maximum_peak_reserved_bytes": maximum,
            "all_rank_peak_reserved_strictly_below_limit": True,
        },
        "tensor_parity_evidence": tensor_evidence,
        "r10_tensor_parity_authority": r10_authority,
        "physical_safe_open_performed": True,
    ...
    return _load_physical_smoke_receipt(output, observed)
...
def _expected_gpu_admission(
    ...
    smoke_host_path = _plain_file(
        run_root / "logs/host-cgroup-memory-compile-smoke-gate.json",
        label="compile-smoke host memory gate",
    )
    smoke_host_sha = file_sha256(smoke_host_path)
    try:
        smoke_host, _, _ = resource_contract.load_host_cgroup_memory_gate(
            smoke_host_path,
            smoke_host_sha,
        ...
        raise GenericActionDataPrepError(str(error)) from error
    physical_smoke_path = _plain_file(
        run_root / "logs/compile-smoke-physical-tensor-evidence.json",
        label="physical smoke receipt",
    )
    physical_smoke_sha = file_sha256(physical_smoke_path)
    physical_smoke = _load_physical_smoke_receipt(
        physical_smoke_path, physical_smoke_sha
    )
    if (
        physical_smoke["candidate_id"]
        != smoke_receipt["smoke_task"]["candidate_id"]
        or physical_smoke["pair_receipt"]["receipt_digest"]
        != smoke_receipt["candidate_evidence"]["candidate_receipt_digest"]
        or physical_smoke["native_receipt"]["receipt_digest"]
        != smoke_receipt["candidate_evidence"]["native_receipt_digest"]
        or physical_smoke["confirmation_plan"] != smoke_receipt["plan"]
    ):
        fail("physical smoke/compile receipt binding differs")
    unsigned = {
        ...
        "compile_smoke": {
            ...
            "physical_tensor_receipt": {
                "path": str(physical_smoke_path),
                "file_sha256": physical_smoke_sha,
                "receipt_digest": physical_smoke["receipt_digest"],
            },
            "tensor_parity_evidence": physical_smoke["tensor_parity_evidence"],
            "host_cgroup_memory_gate_reference": {
                "path": str(smoke_host_path),
                "file_sha256": smoke_host_sha,
                "receipt_digest": smoke_host["receipt_digest"],
            },
            ...
            "physical_safe_open_before_disposable_delete_asserted": True,
            "disposable_smoke_is_confirmation_evidence": False,
            "world_size": 4,
    ...
    *,
    ...
    try:
        validated, replayed_path, replayed_sha = (
            resource_contract.load_host_cgroup_memory_gate(
                path,
                file_sha256(path),
                expected_phase=(
...
def validate_completion(
    ...
    for identity in GPU_SHARD_IDENTITIES:
        ...
        binding_rows.append(
            {
                "ordinal": identity[0],
                "shard_id": f"{identity[1]}-{identity[2]}-confirmation",
                "path": str(path),
                "file_sha256": sha,
    ...
    result_path = _plain_file(
        run_root / "generation-audit.json", label="confirmation generation audit"
    )
    generation_roots = [
    ...
    unsigned = {
        ...
        "compile_smoke_physical_tensor_receipt_path": compile_smoke[
            "physical_tensor_receipt"
        ]["path"],
        "compile_smoke_physical_tensor_receipt_file_sha256": compile_smoke[
            "physical_tensor_receipt"
        ]["file_sha256"],
        "compile_smoke_physical_tensor_receipt_digest": compile_smoke[
            "physical_tensor_receipt"
        ]["receipt_digest"],
        "compile_smoke_tensor_parity_evidence_digest": object_sha256(
            compile_smoke["tensor_parity_evidence"]
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    compile_host = commands.add_parser("seal-compile-host-memory-gate")
    compile_host.add_argument("--output", required=True)
    physical_smoke = commands.add_parser("seal-physical-smoke-receipt")
    physical_smoke.add_argument("--plan", required=True)
    physical_smoke.add_argument("--expected-plan-sha256", required=True)
    physical_smoke.add_argument("--candidate-output", required=True)
    physical_smoke.add_argument("--r10-compile-smoke-receipt", required=True)
    physical_smoke.add_argument(
        "--expected-r10-compile-smoke-receipt-sha256", required=True
    )
    physical_smoke.add_argument("--r10-generation-log", required=True)
    physical_smoke.add_argument(
        "--expected-r10-generation-log-sha256", required=True
    )
    physical_smoke.add_argument("--output", required=True)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "seal-gpu-admission-receipt":
        ...
        print(canonical_json_bytes(value).decode("ascii"), flush=True)
        return 0
    if args.command == "seal-compile-host-memory-gate":
        value = seal_compile_host_memory_gate(Path(args.output))
        print(canonical_json_bytes(value).decode("ascii"), flush=True)
        return 0
    if args.command == "seal-physical-smoke-receipt":
        value = seal_physical_smoke_receipt(
            plan_path=Path(args.plan),
            expected_plan_sha256=args.expected_plan_sha256,
            candidate_output=Path(args.candidate_output),
            r10_compile_smoke_receipt=Path(args.r10_compile_smoke_receipt),
            expected_r10_compile_smoke_receipt_sha256=(
                args.expected_r10_compile_smoke_receipt_sha256
            ),
            r10_generation_log=Path(args.r10_generation_log),
            expected_r10_generation_log_sha256=(
