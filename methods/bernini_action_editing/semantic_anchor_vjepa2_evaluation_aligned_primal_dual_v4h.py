#!/usr/bin/env python3
"""Unreleased v4-H evaluation-aligned primal-dual codec.
...
trainable parameters, exact outer/inner splits, optimizer, batch size, fixed
1,200-step budget, unchanged v4-F inner/final gate statistics and thresholds,
and a v4-H-specific deterministic inner-bootstrap namespace.

The sole scientific change frozen before any v4-H execution is an
evaluation-aligned deterministic primal-dual hinge.  Its fixed order is five
raw-MSE ratio-of-means fidelity constraints followed by three decoded-margin
retention constraints.  The primal objective is the mean of the five ratios;
the eight duals start at zero and update only after each AdamW step with
``eta=1/sqrt(1200)``.  There is no cap, rho, hand loss weight, sweep, early
stopping, or checkpoint selection.  Independent binary constraint, dual, and
objective traces are create-only, strongly reloaded, and receipt/checkpoint
bound.
...
independently repeats it before the first OOF tensor request.  One failed inner
gate therefore leaves all-fold OOF reads exact zero and forbids aggregation.
...
SCHEMA = "semantic-anchor-vjepa2-evaluation-aligned-primal-dual-exact5-receipt-v4h"
INNER_SCHEMA = "semantic-anchor-vjepa2-evaluation-aligned-primal-dual-inner-receipt-v4h"
FOLD_SCHEMA = "semantic-anchor-vjepa2-evaluation-aligned-primal-dual-fold-receipt-v4h"
CHECKPOINT_SCHEMA = "semantic-anchor-vjepa2-evaluation-aligned-primal-dual-checkpoint-v4h"
BARRIER_SCHEMA = (
    "semantic-anchor-vjepa2-evaluation-aligned-primal-dual-global-inner-barrier-v4h"
)
STATUS = "V4H_EVALUATION_ALIGNED_PRIMAL_DUAL_KNOWN_EXPOSED_DEVELOPMENT"
INNER_PASS_STATUS = "V4H_FIXED1200_INNER_PASS_OOF_UNREAD"
INNER_NO_GO_STATUS = "V4H_FIXED1200_INNER_NO_GO_ALL_OOF_UNREAD"
BARRIER_PASS_STATUS = "V4H_EXACT5_INNER_BARRIER_PASS_OOF_UNREAD"
...
V4F_RUNTIME_DEPENDENCY_SHA256 = "PLACEHOLDER_UNSEALED_V4F_RUNTIME_SHA256"
V4F_BURNED_EXACT22_INNER_NO_GO_SEAL_SHA256 = (
    "ff5a32ae4047f3408da9954711c4e76069eb06edc60f10078833ba961812fcab"
)
V4F_BURNED_EXACT22_INNER_NO_GO_SEAL_SELF_DIGEST = (
    "8971b2910262b85948670b5531ad0df30cf8336280342dc8c9245e1110d95a2d"
)
V4G_BURNED_EXACT26_MANIFEST_SHA256 = (
    "14bf42749c97b20934a2a088a560fa23ed2b1e37555262e9d4c7f2f368e74265"
)
V4G_BURNED_INNER_RECEIPT_SHA256_BY_FOLD = (
...
V4G_BURNED_CONTROLLER_STDERR_SHA256 = (
...
SCIENTIFIC_DESIGN_FROZEN_BEFORE_V4H_EXECUTION = True
SCIENTIFIC_DESIGN_FROZEN_AFTER_SEALED_V4G_RESULT = True
...
INNER_BOOTSTRAP_NAMESPACE = "v4h"
NEGATIVES = frozen.NEGATIVES
EVAL_VIEWS = frozen.EVAL_VIEWS
ClipPCAFit = frozen.ClipPCAFit

FIDELITY_CONSTRAINT_LIMIT = 1.02
RETENTION_CONSTRAINT_TARGET = 0.85
TEACHER_PAIR_SCALE_EPSILON = 1.0e-8
DUAL_LEARNING_RATE = 1.0 / math.sqrt(FIXED_SELECTED_STEP)
CONSTRAINT_ORDER = tuple(
    [f"fidelity:{view}" for view in EVAL_VIEWS]
    + [f"retention:{negative}" for negative in NEGATIVES]
)
CONSTRAINT_COUNT = 8
CONSTRAINT_TRACE_SHAPE = (FIXED_SELECTED_STEP, CONSTRAINT_COUNT)
DUAL_TRACE_SHAPE = (FIXED_SELECTED_STEP + 1, CONSTRAINT_COUNT)
OBJECTIVE_TRACE_SHAPE = (FIXED_SELECTED_STEP, 2)
OBJECTIVE_TRACE_COLUMNS = ("P_t", "L_t")
TRACE_DTYPE = torch.float64
TRACE_SCHEMA = "semantic-anchor-vjepa2-primal-dual-training-trace-v4h"
TRACE_METADATA_KEYS = frozenset({
    "schema_version", "outer_fold", "fixed_step",
    "full_budget_steps_executed", "config_sha256",
    "implementation_sha256", "minibatch_schedule_sha256",
    "runtime_fingerprint",
    "model_fit_original_count", "model_fit_ordered_iids",
    "model_fit_iid_digest",
    "model_fit_original_tensor_sha256",
    "model_fit_all_five_views_tensor_sha256", "model_state_sha256",
    "step0_state_sha256", "final_step_state_sha256",
    "fixed_clip_pca_b384_all_five_views_tensor_sha256", "trace_dtype",
    "constraint_order", "constraint_trace_shape",
    "constraint_trace_tensor_sha256", "dual_trace_shape",
    "dual_trace_tensor_sha256", "objective_trace_shape",
    "objective_trace_columns", "objective_trace_tensor_sha256",
    "training_trace_summary_sha256",
    "formula_contract", "formula_contract_sha256",
    "g_and_P_from_same_pre_adamw_forward",
    "cpu_float64_lambda_cast_to_model_float32_for_primal",
    "adamw_step_completed_before_cpu_float64_dual_update",
    "dual_update_uses_same_recorded_cpu_float64_g",
    "objective_rows_are_pre_adamw_P_t_and_L_t", "metadata_digest",
})
...
class Config:
    ...
    fidelity_constraint_limit: float = FIDELITY_CONSTRAINT_LIMIT
    retention_constraint_target: float = RETENTION_CONSTRAINT_TARGET
    teacher_pair_scale_epsilon: float = TEACHER_PAIR_SCALE_EPSILON
    dual_learning_rate: float = DUAL_LEARNING_RATE

    def validate(self) -> None:
        if self != Config():
            raise ValueError("v4-H configuration is immutable")
        if (
            ...
            or SCIENTIFIC_DESIGN_FROZEN_BEFORE_V4H_EXECUTION is not True
            or SCIENTIFIC_DESIGN_FROZEN_AFTER_SEALED_V4G_RESULT is not True
            or (CODE_TIME, CODE_CHANNELS, CODE_NUMEL) != (12, 32, 384)
            or EXACT_TRAINABLE_PARAMETERS != 79040
            or EVAL_VIEWS != (
                "original", "monotone_warp", "reverse", "block_shuffle",
                "phase_swap",
            )
            or NEGATIVES != ("reverse", "block_shuffle", "phase_swap")
            or CONSTRAINT_ORDER != (
                "fidelity:original", "fidelity:monotone_warp",
                "fidelity:reverse", "fidelity:block_shuffle",
                "fidelity:phase_swap", "retention:reverse",
                "retention:block_shuffle", "retention:phase_swap",
            )
            or self.fidelity_constraint_limit != 1.02
            or self.retention_constraint_target != 0.85
            or self.teacher_pair_scale_epsilon != 1.0e-8
            or self.dual_learning_rate != 1.0 / math.sqrt(1200)
        ):
            raise ValueError("v4-H fixed-candidate primal-dual contract differs")
...
def _require_release_sealed() -> None:
    ...
    if RELEASE_SEALED is not True:
        raise RuntimeError(
            "UNSEALED v4-H primal-dual candidate: detached release not sealed"
        )
...
def _scientific_design_origin_contract() -> dict[str, Any]:
    """Canonical result provenance for proposing the one v4-H candidate."""

    v4f_burned_source = {
        "schema_version": "semantic-anchor-vjepa2-v4f-burned-source-exact22-v1",
        "receipt_name": "inner-no-go-seal.json",
        "receipt_file_sha256": V4F_BURNED_EXACT22_INNER_NO_GO_SEAL_SHA256,
        "receipt_self_digest": (
            V4F_BURNED_EXACT22_INNER_NO_GO_SEAL_SELF_DIGEST
        ),
        "controller_complete": False,
        "scientific_no_go_valid": True,
    }
    v4g_burned_source = {
        "schema_version": "semantic-anchor-vjepa2-v4g-burned-source-exact26-v1",
        "exact26_manifest_sha256": V4G_BURNED_EXACT26_MANIFEST_SHA256,
        "inner_receipt_sha256_by_fold": list(
            V4G_BURNED_INNER_RECEIPT_SHA256_BY_FOLD
        ),
        "controller_stderr_sha256": V4G_BURNED_CONTROLLER_STDERR_SHA256,
        "controller_complete": False,
        "scientific_all_inner_no_go": True,
        "all_oof_semantic_tensor_read_count_exact0": True,
    }
    return {
        "schema_version": "semantic-anchor-vjepa2-v4h-design-origin-v1",
        "chronology": (
            "after_v4g_burned_scientific_all_inner_no_go_"
            "before_any_v4h_execution"
        ),
        "canonical_v4f_burned_source": v4f_burned_source,
        "canonical_v4f_burned_source_sha256": _object_sha(v4f_burned_source),
        "canonical_v4g_burned_source": v4g_burned_source,
        "canonical_v4g_burned_source_sha256": _object_sha(v4g_burned_source),
        "release_manifest_must_pin_v4g_recovery_attestation": True,
        "sealed_v4f_v4g_results_used_only_to_propose_unique_v4h_design": True,
        "sealed_v4f_v4g_result_data_used_as_v4h_training_or_gate_data": False,
        "v4h_internal_parameter_candidate_or_checkpoint_sweep_performed": False,
    }
...
def _config_value(config: Config) -> dict[str, Any]:
    return {
        ...
        "inner_bootstrap_seed_namespace": INNER_BOOTSTRAP_NAMESPACE,
        "teacher_retention": config.teacher_retention,
        "recon_ratio_limit": config.recon_ratio_limit,
        "fidelity_constraint_limit": config.fidelity_constraint_limit,
        "retention_constraint_target": config.retention_constraint_target,
        "teacher_pair_scale_epsilon": config.teacher_pair_scale_epsilon,
        "dual_learning_rate": config.dual_learning_rate,
        "constraint_order": list(CONSTRAINT_ORDER),
        "constraint_count": CONSTRAINT_COUNT,
        "constraint_trace_shape": list(CONSTRAINT_TRACE_SHAPE),
        "dual_trace_shape": list(DUAL_TRACE_SHAPE),
        "objective_trace_shape": list(OBJECTIVE_TRACE_SHAPE),
        "objective_trace_columns": list(OBJECTIVE_TRACE_COLUMNS),
        ...
        "scientific_design_frozen_before_v4h_execution": True,
        "scientific_design_frozen_after_sealed_v4g_result": True,
        "sealed_v4f_v4g_results_used_to_propose_unique_v4h_design": True,
        "scientific_design_origin": _scientific_design_origin_contract(),
        "scientific_design_origin_sha256": _object_sha(
            _scientific_design_origin_contract()
        ),
        "v4h_internal_sweep_performed": False,
        "dual_initialization_exact_zero": True,
        "adamw_primal_step_before_projected_dual_update": True,
        "dual_cap_used": False,
        "rho_used": False,
        "manual_loss_weight_used": False,
    }
...
class ClipPCAInitializedVJepaGlobalCodec(nn.Module):
    ...
    def __init__(self, fitted: ClipPCAFit, fit_only_rms: torch.Tensor) -> None:
        ...
        if count != EXACT_TRAINABLE_PARAMETERS or count >= MAX_TRAINABLE_PARAMETERS:
            raise RuntimeError("v4-H codec parameter closure differs")
...
def _training_formula_contract() -> dict[str, Any]:
    """Canonical, hashable statement of the sole v4-H optimizer contract."""

    return {
        "view_order": list(EVAL_VIEWS),
        "negative_order": list(NEGATIVES),
        "constraint_order": list(CONSTRAINT_ORDER),
        "raw_mse": "mean_over_time_and_feature((prediction-target)^2)",
        "fidelity_ratio": (
            "sum_i(candidate_raw_mse_i_v)/sum_i(b384_raw_mse_i_v)"
        ),
        "fidelity_raw_denominator_rule": "finite_and_strictly_positive_no_epsilon",
        "primal": "P_t=mean_over_five_views(R_v_t)",
        "fidelity_constraint": "gF_v_t=R_v_t-1.02",
        "decoded_margin": (
            "m_n(x)=d(x_original,x_n)-d(x_original,x_monotone_warp)"
        ),
        "teacher_scale": "s_i=mean_all_ten_teacher_pair_distances+1e-8",
        "retention_constraint": (
            "gR_n_t=(0.85*mean_i_teacher_margin_i_n-"
            "mean_i_candidate_margin_i_n)/mean_i_s_i"
        ),
        "hinge": "relu(g)=max(g,0)",
        "loss": "L_t=P_t+sum_k(lambda_t_k*relu(g_t_k))",
        "dual_initialization": "lambda_0=zeros(8),CPU,float64",
        "dual_learning_rate": DUAL_LEARNING_RATE,
        "update_order": (
            "pre-AdamW forward yields P_t,g_t; AdamW updates theta; then "
            "lambda_{t+1}=max(0,lambda_t+eta*stopgrad(g_t)) on CPU float64"
        ),
        "primal_dual_cast": (
            "CPU-float64 lambda_t cast to model device/dtype for primal only"
        ),
        "dual_cap": None,
        "rho": None,
        "manual_loss_weight": None,
        "candidate_or_checkpoint_selection": None,
    }


def _training_trace_summary(
    constraint_trace: torch.Tensor, dual_trace: torch.Tensor,
    objective_trace: torch.Tensor,
) -> dict[str, Any]:
    """Canonical receipt summary derived only from the three trace tensors."""

    return {
        "constraint_order": list(CONSTRAINT_ORDER),
        "constraint_trace_shape": list(constraint_trace.shape),
        "constraint_trace_tensor_sha256": _tensor_sha(constraint_trace),
        "dual_trace_shape": list(dual_trace.shape),
        "dual_trace_tensor_sha256": _tensor_sha(dual_trace),
        "objective_trace_shape": list(objective_trace.shape),
        "objective_trace_columns": list(OBJECTIVE_TRACE_COLUMNS),
        "objective_trace_tensor_sha256": _tensor_sha(objective_trace),
        "dual_initial_values": [float(value) for value in dual_trace[0]],
        "dual_final_values": [float(value) for value in dual_trace[-1]],
        "dual_max_values": [
            float(value) for value in dual_trace.max(dim=0).values
        ],
        "constraint_active_step_counts": [
            int(value) for value in (constraint_trace > 0).sum(dim=0)
        ],
    }


def _trace_metadata_unsigned(
    *, fold_index: int, training: Mapping[str, Any],
    run_binding: Mapping[str, str], constraint_trace: torch.Tensor,
    dual_trace: torch.Tensor, objective_trace: torch.Tensor,
) -> dict[str, Any]:
    return {
        ...
        "model_fit_original_tensor_sha256": training[
            "model_fit_original_tensor_sha256"
        ],
        "model_fit_all_five_views_tensor_sha256": training[
            "model_fit_all_five_views_tensor_sha256"
        ],
        "model_state_sha256": training["final_step_state_sha256"],
        "step0_state_sha256": training["step0_state_sha256"],
        "final_step_state_sha256": training["final_step_state_sha256"],
        "fixed_clip_pca_b384_all_five_views_tensor_sha256": training[
        ...
        "objective_trace_columns": list(OBJECTIVE_TRACE_COLUMNS),
        "objective_trace_tensor_sha256": _tensor_sha(objective_trace),
        "training_trace_summary_sha256": _object_sha(
            _training_trace_summary(
                constraint_trace, dual_trace, objective_trace
            )
        ),
        "formula_contract": _training_formula_contract(),
        "formula_contract_sha256": _object_sha(_training_formula_contract()),
        "g_and_P_from_same_pre_adamw_forward": True,
        "cpu_float64_lambda_cast_to_model_float32_for_primal": True,
        "adamw_step_completed_before_cpu_float64_dual_update": True,
        "dual_update_uses_same_recorded_cpu_float64_g": True,
        "objective_rows_are_pre_adamw_P_t_and_L_t": True,
    }


def _load_primal_dual_trace_sealed(
    path: Path, expected: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, torch.Tensor], dict[str, Any]]:
    """Strongly load and semantically replay the exact-four-key trace."""

    required = {
        "outer_fold", "implementation_sha256", "minibatch_schedule_sha256",
        "runtime_fingerprint", "model_state_sha256",
        "fixed_clip_pca_b384_all_five_views_tensor_sha256",
        "model_fit_original_count", "model_fit_ordered_iids",
        "model_fit_iid_digest",
        "model_fit_original_tensor_sha256",
        "model_fit_all_five_views_tensor_sha256", "step0_state_sha256",
    }
    if not required.issubset(expected):
        raise RuntimeError("primal-dual trace expectation is incomplete")
    if (
        not path.is_absolute() or path.is_symlink()
        or str(path) != str(path.resolve(strict=True))
    ):
        raise ValueError("primal-dual trace path must be canonical/non-symlink")
    before = path.lstat()
    if (
        ...
        raise RuntimeError("primal-dual trace pre-open seal differs")
    ...
    with os.fdopen(descriptor, "rb") as handle:
        ...
        physical = {
            ...
            "nlink": before.st_nlink, "mtime_ns": before.st_mtime_ns,
        ...
        if (
            ...
            raise RuntimeError(
                "primal-dual trace expected binding differs before torch parse"
            )
        handle.seek(0)
    ...
    if (
        ...
        raise RuntimeError("primal-dual trace single-FD identity/SHA differs")
    if type(payload) is not dict or set(payload) != {
        "metadata", "constraint_trace", "dual_trace", "objective_trace",
    }:
        raise RuntimeError("primal-dual trace envelope is not exact4 keys")
    ...
    if type(metadata) is not dict or set(metadata) != TRACE_METADATA_KEYS:
        raise RuntimeError("primal-dual trace metadata envelope differs")
    unsigned = dict(metadata)
    metadata_digest = unsigned.pop("metadata_digest", None)
    expected_shapes = {
        "constraint_trace": CONSTRAINT_TRACE_SHAPE,
        "dual_trace": DUAL_TRACE_SHAPE,
        "objective_trace": OBJECTIVE_TRACE_SHAPE,
    }
    if (
        metadata_digest != _object_sha(unsigned)
        or metadata.get("schema_version") != TRACE_SCHEMA
        ...
        or metadata.get("config_sha256")
            != _object_sha(_config_value(Config()))
        ...
        or metadata.get("final_step_state_sha256")
            != expected.get("model_state_sha256")
        or metadata.get("step0_state_sha256")
            != expected.get("step0_state_sha256")
        or metadata.get("model_fit_original_count")
            != expected.get("model_fit_original_count")
        or metadata.get("model_fit_ordered_iids")
            != expected.get("model_fit_ordered_iids")
        or metadata.get("model_fit_iid_digest")
            != expected.get("model_fit_iid_digest")
        or metadata.get("model_fit_original_tensor_sha256")
            != expected.get("model_fit_original_tensor_sha256")
        or metadata.get("model_fit_all_five_views_tensor_sha256")
            != expected.get("model_fit_all_five_views_tensor_sha256")
        or metadata.get("fixed_clip_pca_b384_all_five_views_tensor_sha256")
            != expected.get(
                "fixed_clip_pca_b384_all_five_views_tensor_sha256"
            )
        or metadata.get("formula_contract") != _training_formula_contract()
        or metadata.get("formula_contract_sha256")
            != _object_sha(_training_formula_contract())
        or metadata.get("trace_dtype") != str(TRACE_DTYPE)
        or metadata.get("constraint_order") != list(CONSTRAINT_ORDER)
        or metadata.get("objective_trace_columns")
            != list(OBJECTIVE_TRACE_COLUMNS)
        or metadata.get("g_and_P_from_same_pre_adamw_forward") is not True
        or metadata.get(
            "cpu_float64_lambda_cast_to_model_float32_for_primal"
        ) is not True
        or metadata.get(
        ...
        or metadata.get("dual_update_uses_same_recorded_cpu_float64_g")
            is not True
        or metadata.get("objective_rows_are_pre_adamw_P_t_and_L_t")
            is not True
        or expected.get("metadata_digest") not in (None, metadata_digest)
    ):
        raise RuntimeError("primal-dual trace semantic metadata differs")
    for name, tensor in tensors.items():
        if (
            type(tensor) is not torch.Tensor or tensor.device.type != "cpu"
            or tensor.dtype != TRACE_DTYPE
            or tuple(tensor.shape) != expected_shapes[name]
            or not tensor.is_contiguous()
            or not bool(torch.isfinite(tensor).all())
            or metadata.get(f"{name}_shape") != list(expected_shapes[name])
            or metadata.get(f"{name}_tensor_sha256") != _tensor_sha(tensor)
            or expected.get(f"{name}_tensor_sha256") not in (
                None, _tensor_sha(tensor),
            )
        ):
            raise RuntimeError(f"{name} tensor closure differs")
    constraint_trace = tensors["constraint_trace"]
    dual_trace = tensors["dual_trace"]
    objective_trace = tensors["objective_trace"]
    trace_summary = _training_trace_summary(
        constraint_trace, dual_trace, objective_trace
    )
    if metadata.get("training_trace_summary_sha256") != _object_sha(
        trace_summary
    ):
        raise RuntimeError("primal-dual trace canonical training summary differs")
    replay_dual = torch.clamp_min(
        dual_trace[:-1]
        + Config().dual_learning_rate * constraint_trace,
        0.0,
    )
    if (
        not torch.equal(dual_trace[0], torch.zeros(CONSTRAINT_COUNT,
                                                   dtype=TRACE_DTYPE))
        or bool((dual_trace < 0).any())
        or not torch.equal(dual_trace[1:], replay_dual)
    ):
        raise RuntimeError("primal-dual trace exact recurrence differs")
    replay_losses = []
    for step in range(FIXED_SELECTED_STEP):
        primal32 = objective_trace[step, 0].to(torch.float32)
        constraints32 = constraint_trace[step].to(torch.float32)
        duals32 = dual_trace[step].to(torch.float32)
        penalty32 = torch.zeros((), dtype=torch.float32)
        for index in range(CONSTRAINT_COUNT):
            penalty32 = penalty32 + (
                duals32[index] * torch.relu(constraints32[index])
            )
        replay_losses.append((primal32 + penalty32).to(TRACE_DTYPE))
    if not torch.equal(objective_trace[:, 1], torch.stack(replay_losses)):
        raise RuntimeError("primal-dual objective trace exact replay differs")
    binding = {
        ...
        "constraint_trace_tensor_sha256": metadata[
            "constraint_trace_tensor_sha256"
        ],
        "dual_trace_tensor_sha256": metadata["dual_trace_tensor_sha256"],
        "objective_trace_tensor_sha256": metadata[
            "objective_trace_tensor_sha256"
        ],
        "training_trace_summary_sha256": _object_sha(trace_summary),
        "single_fd_pre_post_sha256_exact": True,
        "semantic_formula_and_recurrence_replayed": True,
    }
    if expected.get("physical_identity") not in (None, physical):
        raise RuntimeError("primal-dual trace expected physical identity differs")
    return metadata, tensors, binding


def _save_primal_dual_trace_create_only(
    path: Path, *, fold_index: int, training: Mapping[str, Any],
    run_binding: Mapping[str, str], constraint_trace: torch.Tensor,
    dual_trace: torch.Tensor, objective_trace: torch.Tensor,
) -> dict[str, Any]:
    if (
        ...
        raise ValueError("primal-dual trace must be a fresh absolute child")
    metadata = _trace_metadata_unsigned(
        fold_index=fold_index, training=training, run_binding=run_binding,
    ...
    envelope = {
        "metadata": metadata,
        "constraint_trace": constraint_trace.detach().cpu().contiguous().clone(),
        "dual_trace": dual_trace.detach().cpu().contiguous().clone(),
        "objective_trace": objective_trace.detach().cpu().contiguous().clone(),
    }
    if not hasattr(os, "O_NOFOLLOW"):
        raise RuntimeError("primal-dual trace requires O_NOFOLLOW")
    descriptor = os.open(
        path,
    ...
    with os.fdopen(descriptor, "w+b") as handle:
        ...
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            creation_digest.update(chunk)
        creation_sha = creation_digest.hexdigest()
        creation_stat = os.fstat(handle.fileno())
    creation_physical = {
        "device": creation_stat.st_dev, "inode": creation_stat.st_ino,
        "size_bytes": creation_stat.st_size, "mode_octal": "0444",
        "nlink": creation_stat.st_nlink,
        "mtime_ns": creation_stat.st_mtime_ns,
        "ctime_ns": creation_stat.st_ctime_ns,
    }
    expected = {
        **metadata,
        "metadata_digest": metadata["metadata_digest"],
    }
    loaded_metadata, loaded_tensors, binding = (
        _load_primal_dual_trace_sealed(path, expected)
    )
    if (
        loaded_metadata != metadata
        or any(not torch.equal(loaded_tensors[name], envelope[name])
               for name in loaded_tensors)
        or binding["file_sha256"] != creation_sha
        or binding["physical_identity"] != creation_physical
    ):
        raise RuntimeError("fresh primal-dual trace strong reload differs")
    return binding


def _evaluation_aligned_primal_dual_loss(
    prediction: torch.Tensor, target: torch.Tensor, baseline: torch.Tensor,
    duals: torch.Tensor, config: Config,
) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
    """Return the sole preregistered primal loss and ordered constraints."""

    config.validate()
    if (
        prediction.shape != target.shape or baseline.shape != target.shape
        or prediction.ndim != 4 or prediction.shape[0] == 0
        or prediction.shape[1] != len(EVAL_VIEWS)
        or tuple(prediction.shape[-2:]) != (TIME_STEPS, FEATURE_DIM)
        or tuple(duals.shape) != (CONSTRAINT_COUNT,)
        or prediction.dtype != target.dtype or baseline.dtype != target.dtype
        or duals.dtype != target.dtype or duals.device != target.device
        or bool((duals < 0).any())
    ):
        raise ValueError("evaluation-aligned primal-dual geometry differs")
    if not all(bool(torch.isfinite(value).all()) for value in (
        prediction, target, baseline, duals,
    )):
        raise RuntimeError("primal-dual input is non-finite")

    candidate_mse = (prediction - target).square().mean(dim=(2, 3))
    baseline_mse = (baseline - target).square().mean(dim=(2, 3))
    fidelity_denominators = baseline_mse.sum(dim=0)
    if (
        tuple(fidelity_denominators.shape) != (len(EVAL_VIEWS),)
        or not bool(torch.isfinite(fidelity_denominators).all())
        or not bool((fidelity_denominators > 0).all())
    ):
        raise RuntimeError(
            "raw ratio-of-means baseline denominator must be finite and >0"
        )
    fidelity_ratios = candidate_mse.sum(dim=0) / fidelity_denominators
    primal = fidelity_ratios.mean()
    ...
    constraints = torch.cat((fidelity_constraints, retention_constraints))
    dual_penalty = prediction.new_zeros(())
    for index in range(CONSTRAINT_COUNT):
        dual_penalty = dual_penalty + (
            duals[index].detach() * torch.relu(constraints[index])
        )
    loss = primal + dual_penalty
    if (
        tuple(constraints.shape) != (CONSTRAINT_COUNT,)
        or not bool(torch.isfinite(teacher_scale_mean))
        or not bool(teacher_scale_mean > 0)
        or not bool(torch.isfinite(teacher_margins).all())
        or not bool(torch.isfinite(candidate_margins).all())
        or not bool(torch.isfinite(constraints).all())
        or not bool(torch.isfinite(primal))
        or not bool(torch.isfinite(loss))
    ):
        raise RuntimeError("primal objective or constraint is non-finite")
    components: dict[str, Any] = {
        "constraint_order": list(CONSTRAINT_ORDER),
        "fidelity_ratio_of_means": [
            float(value) for value in fidelity_ratios.detach().cpu()
        ],
        "fidelity_constraints": [
            float(value) for value in fidelity_constraints.detach().cpu()
        ],
        "teacher_margin_means": [
            float(value) for value in teacher_margins.mean(dim=0).cpu()
        ],
        "candidate_margin_means": [
            float(value) for value in candidate_margins.detach().mean(dim=0).cpu()
        ],
        "teacher_pair_scale_mean": float(teacher_scale_mean.detach().cpu()),
        "retention_constraints": [
            float(value) for value in retention_constraints.detach().cpu()
        ],
        "primal_mean_five_ratio_of_means": float(primal.detach().cpu()),
        "dual_values_used_for_primal_step": [
            float(value) for value in duals.detach().cpu()
        ],
        "total": float(loss.detach().cpu()),
    }
    return loss, constraints, components
...
def _train_fold_model(
    ...
    VJepa2GlobalCodec, dict[str, Any], torch.Tensor, torch.Tensor, torch.Tensor,
]:
    ...
    with torch.no_grad():
        baseline_views = frozen._analytic_clip_pca_decode(
            fit_views.flatten(0, 1), fitted
        ).reshape_as(fit_views)
    if not bool(torch.isfinite(baseline_views).all()):
        raise RuntimeError("fixed clip-PCA-B384 model-fit baseline is non-finite")
    ...
    duals = torch.zeros(CONSTRAINT_COUNT, dtype=TRACE_DTYPE, device="cpu")
    constraint_trace = torch.empty(CONSTRAINT_TRACE_SHAPE, dtype=TRACE_DTYPE)
    dual_trace = torch.empty(DUAL_TRACE_SHAPE, dtype=TRACE_DTYPE)
    objective_trace = torch.empty(OBJECTIVE_TRACE_SHAPE, dtype=TRACE_DTYPE)
    dual_trace[0].copy_(duals.detach().cpu())
    model.train()
    for step in range(1, config.max_steps + 1):
        indices = minibatches[step - 1].to(device)
        target = fit_views.index_select(0, indices)
        baseline = baseline_views.index_select(0, indices)
        prediction = model(target.flatten(0, 1)).reshape_as(target)
        primal_duals = duals.to(device=device, dtype=target.dtype)
        loss, constraints, last_components = (
            _evaluation_aligned_primal_dual_loss(
                prediction, target, baseline, primal_duals, config,
            )
        )
        ...
        constraint_row = constraints.detach().to(
            dtype=TRACE_DTYPE, device="cpu"
        )
        constraint_trace[step - 1].copy_(constraint_row)
        objective_trace[step - 1, 0] = last_components[
            "primal_mean_five_ratio_of_means"
        ]
        objective_trace[step - 1, 1] = last_components["total"]
        duals = torch.clamp_min(
            duals + config.dual_learning_rate * constraint_row, 0.0
        )
        if not bool(torch.isfinite(duals).all()):
            raise RuntimeError("updated dual is non-finite")
        dual_trace[step].copy_(duals.detach().cpu())
    ...
    audit = {
        ...
        **_training_trace_summary(
        ...
        "adamw_primal_step_before_projected_dual_update": True,
        "dual_update_stopgrad_constraints": True,
        "dual_cap_used": False,
        "manual_loss_weight_used": False,
        "rho_used": False,
        ...
        "fixed_clip_pca_b384_all_five_views_tensor_sha256": _tensor_sha(
            baseline_views
        ),
        ...
        "evaluation_aligned_five_ratio_of_means_primal": True,
        "exact_eight_primal_dual_hinge_constraints": True,
    ...
    return model, audit, constraint_trace, dual_trace, objective_trace
...
def _inner_candidate_gate(
    ...
    """Apply v4-F gate statistics/thresholds with the frozen v4-H seed namespace."""
    ...
    for view in EVAL_VIEWS:
        ...
        statistics = frozen._paired_ratio_ucb(
            ...
            f"{prefix}:recon:{view}", namespace=INNER_BOOTSTRAP_NAMESPACE,
        )
...
def _load_checkpoint_sealed(
    ...
    required_expectations = {
        ...
        "primal_dual_trace_binding_sha256",
        "fixed_clip_pca_b384_all_five_views_tensor_sha256",
    }
    ...
    if (
        ...
        or metadata.get("runtime_fingerprint")
            != expected.get("runtime_fingerprint")
        or metadata.get("primal_dual_trace_artifact")
            != expected.get("primal_dual_trace_artifact")
        or metadata.get("primal_dual_trace_binding_sha256")
            != expected.get("primal_dual_trace_binding_sha256")
        or metadata.get("primal_dual_trace_binding_sha256")
            != _object_sha(metadata.get("primal_dual_trace_artifact"))
        or metadata.get(
            "fixed_clip_pca_b384_all_five_views_tensor_sha256"
        ) != expected.get(
            "fixed_clip_pca_b384_all_five_views_tensor_sha256"
    ...
    binding = {
        ...
        "primal_dual_trace_artifact": metadata[
            "primal_dual_trace_artifact"
        ],
        "primal_dual_trace_binding_sha256": metadata[
            "primal_dual_trace_binding_sha256"
        ],
        "fixed_clip_pca_b384_all_five_views_tensor_sha256": metadata[
            "fixed_clip_pca_b384_all_five_views_tensor_sha256"
        ],
...
def _save_checkpoint_create_only(
    ...
    metadata: dict[str, Any] = {
        ...
        "minibatch_schedule_sha256": training["minibatch_schedule_sha256"],
        "runtime_fingerprint": training["runtime_fingerprint"],
        "primal_dual_trace_artifact": training[
            "primal_dual_trace_artifact"
        ],
        "primal_dual_trace_binding_sha256": training[
            "primal_dual_trace_binding_sha256"
        ],
        "fixed_clip_pca_b384_all_five_views_tensor_sha256": training[
            "fixed_clip_pca_b384_all_five_views_tensor_sha256"
...
def _verify_distinct_checkpoint_pair(
    ...
    provenance_fields = (
        ...
        "inner_validation_iid_digest", "fixed_clip_pca_fit_input_sha256",
        "minibatch_schedule_sha256", "runtime_fingerprint",
        "implementation_sha256", "primal_dual_trace_artifact",
        "primal_dual_trace_binding_sha256",
        "fixed_clip_pca_b384_all_five_views_tensor_sha256",
    )
    if (
    ...
    return {
        ...
        "same_primal_dual_trace_full_binding": True,
        "primal_dual_trace_binding_sha256": fixed[
            "primal_dual_trace_binding_sha256"
        ],
...
def _resolve_fold_root(
    ...
    primal_dual_trace = root / "primal_dual_trace.pt"
    inner = root / "inner.json"
    fold = root / "fold.json"
    if fresh_train and any(
        path.exists() or path.is_symlink()
        for path in (preselection, fixed, primal_dual_trace, inner, fold)
    ):
        raise ValueError("train-fold outputs must all be fresh")
    return root, preselection, fixed, primal_dual_trace, inner
...
def _checkpoint_expectations_from_inner_receipt(
    ...
    *, expected_runtime_fingerprint: Mapping[str, Any] | None = None,
    ...
    trace_artifact = receipt.get("primal_dual_trace_artifact")
    trace_binding_sha = receipt.get("primal_dual_trace_binding_sha256")
    baseline_views_sha = training.get(
        "fixed_clip_pca_b384_all_five_views_tensor_sha256"
    )
    common_expected = {
        "outer_fold": fold_index,
        ...
        "primal_dual_trace_artifact": trace_artifact,
        "primal_dual_trace_binding_sha256": trace_binding_sha,
        "fixed_clip_pca_b384_all_five_views_tensor_sha256": baseline_views_sha,
    }
    ...
    if (
        ...
        or type(trace_artifact) is not dict
        or training.get("primal_dual_trace_artifact") != trace_artifact
        or training.get("primal_dual_trace_binding_sha256") != trace_binding_sha
        or trace_binding_sha != _object_sha(trace_artifact)
        or training.get("fixed_step") != FIXED_SELECTED_STEP
...
def _load_inner_receipt_sealed(
    ...
    (
        root, preselection_path, fixed_path, trace_path, inner_path,
    ) = _resolve_fold_root(fold_root)
    ...
    trace_artifact = value.get("primal_dual_trace_artifact")
    ...
    if (
        ...
        or value.get("scientific_design_origin")
            != _scientific_design_origin_contract()
        or value.get("scientific_design_origin_sha256")
            != _object_sha(_scientific_design_origin_contract())
        ...
        or type(preselection) is not dict or type(fixed) is not dict
        or type(training) is not dict or type(trace_artifact) is not dict
        or trace_artifact.get("path") != str(trace_path.resolve(strict=True))
        or value.get("primal_dual_trace_binding_sha256")
            != _object_sha(trace_artifact)
        or training.get("primal_dual_trace_artifact") != trace_artifact
        or training.get("primal_dual_trace_binding_sha256")
            != _object_sha(trace_artifact)
    ...
    trace_expected = {
        **trace_artifact,
        "outer_fold": fold_index,
        "implementation_sha256": expected_binding["implementation_sha256"],
        "minibatch_schedule_sha256": training["minibatch_schedule_sha256"],
        "runtime_fingerprint": training["runtime_fingerprint"],
        "model_state_sha256": training["final_step_state_sha256"],
        "step0_state_sha256": training["step0_state_sha256"],
    ...
    _, replayed_trace_tensors, replayed_trace_binding = (
        _load_primal_dual_trace_sealed(
        trace_path, trace_expected
        )
    )
    replayed_trace_summary = _training_trace_summary(
        replayed_trace_tensors["constraint_trace"],
        replayed_trace_tensors["dual_trace"],
        replayed_trace_tensors["objective_trace"],
    )
    if (
        replayed_trace_binding != trace_artifact
        or replayed_trace_binding.get("training_trace_summary_sha256")
            != _object_sha(replayed_trace_summary)
        or any(training.get(key) != value
               for key, value in replayed_trace_summary.items())
    ):
        raise RuntimeError("inner receipt primal-dual trace binding differs")
    ...
    return value, {
        ...
        "primal_dual_trace_artifact": replayed_trace_binding,
        "primal_dual_trace_binding_sha256": _object_sha(
            replayed_trace_binding
        ),
...
def _recompute_model_fit_provenance_from_authority(
    ...
    baseline_all_five = frozen._analytic_clip_pca_decode(
        all_five.to(device).flatten(0, 1), fitted
    ).reshape_as(all_five.to(device))
...
def _independently_replay_all_inner_gates_before_oof(
    ...
    for fold_index, (receipt, receipt_binding) in enumerate(
        ...
        training = receipt["training"]
        trace_artifact = receipt["primal_dual_trace_artifact"]
        trace_expected = {
            **trace_artifact,
            "outer_fold": fold_index,
            "implementation_sha256": receipt["implementation"][
                "implementation_sha256"
            ],
            "minibatch_schedule_sha256": training[
        ...
        trace_metadata, trace_tensors, trace_binding = (
            _load_primal_dual_trace_sealed(
            root / "primal_dual_trace.pt", trace_expected
            )
        )
        trace_summary = _training_trace_summary(
            trace_tensors["constraint_trace"], trace_tensors["dual_trace"],
            trace_tensors["objective_trace"],
        )
        ...
        if (
            ...
            or trace_binding != trace_artifact
            or checkpoint_binding.get("primal_dual_trace_artifact")
                != trace_artifact
            or any(training.get(key) != value
                   for key, value in trace_summary.items())
            or receipt_binding.get("primal_dual_trace_artifact")
                != trace_binding
        ...
        inner_replay_binding = {
            ...
            "primal_dual_trace_file_sha256": trace_binding["file_sha256"],
            "primal_dual_trace_metadata_digest": trace_binding[
                "metadata_digest"
            ],
        }
        replay_ledger.append({
            ...
            "primal_dual_trace_full_binding_receipt_checkpoint_join": (
                trace_binding == receipt["primal_dual_trace_artifact"]
                == checkpoint_binding["primal_dual_trace_artifact"]
            ),
            ...
            "primal_dual_trace_formula_recurrence_strongly_replayed": True,
...
def _barrier_members(
    ...
    return [{
        ...
        "primal_dual_trace_artifact": receipt[
            "primal_dual_trace_artifact"
        ],
...
def _barrier_replay_semantically_complete(
    ...
    for fold_index, (member, row) in enumerate(zip(members, replay)):
        ...
        trace = member.get("primal_dual_trace_artifact")
        inner_binding = member.get("inner_receipt_binding")
        if (
            type(provenance) is not dict or type(inner_replay) is not dict
            or type(fixed) is not dict or type(trace) is not dict
            or type(inner_binding) is not dict
            ...
            or row.get("primal_dual_trace_file_sha256")
                != trace.get("file_sha256")
            or row.get("primal_dual_trace_metadata_digest")
                != trace.get("metadata_digest")
            or inner_binding.get("primal_dual_trace_artifact") != trace
            ...
            or inner_replay != {
                ...
                "primal_dual_trace_file_sha256": row.get(
                    "primal_dual_trace_file_sha256"
                ),
                "primal_dual_trace_metadata_digest": row.get(
                    "primal_dual_trace_metadata_digest"
                ),
            }
...
def _load_barrier_receipt_sealed(
    ...
    if (
        ...
        or value.get(
            "all_five_primal_dual_trace_full_bindings_reverified"
        ) is not True
        or value.get(
...
def _train_inner_fold(
    ...
    trace_path: Path, run_binding: Mapping[str, str],
) -> dict[str, Any]:
    ...
    (
        model, training, constraint_trace, dual_trace, objective_trace,
    ) = _train_fold_model(
        model_fit_rows, inner_iids, fitted, config, fold_index, device
    )
    primal_dual_trace = _save_primal_dual_trace_create_only(
        trace_path, fold_index=fold_index, training=training,
        run_binding=run_binding, constraint_trace=constraint_trace,
        dual_trace=dual_trace, objective_trace=objective_trace,
    )
    training["primal_dual_trace_artifact"] = primal_dual_trace
    training["primal_dual_trace_binding_sha256"] = _object_sha(
        primal_dual_trace
    )
    ...
    receipt: dict[str, Any] = {
        ...
        "runtime_fingerprint": training["runtime_fingerprint"],
        "training": training,
        "primal_dual_trace_artifact": primal_dual_trace,
        "primal_dual_trace_binding_sha256": _object_sha(primal_dual_trace),
        ...
        "inner_gate_statistics_and_thresholds_exactly_reused_from_v4f": True,
        "inner_bootstrap_seed_namespace": INNER_BOOTSTRAP_NAMESPACE,
        "inner_bootstrap_seed_ledger_reused_from_v4f": False,
        "scientific_design_frozen_after_sealed_v4g_result": True,
        "scientific_design_frozen_before_any_v4h_execution": True,
        "sealed_v4f_v4g_results_used_only_to_propose_the_unique_v4h_design": True,
        "scientific_design_origin": _scientific_design_origin_contract(),
        "scientific_design_origin_sha256": _object_sha(
            _scientific_design_origin_contract()
        ),
        "v4h_internal_parameter_candidate_or_checkpoint_sweep_performed": False,
        "all_five_known_view_tensors_used_as_model_inputs_during_training": True,
...
def run_train_fold(args: argparse.Namespace) -> dict[str, Any]:
    ...
    receipt = _train_inner_fold(
        ...
        fixed_path, trace_path, run_binding,
    )
...
def run_evaluate_fold(args: argparse.Namespace) -> dict[str, Any]:
    ...
    trace_artifact = target["primal_dual_trace_artifact"]
    trace_expected = {
        **trace_artifact,
        "outer_fold": args.fold_index,
        "implementation_sha256": run_binding["implementation_sha256"],
        "minibatch_schedule_sha256": training["minibatch_schedule_sha256"],
        "runtime_fingerprint": training["runtime_fingerprint"],
        "model_state_sha256": training["final_step_state_sha256"],
    ...
    _, _, trace_evaluate_binding = _load_primal_dual_trace_sealed(
        target_root / "primal_dual_trace.pt", trace_expected
    )
    if (
        fixed_binding["file_sha256"] != fixed_artifact["file_sha256"]
        or trace_evaluate_binding != trace_artifact
        or fixed_binding["primal_dual_trace_artifact"] != trace_artifact
        or inner_bindings[args.fold_index]["primal_dual_trace_artifact"]
            != trace_artifact
    ):
    ...
    receipt: dict[str, Any] = {
        ...
        "global_inner_barrier": {
            ...
            "all_five_primal_dual_traces_strongly_replayed": True,
        ...
        "primal_dual_trace_artifact": trace_artifact,
        "primal_dual_trace_evaluate_binding": trace_evaluate_binding,
        "inner_split": split,
        "oof_original_count": len(oof_iids),
        "oof_ordered_iids": oof_iids,
        "oof_iid_digest": _object_sha(oof_iids),
...
def _load_fold_receipt_sealed(
    ...
    trace_artifact = value.get("primal_dual_trace_artifact")
    trace_binding = value.get("primal_dual_trace_evaluate_binding")
    barrier_inner_bindings = (
        barrier.get("inner_receipt_bindings") if type(barrier) is dict else None
    )
    independent_replay = (
    ...
    if (
        ...
        or barrier.get("all_five_primal_dual_traces_strongly_replayed")
            is not True
        ...
        or type(trace_artifact) is not dict or type(trace_binding) is not dict
        or type(barrier_inner_bindings) is not list
        or len(barrier_inner_bindings) != OUTER_FOLDS
        or type(barrier_inner_bindings[fold_index]) is not dict
        or trace_binding != trace_artifact
        or fixed_artifact.get("primal_dual_trace_artifact") != trace_artifact
        or fixed_binding.get("primal_dual_trace_artifact") != trace_artifact
        or barrier_inner_bindings[fold_index].get(
            "primal_dual_trace_artifact"
        ) != trace_artifact
    ...
    return value, {
        ...
        "nlink": path.lstat().st_nlink,
        "primal_dual_trace_artifact": trace_artifact,
        "primal_dual_trace_binding_sha256": _object_sha(trace_artifact),
    }


def _verify_fold_inner_oof_join(
    fold: Mapping[str, Any], inner: Mapping[str, Any],
) -> None:
    """Bind each evaluated OOF population to its authority-verified inner split."""

    fold_index = inner.get("fold_index")
    fold_iids = fold.get("oof_ordered_iids")
    inner_iids = inner.get("oof_ordered_iids")
    if (
        type(fold_index) is not int or not 0 <= fold_index < OUTER_FOLDS
        or fold.get("fold_index") != fold_index
        or type(fold_iids) is not list or type(inner_iids) is not list
        or any(type(iid) is not str for iid in fold_iids + inner_iids)
        or len(set(fold_iids)) != len(fold_iids)
        or len(set(inner_iids)) != len(inner_iids)
        or type(fold.get("oof_original_count")) is not int
        or type(inner.get("oof_original_count")) is not int
        or fold.get("oof_original_count") != FROZEN_OOF_COUNTS[fold_index]
        or inner.get("oof_original_count") != FROZEN_OOF_COUNTS[fold_index]
        or fold.get("oof_original_count") != len(fold_iids)
        or inner.get("oof_original_count") != len(inner_iids)
        or fold_iids != inner_iids
        or fold.get("oof_iid_digest") != _object_sha(fold_iids)
        or inner.get("oof_iid_digest") != _object_sha(inner_iids)
        or fold.get("oof_iid_digest") != inner.get("oof_iid_digest")
    ):
        raise RuntimeError(
            "evaluated fold OOF population differs from authority-verified inner receipt"
        )
...
def run_aggregate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for fold, inner, inner_binding, fold_binding in zip(
        folds, inner_receipts, inner_bindings, fold_bindings
    ):
        _verify_fold_inner_oof_join(fold, inner)
        if (
            ...
            or fold.get("primal_dual_trace_artifact")
                != inner.get("primal_dual_trace_artifact")
            or fold.get("primal_dual_trace_evaluate_binding")
                != inner.get("primal_dual_trace_artifact")
            or fold.get("fixed1200_evaluate_checkpoint_binding", {}).get(
                "primal_dual_trace_artifact"
            ) != inner.get("primal_dual_trace_artifact")
            or inner_binding.get("primal_dual_trace_artifact")
                != inner.get("primal_dual_trace_artifact")
            or fold_binding.get("primal_dual_trace_artifact")
                != inner.get("primal_dual_trace_artifact")
    ...
    receipt: dict[str, Any] = {
        ...
        "upstream_authorities": {
            ...
            "v4h_scientific_design_frozen_after_sealed_v4g_result": True,
            "v4h_scientific_design_frozen_before_any_v4h_execution": True,
            "sealed_v4f_v4g_results_used_to_propose_unique_v4h_design": True,
            "v4h_internal_parameter_candidate_or_checkpoint_sweep_performed": False,
        ...
        "training_contract": {
            ...
            "formula_contract": _training_formula_contract(),
            "formula_contract_sha256": _object_sha(
                _training_formula_contract()
            ),
            "five_ratio_of_sums_raw_mse_primal": True,
            "exact_eight_hinge_constraints": True,
            "fidelity_training_limit": config.fidelity_constraint_limit,
            "retention_training_target": config.retention_constraint_target,
            "teacher_pair_scale_epsilon": config.teacher_pair_scale_epsilon,
            "raw_fidelity_denominator_epsilon": None,
            "raw_fidelity_denominator_must_be_strictly_positive": True,
            "dual_initialization": "CPU float64 zeros(8)",
            "dual_learning_rate": config.dual_learning_rate,
            "adamw_step_before_projected_dual_update": True,
            "dual_cap": None,
            "rho": None,
            "manual_loss_weight": None,
            "constraint_trace_shape": list(CONSTRAINT_TRACE_SHAPE),
        ...
        "primal_dual_trace_artifacts": {
            "count": OUTER_FOLDS,
            "bindings": [
                row["primal_dual_trace_artifact"] for row in inner_receipts
            ],
            "bindings_sha256": _object_sha([
                row["primal_dual_trace_artifact"] for row in inner_receipts
            ]),
            "checkpoint_inner_barrier_fold_aggregate_full_join": True,
        },
        "folds": folds,
        "oof_closure": {
            "unique_original_iids": 644,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="NO-GO until sealed: v4-H evaluation-aligned primal-dual"
    )
