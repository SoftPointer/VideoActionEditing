#!/usr/bin/env python3
"""Linux CPU-only integration probe for the AUH r5f consumption fix.
...
``load_consumption_input`` while the r5f rank patch is active.  The frozen
...
SCHEMA = "full644-exploratory-matched-r5f-cpu-consumption-probe-v1"
CHILD_SCHEMA = "full644-exploratory-matched-r5f-cpu-consumption-child-v1"
INHERITED_ENV = "APV2_EVAL_INHERITED_AUTHORITY_FDS"
CHILD_TOKEN = "--_r5f-cpu-consumption-child"
SOURCE_CLOSURE_DIGEST = (
    "5867407fe5e18fdc8ae58ba044a08946ad6ea77b3d79881d67dcdb44bdbcf96c"
)
SOURCE_SPECS = {
    "r5f_adapter": (
...
R5F_MODULE_NAME = "_full644_r5f_cpu_consumption_probe_adapter"
...
class R5FCPUConsumptionProbeError(RuntimeError):
...
def canonical_bytes(value: Any) -> bytes:
    try:
        ...
        raise R5FCPUConsumptionProbeError("value is not canonical JSON") from error
...
def _validate_isolated_linux() -> None:
    if (
        ...
        raise R5FCPUConsumptionProbeError(
...
def classify_child_stderr(raw: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise R5FCPUConsumptionProbeError("child stderr is not UTF-8") from error
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("child stderr is not exact ROCm COMGR noise")
    return {
...
def _stable_source(path: Path, expected_sha256: str, expected_size: int) -> tuple[str, dict[str, Any]]:
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError(f"sealed source differs: {path}")
    try:
        source = raw.decode("utf-8", "strict")
    except UnicodeDecodeError as error:
        raise R5FCPUConsumptionProbeError(f"source is not UTF-8: {path}") from error
    return source, {
        "path": str(path),
...
def _pread_sha256(descriptor: int, size: int) -> str:
    ...
    while offset < size:
        ...
        if not block:
            raise R5FCPUConsumptionProbeError("retained probe source short read")
        digest.update(block)
        offset += len(block)
    if os.pread(descriptor, 1, size):
        raise R5FCPUConsumptionProbeError("retained probe source grew")
...
def open_probe_code_authority(path: Path, expected_sha256: str) -> tuple[str, dict[str, Any]]:
    ...
    try:
        ...
        if (
            ...
            raise R5FCPUConsumptionProbeError("retained probe source differs")
        try:
            source = raw.decode("utf-8", "strict")
        except UnicodeDecodeError as error:
            raise R5FCPUConsumptionProbeError(
...
def _replay_child_probe_code(binding_fds: Sequence[int], private_fds: Sequence[int]) -> dict[str, Any]:
    try:
        row = json.loads(os.environ["R5F_PROBE_CODE_AUTHORITY"])
    except (KeyError, TypeError, ValueError) as error:
        raise R5FCPUConsumptionProbeError("child probe-code authority differs") from error
    if not isinstance(row, dict) or set(row) != {
        "fd", "named_path", "sha256", "size", "identity", "mode", "nlink"
    }:
        raise R5FCPUConsumptionProbeError("child probe-code fields differ")
    descriptor = row["fd"]
    try:
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("child retained probe-code replay differs")
...
def _replay_parent_probe_code(row: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("parent retained probe-code replay differs")
...
def build_child_command(
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("captured child source authority differs")
    return [
        "/proc/self/exe",
        "-I",
        "-S",
...
def proc_self_executable_authority() -> dict[str, Any]:
    try:
        ...
        raise R5FCPUConsumptionProbeError("/proc/self/exe is unavailable") from error
    if not stat.S_ISREG(info.st_mode):
        raise R5FCPUConsumptionProbeError("/proc/self/exe type differs")
    return {"proc_path": "/proc/self/exe", "identity": stat_identity(info)}


def _replay_child_executable() -> dict[str, Any]:
    try:
        expected = json.loads(os.environ["R5F_PROBE_PYTHON_EXEC_AUTHORITY"])
    except (KeyError, TypeError, ValueError) as error:
        raise R5FCPUConsumptionProbeError("child Python authority differs") from error
    observed = proc_self_executable_authority()
    if expected != observed:
        raise R5FCPUConsumptionProbeError("child /proc/self/exe identity differs")
    return observed
...
def validate_source_closure(methods_root: str | Path) -> tuple[Path, dict[str, dict[str, Any]]]:
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("production methods root differs")
    ...
    if object_sha256(closure_rows) != SOURCE_CLOSURE_DIGEST:
        raise R5FCPUConsumptionProbeError("production source closure digest differs")
    return root, rows
...
def load_production_sources(
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("site-packages authority differs")
    forbidden = {
        R5F_MODULE_NAME,
    ...
    if forbidden.intersection(sys.modules) or any(
        ...
        raise R5FCPUConsumptionProbeError("production source was importable before source-only gate")
    sys.path.append(str(site_root))
    if sys.path[-1] != str(site_root):
        raise R5FCPUConsumptionProbeError("site-packages activation differs")
    r5f_path = root / SOURCE_SPECS["r5f_adapter"][0]
    source, _ = _stable_source(r5f_path, SOURCE_SPECS["r5f_adapter"][1], SOURCE_SPECS["r5f_adapter"][2])
    module = types.ModuleType(R5F_MODULE_NAME)
    module.__file__ = str(r5f_path)
    module.__package__ = ""
    module.__loader__ = None
    module.__cached__ = None
    module.__spec__ = importlib.machinery.ModuleSpec(R5F_MODULE_NAME, loader=None, origin=str(r5f_path))
    sys.modules[R5F_MODULE_NAME] = module
    path_snapshot = list(sys.path)
    try:
        exec(compile(source, str(r5f_path), "exec", dont_inherit=True), module.__dict__)
    except BaseException:
        sys.modules.pop(R5F_MODULE_NAME, None)
        raise
    finally:
        sys.path[:] = path_snapshot
    authority = module.model_authority
    expected_origins = {
        R5F_MODULE_NAME: root / SOURCE_SPECS["r5f_adapter"][0],
    ...
    for name, expected in expected_origins.items():
        ...
        if (
            ...
            raise R5FCPUConsumptionProbeError(f"source-only origin differs: {name}")
    if (
        ...
        raise R5FCPUConsumptionProbeError("source origin/sitecustomize gate differs")
...
def write_create_only_canonical(path: str | Path, value: Mapping[str, Any], *, mode: int = 0o400) -> dict[str, Any]:
    ...
    if not target.is_absolute() or target.name in ("", ".", "..") or target.exists():
        raise R5FCPUConsumptionProbeError("create-only output path differs")
    ...
    try:
        ...
        while offset < len(raw):
            ...
            if written <= 0:
                raise R5FCPUConsumptionProbeError("create-only output short write")
            offset += written
        os.fsync(descriptor)
        ...
        if (
            ...
            raise R5FCPUConsumptionProbeError("create-only output replay differs")
...
def write_receipt(path: str | Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if row != {key: reference[key] for key in ("path", "sha256", "size", "mode")}:
        raise R5FCPUConsumptionProbeError("receipt commit reference differs")
    return reference
...
def prepare_receipt(
    ...
    if "receipt_digest" in unsigned:
        raise R5FCPUConsumptionProbeError("receipt digest is caller-supplied")
...
def commit_receipt_and_exit(
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("final receipt reference differs")
    flags = (
        os.O_RDWR
        | os.O_CREAT
        | os.O_EXCL
    ...
    try:
        ...
        while offset < len(raw):
            written = os.write(descriptor, raw[offset:])
            if written <= 0:
                raise R5FCPUConsumptionProbeError("final receipt short write")
            offset += written
        os.fsync(descriptor)
        current = os.fstat(descriptor)
        named = target.lstat()
        if (
            ...
            raise R5FCPUConsumptionProbeError("final receipt precommit replay differs")
...
def _write_fixture_file(path: Path, raw: bytes, *, mode: int = 0o600) -> None:
    ...
    try:
        ...
        while offset < len(raw):
            written = os.write(descriptor, raw[offset:])
            if written <= 0:
                raise R5FCPUConsumptionProbeError("fixture short write")
            offset += written
        os.fsync(descriptor)
        os.fchmod(descriptor, mode)
    finally:
...
def _write_member(
    ...
    try:
        ...
        while offset < len(raw):
            written = os.write(descriptor, raw[offset:])
            if written <= 0:
                raise R5FCPUConsumptionProbeError("task receipt short write")
            offset += written
        os.fsync(descriptor)
        os.fchmod(descriptor, 0o400)
    finally:
...
def _build_model(authority: Any, state: Path, view_parent: Path, parent_fd: int) -> Any:
    ...
    for index, relative in enumerate(authority.MODEL_RELATIVE_FILES):
        raw = f"r5f-cpu-model:{index}:{relative}\n".encode("utf-8")
...
def _build_adapter(
    ...
    payloads = {
        "receipt.json": b'{"fixture":"r5f-cpu"}\n',
        "adapter/README.md": b"r5f cpu fixture\n",
        "adapter/adapter_config.json": b'{"peft_type":"LORA"}\n',
        "adapter/adapter_model.safetensors": b"r5f-cpu-adapter",
...
def _replace_binding_row(
    ...
    *,
    ...
    if len(matches) != 1:
        raise R5FCPUConsumptionProbeError("hostile binding row differs")
    matches[0].update(
        {
            "fd": descriptor,
...
def _validate_private_parent_roles(
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("private-parent role binding differs")
...
def _child_main() -> int:
    ...
    try:
        ...
        private_roles = json.loads(os.environ["R5F_PROBE_PRIVATE_PARENT_ROLES"])
    except (ValueError, TypeError, KeyError) as error:
        raise R5FCPUConsumptionProbeError("child environment differs") from error
    private_roles = _validate_private_parent_roles(
        binding, private_roles, os.environ.get("R5F_PROBE_ARM")
    )
    private_fds = sorted(set(private_roles.values()))
    if canonical_bytes(binding).decode("utf-8") != raw_binding:
        raise R5FCPUConsumptionProbeError("child inherited binding is not canonical")
    ...
    if tuple(sorted(inherited)) != inherited or set(inherited).intersection(private_fds):
        raise R5FCPUConsumptionProbeError("private parent entered child allowlist")
    ...
    r5f, authority, source = load_production_sources(
        os.environ["R5F_PROBE_METHODS_ROOT"],
        os.environ["R5F_PROBE_SITE_PACKAGES_ROOT"],
    )
    outcome = "SUCCESS"
    ...
    try:
        with r5f.patched_rank_validation():
            _, model, adapter = authority.load_consumption_input(
                os.environ["R5F_PROBE_INPUT_PATH"],
                expected_sha256=os.environ["R5F_PROBE_INPUT_SHA256"],
                expected_digest=os.environ["R5F_PROBE_INPUT_DIGEST"],
                verify_views=True,
            )
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("GPU/model runtime entered CPU probe")
    result = {
        "schema_version": CHILD_SCHEMA,
        "arm": os.environ["R5F_PROBE_ARM"],
        "outcome": outcome,
        "error": error_text,
...
def _spawn_child(
    *,
    ...
    if _PROBE_CODE_AUTHORITY is None or _PROBE_CAPTURED_SOURCE is None:
        raise R5FCPUConsumptionProbeError("parent probe-code authority is absent")
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("child pass_fds closure differs")
    environment = {
        "PATH": "/usr/bin:/bin",
        "LANG": "C.UTF-8",
        "LC_ALL": "C.UTF-8",
        ...
        "R5F_PROBE_PRIVATE_PARENT_ROLES": canonical_bytes(dict(private_parent_roles)).decode("utf-8"),
        "R5F_PROBE_METHODS_ROOT": str(methods_root),
        "R5F_PROBE_SITE_PACKAGES_ROOT": str(site_root),
        "R5F_PROBE_INPUT_PATH": input_path,
        "R5F_PROBE_INPUT_SHA256": input_sha256,
        "R5F_PROBE_INPUT_DIGEST": input_digest,
        "R5F_PROBE_ARM": arm,
        "R5F_PROBE_CODE_AUTHORITY": canonical_bytes(_PROBE_CODE_AUTHORITY).decode("utf-8"),
        "R5F_PROBE_PYTHON_EXEC_AUTHORITY": canonical_bytes(python_authority).decode("utf-8"),
    }
    ...
    if proc_self_executable_authority() != python_authority:
        raise R5FCPUConsumptionProbeError("parent /proc/self/exe identity changed")
    _replay_parent_probe_code(_PROBE_CODE_AUTHORITY)
    ...
    try:
        ...
        raise R5FCPUConsumptionProbeError("child stdout is not JSON") from error
    if completed.stdout != canonical_bytes(result) + b"\n" or result.get("schema_version") != CHILD_SCHEMA:
        raise R5FCPUConsumptionProbeError("child stdout contract differs")
...
def _require_child(
    ...
    if error_contains is None:
        if error is not None:
            raise R5FCPUConsumptionProbeError("successful child reported an error")
...
def run_probe(
    *, methods_root: Path, site_root: Path, state: Path
    ...
    if gpu_device_descriptors():
        raise R5FCPUConsumptionProbeError("GPU descriptor was open before probe")
    r5f, authority, source_authority = load_production_sources(
        methods_root, site_root
    )
    if r5f.__name__ == "__main__":
        raise R5FCPUConsumptionProbeError("r5f unexpectedly used production main")
    ...
    try:
        ...
        task_binding["task_id"] = "r5f-cpu-hostile-task"
        ...
        if (
            ...
            raise R5FCPUConsumptionProbeError("GPU runtime entered parent probe")
        return {
            ...
            "execution_contract": {
                ...
                "gpu_device_descriptors": [],
                "r5f_imported_as_module_not_production_main": True,
        ...
        if adapter is not None and not adapter_closed:
            try:
                adapter.abort(reason="r5f cpu probe cleanup")
            except Exception:
                pass
        if model is not None and not model_closed:
            try:
                model.abort(reason="r5f cpu probe cleanup")
            except Exception:
                pass
        os.close(task_fd)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        raise R5FCPUConsumptionProbeError("fresh work-root/receipt contract differs")
    ...
    if not probe_source.startswith("#!/usr/bin/env python3\n"):
        ...
        raise R5FCPUConsumptionProbeError("probe source framing differs")
    _PROBE_CODE_AUTHORITY = probe_authority
    _PROBE_CAPTURED_SOURCE = probe_source
    try:
        with tempfile.TemporaryDirectory(prefix=".r5f-cpu-state-", dir=work_root) as temporary:
            result = run_probe(
                methods_root=methods_root,
                site_root=site_root,
    ...
    if any(work_root.iterdir()):
        raise R5FCPUConsumptionProbeError("temporary probe state was not removed")
    payload = {
    ...
    raise R5FCPUConsumptionProbeError("final receipt commit returned")
