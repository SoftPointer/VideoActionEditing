#!/usr/bin/env python3
"""AUH r5d rank adapter for the retained model namespace-root contract.
...
This eval-only adapter keeps the frozen r5c implementation and frozen
``infer_lora`` unchanged.  During one rank invocation it replaces only the
capture-binding step with a validator that:
...
* restores the frozen validator on all exit paths; and
...
class MatchedInferAdapterR5DError(RuntimeError):
    """The r5d rank-only authority or patch lifecycle differs."""
...
def _read_pinned_source(
    ...
    if (
        ...
        raise MatchedInferAdapterR5DError(f"frozen {label} source differs")
    try:
        return raw.decode("utf-8", "strict")
    except UnicodeDecodeError as error:
        raise MatchedInferAdapterR5DError(
...
def _production_import_preflight() -> None:
    ...
    if forbidden.intersection(sys.modules):
        raise MatchedInferAdapterR5DError(
            "frozen inference source was imported before r5d bootstrap"
        )
    for value in sys.path:
        if type(value) is not str:
            raise MatchedInferAdapterR5DError("rank import path entry differs")
        if Path(value or os.curdir).resolve(strict=False) == _METHOD_ROOT:
            raise MatchedInferAdapterR5DError(
                "method root was importable before r5d bootstrap"
            )
...
def _configure_rank_cache_before_frozen_import() -> tuple[Path, Path]:
    ...
    if base_value is None or rank_value is None:
        raise MatchedInferAdapterR5DError("rank-cache authority is absent")
    try:
        rank = int(rank_value)
    except ValueError as error:
        raise MatchedInferAdapterR5DError("LOCAL_RANK differs") from error
    ...
    if (
        ...
        raise MatchedInferAdapterR5DError("rank-cache root differs")
...
def _activate_site_packages_before_frozen_import() -> Path:
    ...
    if raw is None:
        raise MatchedInferAdapterR5DError("site-packages authority is absent")
    root = Path(raw)
    if (
        ...
        raise MatchedInferAdapterR5DError("site-packages authority differs")
...
def _prepare_production_before_base_import() -> None:
    ...
    try:
        ...
        raise MatchedInferAdapterR5DError(str(error)) from error
...
def _load_base_adapter() -> Any:
    ...
    if module.__cached__ is not None or module.__file__ != str(_BASE_ADAPTER_PATH):
        raise MatchedInferAdapterR5DError("frozen r5c adapter origin differs")
    return module
...
if __name__ == "__main__" and model_authority is not _EARLY_MODEL_AUTHORITY:
    raise MatchedInferAdapterR5DError("early model authority origin differs")
...
def _one_namespace_row(
    ...
    if len(rows) != 1:
        raise model_authority.ModelConsumptionAuthorityError(
            f"r5d inherited {scope} namespace-root closure differs"
        )
    return rows[0]
...
def _safe_relative(value: Any, *, label: str) -> str:
    if type(value) is not str:
        raise model_authority.ModelConsumptionAuthorityError(
            f"r5d {label} relative path differs"
        )
...
def _verify_capture_through_namespace_fd(
    ...
    *,
    ...
    if namespace != expected_namespace:
        raise model_authority.ModelConsumptionAuthorityError(
            f"r5d inherited {scope} namespace-root binding differs"
        )
    ...
    for item in directory_rows:
        ...
        if parent not in expected_children:
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d captured {scope} view directory topology differs"
            )
    ...
    for item in capture["files"]:
        ...
        if parent not in expected_children:
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d captured {scope} view leaf topology differs"
            )
        expected_children[parent].add(pure.name)
    for item in directory_rows:
        ...
        try:
            ...
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d inherited {scope} view directory is unavailable"
            ) from error
        if (
            ...
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d inherited {scope} view directory identity differs"
            )
    ...
    for item in capture["files"]:
        ...
        if bound is None or bound != {
            ...
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d inherited {scope} file binding differs"
            )
        try:
            ...
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d inherited {scope} FD-view leaf is unavailable"
            ) from error
        if (
            ...
            raise model_authority.ModelConsumptionAuthorityError(
                f"r5d inherited {scope} FD-view leaf identity differs"
            )
...
def validate_inherited_fd_binding_r5d(
    value: Any,
    *,
    ...
    if (
        ...
        raise model_authority.ModelConsumptionAuthorityError(
            "r5d inherited capture digest/task binding differs"
        )
    ...
    if inherited_numbers.intersection(private_parent_numbers):
        raise model_authority.ModelConsumptionAuthorityError(
            "r5d private-parent FD was unexpectedly inherited"
        )
    ...
    if sorted(observed_rows, key=lambda item: item["fd"]) != sorted(
        ...
        raise model_authority.ModelConsumptionAuthorityError(
            "r5d inherited FD/capture binding differs"
        )
    if verify_open_fds:
...
def pinned_dependency_import_paths_r5d(site_root: str | Path) -> Any:
    return preserve_primary_context(_FROZEN_DEPENDENCY_CONTEXT(site_root))


@contextmanager
def patched_rank_validation() -> Iterator[None]:
    """Install the two r5d rank-only fixes and restore frozen globals."""
    ...
    if (
        ...
        raise MatchedInferAdapterR5DError("r5d patch origin differs")
    model_authority.validate_inherited_fd_binding = (
        validate_inherited_fd_binding_r5d
    )
    base.pinned_dependency_import_paths = pinned_dependency_import_paths_r5d
    try:
        yield
    finally:
        changed = (
            model_authority.validate_inherited_fd_binding
            is not validate_inherited_fd_binding_r5d
            or base.pinned_dependency_import_paths
            is not pinned_dependency_import_paths_r5d
        )
        model_authority.validate_inherited_fd_binding = (
        ...
        if changed:
            raise MatchedInferAdapterR5DError("r5d patch lifecycle differs")
...
def _activate_production_rank() -> None:
    ...
    if (
        ...
        raise MatchedInferAdapterR5DError("publication handoff task differs")
    ...
    if (
        ...
        raise MatchedInferAdapterR5DError("early rank isolation disappeared")
