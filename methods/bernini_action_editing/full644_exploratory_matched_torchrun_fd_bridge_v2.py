#!/usr/bin/env python3
"""Exact retained-FD relay from one torchrun coordinator to four ranks.
...
SCHEMA = "full644-exploratory-matched-torchrun-fd-bridge-v2"
...
def _site_packages_for_torch_sources(
    torchrun_source: str | Path,
    handler_source: str | Path,
    local_agent_source: str | Path,
    dynamic_rendezvous_source: str | Path,
    multiprocessing_api_source: str | Path,
) -> Path:
    run = Path(torchrun_source).resolve(strict=True)
    handler = Path(handler_source).resolve(strict=True)
    local_agent = Path(local_agent_source).resolve(strict=True)
    dynamic_rendezvous = Path(dynamic_rendezvous_source).resolve(strict=True)
    multiprocessing_api = Path(multiprocessing_api_source).resolve(strict=True)
    try:
...
def _activate_pinned_site_packages(
    ...
    root = _site_packages_for_torch_sources(
        torchrun_source,
        handler_source,
        local_agent_source,
...
def validate_rank_environment(
    ...
    *,
    ...
    if (
        ...
        # Pinned PyTorch 2.7.1 --standalone selects DynamicRendezvousHandler.
        # Its use_agent_store property is True unless the coordinator sets
        # TORCH_DISABLE_SHARE_RDZV_TCP_STORE=1; that key is absent from this
        # bridge's exact coordinator allowlist.
        or environment.get("TORCHELASTIC_USE_AGENT_STORE") != "True"
...
@contextmanager
def observed_dynamic_rendezvous_creation(
    dynamic_module: Any,
    registry_module: Any,
) -> Iterator[list[str]]:
    """Prove the pinned dynamic handler is the one consumed by standalone."""

    original = dynamic_module.create_handler
    registry_original = registry_module.create_handler
    handler_type = dynamic_module.DynamicRendezvousHandler
    run_ids: list[str] = []
    if registry_original is not original:
        raise TorchrunFDBridgeV2Error(
            "dynamic rendezvous registry alias differs"
        )

    def exact_create_handler(*args: Any, **kwargs: Any) -> Any:
        handler = original(*args, **kwargs)
        run_id = handler.get_run_id()
        if (
            type(handler) is not handler_type
            or handler.use_agent_store is not True
            or type(run_id) is not str
            or _UUID_RE.fullmatch(run_id) is None
        ):
            raise TorchrunFDBridgeV2Error(
                "dynamic rendezvous handler consumption differs"
            )
        run_ids.append(run_id)
        return handler

    dynamic_module.create_handler = exact_create_handler
    registry_module.create_handler = exact_create_handler
    try:
        yield run_ids
    finally:
        dynamic_changed = (
            getattr(dynamic_module, "create_handler", None)
            is not exact_create_handler
        )
        registry_changed = (
            getattr(registry_module, "create_handler", None)
            is not exact_create_handler
        )
        dynamic_module.create_handler = original
        registry_module.create_handler = registry_original
        if (
            dynamic_changed
            or registry_changed
            or dynamic_module.create_handler is not original
            or registry_module.create_handler is not registry_original
            or registry_module.create_handler is not dynamic_module.create_handler
        ):
            raise TorchrunFDBridgeV2Error(
                "dynamic rendezvous handler hook was not restored"
            )
...
def _load_pinned_torchrun(
    *,
    torchrun_source: str,
    handler_source: str,
    local_agent_source: str,
    dynamic_rendezvous_source: str,
    multiprocessing_api_source: str,
) -> tuple[Any, type, dict[str, Any]]:
    identities = {
        "torchrun": _stable_file_identity(
            torchrun_source, TORCHRUN_SOURCE_SHA256
        ),
        "subprocess_handler": _stable_file_identity(
            handler_source, TORCHRUN_HANDLER_SHA256
        ),
        "local_elastic_agent": _stable_file_identity(
            local_agent_source, TORCH_LOCAL_ELASTIC_AGENT_SHA256
        ),
        "dynamic_rendezvous": _stable_file_identity(
            dynamic_rendezvous_source, TORCH_DYNAMIC_RENDEZVOUS_SHA256
        ),
        "multiprocessing_api": _stable_file_identity(
            multiprocessing_api_source, TORCH_MULTIPROCESSING_API_SHA256
        ),
    }
    ...
    local_agent_module = importlib.import_module(
    ...
    multiprocessing_package = importlib.import_module(
        "torch.distributed.elastic.multiprocessing"
    )
    launcher_module = importlib.import_module("torch.distributed.launcher.api")
    handler_class = handler_module.SubprocessHandler
    ordinary_modules = {
        "local_elastic_agent": local_agent_module,
        "dynamic_rendezvous": dynamic_module,
        "multiprocessing_api": api_module,
    }
    cache_root = Path(sys.pycache_prefix or "")
    if (
        Path(run_module.__file__).resolve(strict=True)
        != Path(identities["torchrun"]["path"])
        or Path(handler_module.__file__).resolve(strict=True)
        != Path(identities["subprocess_handler"]["path"])
        ...
        or launcher_module.LocalElasticAgent is not local_agent_module.LocalElasticAgent
        or local_agent_module.start_processes
        is not multiprocessing_package.start_processes
        or multiprocessing_package.DefaultLogsSpecs
        is not api_module.DefaultLogsSpecs
        or multiprocessing_package.SubprocessContext
        is not api_module.SubprocessContext
        or any(
            Path(module.__file__).resolve(strict=True)
            != Path(identities[role]["path"])
            or type(module.__loader__) is not importlib.machinery.SourceFileLoader
            or not isinstance(module.__cached__, str)
            or not Path(module.__cached__).is_absolute()
            or cache_root not in Path(module.__cached__).parents
            or Path(module.__cached__).exists()
            for role, module in ordinary_modules.items()
        )
        or run_module.__cached__ is not None
        or handler_module.__cached__ is not None
        or dynamic_module.DynamicRendezvousHandler.use_agent_store.fget
        is None
        or registry_module.create_handler is not dynamic_module.create_handler
        or registry_module._create_c10d_handler.__module__
        != "torch.distributed.elastic.rendezvous.registry"
    ):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--torch-local-agent-source-sha256",
        default=TORCH_LOCAL_ELASTIC_AGENT_SHA256,
    )
    parser.add_argument("--torch-dynamic-rendezvous-source", required=True)
    parser.add_argument(
        "--torch-dynamic-rendezvous-source-sha256",
        default=TORCH_DYNAMIC_RENDEZVOUS_SHA256,
    )
    parser.add_argument("--torch-multiprocessing-api-source", required=True)
    parser.add_argument(
        "--torch-multiprocessing-api-source-sha256",
        default=TORCH_MULTIPROCESSING_API_SHA256,
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_module, handler_class, torch_identities = _load_pinned_torchrun(
        ...
        local_agent_source=args.torch_local_agent_source,
        dynamic_rendezvous_source=args.torch_dynamic_rendezvous_source,
        multiprocessing_api_source=args.torch_multiprocessing_api_source,
    )
    ...
    dynamic_module = sys.modules[
        "torch.distributed.elastic.rendezvous.dynamic_rendezvous"
    ]
    registry_module = sys.modules[
        "torch.distributed.elastic.rendezvous.registry"
    ]
    local_agent_module = sys.modules[
        "torch.distributed.elastic.agent.server.local_elastic_agent"
    ]
    api_module = sys.modules["torch.distributed.elastic.multiprocessing.api"]
    multiprocessing_package = sys.modules[
        "torch.distributed.elastic.multiprocessing"
    ]
    launcher_module = sys.modules["torch.distributed.launcher.api"]
    identities_after: dict[str, Any] = {}
    try:
        with observed_dynamic_rendezvous_creation(
            dynamic_module,
            registry_module,
        ) as dynamic_run_ids, patched_rank_spawner(
                handler_class,
                inherited=inherited,
                exec_authority=exec_authority,
                publication_handoff=publication_handoff,
        ...
        if (
            ...
            != "torch.distributed.elastic.multiprocessing.subprocess_handler.subprocess_handler"
            or launcher_module.LocalElasticAgent
            is not local_agent_module.LocalElasticAgent
            or local_agent_module.start_processes
            is not multiprocessing_package.start_processes
            or multiprocessing_package.DefaultLogsSpecs
            is not api_module.DefaultLogsSpecs
            ...
            is not local_agent_module
            or sys.modules.get(
                "torch.distributed.elastic.rendezvous.dynamic_rendezvous"
            )
            is not dynamic_module
            or sys.modules.get(
                "torch.distributed.elastic.rendezvous.registry"
            )
            is not registry_module
            or registry_module.create_handler is not dynamic_module.create_handler
            or sys.modules.get("torch.distributed.elastic.multiprocessing.api")
            is not api_module
            or any(
                Path(module.__file__).resolve(strict=True)
                != Path(identities_after[role]["path"])
                or type(module.__loader__)
                is not importlib.machinery.SourceFileLoader
                or not isinstance(module.__cached__, str)
                or Path(coordinator_cache) not in Path(module.__cached__).parents
                or Path(module.__cached__).exists()
                for role, module in (
                    ("local_elastic_agent", local_agent_module),
                    ("dynamic_rendezvous", dynamic_module),
    ...
    if (
        return_value not in (None, 0)
        or spawned_ranks != {0, 1, 2, 3}
        or len(dynamic_run_ids) != 1
    ):
    ...
    receipt = {
        ...
        "max_restarts": 0,
        "dynamic_rendezvous_run_id": dynamic_run_ids[0],
        "dynamic_rendezvous_create_handler_called_once": True,
        "dynamic_rendezvous_registry_alias_exact_and_restored": True,
        ...
        "captured_source_execution": ["torchrun", "subprocess_handler"],
        "named_source_execution_under_trusted_site_root": [
            "dynamic_rendezvous",
            "local_elastic_agent",
            "multiprocessing_api",
        ],
