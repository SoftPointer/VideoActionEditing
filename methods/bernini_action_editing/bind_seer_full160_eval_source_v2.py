...
SCHEMA_VERSION = "bernini-seer-full160-eval-source-binding-v2"
CASE_BINDING_SCHEMA_VERSION = "bernini-seer-full160-eval-case-binding-v2"
...
def verify_binding(
    *,
    ...
    recovery_binding = _verify_recovery_binding(
    ...
    return {
        ...
        "checkpoint_recovery": recovery_binding,
...
def verify_receipt(path: Path) -> dict[str, Any]:
    ...
    recovery_row = value.get("checkpoint_recovery")
    if not isinstance(recovery_row, Mapping):
        raise SourceBindingError("binding recovery receipt is absent")
    checked = _verify_recovery_binding(
        Path(str(recovery_row.get("receipt_path", ""))),
        expected_sha256=str(recovery_row.get("sha256", "")),
        expected_digest=str(recovery_row.get("receipt_digest", "")),
    )
    if checked != recovery_row:
        raise SourceBindingError("binding recovery identity differs")
    return value
...
def _verify_recovery_binding(
    path: Path, *, expected_sha256: str, expected_digest: str
) -> dict[str, Any]:
    path = _plain_file(path, label="FM160 recovery receipt")
    if file_sha256(path) != expected_sha256:
        raise SourceBindingError("FM160 recovery receipt SHA differs")
    try:
        value = recovery.verify_receipt(path)
    except recovery.RecoveryError as error:
        raise SourceBindingError(str(error)) from error
    if value.get("receipt_digest") != expected_digest:
        raise SourceBindingError("FM160 recovery receipt digest differs")
    final = value.get("final_checkpoint")
    if not isinstance(final, Mapping):
        raise SourceBindingError("FM160 recovery final checkpoint is absent")
    return {
        ...
        "final_checkpoint_path": final.get("path"),
        "final_training_receipt_sha256": final.get("receipt", {}).get("sha256"),
        "final_training_receipt_digest": final.get("receipt", {}).get("digest"),
        "final_adapter_model_sha256": final.get("adapter_model", {}).get("sha256"),
    }
...
def finalize_case_binding(
    *, source_binding_path: Path, pair_receipt_path: Path
    ...
    recovered = source["checkpoint_recovery"]
    if (
        trained.get("checkpoint_root") != recovered.get("final_checkpoint_path")
        or trained.get("training_receipt_sha256")
        != recovered.get("final_training_receipt_sha256")
        or trained.get("training_receipt_digest")
        != recovered.get("final_training_receipt_digest")
        or trained.get("adapter_model_sha256")
        != recovered.get("final_adapter_model_sha256")
    ):
        raise SourceBindingError("trained adapter/recovery checkpoint cross-bind differs")
    ...
    return {
        ...
        "checkpoint_recovery": dict(recovered),
        "paired_receipt": {
...
def verify_case_binding_receipt(path: Path) -> dict[str, Any]:
    ...
    recovery_row = value.get("checkpoint_recovery")
    if not isinstance(recovery_row, Mapping):
        raise SourceBindingError("case eval recovery binding is absent")
    checked_recovery = _verify_recovery_binding(
        Path(str(recovery_row.get("receipt_path", ""))),
        expected_sha256=str(recovery_row.get("sha256", "")),
        expected_digest=str(recovery_row.get("receipt_digest", "")),
    )
    if checked_recovery != recovery_row:
        raise SourceBindingError("case eval recovery binding differs")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bind.add_argument("--recovery-receipt", required=True)
    bind.add_argument("--expected-recovery-receipt-sha256", required=True)
    bind.add_argument("--expected-recovery-receipt-digest", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "verify-receipt":
        ...
        for value, label in (
            ...
            (args.expected_recovery_receipt_sha256, "recovery receipt SHA"),
            (args.expected_recovery_receipt_digest, "recovery receipt digest"),
        ):
        ...
        unsigned = verify_binding(
            ...
            recovery_receipt_path=_plain_file(
                args.recovery_receipt, label="FM160 recovery receipt"
            ),
