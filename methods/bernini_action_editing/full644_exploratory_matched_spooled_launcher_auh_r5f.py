#!/usr/bin/env python3
"""Materialize the fresh AUH r5f root-launch release.

The runner and Torchrun FD bridge already carry the adapter path and digest as
authenticated launch inputs, so their frozen bytes do not change for r5f.
...
SCHEMA = "full644-exploratory-matched-root-launch-release-auh-r5f"
...
_ADAPTER_BASENAME = "full644_exploratory_matched_infer_adapter_auh_r5f.py"
...
class R5FLauncherBootstrapError(RuntimeError):
    """The pinned r5c builder or the r5f pin transformation differs."""
...
def _read_pinned_source(path: Path, expected_sha256: str) -> str:
    ...
    if (
        ...
        raise R5FLauncherBootstrapError("frozen r5c launcher source differs")
    try:
        return raw.decode("utf-8", "strict")
    except UnicodeDecodeError as error:
        raise R5FLauncherBootstrapError(
...
def _validate_adapter_dependency_paths(value: Mapping[str, Any]) -> None:
    try:
        ...
        raise R5FLauncherBootstrapError(
            "r5f adapter dependency path is absent"
        ) from error
    if (
        not all(
            path.is_absolute()
        ...
        raise R5FLauncherBootstrapError(
            "r5f adapter dependency adjacency differs"
        )


def _r5f_input_loader(module: types.ModuleType) -> Any:
    def load_input(
        path_value: str | Path,
        ...
        except R5FLauncherBootstrapError as error:
...
def _transform_base(module: types.ModuleType) -> types.ModuleType:
    ...
    if (
        ...
        raise R5FLauncherBootstrapError("frozen r5c launcher contract differs")
...
if identity_roles!={identity_roles!r}: raise RuntimeError("r5f exact16 release identity closure differs")
...
if type(adapter_row) is not dict or type(base_adapter_row) is not dict or type(model_authority_row) is not dict: raise RuntimeError("r5f adapter dependency rows differ")
...
for source_label,source_row,source_pin in (("r5f adapter",adapter_row,{R5F_ADAPTER_SHA256!r}),("r5c base adapter",base_adapter_row,{BASE_ADAPTER_SHA256!r}),("model authority",model_authority_row,{MODEL_AUTHORITY_SHA256!r})):
 ...
 if not stat.S_ISREG(source_before.st_mode) or source_before.st_nlink!=1 or stat.S_IMODE(source_before.st_mode)!=0o444 or ident(source_before)!=ident(source_after) or ident(source_before)!=ident(source_named) or ident(source_before)!=source_row.get("identity") or source_row.get("sha256")!=source_pin or hashlib.sha256(source_raw).hexdigest()!=source_pin: raise RuntimeError(source_label+" identity differs")'''
    ...
    if (
        ...
        raise R5FLauncherBootstrapError("r5f root-bootstrap schema differs")

    expected_static["adapter"] = R5F_ADAPTER_SHA256
    ...
    module._load_input = _r5f_input_loader(module)

    original_build_release = module.build_release

    def build_release_r5f(
        ...
        if (
            ...
            raise module.RootLaunchReleaseError(
                "r5f exact16 adapter release closure differs"
            )
        return release, payload

    module.build_release = build_release_r5f
    return module
...
# their globals, where the r5f transformation above is installed exactly once.
