#!/usr/bin/env python3
"""Fail-closed activation core for the Round37 native-only oracle canary.

This module is intentionally separate from the frozen v1 safe-BLOCKED
scaffold.  Production authority is an exact, independently reviewed packet
    and a distinct model-review ledger receipt whose SHA-256 values are compiled into
...
The checked-in moving candidate keeps both constants ``None``.  It therefore
    cannot mint an execution capability or run a GPU sample until the e02 explicit
gate, both cases' VAE/prompt receipts, and the external ledger are reviewed.
...
SCHEMA_VERSION = "bernini-oracle-regeneration-activation-v2-core-v1"
...
EXPECTED_ARMS_E03 = (ARM_OFFICIAL,)
...
EXPECTED_CASE_BINDINGS = {
    "e02": {
        ...
        "structured_action_program_sha256": "0b053e79fbdce086c1dee5bf224941a29f1318d26fc7f6b40e07d06b4ca098d1",
    },
    "e03": {
        ...
        "structured_action_program_sha256": "855a3456c22af50ddf05a415c7dfba1718d0f575735fd02d06e1c1a77d4866dd",
    },
}

# Deliberately unavailable until external authoring/review is complete.  A
# future reviewed moving-byte revision replaces these two ``None`` literals
# with lowercase SHA-256 values.  The authority packet MUST NOT contain the
# core/runner/spec/launcher hash, so this exact-SHA trust graph has no cycle.
...
def verify_frozen_dependency_pins_v2() -> Mapping[str, str]:
    ...
    for label, (relative, expected_sha256) in FROZEN_DEPENDENCY_PINS.items():
        ...
        module_path = Path(str(getattr(modules[label], "__file__", ""))).resolve(
            strict=True
        )
        if module_path != path:
...
def _validate_packet_case_v2(
    ...
    reference_path, reference_sha, reference_seal = _artifact_binding(
        row.get("vae_reference_receipt"),
        label=f"{expected_case_id} VAE reference receipt",
        require_frozen=True,
    )
    prompt_path, prompt_sha, prompt_seal = _artifact_binding(
        row.get("prompt_receipt"),
        label=f"{expected_case_id} prompt receipt",
        require_frozen=True,
    )
...
def validate_reference_receipt_v2(
    ...
    *,
    ...
    if case_id not in authority.cases:
        raise OracleActivationV2Error("reference receipt case differs")
    case = authority.cases[case_id]
    ...
    if (
        observed_source_sha != source_latent_sha
        or observed_reference_sha != reference_sha_tuple
        or bool(set(reference_sha_tuple).intersection(source_slice_sha))
    ):
        raise OracleActivationV2Error(
...
def validate_prompt_receipt_v2(
    ...
    *,
    ...
    revalidate_compiled_activation_authority_v2(authority)
    if case_id not in authority.cases:
        raise OracleActivationV2Error("prompt receipt case differs")
    case = authority.cases[case_id]
    raw = _bound_authority_json_v2(
        case,
        artifact_key="prompt_receipt",
