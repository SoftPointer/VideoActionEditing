#!/usr/bin/env python3
"""One-shot external runner for the full644 shared-8 matched evaluation.

The runner consumes a frozen v1 16-task plan and executes every task exactly
once, in plan order, under one holder-lifetime exact-23 model capture.  It
...
SCHEMA = "full644-exploratory-matched-runner-attestation-v2"
TASK_SCHEMA = "full644-exploratory-matched-runner-task-v2"
FAILURE_SCHEMA = "full644-exploratory-matched-runner-failure-v2"
...
def execute_task_sequence(
    ...
    if len(tasks) != 16 or tuple(task.get("task_id") for task in tasks) != TASK_IDS:
...
class RunnerExecution:
    def __init__(self, args: argparse.Namespace, plan: Mapping[str, Any]) -> None:
        self.args = args
        self.plan = dict(plan)
        self.tasks = validate_task_order(plan)
    ...
    def run(self) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        try:
            ...
            self.task_results = execute_task_sequence(self.tasks, self.execute_one)
            model_final = self.model.finalize(expected_task_count=16)
            self.model.close()
...
def _allocation_authority(
    ...
    job_gpu_raw = os.environ.get("SLURM_JOB_GPUS")
    step_gpu_raw = os.environ.get("SLURM_STEP_GPUS")
    node_count = os.environ.get("SLURM_JOB_NUM_NODES")
    job_gpus = _exact_gpu_indices(
        job_gpu_raw, expected_gpu_count, label="Slurm job"
    )
    ...
    return {
        ...
        "slurm_job_gpus_raw": job_gpu_raw,
        "slurm_job_gpu_indices": job_gpus,
        "slurm_step_gpus_raw": step_gpu_raw,
        "slurm_step_gpu_indices": step_gpus,
        "world_size": 4,
        "ulysses_size": 4,
...
def _complete_execution(
    ...
    if (
        ...
        or len(verified_rows) != 16
        or {row.get("task_id") for row in verified_rows} != set(TASK_IDS)
    ...
    attestation: dict[str, Any] = {
        ...
        "v2_report_exactly_cross_linked_to_all_task_chains": True,
        "runner_task_json_replayed_for_all_tasks": True,
        "all_tasks_attempted_exactly_once": True,
    ...
    if (
        attestation["same_model_capture_all_16"] is not True
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tasks = validate_task_order(plan)
    final_artifacts = _preflight_final_artifacts(args, tasks)
    identities = {
    ...
    try:
        ...
        execution = RunnerExecution(args, plan)
