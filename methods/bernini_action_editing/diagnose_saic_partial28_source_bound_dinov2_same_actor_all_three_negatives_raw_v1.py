#!/usr/bin/env python3
"""Operational r6 exact28 DINO diagnostic against all three same-actor sources."""
...
FROZEN_CYCLIC_SOURCE_NAME = "diagnose_saic_partial28_source_bound_dinov2_raw_v1.py"
FROZEN_CYCLIC_SOURCE_SHA256 = "e732c37353eb53f6ada5ab493ce00bc53a2f984eadca1d69d5d34702cbbe1521"
EXPECTED_LEGACY_CYCLIC_INPUT_MANIFEST_SHA256 = "8f00dc5e9650fab76f93e28c7129544a732fe925782af7c83cd4d4cee06cff96"
EXPECTED_LEGACY_CYCLIC_AGGREGATE_SHA256 = "cb474ebca8c1aa0cb8c5443c40cf778e9643fe3ee67164886828dd3fec91ac29"
EXPECTED_LEGACY_CYCLIC_AGGREGATE_RECEIPT_DIGEST = "f5a11f474063bfe5ac447581090f90689e9b79254e6ce0404959ac7b08bdca00"
...
if (
    ...
    raise RuntimeError("frozen r6 exact28 cyclic source-bound diagnostic differs")

import diagnose_saic_partial47_source_bound_dinov2_raw_v1 as frozen  # noqa: E402
import diagnose_saic_partial28_source_bound_dinov2_raw_v1 as cyclic_frozen  # noqa: E402


core = frozen.core
SCHEMA_VERSION = (
    "bernini-saic-r6-partial28-source-bound-dinov2-"
...
EXPECTED_CANDIDATE_NEGATIVE_PAIR_COUNT = 84
...
def _configure_core() -> None:
    # Importing the frozen exact28 cyclic specialization rewrites both the
...
def partition_indices(count: int, rank: int, world_size: int) -> tuple[int, ...]:
    ...
    if sizes != EXPECTED_PARTITION_SIZES:
        raise AllThreeNegativeRawError("r6 exact28 all-three partition sizes differ")
    return indices
...
def _legacy_aggregate(
    ...
    if (
        ...
        or value.get("coverage") != "exactly_once_complete_r6_partial28_source_bound_raw"
...
def build_manifest(args: Any) -> int:
    ...
    cyclic_frozen._validate_fixed_build_inputs(args)
    ...
    if len(paths) != EXPECTED_ATTEMPT_COUNT:
        raise AllThreeNegativeRawError(f"r6 exact28 bank receipt count differs: {len(paths)}")
    rows = []
    for path in paths:
    ...
    *,
    ...
    return {
        **evidence,
        ...
        "all_28_legacy_cyclic_raw_metrics_exact": True,
    }
...
def aggregate(args: Any) -> int:
    ...
    if set(by_index) != set(range(EXPECTED_ATTEMPT_COUNT)):
        raise AllThreeNegativeRawError("shards do not cover r6 exact28")
    ...
    if (
        ...
        raise AllThreeNegativeRawError("aggregate 84/executed15/registered24 pair closure differs")
    ...
    unsigned = {
        ...
        "coverage": "exactly_once_complete_r6_partial28_same_actor_all_three_negatives_raw",
