#!/usr/bin/env python3
"""Render forward/noop shards from the frozen prospective calibration split."""
...
SCHEMA_VERSION = "bernini-prospective-calibration-anchor-shard-v1"
RELEASE_BRANCHES = ("noop", "forward")
AUTHORITY = {
    "calibration_generation_only": True,
...
class CalibrationAnchorShardError(RuntimeError):
    """Raised before an incomplete calibration shard can be accepted."""
...
def released_rows(
    ...
    if (
        ...
        or any(row["analysis_split"] != "calibration" for row in rows)
        or len(rows) != 2 * len(cells)
    ):
        raise CalibrationAnchorShardError("calibration forward/noop closure differs")
...
def receipt(
    *, manifest_sha256: str, manifest_digest: str,
    ...
    unsigned = {
        ...
        "status": "calibration_forward_noop_complete_decoded_review_pending",
...
def command_run(args: argparse.Namespace) -> int:
    ...
    if any(entries_root.iterdir()):
        raise CalibrationAnchorShardError("entries root must be fresh")
    receipts = [
        base._run_entry(
            row,
    ...
    base._write_create_only(output_root / "calibration-anchor-shard.receipt.json", value)
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    observed = base._read_object(
        base._plain_file(
            output_root / "calibration-anchor-shard.receipt.json",
            label="calibration anchor shard receipt",
        ),
        label="calibration anchor shard receipt",
    )
    if observed != expected:
        raise CalibrationAnchorShardError("calibration anchor shard receipt differs")
...
if __name__ == "__main__":
    ...
    except (CalibrationAnchorShardError, base.FactorialBranchRunError) as error:
        print(f"[calibration-anchor-shard] ERROR: {error}", file=sys.stderr)
        raise SystemExit(2)
