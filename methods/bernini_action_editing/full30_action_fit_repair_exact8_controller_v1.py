#!/usr/bin/env python3
"""Fail-closed controller for BOX-EXP-011 fit action/incomplete repair exact8.
...
import full30_action_fit_repair_exact8_plan_v1 as plan_contract  # noqa: E402
...
CONTROLLER_PLAN_SCHEMA = "bernini-full30-action-fit-repair-exact8-controller-plan-v1"
REVIEW_SCHEMA = "bernini-full30-action-fit-repair-exact8-review-admission-v1"
MATERIALIZER_GATE_SCHEMA = (
    "bernini-full30-action-fit-repair-exact8-materializer-gate-v1"
)
TERMINAL_HOST_GATE_SCHEMA = (
    "bernini-full30-action-fit-repair-exact8-terminal-host-memory-gate-v1"
)
COMPLETION_SCHEMA = "bernini-full30-action-fit-repair-exact8-completion-v1"
HOLDER_JOB = "136140"
...
class FitRepairExact8ControllerError(RuntimeError):
    """Raised before unbound or over-authorized exact8 state can pass."""


def fail(message: str) -> NoReturn:
    raise FitRepairExact8ControllerError(message)
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise FitRepairExact8ControllerError(
...
def plain_file(value: str | Path, label: str) -> Path:
    ...
    try:
        ...
        raise FitRepairExact8ControllerError(f"{label} is unavailable") from error
    require(
        resolved == path
    ...
    try:
        result = json.loads(
            ...
            parse_constant=lambda token: (_ for _ in ()).throw(
                FitRepairExact8ControllerError(
                    f"non-finite JSON constant is forbidden: {token}"
                )
            ),
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise FitRepairExact8ControllerError(f"{label} is not valid JSON") from error
...
def validate_release_tree(
    *, method_root: str | Path, manifest_path: str | Path,
    ...
    try:
        ...
        raise FitRepairExact8ControllerError("release method root is unavailable") from error
    require(
        resolved_root == root
    ...
    except release.FitRepairExact8ReleaseError as error:
        raise FitRepairExact8ControllerError(str(error)) from error
    require(
...
def _controller_plan_value(
    *, method_root: Path, manifest_path: Path, manifest_sha256: str,
    ...
    unsigned = {
        ...
        "purpose": "repair locked fit-r13 action/incomplete authoring failures",
        "scientific_target": "unchanged action-versus-before-terminal media eligibility",
        "learning_target": "N/A; optimizer-free frozen data authoring",
        "numeric_target": {
            "action_full81_pass": [4, 4],
            "incomplete_full81_pass": [4, 4],
            "complete_pair_full81_pass": [4, 4],
            "optimizer_updates": 0,
        },
        "dataset": "fit_action_incomplete_repair_exact8",
        "training_steps": 0,
        "baseline": (
            "frozen fit-r13 action/incomplete under the locked blind verdict: "
            "action 2/4, incomplete 0/4, complete pair 0/4"
        ),
        "core_validation": (
        ...
        "exact8_plan": {
            ...
            "prompt_bundle_sha256": plan_contract.PROMPT_BUNDLE_SHA256,
        },
        "holder": {
            "job_id": 136140,
            "node": HOLDER_NODE,
            "choice": "free preferred holder; avoids waiting behind confirmation",
...
def validate_review_admission(
    ...
    require(
        ...
        and value.get("review_population") == "fit_action_incomplete_repair_exact8"
...
def validate_exact8_audit(
    ...
    expected_fields = {
        ...
        "branch_order_per_cell",
        "candidate_receipts",
        "action_incomplete_gaussian_pair_proofs",
        "all_candidates_exact81",
        "independent_full81_review_performed",
        "review_admission_authorized",
        "materializer_same_state_threshold_gate_present",
        "q_input_authorized",
    ...
    require(set(value) == expected_fields, "exact8 generation audit field closure differs")
    require(
        isinstance(value.get("plan_path"), str)
        and isinstance(value.get("plan_file_sha256"), str)
        and SHA256_RE.fullmatch(value["plan_file_sha256"]) is not None,
        "exact8 generation audit plan reference differs",
    )
    try:
        audit_plan, audit_plan_path, audit_plan_sha = plan_contract.load_plan(
            value["plan_path"], value["plan_file_sha256"]
        )
    except plan_contract.FitRepairExact8PlanError as error:
        raise FitRepairExact8ControllerError(str(error)) from error
    require(
        audit_plan == exact8_plan
        and str(audit_plan_path) == value["plan_path"]
        and audit_plan_sha == value["plan_file_sha256"],
        "exact8 generation audit plan replay differs",
    )
    expected_ids = [task["candidate_id"] for task in exact8_plan["admission_tasks"]]
    rows = value.get("candidate_receipts")
    require(
        value.get("plan_digest") == exact8_plan["plan_digest"]
        and value.get("dataset") == "fit_action_incomplete_repair_exact8"
        and value.get("candidate_count") == 8
        and value.get("comparator_cell_count") == 4
        and value.get("branch_order_per_cell")
        == list(plan_contract.ADMISSION_BRANCH_ORDER)
        ...
        and value.get("materializer_same_state_threshold_gate_present") is False
    ...
    resource = generator.load_resource_contract(
        generator.METHOD_ROOT
        / "tools"
        / generator.RESOURCE_SPECIALIZED_BASENAME
    )
    validated_receipts: dict[str, Mapping[str, Any]] = {}
    receipt_fields = {
        "candidate_id",
    ...
    for row, task in zip(rows, exact8_plan["admission_tasks"]):
        require(
            ...
            and row.get("semantic_branch") == task["semantic_branch"]
            and isinstance(row.get("path"), str)
            and isinstance(row.get("file_sha256"), str)
            and SHA256_RE.fullmatch(row["file_sha256"]) is not None
            and isinstance(row.get("receipt_digest"), str)
            and SHA256_RE.fullmatch(row["receipt_digest"]) is not None,
            f"candidate receipt reference differs: {task['candidate_id']}",
        )
        receipt_path = plain_file(
            row["path"], f"formal candidate receipt {task['candidate_id']}"
        )
        require(
            str(receipt_path) == row["path"]
            and receipt_path.name == "pair-v5-t2v-calibration-receipt.json"
            and receipt_path.parent.name == task["candidate_id"]
            and file_sha256(receipt_path) == row["file_sha256"],
            f"candidate receipt path/SHA differs: {task['candidate_id']}",
        )
        try:
            receipt = generator._validate_candidate_receipt(
                resource, task, receipt_path
            )
        except generator.FitRepairExact8GenerationError as error:
            raise FitRepairExact8ControllerError(str(error)) from error
        require(
            receipt.get("receipt_digest") == row["receipt_digest"]
            and file_sha256(receipt_path) == row["file_sha256"],
            f"candidate receipt digest differs: {task['candidate_id']}",
        )
        validated_receipts[task["candidate_id"]] = receipt

    recomputed_proofs: list[Mapping[str, Any]] = []
    for cell in exact8_plan["seed_cells"]:
        tasks = [
            task
            for task in exact8_plan["admission_tasks"]
            if task["calibration_group_id"] == cell["calibration_group_id"]
        ]
        require(
            [task["candidate_id"] for task in tasks] == cell["candidate_ids"],
            f"Gaussian proof cell task order differs: {cell['calibration_group_id']}",
        )
        try:
            recomputed_proofs.append(
                generator._gaussian_pair_proof(
                    tasks,
                    [validated_receipts[task["candidate_id"]] for task in tasks],
                )
            )
        except generator.FitRepairExact8GenerationError as error:
            raise FitRepairExact8ControllerError(str(error)) from error
    require(
        value.get("action_incomplete_gaussian_pair_proofs")
        == recomputed_proofs,
        "exact8 action/incomplete Gaussian proofs do not replay",
    )
    return value
...
def seal_terminal_host_gate(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    unsigned = {
        ...
        "measurement_phase": "terminal_after_fit_repair_exact8",
