#!/usr/bin/env python3
"""Strict source-only inference for Bernini CDF-LoRA.
...
INFERENCE_RECEIPT_SCHEMA = "bernini-r-1p3b-cdf-lora-inference-receipt-v1"
LEGACY_TRAINING_RECEIPT_SCHEMA = "bernini-r-1p3b-cdf-lora-receipt-v1"
LEGACY_TRAINING_METHOD = "counterfactual-delta-field-lora-v1"
SAMPLING_MODES = ("differential", "standard")


class DeltaInferenceError(RuntimeError):
    """Raised before output when a CDF-LoRA inference contract is violated."""
...
def validate_training_adapter_contract(
    ...
    *,
    ...
    # This legacy sampler evaluates action/no-op on two different noisy paths.
    # It must never silently accept the v2 exact-same-state C2FR adapter merely
    # because train_delta_lora's current module constants changed.
    if receipt.get("schema_version") != LEGACY_TRAINING_RECEIPT_SCHEMA:
        raise DeltaInferenceError("training receipt schema differs")
    if receipt.get("method") != LEGACY_TRAINING_METHOD:
...
def validate_runtime_exact_lora_targets(
    base_model: Any,
    targets: Sequence[str],
    *,
    expected_named_scope: Optional[str] = None,
) -> list[str]:
    """Rebind an audited receipt target list to the runtime Wan model.

    Receipt validation, rather than this low-level loader, owns the named LoRA
    scope policy.  The loader only accepts canonical, fully-qualified, unique
    attention projection names that exist exactly in the runtime model.  This
    keeps legacy named scopes strict while allowing a newer method to carry an
    independently audited exact subset such as v7's 46 projections.
    """

    if isinstance(targets, (str, bytes)):
        raise DeltaInferenceError("runtime LoRA targets must be a sequence")
    ...
    if (
        ...
        raise DeltaInferenceError(
            "runtime LoRA targets must be sorted unique exact names"
        )
    try:
    ...
    missing = [name for name in normalized if name not in available_set]
    if missing:
        raise DeltaInferenceError(
            f"runtime Bernini lacks exact receipt LoRA targets: {missing[:4]}"
        )
    if expected_named_scope is not None:
        if expected_named_scope not in motion.MODULE_SCOPES:
            raise DeltaInferenceError("runtime named LoRA scope is invalid")
        try:
            scoped = motion.select_lora_scope(available, expected_named_scope)
        except motion.MotionContractError as error:
            raise DeltaInferenceError(str(error)) from error
        if scoped != normalized:
            raise DeltaInferenceError(
                "runtime exact targets differ from the receipt named LoRA scope"
            )
    return normalized
...
def _strict_load_adapter(
    *,
    ...
    exact_targets = validate_runtime_exact_lora_targets(
        base_model,
        targets,
        expected_named_scope=expected_named_scope,
    )
    config = LoraConfig.from_pretrained(str(adapter_dir), local_files_only=True)
    config.target_modules = set(exact_targets)
    ...
    expected = expected_adapter_state_keys(exact_targets)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run strict source-only Bernini CDF-LoRA inference"
    )
    ...
    parser.add_argument("--sampling-mode", choices=SAMPLING_MODES, default="differential")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    model, adapter_tensor_count = _strict_load_adapter(
        ...
        expected_named_scope=adapter_identity["scope"],
    )
