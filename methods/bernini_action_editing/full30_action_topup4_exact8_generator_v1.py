#!/usr/bin/env python3
"""Generate and audit the BOX-EXP-010 cross-anchor topup4 exact8 media.
...
SHARD_SCHEMA = "bernini-full30-action-topup4-exact8-shard-v1"
AUDIT_SCHEMA = "bernini-full30-action-topup4-exact8-audit-v1"
GAP_SCHEMA = "bernini-full30-action-topup4-exact8-gap-v1"
...
class Topup4Exact8GenerationError(RuntimeError):
    """Raised before partial or widened topup4 output can pass."""


def fail(message: str) -> NoReturn:
    raise Topup4Exact8GenerationError(message)
...
def canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise Topup4Exact8GenerationError(
...
def plain_file(value: str | Path, label: str) -> Path:
    ...
    try:
        ...
        raise Topup4Exact8GenerationError(f"{label} is unavailable") from error
    require(
        resolved == path
...
def load_resource_contract(value: str | Path) -> ModuleType:
    ...
    module_name = "_bernini_full30_topup4_resource_aa2f5c01"
...
def _runtime_binding(
    ...
    try:
        ...
        raise Topup4Exact8GenerationError(str(error)) from error
    require(
...
def run_shard(args: argparse.Namespace) -> int:
    ...
    require(
        ...
        "sealed topup4 exact4 shard scope differs",
    )
    ...
    for task in matching:
        ...
        try:
            ...
            if isinstance(error, Topup4Exact8GenerationError):
                raise
            raise Topup4Exact8GenerationError(str(error)) from error
    ...
    unsigned = {
        ...
        "counts_toward_topup4_exact8": True,
    ...
    write_create_only(output / "topup4-exact8-shard-receipt-v1.json", receipt)
    return 0
...
def audit_exact8(
    *, plan_path: str | Path, expected_plan_sha256: str,
    ...
    require(len(generation_roots) == 2, "topup4 exact8 requires exactly two roots")
