#!/usr/bin/env python3
"""Operational exact47 DINO diagnostic against all three same-actor sources."""
...
FROZEN_CYCLIC_SOURCE_NAME = "diagnose_saic_partial47_source_bound_dinov2_raw_v1.py"
FROZEN_CYCLIC_SOURCE_SHA256 = "ffbc9ba149d1ddadf704dd8258678a8893235e328da4c7601e98d63ba37aa7a2"
EXPECTED_LEGACY_CYCLIC_AGGREGATE_SHA256 = "1eb8f31ca05c8d4b4e1008a995f69a36da7b1f8da69494479c5468ca79d79884"
...
if (
    ...
    raise RuntimeError("frozen exact47 cyclic source-bound diagnostic differs")
...
EXPECTED_CANDIDATE_NEGATIVE_PAIR_COUNT = 141
EXPECTED_REGISTERED_DIRECTED_SOURCE_PAIR_COUNT = 24
EXPECTED_EXECUTED_DIRECTED_SOURCE_PAIR_COUNT = 21
EXPECTED_EXECUTED_CORRECT_SOURCE_COUNT = 7
...
FROZEN_CYCLIC_AGGREGATE_SCHEMA = frozen.AGGREGATE_SCHEMA
FROZEN_CYCLIC_AUTHORITY_CLOSURE = dict(frozen.AUTHORITY_CLOSURE)
AUTHORITY_CLOSURE = {
...
def partition_indices(count: int, rank: int, world_size: int) -> tuple[int, ...]:
    ...
    if sizes != EXPECTED_PARTITION_SIZES:
        raise AllThreeNegativeRawError("partial47 all-three partition sizes differ")
    return indices
...
def _legacy_aggregate(
    ...
    if (
        ...
        or value.get("coverage") != "exactly_once_complete_partial47_source_bound_raw"
...
def build_manifest(args: Any) -> int:
    ...
    if len(paths) != EXPECTED_ATTEMPT_COUNT:
        raise AllThreeNegativeRawError(f"exact47 bank receipt count differs: {len(paths)}")
    rows = []
    for path in paths:
...
def load_input_manifest(
    ...
    *,
    ...
    if (
        ...
        or missing_correct_iids != {EXPECTED_MISSING_CORRECT_SOURCE_IID}
    ...
    *,
    ...
    return {
        **evidence,
        ...
        "all_47_legacy_cyclic_raw_metrics_exact": True,
    }
...
def aggregate(args: Any) -> int:
    ...
    if set(by_index) != set(range(EXPECTED_ATTEMPT_COUNT)):
        raise AllThreeNegativeRawError("shards do not cover exact partial47")
    ...
    if (
        ...
        raise AllThreeNegativeRawError("aggregate 141/executed21/registered24 pair closure differs")
    ...
    unsigned = {
        ...
        "missing_correct_source_iids": [EXPECTED_MISSING_CORRECT_SOURCE_IID],
        ...
        "coverage": "exactly_once_complete_partial47_same_actor_all_three_negatives_raw",
