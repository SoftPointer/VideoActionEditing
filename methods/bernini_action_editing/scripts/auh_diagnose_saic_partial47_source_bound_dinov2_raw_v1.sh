#!/usr/bin/env bash
# Run exact47 source-bound raw diagnostics inside an owned 8-GPU allocation.
...
if [[ ! -d "$attempts_root" || ! -f "$source_manifest" || -L "$source_manifest" || ! -d "$visual_checkpoint" || ! -f "$checkpoint_manifest" || ! -f "$evaluator_spec" || ! -f "$visual_scorer_source" || ! -f "$visual_contract_source" ]]; then
...
echo "partial47 source-bound DINO raw ${mode} complete: $output_root"
