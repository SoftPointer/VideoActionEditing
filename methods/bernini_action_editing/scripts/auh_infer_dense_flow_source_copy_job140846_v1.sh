...
source_tree="$root/stage1/source-97bea69-overlay"
...
decode_one() {
  ...
  local scale="$4"
  local scale_tag="$5"
  local source_copy_checkpoint="$root/stage1/train_source_copy_${mode_tag}_out3e4_cond3e4_s160_r1/checkpoint-00000${step}"
  local output_dir="$root/stage1/infer/source_copy_${mode_tag}_s${step}_k${scale_tag}_${row}_r1"
  local scratch="/tmp/dense-flow-source-copy-${mode_tag}-${step}-${scale_tag}-${row}-140846-${SLURM_STEP_ID}"
  test ! -e "$output_dir"
  ...
  "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
    ...
    --method-source-revision 97bea69beeb174e0e8bf36128cfda124e89f3281 \
    --method-source-archive-sha256 8093a8bec2f8e4b4319852b31c4ce73dae19f7be170e68db4b427ae72e314e05
}

decode_one phase_aligned pa 020 1.0 100
decode_one phase_aligned pa 080 1.0 100
decode_one phase_aligned pa 160 1.0 100
decode_one phase0_broadcast p0 020 1.0 100
decode_one phase0_broadcast p0 080 1.0 100
decode_one phase0_broadcast p0 160 1.0 100
