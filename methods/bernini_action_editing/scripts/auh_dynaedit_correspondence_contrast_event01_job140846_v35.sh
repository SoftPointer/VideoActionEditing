...
# Round 35 fixes the failed cross-scene seam.  Every target token first finds
# its pure-T2V anchor counterpart using phase-0 value features.  Dynamic-minus-
# static anchor trajectories are then routed through Q/K or attention output.
# Hard arms replace selected temporal trajectories but retain current phase 0.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=CORRCON_QK_A025_P16_R8
    export TRANSPORT_OVERRIDE=temporal_correspondence_contrast_qk
    export STRENGTH_OVERRIDE=0.25
    ;;
  auh7-1b-gpu-246:1)
    export LABEL_OVERRIDE=CORRCON_ATTN_A025_P16_R8
    export TRANSPORT_OVERRIDE=temporal_correspondence_contrast_attn_output
    export STRENGTH_OVERRIDE=0.25
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=CORRCON_HARD_QK_P16_R8
    export TRANSPORT_OVERRIDE=temporal_correspondence_hard_contrast_qk
    export STRENGTH_OVERRIDE=1.0
    ;;
  auh7-1b-gpu-247:1)
    export LABEL_OVERRIDE=CORRCON_HARD_ATTN_P16_R8
    export TRANSPORT_OVERRIDE=temporal_correspondence_hard_contrast_attn_output
    export STRENGTH_OVERRIDE=1.0
    ;;
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_correspondence_contrast_event01_v35"
