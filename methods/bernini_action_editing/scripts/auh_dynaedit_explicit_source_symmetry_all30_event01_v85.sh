...
case "$host" in
  auh7-1b-gpu-246)
    label=NATIVE_EXPLSRC_ANCHORREL_PATCHV_A100_ALL40_ALL30_P24_R8
    transport=event01_dynamic_role_graph_source_patch_value_attn_output
    ;;
  auh7-1b-gpu-247)
    label=NATIVE_EXPLSRC_SIDEALIGN_PATCHV_A100_ALL40_ALL30_P24_R8
    transport=event01_dynamic_role_graph_source_patch_value_side_aligned_attn_output
    ;;
  *)
    echo "Round 85 is restricted to nodes 246/247" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_explicit_source_symmetry_all30_event01_v85"
