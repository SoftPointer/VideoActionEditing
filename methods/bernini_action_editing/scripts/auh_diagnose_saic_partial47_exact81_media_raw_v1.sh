...
fail() {
  echo "[saic-r4-exact81-media] ERROR: $*" >&2
  exit 2
}
...
task_scratch="$(mktemp -d "${scratch_parent%/}/saic-r4-exact81-media-${SLURM_JOB_ID}.XXXXXX")"
...
cleanup() {
  ...
  case "${task_scratch:-}" in
    "${scratch_parent%/}/saic-r4-exact81-media-${SLURM_JOB_ID}."*) ;;
    *) exit 2 ;;
  esac
...
if [[ "$mode" == "preflight" ]]; then
  ...
  [[ "$(find "$output_root/diagnostics" -maxdepth 1 -type f -name '*.json' | wc -l)" -eq 47 ]] || \
    fail "full exact47 diagnostic count differs"
fi

echo "[saic-r4-exact81-media] PASS mode=$mode candidates=$([[ "$mode" == full ]] && echo 47 || echo 1) exact81=true full80=true CPU_only=true authority=zero ranking=false selection=false output=$output_root"
