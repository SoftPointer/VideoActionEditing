...
case "$host:$mode" in
  auh7-1b-gpu-246:value)
    label=NATIVE_DYNROLE_OBJECTV_FORCEDP1_A050_B4_9_CORRIDOR_P24_R8
    transport=event01_dynamic_source_object_value_attn_output
    ;;
  auh7-1b-gpu-247:output)
    label=NATIVE_DYNROLE_OBJECTOUT_FORCEDP1_A050_B4_9_CORRIDOR_P24_R8
    transport=event01_dynamic_source_object_output_attn_output
    ;;
  *)
    echo "Round 71 mode $mode is not assigned to host $host" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_forcedp1_attention_event01_v71"
export TRANSPORT_OVERRIDE="$transport"
export STRENGTH_OVERRIDE=0.5
