...
# In-distribution Bernini edit/no-op field under DynaEdit's published raw
# source/target CFG pairs.  Full source motion is still absent from the visual
# prefix; only its initial latent phase is repeated as the identity carrier.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=STATICEDITOR_CFG25_45_SGA001C_NOANCHOR
    source_cfg=2.5
    target_cfg=4.5
    transport_steps=0
    ;;
  auh7-1b-gpu-246:1)
    label=STATICEDITOR_CFG25_45_SGA001C_ATTCON_A025_ALL40
    source_cfg=2.5
    target_cfg=4.5
    transport_steps=40
    ;;
  auh7-1b-gpu-247:0)
    label=STATICEDITOR_CFG45_85_SGA001C_NOANCHOR
    source_cfg=4.5
    target_cfg=8.5
    transport_steps=0
    ;;
  auh7-1b-gpu-247:1)
    label=STATICEDITOR_CFG45_85_SGA001C_ATTCON_A025_ALL40
    source_cfg=4.5
    target_cfg=8.5
    transport_steps=40
    ;;
...
output_root="$stage/dynaedit_editor_raw_cfg_event01_v21"
...
scratch="/tmp/dynaedit-editor-raw-cfg-$label-140846"
...
exec "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  ...
  --field-model first_phase_source_rv2v \
