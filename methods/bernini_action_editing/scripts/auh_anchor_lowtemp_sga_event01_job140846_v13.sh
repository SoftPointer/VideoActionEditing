...
# Correct DynaEdit low-temperature SGA.  Node 246 isolates the temperature fix
# on attention-output transport; node 247 moves the full pure-T2V velocity
# trajectory after cancelling each field's frame-0 basis.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=SGA001_RAW_TRO_A050_EARLY8; transport=temporal_residual_attn_output; strength=0.50 ;;
  auh7-1b-gpu-247:0)
    label=SGA001_RAW_TRVEL_A025_EARLY8; transport=temporal_residual_velocity; strength=0.25 ;;
...
output_root="$stage/anchor_lowtemp_sga_event01_v13"
...
scratch="/tmp/anchor-lowtemp-sga-$label-140846"
