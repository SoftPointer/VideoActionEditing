#!/usr/bin/env bash
# Run r6 exact28 all-three same-actor raw diagnostics in one owned 8-GPU allocation.
...
readonly expected_legacy_sha=cb474ebca8c1aa0cb8c5443c40cf778e9643fe3ee67164886828dd3fec91ac29
...
if [[ "$attempts_root" != "$expected_attempts" || "$root_spec_sha256" != "$expected_root_sha" || "$source_manifest" != "$expected_source_manifest" || "$source_manifest_sha256" != "$expected_source_manifest_sha" ]]; then
  echo "r6 exact28 root/spec/source arguments differ" >&2
  exit 66
fi
if [[ "$legacy_cyclic_aggregate_sha256" != "$expected_legacy_sha" ]]; then
  echo "r6 exact28 legacy cyclic aggregate SHA argument differs" >&2
  exit 66
fi
...
if [[ "$(sha256sum "$legacy_cyclic_aggregate" | awk '{print $1}')" != "$expected_legacy_sha" ]]; then
  echo "r6 exact28 legacy cyclic aggregate file SHA differs" >&2
  exit 66
fi
...
echo "r6 exact28 all-three same-actor DINO raw ${mode} complete: $output_root"
