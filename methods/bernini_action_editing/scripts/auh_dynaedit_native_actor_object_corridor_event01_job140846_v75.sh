...
case "$host:$mode" in
  auh7-1b-gpu-246:observer)
    label=NATIVE_ACTOROBJ_CORRIDOR_OBSERVER_ENVELOPE_P24_R8
  ...
  auh7-1b-gpu-247:value)
    label=NATIVE_ACTOROBJ_CORRIDOR_OBJECTV_FORCEDP1_A050_ALL40_P24_R8
    ...
    ;;
  *)
    echo "Round 75 mode $mode is not assigned to host $host" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_actor_object_corridor_event01_v75"
