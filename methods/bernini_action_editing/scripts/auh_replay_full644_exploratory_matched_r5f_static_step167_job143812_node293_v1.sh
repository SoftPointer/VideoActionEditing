...
try: os.stat(EVIDENCE,dir_fd=parent,follow_symlinks=False)
except FileNotFoundError: pass
else: raise RuntimeError("evidence not fresh")
efd=os.open(EVIDENCE,os.O_RDWR|os.O_CREAT|os.O_EXCL|getattr(os,"O_NOFOLLOW",0),0,dir_fd=parent); eb=os.fstat(efd)
if not stat.S_ISREG(eb.st_mode) or stat.S_IMODE(eb.st_mode)!=0 or eb.st_uid!=2012 or eb.st_gid!=2000 or eb.st_nlink!=1 or eb.st_size!=0: raise RuntimeError("evidence staging differs")
...
if step!=FULL or name!=JOB_NAME or state!="COMPLETED" or exitcode!="0:0" or elapsed!="1" or cpus!="4" or node!=NODE or maxrss!="1500K": raise RuntimeError("sacct semantics differ")
