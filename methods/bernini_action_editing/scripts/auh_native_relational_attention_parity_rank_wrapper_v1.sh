...
# Rank-private MIOpen state must be bound before Python imports torch.  AUH's
# shared /tmp can contain another process' read-only find-db, which is an
# infrastructure failure unrelated to the observer parity gate.
if [[ "$#" -ne 2 ]]; then
...
local_tmp_root="/tmp/bernini-relational-parity-u${UID}-j${SLURM_JOB_ID:?SLURM_JOB_ID is required}-s${SLURM_STEP_ID:?SLURM_STEP_ID is required}"
