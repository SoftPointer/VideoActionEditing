...
# Round-28 R3 direct four-node payload. Each node owns one four-rank SP4
# model. Only rank zero loads/encodes UMT5; the seven prompt embeddings are
# broadcast after UMT5 is retired. The anchor teacher uses a matched capped
# (latent sigma, model timestep) pair. This is inference-only.
if [ "$#" -ne 0 ]; then
  echo "usage: $0" >&2
  exit 2
fi
...
case "$(hostname -s)" in
  auh7-1b-gpu-246)
    label=DYNSTATIC_VEL_A010_EARLY8_CAP08
    ...
    ;;
  auh7-1b-gpu-248)
    label=DYNSTATIC_VEL_A010_EARLY8_CAP06
    transport=temporal_contrast_velocity
    strength=0.10
    blocks=8-13
    anchor_sigma_cap=0.6
    ;;
  auh7-1b-gpu-279)
    label=DYNSTATIC_SELFATT_A003_EARLY8_B8_13_CAP08
    ...
    ;;
  auh7-1b-gpu-247)
    label=DYNSTATIC_CROSSATT_A003_EARLY8_B4_9_CAP08
...
output_root="$stage/dynaedit_anchor_sigma_cap_event01_v28r3_c9fb3668_5d11aada_3e7f8e44"
...
scratch="/tmp/dynaedit-anchor-sigma-cap-r3-$label-140846-four-nodes"
