...
case "$host" in
  auh7-1b-gpu-246)
    label=NATIVE_TIMED_TARGETSTATE_TEMPORAL_FULL_ALL40_FORCEDC0_NOPRES
    transport=native_targetstate_temporal_delta_replacement
    ;;
  auh7-1b-gpu-247)
    label=NATIVE_TIMED_TARGETSTATE_TEMPORAL_SPARSE25_ALL40_FORCEDC0_NOPRES
    transport=native_targetstate_sparse25_temporal_delta_replacement
    ;;
  *)
    echo "Round 58 is restricted to Job 140846 nodes 246-247" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_targetstate_temporal_event01_v58"
