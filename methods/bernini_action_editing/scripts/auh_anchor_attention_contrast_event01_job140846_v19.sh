...
# Prepared, but launched only after Round 18 visual audit.  This transports the
# action-minus-noop difference at integrated self-attention output, using two
# pure-T2V anchor forwards at the same state/candidate and never copying an
# absolute anchor attention map.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=CAPTIONSTATIC_SGA001C_ATTCON_A025_ALL40_B8_13
    field_model=first_phase_caption_i2v
    strength=0.25
    transport_steps=40
    ;;
  auh7-1b-gpu-246:1)
    label=CAPTIONSTATIC_SGA001C_ATTCON_A050_EARLY8_B8_13
    field_model=first_phase_caption_i2v
    strength=0.50
    transport_steps=8
    ;;
  auh7-1b-gpu-247:0)
    label=STATICFIELD_SGA001C_ATTCON_A025_ALL40_B8_13
    field_model=first_phase_source_rv2v
    strength=0.25
    transport_steps=40
    ;;
  auh7-1b-gpu-247:1)
    label=STATICFIELD_SGA001C_ATTCON_A050_EARLY8_B8_13
    field_model=first_phase_source_rv2v
    strength=0.50
    transport_steps=8
    ;;
...
output_root="$stage/anchor_attention_contrast_event01_v19"
...
scratch="/tmp/anchor-attention-contrast-$label-140846"
