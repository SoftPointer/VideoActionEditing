#!/usr/bin/env bash
# BOX-EXP-010 minimal cross-anchor topup4 exact8 on retained 136140/gpu215.
#
# Generation only: one disposable fit-r13 physical safe_open/r10-r13 parity
# smoke, followed by two serial WORLD4 shards containing exactly four selected
# comparator cells x (action, incomplete).  There is no diagnostic lane and no
# optimizer.  This script never cancels, releases, or requeues the parent.

set -Eeuo pipefail
umask 077

fail() { echo "[full30-topup4-exact8-v1] ERROR: $*" >&2; exit 2; }
sha256_file() { sha256sum "$1" | awk '{print $1}'; }

readonly confirm="${T4_CONFIRM:?explicit launch confirmation required}"
readonly run_root="${T4_RUN_ROOT:?set fresh run root}"
readonly method_root="${T4_METHOD_ROOT:?set extracted release method root}"
readonly method_archive="${T4_METHOD_ARCHIVE:?set release archive}"
readonly method_archive_sha="${T4_METHOD_ARCHIVE_SHA256:?pin release archive}"
readonly method_manifest="${T4_METHOD_MANIFEST:?set release manifest}"
readonly method_manifest_sha="${T4_METHOD_MANIFEST_SHA256:?pin release manifest}"
readonly seed1_spec="${T4_SEED1_SPEC:?set seed1 reserve4 spec for disposable smoke}"
readonly seed2_spec="${T4_SEED2_SPEC:?set seed2 reserve4 spec for disposable smoke}"
readonly python_bin="${T4_PYTHON_BIN:?set frozen Python}"
readonly bernini_root="${T4_BERNINI_ROOT:?set Bernini root}"
readonly veomni_root="${T4_VEOMNI_ROOT:?set VeOmni root}"
readonly checkpoint="${T4_CHECKPOINT:?set Bernini checkpoint}"
readonly checkpoint_manifest="${T4_CHECKPOINT_MANIFEST:?set checkpoint manifest}"
readonly method_revision="${T4_METHOD_REVISION:?set content revision}"
readonly master_port="${T4_MASTER_PORT:?set two-port base}"
readonly r10_receipt="${T4_R10_COMPILE_SMOKE_RECEIPT:?bind r10 receipt}"
readonly r10_receipt_sha="${T4_R10_COMPILE_SMOKE_RECEIPT_SHA256:?pin r10 receipt}"
readonly r10_log="${T4_R10_GENERATION_LOG:?bind r10 log}"
readonly r10_log_sha="${T4_R10_GENERATION_LOG_SHA256:?pin r10 log}"

readonly holder_job=136140
readonly holder_node=auh7-1b-gpu-215
readonly launch_confirmation=launch-approved-BOX-EXP-010-minimal-cross-anchor-topup4-exact8-136140
...
[[ "${master_port}" =~ ^[0-9]+$ ]] && (( master_port >= 1024 && master_port <= 65534 )) || fail "two-port range differs"
...
readonly controller="${method_root}/full30_action_topup4_exact8_controller_v1.py"
readonly plan_tool="${method_root}/full30_action_topup4_exact8_plan_v1.py"
readonly generator="${method_root}/full30_action_topup4_exact8_generator_v1.py"
readonly resource="${method_root}/tools/reserve4_fixed_generation_sp4_136140_specialized_v1.py"
readonly release_builder="${method_root}/tools/build_full30_action_topup4_exact8_release_v1.py"
readonly launcher="${method_root}/scripts/auh_full30_action_topup4_exact8_136140_world4_v1.sh"
readonly selection="${method_root}/assets/full30_action_minimal_cross_anchor_topup4_v1.json"
readonly parent_registry="${method_root}/assets/mosaic_event_population_compact6_topup20_v1.json"

for path in "${method_archive}" "${method_manifest}" "${seed1_spec}" "${seed2_spec}" "${python_bin}" "${checkpoint_manifest}" "${r10_receipt}" "${r10_log}" "${controller}" "${plan_tool}" "${generator}" "${resource}" "${release_builder}" "${launcher}" "${selection}" "${parent_registry}"; do
...
[[ "$(sha256_file "${resource}")" == aa2f5c01c9d231ad5340cbb572c1523546fa2e148143ee1b5bf04f53f005f017 ]] || fail "136140 resource specialization differs"
[[ "$(sha256_file "${selection}")" == 72a1d58ede5381f57d2fa8ef895a7e9d5c11b3872e87ecbc3e08fec0cc5ef38e ]] || fail "topup4 selection differs"
[[ "$(sha256_file "${parent_registry}")" == 71906510d162e6626338b5785fd1cf55b437de5ba77d9b9b122ad761694f8e62 ]] || fail "parent registry differs"
...
if [[ "${role}" == child ]]; then
  ...
  "${python_bin}" -B "${controller}" validate-runtime \
    --controller-plan "${T4_CONTROLLER_PLAN}" \
    --expected-controller-plan-sha256 "${T4_CONTROLLER_PLAN_SHA256}" >/dev/null || fail "child runtime validation failed"
  ...
  task_scratch="$(mktemp -d -- "${scratch_parent}/topup4-exact8-${SLURM_JOB_ID}-${SLURM_STEP_ID}.XXXXXXXX")"
  ...
  "${python_bin}" -B "${resource}" smoke-sp4 \
    ...
    --r10-generation-log "${r10_log}" --expected-r10-generation-log-sha256 "${r10_log_sha}" || fail "physical safe_open/r10-r13 compile smoke failed"
  readonly smoke_receipt_sha="$(sha256_file "${smoke_receipt}")"

  run_exact4_shard() {
    local group_id="$1" visible="$2" port="$3" output
    ...
    output="${run_root}/generation/${group_id}"
    "${python_bin}" -B "${generator}" run-sp4 \
      --plan "${T4_EXACT8_PLAN}" --expected-plan-sha256 "${T4_EXACT8_PLAN_SHA256}" \
      ...
      --checkpoint "${checkpoint}" --checkpoint-content-manifest "${checkpoint_manifest}" \
      --method-source-revision "${method_revision}" --method-source-archive-sha256 "${method_archive_sha}" \
      --master-port "${port}" --output-dir "${output}" || fail "${group_id} exact4 generation failed"
    "${python_bin}" -B "${resource}" assert-host-memory-monitor-live >/dev/null || fail "post-shard host monitor boundary failed"
  }

  run_exact4_shard sp4-a 0,1,2,3 "${master_port}"
  run_exact4_shard sp4-b 4,5,6,7 "$((master_port + 1))"
  unset ROCR_VISIBLE_DEVICES
  "${python_bin}" -B "${generator}" audit-exact8 \
    --plan "${T4_EXACT8_PLAN}" --expected-plan-sha256 "${T4_EXACT8_PLAN_SHA256}" \
    --generation-root "${run_root}/generation/sp4-a" \
    --generation-root "${run_root}/generation/sp4-b" \
  ...
  "${python_bin}" -B "${controller}" seal-terminal-host-gate \
    ...
    --output "${run_root}/logs/terminal-topup4-exact8-host-gate.json" >/dev/null || fail "terminal exact8 host gate failed"
...
[[ "${parent_state}" == "RUNNING|${holder_node}|guangyi.chen" ]] || fail "retained parent 136140/gpu215 differs"
...
exact8_json="$("${python_bin}" -B "${plan_tool}" build-plan \
  --selection "${selection}" --expected-selection-sha256 72a1d58ede5381f57d2fa8ef895a7e9d5c11b3872e87ecbc3e08fec0cc5ef38e \
  --parent-registry "${parent_registry}" --expected-parent-registry-sha256 71906510d162e6626338b5785fd1cf55b437de5ba77d9b9b122ad761694f8e62 \
...
srun --jobid="${holder_job}" --nodelist="${holder_node}" --nodes=1 --ntasks=1 \
  ...
  env T4_CONTROLLER_PLAN="${controller_plan}" T4_CONTROLLER_PLAN_SHA256="${controller_plan_sha}" \
      T4_EXACT8_PLAN="${exact8_plan}" T4_EXACT8_PLAN_SHA256="${exact8_plan_sha}" \
      bash "${launcher}" __child >"${run_root}/logs/topup4-exact8-generation.log" 2>&1 &
child_pid=$!
wait "${child_pid}"
status=$?
child_pid=""
set -e
if (( status != 0 )); then tail -n 240 "${run_root}/logs/topup4-exact8-generation.log" >&2 || true; fail "all8 serial exact8 child failed status=${status}"; fi
[[ -f "${run_root}/generation-exact8-audit.json" && -f "${run_root}/logs/terminal-topup4-exact8-host-gate.json" ]] || fail "exact8 outputs are incomplete"

printf '%s\n' \
  'schema=bernini-full30-action-topup4-exact8-launch-status-v1' \
  'experiment_id=BOX-EXP-010' \
  'dataset=minimal_cross_anchor_topup4_exact8' \
  'formal_comparator_cell_count=4' \
  ...
  'r10_r13_parity=true' \
  'same_gaussian_action_incomplete_per_cell=true' \
  'independent_full81_review=pending' \
  'same_state_materializer_threshold_gate=pending' \
  'optimizer_created=false' \
  'optimizer_authorized=false' \
  'training_performed=false' \
  'parent_136140_cancelled_released_or_requeued=false' >"${run_root}/launcher.status"
echo 'BOX_EXP_010_TOPUP4_EXACT8_GENERATION_PENDING_REVIEW_AND_MATERIALIZER candidate_count=8 diagnostics=0 optimizer=false'
