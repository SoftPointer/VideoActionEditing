#!/usr/bin/env python3
"""One-shot receipt-gated exact35 + exact5 overlay -> fresh r64 HOLD package.
...
SCHEMA = "case01-object-trajectory-exact5-r64-overlay-package-controller-v2"
...
ATTEMPT_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v2."
    "materialize_attempt_v2.json"
)
EVIDENCE_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v2."
    "materialize_controller_evidence_v2.json"
)
INTERNAL_RECEIPT_RELATIVE = "authority/package_materialization_receipt_v3.json"
...
OVERLAY_ROOT_IDENTITY: list[int] = [
    48, 7093358014530864437, 2012, 2000, 16749, 2, 0, 4096, 0,
    1787369767167114564, 1787369814456054102,
]
VACE_PYTHON = Path(
...
OVERLAY_RECEIPT_SCHEMA = (
    "case01-object-trajectory-exact5-r5f-v3-source-overlay-v1-receipt"
)
...
def validate_overlay_receipt(
    ...
    if actual_files != set(OVERLAY_RELATIVES) or actual_directories != expected_directories:
        raise PackageControllerError("overlay exact5 physical tree differs")
...
def controller() -> dict[str, Any]:
    ...
    try:
        ...
        # is held.  All five source descriptors remain held through the child.
