...
decode_one() {
  ...
  if [ "$row" = row0 ]; then
    ...
    echo "row must be row0 or row3strict" >&2
    exit 2
  fi
  local checkpoint="$source_copy_root/checkpoint-00000${step}"
  local output_dir="$root/stage1/infer_opt2/${arm}_s${step}_start${schedule_start}_${row_tag}_r1"
  local scratch="/tmp/${arm}-s${step}-start${schedule_start}-${row_tag}-g${gpu_group}-140846"
