...
case "$host:$mode" in
  auh7-1b-gpu-246:action)
    label=NATIVE_OBSERVER_SGA_ACTIONGRAM_EARLY3_B4_9_P24_R8
    score=background_trust_anchor_action_003
    ;;
  auh7-1b-gpu-247:envelope)
    label=NATIVE_OBSERVER_SGA_ENVELOPE_EARLY3_B4_9_P24_R8
    score=background_trust_anchor_envelope_003
    ;;
  *)
    echo "Round 67 mode $mode is not assigned to host $host" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_observer_reward_event01_v67"
