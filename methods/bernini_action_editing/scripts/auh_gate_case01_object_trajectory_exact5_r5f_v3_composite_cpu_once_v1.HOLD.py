#!/usr/bin/env python3
"""One-shot four-process CPU admission for the fresh r5f-v3 composite.
...
SCHEMA = "case01-object-trajectory-exact5-r5f-v3-composite-cpu-controller-v1"
ATTEMPT_SCHEMA = SCHEMA + "-attempt"
RECEIPT_SCHEMA = (
    "case01-object-trajectory-exact5-r5f-v3-composite-cpu-admission-v1"
)
CONTROLLER_STATE = "HOLD_PENDING_FRESH_PACKAGE_PINS"
...
ATTEMPT_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v2."
    "composite_cpu_admission_attempt_v1.json"
)
...
STDOUT_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v2."
    "composite_cpu_admission_srun_v1.stdout.log"
)
STDERR_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v2."
    "composite_cpu_admission_srun_v1.stderr.log"
)
...
CPU_RECEIPT_FIELDS = frozenset({
    ...
    "module_binding", "side_effects", "cache_lifecycle",
...
PUBLICATION_SIZE: int | str = 2_530
PUBLICATION_DIGEST = (
...
MATERIALIZATION_SIZE: int | str = 41_351
...
PACKAGE_CONTROLLER_SIZE: int | str = 8_113
...
def read_fd(fd,size):
 ...
 return raw
if len(sys.argv)!=8: raise RuntimeError("composite child argv differs")
composite_fd=int(sys.argv[1]);composite_path=sys.argv[2];composite_sha=sys.argv[3];composite_size=int(sys.argv[4]);shared_fd=int(sys.argv[5]);shared_sha=sys.argv[6];rank=int(sys.argv[7])
...
module=types.ModuleType("_case01_r5f_v3_composite_cpu_rank_%d"%rank);module.__file__=composite_path;module.__package__=None;module.__loader__=None;module.__cached__=None;module.__builtins__=__builtins__;sys.modules[module.__name__]=module
...
row={"rank":rank,"pid":os.getpid(),"private_parent_fd_number":private_fd,"private_parent_replacement_inode":replacement.st_ino,"pread_bytes_sha256":shared_sha,"pread_offset_before":before,"pread_offset_after":after}
...
try:
 ...
 cache_root="/tmp/bernini-case01-object-trajectory-r5f-v3-composite-cpu-job%s-step%s-cache"%(job_id,step_id)
 ...
 for rank in range(4):
  children.append(subprocess.Popen(["/proc/self/fd/%d"%python_fd,"-I","-S","-B","-c",child,str(composite_fd),rows["adapter"]["path"],rows["adapter"]["sha256"],str(rows["adapter"]["size"]),str(shared_fd),shared_sha,str(rank)],stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.PIPE,close_fds=True,pass_fds=pass_fds,env=environment,start_new_session=False))
 rank_rows=[]
 ...
 receipt={"schema_version":"case01-object-trajectory-exact5-r5f-v3-composite-cpu-admission-v1","status":"PASS_COMPOSITE_CPU_EXACT26_HOLD","holder_job_id":job_id,"node":node,"slurm_step_id":step_id,"package":package,"world_size":4,"rank_count":4,"rank_rows":rank_rows,"isolated_runtime":{"python_flags":["-I","-S","-B"],"isolated":1,"no_site":1,"dont_write_bytecode":True,"entry_via_proc_self_fd":True},"private_parent_fd":{"synthetic_model_capture":True,"captured_parent_omitted":True,"captured_parent_closed_or_reused":True,"frozen_validator_rejected":True,"r5f_validator_accepted":True,"r5f_pread_path_exercised":True},"shared_ofd_pread":{"rank_count":4,"all_reads_exact":True,"offsets_unchanged":True},"module_binding":{"module_name":"infer_lora","base_infer_lora_same_object":True,"object_cli_applied_to_base_module":True,"translated_publication_applied_to_base_module":True,"legacy_module_instance_count":1,"duplicate_legacy_module_loaded":False},"side_effects":{"gpu_requested":False,"torch_imported":False,"renderer_or_vae_loaded":False,"publication_performed":False},"cache_lifecycle":{"admission_cache_root":completed_cache_root,"admission_cache_fresh":True,"admission_cache_cleanup_performed":True,"admission_cache_absent_terminal":True,"production_rank_cache":release["production_rank_cache"],"production_rank_cache_untouched":True,"production_rank_cache_absent_before_and_after":True},"process_cleanup":{"all_rank_returncodes_zero":True,"rank_processes_zero":True,"torchrun_processes_zero":True,"child_processes_terminal":True},"launch_allowed":False}
...
def build_srun_argv() -> list[str]:
    return [
        ...
        "--job-name=case01-object-r5f-v3-composite-cpu",
...
def _terminate_process_group(process: subprocess.Popen[bytes]) -> None:
    ...
    try:
        ...
        pass
    if not _process_group_absent(group, TERMINATE_GRACE_SECONDS):
        try:
    ...
    if not _process_group_absent(group, TERMINATE_GRACE_SECONDS):
        raise CompositeCPUError("CPU admission process group remains")
...
def run_srun(command: Sequence[str], payload: bytes) -> tuple[int, bytes, bytes, int]:
    ...
    if process.poll() is None or not _process_group_absent(
        process.pid, TERMINATE_GRACE_SECONDS,
    ):
        _terminate_process_group(process)
        raise CompositeCPUError("CPU admission srun cleanup differed")
    return int(process.returncode), stdout, stderr, process.pid
...
def validate_receipt(
    ...
    if (
        ...
        or value.get("status") != "PASS_COMPOSITE_CPU_EXACT26_HOLD"
...
def controller() -> dict[str, Any]:
    ...
    try:
        ...
        evidence = {
            ...
            "status": "PASS_COMPOSITE_CPU_CONTROLLER",
            "holder_job_id": HOLDER_JOB_ID, "node": NODE,
