#!/bin/bash
# One-shot handoff for the fresh r5g full16 package from its sealed 21-file snapshot.
#
...
[[ "$0" == /bin/bash && "$#" -eq 0 && "$-" == *p* && "$-" != *i* ]] || {
  /usr/bin/printf 'r5f materialization refused: shell entry differs\n' >&2
  exit 96
}
[[ "${PATH:-}" == /usr/bin:/bin && "${LC_ALL:-}" == C \
  && "${LANG:-}" == C && "${HOME:-}" == /vast/users/guangyi.chen \
  && "${BASH_ENV:-}" == /dev/null ]] || {
  /usr/bin/printf 'r5f materialization refused: entry environment differs\n' >&2
  exit 96
}
[[ -z "${ENV:-}" && -z "${LD_PRELOAD:-}" && -z "${LD_LIBRARY_PATH:-}" \
  ...
  /usr/bin/printf 'r5f materialization refused: loader or GPU environment differs\n' >&2
  exit 96
}
if builtin declare -F | /usr/bin/grep . >/dev/null; then
  /usr/bin/printf 'r5f materialization refused: preloaded Bash function exists\n' >&2
  exit 96
fi
...
readonly R5D_SOURCE_ROOT=/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_full644_exploratory_matched_r5g_full16_source_snapshot_21_20260820_r1
readonly R5D_TARGET_ROOT=/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_full644_exploratory_matched_eval_auh_r5f_job143812_node293_full16_847b91a2_c91de7eb_d70eac5c_r1
readonly R5D_PYTHON=/vast/users/guangyi.chen/anaconda3/envs/vace/bin/python3.12
readonly R5D_MATERIALIZER="${R5D_SOURCE_ROOT}/methods/bernini_action_editing/tools/materialize_full644_exploratory_matched_r5g_full16_package_v1.py"

[[ ! -e "${R5D_TARGET_ROOT}" && ! -L "${R5D_TARGET_ROOT}" ]] || {
  /usr/bin/printf 'r5f materialization refused: fresh target already exists\n' >&2
  exit 96
}
[[ -f "${R5D_PYTHON}" && -x "${R5D_PYTHON}" && ! -L "${R5D_PYTHON}" \
  && -f "${R5D_MATERIALIZER}" && ! -L "${R5D_MATERIALIZER}" ]] || {
  /usr/bin/printf 'r5f materialization refused: held entry file differs\n' >&2
  exit 96
}
...
[[ "${R5D_PYTHON_FD}" =~ ^[0-9]+$ \
  ...
  /usr/bin/printf 'r5f materialization refused: held descriptor allocation differs\n' >&2
  exit 96
}
...
if python_path!="/vast/users/guangyi.chen/anaconda3/envs/vace/bin/python3.12" or materializer_path!=source_root+"/methods/bernini_action_editing/tools/materialize_full644_exploratory_matched_r5g_full16_package_v1.py":
    fail("bootstrap frozen entry paths differ")
if source_root!="/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_full644_exploratory_matched_r5g_full16_source_snapshot_21_20260820_r1" or target_root!="/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_full644_exploratory_matched_eval_auh_r5f_job143812_node293_full16_847b91a2_c91de7eb_d70eac5c_r1":
...
source_specs=(
    ...
    ("methods/bernini_action_editing/full644_exploratory_matched_spooled_launcher_auh_r5f.py","d70eac5c0ee5fbcbfa84bc3a711fc2e836fa8cc0331555502d2b9b832e7c6b4e"),
    ...
    ("methods/bernini_action_editing/full644_exploratory_matched_r5g_root_bootstrap_probe_runner_v1.py","9dd6c07e47fcb14ea50e7d1ed7aa4b5adb2e17a6320f4691aa9c8780d7791c32"),
    ("methods/bernini_action_editing/full644_exploratory_matched_r5g_static_nomodel_probe_v1.py","e3a603ad1d94f4d53c7a26fcb29f9313592c571b4483431bdd04dfae5ba68d12"),
    ("methods/bernini_action_editing/full644_exploratory_matched_r5f_cpu_consumption_probe_v1.py","fd64fafc9580c8f25c88d79ca603a0dbf192ea98f77403e82f14d4e17c6905f6"),
)
...
materializer_raw=stable_fd(materializer_fd,materializer_path,"9eac713809960a319013cf6c8ce00f849a28dff5b631d91a5e33316d90091001",0o644,False,36435)
try:
...
if ident(os.fstat(materializer_fd))!=ident(os.lstat(materializer_path)) or hashlib.sha256(read_fd(materializer_fd,os.fstat(materializer_fd).st_size)).hexdigest()!="9eac713809960a319013cf6c8ce00f849a28dff5b631d91a5e33316d90091001":
