#!/usr/bin/env bash
# Isolate one R64 held-out WORLD4 rank without changing HOME.

set -Eeuo pipefail
umask 077

token="${R64_HELDOUT_CACHE_TOKEN:?set rank cache token}"
python_bin="${R64_HELDOUT_PYTHON_BIN:?set Python executable}"
...
[[ "${token}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$ ]] || {
  echo "invalid rank cache token" >&2; exit 2;
}
[[ -x "${python_bin}" && ! -L "${python_bin}" ]] || {
  echo "invalid Python executable" >&2; exit 2;
}
...
rank_root="$(mktemp -d -- "${scratch_real}/r64-heldout-${job_id}-${step_id}-r${global_rank}.XXXXXXXX")"
