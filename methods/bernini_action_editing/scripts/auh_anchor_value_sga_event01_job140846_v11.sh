...
# Matched DynaEdit SGA+ANC pair for the block-level value-transport canary.
# The first three solver cells use five independent ANC noise candidates.  The
# pure-T2V action video is queried online for every candidate before projected
# clean states are aggregated by source cosine similarity.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=SGA5_RAW_TRQKV_A025_EARLY8; transport=temporal_residual_qkv; strength=0.25 ;;
  auh7-1b-gpu-247:0)
    label=SGA5_RAW_TRV_A050_EARLY8; transport=temporal_residual_v; strength=0.50 ;;
...
output_root="$stage/anchor_value_sga_event01_v11"
...
scratch="/tmp/anchor-value-sga-$label-140846"
