#!/usr/bin/env python3
"""Replay retry8 authority/G1/G2a gates without model, claim, or optimizer."""
...
def main() -> int:
    ...
    if output.exists() or output.is_symlink() or claim.exists() or claim.is_symlink():
        raise RuntimeError("retry8 authority-only audit requires fresh output and claim")
    sys.path.insert(0, str(method_root))
    import torch
    import train_action_repr_target_t0_canary_retry8_v1 as runner
    ...
    def forbidden_adamw(*args, **kwargs):
        ...
        raise RuntimeError("retry8 authority-only audit reached AdamW")

    torch.optim.AdamW = forbidden_adamw
    os.environ["STAGE_B_T0_AUTHORITY_SHA256"] = args.authority_sha256
    try:
        authority = runner.retry7.authorize_preoptimizer_inputs(
    ...
    if optimizer_attempted or output.exists() or output.is_symlink() or claim.exists() or claim.is_symlink():
        raise RuntimeError("retry8 authority-only audit crossed its zero-mutation boundary")
    evidence = {
        "schema_version": "bernini-action-repr-retry8-authority-only-audit-v1",
        "passed": True,
        ...
        "batch_replay_diagnostic_receipt_digest": authority.authorization[
            "representation_contract"
        ]["batch_replay_diagnostic"]["receipt_digest"],
        "same_runtime_g2a_required": True,
