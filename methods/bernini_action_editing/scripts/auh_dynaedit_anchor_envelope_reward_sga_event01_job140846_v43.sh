...
# Round 43: rank complete early DynaEdit candidates by the fixed action
# anchor's canonical dense temporal-derivative motion envelope.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=ANCHOR_ENVELOPE_SGA_ADD005_P16_R8
    export SGA_SCORE_MODE=background_plus_anchor_envelope_005
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=ANCHOR_ENVELOPE_SGA_TRUST003_P16_R8
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_anchor_envelope_reward_sga_event01_v43"
