...
decode_one() {
  local arm="$1"
  local step="$2"
  local source_copy_checkpoint="$root/stage1/train_source_copy_p0_${arm}_out3e4_cond3e4_s160_r1/checkpoint-00000${step}"
  local output_dir="$root/stage1/infer/source_copy_p0_${arm}_s${step}_k100_${row}_r1"
  local scratch="/tmp/dense-flow-source-copy-${arm}-${step}-${row}-140846-${SLURM_STEP_ID}"
  test ! -e "$output_dir"
  ...
  "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
    ...
    --dense-flow-source-copy-mode phase0_broadcast \
