...
# R3 compute bridge.  This file is itself a phase-capability boundary: direct
# invocation cannot select an arm without the matching authorization/token;
# C additionally requires a freshly revalidated current A/B gate and A marker.
if [ "$#" -ne 0 ]; then echo "usage: $0" >&2; exit 2; fi
: "${E00_R3_ARM_ROLE:?missing E00_R3_ARM_ROLE}"
: "${E00_R3_PHASE:?missing E00_R3_PHASE}"
: "${E00_R3_OUTPUT_ROOT:?missing E00_R3_OUTPUT_ROOT}"
: "${E00_R3_PACKAGE_MANIFEST:?missing E00_R3_PACKAGE_MANIFEST}"
: "${E00_R3_AUTHORIZATION:?missing E00_R3_AUTHORIZATION}"
: "${E00_R3_CAPABILITY_TOKEN:?missing E00_R3_CAPABILITY_TOKEN}"
...
test "$E00_R3_PACKAGE_MANIFEST" = "$manifest"
protocol="$method_root/assets/e00_three_vessel_clean_diag_r3_protocol_20260821.json"
wrapper="$method_root/e00_legacy_infer_fixed_rng_wrapper_r3.py"
...
for file in "$manifest" "$protocol" "$wrapper" "$validator" "$builder" "$infer" "$controller" "$qk_transport" "$E00_R3_AUTHORIZATION"; do
...
capability_token_sha="$(printf '%s' "$E00_R3_CAPABILITY_TOKEN" | sha256sum)"; capability_token_sha="${capability_token_sha%% *}"
capability_args=(
  bridge-capability --protocol "$protocol" --phase "$E00_R3_PHASE"
  --arm-role "$E00_R3_ARM_ROLE" --package-manifest "$manifest"
  --authorization "$E00_R3_AUTHORIZATION"
  --capability-token-sha256 "$capability_token_sha"
)
case "$E00_R3_PHASE" in
  A)
    test "$E00_R3_ARM_ROLE" = pure_noobserver_output_routeoff
    test -z "${E00_R3_PHASE_A_MARKER:-}"
    test -z "${E00_R3_AB_GATE:-}"
    ;;
  B)
    test "$E00_R3_ARM_ROLE" = observer_matched_output_routeoff
    : "${E00_R3_PHASE_A_MARKER:?phase B requires E00_R3_PHASE_A_MARKER}"
    test -z "${E00_R3_AB_GATE:-}"
    capability_args+=(--phase-a-marker "$E00_R3_PHASE_A_MARKER")
    ;;
  C)
    test "$E00_R3_ARM_ROLE" = old_pureqk_temporal_routeon
    : "${E00_R3_PHASE_A_MARKER:?phase C requires E00_R3_PHASE_A_MARKER}"
    : "${E00_R3_AB_GATE:?phase C requires E00_R3_AB_GATE}"
    capability_args+=(--phase-a-marker "$E00_R3_PHASE_A_MARKER" --ab-gate "$E00_R3_AB_GATE")
    ;;
  *) echo "unknown R3 bridge phase" >&2; exit 2 ;;
esac
# This validation occurs before any Torch/model import.  For C it reloads the
# current A/B audits, native receipts, all eight RNG receipts, MP4s, gate,
# marker, package manifest, and authorization.
...
label="$(jq -er --arg role "$E00_R3_ARM_ROLE" '.arms[] | select(.arm_role == $role) | .label' "$spec")"
transport="$(jq -er --arg role "$E00_R3_ARM_ROLE" '.arms[] | select(.arm_role == $role) | .legacy_transport' "$spec")"
transport_steps="$(jq -er --arg role "$E00_R3_ARM_ROLE" '.arms[] | select(.arm_role == $role) | .legacy_transport_steps' "$spec")"
if [ "$E00_R3_PHASE" = A ]; then
...
output="$E00_R3_OUTPUT_ROOT/$label.mp4"
native_receipt="$output.receipt.json"
rng_prefix="$output.fixed-rng-r3"
audit="$output.e00-r3-arm-audit.json"
...
test ! -L "$E00_R3_OUTPUT_ROOT"

scratch="/tmp/e00-clean-diag-r3-${E00_R3_PHASE}-${SLURM_STEP_ID:-nostep}"
...
# Preserve Slurm's assigned visible-device namespace exactly.
...
"$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  "$wrapper" --wrapper-arm-role "$E00_R3_ARM_ROLE" \
  --wrapper-audit-prefix "$rng_prefix" --wrapper-protocol "$protocol" \
  ...
  --source-video "$source_video" --expected-source-sha256 "$source_sha" \
  --anchor-video "$anchor_video" --expected-anchor-sha256 "$anchor_sha" \
  --instruction "$instruction" --source-caption "$source_caption" \
  --target-caption "$target_caption" --anchor-caption "$anchor_caption" \
  --anchor-noop-caption "$anchor_noop_caption" --arm AQK_IID1 \
  ...
  --field-guidance raw_cfg --field-model first_phase_caption_i2v \
  --source-cfg-scale 4.5 --target-cfg-scale 4.5 --sga-temperature 0.01 \
  --early-candidate-count 5 --initial-noise-proposal-mode keyed_only \
  --anchor-state-mode clean_noised --anchor-cfg-scope shared \
  --anchor-contrast-mode dynamic_static_same_caption --anchor-sigma-cap 1.0 \
  --preservation-mode none --preservation-keep-fraction 0.20 \
  --preservation-outside-scale 0.0 --preservation-dilation 1 \
  --preservation-residual-fraction 0.0 --preservation-object-identity-strength 0.0 \
  --preservation-start-step 0 --preservation-ramp-steps 1 \
  --sga-score-mode global_source_cosine --anchor-candidate-mode single_shared \
  --anchor-spatial-alignment none --event01-forced-role-proposal-index -1 \
  --output "$output"
...
"$python_bin" -B "$validator" arm --protocol "$protocol" \
  --arm-role "$E00_R3_ARM_ROLE" --native-receipt "$native_receipt" --video "$output" \
  --rng-receipt "$rng_prefix.rank0.json" --rng-receipt "$rng_prefix.rank1.json" \
  --rng-receipt "$rng_prefix.rank2.json" --rng-receipt "$rng_prefix.rank3.json" \
  --audit-output "$audit"
printf 'E00_CLEAN_DIAG_R3_ARM_COMPLETE %s %s %s\n' "$E00_R3_ARM_ROLE" "$output" "$audit"
