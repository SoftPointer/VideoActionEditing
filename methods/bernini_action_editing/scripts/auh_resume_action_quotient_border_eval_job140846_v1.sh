#!/usr/bin/env bash
# Resume only missing action_start_nuisance_border fitted rollouts after node279 SIGBUS.
...
arm=action_start_nuisance_border
...
train_root="$1"; eval_root="$2"; one="$3"; manifest="$4"; python_bin="$5"; arm=action_start_nuisance_border
...
# Give the failed renderer ranks time to release all mappings before reload.
sleep 60
srun --jobid="${job_id}" --overlap --nodes=1 --ntasks=1 --nodelist=auh7-1b-gpu-279 \
  ...
  >"${eval_root}/logs/worker-3-resume.log" 2>&1
