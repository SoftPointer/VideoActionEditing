...
# Round 42: the fixed pure-T2V action video ranks complete early DynaEdit
# candidate edit directions.  It is never copied into target features/content.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=ANCHOR_ACTION_SGA_ADD002_P16_R8
    export SGA_SCORE_MODE=background_plus_anchor_action_002
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=ANCHOR_ACTION_SGA_TRUST003_P16_R8
    export SGA_SCORE_MODE=background_trust_anchor_action_003
    ;;
  *) echo "this experiment uses Job 140846 nodes 246-247 group 0" >&2; exit 3 ;;
esac

# Zero transport steps is intentional: isolate reward-driven SGA selection
# from the failed hard feature interventions in Rounds 39-41.
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_anchor_action_reward_sga_event01_v42"
