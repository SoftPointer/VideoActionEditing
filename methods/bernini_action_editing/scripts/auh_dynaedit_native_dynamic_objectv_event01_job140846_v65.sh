...
if [ "$#" -ne 1 ]; then
  echo "usage: $0 value050|value100" >&2
  exit 2
fi
...
host=$(hostname -s)
case "$host:$mode" in
  auh7-1b-gpu-246:value050)
    label=NATIVE_DYNROLE_OBJECTV_SGA_A050_ALL40_B4_9_P24_R8
    strength=0.50
    ;;
  auh7-1b-gpu-247:value100)
    label=NATIVE_DYNROLE_OBJECTV_SGA_A100_ALL40_B4_9_P24_R8
    strength=1.0
    ;;
  *)
    echo "Round 65 mode $mode is not assigned to host $host" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_dynamic_objectv_event01_v65"
