...
# Round-33 R2 direct four-node payload. Each node owns one four-rank SP4
# model. Only rank zero loads/encodes UMT5; the seven prompt embeddings are
# broadcast after UMT5 is retired. Unlike the early-eight Round-32 arms, the
# pure-T2V teacher is queried across all 40 solver steps while the matched
# 1.5% residual preservation basin remains fixed. This is inference-only.
if [ "$#" -ne 0 ]; then
  echo "usage: $0" >&2
  exit 2
fi
...
case "$(hostname -s)" in
  ...
  auh7-1b-gpu-248)
    ...
    ;;
  auh7-1b-gpu-279)
    label=FULLPATH_DYNQK_A025_B4_9
    ...
    ;;
  auh7-1b-gpu-247)
    label=FULLPATH_DYNATTN_A010_B4_9
...
stage="$root/stage1"
dev="$stage/anchor_sigma_cap_v28r3_c9fb3668_5d11aada_3e7f8e44"
...
output_root="$stage/dynaedit_fullpath_anchor_event01_v33r2_c9fb3668_5d11aada_3e7f8e44"
...
require_sha256 "$dev/infer_anchor_sga_anc_event_v1.py" c9fb3668ce0a505bd574fe2af7ade2cba9dfe51608383a9d76a083bf1477d779
require_sha256 "$dev/anchor_sga_anc_controller.py" 5d11aadaa25b4da66918c6b6399343e3d8897713a3c11716901540600321af8f
...
scratch="/tmp/dynaedit-fullpath-anchor-v33r2-$label-140846-four-nodes"
