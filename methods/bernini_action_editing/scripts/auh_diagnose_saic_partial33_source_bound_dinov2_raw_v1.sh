#!/usr/bin/env bash
# Run exact33 source-bound raw diagnostics inside an owned 8-GPU allocation.
...
if [[ ! -x "$python_bin" || ! -f "$diagnostic_source" || -L "$diagnostic_source" ]]; then echo "python or diagnostic source is not a plain executable input" >&2; exit 66; fi
if [[ ! -d "$attempts_root" || ! -f "$source_manifest" || -L "$source_manifest" || ! -d "$visual_checkpoint" || ! -f "$checkpoint_manifest" || ! -f "$evaluator_spec" || ! -f "$visual_scorer_source" || ! -f "$visual_contract_source" ]]; then echo "one sealed input path is absent" >&2; exit 66; fi
...
echo "partial33 source-bound DINO raw ${mode} complete: $output_root"
