#!/bin/bash -p
# Trusted-stdin v3, create-only construction of the sealed exact5 source snapshot.
...
readonly BUILDER="$STAGING_ROOT/methods/bernini_action_editing/tools/build_case01_source_bone_exact5_source_snapshot_v2.py"
...
def held(fd,path,digest,size,mode,uid,gid,process=False):
 ...
 return first
if len(sys.argv)!=8: fail("snapshot bootstrap argv differs")
...
if staging!=expected_staging or old!=expected_old or target!=expected_target or builder_path!=staging+"/methods/bernini_action_editing/tools/build_case01_source_bone_exact5_source_snapshot_v2.py": fail("snapshot path binding differs")
...
builder_raw=held(srcfd,builder_path,"982732646552185886f6656a9dba13064c7bd65ce83f4653f31cba2e65a43783",21092,0o644,2012,2000)
...
if ident(os.fstat(srcfd))!=ident(os.lstat(builder_path)) or hashlib.sha256(read(srcfd,os.fstat(srcfd).st_size)).hexdigest()!="982732646552185886f6656a9dba13064c7bd65ce83f4653f31cba2e65a43783": fail("held snapshot builder changed")
