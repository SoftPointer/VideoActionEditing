...
source_root="$experiment_root/source_stage_b_t0_decode_v2"
...
decode_root="$experiment_root/stage_b_t0_retry8/matched_decode_v2/0be6494dfac3"
log_root="$experiment_root/logs/stage_b_t0_retry8/matched_decode_v2"
...
expected_runner_sha=714c1fa70b640415d983d4c4e47daf562fbdce8be96b4e1d8e8891307e554533
expected_test_sha=a96ecf7bd6e165559affcb43b1bf62b3c5c977c4e0bd05b50d9cc777def9c574
...
fail() {
  printf '[stage-b-t0-decode-v2] ERROR: %s\n' "$*" >&2
  exit 1
}
...
run_worker() {
  ...
  scratch="/tmp/action-repr-t0-decode-v2-${step}-${route}-${job}-${step_token}"
  ...
  printf '[stage-b-t0-decode-v2] DECODE_PASS step=%s route=%s output=%s\n' "$step" "$route" "$output"
}
...
launch_decode() {
  ...
  printf '[stage-b-t0-decode-v2] srun complete job=%s step=%s route=%s\n' "$job" "$step" "$route"
}
...
case "$command" in
  preflight)
    [ "$#" -eq 0 ] || usage
    common_preflight
    echo '[stage-b-t0-decode-v2] PREFLIGHT_PASS WORLD4_SP4=true steps=40 matched=true pyav=true'
    ;;
  decode)
    [ "$#" -eq 3 ] || usage
    launch_decode "$1" "$2" "$3"
  ...
  status)
    ...
    if [ ! -d "$decode_root" ]; then
      echo '[stage-b-t0-decode-v2] NOT_STARTED'
      exit 0
    fi
