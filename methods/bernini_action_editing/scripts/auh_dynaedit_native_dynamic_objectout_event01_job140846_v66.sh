...
case "$host:$mode" in
  auh7-1b-gpu-246:sga)
    label=NATIVE_DYNROLE_OBJECTOUT_SGA_A050_ALL40_B4_9_P24_R8
    arm=AQK_SGA5
    ;;
  auh7-1b-gpu-247:avg)
    label=NATIVE_DYNROLE_OBJECTOUT_AVG_A050_ALL40_B4_9_P24_R8
    arm=AQK_AVG5
    ;;
  *)
    echo "Round 66 mode $mode is not assigned to host $host" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_dynamic_objectout_event01_v66"
export TRANSPORT_OVERRIDE=event01_dynamic_source_object_output_attn_output
