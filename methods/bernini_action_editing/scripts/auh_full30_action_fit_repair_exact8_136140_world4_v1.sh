#!/usr/bin/env bash
# BOX-EXP-011 minimal fit action/incomplete repair exact8 on retained 136140/gpu215.
#
...
fail() { echo "[full30-fit-repair-exact8-v1] ERROR: $*" >&2; exit 2; }
sha256_file() { sha256sum "$1" | awk '{print $1}'; }

readonly confirm="${F11_CONFIRM:?explicit launch confirmation required}"
readonly run_root="${F11_RUN_ROOT:?set fresh run root}"
readonly method_root="${F11_METHOD_ROOT:?set extracted release method root}"
readonly method_archive="${F11_METHOD_ARCHIVE:?set release archive}"
readonly method_archive_sha="${F11_METHOD_ARCHIVE_SHA256:?pin release archive}"
readonly method_manifest="${F11_METHOD_MANIFEST:?set release manifest}"
readonly method_manifest_sha="${F11_METHOD_MANIFEST_SHA256:?pin release manifest}"
readonly seed1_spec="${F11_SEED1_SPEC:?set seed1 reserve4 spec for disposable smoke}"
readonly seed2_spec="${F11_SEED2_SPEC:?set seed2 reserve4 spec for disposable smoke}"
readonly python_bin="${F11_PYTHON_BIN:?set frozen Python}"
readonly bernini_root="${F11_BERNINI_ROOT:?set Bernini root}"
readonly veomni_root="${F11_VEOMNI_ROOT:?set VeOmni root}"
readonly checkpoint="${F11_CHECKPOINT:?set Bernini checkpoint}"
readonly checkpoint_manifest="${F11_CHECKPOINT_MANIFEST:?set checkpoint manifest}"
readonly method_revision="${F11_METHOD_REVISION:?set content revision}"
readonly master_port="${F11_MASTER_PORT:?set two-port base}"
readonly r10_receipt="${F11_R10_COMPILE_SMOKE_RECEIPT:?bind r10 receipt}"
readonly r10_receipt_sha="${F11_R10_COMPILE_SMOKE_RECEIPT_SHA256:?pin r10 receipt}"
readonly r10_log="${F11_R10_GENERATION_LOG:?bind r10 log}"
readonly r10_log_sha="${F11_R10_GENERATION_LOG_SHA256:?pin r10 log}"

readonly holder_job=136140
readonly holder_node=auh7-1b-gpu-215
readonly launch_confirmation=launch-approved-BOX-EXP-011-fit-action-incomplete-repair-exact8-136140
...
readonly controller="${method_root}/full30_action_fit_repair_exact8_controller_v1.py"
readonly plan_tool="${method_root}/full30_action_fit_repair_exact8_plan_v1.py"
readonly generator="${method_root}/full30_action_fit_repair_exact8_generator_v1.py"
readonly resource="${method_root}/tools/reserve4_fixed_generation_sp4_136140_specialized_v1.py"
readonly release_builder="${method_root}/tools/build_full30_action_fit_repair_exact8_release_v1.py"
readonly launcher="${method_root}/scripts/auh_full30_action_fit_repair_exact8_136140_world4_v1.sh"
...
if [[ "${role}" == child ]]; then
  ...
  "${python_bin}" -B "${controller}" validate-runtime \
    --controller-plan "${F11_CONTROLLER_PLAN}" \
    --expected-controller-plan-sha256 "${F11_CONTROLLER_PLAN_SHA256}" >/dev/null || fail "child runtime validation failed"
  ...
  task_scratch="$(mktemp -d -- "${scratch_parent}/fit-repair-exact8-${SLURM_JOB_ID}-${SLURM_STEP_ID}.XXXXXXXX")"
  ...
  run_exact4_shard() {
    ...
    "${python_bin}" -B "${generator}" run-sp4 \
      --plan "${F11_EXACT8_PLAN}" --expected-plan-sha256 "${F11_EXACT8_PLAN_SHA256}" \
  ...
  "${python_bin}" -B "${controller}" seal-terminal-host-gate \
    ...
    --output "${run_root}/logs/terminal-fit-repair-exact8-host-gate.json" >/dev/null || fail "terminal exact8 host gate failed"
...
srun --jobid="${holder_job}" --nodelist="${holder_node}" --nodes=1 --ntasks=1 \
  ...
  env F11_CONTROLLER_PLAN="${controller_plan}" F11_CONTROLLER_PLAN_SHA256="${controller_plan_sha}" \
      F11_EXACT8_PLAN="${exact8_plan}" F11_EXACT8_PLAN_SHA256="${exact8_plan_sha}" \
      bash "${launcher}" __child >"${run_root}/logs/fit-repair-exact8-generation.log" 2>&1 &
child_pid=$!
wait "${child_pid}"
status=$?
child_pid=""
set -e
if (( status != 0 )); then tail -n 240 "${run_root}/logs/fit-repair-exact8-generation.log" >&2 || true; fail "all8 serial exact8 child failed status=${status}"; fi
[[ -f "${run_root}/generation-exact8-audit.json" && -f "${run_root}/logs/terminal-fit-repair-exact8-host-gate.json" ]] || fail "exact8 outputs are incomplete"

printf '%s\n' \
  'schema=bernini-full30-action-fit-repair-exact8-launch-status-v1' \
  'experiment_id=BOX-EXP-011' \
...
echo 'BOX_EXP_011_FIT_REPAIR_EXACT8_GENERATION_PENDING_REVIEW_AND_MATERIALIZER candidate_count=8 diagnostics=0 optimizer=false'
