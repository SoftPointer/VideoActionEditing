...
if [ "$#" -ne 2 ]; then
  echo "usage: $0 SOURCE_COPY_MODE TAG" >&2
  exit 2
fi
source_copy_mode="$1"
tag="$2"
case "$source_copy_mode" in
  phase_aligned|phase0_broadcast) ;;
  *) echo "unsupported source-copy mode: $source_copy_mode" >&2; exit 2 ;;
esac
...
output="$root/stage1/train_source_copy_${tag}_out3e4_cond3e4_s160_r1"

test ! -e "$output"
test -f "$motion_checkpoint/adapter_model.safetensors"
test -f "$motion_checkpoint/receipt.json"
scratch="/tmp/dense-flow-source-copy-${tag}-140846-${SLURM_STEP_ID}"
...
"$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  ...
  --source-copy-mode "$source_copy_mode" \
  --seed 2026081701 \
  --max-grad-norm 10 \
