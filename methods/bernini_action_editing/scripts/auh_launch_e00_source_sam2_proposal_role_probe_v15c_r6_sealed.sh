...
readonly PYTHON_FD="/proc/self/fd/8"
...
readonly RELEASE_REL="methods/bernini_action_editing/assets/e00_source_sam2_proposal_role_probe_v15c_r6_release.json"

fail() {
  echo "v15c-r6 sealed worker: $*" >&2
  exit 1
}

[[ "${1:-}" == "--sealed-worker" && "$#" -eq 8 ]] || \
  fail "entry requires externally authenticated sealed-worker arguments"
[[ "${V15C_R6_EXTERNAL_BOOTSTRAP:-}" == "1" ]] || fail "external bootstrap marker absent"
[[ -r "${PYTHON_FD}" ]] || fail "authenticated Python descriptor absent"
readonly RUN_ROOT="$2"
readonly SNAPSHOT="$3"
...
readonly PASSED_PYTHON_AUTHORITY="$8"
readonly METHOD_ROOT="${SNAPSHOT}/methods/bernini_action_editing"
readonly SPEC="${METHOD_ROOT}/assets/e00_source_sam2_proposal_role_probe_v15c_r6.json"
readonly RELEASE="${SNAPSHOT}/${RELEASE_REL}"
readonly MATERIALIZER="${METHOD_ROOT}/materialize_source_sam2_proposal_tracks_v15c_r6.py"
readonly RUNNER="${METHOD_ROOT}/run_source_object_proposal_role_probe_v15c_r6.py"
readonly POSTFLIGHT="${METHOD_ROOT}/postflight_source_sam2_proposal_role_probe_v15c_r6.py"
readonly OVERLAY="${METHOD_ROOT}/tools/build_source_object_proposal_role_v15c_r6_review.py"
readonly FINALIZER="${METHOD_ROOT}/finalize_source_sam2_proposal_role_probe_v15c_r6.py"
...
[[ "${PASSED_PYTHON_AUTHORITY}" == "${PYTHON_AUTHORITY}" ]] || \
  fail "Python canonical authority differs"
...
# Every Python process is executed from the already-authenticated descriptor.
# exec -a supplies the same canonical authority path expected by CPython's
# sys.executable and by the frozen SAM2 runtime contract.
trusted_python() {
  [[ "$#" -ge 1 ]] || fail "trusted Python arguments absent"
  /bin/bash -c '
set -euo pipefail
authority="$1"
python_fd="$2"
...
' v15c-r6-python "${PYTHON_AUTHORITY}" "${PYTHON_FD}" "$@"
}

verify_runtime() {
  trusted_python -E -s -B "${FINALIZER}" verify-runtime "${RUNTIME_ARGS[@]}"
}
...
verify_runtime
trusted_python -E -s -B "${MATERIALIZER}" \
...
verify_runtime
trusted_python -E -s -B "${POSTFLIGHT}" \
  --spec "${SPEC}" --source-video "${SOURCE_VIDEO}" \
  --r6-receipt "${R6_ROOT}/e00_v15b_r6_probe_receipt.json" \
  --r6-tensors "${R6_ROOT}/e00_v15b_r6_affinity.safetensors" \
  --track-receipt "${RUN_ROOT}/tracks/track_receipt.json" \
...
verify_runtime
trusted_python -E -s -B "${FINALIZER}" complete \
  "${RUNTIME_ARGS[@]}" \
  --job-id "${SLURM_JOB_ID}" --node "$(/bin/hostname -s)" \
  --gpu-index "0" --gpu-name "AMD Instinct MI210"

trusted_python -E -s -B "${FINALIZER}" verify-complete --run-root "${RUN_ROOT}"
echo "v15c-r6 observer-only COMPLETE (reject-only, ROUTE_NO_GO): ${RUN_ROOT}"
