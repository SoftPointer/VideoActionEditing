...
case "$-" in
  *p*) ;;
  *) echo "[submit-graft-short-traj-r2] ERROR privileged bash required" >&2; exit 2 ;;
esac
set -Eeuo pipefail
umask 077
...
argv=["/usr/bin/sbatch","--parsable","--export="+export_arg,"--job-name=graft-short-traj-r2","--partition=faculty","--qos=bgqos","--nodes=1","--ntasks=1","--cpus-per-task=32","--mem=256G","--gres=gpu:mi210:8","--time=24:00:00","--exclude=auh7-1b-gpu-185,auh7-1b-gpu-187,auh7-1b-gpu-195,auh7-1b-gpu-233,auh7-1b-gpu-318","--output=/vast/users/guangyi.chen/slurm-%j.out",launcher]
