...
readonly experiment_root=/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_full644_exploratory_r64_job141620_v4
readonly launcher_root="${experiment_root}/launchers/full644-exploratory-r64-job141620-v4"
readonly helper="${launcher_root}/full644_exploratory_r64_release_v1.py"
readonly helper_sha256=d2e2f10d3795a62e3a14a44a588908c9d2d85623ff722c523271f58c69c8b449
readonly helper_size=66417
readonly release_root="${experiment_root}/releases/full644-exploratory-r64-source-d4647b39b3f8"
readonly trainer="${release_root}/methods/bernini_action_editing/train_lora.py"
readonly trainer_sha256=7a5a2147c1ba11efb2df1b6ef476266c414b7e5580c0c2463fa6bd0c4f5a87aa
readonly trainer_size=156400
readonly method_source_revision=8f3ef5f5cc29e0f76447f5dcc965e522f1ef554e
readonly method_source_archive_sha256=d4647b39b3f8485043192ac2e9ee3082cca6173eb91d15b7d754a73f1b8b401c
...
readonly capacity_output="${experiment_root}/runs/prelaunch-capacity-sft-r64-step1-v4"
readonly full644_output="${experiment_root}/runs/full644-r64-reference-dpo-preservation-one-pass-v4"
readonly attempt_root="${experiment_root}/attempts/prelaunch-capacity-then-full644-v4"
...
readonly run_mode="$1"
run_output="$(output_for_mode "${run_mode}")" || fail "mode output resolution failed"
readonly run_output
...
[[ -d "${run_output%/*}" && ! -L "${run_output%/*}" ]] || fail "training output parent differs"
...
if [[ "${run_mode}" == FULL644_EXPLORATORY ]]; then
  ...
  command=(
    ...
    --expected-full644-source-authority-sha256 "${source_authority_sha256}" --output "${run_output}"
...
"${python_bin}" -I -B "${helper}" publish-runner-completion \
  --mode "${helper_mode}" --output "${run_output}" --receipt-output "${completion_path}" \
