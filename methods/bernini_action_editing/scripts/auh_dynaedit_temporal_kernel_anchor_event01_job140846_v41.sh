...
# Round 41: DynaEdit SGA/ANC with one fixed pure-T2V action anchor.  Transfer
# only its dynamic-minus-static per-head T x T temporal attention topology and
# apply that operator to the target's own V stream.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=FIXEDV0_TEMPORAL_KERNEL_ATTN_B0_5_P16_R8
    export BLOCKS_OVERRIDE=0-5
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=FIXEDV0_TEMPORAL_KERNEL_ATTN_B4_9_P16_R8
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_temporal_kernel_anchor_event01_v41"
