#!/usr/bin/env python3
"""Fresh-v7 receipt-gated one-shot GPU controller for exact-five.
...
SCHEMA = "case01-object-trajectory-exact5-r64-gpu-controller-v7"
ATTEMPT_SCHEMA = SCHEMA + "-attempt"
DISPATCH_SCHEMA = SCHEMA + "-dispatch"
EVIDENCE_SCHEMA = SCHEMA + "-evidence"
RUNTIME_SCHEMA = "case01-object-trajectory-exact5-r64-gpu-controller-v4-runtime"
RANK_CACHE_SCHEMA = (
...
REVOKED_COMPOSITE_CPU_V3_PATHS = (
    EXPERIMENTS / (
        "bernini_case01_object_trajectory_exact5_r64_canary_v3."
        "composite_cpu_admission_receipt_v3.json"
    ),
    EXPERIMENTS / (
        "bernini_case01_object_trajectory_exact5_r64_canary_v3."
        "composite_cpu_admission_controller_evidence_v3.json"
    ),
)
REVOKED_COMPOSITE_CPU_PRIOR_PATHS = (
    REVOKED_COMPOSITE_CPU_V2_PATHS + REVOKED_COMPOSITE_CPU_V3_PATHS
)
HOLD_PLAN_PATH = (
...
ATTEMPT_PATH = PACKAGE_ROOT / "evidence/exact5_gpu_attempt_v7.json"
DISPATCH_PATH = PACKAGE_ROOT / "evidence/exact5_gpu_dispatch_v7.json"
EVIDENCE_PATH = PACKAGE_ROOT / "evidence/exact5_gpu_controller_receipt_v7.json"
STDOUT_PATH = PACKAGE_ROOT / "logs/exact5_gpu_srun_v7.stdout.log"
STDERR_PATH = PACKAGE_ROOT / "logs/exact5_gpu_srun_v7.stderr.log"
REVOKED_GPU_V4_OUTER_PATHS = (
    PACKAGE_ROOT / "evidence/exact5_gpu_attempt_v4.json",
    PACKAGE_ROOT / "evidence/exact5_gpu_dispatch_v4.json",
    PACKAGE_ROOT / "evidence/exact5_gpu_controller_receipt_v4.json",
    PACKAGE_ROOT / "logs/exact5_gpu_srun_v4.stdout.log",
    PACKAGE_ROOT / "logs/exact5_gpu_srun_v4.stderr.log",
)
REVOKED_GPU_V5_OUTER_PATHS = (
    PACKAGE_ROOT / "evidence/exact5_gpu_attempt_v5.json",
    PACKAGE_ROOT / "evidence/exact5_gpu_dispatch_v5.json",
    PACKAGE_ROOT / "evidence/exact5_gpu_controller_receipt_v5.json",
    PACKAGE_ROOT / "logs/exact5_gpu_srun_v5.stdout.log",
    PACKAGE_ROOT / "logs/exact5_gpu_srun_v5.stderr.log",
)
REVOKED_GPU_V6_OUTER_PATHS = (
    PACKAGE_ROOT / "evidence/exact5_gpu_attempt_v6.json",
    PACKAGE_ROOT / "evidence/exact5_gpu_dispatch_v6.json",
    PACKAGE_ROOT / "evidence/exact5_gpu_controller_receipt_v6.json",
    PACKAGE_ROOT / "logs/exact5_gpu_srun_v6.stdout.log",
    PACKAGE_ROOT / "logs/exact5_gpu_srun_v6.stderr.log",
)
REVOKED_GPU_PRIOR_OUTER_PATHS = (
    REVOKED_GPU_V4_OUTER_PATHS + REVOKED_GPU_V5_OUTER_PATHS
    + REVOKED_GPU_V6_OUTER_PATHS
)
OUTPUT_REPORT_PATH = (
...
COMPOSITE_CPU_SCHEMA = (
    "case01-object-trajectory-exact5-r5f-v4-composite-cpu-admission-v4"
)
COMPOSITE_CPU_EVIDENCE_SCHEMA = (
    "case01-object-trajectory-exact5-r5f-v4-composite-cpu-controller-v4-evidence"
)
REVOKED_COMPOSITE_CPU_V2_SCHEMAS = frozenset({
...
REVOKED_COMPOSITE_CPU_V3_SCHEMAS = frozenset({
    "case01-object-trajectory-exact5-r5f-v4-composite-cpu-admission-v3",
    "case01-object-trajectory-exact5-r5f-v4-composite-cpu-controller-v3-evidence",
})
REVOKED_COMPOSITE_CPU_PRIOR_SCHEMAS = (
    REVOKED_COMPOSITE_CPU_V2_SCHEMAS | REVOKED_COMPOSITE_CPU_V3_SCHEMAS
)
if (
    {COMPOSITE_CPU_RECEIPT_PATH, COMPOSITE_CPU_EVIDENCE_PATH}
    & set(REVOKED_COMPOSITE_CPU_PRIOR_PATHS)
    or {COMPOSITE_CPU_SCHEMA, COMPOSITE_CPU_EVIDENCE_SCHEMA}
    & REVOKED_COMPOSITE_CPU_PRIOR_SCHEMAS
):
    raise RuntimeError("revoked composite CPU authority reappeared")
...
COMPOSITE_CPU_EVIDENCE_FIELDS = frozenset({
    ...
    "renderer_or_vae_loaded", "publication_performed",
    "normal_exit_passive_grace_performed", "normal_exit_signal_sent",
    "terminal_pipes_closed", "evidence_digest",
})
...
def validate_materialization_report(held: HeldAuthority) -> dict[str, Any]:
    ...
    for relative, (expected_sha256, expected_size) in CORE4_RELEASE_PINS.items():
        if release_by_path.get(relative) != {
            ...
            "size": expected_size,
            "provenance": "receipt_gated_exact6_overlay",
        }:
...
def validate_composite_cpu_admission(
    ...
    if (
        ...
        != "PASS_FRESH_CANARY_V3_COMPOSITE_CPU_CONTROLLER_V4"
        ...
        or evidence.get("normal_exit_passive_grace_performed") is not True
        or evidence.get("normal_exit_signal_sent") is not False
        or evidence.get("terminal_pipes_closed") is not True
    ):
...
def require_fresh_outputs() -> None:
    ...
    # Exact-empty evidence/log namespaces also prove every revoked GPU-v4/v5/v6
    # outer path absent; none of those old paths is opened or consumed.
...
def build_srun_argv(max_gate_step: int) -> list[str]:
    ...
    return [
        ...
        "--job-name=case01-object-trajectory-exact5-r64-gpu-v7",
