...
if [ "$#" -ne 2 ]; then
  echo "usage: $0 observer|explicit 37|38" >&2
  exit 2
fi
...
case "$host:$mode:$start" in
  auh7-1b-gpu-246:observer:37|auh7-1b-gpu-246:observer:38)
    ...
    ;;
  auh7-1b-gpu-247:explicit:37|auh7-1b-gpu-247:explicit:38)
    ...
    ;;
  *)
    echo "Round 88 assignment differs: observer on 246, explicit on 247, P37/P38 only" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_preservation_terminal_boundary_event01_v88"
