#!/usr/bin/env bash
# Create-once Stage-B target-T0 retry-7 one-update canary launcher.
#
...
source_root="$experiment_root/source_stage_b_t0_retry7"
method_root="$source_root/methods/bernini_action_editing"
authority_addendum="$source_root/stage_b_t0_single_update_retry7_authority_addendum.json"
stage_root="$experiment_root/stage_b_t0_retry7"
log_root="$experiment_root/logs/stage_b_t0_retry7"

runner="$method_root/train_action_repr_target_t0_canary_retry7_v1.py"
...
attempt_claim="$stage_root/target_t0/0be6494dfac3/.single_update.retry7.attempt_claim.json"
...
usage() {
  cat >&2 <<'EOF'
usage:
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry7.sh preflight
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry7.sh launch <parent-job-id>
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry7.sh status
...
require_active_authority_contract() {
  local declared_pins_digest canonical_pins observed_pins_digest
  require_authority_hash
  ! grep -Eq '__FINAL_[A-Z0-9_]+_SHA256__' "$authority_addendum" \
    || fail "active authority contains an unresolved source placeholder"
  test "$(authority_value '.schema_version')" = bernini-action-repr-stage-b-t0-single-update-retry7-authority-addendum-v1 \
  ...
  test "$(authority_value '.document_role')" = create_once_stage_b_target_t0_single_update_retry7_authority \
    || fail "Stage-B retry7 authority document role differs"
  declared_pins_digest="$(authority_value '.source_hash_pins_digest')"
  canonical_pins="$(jq -cS '.source_hash_pins' "$authority_addendum")"
  observed_pins_digest="$(printf '%s' "$canonical_pins" | sha256sum | awk '{print $1}')"
  test "$declared_pins_digest" = "$observed_pins_digest" \
    || fail "authority source_hash_pins_digest differs"
  ...
  test "$(authority_value '.distributed_contract.rank0_atomic_preoptimizer_attempt_claim')" = true \
    || fail "rank0 atomic preoptimizer claim is required"
  test "$(authority_value '.distributed_contract.rank0_claim_outcome_broadcast_before_peer_replay')" = true \
    || fail "rank0 claim outcome broadcast is required"
  test "$(authority_value '.distributed_contract.all_ranks_replay_attempt_claim_from_disk')" = true \
    || fail "all ranks must replay the attempt claim"
  test "$(authority_value '.distributed_contract.claim_creation_before_cache_model_adapter_optimizer')" = true \
    || fail "attempt claim ordering contract differs"
  test "$(authority_value '.g1_replay_thread_contract.torch_intraop_threads')" = 1 \
    || fail "G1 replay torch intra-op thread count differs"
  test "$(authority_value '.g1_replay_thread_contract.torch_interop_threads')" = 1 \
    || fail "G1 replay torch inter-op thread count differs"
  test "$(authority_value '.g1_replay_thread_contract.strict_legacy_diagnostics_bitwise_replay')" = true \
    || fail "strict legacy G1 diagnostics replay is not required"
  test "$(authority_value '.g1_replay_thread_contract.must_apply_before_g1_authority_replay')" = true \
    || fail "G1 replay thread contract is not ordered before authority replay"
  jq -e '
  ...
  ' "$authority_addendum" >/dev/null \
    || fail "retry7 runtime execution contract differs"
  ...
  test "$(authority_value '.output_contract.atomic_preoptimizer_attempt_claim_required')" = true \
    || fail "atomic preoptimizer attempt claim is required"
  test "$(authority_value '.output_contract.attempt_claim_marker_name')" = .single_update.retry7.attempt_claim.json \
    || fail "attempt claim marker name differs"
  test "$(authority_value '.output_contract.canonical_output_path')" = "$output_root" \
    || fail "exact canonical output path differs"
  test "$(authority_value '.output_contract.attempt_claim_marker_path')" = "$attempt_claim" \
    || fail "exact attempt claim marker path differs"
  test "$(authority_value '.output_contract.attempt_claim_permanent_on_failure')" = true \
    || fail "attempt claim must remain permanent on failure"
  test "$(authority_value '.output_contract.existing_claim_requires_new_revision')" = true \
    || fail "existing claim must require a new revision"
  test "$(authority_value '.output_contract.next_revision_after_claim_failure')" = retry8 \
    || fail "retry7 consumed claim must require retry8"
  test "$(authority_value '.output_contract.claim_path_file_sha_and_object_digest_receipted')" = true \
    || fail "attempt claim receipt binding differs"
  ...
  test "$(authority_value '.runtime_paths.fresh_source_root_name')" = source_stage_b_t0_retry7 \
    || fail "fresh source revision differs"
  test "$(authority_value '.runtime_paths.fresh_stage_root_name')" = stage_b_t0_retry7 \
  ...
  require_source_pin methods/bernini_action_editing/train_action_repr_target_t0_canary_retry7_v1.py
  ...
  require_source_pin methods/bernini_action_editing/score_g1_joint_action_repr_admission_v1.py
  require_source_pin methods/bernini_action_editing/evaluate_g1_action_repr_selectivity_v1.py
  require_source_pin methods/bernini_action_editing/materialize_g1_flow_control_cohort_v1.py
  require_source_pin methods/bernini_action_editing/materialize_g1_middle_control_cohort_v1.py
  ...
  require_source_pin methods/bernini_action_editing/scripts/auh_stage_b_t0_single_update_20260824_retry7.sh
  require_source_pin methods/bernini_action_editing/tests/test_train_action_repr_target_t0_canary_retry7_v1.py
  require_source_pin tests/test_auh_stage_b_t0_single_update_20260824_retry7.py
}

require_unclaimed_attempt() {
  if [ -L "$attempt_claim" ] || [ -e "$attempt_claim" ]; then
    if [ ! -e "$output_root" ] && [ ! -L "$output_root" ]; then
      fail "CLAIMED_INCOMPLETE retry7 attempt is permanently consumed; use retry8: $attempt_claim"
    fi
    fail "retry7 attempt claim already exists; no second optimizer is allowed; use retry8"
  fi
  test ! -e "$output_root" && test ! -L "$output_root" \
    || fail "retry7 output exists without its permanent preoptimizer claim"
}
...
validate_completed_output() {
  local receipt="$output_root/receipt.json" receipt_rocr receipt_rocr_sha computed_rocr_sha claim_file_sha
  test -f "$attempt_claim" || fail "retry7 permanent attempt claim is absent"
  test ! -L "$attempt_claim" || fail "retry7 permanent attempt claim may not be a symlink"
  test -f "$receipt" || fail "T0 runner returned without receipt.json"
  test ! -L "$receipt" || fail "T0 receipt may not be a symlink"
  "$python_bin" -B -c \
    'import sys; from train_action_repr_target_t0_canary_retry7_v1 import validate_published_t0_output; validate_published_t0_output(sys.argv[1])' \
    "$output_root"
  test "$(jq -er '.schema_version' "$receipt")" = bernini-action-repr-target-t0-one-step-retry7-receipt-v1 \
  ...
  test "$(jq -er '.upstream_authority.source_hash_pins_digest' "$receipt")" = "$(authority_value '.source_hash_pins_digest')" \
    || fail "T0 receipt is not bound to the authority source pins digest"
  test "$(jq -er '.runtime.preoptimizer_attempt_claim.path' "$receipt")" = "$attempt_claim" \
    || fail "T0 receipt attempt claim path differs"
  claim_file_sha="$(sha256_file "$attempt_claim")"
  test "$(jq -er '.runtime.preoptimizer_attempt_claim.file_sha256' "$receipt")" = "$claim_file_sha" \
    || fail "T0 receipt attempt claim file SHA-256 differs"
  test "$(jq -er '.runtime.preoptimizer_attempt_claim.object_digest' "$receipt")" = "$(jq -er '.claim_digest' "$attempt_claim")" \
    || fail "T0 receipt attempt claim object digest differs"
  ...
  ' "$receipt" >/dev/null \
    || fail "T0 receipt strict legacy G1 replay runtime differs"
...
launch_canary() {
  ...
  srun --jobid="$job" --exclusive --exact --nodelist="$node" --nodes=1 --ntasks=1 \
    ...
    "$method_root/scripts/auh_stage_b_t0_single_update_20260824_retry7.sh" \
...
case "$command" in
  ...
  status)
    ...
    if [ -L "$attempt_claim" ] || [ -e "$attempt_claim" ]; then
      if [ ! -e "$output_root" ] && [ ! -L "$output_root" ]; then
        echo "[stage-b-t0] CLAIMED_INCOMPLETE retry7 attempt permanently consumed; use retry8 claim=$attempt_claim" >&2
        exit 1
      fi
      validate_completed_output
    ...
    fail "retry7 output exists without its permanent attempt claim"
    ;;
  *)
    usage
    ;;
