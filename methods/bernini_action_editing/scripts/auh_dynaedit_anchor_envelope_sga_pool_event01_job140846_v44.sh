...
# Round 44: hold the canonical anchor-action reward fixed and expand only
# DynaEdit's independent early noise-proposal pool.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=ANCHOR_ENVELOPE_SGA_TRUST003_K8_P16_R8
    export EARLY_CANDIDATE_COUNT=8
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=ANCHOR_ENVELOPE_SGA_TRUST003_K12_P16_R8
    export EARLY_CANDIDATE_COUNT=12
    ;;
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_anchor_envelope_sga_pool_event01_v44"
