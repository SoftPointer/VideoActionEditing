...
readonly work_job0="${BERNINI_SNC_STAGE_B_INFER_WORK_JOB0:?set first work holder}"
readonly work_job1="${BERNINI_SNC_STAGE_B_INFER_WORK_JOB1:?set second work holder}"
...
readonly expected_runtime_sha=8dfef53af90cd53af6fbda6baa8ac3451fe38ef81d740c6d8b0a0202bccfa1bd
readonly expected_dependency_sha=62df125ac130697b03aaea167b17a02d7fcb9d766a72f0bef71037924114e59f
readonly expected_release_archive_sha=298a0a63dda44843a828b70b87c9b41441c84a004b7cfec9f4340e2497ba54c1
readonly expected_release_manifest_sha=72a787ce5e645e7cb44d3331777899e93d35d07e7155c9bff97682367fe38766
readonly expected_release_revision=65440005f770fa2fb0691d2d6dfe1fcbc3bce357
...
usage() {
  cat >&2 <<'EOF'
usage: auh_infer_source_noised_carrier_stage_b_two_holder_v4.sh run
...
_TRAINING_OUTPUT, _SOURCE_VIDEO, _PYTHON_BIN, _WORK_JOB0, _WORK_JOB1,
...
assert m["schema_version"]=="bernini-source-noised-carrier-stage-b-inference-release-v2"
assert m["release_generation"]=="r2" and m["file_count"]==14 and m["content_closure_sha1"]==revision
...
readonly decode_smoke_receipt="${run_root}/real-numpy-decode-smoke.json"
CUDA_VISIBLE_DEVICES= HIP_VISIBLE_DEVICES= ROCR_VISIBLE_DEVICES= \
  "${python_bin}" -I -B - "${method_root}" "${decode_smoke_receipt}" "${run_root}/.real-numpy-smoke.mp4" <<'PY'
import hashlib
import json
import os
from pathlib import Path
...
fake_video_path = Path(sys.argv[3])
assert receipt_path.is_absolute() and fake_video_path.is_absolute()
assert not receipt_path.exists() and not fake_video_path.exists()
assert os.environ.get("CUDA_VISIBLE_DEVICES") == ""
assert os.environ.get("HIP_VISIBLE_DEVICES") == ""
assert os.environ.get("ROCR_VISIBLE_DEVICES") == ""
sys.path.insert(0, str(method_root))
import numpy as np
import infer_source_noised_carrier_stage_b_v1 as stage_b_infer

decoded = np.linspace(0.0, 1.0, num=81 * 2 * 3 * 3, dtype=np.float32).reshape(81, 2, 3, 3)
writer_calls = []
def writer(value, path, *, fps):
    writer_calls.append((value, path, fps))

audit = stage_b_infer.save_validated_vae_decoded_clip(
    decoded,
    output_path=fake_video_path,
    expected_height=2,
    expected_width=3,
    fps=25,
...
assert writer_calls[0][0] is decoded
assert writer_calls[0][1] == str(fake_video_path)
assert writer_calls[0][2] == 25
assert audit == {
...
assert not fake_video_path.exists()
unsigned = {
    "schema_version": "bernini-stage-b-real-numpy-decode-smoke-v1",
    "complete": True,
    "gpu_visibility_disabled": True,
    "real_numpy_ndarray": True,
    "original_array_identity_preserved": True,
    "writer_called_once": True,
    "fake_video_not_written": True,
    "audit": audit,
}
canonical = json.dumps(unsigned, sort_keys=True, separators=(",", ":"), ensure_ascii=True, allow_nan=False).encode("ascii")
receipt = {**unsigned, "receipt_digest": hashlib.sha256(canonical).hexdigest()}
raw = json.dumps(receipt, sort_keys=True, separators=(",", ":"), ensure_ascii=True, allow_nan=False).encode("ascii") + b"\n"
...
PY
[[ -f "${decode_smoke_receipt}" && ! -L "${decode_smoke_receipt}" ]] || fail "real NumPy decode smoke receipt missing"
...
run_branch() {
  local branch="$1" mode="$2" arm="$3" port="$4" output pid0 pid1
  output="${run_root}/${branch}"
  ...
  launch_node() {
    ...
    launch_child "${job}" "${node}" "${log}" --ntasks=1 --cpus-per-task=16 --mem=56G --gres=gpu:mi210:2 \
      env BERNINI_SNC_STAGE_B_INFER_CONTROLLER_SHA256="${controller_sha}" \
        BERNINI_SNC_STAGE_B_INFER_WORK_JOB0="${work_job0}" BERNINI_SNC_STAGE_B_INFER_WORK_JOB1="${work_job1}" \
...
assert_all_parents_running
printf 'COMPLETE_STAGE_B_TRAIN_FIT_FRESH_NOISE_ENGINEERING_DECODE_ONLY iid=%s style_id=%s seed=%s modes=registered-probes,full40-evolved-target-all40-route-extrapolation heldout=false action_success=false method_success=false scientific_claim=false anchor_condition=false work_holders=%s,%s retained_only=%s parents_retained=135407,135411,135412\n' \
  "${iid}" "${style_id}" "${seed}" "${work_job0}" "${work_job1}" "${retained_job}" >"${run_root}/controller.COMPLETE"
