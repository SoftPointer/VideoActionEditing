#!/usr/bin/env bash
# Create-once Stage-B target-T0 retry-1 one-update canary launcher.
#
...
source_root="$experiment_root/source_stage_b_t0_retry1"
method_root="$source_root/methods/bernini_action_editing"
authority_addendum="$source_root/stage_b_t0_single_update_retry_authority_addendum.json"
stage_root="$experiment_root/stage_b_t0_retry1"
log_root="$experiment_root/logs/stage_b_t0_retry1"
...
usage() {
  cat >&2 <<'EOF'
usage:
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry1.sh preflight
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry1.sh launch <parent-job-id>
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry1.sh status
...
require_active_authority_contract() {
  ...
  test "$(authority_value '.runtime_paths.fresh_source_root_name')" = source_stage_b_t0_retry1 \
    || fail "fresh source revision differs"
  test "$(authority_value '.runtime_paths.fresh_stage_root_name')" = stage_b_t0_retry1 \
  ...
  require_source_pin methods/bernini_action_editing/scripts/auh_stage_b_t0_single_update_20260824_retry1.sh
...
launch_canary() {
  ...
  srun --jobid="$job" --exclusive --exact --nodelist="$node" --nodes=1 --ntasks=1 \
    ...
    "$method_root/scripts/auh_stage_b_t0_single_update_20260824_retry1.sh" \
