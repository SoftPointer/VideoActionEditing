#!/usr/bin/env bash
# Sealed reserve4 fit40 authoring-media generation on 136141/gpu299.
#
...
# forbid all training, optimizer, confirmation, Phi, and P/O operations.

set -Eeuo pipefail
umask 077

fail() { echo "[generic-action-fit40-v13] ERROR: $*" >&2; exit 2; }
...
readonly launch_confirmation=launch-approved-generic-action-fit40-generation-136141
...
[[ "${phase}" == generation ]] || fail "only fit generation is present"
[[ "${split}" == fit ]] || fail "confirmation is absent from this release"
...
for digest_name in controller_plan_sha upstream_plan_sha method_archive_sha method_manifest_sha python_sha controller_sha launcher_sha generator_sha r10_compile_smoke_receipt_sha r10_generation_log_sha; do
...
readonly controller="${method_root}/generic_action_data_prep_controller_v1.py"
readonly launcher="${method_root}/scripts/auh_generic_action_data_prep_136141_world4_v1.sh"
readonly generator="${method_root}/tools/reserve4_fixed_generation_sp4_v1.py"
readonly release_builder="${method_root}/tools/build_generic_action_data_prep_release_v1.py"
for path in "${controller_plan}" "${upstream_plan}" "${method_archive}" "${method_manifest}" "${controller}" "${launcher}" "${generator}" "${release_builder}" "${checkpoint_manifest}" "${r10_compile_smoke_receipt}" "${r10_generation_log}"; do
...
if [[ "${role}" == child ]]; then
  ...
  task_scratch="$(mktemp -d -- "${scratch_parent}/generic-action-fit40-${SLURM_JOB_ID}-${SLURM_STEP_ID}.XXXXXXXX")"
  ...
  for path in "${host_memory_sample_journal}" "${host_memory_monitor_start_receipt}" "${host_memory_compile_gate}" "${host_memory_terminal_receipt}" "${host_memory_stop_path}"; do
    [[ ! -e "${path}" && ! -L "${path}" ]] || fail "host memory monitor output is not fresh: ${path}"
  done
  host_memory_monitor_pid=""
  "${python_bin}" -B "${generator}" host-memory-monitor \
  ...
  for _ in $(seq 1 2000); do
    ...
    if [[ -f "${host_memory_monitor_start_receipt}" && ! -L "${host_memory_monitor_start_receipt}" ]] && \
       "${python_bin}" -B "${generator}" assert-host-memory-monitor-live >/dev/null 2>&1; then
  ...
  assert_only_host_memory_monitor() {
    ...
    "${python_bin}" -B "${generator}" assert-host-memory-monitor-live >/dev/null || fail "host memory monitor health replay failed"
  }
  assert_only_host_memory_monitor
  ...
  run_compile_smoke() {
    ...
    "${python_bin}" -B "${generator}" smoke-sp4 \
      ...
      --master-port "${master_port}" --receipt-output "${receipt}" \
      --host-memory-gate-output "${host_memory_compile_gate}" \
      --r10-compile-smoke-receipt "${r10_compile_smoke_receipt}" \
      --expected-r10-compile-smoke-receipt-sha256 "${r10_compile_smoke_receipt_sha}" \
      --r10-generation-log "${r10_generation_log}" \
      --expected-r10-generation-log-sha256 "${r10_generation_log_sha}" || fail "full native40 compile smoke failed"
  ...
  "${python_bin}" -B "${generator}" seal-terminal-host-memory-gate \
...
assert_idle_once() {
  local steps snapshot count memory_count busy
  assert_parent_running
...
srun --jobid="${holder_job}" --nodelist="${holder_node}" --nodes=1 --ntasks=1 \
  ...
  env \
    ...
    bash "${launcher}" __child >"${run_root}/logs/generation-fit-all8-serial4.log" 2>&1 &
child_pid=$!
wait "${child_pid}"; status=$?
child_pid=""
set -e
if (( status != 0 )); then tail -n 240 "${run_root}/logs/generation-fit-all8-serial4.log" >&2 || true; fail "all8 serial-four-shard child failed status=${status}"; fi
...
"${python_bin}" -B "${generator}" audit \
  ...
  --gap-output "${run_root}/generation-gap-after-run.json" >/dev/null || fail "fit generation audit failed"
[[ -f "${run_root}/generation-audit.json" && ! -L "${run_root}/generation-audit.json" ]] || fail "fit generation audit receipt missing"
...
"${python_bin}" -B "${controller}" seal-generation-closure \
  ...
  --output "${run_root}/generation-closure-receipt.json" >/dev/null || fail "exact fit40 generation directory/receipt closure failed"
