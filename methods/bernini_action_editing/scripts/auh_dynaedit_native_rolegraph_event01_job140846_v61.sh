...
case "$host:$mode" in
  auh7-1b-gpu-246:sga)
    label=NATIVE_ROLEGRAPH_SGA_ALL40_B4_9_P24_R8
    arm=AQK_SGA5
    ;;
  auh7-1b-gpu-247:avg)
    label=NATIVE_ROLEGRAPH_AVG_ALL40_B4_9_P24_R8
    arm=AQK_AVG5
    ;;
  *)
    echo "Round 61 mode $mode is not assigned to host $host" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_native_rolegraph_event01_v61"
