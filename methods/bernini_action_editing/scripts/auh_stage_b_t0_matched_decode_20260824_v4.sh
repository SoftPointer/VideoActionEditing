...
source_root="$experiment_root/source_stage_b_t0_decode_v4"
...
decode_root="$experiment_root/stage_b_t0_retry8/matched_decode_v4/0be6494dfac3"
log_root="$experiment_root/logs/stage_b_t0_retry8/matched_decode_v4"
...
expected_runner_sha=6b41d250acc44dc868a708a36bd9b73bfb6b3cd96391e1730f196d008cc2d38d
expected_test_sha=98713ea12a3701b1fccf23b2acc80e0e9102fa0c44142152f8ba8a6898dcc3e5
...
fail() {
  printf '[stage-b-t0-decode-v4] ERROR: %s\n' "$*" >&2
  exit 1
}
...
run_worker() {
  ...
  scratch="/tmp/action-repr-t0-decode-v4-${step}-${route}-${job}-${step_token}"
  ...
  printf '[stage-b-t0-decode-v4] DECODE_PASS step=%s route=%s output=%s\n' "$step" "$route" "$output"
}
...
launch_decode() {
  ...
  printf '[stage-b-t0-decode-v4] srun complete job=%s step=%s route=%s\n' "$job" "$step" "$route"
}
...
case "$command" in
  preflight)
    [ "$#" -eq 0 ] || usage
    common_preflight
    echo '[stage-b-t0-decode-v4] PREFLIGHT_PASS WORLD4_SP4=true steps=40 matched=true pyav=true argparse_abbrev=false inference_audit=frozen'
    ;;
  decode)
    [ "$#" -eq 3 ] || usage
    launch_decode "$1" "$2" "$3"
  ...
  status)
    ...
    if [ ! -d "$decode_root" ]; then
      echo '[stage-b-t0-decode-v4] NOT_STARTED'
      exit 0
    fi
