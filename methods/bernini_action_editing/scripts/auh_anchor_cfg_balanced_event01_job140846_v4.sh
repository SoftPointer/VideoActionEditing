...
# Apply the pure-T2V attention route to both target APG branches.  This keeps
# anchor geometry outside the text-CFG residual instead of amplifying the patch
# by guidance scale 4.  Direct SSH is required inside the existing allocation:
# Slurm Job 140846 requested only 64G/node although the nodes have 2.2T RAM.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:1)
    label=TRQK_CFG2_A010_EARLY3_MID8_21; transport=temporal_residual_qk; strength=0.10; steps=3; blocks=8-21 ;;
  auh7-1b-gpu-247:1)
    label=TRQK_CFG2_A010_EARLY8_MID8_21; transport=temporal_residual_qk; strength=0.10; steps=8; blocks=8-21 ;;
...
output_root="$stage/anchor_cfg_balanced_event01_v4"
...
scratch="/tmp/anchor-cfg-balanced-$label-140846"
