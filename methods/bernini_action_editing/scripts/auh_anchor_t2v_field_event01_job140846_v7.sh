...
# DynaEdit-style clean edit ODE over Bernini's source-free generation field,
# rather than its already source-conditioned RV2V editor field.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=T2V_FIELD_ANC1_NO_ANCHOR; steps=0 ;;
  auh7-1b-gpu-247:0)
    label=T2V_FIELD_TRQK_A010_EARLY3; steps=3 ;;
...
output_root="$stage/anchor_t2v_field_event01_v7"
...
source_caption="An exactly 81-frame realistic continuous locked-camera video at 25 fps shows one young child in a pale blue dress walking slowly away from the camera along a white gravel garden path with large flat stepping stones. Preserve the child, pale blue clothing, every stone, trees, shrubs, houses, lighting, framing and camera."

scratch="/tmp/anchor-t2v-field-$label-140846"
