...
# Target-caption precision ablation.  The prior short caption said only "a
# stone", allowing high CFG to turn a salient white stepping stone into the
# lifted object.  This round names a loose grey pebble and explicitly freezes
# all large flat white stepping stones.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=PRECISEOBJ_CFG45_85_NOANCHOR
    source_cfg=4.5
    target_cfg=8.5
    transport_steps=0
    ;;
  auh7-1b-gpu-246:1)
    label=PRECISEOBJ_CFG45_85_CONDATT_A003_EARLY8
    source_cfg=4.5
    target_cfg=8.5
    transport_steps=8
    ;;
  auh7-1b-gpu-247:0)
    label=PRECISEOBJ_CFG40_75_NOANCHOR
    source_cfg=4.0
    target_cfg=7.5
    transport_steps=0
    ;;
  auh7-1b-gpu-247:1)
    label=PRECISEOBJ_CFG40_75_CONDATT_A003_EARLY8
    source_cfg=4.0
    target_cfg=7.5
    transport_steps=8
    ;;
...
output_root="$stage/dynaedit_precise_object_caption_event01_v24"
...
target_caption="In the same locked-camera landscaped garden, the same child in the same pale blue outfit stops walking, bends both knees into a deep crouch, reaches one hand to one small loose grey pebble in the gravel beside the child's shoe, closes the fingers around that exact pebble with visible contact, lifts only that same small pebble, rises back to standing, and holds the small pebble visibly in one hand. The pebble remains small and grey. Its vacated gravel spot stays empty. Every large flat white stepping stone remains on the ground with unchanged size and shape; all other stones, plants, framing and lighting remain unchanged."
...
scratch="/tmp/dynaedit-precise-object-$label-140846"
...
exec "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  ...
  --source-cfg-scale "$source_cfg" \
  --target-cfg-scale "$target_cfg" \
