#!/usr/bin/env bash
# Run exact47 diagnostics inside one full-node srun step of an owned 8-GPU allocation.

set -euo pipefail
umask 077

if [[ "$#" -ne 16 ]]; then
  echo "usage: $0 <preflight|full> <allocation-job-id> <python-bin> <diagnostic.py> <diagnostic-sha256> <attempts-root> <root-spec-sha256> <visual-checkpoint> <checkpoint-manifest> <evaluator-spec> <evaluator-spec-sha256> <visual-scorer.py> <visual-scorer-sha256> <visual-contract.py> <visual-contract-sha256> <fresh-output-root>" >&2
  exit 64
fi
...
root_spec_sha256="$7"
visual_checkpoint="$8"
checkpoint_manifest="$9"
evaluator_spec="${10}"
evaluator_spec_sha256="${11}"
visual_scorer_source="${12}"
visual_scorer_sha256="${13}"
visual_contract_source="${14}"
visual_contract_sha256="${15}"
output_root="${16}"
...
if [[ ! -d "$attempts_root" || ! -d "$visual_checkpoint" || ! -f "$checkpoint_manifest" || ! -f "$evaluator_spec" || ! -f "$visual_scorer_source" || ! -f "$visual_contract_source" ]]; then
...
echo "partial47 DINO ${mode} complete: $output_root"
