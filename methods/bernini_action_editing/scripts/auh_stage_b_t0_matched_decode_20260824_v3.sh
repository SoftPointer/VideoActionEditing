...
source_root="$experiment_root/source_stage_b_t0_decode_v3"
...
decode_root="$experiment_root/stage_b_t0_retry8/matched_decode_v3/0be6494dfac3"
log_root="$experiment_root/logs/stage_b_t0_retry8/matched_decode_v3"
...
expected_runner_sha=1f86b174306fdc2fc711f9f5bb0e99cfc814f79468b9470c5e8181c5b0badf5d
expected_test_sha=4e56ea045d5743e31caeaac7adb7a43049e31bf339b6266da563a9604af42c20
...
fail() {
  printf '[stage-b-t0-decode-v3] ERROR: %s\n' "$*" >&2
  exit 1
}
...
run_worker() {
  ...
  scratch="/tmp/action-repr-t0-decode-v3-${step}-${route}-${job}-${step_token}"
  ...
  printf '[stage-b-t0-decode-v3] DECODE_PASS step=%s route=%s output=%s\n' "$step" "$route" "$output"
}
...
launch_decode() {
  ...
  printf '[stage-b-t0-decode-v3] srun complete job=%s step=%s route=%s\n' "$job" "$step" "$route"
}
...
case "$command" in
  preflight)
    [ "$#" -eq 0 ] || usage
    common_preflight
    echo '[stage-b-t0-decode-v3] PREFLIGHT_PASS WORLD4_SP4=true steps=40 matched=true pyav=true argparse_abbrev=false'
    ;;
  decode)
    [ "$#" -eq 3 ] || usage
    launch_decode "$1" "$2" "$3"
  ...
  status)
    ...
    if [ ! -d "$decode_root" ]; then
      echo '[stage-b-t0-decode-v3] NOT_STARTED'
      exit 0
    fi
