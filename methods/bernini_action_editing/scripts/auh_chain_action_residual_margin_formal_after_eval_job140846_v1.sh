#!/usr/bin/env bash
# Train eight formal residual-margin arms only after all V1 decoding is done.

set -Eeuo pipefail

job_id=140846
root="${ACTION_RESIDUAL_EXPERIMENT_ROOT:?set residual experiment root}"
prior_eval_root="${ACTION_RESIDUAL_PRIOR_EVAL_ROOT:?set V1 unseen eval root}"
...
canary_receipt="${root}/train-canary-margin-010-r2/checkpoint-00000001/receipt.json"
...
[[ -f "${canary_receipt}" && ! -e "${root}/TRAINING_COMPLETE" ]]
...
python3 - "${canary_receipt}" "${revision}" "${archive_sha}" "${expected_cache_sha}" <<'PY'
import json
import sys
...
while [[ ! -f "${prior_eval_root}/EVALUATION_COMPLETE" ]]; do sleep 60; done
sleep 60
