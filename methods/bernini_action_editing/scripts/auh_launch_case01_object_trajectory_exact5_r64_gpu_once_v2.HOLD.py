#!/usr/bin/env python3
"""Receipt-gated one-shot GPU controller for object-trajectory exact5.

This checked-in copy is deliberately HOLD.  A READY copy may be derived only
after the package static, captured-root, and package-world4 admissions have
...
SCHEMA = "case01-object-trajectory-exact5-r64-gpu-controller-v2"
...
READY_PLAN_SCHEMA = "case01-object-trajectory-exact5-plan-v1"
CONTROLLER_STATE = "HOLD_PENDING_STATIC_ROOTFAKE_WORLD4_AND_ROOT_PINS"
...
HOLD_PAYLOAD_PATH = PACKAGE_ROOT / "launch/root_launch_payload_HOLD_v1.sh"

READY_PLAN_PATH = (
    PACKAGE_ROOT / "runtime/case01_object_trajectory_exact5_r64_READY_plan_v2.json"
)
RUNTIME_RECEIPT_PATH = PACKAGE_ROOT / "runtime/exact5_gpu_runtime_v2.json"
RANK_CACHE_RECEIPT_PATH = (
    PACKAGE_ROOT / "runtime/exact5_gpu_rank_cache_receipt_v2.json"
)
ATTEMPT_PATH = PACKAGE_ROOT / "evidence/exact5_gpu_attempt_v2.json"
DISPATCH_PATH = PACKAGE_ROOT / "evidence/exact5_gpu_dispatch_v2.json"
EVIDENCE_PATH = PACKAGE_ROOT / "evidence/exact5_gpu_controller_receipt_v2.json"
STDOUT_PATH = PACKAGE_ROOT / "logs/exact5_gpu_srun_v2.stdout.log"
STDERR_PATH = PACKAGE_ROOT / "logs/exact5_gpu_srun_v2.stderr.log"
OUTPUT_REPORT_PATH = (
...
OUTPUT_ROOT = PACKAGE_ROOT / "outputs/media"
...
WORLD4_SCHEMA = "case01-object-trajectory-exact5-world4-admission-v5"
WORLD4_CONTROLLER_SCHEMA = (
    "case01-object-trajectory-exact5-package-world4-controller-v3"
)
...
RUNTIME_RECEIPT_FIELDS = frozenset({
    ...
    "r64_checkpoint_manifest_sha256", "exact25_identity_set_digest",
...
SEALED_HOLD_PLAN_SHA256 = (
    "2ac25f2501e15550a314d4888554428339d3a5353765e10e38859af2e1379b45"
)
SEALED_HOLD_PLAN_SIZE = 31_624
SEALED_HOLD_PLAN_DIGEST = (
    "ed93ebb608fbd1cfbe485be2db431e7ed30a5670026e94b6bbfe1f065e2e0517"
)
SEALED_LAUNCH_INPUT_SHA256 = (
    "ba5362ddf4a6661bb1f7da3e5ca9db166565ce6d77851338e74db9728265f87c"
)
SEALED_LAUNCH_INPUT_SIZE = 9_424
NAMED_HOLD_PAYLOAD_SHA256 = (
    "1338c2b45a2a4dddf8eb1768b6f5d76f384d34d024c4406374388651213b9933"
)
NAMED_HOLD_PAYLOAD_SIZE = 12_271

# Learned only from completed immutable admissions.  The generic source must
# remain HOLD while even one of these is not an exact final literal.
...
WORLD4_RECEIPT_SIZE: int | str = 49_351
...
WORLD4_EVIDENCE_SIZE: int | str = 14_866
...
def dynamic_pin_values() -> dict[str, Any]:
    return {
        ...
        "world4_receipt_sha256": WORLD4_RECEIPT_SHA256,
        "world4_receipt_size": WORLD4_RECEIPT_SIZE,
        "world4_receipt_digest": WORLD4_RECEIPT_DIGEST,
        "world4_evidence_sha256": WORLD4_EVIDENCE_SHA256,
        "world4_evidence_size": WORLD4_EVIDENCE_SIZE,
        "world4_evidence_digest": WORLD4_EVIDENCE_DIGEST,
...
def validate_publication_receipt(held: HeldAuthority) -> dict[str, Any]:
    ...
    if (
        value.get("schema_version") != PACKAGE_PUBLICATION_SCHEMA
...
def validate_static_receipt(
    ...
    _self_digested(value, "receipt_digest", STATIC_RECEIPT_DIGEST)
    launch = report["launch"]
    if (
        value.get("schema_version") != STATIC_SCHEMA
        ...
        or value.get("input_sha256") != launch["input"]["sha256"]
    ):
        raise GPUControllerError("static admission receipt differs")
    return value
...
def validate_root_fake_receipt(
    ...
    _self_digested(value, "receipt_digest", ROOT_FAKE_RECEIPT_DIGEST)
    release = report["launch"]["release"]
    if (
        value.get("schema_version") != ROOT_FAKE_SCHEMA
        ...
        or value.get("task_ids") != list(TASK_IDS)
        or value.get("arm_order") != list(ARM_ORDER)
        or value.get("identity_set_digest")
        != object_digest(release["identities"])
        or value.get("launch_input_sha256")
        != report["launch"]["input"]["sha256"]
        or value.get("plan_sha256") != report["plan"]["sha256"]
        ...
        raise GPUControllerError("captured-root admission receipt differs")
    return value
...
def validate_world4(
    ...
    receipt = strict_json(receipt_held.raw, label="package world4 receipt")
    _self_digested(receipt, "receipt_digest", WORLD4_RECEIPT_DIGEST)
    if (
        receipt.get("schema_version") != WORLD4_SCHEMA
        or receipt.get("status") != "ADMITTED_WORLD4_TENSOR_ABI_HOLD_ONLY"
        or receipt.get("launch_allowed") is not False
        or receipt.get("publication_performed") is not False
        or receipt.get("renderer_or_vae_loaded") is not False
        or receipt.get("happy_scheduler_steps") != 40
        or receipt.get("real_torchrun_process_count_per_scenario") != 4
        or receipt.get("expected_runtime_versions") != {
            "torch": EXPECTED_TORCH_VERSION, "hip": EXPECTED_HIP_VERSION,
        }
        or len(receipt.get("scenarios", [])) != 7
        or any(row.get("process_group_reaped") is not True
               for row in receipt.get("scenarios", []))
    ):
        raise GPUControllerError("package world4 receipt differs")
    evidence = strict_json(evidence_held.raw, label="package world4 evidence")
    _self_digested(evidence, "evidence_digest", WORLD4_EVIDENCE_DIGEST)
    if (
        evidence.get("schema_version") != WORLD4_CONTROLLER_SCHEMA
        or evidence.get("status") != "PASS_PACKAGE_WORLD4"
        ...
        or evidence.get("world4_receipt_sha256")
        != hashlib.sha256(receipt_held.raw).hexdigest()
        or evidence.get("world4_receipt_digest") != receipt["receipt_digest"]
        or evidence.get("stderr_empty") is not True
        or evidence.get("process_group_zero") is not True
        or evidence.get("publication_empty") is not True
        or evidence.get("stage_cache_absent") is not True
        or evidence.get("scenario_count") != 7
        or evidence.get("runtime_rows_equal") is not True
        or evidence.get("project_rows_equal") is not True
        or evidence.get("old_cpu_admission_receipt_consumed") is not False
        or evidence.get("launch_allowed") is not False
        or evidence.get("renderer_or_vae_loaded") is not False
    ):
        raise GPUControllerError("package world4 evidence differs")
...
def open_package_gate() -> PackageGate:
    ...
    try:
        ...
        authorities.append(static); static_value = validate_static_receipt(static, report)
        root_fake = open_authority(
        ...
        authorities.append(root_fake)
        root_fake_value = validate_root_fake_receipt(root_fake, report)
        world4 = open_authority(
            WORLD4_RECEIPT_PATH, expected_sha256=WORLD4_RECEIPT_SHA256,
            expected_size=WORLD4_RECEIPT_SIZE, expected_mode=RECEIPT_MODE,
            maximum_size=MAX_JSON_SIZE,
        )
        authorities.append(world4)
        world4_evidence = open_authority(
            WORLD4_EVIDENCE_PATH, expected_sha256=WORLD4_EVIDENCE_SHA256,
            expected_size=WORLD4_EVIDENCE_SIZE, expected_mode=RECEIPT_MODE,
            maximum_size=MAX_JSON_SIZE,
        )
        authorities.append(world4_evidence)
        world4_value, world4_evidence_value = validate_world4(
            world4, world4_evidence,
        )
        root = open_directory(
        ...
        gate = PackageGate(authorities, root, {
            ...
            "static": static_value, "root_fake": root_fake_value,
            "world4": world4_value, "world4_evidence": world4_evidence_value,
        })
...
def require_fresh_outputs() -> None:
    # Admission artifacts are the only pre-existing mutable-package leaves.
    _exact_names(
        PACKAGE_ROOT / "evidence",
        {
            STATIC_RECEIPT_PATH.name, ROOT_FAKE_RECEIPT_PATH.name,
            WORLD4_RECEIPT_PATH.name, "exact5_world4_attempt_v3.json",
            WORLD4_EVIDENCE_PATH.name,
        },
        label="pre-GPU evidence",
    )
    _exact_names(
        PACKAGE_ROOT / "logs",
        {"exact5_world4_srun_v3.stdout.log", "exact5_world4_srun_v3.stderr.log"},
        label="pre-GPU logs",
    )
...
def _open_package_identities(
    ...
    try:
        ...
        if tuple(key for key in result if key != "launch_input") != IDENTITY_ROLES:
            raise GPUControllerError("held exact25 order differs")
...
fields={"schema_version","entry_mode","external_root_of_trust","bash_privileged_mode","slurm_export_none","named_hold_payload_executed","ready_plan_is_derived_overlay","python_is_executed_from_held_fd","runner_is_compiled_from_captured_fd_bytes","all_exact25_named_identities_replayed_before_runner","expected_allocation_gpu_count","campaign_mode","task_count","selected_task_ids","arm_order","all_arms_attempted_exactly_once_by_runner","retry_allowed","partial_outputs_are_not_results","expected_runtime_versions","expected_gpu_name","runtime_receipt_path","rank_cache_receipt_path","rank_cache_root","runner_attestation_path","torch_package_init_authority","directory_authorities","holder_job_id","expected_node","identities","runner_arguments"}
if (type(release) is not dict or set(release)!=fields or release_raw!=canonical(release) or digest(release)!=release_digest
 or release.get("schema_version")!="case01-object-trajectory-exact5-r64-gpu-release-v2" or release.get("entry_mode")!="trusted_controller_streamed_stdin"
 ...
 or release.get("runner_is_compiled_from_captured_fd_bytes") is not True or release.get("all_exact25_named_identities_replayed_before_runner") is not True
 or release.get("expected_allocation_gpu_count")!=8 or release.get("campaign_mode")!="case01-object-trajectory-exact5-r64-engineering-oracle"
 ...
 or release.get("rank_cache_root")!="/tmp/bernini-case01-object-trajectory-exact5-r64-job143808-node292-r1-rank-cache"):
 raise RuntimeError("GPU release differs")
identities=release.get("identities");arguments=release.get("runner_arguments")
if type(identities) is not dict or set(identities)!=set(roles) or len(identities)!=25 or len({row.get("path") for row in identities.values() if type(row) is dict})!=25: raise RuntimeError("exact25 production identity closure differs")
...
try:
 ...
 runtime={"schema_version":"case01-object-trajectory-exact5-r64-gpu-controller-v2-runtime","status":"PASS_GPU_RUNTIME_BEFORE_RUNNER","holder_job_id":job_id,"node":step_nodelist,"slurm_step_id":step_id,"torch_version":torch.__version__,"hip_version":torch.version.hip,"device_count":torch.cuda.device_count(),"device_names":names,"r64_checkpoint_manifest_sha256":identities["r64_checkpoint_manifest"]["sha256"],"exact25_identity_set_digest":digest(identities),"renderer_started":False}
...
arms=["null_before","route_off","trajectory_bone_only","trajectory_dog_bone","null_after"]
fields={"schema_version","entry_mode","external_root_of_trust","bash_privileged_mode","slurm_export_none","named_hold_payload_executed","ready_plan_is_derived_overlay","python_is_executed_from_held_fd","runner_is_compiled_from_captured_fd_bytes","all_exact25_named_identities_replayed_before_runner","expected_allocation_gpu_count","campaign_mode","task_count","selected_task_ids","arm_order","all_arms_attempted_exactly_once_by_runner","retry_allowed","partial_outputs_are_not_results","expected_runtime_versions","expected_gpu_name","runtime_receipt_path","rank_cache_receipt_path","rank_cache_root","runner_attestation_path","torch_package_init_authority","directory_authorities","holder_job_id","expected_node","identities","runner_arguments"}
if (set(release)!=fields or digest(release)!=release_digest or release.get("schema_version")!="case01-object-trajectory-exact5-r64-gpu-release-v2" or release.get("entry_mode")!="trusted_controller_streamed_stdin" or release.get("external_root_of_trust")!="receipt-gated-controller-held-bytes" or release.get("bash_privileged_mode") is not True or release.get("slurm_export_none") is not True or release.get("named_hold_payload_executed") is not False or release.get("ready_plan_is_derived_overlay") is not True or release.get("python_is_executed_from_held_fd") is not True or release.get("runner_is_compiled_from_captured_fd_bytes") is not True or release.get("all_exact25_named_identities_replayed_before_runner") is not True or release.get("expected_allocation_gpu_count")!=8 or release.get("campaign_mode")!="case01-object-trajectory-exact5-r64-engineering-oracle" or release.get("task_count")!=5 or release.get("selected_task_ids")!=tasks or release.get("arm_order")!=arms or release.get("all_arms_attempted_exactly_once_by_runner") is not True or release.get("retry_allowed") is not False or release.get("partial_outputs_are_not_results") is not True or release.get("expected_runtime_versions")!={"hip":"6.3.42131-fa1d09cbd","torch":"2.7.1+rocm6.3"} or release.get("expected_gpu_name")!="AMD Instinct MI210" or release.get("holder_job_id")!=job_id or release.get("expected_node")!=step_nodelist or job_nodelist!=step_nodelist or gpus_on_node!="8" or gpus_per_node!="8" or step_gpus!="0,1,2,3,4,5,6,7" or node_count!="1" or step_node_count!="1" or release.get("rank_cache_root")!="/tmp/bernini-case01-object-trajectory-exact5-r64-job143808-node292-r1-rank-cache"): raise RuntimeError("GPU release differs")
...
expected_dirs={"output":os.path.join(package_root,"outputs","media"),"final":os.path.join(package_root,"final"),"runtime":os.path.join(package_root,"runtime"),"site_packages":"/vast/users/guangyi.chen/anaconda3/envs/vace/lib/python3.12/site-packages"}
if argmap["--output-report"]!=os.path.join(expected_dirs["final"],"object_trajectory_exact5_report_v1.json") or release["runner_attestation_path"]!=os.path.join(expected_dirs["final"],"object_trajectory_exact5_runner_attestation_v1.json") or release["runtime_receipt_path"]!=os.path.join(expected_dirs["runtime"],"exact5_gpu_runtime_v2.json") or release["rank_cache_receipt_path"]!=os.path.join(expected_dirs["runtime"],"exact5_gpu_rank_cache_receipt_v2.json"): raise RuntimeError("compute target path closure differs")
...
def build_production_release(
    ...
    release: dict[str, Any] = {
        "schema_version": "case01-object-trajectory-exact5-r64-gpu-release-v2",
        ...
        "all_exact25_named_identities_replayed_before_runner": True,
...
def build_srun_argv(max_gate_step: int) -> list[str]:
    if type(max_gate_step) is not int or max_gate_step < 0:
        raise GPUControllerError("world4 step floor differs")
    return [
        SRUN_AUTHORITY["path"], f"--jobid={HOLDER_JOB_ID}",
        "--job-name=case01-object-trajectory-exact5-r64-gpu-v2",
...
def _nested_python_argv_upper_bound(release: Mapping[str, Any]) -> int:
...
def validate_srun_transport(
    command: Sequence[str], *, payload: bytes | None = None,
    release: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    expected = build_srun_argv(0)
    ...
    nested_width = _nested_python_argv_upper_bound(release)
    if (
...
def run_single_srun(
    ...
    validate_srun_transport(command, payload=payload, release=release)
    environment = srun_environment()
...
def validate_compute_binding_closure(
    *, runtime: Mapping[str, Any], runtime_raw: bytes,
    ...
    if (
        ...
        or runtime.get("exact25_identity_set_digest")
...
def validate_postflight(
    *, ready_plan: Mapping[str, Any], ready_plan_raw: bytes,
    ...
    try:
        ...
        log_names = {
            "exact5_world4_srun_v3.stdout.log",
            "exact5_world4_srun_v3.stderr.log",
...
def controller() -> dict[str, Any]:
    ...
    try:
        ...
        # held stdin channel.
        transport_preflight = validate_srun_transport(command)
        ...
        attempt: dict[str, Any] = {
            ...
            "held_exact25_identity_set_digest": object_digest({
        ...
        transport = validate_srun_transport(
            command, payload=payload, release=release,
        )
        ...
        evidence_names_before = {
            STATIC_RECEIPT_PATH.name, ROOT_FAKE_RECEIPT_PATH.name,
            WORLD4_RECEIPT_PATH.name, "exact5_world4_attempt_v3.json",
            WORLD4_EVIDENCE_PATH.name, ATTEMPT_PATH.name, DISPATCH_PATH.name,
        }
        _exact_names(
