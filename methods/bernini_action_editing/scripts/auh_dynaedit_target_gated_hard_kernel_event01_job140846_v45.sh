...
# Round 45: online fixed pure-T2V anchor in every solver cell/candidate.
# Hard-replace only target-active temporal trajectories using target-owned V.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=TARGETGATED_HARDKERNEL_TOP10_ALL40_B4_9_P16_R8
    export TRANSPORT_OVERRIDE=target_gated_hard_kernel_top10_attn_output
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=TARGETGATED_HARDKERNEL_TOP25_ALL40_B4_9_P16_R8
    export TRANSPORT_OVERRIDE=target_gated_hard_kernel_top25_attn_output
    ;;
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_target_gated_hard_kernel_event01_v45"
