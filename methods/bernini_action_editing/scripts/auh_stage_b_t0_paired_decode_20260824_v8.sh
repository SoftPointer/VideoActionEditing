...
source_root="$experiment_root/source_stage_b_t0_decode_v8_paired"
...
decode_root="$experiment_root/stage_b_t0_retry8/matched_decode_v8_paired/0be6494dfac3"
log_root="$experiment_root/logs/stage_b_t0_retry8/matched_decode_v8_paired"
...
expected_runner_sha=c3bfb70b6ba303fddf6edd10544d169122f9fdd6832e353dfa4a4504ff974489
expected_test_sha=be9afba6a1960f1c0703ad899b0ea4e823439f585e579f299892666cd7483877
...
fail() {
  printf '[stage-b-t0-paired-v8] ERROR: %s\n' "$*" >&2
  exit 1
}
...
print('[stage-b-t0-paired-v8] OUTPUT_REPLAY_PASS cells=%d baseline_gate=%s route_effect=%s' % (
...
run_worker() {
  ...
  mkdir -p "$decode_root/cells"
  local key
  for key in s0_route_off_a s0_zero s0_correct s1_route_off s1_zero s1_correct s1_temporal_shuffle s1_reverse s1_incomplete s1_wrong_action s0_route_off_b; do
    mkdir -p "$decode_root/cells/$key"
  done
  mkdir -p "$log_root"
  ...
  scratch="/tmp/action-repr-t0-paired-v8-${job}-${step_token}"
  ...
  if [ "$rc" -eq 4 ]; then
    validate_output
    fail 'paired latent determinism gate failed; preserve v8 evidence and do not compare step1'
  fi
  [ "$rc" -eq 0 ] || fail "WORLD4 paired decode failed rc=$rc; preserve v8 evidence"
  validate_output
  printf '[stage-b-t0-paired-v8] PAIRED_DECODE_PASS output=%s\n' "$decode_root"
}
...
launch_decode() {
  ...
  validate_output
  printf '[stage-b-t0-paired-v8] srun complete job=%s\n' "$job"
}
...
case "$command" in
  preflight)
    [ "$#" -eq 0 ] || usage
    common_preflight
    echo '[stage-b-t0-paired-v8] PREFLIGHT_PASS WORLD4_SP4=true cells=11 one_model_load=true prepared_cell_tree=true per_cell_real_clamp=true latent_rank_hash=true baseline_exact_gate=true videos=true no_cpu_fallback=true'
    ;;
  decode)
    [ "$#" -eq 1 ] || usage
    launch_decode "$1"
  ...
  status)
    ...
    if [ ! -d "$decode_root" ]; then
      echo '[stage-b-t0-paired-v8] NOT_STARTED'
      exit 0
    fi
