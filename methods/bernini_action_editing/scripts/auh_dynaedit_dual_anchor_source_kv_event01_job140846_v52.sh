...
case "$host" in
  auh7-1b-gpu-246)
    label=DUAL_ANCHORQ_EARLY_B4_9_SOURCEKV_LATE_B18_29_ALL
    transport=dual_hard_q_early_source_kv_late_all
    ;;
  auh7-1b-gpu-247)
    label=DUAL_ANCHORQ_EARLY_B4_9_SOURCEKV_LATE_B18_29_STATIC75
    transport=dual_hard_q_early_source_kv_late_static75
    ;;
  *)
    echo "Round 52 is restricted to Job 140846 nodes 246-247" >&2
    exit 3
    ;;
esac
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_dual_anchor_source_kv_event01_v52"
export TRANSPORT_OVERRIDE="$transport"
export STRENGTH_OVERRIDE=1.0
export TRANSPORT_STEPS_OVERRIDE=3
export BLOCKS_OVERRIDE=4-29
