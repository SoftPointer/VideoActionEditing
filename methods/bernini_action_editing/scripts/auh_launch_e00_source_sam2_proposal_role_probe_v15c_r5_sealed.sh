...
readonly RELEASE_REL="methods/bernini_action_editing/assets/e00_source_sam2_proposal_role_probe_v15c_r5_release.json"

fail() {
  echo "v15c-r5 sealed worker: $*" >&2
  exit 1
}

[[ "${1:-}" == "--sealed-worker" && "$#" -eq 8 ]] || \
  fail "entry requires externally authenticated sealed-worker arguments"
[[ "${V15C_R5_EXTERNAL_BOOTSTRAP:-}" == "1" ]] || fail "external bootstrap marker absent"
...
readonly FINALIZER="${METHOD_ROOT}/finalize_source_sam2_proposal_role_probe_v15c_r5.py"
...
' v15c-r5-python "${PYTHON_AUTHORITY}" "${PYTHON_FD}" "$@"
}
...
echo "v15c-r5 observer-only COMPLETE (reject-only, ROUTE_NO_GO): ${RUN_ROOT}"
