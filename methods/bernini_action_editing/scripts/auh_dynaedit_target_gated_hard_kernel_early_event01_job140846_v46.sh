...
# Round 46: keep the target-local hard route, but stop it before low-noise
# detail formation.  This isolates coarse action initiation from endpoint drift.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=TARGETGATED_HARDKERNEL_TOP10_EARLY3_B4_9_P16_R8
    export TRANSPORT_STEPS_OVERRIDE=3
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=TARGETGATED_HARDKERNEL_TOP10_EARLY8_B4_9_P16_R8
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_target_gated_hard_kernel_early_event01_v46"
