#!/usr/bin/env bash
# Create-once Stage-B target-T0 retry-6 one-update canary launcher.
#
...
source_root="$experiment_root/source_stage_b_t0_retry6"
method_root="$source_root/methods/bernini_action_editing"
authority_addendum="$source_root/stage_b_t0_single_update_retry6_authority_addendum.json"
stage_root="$experiment_root/stage_b_t0_retry6"
log_root="$experiment_root/logs/stage_b_t0_retry6"

runner="$method_root/train_action_repr_target_t0_canary_retry6_v1.py"
...
attempt_claim="$stage_root/target_t0/0be6494dfac3/.single_update.retry6.attempt_claim.json"
...
usage() {
  cat >&2 <<'EOF'
usage:
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry6.sh preflight
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry6.sh launch <parent-job-id>
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_retry6.sh status
...
require_active_authority_contract() {
  ...
  test "$(authority_value '.schema_version')" = bernini-action-repr-stage-b-t0-single-update-retry6-authority-addendum-v1 \
  ...
  test "$(authority_value '.document_role')" = create_once_stage_b_target_t0_single_update_retry6_authority \
    || fail "Stage-B retry6 authority document role differs"
  ...
  test "$(authority_value '.output_contract.attempt_claim_marker_name')" = .single_update.retry6.attempt_claim.json \
  ...
  test "$(authority_value '.output_contract.existing_claim_requires_new_revision')" = true \
    || fail "existing claim must require retry7"
  ...
  test "$(authority_value '.runtime_paths.fresh_source_root_name')" = source_stage_b_t0_retry6 \
    || fail "fresh source revision differs"
  test "$(authority_value '.runtime_paths.fresh_stage_root_name')" = stage_b_t0_retry6 \
  ...
  require_source_pin methods/bernini_action_editing/train_action_repr_target_t0_canary_retry6_v1.py
  ...
  require_source_pin methods/bernini_action_editing/scripts/auh_stage_b_t0_single_update_20260824_retry6.sh
  require_source_pin methods/bernini_action_editing/tests/test_train_action_repr_target_t0_canary_retry6_v1.py
  require_source_pin tests/test_auh_stage_b_t0_single_update_20260824_retry6.py
}

require_unclaimed_attempt() {
  if [ -L "$attempt_claim" ] || [ -e "$attempt_claim" ]; then
    if [ ! -e "$output_root" ] && [ ! -L "$output_root" ]; then
      fail "CLAIMED_INCOMPLETE retry6 attempt is permanently consumed; use retry7: $attempt_claim"
    fi
    fail "retry6 attempt claim already exists; no second optimizer is allowed; use retry7"
  fi
  test ! -e "$output_root" && test ! -L "$output_root" \
    || fail "retry6 output exists without its permanent preoptimizer claim"
}
...
validate_completed_output() {
  ...
  test -f "$attempt_claim" || fail "retry6 permanent attempt claim is absent"
  test ! -L "$attempt_claim" || fail "retry6 permanent attempt claim may not be a symlink"
  test -f "$receipt" || fail "T0 runner returned without receipt.json"
  test ! -L "$receipt" || fail "T0 receipt may not be a symlink"
  "$python_bin" -B -c \
    'import sys; from train_action_repr_target_t0_canary_retry6_v1 import validate_published_t0_output; validate_published_t0_output(sys.argv[1])' \
    "$output_root"
  test "$(jq -er '.schema_version' "$receipt")" = bernini-action-repr-target-t0-one-step-retry6-receipt-v1 \
...
launch_canary() {
  ...
  srun --jobid="$job" --exclusive --exact --nodelist="$node" --nodes=1 --ntasks=1 \
    ...
    "$method_root/scripts/auh_stage_b_t0_single_update_20260824_retry6.sh" \
...
case "$command" in
  ...
  status)
    ...
    if [ -L "$attempt_claim" ] || [ -e "$attempt_claim" ]; then
      if [ ! -e "$output_root" ] && [ ! -L "$output_root" ]; then
        echo "[stage-b-t0] CLAIMED_INCOMPLETE retry6 attempt permanently consumed; use retry7 claim=$attempt_claim" >&2
        exit 1
      fi
      validate_completed_output
    ...
    fail "retry6 output exists without its permanent attempt claim"
    ;;
  *)
    usage
    ;;
