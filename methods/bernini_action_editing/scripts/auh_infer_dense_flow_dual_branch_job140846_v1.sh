...
decode_one() {
  ...
  local scale="$3"
  local scale_tag="$4"
  local preservation_checkpoint="$root/stage1/train_preservation_only_lr${lr_tag}_s160_r1/checkpoint-00000${step}"
  local output_dir="$root/stage1/infer/dual_p${lr_tag}_s${step}_k${scale_tag}_${row}_r1"
  local scratch="/tmp/dense-flow-dual-${lr_tag}-${step}-${scale_tag}-${row}-140846-${SLURM_STEP_ID}"
  test ! -e "$output_dir"
  ...
  "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
    ...
    --dense-flow-preservation-checkpoint "$preservation_checkpoint" \
    --dense-flow-preservation-scale "$scale" \
...
decode_one 1e4 020 0.50 050
decode_one 1e4 080 0.50 050
decode_one 1e4 160 0.50 050
decode_one 3e4 020 0.50 050
decode_one 3e4 080 0.50 050
decode_one 3e4 160 0.50 050
