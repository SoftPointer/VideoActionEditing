#!/usr/bin/bash
# Zero-science operational payload for an in-allocation retained-FD step.
...
readonly expected_parent_allocation_job_id=134936
...
step_launch_receipt="${SAIC_FV2_FD_CANARY_STEP_LAUNCH_RECEIPT:?}"
step_launch_receipt_device="${SAIC_FV2_FD_CANARY_STEP_LAUNCH_RECEIPT_DEVICE:?}"
step_launch_receipt_inode="${SAIC_FV2_FD_CANARY_STEP_LAUNCH_RECEIPT_INODE:?}"
...
compute_step_launch_receipt_device="${SAIC_FV2_FD_CANARY_COMPUTE_STEP_LAUNCH_RECEIPT_DEVICE:?}"
compute_step_launch_receipt_inode="${SAIC_FV2_FD_CANARY_COMPUTE_STEP_LAUNCH_RECEIPT_INODE:?}"
step_launch_receipt_fd_number="${SAIC_FV2_FD_CANARY_STEP_LAUNCH_RECEIPT_FD_NUMBER:?}"
stage0_guard_fd_number="${SAIC_FV2_FD_CANARY_STAGE0_GUARD_FD_NUMBER:?}"
stage0_wrapper_fd_number="${SAIC_FV2_FD_CANARY_STAGE0_WRAPPER_FD_NUMBER:?}"
...
stage0_python_fd_number="${SAIC_FV2_FD_CANARY_STAGE0_PYTHON_FD_NUMBER:?}"
...
for path in guard_fd_path payload_fd_path runtime source_archive python_bin output_parent \
  step_launch_receipt payload_release guard_release wrapper_path postflight \
...
[[ "${SLURM_JOB_ID:?}" == "${expected_parent_allocation_job_id}" && \
   "${SLURM_STEP_ID:?}" =~ ^[0-9]+$ && \
   "${SLURM_JOB_NODELIST:?}" == "${expected_node}" && \
   "${SLURMD_NODENAME:?}" == "${expected_node}" ]] || fail "Slurm step identity differs"
step_id="${SLURM_STEP_ID}"
job_step_id="${SLURM_JOB_ID}.${step_id}"
for identity_component in output_parent_device output_parent_inode \
  step_launch_receipt_device step_launch_receipt_inode \
  compute_output_parent_device compute_output_parent_inode \
  compute_step_launch_receipt_device compute_step_launch_receipt_inode; do
...
[[ "${step_launch_receipt_fd_number}" =~ ^[0-9]+$ && \
   "${step_launch_receipt_fd_number}" -ge 3 ]] || \
  fail "retained step_launch receipt fd number differs"
step_launch_receipt_fd_path="/proc/$$/fd/${step_launch_receipt_fd_number}"
for stage0_fd in stage0_guard_fd_number stage0_wrapper_fd_number \
  stage0_payload_fd_number stage0_probe_validator_fd_number \
  stage0_source_archive_fd_number stage0_receipt_fd_number \
  stage0_python_fd_number stage0_output_fd_number; do
  [[ "${!stage0_fd}" =~ ^[0-9]+$ && "${!stage0_fd}" -ge 3 ]] || \
    fail "${stage0_fd} differs"
done
[[ "${stage0_receipt_fd_number}" == "${step_launch_receipt_fd_number}" ]] || \
  fail "stage0 receipt fd binding differs"
[[ "$(printf '%s\n' "${stage0_guard_fd_number}" "${stage0_wrapper_fd_number}" \
     "${stage0_payload_fd_number}" "${stage0_probe_validator_fd_number}" \
     "${stage0_source_archive_fd_number}" "${stage0_receipt_fd_number}" \
     "${stage0_python_fd_number}" "${stage0_output_fd_number}" | \
     sort -u | wc -l | tr -d ' ')" == 8 ]] || \
...
stage0_python_fd_path="/proc/$$/fd/${stage0_python_fd_number}"
python_fd_exec_loader="${SAIC_FV2_FD_CANARY_PYTHON_FD_EXEC_LOADER:?}"
[[ "${python_fd_exec_loader}" == \
   'import os,sys;fd=int(sys.argv[1]);os.execve(fd,[sys.argv[2],*sys.argv[3:]],os.environ)' ]] || \
  fail "science Python retained-fd exec loader differs"
python_exec=(/usr/bin/python3 -I -B -c "${python_fd_exec_loader}" \
  "${stage0_python_fd_number}" "${python_bin}")
...
[[ -f "${step_launch_receipt}" && ! -L "${step_launch_receipt}" && \
   "$(stat -c '%d:%i:%h:%a' -- "${step_launch_receipt}")" == \
   "${compute_step_launch_receipt_device}:${compute_step_launch_receipt_inode}:1:444" && \
   "$(stat -Lc '%d:%i:%h:%a' -- "${step_launch_receipt_fd_path}")" == \
   "${compute_step_launch_receipt_device}:${compute_step_launch_receipt_inode}:1:444" && \
   "${compute_step_launch_receipt_inode}" == "${step_launch_receipt_inode}" ]] || \
  fail "step_launch receipt retained identity differs"
...
[[ -x "${stage0_python_fd_path}" && -f "${stage0_python_fd_path}" && \
   "$(stat -Lc '%h' -- "${stage0_python_fd_path}")" == 1 && \
   "$(sha256sum "${stage0_python_fd_path}" | awk '{print $1}')" == "${python_sha}" ]] || \
  fail "Python differs"
...
"${python_exec[@]}" -I -B - "${stage0_source_archive_fd_number}" \
...
env -u PYTHONPATH -u PYTHONHOME PYTHONNOUSERSITE=1 \
  "${python_exec[@]}" -I -B - "${runtime_import}" \
...
import contextlib, hashlib, importlib.util, io, json, os, stat, sys
...
with contextlib.redirect_stdout(help_stdout), contextlib.redirect_stderr(help_stderr):
    try:
        ...
        result = error.code
help_stdout_bytes = help_stdout.getvalue().encode("utf-8")
help_stderr_bytes = help_stderr.getvalue().encode("utf-8")
if (result not in (None, 0)
        or len(help_stdout_bytes) != 541
        or hashlib.sha256(help_stdout_bytes).hexdigest()
        != "816208624de9125d1ea49204a2fbaf07ce74ee4e798a87530c316400f000a692"
        or help_stderr_bytes != b""):
...
def retained_sha256(path):
    ...
    try:
        ...
        if (not stat.S_ISREG(before.st_mode) or before.st_nlink != 1
            ...
            raise SystemExit("runtime retained import origin differs")
        chunks = []
        while True:
            chunk = os.read(descriptor, 1024 * 1024)
            if not chunk:
        ...
        if (identity(before) != identity(after) or after.st_size != len(raw)
                or (after.st_dev, after.st_ino)
                != (leaf_after.st_dev, leaf_after.st_ino)):
            raise SystemExit("runtime retained import origin changed")
        return hashlib.sha256(raw).hexdigest()
    finally:
        os.close(descriptor)
rows = []
canonical_runtime_origin_verified = False
for module_name, imported in sorted(sys.modules.items()):
    ...
    try:
        ...
        continue
    observed_sha = retained_sha256(path)
    if module_name == canonical_module_name:
        if (path != runtime.resolve(strict=True)
                or relative != canonical_module_name + ".py"
                or observed_sha != runtime_sha):
            raise SystemExit("canonical runtime import origin differs")
...
if (not canonical_runtime_origin_verified
        or {row["module"] for row in rows} != expected_modules or len(rows) != 14
        or origin_sha != expected_origin_sha or len(raw_rows) != 2331
        or retained_sha256(runtime) != runtime_sha):
...
core = {
    ...
    "canonical_runtime_origin_path": "methods/bernini_action_editing/" + runtime.name,
...
run_group() (
  ...
  for ((ordinal=1; ordinal<=16; ordinal++)); do
    ...
    if env -u PYTHONPATH -u PYTHONHOME PYTHONNOUSERSITE=1 \
      "${python_exec[@]}" -I -B -m torch.distributed.run \
      --no-python --nproc-per-node=4 --max-restarts=0 \
    ...
    if [[ "${status}" != 0 ]]; then
      ...
      if "${python_exec[@]}" -B "${guard_fd_path}" admit-collision \
        --expected-runtime-sha256 "${runtime_sha}" --slurm-job-id "${SLURM_JOB_ID}" \
        --group-id "${group_id}" --candidate-index 0 --candidate-id "${candidate_id}" \
        --launch-ordinal "${ordinal}" --expected-rdzv-id "${rdzv_id}" \
        --claim-root "${claim_root}" --claim-root-identity "${claim_identity}" \
    ...
    "${python_exec[@]}" -B - "${guard_fd_path}" "${life}" "${claim_root}" \
...
if runtime_sha != "3372f1f48b9cb235d269ee6352ad4f289a6ee4a4a781c69ce0f7b1862ce77d36":
    raise SystemExit("fd canary runtime SHA argument differs")
...
runtime_origin = module.load_sealed(
    ...
    exact_fields={"schema_version", "status", "source_archive_sha256",
                  ...
                  "canonical_runtime_origin_verified",
                  "canonical_runtime_origin_sha256",
...
if (not isinstance(archive_binding["extracted_tree_manifest_sha256"], str)
        ...
        or runtime_origin["canonical_runtime_origin_verified"] is not True
        or runtime_origin["canonical_runtime_origin_sha256"] != runtime_sha
        or runtime_origin["canonical_runtime_origin_path"]
...
core = {
    ...
    "parent_allocation_job_id": job, "group_id": group, "candidate_index": 0,
    "candidate_id": candidate, "launch_ordinal": int(ordinal), "rdzv_id": rdzv_id,
    ...
    "canonical_runtime_origin_verified": True,
    "canonical_runtime_origin_sha256": runtime_origin[
        "canonical_runtime_origin_sha256"],
    "canonical_runtime_origin_path": runtime_origin[
        "canonical_runtime_origin_path"],
...
terminal_status=0
"${python_exec[@]}" -B - "${guard_fd_path}" "${output_root}" "${scratch}" \
  ...
  "${step_launch_receipt}" "${wrapper_sha}" \
  "${output_parent_device}" "${output_parent_inode}" \
  "${step_launch_receipt_device}" "${step_launch_receipt_inode}" \
  "${compute_output_parent_device}" "${compute_output_parent_inode}" \
  "${compute_step_launch_receipt_device}" "${compute_step_launch_receipt_inode}" \
  "${step_launch_receipt_fd_number}" \
...
job_step_id = f"{job}.{step_id}"
node = os.environ.get("SLURMD_NODENAME", "")
guard_retained, guard_decoy, payload_retained, payload_decoy = map(
    Path, sys.argv[10:14]
)
step_launch_receipt = Path(sys.argv[14])
wrapper_sha = sys.argv[15]
output_parent_identity = (int(sys.argv[16]), int(sys.argv[17]))
step_launch_receipt_identity = (int(sys.argv[18]), int(sys.argv[19]))
compute_output_parent_identity = (int(sys.argv[20]), int(sys.argv[21]))
compute_step_launch_receipt_identity = (int(sys.argv[22]), int(sys.argv[23]))
step_launch_receipt_fd_number = int(sys.argv[24])
...
authority = {"scientific": False, "generation": False, "training": False,
             "publication": False, "formal_job_authorized": False}
completion_fields = {
    "schema_version", "status", "parent_allocation_job_id", "group_id", "candidate_index",
    ...
    "canonical_runtime_origin_verified", "canonical_runtime_origin_sha256",
    "canonical_runtime_origin_path",
...
if (archive_binding.get("status") != "exact_formal_source_archive_extracted_and_verified"
        ...
        or runtime_origin.get("canonical_runtime_origin_verified") is not True
        or runtime_origin.get("canonical_runtime_origin_sha256") != runtime_sha
        or runtime_origin.get("canonical_runtime_origin_path")
        != "methods/bernini_action_editing/generate_saic_pure_t2v_event_bank_topup_v2.py"
...
for group in ("sp4-a", "sp4-b"):
    ...
    module._validate_generic_admission(claim, require_rank_packets=True)
    if (completion.get("group_id") != group or completion.get("parent_allocation_job_id") != job
            ...
            or completion.get("canonical_runtime_origin_verified") is not True
            or completion.get("canonical_runtime_origin_sha256") != runtime_sha
            or completion.get("canonical_runtime_origin_path")
            != "methods/bernini_action_editing/generate_saic_pure_t2v_event_bank_topup_v2.py"
    ...
    rows.append({
        ...
        "canonical_runtime_origin_verified": completion[
            "canonical_runtime_origin_verified"],
        "canonical_runtime_origin_sha256": completion[
            "canonical_runtime_origin_sha256"],
        "canonical_runtime_origin_path": completion[
            "canonical_runtime_origin_path"],
...
if step_launch_receipt_fd_number < 3:
    raise SystemExit("terminal retained step_launch fd number differs")
step_descriptor = os.dup(step_launch_receipt_fd_number)
try:
    before = os.fstat(step_descriptor)
    leaf = step_launch_receipt.lstat()
    if (not stat.S_ISREG(before.st_mode) or before.st_nlink != 1
            or stat.S_IMODE(before.st_mode) != 0o444
            or not stat.S_ISREG(leaf.st_mode) or stat.S_ISLNK(leaf.st_mode)
            or leaf.st_nlink != 1
            or (before.st_dev, before.st_ino) != compute_step_launch_receipt_identity
            or (leaf.st_dev, leaf.st_ino) != compute_step_launch_receipt_identity
            or before.st_ino != step_launch_receipt_identity[1]):
        raise SystemExit("terminal retained step_launch identity differs")
    os.lseek(step_descriptor, 0, os.SEEK_SET); chunks = []
    while True:
        chunk = os.read(step_descriptor, 65536)
        if not chunk: break
        chunks.append(chunk)
    step_launch_raw = b"".join(chunks)
    after = os.fstat(step_descriptor); leaf_after = step_launch_receipt.lstat()
    if ((after.st_dev, after.st_ino, after.st_size, after.st_mtime_ns)
            != (before.st_dev, before.st_ino, before.st_size, before.st_mtime_ns)
            or (leaf_after.st_dev, leaf_after.st_ino) != compute_step_launch_receipt_identity
            or after.st_size != len(step_launch_raw)):
        raise SystemExit("terminal retained step_launch changed")
finally:
    os.lseek(step_descriptor, 0, os.SEEK_SET)
    os.close(step_descriptor)
step_launch = module._decode_sealed(
    step_launch_raw,
    schema_version="saic-formal-v2-retained-fd-world8-inallocation-r14-step-launch-v1",
    exact_fields={"schema_version", "status", "parent_allocation_job_id",
                  "step_id", "job_step_id", "node", "exact_srun_argv",
                  "exact_srun_argv_digest", "output_parent_identity",
                  "log_directory_identity", "compute_bootstrap_sha256",
                  "compute_bootstrap_size_bytes",
                  "release_manifest",
                  "release_manifest_file_sha256", "release_manifest_digest",
                  "step_success", "parent_job_success", "bootstrap_boundary",
                  "authority", "receipt_digest"},
)
if (step_launch.get("status") != "compute_step_bootstrap_admitted"
        or step_launch.get("parent_allocation_job_id") != job
        or step_launch.get("step_id") != step_id
        or step_launch.get("job_step_id") != job_step_id
        or step_launch.get("node") != node
        or step_launch.get("output_parent_identity")
        != f"{output_parent_identity[0]}:{output_parent_identity[1]}"
        or not isinstance(step_launch.get("log_directory_identity"), str)
        or step_launch.get("step_success") is not None
        or step_launch.get("parent_job_success") is not None
        or step_launch.get("release_manifest") != release_manifest
        or step_launch.get("release_manifest_file_sha256") != release_manifest_sha
        or step_launch.get("release_manifest_digest") != release_manifest_digest
        or hashlib.sha256(module.canonical_json_bytes(
            step_launch["exact_srun_argv"]
        )).hexdigest()
        != step_launch.get("exact_srun_argv_digest")
        or step_launch.get("exact_srun_argv_digest")
        != os.environ.get("SAIC_FV2_FD_CANARY_EXACT_SRUN_ARGV_DIGEST")
        or step_launch.get("bootstrap_boundary") != {
        ...
        or step_launch.get("authority") != authority):
    raise SystemExit("terminal own step launch linkage differs")
...
core = {
    "schema_version": "saic-formal-v2-retained-fd-world8-inallocation-r14-operational-evidence-v1",
    "status": "retained_fd_operational_evidence_complete",
    "parent_allocation_job_id": job,
    "step_id": step_id,
    "job_step_id": job_step_id,
    "node": node,
    "exact_srun_argv_digest": step_launch["exact_srun_argv_digest"],
    "compute_bootstrap_sha256": step_launch["compute_bootstrap_sha256"],
    "compute_bootstrap_size_bytes": step_launch["compute_bootstrap_size_bytes"],
    "step_success": None,
    ...
    "runtime_origin_manifest_sha256": runtime_origin[
        "project_module_manifest_sha256"],
    "runtime_origin_receipt_digest": runtime_origin["receipt_digest"],
    "canonical_runtime_origin_verified": runtime_origin[
        "canonical_runtime_origin_verified"],
    "canonical_runtime_origin_sha256": runtime_origin[
        "canonical_runtime_origin_sha256"],
    "canonical_runtime_origin_path": runtime_origin[
    ...
    "external_step_postflight_admission_required": True,
    "step_launch_receipt_path": str(step_launch_receipt),
    "step_launch_receipt_file_sha256": hashlib.sha256(step_launch_raw).hexdigest(),
    "step_launch_receipt_digest": step_launch["receipt_digest"],
    "output_parent_identity": f"{output_parent_identity[0]}:{output_parent_identity[1]}",
    "step_launch_receipt_identity": (
        f"{step_launch_receipt_identity[0]}:{step_launch_receipt_identity[1]}"
    ),
    "compute_output_parent_identity": (
        f"{compute_output_parent_identity[0]}:{compute_output_parent_identity[1]}"
    ),
    "compute_step_launch_receipt_identity": (
        f"{compute_step_launch_receipt_identity[0]}:{compute_step_launch_receipt_identity[1]}"
    ),
    "step_launch_receipt_retained_fd_number": step_launch_receipt_fd_number,
    "step_launch_receipt_read_from_retained_fd": True,
    ...
    "stage0_inherited_source_fd_numbers": {
        ...
        "inallocation_step_wrapper": int(os.environ[
            "SAIC_FV2_FD_CANARY_STAGE0_WRAPPER_FD_NUMBER"
        ]),
        ...
        "science_python": int(os.environ[
            "SAIC_FV2_FD_CANARY_STAGE0_PYTHON_FD_NUMBER"
        ]),
        "source_archive": source_archive_fd_number,
        "step_launch_receipt": int(os.environ[
            "SAIC_FV2_FD_CANARY_STAGE0_RECEIPT_FD_NUMBER"
        ]),
    },
    "stage0_source_fds_distinct": True,
    "science_python_opened_o_nofollow_inside_step": True,
    "science_python_controller_invocations_executed_from_retained_fd": True,
    ...
    "probe_admission_binding": json.loads(os.environ[
        "SAIC_FV2_FD_CANARY_PROBE_ADMISSION_BINDING"
    ]),
    "scientific_generation_entered": False,
