...
readonly TRAIN_BUNDLE_SHA=c448bf751cf6158fe5abfa1bed1de6b9ef844eef1beb3f2fb2229946745c44de
...
case "$ARM" in
  direct_beta_vae)
    ...
    readonly OUTPUT="$BASE/runs/full_direct_beta_vae_55a4e6e1_v1"
    ;;
  deterministic_ae)
    readonly EXPECTED_JOB=143811
    readonly EXPECTED_HOST=auh7-1b-gpu-306
    readonly OUTPUT="$BASE/runs/full_deterministic_ae_55a4e6e1_v1"
    ;;
  *)
    echo "arm differs" >&2
    exit 64
...
umask 077
readonly CACHE_ROOT="/tmp/semantic-action-direct-full-$ARM-j${SLURM_JOB_ID}-s${SLURM_STEP_ID}"
mkdir "$CACHE_ROOT"
