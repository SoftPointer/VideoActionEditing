#!/usr/bin/env bash
# Create-once Stage-B target-T0 one-update canary launcher.
#
...
source_root="$experiment_root/source_stage_b_t0_v1"
method_root="$source_root/methods/bernini_action_editing"
authority_addendum="$source_root/stage_b_t0_single_update_authority_addendum.json"
stage_root="$experiment_root/stage_b_t0_v1"
log_root="$experiment_root/logs/stage_b_t0_v1"
...
usage() {
  cat >&2 <<'EOF'
usage:
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_v1.sh preflight
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_v1.sh launch [147881]
  STAGE_B_T0_AUTHORITY_SHA256=<64-hex> auh_stage_b_t0_single_update_20260824_v1.sh status
...
require_active_authority_contract() {
  ...
  test "$(authority_value '.runtime_paths.fresh_source_root_name')" = source_stage_b_t0_v1 \
    || fail "fresh source revision differs"
  test "$(authority_value '.runtime_paths.fresh_stage_root_name')" = stage_b_t0_v1 \
  ...
  require_source_pin methods/bernini_action_editing/scripts/auh_stage_b_t0_single_update_20260824_v1.sh
...
expected_node_for_job() {
  case "$1" in
    ...
    147873) printf '%s\n' auh7-1b-gpu-284 ;;
    147881) printf '%s\n' auh7-1b-gpu-213 ;;
    *) fail "parent job is not authorized for this canary: $1" ;;
  esac
}
...
launch_canary() {
  ...
  srun --jobid="$job" --exclusive --exact --nodelist="$node" --nodes=1 --ntasks=1 \
    ...
    "$method_root/scripts/auh_stage_b_t0_single_update_20260824_v1.sh" \
