#!/usr/bin/env bash
# Per-rank node-local caches for packed preservation v2.  HOME is deliberately
# left untouched; all task-specific mutable caches have dedicated variables.

set -Eeuo pipefail
umask 077

cache_token="${BERNINI_PRESV2_RANK_CACHE_TOKEN:?set rank cache token}"
...
[[ "${world_size}" == 8 && "${local_rank}" =~ ^[0-7]$ && "${global_rank}" == "${local_rank}" ]] || {
  echo "invalid WORLD8 rank identity" >&2; exit 2;
}
[[ "${cache_token}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$ ]] || {
  echo "invalid cache token" >&2; exit 2;
}
...
rank_root="$(mktemp -d -- "${scratch_real}/presv2-${job_id}-${step_id}-r${global_rank}.XXXXXXXX")"
...
python_bin="${BERNINI_PRESV2_PYTHON_BIN:?set frozen Python executable}"
