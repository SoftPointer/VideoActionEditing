...
# Event-01 hard gate for the corrected DynaEdit decomposition:
#   clean edit state = full source video;
#   explicit visual condition = source phase 0 repeated as a static carrier;
#   source/target velocity difference = noop vs requested action;
#   pure-T2V anchor = an online third forward, never an appearance target.
# Four mutually exclusive GPU groups test the outer solver, full-field action
# transport, attention correspondence transport, and a source-free field.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=STATICFIELD_SGA001C_NOANCHOR
    field_model=first_phase_source_rv2v
    transport=temporal_residual_velocity
    strength=0.50
    transport_steps=0
    blocks=8-21
    ;;
  auh7-1b-gpu-246:1)
    label=STATICFIELD_SGA001C_TRVEL_A050_EARLY8
    field_model=first_phase_source_rv2v
    transport=temporal_residual_velocity
    strength=0.50
    transport_steps=8
    ...
    ;;
  auh7-1b-gpu-247:0)
    label=STATICFIELD_SGA001C_CORRO_A050_EARLY3_B8_13
    field_model=first_phase_source_rv2v
    transport=temporal_correspondence_attn_output
    strength=0.50
    transport_steps=3
    blocks=8-13
    ;;
  auh7-1b-gpu-247:1)
    label=T2VFIELD_SGA001C_TRVEL_A050_EARLY8
...
output_root="$stage/anchor_first_phase_field_event01_v17"
...
scratch="/tmp/anchor-first-phase-$label-140846"
