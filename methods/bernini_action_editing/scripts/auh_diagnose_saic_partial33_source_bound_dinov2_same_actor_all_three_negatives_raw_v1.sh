#!/usr/bin/env bash
# Run r5 exact33 all-three same-actor raw diagnostics in one owned 8-GPU allocation.
...
readonly expected_attempts=/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_saic_v1_20260809/runs/t2v-events-topup-v2-db62886-r5/attempts
readonly expected_root_sha=d693d0784530f007888e2825d15db3db808fdf4f1d111b5d080d968c894ff145
readonly expected_source_manifest=/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_saic_v1_20260809/runs/t2v-events-topup-v2-db62886-r5/sealed-saic-source-manifest.json
readonly expected_source_manifest_sha=899b5a1dd66fc0bf6d4d0192fb6157f4afe691c50633246dddcaa1db2c2a98a9
readonly expected_legacy_sha=2f85ca48b9338920f050bc7b6bfe222b7e324087fc6ff99ff020a9a3c600092c
...
if [[ "$python_bin" != "$expected_python" || ! -x "$python_bin" || ! -f "$diagnostic_source" || -L "$diagnostic_source" ]]; then
  echo "python or diagnostic source is not a plain executable input" >&2
  exit 66
fi
if [[ "$attempts_root" != "$expected_attempts" || "$root_spec_sha256" != "$expected_root_sha" || "$source_manifest" != "$expected_source_manifest" || "$source_manifest_sha256" != "$expected_source_manifest_sha" ]]; then
  echo "r5 exact33 root/spec/source arguments differ" >&2
  exit 66
fi
if [[ "$legacy_cyclic_aggregate_sha256" != "$expected_legacy_sha" ]]; then
  echo "r5 exact33 legacy cyclic aggregate SHA argument differs" >&2
  exit 66
fi
...
if [[ "$(sha256sum "$legacy_cyclic_aggregate" | awk '{print $1}')" != "$expected_legacy_sha" ]]; then
  echo "r5 exact33 legacy cyclic aggregate file SHA differs" >&2
  exit 66
fi
if [[ ! -x "$portable_ffprobe" || -L "$portable_ffprobe" || "$(sha256sum "$portable_ffprobe" | awk '{print $1}')" != "$portable_ffprobe_sha" ]]; then
  echo "portable ffprobe identity differs" >&2
  exit 66
fi
export PATH="$portable_ffprobe_dir:/usr/bin:/bin"
if [[ "$(command -v ffprobe)" != "$portable_ffprobe" ]]; then
  echo "portable ffprobe is not PATH-first" >&2
  exit 66
fi
...
echo "r5 exact33 all-three same-actor DINO raw ${mode} complete: $output_root"
