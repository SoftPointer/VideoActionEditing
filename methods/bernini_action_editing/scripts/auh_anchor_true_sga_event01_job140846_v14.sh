...
# Full DynaEdit-style SGA+ANC: tau=.01 projected-clean selection followed by a
# variance-normalized weighted collapse of the five ANC noise chains.  This
# removes the historical guided-canary shortcut that always retained chain 0.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:1)
    label=SGA001C_RAW_TRO_A050_EARLY8; transport=temporal_residual_attn_output; strength=0.50 ;;
  auh7-1b-gpu-247:1)
    label=SGA001C_RAW_TRVEL_A025_EARLY8; transport=temporal_residual_velocity; strength=0.25 ;;
...
output_root="$stage/anchor_true_sga_event01_v14"
...
scratch="/tmp/anchor-true-sga-$label-140846"
