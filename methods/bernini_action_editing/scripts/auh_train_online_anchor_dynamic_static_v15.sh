...
if [ "$(hostname -s)" != "$expected_node" ]; then
  echo "v15 worker is restricted to $expected_node" >&2
  exit 3
fi
if [ "${SLURM_JOB_ID:-}" != "$expected_job" ]; then
  echo "v15 worker is restricted to allocation $expected_job" >&2
  exit 4
fi

experiment="${ONLINE_ANCHOR_EXPERIMENT:?unique v15 experiment label required}"
case "$experiment" in *[!A-Za-z0-9_.-]*|'') echo "invalid experiment label" >&2; exit 5 ;; esac
max_steps="${ONLINE_ANCHOR_MAX_STEPS:?2, 8, or 32 required}"
case "$max_steps" in 2|8|32) ;; *) echo "v15 permits 2, 8, or 32 steps" >&2; exit 6 ;; esac
...
method_archive="${ONLINE_ANCHOR_METHOD_ARCHIVE:-$release/online-anchor-dynamic-static-v15.method-source.tar}"
method_revision_file="${ONLINE_ANCHOR_METHOD_REVISION_FILE:-$release/online-anchor-dynamic-static-v15.method-source.revision}"
...
trainer="$source_tree/methods/bernini_action_editing/train_online_anchor_attention_dynamic_static_v15.py"
...
scratch="/tmp/online-anchor-dynamic-static-v15-${experiment}-${SLURM_JOB_ID}"
