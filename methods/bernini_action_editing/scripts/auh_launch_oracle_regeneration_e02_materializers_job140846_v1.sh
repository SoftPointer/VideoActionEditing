#!/usr/bin/env bash
# Launch only the two e02 source-only activation-v2 materializers inside allocation 140846.
# Nodes 248 and 279 are protected: pre-existing non-base steps are accepted only
# when every expanded host is node 246 or 247.
# This does not launch sampling, training, or an optimizer.

set -Eeuo pipefail

fail() {
  echo "[oracle-e02-materializer-outer] ERROR: $*" >&2
  exit 2
}

stage="/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/action_flow_noise_stage0_job140846_v1/stage1"
bundle="${stage}/oracle_activation_v2_materializer_r2_ef97259d_07b40cdd_5bba9f97_6da8c414"
run_root="${stage}/oracle_activation_v2_materializations_e02_only_ef97259d_07b40cdd_5bba9f97_r1"
...
[[ -z "${SLURM_JOB_ID:-}" && -z "${SLURM_JOBID:-}" && -z "${SLURM_STEP_ID:-}" ]] || \
  fail "outer must run on the login node, not inside any allocation step"
...
check_active_step_placement() {
  local step_rows step_id node_expression expanded_nodes node
  local saw_batch=0
  local saw_extern=0

  if ! step_rows="$(squeue -s -j 140846 -h -o '%i|%N')"; then
    fail "cannot query active allocation steps"
  fi
  while IFS='|' read -r step_id node_expression; do
    [[ -n "${step_id}" ]] || continue
    case "${step_id}" in
      140846.batch)
        saw_batch=1
      ...
      140846.extern)
        ...
        ;;
      140846.*)
        [[ -n "${node_expression}" && "${node_expression}" != "(null)" && \
          "${node_expression}" != "N/A" ]] || \
          fail "active non-base step has no auditable node placement: ${step_id}"
        if ! expanded_nodes="$(scontrol show hostnames "${node_expression}")"; then
          fail "cannot expand active-step node placement: ${step_id}:${node_expression}"
        fi
        [[ -n "${expanded_nodes}" ]] || \
          fail "active non-base step expands to no nodes: ${step_id}:${node_expression}"
        while read -r node; do
          [[ -n "${node}" ]] || continue
          case "${node}" in
            auh7-1b-gpu-246|auh7-1b-gpu-247) ;;
            auh7-1b-gpu-248|auh7-1b-gpu-279)
              fail "active non-base step touches a protected e02 node: ${step_id}:${node}"
              ;;
            *)
              fail "active non-base step is outside the permitted 246/247 set: ${step_id}:${node}"
              ;;
          esac
        done <<< "${expanded_nodes}"
        ;;
      *)
        fail "unexpected active allocation step id: ${step_id}"
        ;;
    esac
  done <<< "${step_rows}"
  (( saw_batch == 1 && saw_extern == 1 )) || \
    fail "allocation must retain both batch and extern base steps"
}

# Check before claiming an immutable run root, then check once more after every
# reservation and log exists and immediately before the first srun is spawned.
check_active_step_placement
[[ "${run_root}" != / && "$(realpath -m -- "${run_root}")" == "${run_root}" ]] || \
  fail "run root must be a canonical absolute path"
[[ -d "$(dirname -- "${run_root}")" && ! -L "$(dirname -- "${run_root}")" ]] || \
  fail "run root parent differs"
...
labels=(e02-vae e02-prompt)
nodes=(auh7-1b-gpu-248 auh7-1b-gpu-279)
kinds=(vae prompt)
cases=(e02 e02)
ports=(41823 41824)
...
for label in "${labels[@]}"; do
  ...
  [[ "$(stat -c '%F:%a' "${run_root}/reservations/${label}")" == "directory:700" ]] || \
    fail "reservation type or mode differs: ${label}"
  [[ -f "${run_root}/logs/${label}.log" && ! -L "${run_root}/logs/${label}.log" ]] || \
    fail "log type differs: ${label}"
  [[ "$(stat -c '%a:%h' "${run_root}/logs/${label}.log")" == "600:1" ]] || \
    fail "log mode or link count differs: ${label}"
done
set +o noclobber
...
for index in 0 1; do
  if ! wait "${pids[${index}]}"; then
    echo "[oracle-e02-materializer-outer] ${labels[${index}]} failed" >&2
    status=1
  fi
done
...
echo "[oracle-e02-materializer-outer] both e02 source-only materializers completed"
