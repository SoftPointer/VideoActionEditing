...
# Round-28 direct four-node payload.  Each node owns one four-rank SP4 model;
# the teacher branch uses a matched capped (latent sigma, model timestep) pair.
if [ "$#" -ne 0 ]; then
  echo "usage: $0" >&2
  exit 2
fi
...
stage="$root/stage1"
dev="$stage/anchor_sigma_cap_v28_9648a402_b97faa62"
...
require_sha256 "$dev/infer_anchor_sga_anc_event_v1.py" 9648a402a57b89708d325a0b4094dacf2590403d06ddb549a83704ecf2f94ed5
require_sha256 "$dev/anchor_sga_anc_controller.py" b97faa62d59faa387a371aae733f63c44f02a4a8007f1bf839c79013a4f6746f
...
require_sha256 "$runtime/guided_source_aligned_controller.py" 6d7528534bc27940422afae9c21dec7ac9d60970d765fc75896eb9c6326362a3
...
scratch="/tmp/dynaedit-anchor-sigma-cap-$label-140846-four-nodes"
