#!/bin/bash -p
# One-shot exact8 no-torch admission with a bounded receipt-visibility barrier.
...
readonly RECEIPT_VISIBILITY_TIMEOUT_SECONDS=30
readonly RECEIPT_VISIBILITY_POLL_SECONDS=0.05
...
set +o noclobber
set +e
/usr/bin/srun --jobid=143808 --job-name=case01-exact5-root-fake-v2 \
...
# The compute process can return before a newly created VAST child is visible
# through the controller's pre-srun directory descriptor.  Acquire the same
# parent again with no-follow directory semantics, prove that it is still the
# held parent object, and wait a bounded interval for a sealed canonical
# receipt.  Any failure exits here; the controller has no retry or second srun.
"/proc/self/fd/$ROOT_PYTHON_FD" -I -S -B - \
  "$EVIDENCE_DIR_FD" "$EVIDENCE_DIR" "$RECEIPT" \
  "$RECEIPT_VISIBILITY_TIMEOUT_SECONDS" \
  "$RECEIPT_VISIBILITY_POLL_SECONDS" 2012 2000 <<'PY'
...
def object_id(value): return (value.st_dev,value.st_ino,value.st_rdev)
def full_id(value): return (value.st_dev,value.st_ino,value.st_uid,value.st_gid,value.st_mode,value.st_nlink,value.st_rdev,value.st_size,getattr(value,"st_blocks",0),value.st_mtime_ns,value.st_ctime_ns)
def parent_contract(heldfd,freshfd,parent,uid,gid):
 held=os.fstat(heldfd);fresh=os.fstat(freshfd);named=os.lstat(parent)
 if os.path.realpath(parent)!=parent: raise RuntimeError("root-fake visibility parent is not canonical")
 for value in (held,fresh,named):
  if not stat.S_ISDIR(value.st_mode) or stat.S_IMODE(value.st_mode)!=0o755 or value.st_uid!=uid or value.st_gid!=gid or value.st_nlink<1: raise RuntimeError("root-fake visibility parent contract differs")
 if object_id(held)!=object_id(fresh) or object_id(held)!=object_id(named) or held.st_nlink!=fresh.st_nlink or held.st_nlink!=named.st_nlink: raise RuntimeError("root-fake visibility parent identity differs")
def pread(fd,size):
 out=[];offset=0
 while offset<size:
 ...
 return b"".join(out)
def receipt_once(parentfd,name,uid,gid):
 try: named_before=os.stat(name,dir_fd=parentfd,follow_symlinks=False)
 except FileNotFoundError: return None
 if not stat.S_ISREG(named_before.st_mode) or named_before.st_uid!=uid or named_before.st_gid!=gid or named_before.st_nlink!=1: raise RuntimeError("root-fake visibility receipt type/owner/link differs")
 if stat.S_IMODE(named_before.st_mode)!=0o400: return None
 try: fd=os.open(name,os.O_RDONLY|os.O_CLOEXEC|os.O_NOFOLLOW,dir_fd=parentfd)
 except FileNotFoundError: return None
 except OSError as error: raise RuntimeError("root-fake visibility receipt open differs") from error
 try:
  before=os.fstat(fd)
  if before.st_size<=0 or before.st_size>1048576: raise RuntimeError("root-fake visibility receipt size differs")
  raw=pread(fd,before.st_size);after=os.fstat(fd);named_after=os.stat(name,dir_fd=parentfd,follow_symlinks=False)
 finally: os.close(fd)
 for value in (before,after,named_after):
  if not stat.S_ISREG(value.st_mode) or value.st_uid!=uid or value.st_gid!=gid or value.st_nlink!=1: raise RuntimeError("root-fake visibility receipt authority differs")
 if object_id(before)!=object_id(after) or object_id(before)!=object_id(named_before) or object_id(before)!=object_id(named_after): raise RuntimeError("root-fake visibility receipt identity differs")
 if stat.S_IMODE(before.st_mode)!=0o400 or stat.S_IMODE(after.st_mode)!=0o400 or stat.S_IMODE(named_after.st_mode)!=0o400 or full_id(before)!=full_id(after) or full_id(before)!=full_id(named_after) or len(raw)!=before.st_size or not raw: return None
 try: value=json.loads(raw)
 except (UnicodeDecodeError,json.JSONDecodeError): return None
 if type(value) is not dict or raw!=canonical(value)+b"\n": return None
 return raw,full_id(before)
def acquire_parent(parent):
 try: return os.open(parent,os.O_RDONLY|os.O_CLOEXEC|os.O_DIRECTORY|os.O_NOFOLLOW)
 except OSError as error: raise RuntimeError("root-fake fresh visibility parent acquisition failed") from error
def wait_for_receipt(heldfd,parent,path,timeout,poll,uid,gid):
 if os.path.dirname(path)!=parent or os.path.basename(path) in ("",".",".."): raise RuntimeError("root-fake visibility target differs")
 if not (0.0<timeout<=60.0 and 0.0<poll<=min(1.0,timeout)): raise RuntimeError("root-fake visibility bound differs")
 deadline=time.monotonic()+timeout
 while True:
  if time.monotonic()>deadline: raise RuntimeError("root-fake receipt visibility timeout")
  freshfd=acquire_parent(parent)
  try:
   parent_contract(heldfd,freshfd,parent,uid,gid)
   ready=receipt_once(freshfd,os.path.basename(path),uid,gid)
   if ready is not None:
    parent_contract(heldfd,freshfd,parent,uid,gid)
    return ready
  finally: os.close(freshfd)
  remaining=deadline-time.monotonic()
  if remaining<=0: raise RuntimeError("root-fake receipt visibility timeout")
  time.sleep(min(poll,remaining))
if len(sys.argv)!=8: raise RuntimeError("root-fake visibility argv differs")
heldfd=int(sys.argv[1]);parent,path=sys.argv[2:4]
try: timeout=float(sys.argv[4]);poll=float(sys.argv[5]);uid=int(sys.argv[6]);gid=int(sys.argv[7])
except ValueError as error: raise RuntimeError("root-fake visibility argument differs") from error
raw,_=wait_for_receipt(heldfd,parent,path,timeout,poll,uid,gid)
print("CASE01_EXACT5_ROOT_FAKE_VISIBILITY_PASS "+hashlib.sha256(raw).hexdigest(),flush=True)
PY
...
def digest(v): return hashlib.sha256(canonical(v)).hexdigest()
def ident(v): return {"device":v.st_dev,"inode":v.st_ino,"uid":v.st_uid,"gid":v.st_gid,"mode":v.st_mode,"nlink":v.st_nlink,"rdev":v.st_rdev,"size":v.st_size,"blocks":getattr(v,"st_blocks",0),"mtime_ns":v.st_mtime_ns,"ctime_ns":v.st_ctime_ns}
def object_id(v): return (v.st_dev,v.st_ino,v.st_rdev)
...
root=os.path.dirname(os.path.dirname(package_path));post_evidencefd=os.open(root+"/evidence",os.O_RDONLY|os.O_CLOEXEC|os.O_DIRECTORY|os.O_NOFOLLOW);directory(evidencefd,root+"/evidence",0o755);directory(post_evidencefd,root+"/evidence",0o755);directory(logsfd,root+"/logs",0o755)
if object_id(os.fstat(evidencefd))!=object_id(os.fstat(post_evidencefd)) or os.fstat(evidencefd).st_nlink!=os.fstat(post_evidencefd).st_nlink: raise RuntimeError("post fresh evidence directory differs")
for path,mode in ((root,0o755),(root+"/authority",0o555),(root+"/diagnostics",0o755),(root+"/launch",0o555),(root+"/plan",0o555),(root+"/release",0o555),(root+"/outputs",0o755),(root+"/outputs/media",0o755),(root+"/final",0o755),(root+"/runtime",0o755)):
 named_directory(path,mode)
...
receipt_raw,receipt_identity=stable_at(post_evidencefd,os.path.dirname(receipt_path),receipt_path);receipt=strict(receipt_raw);unsigned=dict(receipt);claimed=unsigned.pop("receipt_digest",None)
attempt_raw,attempt_identity=stable_at(post_evidencefd,os.path.dirname(attempt_path),attempt_path);attempt=strict(attempt_raw);attempt_unsigned=dict(attempt);attempt_claimed=attempt_unsigned.pop("attempt_digest",None)
...
value["evidence_digest"]=digest(value);create(post_evidencefd,os.path.dirname(evidence_path),evidence_path,value);os.close(post_evidencefd);print("CASE01_EXACT5_ROOT_FAKE_CONTROLLER_PASS "+value["evidence_digest"])
PY
