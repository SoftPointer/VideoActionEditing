...
for gpu in "$gpu0" "$gpu1" "$gpu2" "$gpu3"; do
  case "$gpu" in 0|1|2|3|4|5|6|7) ;; *) echo "invalid v16r2 GPU device list" >&2; exit 2 ;; esac
done
if [ -n "${gpu_extra:-}" ] || [ "$gpu0" = "$gpu1" ] || [ "$gpu0" = "$gpu2" ] || \
   [ "$gpu0" = "$gpu3" ] || [ "$gpu1" = "$gpu2" ] || [ "$gpu1" = "$gpu3" ] || \
   [ "$gpu2" = "$gpu3" ]; then
  echo "v16r2 requires four distinct GPU device ordinals" >&2
  exit 2
fi
if [ "$(hostname -s)" != "$expected_node" ]; then
  echo "v16r2 worker is restricted to $expected_node" >&2
  exit 3
fi
if [ "${SLURM_JOB_ID:-}" != "$expected_job" ]; then
  echo "v16r2 worker is restricted to allocation $expected_job" >&2
  exit 4
fi
...
experiment="${ONLINE_ANCHOR_EXPERIMENT:?unique v16r2 experiment label required}"
...
case "$max_steps" in 644) ;; *) echo "v16r2 requires one continuous exact644 run" >&2; exit 6 ;; esac
...
scratch="/tmp/online-anchor-full644-dynamic-static-v16r2-${experiment}-${SLURM_JOB_ID}"
