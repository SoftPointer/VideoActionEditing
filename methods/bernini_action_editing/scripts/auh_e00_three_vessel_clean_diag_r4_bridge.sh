...
# R4 phase-capability compute bridge with package cache-bytecode checks.
if [ "$#" -ne 0 ]; then echo "usage: $0" >&2; exit 2; fi
: "${E00_R4_ARM_ROLE:?missing E00_R4_ARM_ROLE}"
: "${E00_R4_PHASE:?missing E00_R4_PHASE}"
: "${E00_R4_OUTPUT_ROOT:?missing E00_R4_OUTPUT_ROOT}"
: "${E00_R4_PACKAGE_MANIFEST:?missing E00_R4_PACKAGE_MANIFEST}"
: "${E00_R4_AUTHORIZATION:?missing E00_R4_AUTHORIZATION}"
: "${E00_R4_CAPABILITY_TOKEN:?missing E00_R4_CAPABILITY_TOKEN}"
...
test "$E00_R4_PACKAGE_MANIFEST" = "$manifest"
protocol="$method_root/assets/e00_three_vessel_clean_diag_r4_protocol_20260821.json"
wrapper="$method_root/e00_legacy_infer_fixed_rng_wrapper_r4.py"
...
for file in "$manifest" "$protocol" "$wrapper" "$validator" "$builder" "$infer" "$controller" "$qk_transport" "$E00_R4_AUTHORIZATION"; do
...
verify_sha "$builder" 9f3e51a68b2360a6a6f5c4e0d1b6107742e0c8b5acdd6e278760fd73f6beb08a
...
token_sha="$(printf '%s' "$E00_R4_CAPABILITY_TOKEN" | sha256sum)"; token_sha="${token_sha%% *}"
capability_args=(
  bridge-capability --protocol "$protocol" --phase "$E00_R4_PHASE"
  --arm-role "$E00_R4_ARM_ROLE" --package-manifest "$manifest"
  --authorization "$E00_R4_AUTHORIZATION" --capability-token-sha256 "$token_sha"
)
case "$E00_R4_PHASE" in
  A)
    test "$E00_R4_ARM_ROLE" = pure_noobserver_output_routeoff
    test -z "${E00_R4_PHASE_A_MARKER:-}"; test -z "${E00_R4_AB_GATE:-}"
    ;;
  B)
    test "$E00_R4_ARM_ROLE" = observer_matched_output_routeoff
    : "${E00_R4_PHASE_A_MARKER:?phase B requires E00_R4_PHASE_A_MARKER}"
    test -z "${E00_R4_AB_GATE:-}"
    capability_args+=(--phase-a-marker "$E00_R4_PHASE_A_MARKER")
    ;;
  C)
    test "$E00_R4_ARM_ROLE" = old_pureqk_temporal_routeon
    : "${E00_R4_PHASE_A_MARKER:?phase C requires E00_R4_PHASE_A_MARKER}"
    : "${E00_R4_AB_GATE:?phase C requires E00_R4_AB_GATE}"
    capability_args+=(--phase-a-marker "$E00_R4_PHASE_A_MARKER" --ab-gate "$E00_R4_AB_GATE")
    ;;
  *) echo "unknown R4 phase" >&2; exit 2 ;;
esac
...
label="$(jq -er --arg role "$E00_R4_ARM_ROLE" '.arms[]|select(.arm_role==$role)|.label' "$spec")"
transport="$(jq -er --arg role "$E00_R4_ARM_ROLE" '.arms[]|select(.arm_role==$role)|.legacy_transport' "$spec")"
transport_steps="$(jq -er --arg role "$E00_R4_ARM_ROLE" '.arms[]|select(.arm_role==$role)|.legacy_transport_steps' "$spec")"
if [ "$E00_R4_PHASE" = A ]; then anchor_video="$package_root/$placeholder_relative"; anchor_sha="$placeholder_sha"; else anchor_video="$action_anchor_video"; anchor_sha="$action_anchor_sha"; fi
verify_sha "$source_video" "$source_sha"; verify_sha "$anchor_video" "$anchor_sha"

output="$E00_R4_OUTPUT_ROOT/$label.mp4"
native_receipt="$output.receipt.json"
rng_prefix="$output.fixed-rng-r4"
audit="$output.e00-r4-arm-audit.json"
...
mkdir -p "$E00_R4_OUTPUT_ROOT"; test ! -L "$E00_R4_OUTPUT_ROOT"
scratch="/tmp/e00-clean-diag-r4-${E00_R4_PHASE}-${SLURM_STEP_ID:-nostep}"
...
"$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  "$wrapper" --wrapper-arm-role "$E00_R4_ARM_ROLE" --wrapper-audit-prefix "$rng_prefix" \
...
"$python_bin" -B "$validator" arm --protocol "$protocol" --arm-role "$E00_R4_ARM_ROLE" \
  ...
  --audit-output "$audit"
check_cache_free
printf 'E00_CLEAN_DIAG_R4_ARM_COMPLETE %s %s %s\n' "$E00_R4_ARM_ROLE" "$output" "$audit"
