#!/usr/bin/env bash
# Run the frozen r8 exact60 all-three source-bound raw diagnostic on owned GPUs.

set -euo pipefail
umask 077

if [[ "$#" -ne 21 ]]; then
  echo "usage: $0 <preflight|full> <allocation-job-id> <python-bin> <diagnostic.py> <diagnostic-sha256> <attempts-root> <root-spec-sha256> <source-manifest> <source-manifest-sha256> <terminal-evidence> <legacy-cyclic-aggregate> <legacy-cyclic-aggregate-sha256> <visual-checkpoint> <checkpoint-manifest> <evaluator-spec> <evaluator-spec-sha256> <visual-scorer.py> <visual-scorer-sha256> <visual-contract.py> <visual-contract-sha256> <fresh-output-root>" >&2
  exit 64
fi
...
terminal_evidence="${10}"; legacy_cyclic_aggregate="${11}"
legacy_cyclic_aggregate_sha256="${12}"; visual_checkpoint="${13}"
checkpoint_manifest="${14}"; evaluator_spec="${15}"
evaluator_spec_sha256="${16}"; visual_scorer_source="${17}"
visual_scorer_sha256="${18}"; visual_contract_source="${19}"
visual_contract_sha256="${20}"; output_root="${21}"
...
readonly expected_diagnostic_sha=442853d0b236672e9922c1d010f2f3d31691f98f121dcfb001e7de2ca0c9e072
...
readonly expected_legacy_cyclic_aggregate=/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/VideoEdit_experiments/bernini_saic_v1_20260809/diagnostics/allocation-134939-r8-sourcebound60-dinov2-full-28396417-r1/aggregate-receipt.json
readonly expected_legacy_cyclic_aggregate_sha=2e10fd8539d42aecb8872bba3e504e26d7e2dfb9a5120b1145080f8b463dc7fb
...
if [[ "$diagnostic_sha256" != "$expected_diagnostic_sha" ]]; then echo "r8 all-three diagnostic SHA argument differs" >&2; exit 66; fi
if [[ "$diagnostic_source" != /* ]]; then echo "diagnostic source must be an absolute path" >&2; exit 66; fi
cyclic_source="${diagnostic_source%/*}/diagnose_saic_r8_exact60_source_bound_dinov2_raw_v1.py"
if [[ ! -f "$cyclic_source" || -L "$cyclic_source" || "$(sha256sum "$cyclic_source" | awk '{print $1}')" != "$expected_cyclic_source_sha" ]]; then echo "frozen r8 cyclic source identity differs" >&2; exit 66; fi
...
if [[ "$legacy_cyclic_aggregate" != "$expected_legacy_cyclic_aggregate" || "$legacy_cyclic_aggregate_sha256" != "$expected_legacy_cyclic_aggregate_sha" ]]; then echo "frozen cyclic aggregate path/SHA differs" >&2; exit 66; fi
...
declare -a pids=() cache_paths=() cache_identities=()
runtime_scratch=""; runtime_scratch_identity=""; current_uid="$(id -u)"
...
remove_runtime_scratch_exact() {
  ...
  runtime_scratch=""; runtime_scratch_identity=""; cache_paths=(); cache_identities=()
}

cleanup_on_exit() {
  local status="$?" pid cleanup_failed=0
  trap - EXIT INT TERM HUP
  for pid in "${pids[@]}"; do if kill "$pid" 2>/dev/null; then wait "$pid" 2>/dev/null || true; fi; done
  pids=()
  if [[ -n "$runtime_scratch" ]] && ! remove_runtime_scratch_exact; then echo "unsafe rank-local cache cleanup refused; retained $runtime_scratch" >&2; cleanup_failed=1; fi
...
runtime_prefix="${scratch_parent%/}/saic-r8-exact60-all3-${allocation_job_id}-${mode}."
...
register_cache_directory() {
  ...
  cache_paths+=("$path"); cache_identities+=("$(stat -c '%d:%i' -- "$path")")
}

prepare_cache_tree() {
  local cache="$1" child
  register_cache_directory "$cache"
  for child in home xdg-config xdg-cache miopen-user miopen-custom tmp torch-extensions triton torchinductor pycache; do register_cache_directory "$cache/$child"; done
}

control_cache="$runtime_scratch/control"; prepare_cache_tree "$control_cache"
declare -a rank_caches=()
for rank in 0 1 2 3 4 5 6 7; do
  printf -v rank_label '%02d' "$rank"; rank_cache="$runtime_scratch/rank-$rank_label"
  prepare_cache_tree "$rank_cache"; rank_caches+=("$rank_cache")
done
validate_cache_roots || { echo "initial rank-local cache closure differs" >&2; exit 67; }

run_isolated() {
  local cache="$1" rocr_device="$2"; shift 2
  ...
  env -u HIP_VISIBLE_DEVICES -u CUDA_VISIBLE_DEVICES -u GPU_DEVICE_ORDINAL \
    ...
    TORCH_EXTENSIONS_DIR="$cache/torch-extensions" TRITON_CACHE_DIR="$cache/triton" \
    TORCHINDUCTOR_CACHE_DIR="$cache/torchinductor" PYTHONPYCACHEPREFIX="$cache/pycache" \
    ...
    MKL_NUM_THREADS=4 TOKENIZERS_PARALLELISM=false ROCR_VISIBLE_DEVICES="$rocr_device" "$@"
}
...
for rank in 0 1 2 3 4 5 6 7; do
  log_prefix="$output_root/${mode}-rank-$(printf '%02d' "$rank")"; rank_cache="${rank_caches[rank]}"
  if [[ "$mode" == "preflight" ]]; then
    run_isolated "$rank_cache" "$rank" "$python_bin" "$diagnostic_source" preflight "${common[@]}" --rank "$rank" >"$log_prefix.out" 2>"$log_prefix.err" &
  else
    run_isolated "$rank_cache" "$rank" "$python_bin" "$diagnostic_source" worker "${common[@]}" --rank "$rank" --world-size 8 >"$log_prefix.out" 2>"$log_prefix.err" &
  fi
  pids+=("$!")
done
failed=0
...
if [[ "$mode" == "full" ]]; then
  run_isolated "$control_cache" "" "$python_bin" "$diagnostic_source" aggregate \
    --input-manifest "$input_manifest" --expected-input-manifest-sha256 "$input_manifest_sha256" \
    --expected-source-sha256 "$diagnostic_sha256" --legacy-cyclic-aggregate "$legacy_cyclic_aggregate" \
    --expected-legacy-cyclic-aggregate-sha256 "$legacy_cyclic_aggregate_sha256" --output-root "$output_root"
fi
...
echo "r8 exact60 all-three source-bound DINO raw ${mode} complete: $output_root authority=zero"
