...
case "$(hostname -s)" in
  ...
  auh7-1b-gpu-247)
    ...
    ;;
  *) echo "source-correspondence training is restricted to Job 140846 nodes 246/247" >&2; exit 3 ;;
esac

corr_weight="${SOURCECORR_WEIGHT:-0.02}"
case "$corr_weight" in
  0.02) weight_tag=w020 ;;
  0.005) weight_tag=w005 ;;
  *) echo "SOURCECORR_WEIGHT must be 0.02 or 0.005" >&2; exit 4 ;;
esac
device_set="${ROCR_DEVICE_SET:-0,1,2,3}"
...
output="$release/train_sourcecorr_cross_s40_${tag}_${weight_tag}_q12x20_micro1_s64_v1"
archive="$release/method-source-sourcecorr-v4.tar"
revision_file="$release/method-source-sourcecorr-v4.revision"
...
scratch="/tmp/complex8-sourcecorr-${tag}-${weight_tag}-140846"
...
exec "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  ...
  --source-attention-correspondence-weight "$corr_weight" \
  --source-attention-correspondence-max-queries 128 \
  --learning-rate 1e-4 \
