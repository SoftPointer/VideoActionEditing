#!/usr/bin/env python3
"""Receipt-first package-root world4 admission controller (checked-in HOLD).
...
SCHEMA = "case01-object-trajectory-exact5-package-world4-controller-v2"
ATTEMPT_SCHEMA = SCHEMA + "-attempt"
COMPUTE_SCHEMA = SCHEMA + "-compute"
CONTROLLER_STATE = "READY_EXPLICIT_SINGLE_SRUN_PACKAGE_WORLD4_ADMISSION"
...
ATTEMPT_PATH = PACKAGE_ROOT / "evidence/exact5_world4_attempt_v2.json"
EVIDENCE_PATH = (
    PACKAGE_ROOT / "evidence/exact5_world4_controller_receipt_v2.json"
)
STDOUT_PATH = PACKAGE_ROOT / "logs/exact5_world4_srun_v2.stdout.log"
STDERR_PATH = PACKAGE_ROOT / "logs/exact5_world4_srun_v2.stderr.log"
STAGE_ROOT = Path(
...
def controller() -> dict[str, Any]:
    ...
    try:
        ...
        command = engine.build_srun_argv(plan_b64)
        if (
