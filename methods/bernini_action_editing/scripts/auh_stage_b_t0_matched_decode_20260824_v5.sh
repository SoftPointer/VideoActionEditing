...
source_root="$experiment_root/source_stage_b_t0_decode_v5"
...
decode_root="$experiment_root/stage_b_t0_retry8/matched_decode_v5/0be6494dfac3"
log_root="$experiment_root/logs/stage_b_t0_retry8/matched_decode_v5"
...
expected_runner_sha=667bd111855c8f45bc76ddfbea476aeef0ba081253c02ae3e0f5f5398a775405
expected_test_sha=8093b9f9a337897486fdca88fbed403185614e651f57e3ba3c3486cf7460c0db
...
fail() {
  printf '[stage-b-t0-decode-v5] ERROR: %s\n' "$*" >&2
  exit 1
}
...
run_worker() {
  ...
  scratch="/tmp/action-repr-t0-decode-v5-${step}-${route}-${job}-${step_token}"
  ...
  printf '[stage-b-t0-decode-v5] DECODE_PASS step=%s route=%s output=%s\n' "$step" "$route" "$output"
}
...
launch_decode() {
  ...
  printf '[stage-b-t0-decode-v5] srun complete job=%s step=%s route=%s\n' "$job" "$step" "$route"
}
...
case "$command" in
  preflight)
    [ "$#" -eq 0 ] || usage
    common_preflight
    echo '[stage-b-t0-decode-v5] PREFLIGHT_PASS WORLD4_SP4=true steps=40 matched=true pyav=true argparse_abbrev=false inference_audit=frozen receipt_base_identity=true'
    ;;
  decode)
    [ "$#" -eq 3 ] || usage
    launch_decode "$1" "$2" "$3"
  ...
  status)
    ...
    if [ ! -d "$decode_root" ]; then
      echo '[stage-b-t0-decode-v5] NOT_STARTED'
      exit 0
    fi
