#!/usr/bin/env bash
# BOX-EXP-009 confirmation action-anchor exact8 on retained 136309/gpu280.
#
# This launcher is generation-only.  It makes one disposable fit-r13 physical
# safe_open/r10-parity smoke and then four serial WORLD4 shards, two formal
# candidates per shard (action, incomplete).  It never invokes the optional
# 32-candidate diagnostic lane and never creates an optimizer.

set -Eeuo pipefail
umask 077

fail() { echo "[full30-confirmation8-v1] ERROR: $*" >&2; exit 2; }
...
readonly launch_confirmation=launch-approved-BOX-EXP-009-confirmation-action-anchor-exact8-136309
...
readonly controller="${method_root}/full30_action_confirmation8_controller_v1.py"
readonly plan_tool="${method_root}/full30_action_confirmation8_plan_v1.py"
readonly generator="${method_root}/full30_action_confirmation8_generator_v1.py"
readonly resource="${method_root}/tools/reserve4_fixed_generation_sp4_136309_specialized_v1.py"
readonly release_builder="${method_root}/tools/build_full30_action_confirmation8_release_v1.py"
readonly launcher="${method_root}/scripts/auh_full30_action_confirmation8_136309_world4_v1.sh"
...
[[ "$(sha256_file "${resource}")" == daa6cb2c128b689129d40fd0597e6e95ca5f3bd9fc5132abc70e0bcbe3cfb4af ]] || fail "136309 resource specialization differs"
...
if [[ "${role}" == child ]]; then
  ...
  run_admission_shard() {
    ...
    "${python_bin}" -B "${generator}" run-sp4 --lane admission \
...
[[ "${parent_state}" == "RUNNING|${holder_node}|guangyi.chen" ]] || fail "retained parent 136309/gpu280 differs"
...
printf '%s\n' \
  ...
  'optional_diagnostic_invocations=0' \
  'optional_diagnostic_is_admission_prerequisite=false' \
  ...
  'parent_136309_cancelled_released_or_requeued=false' >"${run_root}/launcher.status"
echo 'BOX_EXP_009_CONFIRMATION8_GENERATION_PENDING_REVIEW_AND_MATERIALIZER candidate_count=8 diagnostics=0 optimizer=false'
