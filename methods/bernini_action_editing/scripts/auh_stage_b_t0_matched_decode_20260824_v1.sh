...
source_root="$experiment_root/source_stage_b_t0_decode_v1"
...
decode_root="$experiment_root/stage_b_t0_retry8/matched_decode_v1/0be6494dfac3"
log_root="$experiment_root/logs/stage_b_t0_retry8/matched_decode_v1"
...
expected_runner_sha=985aeba10404112b6a5d584c9ac7cb3b8a3738c7efb82bea2b097909403ac864
expected_test_sha=a15a361d6dbd33fb97c00f79032d9eb15875597a1c8c41d4ceaedbeb7dd5341b
...
fail() {
  printf '[stage-b-t0-decode-v1] ERROR: %s\n' "$*" >&2
  exit 1
}
...
validate_output() {
  local output="$1" sidecar probe
  sidecar="$output.action-repr-t0.json"
  [ -s "$output" ] && [ -s "$output.receipt.json" ] && [ -s "$sidecar" ] \
    || fail 'matched decode output closure is incomplete'
  PYTHONPATH="$source_root:$method_root" "$python_bin" -B -c \
    'import json,sys; from infer_action_repr_target_t0_matched_decode_v1 import validate_decode_receipt; validate_decode_receipt(json.load(open(sys.argv[1], encoding="ascii")))' \
    "$sidecar"
  ...
  jq -e '.streams | length == 1 and .[0].width == 656 and .[0].height == 368 and .[0].r_frame_rate == "25/1" and .[0].nb_read_frames == "81"' \
    <<<"$probe" >/dev/null || fail 'matched decode video geometry differs'
}
...
run_worker() {
  ...
  [ ! -e "$output_dir" ] && [ ! -L "$output_dir" ] \
    || fail 'decode coordinate is permanently consumed; use a new decode revision'
  mkdir -p "$output_dir" "$log_root"
  output="$output_dir/output.mp4"
  log="$output_dir/run.log"
  (set -o noclobber; umask 077; : >"$log") 2>/dev/null \
    || fail 'create-only decode log already exists'
  step_token="${SLURM_STEP_ID//[^A-Za-z0-9_.-]/_}"
  scratch="/tmp/action-repr-t0-decode-v1-${step}-${route}-${job}-${step_token}"
  ...
  /usr/bin/timeout --signal=TERM --kill-after=60s 90m \
    ...
    >>"$log" 2>&1 || fail 'WORLD4 matched decode failed; preserve this coordinate'
  validate_output "$output"
  printf '[stage-b-t0-decode-v1] DECODE_PASS step=%s route=%s output=%s\n' "$step" "$route" "$output"
}
...
launch_decode() {
  ...
  (set -o noclobber; umask 077; : >"$outer") 2>/dev/null \
    || fail 'create-only outer log already exists'
  srun --jobid="$job" --exclusive --exact --nodelist="$node" --nodes=1 --ntasks=1 \
    --gres=gpu:mi210:4 --cpus-per-task=16 --mem=0 \
    "$0" worker "$job" "$step" "$route" >>"$outer" 2>&1 \
    || fail 'matched decode srun failed; preserve output/logs and use a new revision'
  validate_output "$decode_root/step000${step}/${route}/output.mp4"
  printf '[stage-b-t0-decode-v1] srun complete job=%s step=%s route=%s\n' "$job" "$step" "$route"
}
...
case "$command" in
  preflight)
    [ "$#" -eq 0 ] || usage
    common_preflight
    echo '[stage-b-t0-decode-v1] PREFLIGHT_PASS WORLD4_SP4=true steps=40 matched=true'
    ;;
  decode)
    [ "$#" -eq 3 ] || usage
    launch_decode "$1" "$2" "$3"
  ...
  status)
    ...
    if [ ! -d "$decode_root" ]; then
      echo '[stage-b-t0-decode-v1] NOT_STARTED'
      exit 0
    fi
