...
# Event-01 action-contrast round.  The pure-T2V anchor is queried with action
# and matched no-op captions at the same noised anchor state.  Their velocity
# difference is the routed action signal; absolute anchor denoising is never
# used as the edit target.  Active arms keep this route online for all 40 cells.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=CAPTIONSTATIC_SGA001C_NOANCHOR
    ...
    ;;
  auh7-1b-gpu-246:1)
    label=CAPTIONSTATIC_SGA001C_CONTRAST_A050_ALL40
    field_model=first_phase_caption_i2v
    transport=temporal_contrast_velocity
    strength=0.50
    transport_steps=40
    ...
    ;;
  auh7-1b-gpu-247:0)
    label=STATICFIELD_SGA001C_CONTRAST_A050_ALL40
    ...
    ;;
  auh7-1b-gpu-247:1)
    label=STATICFIELD_SGA001C_CORRO_A025_ALL40_B8_13
    field_model=first_phase_source_rv2v
...
output_root="$stage/anchor_action_contrast_event01_v18"
...
scratch="/tmp/anchor-action-contrast-$label-140846"
