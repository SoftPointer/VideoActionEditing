...
# Split source, target and self-generated-anchor T2V text roles.  The target
# field retains source appearance; only the independent anchor forward sees the
# anchor's yellow-overalls setup.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=T2V_ROLESPLIT_ANC1_NO_ANCHOR; steps=0 ;;
  auh7-1b-gpu-247:0)
    label=T2V_ROLESPLIT_TRQK_A010_EARLY3; steps=3 ;;
  *) echo "this canary uses GPU group 0 on Job 140846 nodes 246-247" >&2; exit 3 ;;
esac
...
output_root="$stage/anchor_t2v_role_split_event01_v9"
...
source_caption="An exactly 81-frame realistic continuous locked-camera video at 25 fps shows one young child in a pale blue dress walking slowly away from the camera along a white gravel garden path with large flat stepping stones. The child, pale blue clothing, every stone, trees, shrubs, houses, lighting, framing and camera remain unchanged."
target_caption="An exactly 81-frame realistic continuous locked-camera video at 25 fps shows the same young child in the same pale blue dress on the same white gravel garden path with the same large flat stepping stones, trees, shrubs and houses. $action $constraints"

scratch="/tmp/anchor-t2v-role-split-$label-140846"
...
exec "$python_bin" -B -m torch.distributed.run --standalone --nproc_per_node=4 \
  ...
  --arm AQK_ANC1 \
  --transport temporal_residual_qk \
  --transport-strength 0.10 \
  --transport-steps "$steps" \
  --blocks 8-21 \
  --field-model source_free_t2v \
