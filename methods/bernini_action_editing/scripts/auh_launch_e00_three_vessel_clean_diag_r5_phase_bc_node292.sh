...
# R5 BC launcher: B, current-artifact gate, then capability-revalidated C.
...
: "${E00_R5_PHASE_BC_AUTHORIZATION:?missing E00_R5_PHASE_BC_AUTHORIZATION}"
: "${E00_R5_PHASE_BC_TOKEN:?missing E00_R5_PHASE_BC_TOKEN}"
: "${E00_R5_OUTPUT_ROOT:?missing E00_R5_OUTPUT_ROOT}"
if [ "${ALLOW_E00_R5_PHASE_BC:-}" != I_REVIEWED_R5_A_AND_AUTHORIZE_B_GATE_C ]; then
  echo "R5 phase BC remains disabled" >&2; exit 3
fi
...
phase_a_marker="$E00_R5_OUTPUT_ROOT/E00_R5_PHASE_A_STOPPED_REVIEW_REQUIRED.json"

for command_name in squeue srun sha256sum jq find; do command -v "$command_name" >/dev/null; done
for file in "$manifest" "$review_marker" "$bootstrap_root" "$authority" "$protocol" "$base_spec" "$bridge" "$validator" "$builder" "$E00_R5_PHASE_BC_AUTHORIZATION" "$phase_a_marker"; do
...
token_sha="$(printf '%s' "$E00_R5_PHASE_BC_TOKEN" | sha256sum)"; token_sha="${token_sha%% *}"
"$python_bin" -B "$validator" bridge-capability --protocol "$protocol" \
  --phase B --arm-role "$role_b" --package-manifest "$manifest" \
  --authorization "$E00_R5_PHASE_BC_AUTHORIZATION" \
...
video_a="$E00_R5_OUTPUT_ROOT/$label_a.mp4"; audit_a="$video_a.e00-r5-arm-audit.json"
video_b="$E00_R5_OUTPUT_ROOT/$label_b.mp4"; audit_b="$video_b.e00-r5-arm-audit.json"
video_c="$E00_R5_OUTPUT_ROOT/$label_c.mp4"; audit_c="$video_c.e00-r5-arm-audit.json"
...
started="$E00_R5_OUTPUT_ROOT/e00-r5-phase-bc.started.json"
failed="$E00_R5_OUTPUT_ROOT/e00-r5-phase-bc.failed.json"
gate="$E00_R5_OUTPUT_ROOT/e00-r5-ab-current-bit-exact.gate.json"
final_audit="$E00_R5_OUTPUT_ROOT/e00-r5-final-current-artifact.audit.json"
completed="$E00_R5_OUTPUT_ROOT/E00_R5_ABC_DIAGNOSTIC_COMPLETE.json"
for path in "$started" "$failed" "$gate" "$final_audit" "$completed"; do test ! -e "$path"; test ! -L "$path"; done
lock="$E00_R5_OUTPUT_ROOT/.r5-phase-bc-lock"
cleanup_lock() { rmdir "$lock" 2>/dev/null || true; }
finalize() {
  status="$?"; trap - EXIT; cleanup_lock
  if [ "$status" -ne 0 ]; then
    if [ -f "$started" ]; then
      jq -S --argjson code "$status" '.state="bc_failed_c_not_admitted_or_incomplete"|.exit_code=$code' "$started" > "$failed"
    else
      jq -n -S --argjson code "$status" '{schema_version:"bernini-e00-clean-diagnostic-r5-phase-bc-failure-v5",state:"bc_failed_c_not_admitted_or_incomplete",exit_code:$code}' > "$failed"
    fi
  fi
  exit "$status"
}
...
jq -n -S --arg job "$job" --arg node "$node" --arg manifest_sha "$manifest_sha" \
  ...
  --arg role_b "$role_b" --arg role_c "$role_c" '
  {
    ...
    package_cache_bytecode_scan_passed:true,c_requires_bridge_revalidated_current_ab_gate:true
  }
' > "$started"
...
run_arm() {
  ...
  srun --jobid="$job" --exclusive --nodes=1 --ntasks=1 --cpus-per-task=64 \
    ...
    env E00_R5_ARM_ROLE="$role" E00_R5_PHASE="$phase" \
      E00_R5_OUTPUT_ROOT="$E00_R5_OUTPUT_ROOT" E00_R5_PACKAGE_MANIFEST="$manifest" \
      E00_R5_AUTHORIZATION="$E00_R5_PHASE_BC_AUTHORIZATION" \
      E00_R5_CAPABILITY_TOKEN="$E00_R5_PHASE_BC_TOKEN" \
      E00_R5_PHASE_A_MARKER="$phase_a_marker" "$@" bash "$bridge"
  check_cache_free
}
...
check_cache_free
run_arm C "$role_c" E00_R5_AB_GATE="$gate"
...
printf 'E00_R5_ABC_DIAGNOSTIC_COMPLETE %s\n' "$final_audit"
