...
# Round-26 four-node launcher payload.  Job 140846 has only 64 GiB host RAM
# per node, so each node runs exactly one SP4 model even though eight GPUs are
# reserved for the step.  This is a frozen-inference action-route canary, not
# optimizer training or evidence that the anchor representation is qualified.
if [ "$#" -ne 0 ]; then
  echo "usage: $0" >&2
  exit 2
fi
...
case "$(hostname -s)" in
  auh7-1b-gpu-246)
    label=CROSSATT_A003_EARLY8_B4_9
    strength=0.03
    steps=8
    blocks=4-9
    ;;
  auh7-1b-gpu-248)
    label=CROSSATT_A003_EARLY8_B10_15
    strength=0.03
    steps=8
    blocks=10-15
    ;;
  auh7-1b-gpu-279)
    label=CROSSATT_A006_EARLY8_B4_9
    strength=0.06
    steps=8
    blocks=4-9
    ;;
  auh7-1b-gpu-247)
    label=CROSSATT_A003_ALL40_B4_9
    strength=0.03
    steps=40
    blocks=4-9
    ;;
...
output_root="$stage/dynaedit_cross_attention_anchor_event01_v26"
...
require_sha256 "$dev/infer_anchor_sga_anc_event_v1.py" c6818f10a10e2c5b895d69f5c5a543e8c8490551a85c68d611f0f3bc70a07dfa
...
scratch="/tmp/dynaedit-cross-attn-$label-140846-four-nodes"
