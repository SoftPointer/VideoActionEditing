...
SCHEMA = "case01-object-trajectory-exact5-r5f-v4-composite-cpu-controller-v4"
...
CONTROLLER_STATE = "HOLD_PENDING_FRESH_CANARY_V3_CPU_V4_AUDIT"
...
ATTEMPT_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v3."
    "composite_cpu_admission_attempt_v4.json"
)
...
STDOUT_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v3."
    "composite_cpu_admission_srun_v4.stdout.log"
)
STDERR_PATH = EXPERIMENTS / (
    "bernini_case01_object_trajectory_exact5_r64_canary_v3."
    "composite_cpu_admission_srun_v4.stderr.log"
)
...
CPU_EVIDENCE_FIELDS = frozenset({
    ...
    "stderr_empty", "process_group_zero",
    "normal_exit_passive_grace_performed", "normal_exit_signal_sent",
    "terminal_pipes_closed", "launch_allowed",
...
if base_adapter_sha!="7b72e8dc88d95daa34d93604dddacf6dcf4f75a2f92f356f743183cf06fa7120" or module.BASE_ADAPTER_SHA256!=base_adapter_sha or module.base.__file__!=base_adapter_path or module.base.__cached__ is not None: raise RuntimeError("base adapter v3 authority differs")
...
def remove_tree(path):
 for current,directories,_files in os.walk(path,topdown=True,followlinks=False):
  ...
  for name in directories:
   ...
   if not os.path.islink(child): os.chmod(child,0o700)
 shutil.rmtree(path)
...
try:
 ...
 cache_root="/tmp/bernini-case01-object-trajectory-r5f-v4-composite-cpu-v4-job%s-step%s-cache"%(job_id,step_id)
 ...
 completed_cache_root=cache_root;remove_tree(completed_cache_root);cache_root=None
 ...
 if cache_root is not None:
  try:
   if os.path.lexists(cache_root): remove_tree(cache_root)
...
def build_srun_argv() -> list[str]:
    return [
        ...
        "--job-name=case01-object-r5f-v4-composite-cpu-v4",
...
def run_srun(
    command: Sequence[str], payload: bytes,
) -> tuple[int, bytes, bytes, int, dict[str, bool]]:
    ...
    passive_grace_performed = False
    naturally_absent = False
    if primary_error is None:
        if process.returncode is None:
            primary_error = CompositeCPUError(
                "CPU admission srun lacks terminal return code"
            )
        else:
            passive_grace_performed = True
            try:
                # Slurm may keep the step PGID visible briefly after the
                # terminal srun leader has been reaped.  This bounded normal
                # path is deliberately passive: no signal is sent.
                naturally_absent = _process_group_absent(
                    process_group, PROCESS_TERM_GRACE_SECONDS,
                )
            except BaseException as error:
                primary_error = error
            if primary_error is None and naturally_absent:
                try:
                    _close_process_pipes(process)
                except BaseException as error:
                    primary_error = error
                if primary_error is None:
                    if process.poll() is None:
                        raise CompositeCPUError(
                            "CPU admission direct child remains unreaped"
                        )
                    if any(
                        ...
                        raise CompositeCPUError(
                            "CPU admission terminal pipe closure differs"
                        )
                    return (
                        int(process.returncode), stdout, stderr, process_group,
                        {
    ...
    if primary_error is not None:
        raise primary_error
    if passive_grace_performed and not naturally_absent:
        raise CompositeCPUError(
            "terminal CPU admission process group required cleanup after passive grace"
        )
    raise CompositeCPUError("CPU admission terminal cleanup state differs")
...
def controller() -> dict[str, Any]:
    ...
    try:
        ...
        root.replay()
        (
            returncode, stdout, stderr, process_group, terminal_cleanup,
        ) = run_srun(command, payload)
        if (
            returncode != 0 or stderr != b"" or stdout.count(b"\n") != 1
            or terminal_cleanup != {
                "normal_exit_passive_grace_performed": True,
                "normal_exit_signal_sent": False,
                "terminal_pipes_closed": True,
        ...
        evidence = {
            ...
            "stderr_empty": True,
            "process_group_zero": terminal_cleanup["process_group_zero"],
            "normal_exit_passive_grace_performed": terminal_cleanup[
                "normal_exit_passive_grace_performed"
            ],
            "normal_exit_signal_sent": terminal_cleanup[
                "normal_exit_signal_sent"
            ],
            "terminal_pipes_closed": terminal_cleanup[
                "terminal_pipes_closed"
            ],
