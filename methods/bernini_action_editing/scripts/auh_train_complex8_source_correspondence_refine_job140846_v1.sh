...
case "$(hostname -s)" in
  ...
  auh7-1b-gpu-247)
    ...
    ;;
  *) echo "source-correspondence refinement is restricted to Job 140846 nodes 246/247" >&2; exit 3 ;;
esac
...
stage="$root/stage1"
source_tree="$stage/source-crossappearance-sourcecorr-v5refine"
...
output="$release/train_sourcecorr_refine_cross_s40_${tag}_${lr_tag}_q512_micro${micro_records}_s512_v1"
archive="$release/method-source-sourcecorr-v5refine.tar"
revision_file="$release/method-source-sourcecorr-v5refine.revision"
...
scratch="/tmp/complex8-sourcecorr-refine-${tag}-${lr_tag}-140846"
