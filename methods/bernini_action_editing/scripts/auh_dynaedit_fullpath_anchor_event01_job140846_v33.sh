...
# Structural canary after Round 32: keep the DynaEdit SGA/ANC outer solver,
# but require a real pure-T2V anchor query at every one of the 40 solver steps
# and every early SGA candidate. The matched residual-1.5% no-anchor result is
# the control, so all four slots are spent on independent block-level routes.
if [ "$#" -ne 1 ]; then echo "usage: $0 GPU_GROUP" >&2; exit 2; fi
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    export LABEL_OVERRIDE=FULLPATH_HARD_QK_B4_9
    ...
    ;;
  auh7-1b-gpu-246:1)
    export LABEL_OVERRIDE=FULLPATH_HARD_K_B4_9
    ...
    ;;
  auh7-1b-gpu-247:0)
    export LABEL_OVERRIDE=FULLPATH_DYNQK_A025_B4_9
    ...
    ;;
  auh7-1b-gpu-247:1)
    export LABEL_OVERRIDE=FULLPATH_DYNATTN_A010_B4_9
...
export OUTPUT_ROOT_OVERRIDE="$stage/dynaedit_fullpath_anchor_event01_v33"
