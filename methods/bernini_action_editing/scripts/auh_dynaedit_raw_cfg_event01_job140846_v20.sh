...
# DynaEdit's published source/target raw-velocity CFG pairs.  This is distinct
# from the earlier APG clean-prediction experiment and from scale-1 conditional
# fields.  Each CFG branch shares the same visual condition and query state.
...
case "$(hostname -s):$1" in
  auh7-1b-gpu-246:0)
    label=CAPTIONSTATIC_CFG25_45_SGA001C_NOANCHOR
    source_cfg=2.5
    target_cfg=4.5
    transport_steps=0
    ;;
  auh7-1b-gpu-246:1)
    label=CAPTIONSTATIC_CFG25_45_SGA001C_ATTCON_A025_ALL40
    source_cfg=2.5
    target_cfg=4.5
    transport_steps=40
    ;;
  auh7-1b-gpu-247:0)
    label=CAPTIONSTATIC_CFG45_85_SGA001C_NOANCHOR
    source_cfg=4.5
    target_cfg=8.5
    transport_steps=0
    ;;
  auh7-1b-gpu-247:1)
    label=CAPTIONSTATIC_CFG45_85_SGA001C_ATTCON_A025_ALL40
    source_cfg=4.5
    target_cfg=8.5
    transport_steps=40
    ;;
...
output_root="$stage/dynaedit_raw_cfg_event01_v20"
...
scratch="/tmp/dynaedit-raw-cfg-$label-140846"
