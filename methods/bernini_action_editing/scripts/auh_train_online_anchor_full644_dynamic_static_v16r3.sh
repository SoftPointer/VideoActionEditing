...
for gpu in "$gpu0" "$gpu1" "$gpu2" "$gpu3"; do
  case "$gpu" in 0|1|2|3|4|5|6|7) ;; *) echo "invalid v16r3 GPU device list" >&2; exit 2 ;; esac
done
if [ -n "${gpu_extra:-}" ] || [ "$gpu0" = "$gpu1" ] || [ "$gpu0" = "$gpu2" ] || \
   [ "$gpu0" = "$gpu3" ] || [ "$gpu1" = "$gpu2" ] || [ "$gpu1" = "$gpu3" ] || \
   [ "$gpu2" = "$gpu3" ]; then
  echo "v16r3 requires four distinct GPU device ordinals" >&2
  exit 2
fi
if [ "$(hostname -s)" != "$expected_node" ]; then
  echo "v16r3 worker is restricted to $expected_node" >&2
  exit 3
fi
if [ "${SLURM_JOB_ID:-}" != "$expected_job" ]; then
  echo "v16r3 worker is restricted to allocation $expected_job" >&2
  exit 4
fi
...
experiment="${ONLINE_ANCHOR_EXPERIMENT:?unique v16r3 experiment label required}"
...
case "$max_steps" in 644) ;; *) echo "v16r3 requires one continuous exact644 run" >&2; exit 6 ;; esac
...
method_prefix="$release/online-anchor-full644-dynamic-static-v16r3.method-source"
method_archive="${ONLINE_ANCHOR_METHOD_ARCHIVE:-$method_prefix.tar}"
method_revision_file="${ONLINE_ANCHOR_METHOD_REVISION_FILE:-$method_prefix.revision}"
method_checksum_file="${ONLINE_ANCHOR_METHOD_CHECKSUM_FILE:-$method_prefix.sha256}"
...
test -f "$trainer"
test ! -L "$trainer"
test -f "$source_tree/methods/bernini_action_editing/train_online_anchor_attention_full644_dynamic_static_v16r2.py"
...
test ! -L "$method_archive"
test -f "$method_revision_file"
test ! -L "$method_revision_file"
test -f "$method_checksum_file"
test ! -L "$method_checksum_file"
...
test "$(sha256sum "$method_checksum_file" | awk '{print $1}')" = "$method_revision"
(cd "$source_tree" && sha256sum -c "$method_checksum_file" >/dev/null)

scratch="/tmp/online-anchor-full644-dynamic-static-v16r3-${experiment}-${SLURM_JOB_ID}"
