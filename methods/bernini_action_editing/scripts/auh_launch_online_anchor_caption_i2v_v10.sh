...
archive="$release/online-anchor-caption-i2v-paired-delta-v10r2.tar"
revision="$release/online-anchor-caption-i2v-paired-delta-v10r2.revision"
log_root="$release/logs/caption_i2v_v10r2"
mkdir -p "$log_root"
test -f "$runner"
test -f "$authoring"
test -f "$archive"
...
launch() {
  ...
  local target_weight="$5" replay_weight="$6" max_steps="$7"
  ...
  srun --jobid="$job" --overlap --nodes=1 --ntasks=1 --nodelist="$node" \
    env \
      ...
      ONLINE_ANCHOR_REPLAY_WEIGHT="$replay_weight" \
...
if [ "${1:-}" = _one ]; then
  launch "$2" "$3" "$4" "$5" "$6" "$7" "$8"
  exit 0
fi

smoke=corr25_captioni2v_delta_tf025_replay025_s1_smoke_v10r2
launch 141620 auh7-1b-gpu-226 "$smoke" action_noop 0.25 0.25 1
...
jq -e '
  ...
  .training_contract.source_condition_is_repeated_source_owned_phase_zero == true and
...
launch_background() {
  local job="$1" node="$2" experiment="$3" profile="$4"
  local target_weight="$5" replay_weight="$6"
  nohup bash -lc "
    set -euo pipefail
    bash '$(realpath "$0")' _one '$job' '$node' '$experiment' '$profile' '$target_weight' '$replay_weight' 64
  " >"$log_root/controller_${experiment}_${node}.log" 2>&1 &
  echo "$! $job $node $experiment"
}

launch_background 143808 auh7-1b-gpu-233 corr25_captioni2v_delta_tf025_replay025_s64_v10r2 action_noop 0.25 0.25
launch_background 143808 auh7-1b-gpu-268 noanchor_captioni2v_delta_tf025_replay025_s64_v10r2 no_anchor 0.25 0.25
launch_background 143808 auh7-1b-gpu-315 corr25_captioni2v_delta_pure_replay025_s64_v10r2 action_noop 0.0 0.25
launch_background 141620 auh7-1b-gpu-226 corr25_captioni2v_delta_tf025_replay010_s64_v10r2 action_noop 0.25 0.10
