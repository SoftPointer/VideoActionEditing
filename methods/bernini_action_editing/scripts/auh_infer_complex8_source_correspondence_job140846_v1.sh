...
case "$(hostname -s)" in
  ...
  *) echo "source-correspondence inference is restricted to Job 140846 nodes 246/247" >&2; exit 3 ;;
esac

case "${SOURCECORR_WEIGHT:-}" in
  0.02) weight_tag=w020 ;;
  0.005) weight_tag=w005 ;;
  *) echo "SOURCECORR_WEIGHT must be 0.02 or 0.005" >&2; exit 4 ;;
esac
...
stage="$root/stage1"
source_tree="$stage/source-crossappearance-sourceattn-v4corr"
...
source_copy_root="$release/train_sourcecorr_cross_s40_${tag}_${weight_tag}_q12x20_micro1_s64_v1"
...
revision=$(tr -d '\n' < "$release/method-source-sourcecorr-v4.revision")
archive_sha=$(sha256sum "$release/method-source-sourcecorr-v4.tar" | awk '{print $1}')
...
run_one() {
  ...
  local output_dir="$release/infer_sourcecorr/$tag/$weight_tag/$label"
  local output="$output_dir/output.mp4"
  if [ -s "$output" ]; then
    echo "SKIP $tag $weight_tag $label"
    return
  fi
  test -f "$source_copy_checkpoint/adapter_model.safetensors"
  test ! -e "$output_dir"
  mkdir -p "$output_dir"
  local scratch="/tmp/complex8-sourcecorr-infer-${tag}-${weight_tag}-${label}-140846"
  ...
  echo "START $tag $weight_tag $label source_copy_step=$step scale=$scale"
  ...
  echo "DONE $tag $weight_tag $label"
}

# Constant seed/source/instruction.  The k050 sequence exposes early/mid/late
# training drift; s040_k100 isolates inference source-memory strength.
run_one 10 0.5
run_one 40 0.5
run_one 64 0.5
run_one 40 1.0
printf 'complete\n' > "$release/infer_sourcecorr/$tag/$weight_tag/SWEEP_COMPLETE"
