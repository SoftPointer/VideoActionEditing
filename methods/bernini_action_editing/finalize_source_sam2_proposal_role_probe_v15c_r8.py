#!/usr/bin/env python3
"""Independent evidence replay for the local-only v15c-r8 release.
...
RELEASE_SCHEMA = "bernini-source-object-proposal-role-v15c-r8-local-release"
REPLAY_SCHEMA = "bernini-source-object-proposal-role-v15c-r8-evidence-replay"
RELEASE_TAG = "v15c-r8-local-schema-replay"
RELEASE_RELATIVE_PATH = (
    "methods/bernini_action_editing/assets/"
    "e00_source_sam2_proposal_role_probe_v15c_r8_release.json"
)
EXPECTED_CORE_MEMBER_COUNT = 9
EXPECTED_SNAPSHOT_FILE_COUNT = 10
...
MODULE_RELATIVES = {
    "evidence": (
        ...
        "sam2_observer_evidence_v15c_r8.py"
    ),
    "materializer": (
        "methods/bernini_action_editing/"
        "materialize_source_sam2_proposal_tracks_v15c_r8.py"
    ),
    "core": (
        "methods/bernini_action_editing/"
    ...
    "runner": (
        "methods/bernini_action_editing/"
        "run_source_object_proposal_role_probe_v15c_r8.py"
    ),
    "postflight": (
        "methods/bernini_action_editing/"
        "postflight_source_sam2_proposal_role_probe_v15c_r8.py"
    ),
    "builder": (
        "methods/bernini_action_editing/tools/"
        "build_source_object_proposal_role_v15c_r8_review.py"
    ),
}
CONVENTIONAL_MODULE_NAMES = {
    "evidence": "sam2_observer_evidence_v15c_r8",
    "materializer": "materialize_source_sam2_proposal_tracks_v15c_r8",
    "core": "source_object_proposal_role_probe_v15c",
    "runner": "run_source_object_proposal_role_probe_v15c_r8",
    "postflight": "postflight_source_sam2_proposal_role_probe_v15c_r8",
    "builder": "v15c_r8_sealed_review_builder",
}
...
class FinalizeV15CR8Error(RuntimeError):
    """A release, tensor, replay, registry, or media-content gate differs."""


# Internal call sites retain the historical spelling so the large, audited
# media replay implementation remains byte-for-byte easy to compare with r7.
FinalizeV15CR7Error = FinalizeV15CR8Error
...
def _verify_manifest(value: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    ...
    if (
        ...
        or value.get("local_evidence_status") != "LOCAL_SCHEMA_UNAUDITED"
    ...
    required_modules.add(
        "methods/bernini_action_editing/"
        "finalize_source_sam2_proposal_role_probe_v15c_r8.py"
    )
    required_modules.add(
        "methods/bernini_action_editing/tools/v15c_r8_external_bootstrap.py"
    )
    required_modules.add(
        "methods/bernini_action_editing/assets/"
    ...
    if set(paths) != required_modules:
        raise FinalizeV15CR7Error("release does not pin the exact r8 closure")
...
def _expected_result(
    *,
    ...
    local_evidence_replay: Mapping[str, Any],
    ...
    replay["provenance"] = {
        ...
        "observer_evidence_file_sha256": file_sha256(
...
def _expected_postflight(
    *,
    ...
    # r8 executes the sealed pure replay function.  Constructing a dictionary
    # of all-true gates here (the r7 behavior) is forbidden.
    receipt = modules.postflight.replay_postflight(
        spec_path=paths["spec"],
        source_video=paths["source"],
        r6_receipt_path=paths["r6_receipt"],
        r6_tensors=paths["r6_tensors"],
        track_receipt_path=paths["track_receipt"],
        track_tensors=paths["track_tensors"],
        result_json=paths["result"],
    )
    if (
        receipt.get("mechanical_candidate_qualified")
        != result.get("mechanical_candidate_qualified")
        ...
        raise FinalizeV15CR7Error("pure postflight replay claim boundary differs")
    return receipt
...
def replay_observer_evidence(
    *,
    ...
    try:
        ...
        local_evidence_replay = modules.runner.validate_track_bundle(
            track_receipt,
            spec,
            paths["track_receipt"],
            paths["track_tensors"],
    ...
    expected_result, core_receipt = _expected_result(
        ...
        local_evidence_replay=local_evidence_replay,
        paths=paths,
    )
    ...
    return {
        ...
        "local_schema_replay_status": local_evidence_replay["status"],
        "local_schema_replay_sha256": local_evidence_replay["receipt_sha256"],
...
def replay_review_content(
    *,
    ...
    review = run_root / "review"
    replay_root = Path(tempfile.mkdtemp(prefix=".v15c-r8-media-replay-", dir=str(run_root)))
