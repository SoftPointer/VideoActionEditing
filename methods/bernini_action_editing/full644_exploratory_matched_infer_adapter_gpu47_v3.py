#!/usr/bin/env python3
"""GPU4-7 eval-only named-output adapter for retained-FD inference.
...
This GPU47 variant additionally fails before any site-package or Torch import
unless Linux ROCm isolation is expressed by the single lower-level mask
``ROCR_VISIBLE_DEVICES=4,5,6,7``.  The HIP/CUDA/OpenCL compatibility masks are
forbidden because ROCm processes them after the ROCR mask and can therefore
double-filter the already remapped logical device set.
"""
...
def validate_gpu47_mapping_environment(
    environment: Mapping[str, str] = os.environ,
) -> dict[str, Any]:
    """Bind the configured ROCr mask without inferring logical enumeration."""

    if (
        environment.get("ROCR_VISIBLE_DEVICES") != ROCR_PHYSICAL_GPU_MASK
        or any(key in environment for key in _SECONDARY_GPU_MASKS)
    ):
        raise MatchedInferAdapterError("GPU47 ROCr-only visibility differs")
    return {
...
_EARLY_GPU_MAPPING: dict[str, Any] | None = None
...
def run(
    ...
    *,
    ...
    gpu_visibility_contract: Mapping[str, Any],
    ...
    if dict(gpu_visibility_contract) != validate_gpu47_mapping_environment():
        raise MatchedInferAdapterError("GPU47 mapping authority changed")
    ...
    try:
        ...
        return {
            "schema_version": (
                "full644-exploratory-matched-infer-adapter-gpu47-v3"
            ),
            "return_code": 0,
            "rank": rank,
            "world_size": 4,
            ...
            "gpu_visibility_contract": dict(gpu_visibility_contract),
            "gpu_isolation_scope": (
        ...
        if dict(gpu_visibility_contract) != validate_gpu47_mapping_environment():
            raise MatchedInferAdapterError(
                "GPU47 mapping authority changed during inference"
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        _EARLY_GPU_MAPPING is None
        or _EARLY_GPU_MAPPING != validate_gpu47_mapping_environment()
    ):
        raise MatchedInferAdapterError("early GPU47 mapping disappeared")
    result = run(
        sys.argv[1:] if argv is None else argv,
        ffmpeg_authority=_EARLY_FFMPEG_AUTHORITY,
        publication_handoff=_EARLY_PUBLICATION_HANDOFF,
        gpu_visibility_contract=_EARLY_GPU_MAPPING,
    )
    print(
        json.dumps(
            result,
