#!/usr/bin/env python3
"""Exact33 diagnostic-only DINO evaluation for the historical SAIC r5 bank.
...
and schema namespace, and emits an exact33 aggregate with no scientific,
...
SCHEMA_VERSION = "bernini-saic-partial33-dinov2-temporal-diagnostic-v1"
...
Partial33DINOError = core.Partial28DINOError
...
def _configure_core() -> None:
    ...
    # Point self-authentication at this exact33 source while retaining the
...
def partition_indices(count: int, rank: int, world_size: int) -> tuple[int, ...]:
    ...
    if sizes != EXPECTED_PARTITION_SIZES:
        raise Partial33DINOError("partial33 partition sizes differ")
    return indices
...
def aggregate(args: Any) -> int:
    ...
    for rank in range(EXPECTED_WORLD_SIZE):
        ...
        for index, result in zip(checked["partition_indices"], checked["candidate_results"]):
            if index in by_index:
                raise Partial33DINOError("shard partition overlaps")
            by_index[index] = result
    if set(by_index) != set(range(EXPECTED_ATTEMPT_COUNT)):
        raise Partial33DINOError("shards do not cover exact partial33")
    ...
    if [row.get("candidate_id") for row in ordered] != expected_ids:
        raise Partial33DINOError("aggregate candidate order differs")
    unsigned = {
        ...
        "coverage": "exactly_once_complete_partial33",
