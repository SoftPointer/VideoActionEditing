#!/usr/bin/env python3
"""R5 receipt wrapper for one fixed-RNG E00 legacy inference arm."""
...
import e00_legacy_infer_fixed_rng_wrapper_r4 as fixed_rng_core


SCHEMA = "bernini-e00-legacy-fixed-rng-r5-receipt-v5"
...
class E00R5WrapperError(RuntimeError):
    pass
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if file_sha256(Path(legacy_audit.__file__).resolve()) != EXPECTED_LEGACY_AUDIT_SHA256:
        raise E00R5WrapperError("sealed legacy audit wrapper bytes differ")
    protocol_validator = importlib.import_module("validate_e00_three_vessel_clean_diag_r5")
    ...
    if protocol.get("revision_tag") != REVISION_TAG:
        raise E00R5WrapperError("R5 protocol revision differs")
    ...
    if world_size != 4 or rank not in range(4) or local_rank not in range(4):
        raise E00R5WrapperError("R5 diagnostic requires exactly four ranks")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 4:
        raise E00R5WrapperError("R5 diagnostic requires four visible GPUs")
    torch.cuda.set_device(local_rank)
    seed_rows = [row for row in protocol["fixed_initial_rng"]["per_rank"] if row.get("rank") == rank]
    if len(seed_rows) != 1:
        raise E00R5WrapperError("R5 rank seed lookup differs")
    ...
    if not callable(getattr(entrypoint, "main", None)):
        raise E00R5WrapperError("legacy entrypoint has no callable main")
    route_mode = {
    ...
    if return_code not in (None, 0):
        raise E00R5WrapperError(f"legacy entrypoint returned {return_code!r}")
    if [(row["master_seed"], row["step"], row["candidate"]) for row in noise_rows] != [(2027, step, 0) for step in range(40)]:
        raise E00R5WrapperError("runtime keyed-noise call closure differs")
    ...
    if route_audit["call_count"] != expected_route_calls:
        raise E00R5WrapperError("pure-QK route call closure differs")
    if not latent_audit or latent_audit.get("finite") is not True:
        raise E00R5WrapperError("predecode latent audit is absent or non-finite")
    native_proof = legacy_audit._native_adapter_proof(contract["output"]) if rank == 0 else None
    if rank == 0 and native_proof is None:
        raise E00R5WrapperError("rank-zero native receipt is absent")
    ...
    receipt: Mapping[str, Any] = {
        ...
        "legacy_abi": {
            ...
            "phase_a_placeholder_classification": (
                "content_static_lossy_decode_reencode_placeholder" if is_a else None
            ),
            ...
            "honest_control_name": (
                "pure no-observer route-off; content-static lossy frame-0 decode/re-encode placeholder, not bit-exact"
                if is_a
