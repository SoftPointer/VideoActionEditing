#!/usr/bin/env python3
"""Render disclosed retry seeds for failed prospective calibration anchors."""
...
FIT_RETRY_NAME = "run_prospective_forward_anchor_retry_v1.py"
FIT_RETRY_SHA256 = "955254dfc482b2c43e15140504eca74be9149cf42023bc5ffd47342e5b91cde8"
FIT_RETRY_PATH = METHOD_ROOT / FIT_RETRY_NAME
if (
    not FIT_RETRY_PATH.is_file()
    or FIT_RETRY_PATH.is_symlink()
    or hashlib.sha256(FIT_RETRY_PATH.read_bytes()).hexdigest() != FIT_RETRY_SHA256
):
    raise RuntimeError("pinned fit retry dependency differs")

import run_prospective_forward_anchor_retry_v1 as fit_retry  # noqa: E402


base = fit_retry.base
SCHEMA_VERSION = "bernini-prospective-calibration-anchor-retry-v1"
AUTHORITY = {
    "calibration_retry_generation_only": True,
    "decoded_review_required": True,
    "post_review_seed_expansion_disclosed": True,
    "representation_reselection_authorized": False,
    "threshold_freezing_authorized": False,
...
class CalibrationAnchorRetryError(RuntimeError):
    """Raised before an invalid calibration retry can be accepted."""
...
def retry_row(
    ...
    if type(source_id) is not str or len(source_id) != 16:
        raise CalibrationAnchorRetryError("source ID syntax differs")
    if type(retry_seed) is not int or not 0 <= retry_seed < 2**63:
        raise CalibrationAnchorRetryError("retry seed differs")
    rows = [
        dict(row)
        for row in manifest["entries"]
        if row["source_id"] == source_id
        and row["analysis_split"] == "calibration"
        and row["branch"] == "forward"
    ]
    if len(rows) != 2:
        raise CalibrationAnchorRetryError(
            "source does not have two registered calibration rows"
        )
    registered_seeds = sorted(row["seed"] for row in rows)
    if retry_seed in registered_seeds:
        raise CalibrationAnchorRetryError("retry seed aliases a registered seed")
    invariant = (
        "action_family", "analysis_split", "branch", "executor", "instruction",
        "instruction_utf8_sha256", "source_id", "source_video",
        "source_video_sha256",
    )
    template = rows[0]
    if any(row[key] != template[key] for row in rows[1:] for key in invariant):
        raise CalibrationAnchorRetryError(
            "registered calibration forward rows differ beyond seed"
        )
    template["seed"] = retry_seed
    template["entry_id"] = f"{source_id}-cal-retry-s{retry_seed}-forward"
...
def retry_receipt(
    *, manifest_sha256: str, manifest_digest: str, row: Mapping[str, Any],
    ...
    unsigned = {
        ...
        "status": "calibration_retry_rendered_decoded_review_pending",
        "manifest_sha256": manifest_sha256,
        "manifest_digest": manifest_digest,
        "analysis_split": "calibration",
...
def command_run(args: argparse.Namespace) -> int:
    manifest, row, registered = _inputs(args)
    ...
    if any(entries_root.iterdir()):
        raise CalibrationAnchorRetryError("entries root must be fresh")
    ...
    base._write_create_only(
        output_root / "calibration-anchor-retry.receipt.json", value
    )
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    observed = base._read_object(
        base._plain_file(
            output_root / "calibration-anchor-retry.receipt.json",
            label="calibration anchor retry receipt",
        ),
        label="calibration anchor retry receipt",
    )
    if observed != expected:
        raise CalibrationAnchorRetryError("calibration anchor retry receipt differs")
...
if __name__ == "__main__":
    ...
    except (CalibrationAnchorRetryError, base.FactorialBranchRunError) as error:
        print(f"[calibration-anchor-retry] ERROR: {error}", file=sys.stderr)
        raise SystemExit(2)
