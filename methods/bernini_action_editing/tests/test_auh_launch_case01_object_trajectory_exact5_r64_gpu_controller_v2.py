#!/usr/bin/env python3
"""Hostile tests for the receipt-gated exact5 GPU controller v2."""
...
CONTROLLER_PATH = ROOT / (
    ...
    "auh_launch_case01_object_trajectory_exact5_r64_gpu_once_v2.HOLD.py"
)
STATIC_PATH = ROOT / (
    "methods/bernini_action_editing/scripts/"
...
controller = load(CONTROLLER_PATH, "_case01_gpu_controller_v2_test")
...
class DummyGate:
    def __init__(self, report):
        self.values = {"materialization": report}
        self.closed = False
...
class GPUControllerSourceTests(unittest.TestCase):
    ...
    def test_ready_state_without_exact_token_still_blocks_before_io(self):
    ...
    def test_exact_twelve_downstream_final_pins_have_no_blockers(self):
        pins = controller.dynamic_pin_values()
        self.assertEqual(
            {
                key: pins[key] for key in (
                    "static_receipt_sha256", "static_receipt_size",
                    "static_receipt_digest", "root_fake_receipt_sha256",
                    "root_fake_receipt_size", "root_fake_receipt_digest",
                    "world4_receipt_sha256", "world4_receipt_size",
                    "world4_receipt_digest", "world4_evidence_sha256",
                    "world4_evidence_size", "world4_evidence_digest",
                )
            },
            {
...
class ProducerAlignmentTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.static = load(STATIC_PATH, "_case01_gpu_static_producer")
        cls.root_fake = load(ROOT_FAKE_PATH, "_case01_gpu_rootfake_producer")
        cls.world4 = load(WORLD4_PATH, "_case01_gpu_world4_producer")
        cls.world4_engine = load(
            WORLD4_ENGINE_PATH, "_case01_gpu_world4_engine",
        )

    def test_paths_align_with_all_three_producers(self):
        for producer in (self.static, self.root_fake, self.world4):
            self.assertEqual(controller.PACKAGE_ROOT, producer.PACKAGE_ROOT)
            self.assertEqual(
                controller.PACKAGE_PUBLICATION_RECEIPT_PATH,
                producer.PACKAGE_PUBLICATION_RECEIPT_PATH,
            )
            self.assertEqual(
                controller.MATERIALIZATION_REPORT_PATH,
                producer.MATERIALIZATION_REPORT_PATH,
            )
        self.assertEqual(
            controller.STATIC_RECEIPT_PATH, self.static.STATIC_OUTPUT_PATH,
        )
        self.assertEqual(
            controller.STATIC_RECEIPT_PATH, self.root_fake.STATIC_RECEIPT_PATH,
        )
        self.assertEqual(
            controller.STATIC_RECEIPT_PATH, self.world4.STATIC_RECEIPT_PATH,
        )
        self.assertEqual(
            controller.ROOT_FAKE_RECEIPT_PATH,
            self.root_fake.ROOT_FAKE_OUTPUT_PATH,
        )
        self.assertEqual(
            controller.ROOT_FAKE_RECEIPT_PATH,
            self.world4.ROOT_FAKE_RECEIPT_PATH,
        )
        self.assertEqual(controller.WORLD4_RECEIPT_PATH, self.world4.WORLD4_RECEIPT_PATH)
        self.assertEqual(controller.WORLD4_EVIDENCE_PATH, self.world4.EVIDENCE_PATH)
        self.assertEqual(
            controller.WORLD4_CONTROLLER_SCHEMA, self.world4.SCHEMA,
        )

    def test_schema_task_and_runtime_alignment(self):
        for producer in (self.static, self.root_fake, self.world4):
            self.assertEqual(controller.CAMPAIGN, producer.CAMPAIGN)
            self.assertEqual(controller.TASK_IDS, producer.TASK_IDS)
            self.assertEqual(controller.ARM_ORDER, producer.ARM_ORDER)
        self.assertEqual(controller.IDENTITY_ROLES, self.root_fake.IDENTITY_ROLES)
        self.assertEqual(controller.IDENTITY_ROLES, self.world4.IDENTITY_ROLES)
        self.assertEqual(
            controller.EXPECTED_TORCH_VERSION,
            self.world4_engine.EXPECTED_TORCH_VERSION,
        )
        self.assertEqual(
            controller.EXPECTED_HIP_VERSION,
            self.world4_engine.EXPECTED_HIP_VERSION,
        )
...
class TransportWidthTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.report_raw = REAL_MATERIALIZATION_PATH.read_bytes()
        cls.report = controller.validate_materialization_report(
            DummyAuthority(REAL_MATERIALIZATION_PATH, cls.report_raw),
        )

    def test_real_sealed_plan_input_and_named_hold_payload_lengths(self):
        self.assertEqual(len(self.report_raw), controller.MATERIALIZATION_REPORT_SIZE)
        self.assertEqual(
            hashlib.sha256(self.report_raw).hexdigest(),
            controller.MATERIALIZATION_REPORT_SHA256,
        )
        self.assertEqual(
            self.report["plan"],
            {
        ...
        launch_input = real_launch_input_fixture(self.report)
        launch_input_raw = controller.canonical(launch_input) + b"\n"
        self.assertEqual(len(launch_input_raw), controller.SEALED_LAUNCH_INPUT_SIZE)
        self.assertEqual(
            hashlib.sha256(launch_input_raw).hexdigest(),
            controller.SEALED_LAUNCH_INPUT_SHA256,
        )
    ...
    def test_complete_real_package_payload_has_large_transport_headroom(self):
        ...
        transport = controller.validate_srun_transport(
            controller.build_srun_argv(0), payload=payload, release=release,
        )
        self.assertEqual(transport["exact_srun_joined_width"], 284)
        self.assertEqual(transport["exact_srun_execve_argv_width"], 285)
        self.assertEqual(transport["exact_srun_environment_width"], 84)
        self.assertEqual(transport["exact_srun_execve_total_width"], 369)
        self.assertEqual(
            transport["exact_srun_execve_headroom"],
            controller.OBSERVED_AUH_ARG_MAX - 369,
        )
    ...
    def test_transport_rejects_argv_or_stdin_substitution_before_popen(self):
        command = controller.build_srun_argv(0)
        with self.assertRaisesRegex(
            controller.GPUControllerError, "exact fixed GPU srun argv differs",
        ):
            controller.validate_srun_transport(command + ["tampered"])
        with self.assertRaisesRegex(
...
'''
    ...
    def _binding_fixture():
        ...
        runtime = {
            ...
            "exact25_identity_set_digest": controller.object_digest(
                release["identities"]
            ),
        }
...
class OrderingAndCleanupTests(unittest.TestCase):
    def test_package_gate_opens_all_receipts_before_root(self):
        ...
        with mock.patch.object(
                ...
                controller, "validate_static_receipt", return_value={},
            ), mock.patch.object(
                controller, "validate_root_fake_receipt", return_value={},
            ), mock.patch.object(
                controller, "validate_world4", return_value=({}, {}),
            ):
        ...
        self.assertEqual(
            ...
            [
                ...
                "file:" + controller.STATIC_RECEIPT_PATH.name,
                "file:" + controller.ROOT_FAKE_RECEIPT_PATH.name,
                "file:" + controller.WORLD4_RECEIPT_PATH.name,
                "file:" + controller.WORLD4_EVIDENCE_PATH.name,
            ],
        )
        self.assertTrue(events[6].startswith("directory:"))
    ...
    def test_controller_mutation_order_attempt_plan_single_srun(self):
        ...
        def validate_transport(command, *, payload=None, release=None):
    ...
    def test_timeout_terminates_new_process_group_and_never_retries(self):
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(subprocess.TimeoutExpired):
                controller.run_single_srun(
                    controller.build_srun_argv(0), b"payload", {},
                )
