...
class Core4LabelAuthoringTests(unittest.TestCase):
    ...
    def test_author_is_uniquely_bound_to_d541801_v3(self) -> None:
        self.assertIn("d541801", author.TEMPLATE_SCHEMA)
        self.assertIn("d541801", author.LABEL_MANIFEST_SCHEMA)
        self.assertEqual(
            author.REQUIRED_SCORER_SOURCE_REVISION,
            "d541801a162796aacde34c2bfc2b1f0472d954d2",
        )
        self.assertEqual(
            author.REQUIRED_SCORER_SOURCE_SHA256,
            "3d7ce459ddb9a014873acd6384c7c4030b4e3aca9004c1b8486ebbc1f0f5d32e",
        )
        self.assertTrue(author.REQUIRED_SCORE_RECEIPT_SCHEMA.endswith("-v3"))
        self.assertTrue(author.REQUIRED_GROUP_RECEIPT_SCHEMA.endswith("-v3"))

    def test_runtime_rejects_later_scorer_even_with_same_module_name(self) -> None:
        fake_source = self.root / "score_pair_v5_t2v_energy_bank_v3.py"
        fake_source.write_text("# later scorer\n", encoding="ascii")
        ...
        with mock.patch.object(
            author.importlib, "import_module", return_value=fake_runtime
        ):
            with self.assertRaisesRegex(
                author.PairV5Core4LabelAuthoringError,
                "exact frozen d541801/v3 authority",
            ):
                author._scorer_runtime()
