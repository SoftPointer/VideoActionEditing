...
import full644_exploratory_matched_runner_v2 as runner
...
class MatchedRunnerV2Tests(unittest.TestCase):
    ...
    def test_constructor_closes_open_descriptor_if_second_open_fails(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            row = task(0, "base")
            row["output"]["video_path"] = str(output_root / "base.mp4")
            row["output"]["receipt_path"] = str(
                output_root / "base.mp4.receipt.json"
            )
            args = mock.Mock(
            ...
            with mock.patch.object(
                runner, "validate_task_order", return_value=[row]
            ), mock.patch.object(
                runner,
                "validate_exec_authority",
                return_value={
    ...
    def test_allocation_must_match_holder_node_and_step(self) -> None:
        ...
        with mock.patch.dict(
            ...
            runner._allocation_authority("141620", "auh7-1b-gpu-226", 4)
        for job, node in (("wrong", "auh7-1b-gpu-226"), ("141620", "wrong")):
            with self.subTest(job=job, node=node), mock.patch.dict(
                ...
                with self.assertRaises(runner.MatchedRunnerV2Error):
                    runner._allocation_authority("141620", node, 8)
        for hostile_step in (None, "0,1,2,3", "1,2,3,4,5,6,7,8", "0-3"):
