...
import diagnose_saic_partial47_source_bound_dinov2_same_actor_all_three_negatives_raw_v1 as diagnostic
...
def _synthetic_input_manifest() -> dict:
    source_order = [
        ...
        "human-0", "dog-1", "human-1", "dog-2", "human-2", "dog-3", "human-3",
    ]
    sources = {
        iid: {
            "iid": iid,
    ...
    executed_iids = [iid for iid in source_order if iid != diagnostic.EXPECTED_MISSING_CORRECT_SOURCE_IID]
    attempts = []
...
class Exact47AllThreeSameActorRawContractTest(unittest.TestCase):
    ...
    def test_pins_counts_policy_and_required_legacy_source(self) -> None:
        ...
        self.assertEqual(diagnostic.EXPECTED_CANDIDATE_NEGATIVE_PAIR_COUNT, 141)
        self.assertEqual(diagnostic.EXPECTED_REGISTERED_DIRECTED_SOURCE_PAIR_COUNT, 24)
        self.assertEqual(diagnostic.EXPECTED_EXECUTED_DIRECTED_SOURCE_PAIR_COUNT, 21)
        self.assertEqual(
            diagnostic.EXPECTED_LEGACY_CYCLIC_AGGREGATE_SHA256,
            "1eb8f31ca05c8d4b4e1008a995f69a36da7b1f8da69494479c5468ca79d79884",
        )
    ...
    def test_real_schema_manifest_roundtrip_distinguishes_21_executed_from_24_registered(self) -> None:
        ...
        self.assertEqual(len(loaded["attempts"]), 47)
        self.assertEqual(sum(len(row["negative_sources"]) for row in loaded["attempts"]), 141)
        self.assertEqual(len(correct_iids), 7)
        self.assertNotIn(diagnostic.EXPECTED_MISSING_CORRECT_SOURCE_IID, correct_iids)
        self.assertEqual(len(observed_pairs), 21)
    ...
    def test_exact8_cache_and_operational_fd_gap_are_explicit(self) -> None:
        ...
        self.assertIn("all_47_legacy_cyclic_raw_metrics_exact", source)
    ...
    def test_launcher_is_owned_exact8_and_requires_regression_input(self) -> None:
        launcher = (
            METHOD_ROOT
            / "scripts"
            / "auh_diagnose_saic_partial47_source_bound_dinov2_same_actor_all_three_negatives_raw_v1.sh"
        ).read_text("utf-8")
