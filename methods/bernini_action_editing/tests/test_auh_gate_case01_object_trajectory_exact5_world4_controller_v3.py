#!/usr/bin/env python3
"""Hostile tests for the independent package world4 v3 HOLD controller."""
...
CONTROLLER_PATH = METHOD_ROOT / (
    "scripts/auh_gate_case01_object_trajectory_exact5_world4_once_v3.HOLD.py"
)
READY_CONTROLLER_PATH = METHOD_ROOT / (
    "scripts/auh_gate_case01_object_trajectory_exact5_world4_once_v3.READY.py"
)
V2_HOLD_PATH = METHOD_ROOT / (
    "scripts/auh_gate_case01_object_trajectory_exact5_world4_once_v2.HOLD.py"
)
V2_READY_PATH = METHOD_ROOT / (
...
V2_HOLD_SHA256 = "4215b495d400b9c2af565f07ad776c01093f09aaefc2357404dd7e73102ac9e4"
V2_HOLD_SIZE = 57_483
V2_READY_SHA256 = "7484c02823194f0dee96dde53f06d2469ee6efc6e7ba43126dbfa0a1211fa3c5"
V2_READY_SIZE = 57_484
HOLD_STATE = "HOLD_PENDING_V3_WIDE_PLAN_ARGV_REVIEW"
READY_STATE = "READY_EXPLICIT_SINGLE_SRUN_PACKAGE_WORLD4_ADMISSION_V3"
HOLD_SHA256 = "d408a7f1a447a2a1943bbf795bb88e7818096eb78c18c5027842c955f516ad77"
HOLD_SIZE = 60_398
READY_SHA256 = "6100191e2bd154c02c811cee10b3417c0cec3697a63c199912bebc3823476923"
READY_SIZE = 60_415
READY_AUTHORIZATION_TOKEN = (
    "918483109c52fe9f7cbf60c2729a4f8b0781c036cb5301d2c9b79b8cabcc7f84"
)
...
def real_package_gate_evidence() -> dict[str, object]:
    pins = controller.dynamic_pin_values()
    return {
        "schema_version": controller.SCHEMA + "-package-gate",
        "package_root": str(controller.PACKAGE_ROOT),
        "package_root_identity": list(controller.PACKAGE_ROOT_IDENTITY),
        "publication": {
            "path": str(controller.PACKAGE_PUBLICATION_RECEIPT_PATH),
            "sha256": pins["package_publication_receipt_sha256"],
            "size": pins["package_publication_receipt_size"],
            "receipt_digest": pins["package_publication_receipt_digest"],
        },
        "materialization": {
            "path": str(controller.MATERIALIZATION_REPORT_PATH),
            "sha256": pins["materialization_report_sha256"],
            "size": pins["materialization_report_size"],
            "receipt_digest": pins["materialization_report_digest"],
        },
        "static": {
            "path": str(controller.STATIC_RECEIPT_PATH),
            "sha256": pins["static_receipt_sha256"],
            "size": pins["static_receipt_size"],
            "receipt_digest": pins["static_receipt_digest"],
        },
        "root_fake": {
            "path": str(controller.ROOT_FAKE_RECEIPT_PATH),
            "sha256": pins["root_fake_receipt_sha256"],
            "size": pins["root_fake_receipt_size"],
            "receipt_digest": pins["root_fake_receipt_digest"],
        },
        "project_authorities": {
            role: {
                "path": str(controller.PACKAGE_ROOT / row["relative"]),
...
class PackageWorld4ControllerV3Test(unittest.TestCase):
    def test_v3_ready_is_exactly_one_frozen_state_assignment(self) -> None:
        hold = CONTROLLER_PATH.read_bytes()
        ready = READY_CONTROLLER_PATH.read_bytes()
        self.assertEqual(
            (hashlib.sha256(hold).hexdigest(), len(hold)),
            (HOLD_SHA256, HOLD_SIZE),
        )
        self.assertEqual(
            (hashlib.sha256(ready).hexdigest(), len(ready)),
        ...
        self.assertEqual(
            ready, hold.replace(HOLD_ASSIGNMENT, READY_ASSIGNMENT, 1),
        )
        differing = [
            (before, after)
        ...
        self.assertEqual(
            ready_module.authorization_token(), READY_AUTHORIZATION_TOKEN,
        )

    def test_v2_frozen_bytes_are_unchanged_and_v3_is_independent_hold(self) -> None:
        v2_hold = V2_HOLD_PATH.read_bytes()
        v2_ready = V2_READY_PATH.read_bytes()
        self.assertEqual(
            (hashlib.sha256(v2_hold).hexdigest(), len(v2_hold)),
            (V2_HOLD_SHA256, V2_HOLD_SIZE),
        )
        self.assertEqual(
            (hashlib.sha256(v2_ready).hexdigest(), len(v2_ready)),
            (V2_READY_SHA256, V2_READY_SIZE),
        )
        self.assertEqual(controller.CONTROLLER_STATE, HOLD_STATE)
        self.assertEqual(controller.READY_STATE, READY_STATE)
        self.assertNotEqual(controller.CONTROLLER_STATE, controller.READY_STATE)
        self.assertEqual(
            controller.SCHEMA,
    ...
    def test_source_proves_receipt_first_attempt_before_single_srun(self) -> None:
        ...
        build = body.index("command = build_package_srun_argv(plan_b64)")
        ...
        self.assertLess(fresh, build)
        self.assertLess(build, attempt)
        ...
        self.assertNotIn("engine.build_srun_argv", body)
    ...
    def test_real_package_plan_passes_v3_width_after_v2_pre_attempt_refusal(self) -> None:
        engine = load(ENGINE_PATH)
        controller.configure_engine(engine, lambda: None)
        plan = engine.build_compute_plan(real_package_gate_evidence())
        plan_raw = engine.canonical(plan)
        transport = base64.b64encode(plan_raw).decode("ascii")
        with self.assertRaisesRegex(
            engine.CpuAdmissionError, "exact srun argv exceeds admitted width",
        ):
            engine.build_srun_argv(transport)
        argv = controller.build_package_srun_argv(transport)
        self.assertEqual(len(plan_raw), 8_299)
        self.assertEqual(len(transport.encode("ascii")), 11_068)
        self.assertEqual(len(" ".join(argv).encode("ascii")), 11_553)
        ...
        self.assertEqual(argv, [
            "/usr/bin/srun", "--jobid=143808",
            "--job-name=case01-object-world4-cpu-v1",
            "--nodes=1", "--ntasks=1", "--nodelist=auh7-1b-gpu-292",
            "--cpus-per-task=16", "--mem=32G", "--gres=none", "--overlap",
            "--exact", "--kill-on-bad-exit=1", "--immediate=10",
            export_argument, "--time=00:10:00", str(controller.VACE_PYTHON),
            "-I", "-B", "-", "compute", transport,
        ])
        ...
        self.assertEqual(controller.MAX_SRUN_TRANSPORT_BYTES, 32_768)
        self.assertEqual(controller.OBSERVED_AUH_ARG_MAX, 2_097_152)
        self.assertLess(
            controller.MAX_SRUN_TRANSPORT_BYTES,
            controller.OBSERVED_AUH_ARG_MAX,
        )

    def test_v3_rejects_transport_or_joined_width_at_32768_before_attempt(self) -> None:
        with self.assertRaisesRegex(
            controller.PackageWorld4Error,
            "compute plan transport exceeds v3 bound",
        ):
            controller.build_package_srun_argv("A" * 32_768)
        small = controller.build_package_srun_argv("A")
        fixed_width = len(" ".join(small).encode("ascii")) - 1
        joined_limit_payload = controller.MAX_SRUN_TRANSPORT_BYTES - fixed_width
        self.assertGreater(joined_limit_payload, 0)
        self.assertLess(joined_limit_payload, controller.MAX_SRUN_TRANSPORT_BYTES)
        accepted = controller.build_package_srun_argv(
            "A" * (joined_limit_payload - 1)
        )
        self.assertEqual(
            len(" ".join(accepted).encode("ascii")),
            controller.MAX_SRUN_TRANSPORT_BYTES - 1,
        )
        with self.assertRaisesRegex(
            controller.PackageWorld4Error, "exact srun argv exceeds v3 bound",
        ):
            controller.build_package_srun_argv("A" * joined_limit_payload)

    def test_v2_absence_is_documentary_only_and_v3_names_are_fresh(self) -> None:
        self.assertEqual(len(controller.V2_REFUSAL_ABSENCE_EVIDENCE_PATHS), 6)
        source = CONTROLLER_PATH.read_text(encoding="utf-8")
        controller_body = source[source.index("def controller()") :]
        self.assertNotIn("V2_REFUSAL_ABSENCE_EVIDENCE_PATHS", controller_body)
        self.assertNotIn("_require_v2_refusal_absence", source)
        self.assertIn("production evidence, not a runtime dependency", source)
        self.assertEqual(controller.WORLD4_RECEIPT_PATH.name,
                         "exact5_world4_receipt_v3.json")
        self.assertEqual(controller.ATTEMPT_PATH.name,
                         "exact5_world4_attempt_v3.json")
        self.assertEqual(controller.EVIDENCE_PATH.name,
                         "exact5_world4_controller_receipt_v3.json")
        self.assertEqual(controller.STDOUT_PATH.name,
                         "exact5_world4_srun_v3.stdout.log")
        self.assertEqual(controller.STDERR_PATH.name,
                         "exact5_world4_srun_v3.stderr.log")
        self.assertTrue(controller.STAGE_ROOT.name.endswith("node292-v3"))
        self.assertTrue(CONTROLLER_PATH.name.endswith("once_v3.HOLD.py"))
