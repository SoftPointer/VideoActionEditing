...
class R5DSpooledLauncherTests(unittest.TestCase):
    def fixture(self, root: Path) -> tuple[dict[str, str], dict[str, str]]:
        ...
        for index, role in enumerate(launcher.EXPECTED_STATIC_SHA256):
            if role == "adapter":
                ...
                path.write_bytes(f"r5d-{role}-{index}\n".encode("utf-8"))
                path.chmod(0o444)
            files[role] = str(path)
        ...
        ffmpeg_path.write_bytes(b"r5d fixture ffmpeg\n")
        ffmpeg_path.chmod(0o555)
        plan_path = (root / "plan.json").resolve()
        plan_path.write_bytes(b'{"r5d_fixture":true}\n')
    ...
    def test_exact_source_pin_cascade_uses_only_new_adapter_bytes(self) -> None:
        ...
        self.assertNotEqual(
            launcher.R5D_ADAPTER_SHA256, launcher.BASE_ADAPTER_SHA256
        )

    def test_schema_transform_is_exact_and_leaves_r5c_module_on_disk(self) -> None:
        self.assertEqual(
            launcher.SCHEMA,
            "full644-exploratory-matched-root-launch-release-auh-r5d",
        )
        self.assertEqual(
            launcher.INPUT_SCHEMA,
            "full644-exploratory-matched-root-launch-input-auh-r5d",
        )
        self.assertEqual(
            launcher.RECEIPT_SCHEMA,
            "full644-exploratory-matched-root-launch-receipt-auh-r5d",
        )
        ...
        self.assertIn("r5d exact16 release identity closure differs", launcher.ROOT_BOOTSTRAP)
        compile(launcher.ROOT_BOOTSTRAP, "<r5d-root-bootstrap>", "exec")
        self.assertEqual(
            _sha(MODULE_ROOT / "full644_exploratory_matched_spooled_launcher_auh_r5.py"),
            launcher.BASE_LAUNCHER_SHA256,
        )

    def test_release_binds_launcher_to_runner_to_bridge_to_r5d_adapter(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw).resolve(strict=True)
            value, expected = self.fixture(root)
            self.assertEqual(
                expected["adapter"], launcher.R5D_ADAPTER_SHA256
            )
    ...
    def test_old_wrapper_replacement_is_rejected_by_fresh_r5d_pin(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw).resolve(strict=True)
            value, expected = self.fixture(root)
            expected["adapter"] = launcher.R5D_ADAPTER_SHA256
    ...
    def test_materialize_emits_only_r5d_schemas_and_is_create_only(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw).resolve(strict=True)
            value, expected = self.fixture(root)
            launch_input = root / "launch-input.json"
            ...
            runtime = subprocess.run(
                [
                    ...
                    "print(pathlib.Path(r5d.base.__file__).name); "
                    "print(r5d.BASE_ADAPTER_SHA256)",
                ],
                check=False,
                capture_output=True,
                text=True,
