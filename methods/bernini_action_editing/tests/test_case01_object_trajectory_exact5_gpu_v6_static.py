#!/usr/bin/env python3
"""Static boundary tests for the fresh exact5 GPU-v6 HOLD controller."""
...
controller = load(V6_HOLD, "_case01_gpu_v6_static")
ready_controller = load(V6_READY, "_case01_gpu_v6_ready_static")


class GPUV6StaticTests(unittest.TestCase):
    def test_old_authorities_unchanged_and_v6_ready_is_state_only(self):
        self.assertEqual(
        ...
        hold_raw = V6_HOLD.read_bytes()
        ready_raw = V6_READY.read_bytes()
        self.assertEqual(len(hold_raw), 213_522)
        self.assertEqual(hashlib.sha256(hold_raw).hexdigest(),
                         "63a0de7f6d0ff558057609210adefc02885b3c600257f2deb9264c5aeb797921")
        self.assertEqual(len(ready_raw), 213_528)
    ...
    def test_fresh_producer_and_output_namespace_is_exact(self):
        ...
        self.assertTrue(all("v6" in path.name for path in outer_paths))
    ...
    def test_source_and_embedded_payloads_compile_without_asserts(self):
        source = V6_HOLD.read_text(encoding="utf-8")
        ready_source = V6_READY.read_text(encoding="utf-8")
        tree = ast.parse(source, str(V6_HOLD))
        self.assertFalse(any(isinstance(node, ast.Assert) for node in ast.walk(tree)))
        ready_tree = ast.parse(ready_source, str(V6_READY))
        self.assertFalse(any(
            isinstance(node, ast.Assert) for node in ast.walk(ready_tree)
        ))
        for optimize in (0, 1, 2):
            compile(source, str(V6_HOLD), "exec", optimize=optimize)
            compile(ready_source, str(V6_READY), "exec", optimize=optimize)
            compile(controller.ROOT_BOOTSTRAP, "<gpu-v6-root>", "exec",
                    optimize=optimize)
        for symbol in (
