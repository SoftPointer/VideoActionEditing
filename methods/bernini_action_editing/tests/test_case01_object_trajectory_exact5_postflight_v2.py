...
from methods.bernini_action_editing.tools import (
    build_case01_object_trajectory_exact5_html_v2 as html_builder,
)
from methods.bernini_action_editing.tools import (
    case01_object_trajectory_exact5_postflight_v2 as postflight,
)
...
POSTFLIGHT_SCRIPT = (
    ...
    "case01_object_trajectory_exact5_postflight_v2.py"
)
HTML_SCRIPT = (
    REPO_ROOT
    / "methods/bernini_action_editing/tools/"
    "build_case01_object_trajectory_exact5_html_v2.py"
)
        ...
        producer.update(
            {
                **{field: str(path) for field, path in copied_sources.items()},
                ...
                "method_source_revision": "postflight-real-core4-fixture",
...
class ObjectTrajectoryExact5PostflightTests(unittest.TestCase):
    ...
    def test_complete_five_arm_run_produces_and_revalidates_exact_bundle(self) -> None:
        ...
        self.assertEqual(
            bundle["manifest"]["evaluation_authority"],
            {
                ...
                "runner": {
                    "role": "exact5-fresh-core4-runner",
                    "sha256": postflight.EXPECTED_RUNNER_SHA256,
                    "size": postflight.EXPECTED_RUNNER_SIZE,
                },
                "composite": {
                    "role": "exact5-fresh-core4-composite",
                    "sha256": postflight.EXPECTED_COMPOSITE_SHA256,
                    "size": postflight.EXPECTED_COMPOSITE_SIZE,
                },
                "base_adapter": {
                    "role": "exact5-fresh-core4-base-adapter",
                    "sha256": postflight.EXPECTED_BASE_ADAPTER_SHA256,
                    "size": postflight.EXPECTED_BASE_ADAPTER_SIZE,
                },
                "object_wrapper_inner": {
                    "role": "exact5-source-loaded-object-wrapper-inner",
                    "sha256": postflight.EXPECTED_OBJECT_WRAPPER_INNER_SHA256,
                    "size": postflight.EXPECTED_OBJECT_WRAPPER_INNER_SIZE,
                },
                "strict_eval": {
    ...
    def test_local_core4_authority_and_native_replay_reject_core3_tuple(self) -> None:
        authority = postflight._evaluation_authority()
        self.assertEqual(
            set(authority),
            {
                "trajectory_eval", "runner", "composite", "base_adapter",
                "object_wrapper_inner", "strict_eval",
            },
        )
        self.assertEqual(
            (
                authority["trajectory_eval"]["sha256"],
                authority["trajectory_eval"]["size"],
                authority["runner"]["sha256"],
                authority["runner"]["size"],
                authority["composite"]["sha256"],
                authority["composite"]["size"],
                authority["base_adapter"]["sha256"],
                authority["base_adapter"]["size"],
                authority["object_wrapper_inner"]["sha256"],
                authority["object_wrapper_inner"]["size"],
            ),
            (
                "381ba375147bec7580b451226b07b3d1cab9125866978602de05fbba4f16aaa3",
                116_371,
        ...
        retired_local_pins = (
            ...
            (
                "EXPECTED_BASE_ADAPTER_SHA256",
                "53b75aea4897a0ec5ad70c8ea2b2dd314b93d1331cf5e41d65c3b51339f4d4ca",
            ),
        )
        for field, retired_sha256 in retired_local_pins:
            with self.subTest(local_pin=field), mock.patch.object(
                postflight, field, retired_sha256
            ):
                with self.assertRaisesRegex(
                    postflight.PostflightError, "SHA-256 differs"
                ):
                    postflight._evaluation_authority()

        old_plan = copy.deepcopy(self.fixture.plan)
        old_plan["producer"]["inference_wrapper_sha256"] = (
            "b30bba5c9cd233d412ffd88d8413311e9ffbb79d3ddf69aaf6eb2ee96183b489"
        )
        _seal(old_plan, "plan_digest")
        with self.assertRaises(postflight.trajectory_eval.ObjectTrajectoryEvalError):
            postflight.trajectory_eval.validate_plan(
                old_plan,
        ...
        old_physical = copy.deepcopy(
            self.fixture.attestation["physical_bindings"]
        )
        old_physical["identities"]["runner"].update(
            {
                "sha256": "02207e64a129444b26adf8bd92307102c4a91e85d2a029fa60030a7e9e6f45c8",
                "size": 21_716,
            }
        )
        old_physical["captured_runner_entry"].update(
            {
                "runner_sha256": old_physical["identities"]["runner"]["sha256"],
            }
        )
        old_physical["captured_runner_entry"]["runner_identity"]["size"] = 21_716
        _seal(old_physical["captured_runner_entry"], "authority_digest")
        _seal(old_physical, "physical_bindings_digest")
        with self.assertRaisesRegex(
            postflight.PostflightError, "physical source pin differs: runner"
        ):
            postflight._validate_physical_bindings(
                old_physical,
                plan=self.fixture.plan,
                plan_sha256=self.fixture.attestation["plan"]["sha256"],
            )
    ...
    def test_retired_upstream_abis_are_rejected_by_real_v4_replay(self) -> None:
        evaluator = postflight.trajectory_eval
        for old_schema in (
