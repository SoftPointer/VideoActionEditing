...
READY_PATH = ROOT / (
    "methods/bernini_action_editing/scripts/"
    "auh_gate_case01_object_trajectory_exact5_r5f_v4_"
    "composite_cpu_once_v2.READY.py"
)
...
class CompositeCPUGateV4Tests(unittest.TestCase):
    @classmethod
    ...
    def test_ready_is_one_exact_state_line_diff_with_predicted_authority(self):
        old = 'CONTROLLER_STATE = "HOLD_PENDING_FRESH_CANARY_V3_CPU_AUDIT"'
        ...
        self.assertEqual(len(ready_raw), 89357)
        self.assertEqual(
            hashlib.sha256(ready_raw).hexdigest(),
            "a833effb4d3e9a6c3bbc343526ca7e32f010fe46835611a6222bd4538d8bae6c",
        )
        self.assertEqual(
            self.ready.authorization_token(),
            "e6a40305ddea36fc95368ea6c518ab7eb8245851db724b94bd80f1af303431f6",
        )
    ...
    def test_fresh_names_and_cache_are_disjoint_from_gpu_production(self):
        ...
        self.assertTrue(all("canary_v3" in path and "composite_cpu" in path
                            for path in paths))
        self.assertTrue(all("_v2." in path for path in paths))
