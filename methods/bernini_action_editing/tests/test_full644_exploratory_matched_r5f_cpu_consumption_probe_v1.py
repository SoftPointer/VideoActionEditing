...
import full644_exploratory_matched_r5f_cpu_consumption_probe_v1 as probe
...
class R5FCPUConsumptionProbeUnitTests(unittest.TestCase):
    ...
    def test_receipt_is_canonical_0400_nlink1_and_create_only(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises((FileExistsError, probe.R5FCPUConsumptionProbeError)):
    ...
    def test_retained_probe_fd_is_byte_stable_and_named_replacement_rejects(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                with self.assertRaises(probe.R5FCPUConsumptionProbeError):
                    probe._replay_parent_probe_code(authority)
            finally:
    ...
    def test_stderr_classifier_is_closed(self) -> None:
        ...
        for hostile in (
            ...
            with self.subTest(hostile=hostile), self.assertRaises(
                probe.R5FCPUConsumptionProbeError
            ):
    ...
    def test_isolated_eight_source_module_gate_keeps_torch_lazy(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            body = (
                ...
                f"r5f,authority,row=module.load_production_sources(pathlib.Path({str(methods)!r}),pathlib.Path({str(site_candidates[0])!r}))\n"
                "print(json.dumps({'closure':row['source_closure_digest'],'torch':('torch' in sys.modules),'module':r5f.__name__},sort_keys=True,separators=(',',':')))\n"
            )
            completed = subprocess.run(
                [
                    sys.executable,
...
class R5FCPUConsumptionProbeLinuxIntegrationTests(unittest.TestCase):
