#!/usr/bin/env python3
"""Static and optional tensor tests for the unsealed v4-E/EPMC canary core."""
...
MATERIALIZER_PATH = METHOD_ROOT / "materialize_v4e_epmc_gate_state_v1.py"
RUNTIME_PATH = METHOD_ROOT / "infer_v4e_epmc_temporal_gate_canary_v1.py"
BUILDER_PATH = METHOD_ROOT / "tools/build_v4e_epmc_temporal_gate_review_v1.py"
CONTROLLER_PATH = (
    METHOD_ROOT / "scripts/auh_v4e_epmc_temporal_gate_video_canary_v1.sh"
)
...
class V4EEPMCTemporalGateStaticTests(unittest.TestCase):
    ...
    def test_release_placeholders_fail_closed(self) -> None:
        ...
        for name in (
            "EXPECTED_V4E_RECEIPT_SCHEMA",
            "EXPECTED_V4E_RECEIPT_STATUS",
            "EXPECTED_V4E_FOLD_RECEIPT_SCHEMA",
            "EXPECTED_V4E_FOLD_RECEIPT_STATUS",
            "EXPECTED_V4E_CHECKPOINT_SCHEMA",
            "EXPECTED_V4E_IMPLEMENTATION_SHA256",
            "EXPECTED_V4E_RECEIPT_FILE_SHA256",
            "EXPECTED_V4E_RECEIPT_SELF_DIGEST",
    ...
    def test_only_true_aggregate_gate_can_reach_materialization(self) -> None:
        ...
        for fragment in (
            ...
            '"v4e_oof_used_to_select_comparator"',
            '"single_v4e_candidate"',
            '"v4e_aggregate_gate_verified_true": True',
        ):
            self.assertIn(fragment, materializer)
        self.assertIn(
            'payload.get("v4e_aggregate_gate_verified_true") is not True', runtime
        )
        ...
        self.assertIn('"v4e_aggregate_gate_verified_true": True', runtime)
    ...
    def test_v4e_global_codec_and_three_way_authority_join_are_explicit(self) -> None:
        source = _source(MATERIALIZER_PATH)
        for fragment in (
            "semantic_anchor_vjepa2_multiview_global_codec_v4e_alt as v4e",
            "v4e.ClipPCAFit(",
            "v4e.VJepa2GlobalCodec(",
        ...
        self.assertNotIn("tucker", source)
        self.assertNotIn("nonlinear_temporal_codec_v4e", source)
    ...
    def test_unsealed_runtime_and_html_fail_before_io(self) -> None:
        materializer = importlib.import_module(
            "methods.bernini_action_editing.materialize_v4e_epmc_gate_state_v1"
        )
        runtime = importlib.import_module(
            "methods.bernini_action_editing.infer_v4e_epmc_temporal_gate_canary_v1"
        )
        builder = importlib.import_module(
            "methods.bernini_action_editing.tools."
            "build_v4e_epmc_temporal_gate_review_v1"
        )
        with mock.patch.object(materializer, "_load_tensor_runtime") as tensor_runtime:
            with mock.patch.object(materializer, "_plain_absolute_file") as plain:
                with self.assertRaisesRegex(
                    materializer.V4EEPMCGateStateError, "UNSEALED"
                ):
        ...
        with mock.patch.object(Path, "lstat") as lstat:
            with self.assertRaisesRegex(runtime.V4EEPMCVideoCanaryError, "UNSEALED"):
                runtime.load_gate_state("/definitely/not/read.json", expected_sha256="x")
            lstat.assert_not_called()
        with mock.patch.object(Path, "lstat") as lstat:
            with self.assertRaisesRegex(builder.V4EEPMCReviewError, "UNSEALED"):
                builder.run(object())
            lstat.assert_not_called()
        with self.assertRaisesRegex(runtime.V4EEPMCVideoCanaryError, "UNSEALED"):
            runtime.main([])
        with self.assertRaisesRegex(builder.V4EEPMCReviewError, "UNSEALED"):
            builder.main([])
        with self.assertRaisesRegex(materializer.V4EEPMCGateStateError, "UNSEALED"):
            materializer.main([])
...
class V4EEPMCTemporalGateTensorTests(unittest.TestCase):
    @classmethod
    ...
    def test_materializer_unsealed_guard_precedes_input_io(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            args = type(
                ...
                (),
                {
                    "v4e_receipt": str(Path(directory) / "receipt.json"),
