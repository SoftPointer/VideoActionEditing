...
import diagnose_saic_partial33_dinov2_temporal_v1 as diagnostic


class Partial33DINOContractTest(unittest.TestCase):
    def test_exact33_partition_is_balanced_and_exactly_once(self) -> None:
    ...
    def test_schema_and_coverage_namespace_are_exact33(self) -> None:
        self.assertEqual(diagnostic.EXPECTED_ATTEMPT_COUNT, 33)
        self.assertEqual(diagnostic.EXPECTED_WORLD_SIZE, 8)
        self.assertIn("partial33", diagnostic.INPUT_SCHEMA)
        self.assertIn("partial33", diagnostic.SHARD_SCHEMA)
        self.assertIn("partial33", diagnostic.AGGREGATE_SCHEMA)
        source = Path(diagnostic.__file__).read_text("utf-8")
        self.assertIn('"coverage": "exactly_once_complete_partial33"', source)
    ...
    def test_launcher_is_exact33_and_maps_only_rocr_devices(self) -> None:
        launcher = (
            METHOD_ROOT / "scripts" / "auh_diagnose_saic_partial33_dinov2_temporal_v1.sh"
        ).read_text("utf-8")
        ...
        self.assertIn("partial33 DINO", launcher)
