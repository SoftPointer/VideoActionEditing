...
class CompositeCPUGateV4V3Tests(unittest.TestCase):
    @classmethod
    ...
    def test_ready_is_one_exact_state_line_diff_with_locked_identity(self):
        old = 'CONTROLLER_STATE = "HOLD_PENDING_FRESH_CANARY_V3_CPU_V3_AUDIT"'
    ...
    def test_fresh_names_and_cache_are_disjoint_from_gpu_production(self):
        ...
        self.assertTrue(all("canary_v3" in path and "composite_cpu" in path
                            for path in paths))
        self.assertTrue(all("_v3." in path for path in paths))
        self.assertIn("node292-r3-rank-cache", str(self.ready.PRODUCTION_RANK_CACHE))
        self.assertIn("composite-cpu-v3-job", self.ready.ROOT_BOOTSTRAP)
        self.assertNotIn(
        ...
        value = {
            ...
            "cache_lifecycle": {
                "admission_cache_root": (
                    ...
                    "cpu-v3-job143808-step511-cache"
                ),
