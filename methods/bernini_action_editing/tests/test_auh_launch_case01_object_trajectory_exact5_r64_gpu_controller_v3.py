#!/usr/bin/env python3
"""Hostile tests for the fresh receipt-gated exact5 GPU controller v3."""
...
CONTROLLER_PATH = ROOT / (
    ...
    "auh_launch_case01_object_trajectory_exact5_r64_gpu_once_v3.HOLD.py"
)
READY_CONTROLLER_PATH = ROOT / (
    "methods/bernini_action_editing/scripts/"
    "auh_launch_case01_object_trajectory_exact5_r64_gpu_once_v3.READY.py"
)
STATIC_PATH = ROOT / (
    "methods/bernini_action_editing/scripts/"
...
class GPUControllerSourceTests(unittest.TestCase):
    def test_ready_controller_is_exactly_one_state_line_diff(self):
        hold = CONTROLLER_PATH.read_text(encoding="utf-8")
        ready = READY_CONTROLLER_PATH.read_text(encoding="utf-8")
        old = (
            'CONTROLLER_STATE = '
            '"HOLD_PENDING_FRESH_PACKAGE_AND_COMPOSITE_CPU_PINS"'
        )
        new = (
            'CONTROLLER_STATE = '
            '"READY_EXPLICIT_SINGLE_SRUN_EXACT_FIVE_NO_RETRY"'
        )
        self.assertEqual(hold.count(old), 1)
        self.assertEqual(ready.count(new), 1)
        self.assertEqual(hold.replace(old, new, 1), ready)
    ...
    def test_exact_twenty_three_fresh_final_pins_have_no_blockers(self):
        pins = controller.dynamic_pin_values()
        self.assertEqual(len(pins), 23)
        self.assertEqual(controller.blocked_dynamic_pins(), ())
        self.assertEqual(pins["package_publication_receipt_size"], 2530)
        self.assertEqual(pins["materialization_report_size"], 41351)
        self.assertEqual(pins["package_controller_evidence_size"], 8113)
        ...
        self.assertEqual(pins["composite_cpu_receipt_size"], 6082)
        self.assertEqual(pins["composite_cpu_evidence_size"], 1849)
...
class ProducerAlignmentTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.launcher = load(FRESH_LAUNCHER_PATH, "_case01_gpu_v3_launcher")
        cls.cpu = load(COMPOSITE_CPU_GATE_PATH, "_case01_gpu_v3_cpu_gate")
    ...
    def test_schema_roles_and_exact_field_sets_align_with_producers(self):
        ...
        self.assertEqual(controller.COMPOSITE_CPU_EVIDENCE_SCHEMA, self.cpu.SCHEMA)
...
class TransportWidthTests(unittest.TestCase):
    ...
    def test_real_sealed_plan_input_and_named_hold_payload_pins(self):
        self.assertEqual(controller.MATERIALIZATION_REPORT_SIZE, 41351)
        self.assertEqual(
            self.report["plan"],
            {
                "path": str(controller.HOLD_PLAN_PATH),
