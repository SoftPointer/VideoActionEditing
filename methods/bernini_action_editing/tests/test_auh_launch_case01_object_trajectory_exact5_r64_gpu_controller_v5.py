#!/usr/bin/env python3
"""Hostile tests for the fresh receipt-gated exact5 GPU controller v5 HOLD."""
...
CONTROLLER_PATH = ROOT / (
    ...
    "auh_launch_case01_object_trajectory_exact5_r64_gpu_once_v5.HOLD.py"
)
STATIC_PATH = ROOT / (
    "methods/bernini_action_editing/scripts/"
...
controller = load(CONTROLLER_PATH, "_case01_gpu_controller_v5_test")
...
def composite_cpu_fixture(mutator=None, evidence_mutator=None):
    ...
    receipt = {
        ...
        "status": "PASS_COMPOSITE_CPU_EXACT26_ACTIVATION_IMPORT_V3_HOLD",
        ...
        "cache_lifecycle": {
            "admission_cache_root": (
                "/tmp/bernini-case01-object-trajectory-r5f-v4-composite-cpu-v3-"
    ...
    evidence = {
        ...
        "status": "PASS_FRESH_CANARY_V3_COMPOSITE_CPU_CONTROLLER_V3",
        "holder_job_id": controller.HOLDER_JOB_ID, "node": controller.NODE,
...
class GPUControllerSourceTests(unittest.TestCase):
    def test_only_hold_controller_is_checked_in(self):
        ...
        self.assertEqual(
            hold.count(
                'CONTROLLER_STATE = '
                '"HOLD_PENDING_FRESH_V3_COMPOSITE_CPU_PINS"'
            ),
            1,
        )
        self.assertFalse(
    ...
    def test_static_v5_boundary_forbids_old_producer_authorities(self):
        ...
        self.assertTrue(str(controller.COMPOSITE_CPU_RECEIPT_PATH).endswith(
            "canary_v3.composite_cpu_admission_receipt_v3.json"
        ))
        self.assertTrue(str(controller.COMPOSITE_CPU_EVIDENCE_PATH).endswith(
            "canary_v3.composite_cpu_admission_controller_evidence_v3.json"
        ))
        self.assertEqual(
        ...
        self.assertFalse(
            {controller.COMPOSITE_CPU_RECEIPT_PATH,
             controller.COMPOSITE_CPU_EVIDENCE_PATH}
            & set(controller.REVOKED_COMPOSITE_CPU_V2_PATHS)
        )
        self.assertFalse(
            {controller.COMPOSITE_CPU_SCHEMA,
             controller.COMPOSITE_CPU_EVIDENCE_SCHEMA}
            & controller.REVOKED_COMPOSITE_CPU_V2_SCHEMAS
        )
        self.assertNotIn(
            "REVOKED_COMPOSITE_CPU_V2_PATHS",
            inspect.getsource(controller.open_package_gate),
        )

    def test_revoked_gpu_v4_outer_namespace_is_absent_gate_not_input(self):
        active = {
            controller.ATTEMPT_PATH, controller.DISPATCH_PATH,
            controller.EVIDENCE_PATH, controller.STDOUT_PATH,
            controller.STDERR_PATH,
        }
        self.assertFalse(active & set(controller.REVOKED_GPU_V4_OUTER_PATHS))
        self.assertTrue(all("v5" in path.name for path in active))
        ...
        self.assertNotIn("REVOKED_GPU_V4_OUTER_PATHS", fresh_source)
    ...
    def test_scientific_v3_custom_v4_and_off_v5_abi_split(self):
        ...
        self.assertEqual(controller.SCHEMA,
                         "case01-object-trajectory-exact5-r64-gpu-controller-v5")
        self.assertEqual(
        ...
        self.assertTrue(controller.ATTEMPT_PATH.name.endswith("_v5.json"))
        self.assertTrue(controller.EVIDENCE_PATH.name.endswith("_v5.json"))
    ...
    def test_only_fresh_v3_cpu_pins_remain_blocked(self):
...
class ProducerAlignmentTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.cpu = load(COMPOSITE_CPU_GATE_PATH, "_case01_gpu_v5_cpu_gate")
        cls.cpu_ready = load(
            COMPOSITE_CPU_GATE_READY_PATH,
            "_case01_gpu_v5_cpu_gate_ready",
        )
    ...
    def test_paths_align_with_fresh_package_and_cpu_producers(self):
        ...
        self.assertEqual(len(cpu_raw), 91_930)
        self.assertEqual(
            hashlib.sha256(cpu_raw).hexdigest(),
            "ce25db5eb7cc72e1ae59c479f176b8615a5e0988105f3ef696f2eb5fad25c87c",
        )
        self.assertEqual(len(cpu_ready_raw), 91_928)
        self.assertEqual(
            hashlib.sha256(cpu_ready_raw).hexdigest(),
            "1c7ffe4d7752ac0c96c14a09c3f0433ff3edf2f896be3cd0174ed59977eb241e",
        )
        hold_state = (
            b'CONTROLLER_STATE = "HOLD_PENDING_FRESH_CANARY_V3_CPU_V3_AUDIT"'
        )
        ready_state = (
        ...
        self.assertEqual(
            ...
            "092930b3c516c19db80d7e6b77bfb7d541f78f0eb311e8d129bd62b61cf863a1",
        )
    ...
    def test_direct_source_loaded_cpu_v3_cache_lifecycle_contract_joins(self):
        ...
        self.assertEqual(consumer, producer_ready)
        self.assertEqual(producer_ready["admission_cache_root"], (
            "/tmp/bernini-case01-object-trajectory-r5f-v4-composite-cpu-v3-"
            "job143808-step515-cache"
        ))
        self.assertEqual(
            producer_ready["production_rank_cache"],
            str(controller.RANK_CACHE_ROOT),
        )
        self.assertEqual(set(producer_ready), {
...
class OrderingAndCleanupTests(unittest.TestCase):
    def test_package_gate_opens_all_receipts_before_root(self):
        ...
        self.assertFalse(
            {"file:" + path.name for path in controller.REVOKED_GPU_V4_OUTER_PATHS}
            & set(events)
        )
        self.assertFalse(
            {"file:" + path.name
