...
CONTROLLER = METHOD_ROOT / "scripts" / "auh_infer_source_noised_carrier_stage_b_two_holder_v5.sh"
...
V4_CONTROLLER = METHOD_ROOT / "scripts" / "auh_infer_source_noised_carrier_stage_b_two_holder_v4.sh"
R1_RELEASE_ROOT = METHOD_ROOT / "releases" / "source_noised_carrier_stage_b_inference_r1"
R2_RELEASE_ROOT = METHOD_ROOT / "releases" / "source_noised_carrier_stage_b_inference_r2"
RELEASE_ROOT = METHOD_ROOT / "releases" / "source_noised_carrier_stage_b_inference_r3"


class StageBInferenceSelectableTwoHolderControllerV5Tests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_release_authority_and_all_fourteen_members_are_pinned(self) -> None:
        ...
        self.assertEqual(archive_sha, "e3880934c3e6cfcb0dfe56aa34a03f3ffbb2cb192a262fdb8ae1734a02f183ca")
        self.assertEqual(manifest_sha, "6849ed11ad214e4c49f72731e4beb88948f2abf26e79f0ff5cf8c4e2814e62a3")
        self.assertEqual(revision, "051db819ab8cca158a69cce58369b13d1dbb7531")
    ...
    def test_prior_controllers_and_runtime_release_remain_byte_identical(self) -> None:
        ...
        self.assertEqual(
            hashlib.sha256(V4_CONTROLLER.read_bytes()).hexdigest(),
            "e5a519479a0b39945a97599a3750759384c1fe68b71a1fd14d833ca95d37c3b1",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            hashlib.sha256((R2_RELEASE_ROOT / "source.tar").read_bytes()).hexdigest(),
            "298a0a63dda44843a828b70b87c9b41441c84a004b7cfec9f4340e2497ba54c1",
        )
        self.assertEqual(
            hashlib.sha256((R2_RELEASE_ROOT / "source.manifest.json").read_bytes()).hexdigest(),
            "72a787ce5e645e7cb44d3331777899e93d35d07e7155c9bff97682367fe38766",
        )
        self.assertNotEqual(V4_CONTROLLER.read_bytes(), CONTROLLER.read_bytes())
    ...
    def test_real_run_branch_reaches_first_gate_under_nounset(self) -> None:
        ...
        harness = "\n".join(
            (
                "set -u",
                "run_root=/private/tmp/stage-b-v5-run-branch-nounset-$$",
                "fail() { printf 'FAIL:%s\\n' \"$*\" >&2; exit 2; }",
