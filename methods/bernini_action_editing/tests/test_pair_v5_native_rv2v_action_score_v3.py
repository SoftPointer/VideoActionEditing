...
if torch is not None:
    ...
    import pair_v5_native_rv2v_action_score_v3 as native_score  # noqa: E402
...
class PairV5NativeRV2VActionScoreV3Tests(unittest.TestCase):
    ...
    def _fixture(self) -> tuple[dict[str, object], dict[str, object], torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        bundle = {
            ...
            "authorization": {
                ...
                "formal_score_schema": frozen_runtime.SCORE_RECEIPT_SCHEMA,
                "formal_score_filename": frozen_runtime.SCORE_RECEIPT_FILENAME,
                "formal_score_scalar_definition": frozen_runtime.CANONICAL_ACTION_ENERGY_ARITHMETIC_SCHEMA,
                "formal_score_arithmetic_contract_digest": frozen_runtime.canonical_action_energy_arithmetic_contract()["contract_digest"],
    ...
    def test_score_receipt_binds_native_t516_map_prompts_and_safe_pareto(self) -> None:
        ...
        self.assertTrue(checked["schema_version"].endswith("-v4"))
        self.assertEqual(
            checked["mace"]["definition"],
            frozen_runtime.CANONICAL_ACTION_ENERGY_ARITHMETIC_SCHEMA,
        )
        packet = checked["mace"]["canonical_action_energy_packet"]
        self.assertEqual(
            frozen_runtime.validate_canonical_action_energy_packet(packet),
            packet,
        )
        self.assertTrue(packet["origin_device_values_diagnostic_only"])
        self.assertFalse(hasattr(native_score, "formal_v3_compat"))
    ...
    def test_resigned_score_and_calibration_mutations_fail(self) -> None:
        ...
        changed = deepcopy(receipt)
        packet = changed["mace"]["canonical_action_energy_packet"]
        packet["global_negative_log_energy_ratio_by_branch"]["noop"] += 0.25
        packet_unsigned = dict(packet)
        packet_unsigned.pop("packet_digest")
        packet["packet_digest"] = frozen_runtime.object_sha256(packet_unsigned)
        changed["mace"]["global_negative_log_energy_ratio_by_branch"]["noop"] = packet["global_negative_log_energy_ratio_by_branch"]["noop"]
        changed["mace"]["canonical_action_energy_packet_digest"] = packet[
            "packet_digest"
        ]
        _resign(changed)
        with self.assertRaisesRegex(
            native_score.PairV5NativeRV2VActionScoreError,
            "canonical action-energy arithmetic replay",
        ):
            native_score.validate_score_receipt(changed)
    ...
    def test_input_closure_has_no_privileged_or_media_tensor_path(self) -> None:
        ...
        self.assertTrue(closure["active_repository_action_scalar_consumed"])
        self.assertFalse(closure["legacy_v3_compatibility_scalar_consumed"])
        self.assertFalse(
