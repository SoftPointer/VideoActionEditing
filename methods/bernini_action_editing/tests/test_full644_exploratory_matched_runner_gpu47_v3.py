...
import full644_exploratory_matched_runner_gpu47_v3 as runner
...
def gpu47_admission_value(
    *,
    ...
    output_root: str,
    campaign_mode: str = "case00-pair-canary",
) -> dict:
    physical = [
        {
    ...
    # Logical enumeration is intentionally permuted within physical 4-7.
    ...
    value = {
        "schema_version": runner.GPU_ADMISSION_SCHEMA,
        "status": "PASS",
        ...
        "intended_plan_sha256": SHA,
        "intended_output_root": output_root,
        "single_use": True,
        ...
        "all8_runtime_devices": all8,
        "visible_runtime_devices": visible,
    ...
    value["receipt_digest"] = runner.object_sha256(value)
    return value
...
class MatchedRunnerV2Tests(unittest.TestCase):
    ...
    def test_subprocess_environment_scrubs_injection_and_pins_all_gpu_masks(self) -> None:
        ...
        self.assertEqual(environment["ROCR_VISIBLE_DEVICES"], "4,5,6,7")
        for key in (
            "HIP_VISIBLE_DEVICES",
    ...
    def test_campaign_selection_is_only_full16_or_case00_pair(self) -> None:
        tasks = [
            task(index, arm)
            for index in range(8)
            for arm in ("base", "full644")
        ...
        with mock.patch.object(
            runner, "validate_task_order", return_value=tasks
        ):
            full = runner.select_campaign_tasks(
                {"fixture": True}, runner.FULL16_CAMPAIGN
            )
            canary = runner.select_campaign_tasks(
                {"fixture": True}, runner.CASE00_CANARY_CAMPAIGN
            )
            self.assertEqual(tuple(row["task_id"] for row in full), runner.TASK_IDS)
            self.assertEqual(
                tuple(row["task_id"] for row in canary), runner.CANARY_TASK_IDS
            )
            with self.assertRaises(runner.MatchedRunnerV2Error):
                runner.select_campaign_tasks({"fixture": True}, "partial")

    def test_case00_sequence_executes_only_base_then_full644_once(self) -> None:
        ...
        for hostile in (
            ...
            with self.subTest(ids=[row["task_id"] for row in hostile]):
                with self.assertRaises(runner.MatchedRunnerV2Error):
    ...
    def test_case00_report_is_nonformal_and_requires_visual_review(self) -> None:
        ...
        execution = types.SimpleNamespace(
            ...
            publication_authorities={
                task_row["task_id"]: {"fixture": task_row["task_id"]}
        ...
        self.assertEqual(report["unselected_task_ids"], list(runner.TASK_IDS[2:]))
        self.assertEqual(report["unselected_task_count"], 14)
        self.assertFalse(report["unselected_tasks_executed"])
        self.assertFalse(report["unselected_outputs_published"])
        self.assertEqual(
            report["status"], "CANARY_COMPLETE_AWAITING_VISUAL_REVIEW"
        )
    ...
    def test_allocation_must_match_holder_node_and_step(self) -> None:
        ...
        self.assertEqual(value["visible_gpu_indices"], [4, 5, 6, 7])
        self.assertEqual(value["logical_gpu_indices"], [0, 1, 2, 3])
        self.assertEqual(
            value["gpu_visibility_contract"],
            runner.gpu47_visibility_contract(
                {"ROCR_VISIBLE_DEVICES": "4,5,6,7"}
            ),
        )
    ...
    def test_gpu47_mapping_rejects_secondary_or_wrong_masks(self) -> None:
        expected = runner.gpu47_visibility_contract(
            {"ROCR_VISIBLE_DEVICES": "4,5,6,7"}
        )
        self.assertEqual(expected["physical_gpu_indices"], [4, 5, 6, 7])
        self.assertEqual(expected["logical_gpu_indices"], [0, 1, 2, 3])
        self.assertNotIn("logical_to_physical", expected)
        self.assertTrue(expected["logical_to_physical_order_not_inferred_from_mask"])
        self.assertTrue(expected["empirical_pci_uuid_admission_required"])
        for hostile in (
            {},
            ...
            with self.subTest(hostile=hostile), self.assertRaises(
                runner.MatchedRunnerV2Error
            ):
                runner.gpu47_visibility_contract(hostile)

    def test_gpu47_admission_replays_permuted_pci_unique_id_uuid_join(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw).resolve(strict=True)
            output_root = root / "outputs"
            output_root.mkdir()
            now_ns = 10_000_000_000_000
            value = gpu47_admission_value(
                observed_at_unix_ns=now_ns - 1_000_000_000,
                output_root=str(output_root),
            )
            path = root / "gpu47-admission.json"
            payload = runner.canonical_json_bytes(value) + b"\n"
            path.write_bytes(payload)
            path.chmod(0o400)
            observed = runner.validate_gpu47_admission_receipt(
                path,
                hashlib.sha256(payload).hexdigest(),
                holder_job_id="143812",
                expected_node="auh7-1b-gpu-293",
                expected_slurm_step_id="999",
                campaign_mode=runner.CASE00_CANARY_CAMPAIGN,
                plan_sha256=SHA,
            ...
            self.assertEqual(
                [
                    row["physical_index"]
                    for row in observed["visible_runtime_devices"]
                ],
                [6, 4, 7, 5],
            )
            self.assertFalse(observed["device_cgroup_hard_isolation_claimed"])

    def test_gpu47_admission_rejects_leak_stale_baseline_and_identity_tamper(self) -> None:
        now_ns = 20_000_000_000_000
        for label in (
            "physical0-visible",
            "stale",
            "busy-baseline",
            ...
            with self.subTest(label=label), tempfile.TemporaryDirectory() as raw:
                ...
                if label == "physical0-visible":
                    value["visible_runtime_devices"][0] = {
                        "logical_index": 0,
                        **value["physical_inventory"][0],
                    }
                elif label == "stale":
                    value["observed_at_unix_ns"] = (
                        now_ns - runner.GPU_ADMISSION_MAX_AGE_NS - 1
                    )
                elif label == "busy-baseline":
                    value["external_workload_baseline"][
                        "inference_process_count"
                    ] = 1
                elif label == "unique-id-tamper":
                    value["visible_runtime_devices"][0][
                        "rocm_unique_id"
                    ] = "f" * 16
                elif label == "step-mismatch":
                    value["slurm_step_id"] = "1000"
                else:
                    value["intended_campaign_mode"] = runner.FULL16_CAMPAIGN
                ...
                path.chmod(0o400)
                with self.assertRaises(runner.MatchedRunnerV2Error):
                    runner.validate_gpu47_admission_receipt(
                        path,
                        hashlib.sha256(payload).hexdigest(),
                        holder_job_id="143812",
                        expected_node="auh7-1b-gpu-293",
