#!/usr/bin/env python3
"""Static boundary tests for the fresh exact5 GPU-v5 HOLD controller."""
...
controller = load(V5_HOLD, "_case01_gpu_v5_static")


class GPUV5StaticTests(unittest.TestCase):
    def test_old_authorities_are_unchanged_and_no_v5_ready_exists(self):
        self.assertEqual(
    ...
    def test_fresh_producer_and_output_namespace_is_exact(self):
        ...
        self.assertTrue(all("v5" in path.name for path in outer_paths))
        self.assertTrue(all("v4" in path.name for path in compute_paths))
        self.assertFalse(set(outer_paths) & set(controller.REVOKED_GPU_V4_OUTER_PATHS))
    ...
    def test_source_and_embedded_payloads_compile_without_asserts(self):
        source = V5_HOLD.read_text(encoding="utf-8")
        tree = ast.parse(source, str(V5_HOLD))
        self.assertFalse(any(isinstance(node, ast.Assert) for node in ast.walk(tree)))
        for optimize in (0, 1, 2):
            compile(source, str(V5_HOLD), "exec", optimize=optimize)
            compile(controller.ROOT_BOOTSTRAP, "<gpu-v5-root>", "exec",
                    optimize=optimize)
        for symbol in (
