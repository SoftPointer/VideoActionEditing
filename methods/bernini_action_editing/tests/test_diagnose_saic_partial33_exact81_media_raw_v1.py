...
import diagnose_saic_partial33_exact81_media_raw_v1 as diagnostic


class Exact33Exact81MediaRawContractTest(unittest.TestCase):
    def test_exact_r5_input_and_runtime_sources_are_pinned(self) -> None:
        self.assertEqual(diagnostic.EXPECTED_CANDIDATE_COUNT, 33)
        ...
        self.assertEqual(diagnostic.frozen_input.EXPECTED_ATTEMPT_COUNT, 33)
    ...
    def test_nested_exact33_validator_identity_is_restored_after_pollution(self) -> None:
        ...
        diagnostic.frozen_input.core.__file__ = "/tmp/hostile-partial28.py"
        diagnostic._configure_frozen_input()
        expected = (
            METHOD_ROOT / "diagnose_saic_partial33_source_bound_dinov2_raw_v1.py"
        ).resolve()
        ...
        self.assertEqual(
            diagnostic.frozen_input.partition_indices(33, 0, 8),
            (0, 8, 16, 24, 32),
        )
    ...
    def test_launcher_is_allocation_bound_cpu_only_and_exact33(self) -> None:
        launcher = (
            METHOD_ROOT
            / "scripts"
            / "auh_diagnose_saic_partial33_exact81_media_raw_v1.sh"
        ).read_text("utf-8")
