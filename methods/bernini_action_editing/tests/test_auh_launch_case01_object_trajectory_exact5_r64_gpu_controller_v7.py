#!/usr/bin/env python3
"""Hostile tests for the fresh receipt-gated exact5 GPU controller v7 HOLD."""
...
CONTROLLER_PATH = ROOT / (
    ...
    "auh_launch_case01_object_trajectory_exact5_r64_gpu_once_v7.HOLD.py"
)
CONTROLLER_READY_PATH = CONTROLLER_PATH.with_name(
    "auh_launch_case01_object_trajectory_exact5_r64_gpu_once_v7.READY.py"
)
STATIC_PATH = ROOT / (
    "methods/bernini_action_editing/scripts/"
...
REAL_MATERIALIZATION_PATH = ROOT / (
    "methods/bernini_action_editing/tests/fixtures/"
    "case01_object_trajectory_exact5_r64_canary_v3_"
    "materialization_receipt_v4.json"
)
...
COMPOSITE_CPU_GATE_READY_PATH = COMPOSITE_CPU_GATE_PATH.with_name(
    "auh_gate_case01_object_trajectory_exact5_r5f_v4_"
    "composite_cpu_once_v4.READY.py"
)
...
def source_cache_lifecycle_contract(function, receipt_name: str, step_id: str):
    """Evaluate the cache contract literally embedded in loaded producer code."""
    tree = ast.parse(textwrap.dedent(inspect.getsource(function)))
    for comparison in ast.walk(tree):
        left = comparison.left if isinstance(comparison, ast.Compare) else None
        if (
            ...
            and left.func.value.id == receipt_name
            and len(left.args) == 1
            and isinstance(left.args[0], ast.Constant)
            and left.args[0].value == "cache_lifecycle"
        ):
            namespace = dict(function.__globals__)
            namespace[receipt_name] = {"slurm_step_id": step_id}
            expression = ast.Expression(body=comparison.comparators[0])
            return eval(
                compile(ast.fix_missing_locations(expression), "<contract>", "eval"),
                namespace,
            )
    raise AssertionError("source cache_lifecycle contract missing")


controller = load(CONTROLLER_PATH, "_case01_gpu_controller_v7_test")
ready_controller = load(
    CONTROLLER_READY_PATH, "_case01_gpu_controller_v7_ready_test",
)
...
def composite_cpu_fixture(mutator=None, evidence_mutator=None):
    ...
    receipt = {
        ...
        "cache_lifecycle": {
            "admission_cache_root": (
                "/tmp/bernini-case01-object-trajectory-r5f-v4-composite-cpu-v4-"
    ...
    evidence = {
        ...
        "status": "PASS_FRESH_CANARY_V3_COMPOSITE_CPU_CONTROLLER_V4",
        "holder_job_id": controller.HOLDER_JOB_ID, "node": controller.NODE,
        ...
        "normal_exit_signal_sent": False, "terminal_pipes_closed": True,
    }
...
class GPUControllerSourceTests(unittest.TestCase):
    def test_ready_is_exact_single_state_projection_of_frozen_hold(self):
        hold = CONTROLLER_PATH.read_bytes()
        ready = CONTROLLER_READY_PATH.read_bytes()
        hold_state = (
        ...
        self.assertEqual(hold.count(hold_state), 1)
        self.assertEqual(hold.count(ready_state), 0)
        self.assertEqual(ready.count(hold_state), 0)
        self.assertEqual(ready.count(ready_state), 1)
        self.assertEqual(hold.replace(hold_state, ready_state, 1), ready)
        self.assertEqual(
    ...
    def test_static_v7_boundary_forbids_old_producer_authorities(self):
        ...
        self.assertTrue(str(controller.COMPOSITE_CPU_RECEIPT_PATH).endswith(
            "canary_v3.composite_cpu_admission_receipt_v4.json"
        ))
        self.assertTrue(str(controller.COMPOSITE_CPU_EVIDENCE_PATH).endswith(
            "canary_v3.composite_cpu_admission_controller_evidence_v4.json"
        ))
        self.assertEqual(
        ...
        self.assertTrue(all(
            "_v2.json" in path.name
            for path in controller.REVOKED_COMPOSITE_CPU_V2_PATHS
        ))
        self.assertFalse(
        ...
        self.assertTrue(all(
            "_v3.json" in path.name
            for path in controller.REVOKED_COMPOSITE_CPU_V3_PATHS
        ))
        self.assertNotIn(
            "REVOKED_COMPOSITE_CPU_PRIOR_PATHS",
            inspect.getsource(controller.open_package_gate),
        )

    def test_revoked_gpu_v4_v5_v6_outer_namespace_is_absent_gate_not_input(self):
        active = {
            controller.ATTEMPT_PATH, controller.DISPATCH_PATH,
            controller.EVIDENCE_PATH, controller.STDOUT_PATH,
            controller.STDERR_PATH,
        }
        self.assertFalse(active & set(controller.REVOKED_GPU_PRIOR_OUTER_PATHS))
        self.assertTrue(all("v7" in path.name for path in active))
        ...
        fresh_source = inspect.getsource(controller.require_fresh_outputs)
        self.assertIn('_exact_names(PACKAGE_ROOT / "evidence", set()', fresh_source)
        self.assertIn('_exact_names(PACKAGE_ROOT / "logs", set()', fresh_source)
        self.assertNotIn("open_authority", fresh_source)
        self.assertNotIn("REVOKED_GPU_PRIOR_OUTER_PATHS", fresh_source)

    def test_scientific_v3_custom_v4_off_v5_outer_v7_abi_split(self):
        ...
        self.assertEqual(
            controller.RUNTIME_SCHEMA,
            "case01-object-trajectory-exact5-r64-gpu-controller-v4-runtime",
        )
        self.assertEqual(
            controller.RANK_CACHE_SCHEMA,
            "case01-object-trajectory-exact5-r64-gpu-controller-v4-rank-cache",
        )
        self.assertEqual(
            controller.COMPUTE_RESULT_SCHEMA,
            "case01-object-trajectory-exact5-r64-gpu-controller-v4-compute-result",
        )
        self.assertTrue(controller.ATTEMPT_PATH.name.endswith("_v7.json"))
        self.assertTrue(controller.EVIDENCE_PATH.name.endswith("_v7.json"))
        self.assertTrue(controller.RUNTIME_RECEIPT_PATH.name.endswith("_v4.json"))
        self.assertTrue(
            controller.RANK_CACHE_RECEIPT_PATH.name.endswith("_v4.json")
        )
        self.assertEqual(
    ...
    def test_even_forced_ready_state_requires_token_before_io(self):
    ...
    def test_fresh_v4_cpu_pins_are_exact_and_fully_resolved(self):
...
class ProducerAlignmentTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.cpu = load(COMPOSITE_CPU_GATE_PATH, "_case01_gpu_v7_cpu_gate")
        cls.cpu_ready = load(
            COMPOSITE_CPU_GATE_READY_PATH,
            "_case01_gpu_v7_cpu_gate_ready",
        )

    def test_real_materialization_exact6_provenance_and_exact5_is_rejected(self):
        raw = REAL_MATERIALIZATION_PATH.read_bytes()
        self.assertEqual(len(raw), 41_726)
        self.assertEqual(
        ...
        rows = report["release"]["files"]
        self.assertEqual(len(rows), 25)
        self.assertEqual(sum(
            row["provenance"] == "receipt_gated_exact6_overlay"
            for row in rows
        ), 5)
        self.assertEqual(sum(
            row["provenance"] == "receipt_gated_exact35_snapshot"
            for row in rows
        ), 20)
        self.assertFalse(any(
            row["provenance"] == "receipt_gated_exact5_overlay"
            for row in rows
        ))
        held = DummyAuthority(controller.MATERIALIZATION_REPORT_PATH, raw, 0o400)
        self.assertEqual(controller.validate_materialization_report(held), report)

        producer_tree = ast.parse(
            FRESH_MATERIALIZER_PATH.read_text(encoding="utf-8"),
            str(FRESH_MATERIALIZER_PATH),
        )
        materialize = next(
        ...
        producer_literals = {
            node.value for node in ast.walk(materialize)
            if isinstance(node, ast.Constant) and isinstance(node.value, str)
        }
        self.assertIn("receipt_gated_exact6_overlay", producer_literals)
        self.assertNotIn("receipt_gated_exact5_overlay", producer_literals)

        hostile = json.loads(raw.decode("utf-8"))
        overlay_row = next(
            row for row in hostile["release"]["files"]
            if row["provenance"] == "receipt_gated_exact6_overlay"
        )
        overlay_row["provenance"] = "receipt_gated_exact5_overlay"
        hostile["release"]["manifest_digest"] = controller.object_digest(
            hostile["release"]["files"]
        )
        unsigned = dict(hostile); unsigned.pop("receipt_digest")
        hostile["receipt_digest"] = controller.object_digest(unsigned)
        hostile_raw = controller.canonical(hostile) + b"\n"
        with mock.patch.object(
            controller, "MATERIALIZATION_REPORT_DIGEST",
            hostile["receipt_digest"],
        ), self.assertRaisesRegex(
            controller.GPUControllerError, "materialized release25 row differs",
        ):
            controller.validate_materialization_report(DummyAuthority(
                controller.MATERIALIZATION_REPORT_PATH, hostile_raw, 0o400,
            ))

    def test_paths_align_with_fresh_package_and_cpu_producers(self):
        cpu_raw = COMPOSITE_CPU_GATE_PATH.read_bytes()
        cpu_ready_raw = COMPOSITE_CPU_GATE_READY_PATH.read_bytes()
        self.assertEqual(len(cpu_raw), 92_194)
        self.assertEqual(
            hashlib.sha256(cpu_raw).hexdigest(),
            "95f42b87b0750c6e924cb33dfe789aff004d408274899a5c97ec4dd6f0c5d7ee",
        )
        hold_state = (
            b'CONTROLLER_STATE = "HOLD_PENDING_FRESH_CANARY_V3_CPU_V4_AUDIT"'
        )
        ready_state = (
            b'CONTROLLER_STATE = "READY_EXPLICIT_SINGLE_CPU_SRUN_NO_RETRY"'
        )
        self.assertEqual(cpu_raw.count(hold_state), 1)
        self.assertEqual(cpu_raw.count(ready_state), 0)
        self.assertEqual(cpu_ready_raw.count(hold_state), 0)
        self.assertEqual(cpu_ready_raw.count(ready_state), 1)
        self.assertEqual(cpu_raw.replace(hold_state, ready_state, 1), cpu_ready_raw)
        self.assertEqual(len(cpu_ready_raw), 92_192)
        self.assertEqual(
            hashlib.sha256(cpu_ready_raw).hexdigest(),
            "657ada13b5bd511ed63dbd0e85ed117e51ca50aff162368646b66a49bbbc5413",
        )
        self.assertEqual(self.cpu.blocked_dynamic_pins(), ())
        self.assertEqual(self.cpu_ready.blocked_dynamic_pins(), ())
        self.assertEqual(
            self.cpu_ready.authorization_token(),
            "2dbde3a62592e142906f553c678971318a9c87e5dac9475d544683d34a385116",
        )
    ...
    def test_direct_source_loaded_cpu_v4_cache_lifecycle_contract_joins(self):
        producer_payload_source = self.cpu.ROOT_BOOTSTRAP
        self.assertIn(
            "composite-cpu-v4-job%s-step%s-cache",
            producer_payload_source,
        )
        self.assertNotIn(
            "composite-cpu-v3-job%s-step%s-cache",
            producer_payload_source,
        )
        producer_hold = source_cache_lifecycle_contract(
            self.cpu.validate_receipt, "value", "515",
        )
        producer_ready = source_cache_lifecycle_contract(
            self.cpu_ready.validate_receipt, "value", "515",
        )
        consumer = source_cache_lifecycle_contract(
            controller.validate_composite_cpu_admission, "receipt", "515",
        )
        self.assertEqual(producer_hold, producer_ready)
        self.assertEqual(consumer, producer_hold)
        self.assertEqual(producer_hold["admission_cache_root"], (
            "/tmp/bernini-case01-object-trajectory-r5f-v4-composite-cpu-v4-"
            "job143808-step515-cache"
        ))
        self.assertEqual(
            producer_hold["production_rank_cache"],
            str(controller.RANK_CACHE_ROOT),
        )
        self.assertEqual(set(producer_hold), {
            "admission_cache_root", "admission_cache_fresh",
            "admission_cache_cleanup_performed", "admission_cache_absent_terminal",
            "production_rank_cache", "production_rank_cache_untouched",
            "production_rank_cache_absent_before_and_after",
        })
    ...
    def test_schema_roles_and_exact_field_sets_align_with_producers(self):
        ...
        self.assertEqual(self.cpu.RECEIPT_SCHEMA, self.cpu_ready.RECEIPT_SCHEMA)
        self.assertEqual(
            controller.COMPOSITE_CPU_EVIDENCE_SCHEMA,
            self.cpu.SCHEMA + "-evidence",
        )
        self.assertEqual(self.cpu.SCHEMA, self.cpu_ready.SCHEMA)
        self.assertEqual(self.cpu.EVIDENCE_SCHEMA, self.cpu_ready.EVIDENCE_SCHEMA)
        self.assertEqual(
    ...
    def test_gpu_consumer_accepts_exact_p0_join_and_rejects_regressions(self):
        ...
        regressions = {
            "revoked CPU-v2 receipt schema": lambda value: value.__setitem__(
                "schema_version",
                "case01-object-trajectory-exact5-r5f-v4-"
                "composite-cpu-admission-v2",
            ),
            "revoked CPU-v3 receipt schema": lambda value: value.__setitem__(
                "schema_version",
                "case01-object-trajectory-exact5-r5f-v4-"
                "composite-cpu-admission-v3",
            ),
            ...
            "revoked CPU-v3 cache root": lambda value: value[
                "cache_lifecycle"
            ].__setitem__(
                "admission_cache_root",
                "/tmp/bernini-case01-object-trajectory-r5f-v4-"
                "composite-cpu-v3-job143808-step515-cache",
            ),
        }
        ...
        evidence_regressions = {
            "revoked CPU-v2 evidence schema": lambda value: value.__setitem__(
                "schema_version",
                "case01-object-trajectory-exact5-r5f-v4-"
                "composite-cpu-controller-v2-evidence",
            ),
            "revoked CPU-v3 evidence schema": lambda value: value.__setitem__(
                "schema_version",
                "case01-object-trajectory-exact5-r5f-v4-"
                "composite-cpu-controller-v3-evidence",
            ),
            "normal exit signalled": lambda value: value.__setitem__(
                "normal_exit_signal_sent", True
            ),
            "passive grace skipped": lambda value: value.__setitem__(
                "normal_exit_passive_grace_performed", False
            ),
            "terminal pipes open": lambda value: value.__setitem__(
                "terminal_pipes_closed", False
            ),
        }
        for label, mutator in evidence_regressions.items():
            with self.subTest(label=label), self.assertRaisesRegex(
                controller.GPUControllerError, "composite CPU",
            ):
                validate(composite_cpu_fixture(evidence_mutator=mutator))
...
class OrderingAndCleanupTests(unittest.TestCase):
    def test_package_gate_opens_all_receipts_before_root(self):
        ...
        self.assertFalse(
            {"file:" + path.name for path in controller.REVOKED_GPU_PRIOR_OUTER_PATHS}
            & set(events)
        )
        self.assertFalse(
            {"file:" + path.name
             for path in controller.REVOKED_COMPOSITE_CPU_PRIOR_PATHS}
            & set(events)
        )
