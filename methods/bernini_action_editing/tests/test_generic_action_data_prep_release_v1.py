...
LAUNCHER = METHOD_ROOT / "scripts/auh_generic_action_data_prep_136141_world4_v1.sh"
LINK_MATRIX_AWK = (
    r'$1~/^GPU[0-7]$/ && NF==9 {ok=1; '
    r'for(i=2;i<=9;i++)if($i!~/^(0|XGMI|PCIE)$/)ok=0; if(ok)print}'
)
LIVE_GPU299_ROCM_SMI_TOPOLOGY = """\
...
def _native_tensor_parity_receipt(
    root: Path,
    *,
    gaussian_container_sha256: str = "3" * 64,
    clean_container_sha256: str = "4" * 64,
    gaussian_raw_storage_sha256: str = "7" * 64,
    gaussian_content_sha256: str = "8" * 64,
    clean_raw_storage_sha256: str = "9" * 64,
    clean_content_sha256: str = "a" * 64,
    mp4_sha256: str = "2" * 64,
) -> dict:
    """Build the minimal signed native receipt consumed by r13 parity."""

    runner = controller.reserve4_runner
    shape = [1, 4, 3, 2, 2]
    numel = 48

    def identity(label: str, raw: str, content: str) -> dict:
        return {
            "shape": list(shape),
            "dtype": "torch.float32",
            "numel": numel,
    ...
    gaussian_identity = identity(
        "official_initial_gaussian_t2v",
        gaussian_raw_storage_sha256,
        gaussian_content_sha256,
    )
    clean_identity = identity(
        "generated_t2v",
        clean_raw_storage_sha256,
        clean_content_sha256,
    )
    gaussian = {
        "path": str((root / "official-initial-gaussian.safetensors").resolve()),
        "sha256": gaussian_container_sha256,
        "shape": list(shape),
        "dtype": "torch.float32",
        "stored_dtype": "torch.float32",
        "numel": numel,
        "byte_count": numel * 4,
        "content_sha256": gaussian_content_sha256,
        "raw_value_sha256": gaussian_raw_storage_sha256,
        "tensor_value_sha256": gaussian_raw_storage_sha256,
        "generator_initial_seed": 1,
        "tensor_key": "official_initial_gaussian",
        "coordinate": runner.GAUSSIAN_SAFETENSORS_METADATA["coordinate"],
        "origin": runner.GAUSSIAN_SAFETENSORS_METADATA["source"],
        "observer_only": True,
        ...
        "all_rank_identity": {
            "all_rank_exact": True,
            "identity": gaussian_identity,
        },
    }
    clean = {
        "path": str((root / "predecode-clean-latent.safetensors").resolve()),
        "sha256": clean_container_sha256,
        "shape": list(shape),
        "stored_dtype": "torch.float32",
    ...
    unsigned = {
        ...
        "generated_identities": {
            "t2v": {"all_rank_exact": True, "identity": clean_identity}
        },
        "outputs": {
            "t2v": {
                "path": str((root / "t2v.mp4").resolve()),
                "sha256": mp4_sha256,
                "frame_count": 81,
                "fps": 25,
                "height": 480,
...
class _FakeSafeOpen:
    """Dependency-free stand-in exercising the production safe_open boundary."""

    def __init__(self, path: str, *, framework: str, device: str) -> None:
        if framework != "pt" or device != "cpu":
            raise ValueError("safe_open framework/device differs")
        self.payload = json.loads(Path(path).read_text(encoding="ascii"))
    ...
    def get_tensor(self, key: str):
        return dict(self.payload["tensors"][key])


def _fake_tensor_identity(value: dict, *, label: str) -> dict:
    runner = controller.reserve4_runner
    if value.get("finite") is not True:
        raise ValueError(f"{value.get('nonfinite_kind', 'nonfinite')} tensor")
    ...
    return {
        **metadata,
        "content_sha256": hashlib.sha256(
            runner.canonical_json_bytes(metadata) + b"\0" + raw
        ).hexdigest(),
...
def _fake_tensor_spec(
    ...
    *,
    ...
    actual_shape = list(shape or [1, 4, 3, 2, 2])
    numel = 1
    for item in actual_shape:
        numel *= item
    bytes_per_value = 4 if dtype == "torch.float32" else 2
    return {
        "shape": actual_shape,
        "dtype": dtype,
        "numel": numel,
        "raw_hex": bytes([fill & 0xFF] * (numel * bytes_per_value)).hex(),
        "finite": finite,
...
def _write_fake_safetensors(
    ...
    *,
    ...
    alternate_header: bool = False,
) -> str:
    payload = {"metadata": metadata, "tensors": tensors}
    if alternate_header:
        raw = (json.dumps(payload, ensure_ascii=True, indent=2) + "\n").encode("ascii")
    else:
        raw = controller.reserve4_runner.canonical_json_bytes(payload) + b"\n"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(raw)
    return _sha(path)


def _materialize_fake_current_native_receipt(root: Path) -> tuple[dict, dict, dict]:
    runner = controller.reserve4_runner
    gaussian_spec = _fake_tensor_spec(17)
    clean_spec = _fake_tensor_spec(29)
    gaussian_path = root / "official-initial-gaussian.safetensors"
    clean_path = root / "predecode-clean-latent.safetensors"
    gaussian_sha = _write_fake_safetensors(
        gaussian_path,
        tensors={"official_initial_gaussian": gaussian_spec},
        metadata=dict(runner.GAUSSIAN_SAFETENSORS_METADATA),
    )
    clean_sha = _write_fake_safetensors(
        clean_path,
        tensors={"normalized_clean_latent": clean_spec},
        metadata=dict(runner.CLEAN_LATENT_SAFETENSORS_METADATA),
    )
    gaussian_identity = _fake_tensor_identity(
        gaussian_spec, label="official_initial_gaussian_t2v"
    )
    clean_identity = _fake_tensor_identity(clean_spec, label="generated_t2v")
    receipt = _native_tensor_parity_receipt(
        root,
        gaussian_container_sha256=gaussian_sha,
        clean_container_sha256=clean_sha,
        gaussian_raw_storage_sha256=gaussian_identity["raw_storage_sha256"],
        gaussian_content_sha256=gaussian_identity["content_sha256"],
        clean_raw_storage_sha256=clean_identity["raw_storage_sha256"],
        clean_content_sha256=clean_identity["content_sha256"],
    )
    return receipt, gaussian_spec, clean_spec


def _write_r10_tensor_parity_authority_fixture(
    root: Path,
    *,
    native_receipt: dict | None = None,
) -> tuple[dict, Path, str, Path, str]:
    """Seal a self-contained historical r10 receipt/log evidence pair."""

    runner = controller.reserve4_runner
    root.mkdir(parents=True, exist_ok=True)
    native = native_receipt or _native_tensor_parity_receipt(root)
    native_line = runner.canonical_json_bytes(native)
    native_file_sha = hashlib.sha256(native_line + b"\n").hexdigest()
    generation_log = root / "generation-fit-all8-serial4.log"
    generation_log.write_bytes(
        b"historical r10 generation header\n"
        + native_line
        + b"\nhistorical r10 generation footer\n"
    )
    historical_rows = runner._legacy_r10_artifact_identities(native)
    unsigned = {
        "schema_version": runner.R10_COMPILE_SMOKE_SCHEMA,
        "plan": {},
        "smoke_task": {},
        "runtime": {},
        "candidate_evidence": {
            ...
            "artifact_identities": historical_rows,
        },
        "world_size": 4,
    ...
    historical_receipt = {
        **unsigned,
        "receipt_digest": runner.object_sha256(unsigned),
    }
    receipt_path = root / "compile-smoke-receipt.json"
    receipt_path.write_bytes(runner.canonical_json_bytes(historical_receipt) + b"\n")
    authority = runner.load_r10_tensor_parity_authority(
        receipt_path,
        _sha(receipt_path),
        generation_log,
        _sha(generation_log),
...
def _write_host_memory_gate_fixture(
    ...
    *,
    ...
    governing_path = "/sys/fs/cgroup/slurm/job_136141/step_1/user"
    leaf_path = f"{governing_path}/task_0"
    cgroup = {
        "cgroup_version": 2,
        "leaf_cgroup": {
            "relative_path": "/slurm/job_136141/step_1/user/task_0",
            "path": leaf_path,
            "device": 31,
            "inode": 4100,
            "memory_max": "max",
        ...
        "governing_cgroup": {
            "relative_path": "/slurm/job_136141/step_1/user",
            "path": governing_path,
            "device": 31,
            "inode": 4000,
        ...
        "measurement_files": {
            "memory_current": f"{governing_path}/memory.current",
            "memory_max": f"{governing_path}/memory.max",
            "memory_events": f"{governing_path}/memory.events",
        },
    }
    ...
    unsigned_start = {
        ...
        **cgroup,
        ...
        "stop_token_path": str(root / "host-memory-monitor-stop"),
...
class GenericActionDataPrepReleaseTests(unittest.TestCase):
    ...
    def test_release_is_deterministic_exact_fit40_and_data_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(value["release_generation"], "r13")
            self.assertEqual(value["external_evidence"], release.R10_PARITY_EVIDENCE)
    ...
    def test_release_excludes_phi_review_manifest_and_action_capabilities(self) -> None:
        ...
        self.assertEqual(
            set(release.ENTRYPOINTS),
            {
                ...
                "scripts/auh_generic_action_data_prep_136141_world4_v1.sh",
            },
        )
        self.assertIn(
...
class GenericActionDataPrepControllerTests(unittest.TestCase):
    @staticmethod
    def _write_cgroup_hierarchy(
        root: Path,
        *,
        relative: str = "/slurm/job_136141/step_1/user/task_0",
        finite_maxima: dict[str, int] | None = None,
    ) -> tuple[Path, Path, Path]:
        mount = root / "cgroup"
        mount.mkdir(parents=True)
        proc_cgroup = root / "proc-self-cgroup"
        proc_cgroup.write_text(f"0::{relative}\n", encoding="ascii")
        maxima = finite_maxima or {}
        current = mount
        paths = [("/", mount)]
        parts: list[str] = []
        for component in relative.split("/")[1:]:
            ...
            current.mkdir()
            paths.append(("/" + "/".join(parts), current))
        for cgroup_relative, path in paths:
            maximum = maxima.get(cgroup_relative)
            (path / "memory.max").write_text(
                "max" if maximum is None else str(maximum), encoding="ascii"
            )
            (path / "memory.current").write_text(
                str(8 * 1024**3), encoding="ascii"
            )
            (path / "memory.events").write_text(
                "low 0\nhigh 0\nmax 0\noom 0\noom_kill 0\n",
                encoding="ascii",
            )
        return mount, proc_cgroup, current

    def test_auh_leaf_max_step_user60_job64_nearest_governing_sampler(
        self,
    ) -> None:
        runner = controller.reserve4_runner
        with tempfile.TemporaryDirectory() as directory:
            ...
            governing_relative = (
                ...
                "job_136141/step_1/user"
            )
            leaf_relative = f"{governing_relative}/task_0"
            mount, proc_cgroup, leaf = self._write_cgroup_hierarchy(
                root,
                relative=leaf_relative,
                finite_maxima={
                    "/system.slice/slurmstepd.scope/job_136141": (
                        64 * 1024**3
                    ),
                    governing_relative: runner.HOST_MEMORY_LIMIT_BYTES,
                },
            )
            # AUH's cgroup-v2 mount root has no memory.max; only non-root
            # cgroups expose it.  The valid fixture preserves that topology.
            ...
            governing = leaf.parent
            self.assertEqual(binding["leaf_cgroup"]["path"], str(leaf))
            self.assertEqual(
                binding["leaf_cgroup"]["memory_max"], "max"
            )
            self.assertEqual(
                binding["governing_cgroup"]["path"], str(governing)
            )
            self.assertEqual(
                binding["governing_cgroup"]["memory_max_bytes"],
                runner.HOST_MEMORY_LIMIT_BYTES,
            )
            self.assertEqual(
                binding["leaf_cgroup"]["device"], leaf.stat().st_dev
            )
            self.assertEqual(
                binding["governing_cgroup"]["inode"],
                governing.stat().st_ino,
            )
            runner._validate_cgroup_binding_descriptor(
                {
                    **binding,
                    "slurm_job_id": "136141",
                    "slurm_step_id": "1",
            ...
            for case in ("wrong-max", "at-safe-ceiling", "oom", "oom-kill"):
                with self.subTest(case=case):
                    ...
                    (governing / "memory.max").write_text(
                        str(
                            59 * 1024**3
                            if case == "wrong-max"
                    ...
                    (governing / "memory.events").write_text(
                        "low 0\nhigh 0\nmax 0\noom %d\noom_kill %d\n"
    ...
    def test_cgroup_governing_ancestor_rejects_job_leaf_symlink_and_step_drift(
        self,
    ) -> None:
        runner = controller.reserve4_runner
        with tempfile.TemporaryDirectory() as directory:
            ...
            auh_batch = root / "auh-batch-snapshot"
            batch_user = (
                "/system.slice/slurmstepd.scope/"
                "job_136141/step_batch/user"
            )
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                auh_batch,
                relative=f"{batch_user}/task_0",
                finite_maxima={
                    "/system.slice/slurmstepd.scope/job_136141": (
                        64 * 1024**3
                    ),
                    batch_user: 64 * 1024**3,
                },
            )
            (mount / "memory.max").unlink()
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError,
                "nearest finite governing ancestor",
            ):
            ...
            job_only = root / "job-only"
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                job_only,
                finite_maxima={
                    "/slurm/job_136141": 64 * 1024**3,
                },
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError,
            ...
            job_drift = root / "job-drift"
            job_drift_relative = "/slurm/job_999/step_1/user/task_0"
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                job_drift,
                relative=job_drift_relative,
                finite_maxima={
                    "/slurm/job_999/step_1/user": (
                        runner.HOST_MEMORY_LIMIT_BYTES
                    ),
                },
            )
            ...
            outer_lower = root / "outer-lower"
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                outer_lower,
                finite_maxima={
                    "/slurm/job_136141": 59 * 1024**3,
                    "/slurm/job_136141/step_1/user": (
                        runner.HOST_MEMORY_LIMIT_BYTES
                    ),
                },
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError, "below 60 GiB"
            ):
                runner._discover_live_cgroup_v2(
                    slurm_job_id="136141",
                    slurm_step_id="1",
            ...
            missing_nonroot = root / "missing-nonroot-max"
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                missing_nonroot,
                finite_maxima={
                    "/slurm/job_136141/step_1/user": (
            ...
            (mount / "slurm/job_136141/step_1/memory.max").unlink()
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError, "absent outside the cgroup root"
            ):
                runner._discover_live_cgroup_v2(
                    slurm_job_id="136141",
                    slurm_step_id="1",
            ...
            arbitrary = root / "arbitrary-ancestor"
            arbitrary_relative = (
                "/slurm/job_136141/step_1/user/delegated/task_0"
            )
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                arbitrary,
                relative=arbitrary_relative,
                finite_maxima={
                    "/slurm/job_136141/step_1/user": (
                        runner.HOST_MEMORY_LIMIT_BYTES
                    ),
                    "/slurm/job_136141/step_1/user/delegated": (
                        runner.HOST_MEMORY_LIMIT_BYTES
                    ),
                },
            )
            ...
            finite_leaf = root / "finite-leaf"
            leaf_relative = "/slurm/job_136141/step_1/user/task_0"
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                finite_leaf,
                relative=leaf_relative,
                finite_maxima={
                    ...
                    leaf_relative: runner.HOST_MEMORY_LIMIT_BYTES,
                },
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError, "strict ancestor"
            ):
                runner._discover_live_cgroup_v2(
                    slurm_job_id="136141",
                    slurm_step_id="1",
            ...
            drift = root / "step-drift"
            drift_relative = "/slurm/job_136141/step_2/user/task_0"
            mount, proc_cgroup, _ = self._write_cgroup_hierarchy(
                drift,
                relative=drift_relative,
                finite_maxima={
                    "/slurm/job_136141/step_2/user": (
                        runner.HOST_MEMORY_LIMIT_BYTES
                    ),
                },
            )
            ...
            proc_cgroup.write_text(
                "0::/slurm/job_136141/step_1/user/../user/task_0\n",
                encoding="ascii",
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError, "canonical cgroup-v2 leaf"
            ):
                runner._discover_live_cgroup_v2(
                    slurm_job_id="136141",
                    slurm_step_id="1",
            ...
            symlink_root.mkdir()
            mount = symlink_root / "cgroup"
            user = mount / "slurm/job_136141/step_1/user"
            user.mkdir(parents=True)
            target = symlink_root / "target-task"
            target.mkdir()
            (user / "task_0").symlink_to(target, target_is_directory=True)
            proc_cgroup = symlink_root / "proc-self-cgroup"
            proc_cgroup.write_text(
                "0::/slurm/job_136141/step_1/user/task_0\n",
                encoding="ascii",
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError, "non-directory/symlink"
            ):
                runner._discover_live_cgroup_v2(
                    slurm_job_id="136141",
                    slurm_step_id="1",
    ...
    def test_host_cgroup_sampled_gate_rejects_old_missing_resigned_and_unsafe(self) -> None:
        runner = controller.reserve4_runner
        self.assertEqual(
            runner.HOST_CGROUP_MEMORY_MONITOR_START_SCHEMA,
            "bernini-generic-action-fit40-host-cgroup-memory-monitor-start-v2",
        )
        self.assertEqual(
            runner.HOST_CGROUP_MEMORY_GATE_SCHEMA,
            "bernini-generic-action-fit40-host-cgroup-sampled-memory-gate-v3",
        )
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            live_drift = {
                "cgroup_version": valid["cgroup_version"],
                "leaf_cgroup": json.loads(json.dumps(valid["leaf_cgroup"])),
                "governing_cgroup": valid["governing_cgroup"],
                "measurement_files": valid["measurement_files"],
            }
            live_drift["leaf_cgroup"]["inode"] += 1
            with mock.patch.object(
                runner, "_discover_live_cgroup_v2", return_value=live_drift
            ), self.assertRaisesRegex(
                runner.Reserve4GenerationError, "cgroup binding drifted"
            ):
                runner.load_host_cgroup_memory_gate(
                    compile_path,
                    _sha(compile_path),
            ...
            for case in (
                ...
                "at-safe-ceiling", "oom", "oom-kill", "leaf-inode",
                "governing-inode", "governing-device", "governing-max",
                "step-drift",
                "extra", "bad-digest",
            ):
                with self.subTest(case=case):
                    ...
                    elif case == "leaf-inode":
                        hostile["leaf_cgroup"]["inode"] += 1
                    elif case == "governing-inode":
                        hostile["governing_cgroup"]["inode"] += 1
                    elif case == "governing-device":
                        hostile["governing_cgroup"]["device"] += 1
                    elif case == "governing-max":
                        hostile["governing_cgroup"]["memory_max_bytes"] = (
                            64 * 1024**3
                        )
                    elif case == "step-drift":
                        hostile["governing_cgroup"]["slurm_step_id"] = "2"
                    elif case == "extra":
                    ...
                    if case in {
                        "resigned-peak", "leaf-inode", "governing-inode"
                    }:
    ...
    def test_host_gate_requires_explicit_live_tail_and_terminal_stop_marker(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                compile_gate["monitor_stop_token_path"],
                start["stop_token_path"],
            )
            start_path = (
    ...
    def test_terminal_host_cgroup_receipt_rejects_resigned_regression(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for case in (
                "old-schema", "resigned-peak", "oom-kill", "leaf-inode",
                "governing-max", "journal-tamper",
            ):
                with self.subTest(case=case):
    ...
    def test_live_journal_tail_recheck_is_fresh_and_fail_closed(self) -> None:
        runner = controller.reserve4_runner
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory).resolve()
            start, _, _, _, _ = _write_host_memory_gate_fixture(root)
            ...
            def write_tail(latest_monotonic_ns: int) -> None:
                wall = time.time_ns()
                rows = (
                    (
                        0,
                        wall - runner.HOST_MEMORY_SAMPLE_INTERVAL_NS,
                        latest_monotonic_ns
                        - runner.HOST_MEMORY_SAMPLE_INTERVAL_NS,
                        8 * 1024**3,
                        runner.HOST_MEMORY_LIMIT_BYTES,
                        0,
                        0,
                ...
                self.assertEqual(journal.stat().st_size, 2 * record_size)

            write_tail(time.monotonic_ns())
            observed = runner._assert_fresh_live_journal_tail(start)
            self.assertLessEqual(
                observed["observed_tail_age_ns"],
                runner.HOST_MEMORY_MAX_SAMPLE_GAP_NS,
            )
            write_tail(
                time.monotonic_ns() - runner.HOST_MEMORY_MAX_SAMPLE_GAP_NS - 1
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError,
                "tail/cadence/safety gate failed",
            ):
                runner._assert_fresh_live_journal_tail(start)
    ...
    def test_controller_plan_is_fixed_fit40_all8_serial_four_world4(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            old_plan["schema_version"] = (
                "bernini-generic-action-fit40-generation-136141-plan-v10"
            )
    ...
    def test_launcher_environment_is_exactly_cross_bound_to_plan(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            environment = {
                ...
                "GADP_R10_COMPILE_SMOKE_RECEIPT": (
                    plan["r10_parity_evidence"]["compile_smoke_receipt"]["path"]
                ),
                "GADP_R10_COMPILE_SMOKE_RECEIPT_SHA256": (
                    plan["r10_parity_evidence"]["compile_smoke_receipt"]
                    ["file_sha256"]
                ),
                "GADP_R10_GENERATION_LOG": (
                    plan["r10_parity_evidence"]["generation_log"]["path"]
                ),
                "GADP_R10_GENERATION_LOG_SHA256": (
                    plan["r10_parity_evidence"]["generation_log"]
                    ["file_sha256"]
                ),
            }
            with mock.patch.object(
    ...
    def _write_gpu_admission(run_root: Path) -> None:
        ...
        current_root = run_root.parent / f"{run_root.name}-current-smoke"
        current_native, _, _ = _materialize_fake_current_native_receipt(
            current_root
        )
        authority_root = run_root.parent / f"{run_root.name}-r10-authority"
        r10_native = _native_tensor_parity_receipt(
            authority_root,
            gaussian_container_sha256="3" * 64,
            clean_container_sha256="4" * 64,
            gaussian_raw_storage_sha256=current_native[
                "initial_noise_artifacts"
            ]["t2v"]["raw_value_sha256"],
            gaussian_content_sha256=current_native[
                "initial_noise_artifacts"
            ]["t2v"]["content_sha256"],
            clean_raw_storage_sha256=current_native[
                "generated_identities"
            ]["t2v"]["identity"]["raw_storage_sha256"],
            clean_content_sha256=current_native[
                "generated_identities"
            ]["t2v"]["identity"]["content_sha256"],
        )
        r10_authority, _, _, _, _ = _write_r10_tensor_parity_authority_fixture(
            authority_root,
            native_receipt=r10_native,
        )
        tensor_evidence = (
            controller.reserve4_runner._native_smoke_tensor_evidence(
                current_native,
                safe_open_factory=_FakeSafeOpen,
                tensor_identity_fn=_fake_tensor_identity,
            )
        )
        smoke_unsigned = {
            ...
            "runtime": {
                ...
                "host_cgroup_sampled_memory_monitor": {
                    ...
                    "stop_token_path": host_start["stop_token_path"],
            ...
            "candidate_evidence": {
                ...
                "native_receipt_digest": current_native["receipt_digest"],
                "resource_lifecycle": _valid_resource_lifecycle(),
    ...
    def test_completion_never_authorizes_confirmation_phi_po_or_training(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                completion["compile_smoke_host_leaf_cgroup"],
                completion["terminal_host_leaf_cgroup"],
            )
            self.assertEqual(
                completion["compile_smoke_host_governing_cgroup"],
                completion["terminal_host_governing_cgroup"],
            )
            self.assertEqual(
                completion["terminal_host_leaf_cgroup"]["memory_max"],
                "max",
            )
            self.assertEqual(
                completion["terminal_host_governing_cgroup"][
                ...
                controller.HOST_MEMORY_LIMIT_BYTES,
            )
    ...
    def test_r13_physically_reopens_current_tensors_before_r10_parity(self) -> None:
        runner = controller.reserve4_runner
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory).resolve()
            current_native, gaussian_spec, clean_spec = (
                _materialize_fake_current_native_receipt(root / "current")
            )
            r10_root = root / "r10"
            r10_native = _native_tensor_parity_receipt(
                r10_root,
                gaussian_container_sha256="1" * 64,
                clean_container_sha256="2" * 64,
                gaussian_raw_storage_sha256=current_native[
                    "initial_noise_artifacts"
                ]["t2v"]["raw_value_sha256"],
                gaussian_content_sha256=current_native[
            ...
            (
                ...
                generation_log,
                generation_log_sha,
            ) = _write_r10_tensor_parity_authority_fixture(
                r10_root,
                native_receipt=r10_native,
            )
            self.assertEqual(
                authority["source_evidence"]["compile_smoke_receipt"],
                {
            ...
            self.assertEqual(
                authority["source_evidence"]["generation_log"]["file_sha256"],
                generation_log_sha,
            )
            self.assertEqual(
                authority["source_evidence"]["generation_log"]["path"],
                str(generation_log),
            )

            current = runner._native_smoke_tensor_evidence(
                current_native,
                safe_open_factory=_FakeSafeOpen,
                tensor_identity_fn=_fake_tensor_identity,
            )
            self.assertTrue(current["physical_safetensors_reopen"]["performed"])
            self.assertEqual(
                [row["name"] for row in current["physical_safetensors_reopen"]["artifacts"]],
                ["official_initial_gaussian", "predecode_clean_latent"],
            )
            self.assertNotEqual(
                current["official_initial_gaussian"]["artifact_binding"]
                ["container_file_sha256"],
                authority["tensor_evidence"]["official_initial_gaussian"]
                ["artifact_binding"]["container_file_sha256"],
            )
            self.assertNotEqual(
                current["predecode_clean_latent"]["artifact_binding"]
                ["container_file_sha256"],
                authority["tensor_evidence"]["predecode_clean_latent"]
                ["artifact_binding"]["container_file_sha256"],
            )
            runner._validate_r10_smoke_tensor_parity(current, authority)

            # Re-serialize both containers with different header bytes while
            # preserving exact tensors/metadata.  Per-run container SHA changes,
            # but physical tensor parity remains valid.
            gaussian_path = Path(
                current_native["initial_noise_artifacts"]["t2v"]["path"]
            )
            clean_path = Path(
                current_native["outputs"]["t2v"]["normalized_clean_latent"]["path"]
            )
            rewritten = json.loads(json.dumps(current_native))
            rewritten["initial_noise_artifacts"]["t2v"]["sha256"] = (
            ...
            rewritten["outputs"]["t2v"]["normalized_clean_latent"]["sha256"] = (
                _write_fake_safetensors(
                    clean_path,
                    tensors={"normalized_clean_latent": clean_spec},
                    metadata=dict(runner.CLEAN_LATENT_SAFETENSORS_METADATA),
            ...
            rewritten = _resign(rewritten)
            rewritten_evidence = runner._native_smoke_tensor_evidence(
                rewritten,
                safe_open_factory=_FakeSafeOpen,
                tensor_identity_fn=_fake_tensor_identity,
            )
            runner._validate_r10_smoke_tensor_parity(rewritten_evidence, authority)

            # Hostile current tensor bytes are rewritten and every native
            # declaration/container/digest is consistently re-signed.  It must
            # still fail the independently bound r10 tensor-value authority.
            tampered = json.loads(json.dumps(rewritten))
            tampered_spec = _fake_tensor_spec(73)
            tampered_identity = _fake_tensor_identity(
                tampered_spec, label="official_initial_gaussian_t2v"
            )
            gaussian = tampered["initial_noise_artifacts"]["t2v"]
            gaussian["sha256"] = _write_fake_safetensors(
                gaussian_path,
                tensors={"official_initial_gaussian": tampered_spec},
                metadata=dict(runner.GAUSSIAN_SAFETENSORS_METADATA),
            )
            gaussian["raw_value_sha256"] = tampered_identity["raw_storage_sha256"]
            gaussian["tensor_value_sha256"] = tampered_identity[
                "raw_storage_sha256"
            ]
            gaussian["content_sha256"] = tampered_identity["content_sha256"]
            gaussian["shape"] = tampered_identity["shape"]
            gaussian["dtype"] = tampered_identity["dtype"]
            gaussian["stored_dtype"] = tampered_identity["dtype"]
            gaussian["numel"] = tampered_identity["numel"]
            gaussian["byte_count"] = tampered_identity["byte_count"]
            gaussian["all_rank_identity"]["identity"] = tampered_identity
            tampered = _resign(tampered)
            tampered_evidence = runner._native_smoke_tensor_evidence(
                tampered,
                safe_open_factory=_FakeSafeOpen,
                tensor_identity_fn=_fake_tensor_identity,
            )
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError,
                "r13 compile smoke differs from r10",
            ):
                runner._validate_r10_smoke_tensor_parity(
                    tampered_evidence, authority
                )

    def test_r13_physical_reopen_rejects_keys_dtype_shape_nonfinite_and_metadata(self) -> None:
        runner = controller.reserve4_runner
        cases = (
            "wrong-key",
            "extra-key",
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
                root = Path(directory).resolve()
                native, gaussian_spec, _ = _materialize_fake_current_native_receipt(root)
                gaussian = native["initial_noise_artifacts"]["t2v"]
                path = Path(gaussian["path"])
                key = "official_initial_gaussian"
                tensors = {key: gaussian_spec}
                metadata = dict(runner.GAUSSIAN_SAFETENSORS_METADATA)
                if case == "wrong-key":
                    tensors = {"wrong": gaussian_spec}
                elif case == "extra-key":
                    tensors = {key: gaussian_spec, "extra": gaussian_spec}
                elif case == "fp16":
                    tensors = {key: _fake_tensor_spec(17, dtype="torch.float16")}
                elif case == "shape":
                    tensors = {key: _fake_tensor_spec(17, shape=[1, 4, 3, 2, 3])}
                elif case in {"nan", "inf"}:
                    tensors = {
                        key: _fake_tensor_spec(
                            17,
                            finite=False,
                            nonfinite_kind=case,
                        )
                    ...
                    metadata["coordinate"] = "hostile-rewritten-coordinate"
                    gaussian["coordinate"] = metadata["coordinate"]
                gaussian["sha256"] = _write_fake_safetensors(
                    path, tensors=tensors, metadata=metadata
                )
                native = _resign(native)
                with self.assertRaises(runner.Reserve4GenerationError):
                    runner._native_smoke_tensor_evidence(
                        native,
                        safe_open_factory=_FakeSafeOpen,
                        tensor_identity_fn=_fake_tensor_identity,
                    )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            runtime = {
                ...
                "host_cgroup_sampled_memory_monitor": {
                    ...
                    "stop_token_path": monitor_start["stop_token_path"],
            ...
            native_receipt_path = root / "native-receipt.json"
            current_native, _, _ = _materialize_fake_current_native_receipt(root)
            current_tensor_evidence = runner._native_smoke_tensor_evidence(
                current_native,
                safe_open_factory=_FakeSafeOpen,
                tensor_identity_fn=_fake_tensor_identity,
            )
            authority_root = root / "r10-authority"
            r10_native = _native_tensor_parity_receipt(
                authority_root,
                gaussian_container_sha256="3" * 64,
                clean_container_sha256="4" * 64,
            ...
            native_receipt_path.write_bytes(
                runner.canonical_json_bytes(current_native) + b"\n"
            )
            pair_receipt = {
                "receipt_digest": "a" * 64,
                "native_receipt_sha256": _sha(native_receipt_path),
                "native_receipt_digest": current_native["receipt_digest"],
                "native_receipt_path": str(native_receipt_path),
                "artifacts": {
            ...
            args = Namespace(
                ...
                r10_compile_smoke_receipt=str(r10_receipt_path),
                expected_r10_compile_smoke_receipt_sha256=r10_receipt_sha,
                r10_generation_log=str(r10_log_path),
                expected_r10_generation_log_sha256=r10_log_sha,
            )
            with mock.patch.dict(
                ...
                return_value=current_tensor_evidence,
            ), mock.patch.object(
                runner,
            ...
            self.assertTrue(
                receipt["physical_safetensors_safe_open_recomputation_required"]
            )
            self.assertEqual(receipt["r10_tensor_parity_authority"], r10_authority)
            self.assertNotEqual(
                receipt["candidate_evidence"]["tensor_parity_evidence"]
                ["official_initial_gaussian"]["artifact_binding"]
                ["container_file_sha256"],
                r10_authority["tensor_evidence"]
                ["official_initial_gaussian"]["artifact_binding"]
                ["container_file_sha256"],
            )
            ...
            gaussian = parity_tamper["candidate_evidence"][
                "tensor_parity_evidence"
            ]["official_initial_gaussian"]
            gaussian["raw_storage_sha256"] = "f" * 64
            gaussian["tensor_value_sha256"] = "f" * 64
            gaussian["artifact_binding"]["raw_value_sha256"] = "f" * 64
            ...
            with self.assertRaisesRegex(
                runner.Reserve4GenerationError,
                "physical-reopen tensor is not bound to smoke evidence",
            ):
                runner.load_compile_smoke_receipt(receipt_path, _sha(receipt_path))
            receipt_path.write_bytes(original_receipt_bytes)
    ...
    def test_live_gpu299_topology_fixture_passes_and_malformed_hostile_fails(self) -> None:
        source = LAUNCHER.read_text(encoding="utf-8")
        self.assertIn(f"awk '{LINK_MATRIX_AWK}'", source)
        old_rows = subprocess.run(
            ["awk", r'$1~/^GPU[0-7]$/ {print}'],
            input=LIVE_GPU299_ROCM_SMI_TOPOLOGY,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            check=True,
        ...
        self.assertEqual(
            self._live_topology_counts(LIVE_GPU299_ROCM_SMI_TOPOLOGY),
            (8, 24, 32, 4, 4),
        )
        malformed = LIVE_GPU299_ROCM_SMI_TOPOLOGY.replace(
    ...
    def test_launcher_uses_all8_for_one_serial_four_shard_child_and_is_parent_safe(self) -> None:
        ...
        for option in (
            "--r10-compile-smoke-receipt",
            "--expected-r10-compile-smoke-receipt-sha256",
            "--r10-generation-log",
            "--expected-r10-generation-log-sha256",
        ):
        ...
        self.assertIn("GENERIC_ACTION_FIT40_GENERATION_V13_COMPLETE", source)
        ...
        self.assertIn(
            "r10_smoke_mp4_whole_file_sha256_exact_required=true", source
        )
        self.assertIn(
            "r10_smoke_gaussian_tensor_identity_exact_required=true", source
        )
        self.assertIn(
        ...
        self.assertIn(
            "current_smoke_exact_single_tensor_key_required=true", source
        )
        self.assertIn(
            "current_smoke_exact_safetensors_metadata_required=true", source
        )
        self.assertIn(
        ...
        self.assertIn("host_cgroup_current_pid_leaf_identity_bound=true", source)
        self.assertIn("host_cgroup_leaf_memory_max_inherited=true", source)
        self.assertIn(
            "host_cgroup_governing_ancestor_nearest_finite=true", source
        )
        self.assertIn(
            "host_cgroup_governing_scope_exact_slurm_step_user=true", source
        )
        ...
        self.assertIn("--require-live-child-cgroup", source)
