...
import full30_action_confirmation8_plan_v1 as plan  # noqa: E402
import full30_action_confirmation8_generator_v1 as generator  # noqa: E402
import full30_action_confirmation8_controller_v1 as controller  # noqa: E402
import build_full30_action_confirmation8_release_v1 as release  # noqa: E402
    ...
    return _sign(
        {
            ...
            "review_population": "confirmation_action_anchor_exact8",
            "reviewer_independent_of_generator": True,
            "reviewer_independent_of_materializer": True,
            "candidate_count": 8,
            "candidate_reviews": rows,
            "generated_diagnostic_review_required": False,
            "generated_diagnostic_admitted": False,
        }
    )
    ...
    return _sign(
        {
            ...
            "generated_diagnostic_used_by_q": False,
            "generated_diagnostic_used_by_a_min": False,
            "generated_diagnostic_used_by_optimizer": False,
...
class Confirmation8PlanTests(unittest.TestCase):
    def test_exact_partition_is_four_action_incomplete_pairs(self) -> None:
        exact8, diagnostics, cells = plan.partition_confirmation_tasks(
            _fake_source_tasks()
        )
        ...
        self.assertEqual(
            [row["semantic_branch"] for row in diagnostics[:8]],
            list(plan.DIAGNOSTIC_BRANCH_ORDER),
        )
        self.assertFalse(
            {row["candidate_id"] for row in exact8}
    ...
    def test_plan_declares_diagnostic_lane_non_authoritative(self) -> None:
        value = _fake_plan()
        contract = value["execution_contract"]
        self.assertEqual(value["admission_candidate_count"], 8)
        self.assertEqual(value["diagnostic_candidate_count"], 32)
        for field in (
            "optional_diagnostic_generation_is_admission_prerequisite",
            "optional_diagnostic_generation_is_q_input",
            "optional_diagnostic_generation_is_a_min_input",
            "optional_diagnostic_generation_is_optimizer_input",
        ):
            self.assertFalse(contract[field], field)
        self.assertFalse(
            contract["generated_noop_camera_appearance_wrong_controls_admitted"]
        )
        self.assertEqual(
...
class Confirmation8GeneratorTests(unittest.TestCase):
    def test_lane_projection_never_mixes_diagnostics_into_exact8(self) -> None:
        value = _fake_plan()
        admission = generator._lane_tasks(value, "admission")
        diagnostics = generator._lane_tasks(value, "diagnostic")
        self.assertEqual(len(admission), 8)
        self.assertEqual(len(diagnostics), 32)
        hostile = copy.deepcopy(value)
        hostile["admission_tasks"].append(hostile["diagnostic_tasks"][0])
        with self.assertRaisesRegex(
            generator.Confirmation8GenerationError, "task count"
        ):
            generator._lane_tasks(hostile, "admission")
    ...
    def test_release_specialization_is_only_136141_to_136309(self) -> None:
        ...
        self.assertEqual(specialized.count(b"136309"), 7)
        self.assertEqual(
            hashlib.sha256(specialized).hexdigest(),
            generator.RESOURCE_SPECIALIZED_SHA256,
        )
        self.assertEqual(specialized.replace(b"136309", b"136141"), source)
        with self.assertRaisesRegex(
...
class Confirmation8AdmissionTests(unittest.TestCase):
    def test_review_requires_each_action_and_incomplete_full81(self) -> None:
        ...
        hostile = copy.deepcopy(receipt)
        hostile.pop("receipt_digest")
        hostile["candidate_reviews"][0]["candidate_id"] = exact8[
            "diagnostic_tasks"
        ][0]["candidate_id"]
        hostile = _sign(hostile)
        with self.assertRaisesRegex(
    ...
    def test_materializer_gate_is_same_state_and_generated_negative_free(self) -> None:
        exact8 = _fake_plan()
        receipt = _materializer_gate(exact8)
        controller.validate_materializer_gate(receipt, exact8)
        hostile = copy.deepcopy(receipt)
        hostile.pop("receipt_digest")
        hostile["generated_diagnostic_used_by_q"] = True
        hostile = _sign(hostile)
        with self.assertRaisesRegex(
...
class Confirmation8ReleaseAndLauncherTests(unittest.TestCase):
    def test_release_is_deterministic_exact_closure_and_exact8_authority(self) -> None:
        ...
        self.assertFalse(
            manifest["authority"][
                "generated_diagnostic_is_admission_prerequisite"
            ]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            manifest["component_pins"][
                "resource_136309_specialization_sha256"
            ],
    ...
    def test_launcher_runs_only_four_exact2_admission_shards(self) -> None:
        ...
        self.assertIn("run-sp4 --lane admission", source)
        self.assertNotIn("run-sp4 --lane diagnostic", source)
        self.assertIn("formal_candidate_count=8", source)
        self.assertIn("optional_diagnostic_invocations=0", source)
        ...
        self.assertIn("r10_parity=true", source)
        for forbidden in (
            "scancel",
            "scontrol release",
            "scontrol requeue",
