...
import full644_exploratory_matched_torchrun_fd_bridge_gpu47_v3 as bridge
...
class TorchrunFDBridgeV2Tests(unittest.TestCase):
    def test_coordinator_environment_is_an_exact_empty_base_allowlist(self) -> None:
        ...
        for key in bridge._SECONDARY_GPU_MASKS:
        ...
        with mock.patch.object(
            ...
            mapping = bridge.gpu47_visibility_contract(observed)
            self.assertEqual(mapping["physical_gpu_indices"], [4, 5, 6, 7])
            self.assertEqual(mapping["logical_gpu_indices"], [0, 1, 2, 3])
            for raw in ("0,1,2,3", "4-7", "7,6,5,4"):
                hostile = dict(environment)
                hostile["ROCR_VISIBLE_DEVICES"] = raw
                with self.subTest(rocr=raw), self.assertRaises(
                    bridge.TorchrunFDBridgeV2Error
                ):
                    bridge.gpu47_visibility_contract(hostile)
        source = Path(bridge.__file__).read_text(encoding="utf-8")
        main_source = source[source.index("def main(") :]
        self.assertLess(
            main_source.index("gpu_visibility = gpu47_visibility_contract"),
            main_source.index("_load_pinned_torchrun("),
        )
        self.assertNotIn("import torch", main_source[: main_source.index(
    ...
    def test_rank_environment_exact_world4_and_hostiles(self) -> None:
        ...
        static_tcp_store = dict(environment)
        static_tcp_store["TORCHELASTIC_USE_AGENT_STORE"] = "True"
        hostile.append(static_tcp_store)
