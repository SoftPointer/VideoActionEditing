#!/usr/bin/env python3
"""Frozen-SAM2 review-only source/target graph observation scaffold, r2.
...
SPEC_SCHEMA = "crosscase-frozen-sam2-graph-teacher-spec-v2"
GRAPH_SCHEMA = "target-graph-teacher-observation-scaffold-v2"
EVENT_SCHEMA = "target-graph-teacher-event-scaffold-v2"
DELTA_SCHEMA = "target-graph-teacher-source-target-delta-v2"
RECEIPT_SCHEMA = "target-graph-teacher-observer-receipt-v2"
...
def _validate_spec(spec: Mapping[str, Any]) -> None:
    ...
    for node in nodes:
        if not isinstance(node, dict) or set(node) != {
            ...
            "observation_mode", "source_prompt", "real_target_teacher_prompt",
            "color_bgr",
        }:
        ...
        observation_mode = node["observation_mode"]
        if observation_mode not in {"tracked", "persistent_unknown"}:
            raise GraphTeacherObserverError(f"{node_id} observation mode differs")
        if observation_mode == "tracked" and (
            node["source_prompt"] is None
            and node["real_target_teacher_prompt"] is None
        ):
            raise GraphTeacherObserverError(
                f"{node_id} tracked mode requires at least one prompt"
            )
        if observation_mode == "persistent_unknown" and (
            node["source_prompt"] is not None
            or node["real_target_teacher_prompt"] is not None
        ):
            raise GraphTeacherObserverError(
                f"{node_id} persistent-unknown mode forbids prompts"
            )
...
def _phase_status(
    prompt: Any, frame: int, geometry: Any, observation_mode: str
) -> str:
    if observation_mode == "persistent_unknown":
        return "unresolved_persistent_unknown"
    if prompt is None:
...
def _track_media(
    ...
    for node in nodes:
        ...
        statuses = [
            _phase_status(prompt, frame, geometry, node["observation_mode"])
    ...
    for edge in spec_edges(nodes):
        ...
        for phase_index, frame in enumerate(PHASE_FRAMES):
            ...
            row: dict[str, Any] = {
                ...
                "relative_dx_norm": None,
                "relative_dy_norm": None,
                "relative_dx_delta_from_first_joint_observation": None,
                "relative_dy_delta_from_first_joint_observation": None,
                "relative_dx_phase_velocity": None,
                "relative_dy_phase_velocity": None,
                "overlap_iou": None,
                "near_candidate": None,
            ...
            if left_geometry is not None and right_geometry is not None:
                ...
                if baseline_gap is None:
                    baseline_gap = gap
                relative = (
                    float((left_geometry["center"][0] - right_geometry["center"][0]) / 960.0),
                    float((left_geometry["center"][1] - right_geometry["center"][1]) / 540.0),
                )
                ...
                velocity = (
                    ...
                    if previous_relative is None
                        ...
                        float(relative[0] - previous_relative[0]),
                        float(relative[1] - previous_relative[1]),
                    )
                )
                row.update({
                    ...
                    "relative_dx_norm": relative[0],
                    "relative_dy_norm": relative[1],
                    "relative_dx_delta_from_first_joint_observation": float(
                        relative[0] - baseline_relative[0]
                    ),
                    "relative_dy_delta_from_first_joint_observation": float(
                        relative[1] - baseline_relative[1]
                    ),
                    "relative_dx_phase_velocity": velocity[0],
                    "relative_dy_phase_velocity": velocity[1],
                ...
                # Do not bridge velocity across an occlusion or an explicitly
                # unobserved registry node.
                previous_relative = None
            phases.append(row)
        edge_records.append({
            **edge,
    ...
    for target_edge in target["edges"]:
        ...
        for left, right in zip(source_edge["phases"], target_edge["phases"]):
            ...
            phases.append({
                ...
                "target_minus_source_relative_dx_norm": (
                    float(right["relative_dx_norm"] - left["relative_dx_norm"])
                    if observed else None
                ),
                "target_minus_source_relative_dy_norm": (
                    float(right["relative_dy_norm"] - left["relative_dy_norm"])
                    if observed else None
                ),
            })
        rows.append({
