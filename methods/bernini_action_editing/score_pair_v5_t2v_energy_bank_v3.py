...
The formal v4 primary scalar is recomputed from the exact FP32 branch-energy
scalars by a sealed 80-digit Decimal contract.  The origin-device stable
``torch.log1p`` MACE result and the phase-conjunctive minimum are emitted only
as diagnostics; neither is calibration truth.  Score receipts contain
scalar/text/hash provenance but no MP4, media path, source, RV2V target,
donor, mask, flow, pose, track, or trajectory.  This executable performs no
training and makes no action-editing-success claim.
"""
...
SCORE_RECEIPT_FILENAME = "pair-v5-t2v-global-energy-score-v4.json"
GROUP_RECEIPT_FILENAME = "pair-v5-t2v-global-energy-{group_id}-v4.json"
CANONICAL_ACTION_ENERGY_ARITHMETIC_SCHEMA = (
    "bernini-mace-canonical-decimal-action-energy-v1"
)
CANONICAL_ACTION_ENERGY_PACKET_SCHEMA = (
    "bernini-mace-canonical-action-energy-packet-v4"
)
CANONICAL_ACTION_ENERGY_DECIMAL_PRECISION = 80
CANONICAL_ACTION_ENERGY_EPSILON_DECIMAL = "1e-8"
CANONICAL_ACTION_ENERGY_DECIMAL_EMIN = -999999
CANONICAL_ACTION_ENERGY_DECIMAL_EMAX = 999999
...
SCORE_INPUT_CLOSURE = {
    ...
    "primary_score_from_canonical_high_precision_energy_arithmetic": True,
    "origin_device_log1p_used_for_calibration": False,
...
_SCORE_RECEIPT_FIELDS = frozenset(
    {
        ...
        "canonical_negative_log_energy_ratio_decimal_by_branch",
        "canonical_global_action_energy_score_decimal",
        "canonical_action_energy_arithmetic_contract",
        "global_hardest_negative_branch",
        "live_origin_global_action_energy_score_diagnostic",
        "live_origin_global_hardest_negative_branch_diagnostic",
        "live_origin_negative_log_energy_ratio_by_branch_diagnostic",
        "mace_live_tensor_formula_proof_diagnostic",
...
_MACE_LIVE_TENSOR_FORMULA_PROOF_FIELDS = frozenset(
    {
        ...
        "tensor_dtype",
        "origin_device_type",
        "stable_log1p_formula_recomputed_on_origin_device_bit_exact",
        "reward_and_first_argmin_recomputed_on_origin_device_bit_exact",
        "diagnostic_only_not_calibration_truth",
        "digest",
    }
)

_CANONICAL_ACTION_ENERGY_PACKET_FIELDS = frozenset(
    {
        "schema_version",
        "definition",
        "energy_epsilon",
        ...
        "live_origin_negative_log_energy_ratio_by_branch_diagnostic",
        "mace_live_tensor_formula_proof_diagnostic",
        "formal_calibration_truth_from_exact_fp32_energies",
        "origin_device_values_diagnostic_only",
        "cross_device_numeric_tolerance_used_for_formal_gate",
...
def _exact_fp32_scalar(
    value: Any, *, label: str, nonnegative: bool = False
) -> float:
    """Validate one JSON float as an exact finite IEEE-754 binary32 value."""
    ...
    try:
        ...
        raise PairV5T2VEnergyScoringError(
            f"{label} is not an exact FP32 scalar"
        ) from error
    if float(roundtrip) != value:
        raise PairV5T2VEnergyScoringError(
            f"{label} is not an exact FP32 scalar"
        )
    if nonnegative and (value < 0.0 or (value == 0.0 and math.copysign(1.0, value) < 0.0)):
        raise PairV5T2VEnergyScoringError(f"{label} is negative")
    return value


def canonical_action_energy_arithmetic_contract() -> dict[str, Any]:
    """Return the device-independent v4 scalar arithmetic contract."""

    unsigned = {
        "schema_version": CANONICAL_ACTION_ENERGY_ARITHMETIC_SCHEMA,
        "input_energy_encoding": "exact_finite_nonnegative_ieee754_binary32",
        "input_conversion": "Decimal.from_float_exact_binary_value",
        "energy_epsilon_decimal": CANONICAL_ACTION_ENERGY_EPSILON_DECIMAL,
        "decimal_context_precision": CANONICAL_ACTION_ENERGY_DECIMAL_PRECISION,
        "decimal_context_rounding": "ROUND_HALF_EVEN",
        "decimal_context_emin": CANONICAL_ACTION_ENERGY_DECIMAL_EMIN,
        "decimal_context_emax": CANONICAL_ACTION_ENERGY_DECIMAL_EMAX,
        "decimal_context_capitals": 1,
        "decimal_context_clamp": 0,
        "per_negative_formula": (
            "Context.ln(Context.divide(Context.add(E_negative,epsilon),"
            "Context.add(E_action,epsilon)))"
        ),
        "canonical_decimal_text": "str(context_result)",
        "json_score_conversion": "finite_binary64_float(canonical_decimal_text)",
        "hardest_negative_rule": (
            "first_argmin_exact_fp32_negative_energy_in_registered_order"
        ),
        "primary_score_rule": "canonical_ratio_at_hardest_negative",
        "origin_device_log1p_is_diagnostic_only": True,
        "cross_device_tolerance_used_for_formal_gate": False,
    }
    return {**unsigned, "contract_digest": object_sha256(unsigned)}


def _canonical_action_energy_packet(
    *,
    action_energy: Any,
    negative_energy_by_branch: Any,
    energy_epsilon: Any,
) -> dict[str, Any]:
    """Compute formal score truth from exact FP32 energies only.

    Decimal arithmetic is the definition of the v4 receipt scalar, rather
    than an approximation used to check a device ``log`` kernel.  The 80-digit
    result and its binary64 JSON convenience value are both sealed.
    """

    action = _exact_fp32_scalar(
    ...
    negatives = {
        branch: _exact_fp32_scalar(
            negative_energy_by_branch[branch],
    ...
    ordered_negative_values = [
        negatives[branch] for branch in mace.HARD_NEGATIVE_BRANCHES
    ]
    hardest_index = min(
        range(len(ordered_negative_values)),
        key=ordered_negative_values.__getitem__,
    )
    hardest_branch = mace.HARD_NEGATIVE_BRANCHES[hardest_index]
    context = Context(
        prec=CANONICAL_ACTION_ENERGY_DECIMAL_PRECISION,
        rounding=ROUND_HALF_EVEN,
        Emin=CANONICAL_ACTION_ENERGY_DECIMAL_EMIN,
        Emax=CANONICAL_ACTION_ENERGY_DECIMAL_EMAX,
        capitals=1,
        clamp=0,
    )
    epsilon_decimal = Decimal(CANONICAL_ACTION_ENERGY_EPSILON_DECIMAL)
    action_regularized = context.add(
        Decimal.from_float(action), epsilon_decimal
    )
    decimal_by_branch: dict[str, str] = {}
    float_by_branch: dict[str, float] = {}
    for branch in mace.HARD_NEGATIVE_BRANCHES:
        negative_regularized = context.add(
            Decimal.from_float(negatives[branch]), epsilon_decimal
        )
        ratio = context.divide(negative_regularized, action_regularized)
        log_ratio = context.ln(ratio)
        decimal_text = str(log_ratio)
        float_value = float(decimal_text)
        if not math.isfinite(float_value):
            raise PairV5T2VEnergyScoringError(
                f"canonical action ratio for {branch} is non-finite"
            )
        decimal_by_branch[branch] = decimal_text
        float_by_branch[branch] = float_value

    score_decimal = decimal_by_branch[hardest_branch]
    score = float_by_branch[hardest_branch]
    return {
        "action_energy": action,
        "negative_energy_by_branch": negatives,
        "negative_log_energy_ratio_by_branch": float_by_branch,
        "negative_log_energy_ratio_decimal_by_branch": decimal_by_branch,
        "score": score,
        "score_decimal": score_decimal,
        "hardest_negative_branch": hardest_branch,
        "hardest_negative_index": hardest_index,
        "arithmetic_contract": canonical_action_energy_arithmetic_contract(),
    }


def _make_live_mace_tensor_formula_proof(
    energy: Any,
) -> tuple[dict[str, Any], dict[str, float], float, str]:
    """Bind the stable live-device MACE diagnostic before serialization."""
    ...
    with torch.no_grad():
        branch_fp64 = branch.to(torch.float64)
        relative_delta = (branch_fp64[1:] - branch_fp64[:1]) / (
            branch_fp64[:1] + epsilon
        )
        recomputed_ratios = torch.log1p(relative_delta).to(torch.float32)
        recomputed_reward, recomputed_hardest = recomputed_ratios.min(dim=0)
        energy_hardest = branch[1:].min(dim=0).indices
    if not torch.equal(recomputed_ratios, ratios):
        raise PairV5T2VEnergyScoringError(
            "live stable log1p MACE formula is not bit-exact on its origin device"
        )
    ...
    if not torch.equal(energy_hardest, hardest):
        raise PairV5T2VEnergyScoringError(
            "live MACE hardest negative differs from exact energy first argmin"
        )
    ...
    unsigned = {
        ...
        "origin_device_type": branch.device.type,
        "stable_log1p_formula_recomputed_on_origin_device_bit_exact": True,
        "reward_and_first_argmin_recomputed_on_origin_device_bit_exact": True,
        "diagnostic_only_not_calibration_truth": True,
    }
    ...
    return (
        ...
        ratio_by_branch,
        float(reward.item()),
        mace.HARD_NEGATIVE_BRANCHES[hardest_index],
    )


def _validate_live_mace_diagnostic(
    *,
    action_energy: float,
    negative_energy_by_branch: Mapping[str, float],
    negative_log_ratio_by_branch: Any,
    reward: Any,
    hardest_negative_branch: Any,
    live_tensor_formula_proof: Any,
) -> None:
    """Validate origin diagnostic hashes without making it calibration truth."""
    ...
    if not isinstance(negative_log_ratio_by_branch, Mapping) or set(
        ...
        raise PairV5T2VEnergyScoringError(
            "live origin negative log-energy ratio closure differs"
        )
    ratios = {
        branch: _exact_fp32_scalar(
            negative_log_ratio_by_branch[branch],
            label=f"live origin negative log-energy ratio for {branch}",
        )
        for branch in mace.HARD_NEGATIVE_BRANCHES
    }
    reward_value = _exact_fp32_scalar(
        reward, label="live origin global action score diagnostic"
    )
    if (
        type(hardest_negative_branch) is not str
        or hardest_negative_branch not in mace.HARD_NEGATIVE_BRANCHES
    ):
        raise PairV5T2VEnergyScoringError(
            "live origin hardest-negative branch differs"
        )
    energy_tensor = torch.tensor(
        [
            action_energy,
            *(negative_energy_by_branch[name] for name in mace.HARD_NEGATIVE_BRANCHES),
        ],
        dtype=torch.float32,
    ).reshape(-1, 1)
    ratio_tensor = torch.tensor(
        [ratios[name] for name in mace.HARD_NEGATIVE_BRANCHES],
        dtype=torch.float32,
    ).reshape(-1, 1)
    ...
    exact_energy_index = int(energy_tensor[1:, 0].min(dim=0).indices.item())
    if (
        reward_value != float(expected_reward_tensor.item())
        or hardest_negative_branch != mace.HARD_NEGATIVE_BRANCHES[expected_index]
        or expected_index != exact_energy_index
    ):
        raise PairV5T2VEnergyScoringError(
            "live origin diagnostic reward/hardest closure differs"
        )
    ...
    if (
        ...
        or proof["origin_device_type"] not in ("cpu", "cuda")
        or proof[
            "stable_log1p_formula_recomputed_on_origin_device_bit_exact"
        ]
        is not True
        ...
        is not True
        or proof["diagnostic_only_not_calibration_truth"] is not True
        ...
        != tensor_sha256(torch.tensor([expected_index], dtype=torch.int64))
    ):
...
def _validated_energy_scalars(
    *,
    action_energy: Any,
    negative_energy_by_branch: Any,
    canonical_negative_log_ratio_by_branch: Any,
    canonical_negative_log_ratio_decimal_by_branch: Any,
    canonical_score_decimal: Any,
    canonical_arithmetic_contract: Any,
    raw_reward: Any,
    hardest_negative_branch: Any,
    energy_epsilon: Any,
    live_negative_log_ratio_by_branch: Any,
    live_reward: Any,
    live_hardest_negative_branch: Any,
    live_tensor_formula_proof: Any,
) -> tuple[float, dict[str, float], float, str]:
    """Exactly replay the formal v4 scalar and separately audit live provenance."""

    canonical = _canonical_action_energy_packet(
        action_energy=action_energy,
        negative_energy_by_branch=negative_energy_by_branch,
        energy_epsilon=energy_epsilon,
    )
    if (
        not isinstance(canonical_negative_log_ratio_by_branch, Mapping)
        or dict(canonical_negative_log_ratio_by_branch)
        != canonical["negative_log_energy_ratio_by_branch"]
        or not isinstance(canonical_negative_log_ratio_decimal_by_branch, Mapping)
        or dict(canonical_negative_log_ratio_decimal_by_branch)
        != canonical["negative_log_energy_ratio_decimal_by_branch"]
        or type(canonical_score_decimal) is not str
        or canonical_score_decimal != canonical["score_decimal"]
        or canonical_arithmetic_contract != canonical["arithmetic_contract"]
        or type(raw_reward) is not float
        or not math.isfinite(raw_reward)
        or raw_reward != canonical["score"]
    ):
        raise PairV5T2VEnergyScoringError(
            "formal canonical action-energy arithmetic replay differs"
        )
    if hardest_negative_branch != canonical["hardest_negative_branch"]:
        raise PairV5T2VEnergyScoringError(
            "global hardest-negative branch is not the exact energy first argmin"
        )
    _validate_live_mace_diagnostic(
        action_energy=canonical["action_energy"],
        negative_energy_by_branch=canonical["negative_energy_by_branch"],
        negative_log_ratio_by_branch=live_negative_log_ratio_by_branch,
        reward=live_reward,
        hardest_negative_branch=live_hardest_negative_branch,
        live_tensor_formula_proof=live_tensor_formula_proof,
    )
    return (
        canonical["action_energy"],
        canonical["negative_energy_by_branch"],
        canonical["score"],
        canonical["hardest_negative_branch"],
    )


def make_canonical_action_energy_packet(energy: Any) -> dict[str, Any]:
    """Seal active-v4 canonical truth plus non-authorizing live diagnostics.

    This is the public packet boundary for scorers that already hold a live
    :class:`MACEActionEnergy` result.  Only exact FP32 branch energies feed the
    canonical 80-digit arithmetic.  Origin-device ``log1p`` values are bound
    for diagnosis, but cannot define or authorize a calibrated score.
    """

    (
        live_proof,
        live_ratio_by_branch,
        live_reward,
        live_hardest_branch,
    ) = _make_live_mace_tensor_formula_proof(energy)
    branch_energies = getattr(energy, "branch_energies", None)
    values = [float(item) for item in branch_energies[:, 0].tolist()]
    if len(values) != len(mace.BRANCH_ORDER):
        raise PairV5T2VEnergyScoringError(
            "canonical packet branch-energy closure differs"
        )
    action_energy = values[0]
    negative_energies = {
    ...
    unsigned = {
        "schema_version": CANONICAL_ACTION_ENERGY_PACKET_SCHEMA,
        "definition": CANONICAL_ACTION_ENERGY_ARITHMETIC_SCHEMA,
        "energy_epsilon": energy_epsilon,
        "global_action_energy": canonical["action_energy"],
        "global_hard_negative_energy_by_branch": canonical[
            "negative_energy_by_branch"
        ],
        "global_negative_log_energy_ratio_by_branch": canonical[
            "negative_log_energy_ratio_by_branch"
        ],
        "canonical_negative_log_energy_ratio_decimal_by_branch": canonical[
            "negative_log_energy_ratio_decimal_by_branch"
        ],
        "canonical_global_action_energy_score_decimal": canonical[
            "score_decimal"
        ],
        "canonical_action_energy_arithmetic_contract": canonical[
            "arithmetic_contract"
        ],
        "global_hardest_negative_branch": canonical[
            "hardest_negative_branch"
        ],
        "raw_global_action_energy_score": canonical["score"],
        ...
        "mace_live_tensor_formula_proof_diagnostic": live_proof,
        "formal_calibration_truth_from_exact_fp32_energies": True,
        "origin_device_values_diagnostic_only": True,
        "cross_device_numeric_tolerance_used_for_formal_gate": False,
        "optimizer_authorized": False,
        "scientific_action_editing_claim": False,
    }
    packet = {**unsigned, "packet_digest": object_sha256(unsigned)}
    return validate_canonical_action_energy_packet(packet)


def validate_canonical_action_energy_packet(value: Any) -> dict[str, Any]:
    """Replay a public active-v4 packet with no cross-device tolerance gate."""

    if not isinstance(value, Mapping) or set(value) != set(
        _CANONICAL_ACTION_ENERGY_PACKET_FIELDS
    ):
        raise PairV5T2VEnergyScoringError(
            "canonical action-energy packet field closure differs"
        )
    row = dict(value)
    unsigned = dict(row)
    declared = _sha256(
        unsigned.pop("packet_digest", None),
        label="canonical action-energy packet digest",
    )
    if object_sha256(unsigned) != declared:
        raise PairV5T2VEnergyScoringError(
            "canonical action-energy packet digest differs"
        )
    if (
        row["schema_version"] != CANONICAL_ACTION_ENERGY_PACKET_SCHEMA
        or row["definition"] != CANONICAL_ACTION_ENERGY_ARITHMETIC_SCHEMA
        or row["formal_calibration_truth_from_exact_fp32_energies"] is not True
        or row["origin_device_values_diagnostic_only"] is not True
        or row["cross_device_numeric_tolerance_used_for_formal_gate"] is not False
        or row["optimizer_authorized"] is not False
        or row["scientific_action_editing_claim"] is not False
    ):
        raise PairV5T2VEnergyScoringError(
            "canonical action-energy packet authority differs"
        )
    _validated_energy_scalars(
        action_energy=row["global_action_energy"],
        negative_energy_by_branch=row[
            "global_hard_negative_energy_by_branch"
        ],
        canonical_negative_log_ratio_by_branch=row[
            "global_negative_log_energy_ratio_by_branch"
        ],
        canonical_negative_log_ratio_decimal_by_branch=row[
            "canonical_negative_log_energy_ratio_decimal_by_branch"
        ],
        canonical_score_decimal=row[
            "canonical_global_action_energy_score_decimal"
        ],
        canonical_arithmetic_contract=row[
            "canonical_action_energy_arithmetic_contract"
        ],
        ...
        live_negative_log_ratio_by_branch=row[
            "live_origin_negative_log_energy_ratio_by_branch_diagnostic"
        ],
        live_reward=row[
            "live_origin_global_action_energy_score_diagnostic"
        ],
        live_hardest_negative_branch=row[
            "live_origin_global_hardest_negative_branch_diagnostic"
        ],
        live_tensor_formula_proof=row[
            "mace_live_tensor_formula_proof_diagnostic"
        ],
    )
...
def make_score_receipt(
    *,
    ...
    candidate = row["candidate"]
    (
        live_mace_formula_proof,
        live_negative_log_ratios,
        live_reward,
        live_hardest_branch,
    ) = _make_live_mace_tensor_formula_proof(score.energy)
    phase_reward = float(score.phase_energy.reward.item())
    if not math.isfinite(live_reward) or not math.isfinite(phase_reward):
    ...
    reward = canonical["score"]
    hardest_branch = canonical["hardest_negative_branch"]
    _validated_energy_scalars(
        action_energy=action_energy,
        negative_energy_by_branch=negative_energies,
        canonical_negative_log_ratio_by_branch=canonical[
            "negative_log_energy_ratio_by_branch"
        ],
        canonical_negative_log_ratio_decimal_by_branch=canonical[
            "negative_log_energy_ratio_decimal_by_branch"
        ],
        canonical_score_decimal=canonical["score_decimal"],
        canonical_arithmetic_contract=canonical["arithmetic_contract"],
        raw_reward=reward,
        hardest_negative_branch=hardest_branch,
        energy_epsilon=float(mace.DEFAULT_ENERGY_EPSILON),
        live_negative_log_ratio_by_branch=live_negative_log_ratios,
        live_reward=live_reward,
        live_hardest_negative_branch=live_hardest_branch,
    ...
    value = {
        ...
        "mace_live_tensor_formula_proof_diagnostic": live_mace_formula_proof,
...
def validate_score_receipt(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        != set(mace.HARD_NEGATIVE_BRANCHES)
        or set(row["canonical_negative_log_energy_ratio_decimal_by_branch"])
        != set(mace.HARD_NEGATIVE_BRANCHES)
        or set(
            row["live_origin_negative_log_energy_ratio_by_branch_diagnostic"]
        )
        != set(mace.HARD_NEGATIVE_BRANCHES)
    ):
    ...
    _validated_energy_scalars(
        ...
        live_reward=row["live_origin_global_action_energy_score_diagnostic"],
        live_hardest_negative_branch=row[
            "live_origin_global_hardest_negative_branch_diagnostic"
        ],
        live_tensor_formula_proof=row[
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for cell_id, rows in by_cell.items():
        ...
        for row_index, row in enumerate(rows):
            ...
            if distributed.rank == 0:
                ...
                candidate_dir.mkdir()
                receipt_path = candidate_dir / SCORE_RECEIPT_FILENAME
    ...
    if distributed.rank == 0:
        ...
        path = output / GROUP_RECEIPT_FILENAME.format(group_id=args.group_id)
