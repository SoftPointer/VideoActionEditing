#!/usr/bin/env python3
"""Build the no-diagnostic BOX-EXP-009 exact8 plan for 136141/gpu299.

The frozen 40-row confirmation inventory is used only as a sealed provenance
source.  The executable plan contains exactly four seed cells times the
ordered ``action``/``incomplete`` pair: eight exact81 videos, exact40 sampler
steps each, no diagnostic task, and no optimizer authority.
"""
...
PLAN_SCHEMA = "bernini-full30-action-confirmation8-136141-plan-v1"
PLAN_FILENAME = "full30-action-confirmation8-136141-plan-v1.json"
SPEC_AUTHORITIES = {
...
NONFORMAL_SOURCE_BRANCH_ORDER = tuple(
    branch
...
class Confirmation8PlanError(RuntimeError):
    """Raised before a mutable or over-broad 136141 plan is accepted."""
...
def partition_confirmation_tasks(
    ...
    """Validate the frozen 40-row provenance and project only formal exact8."""
    ...
    require(
        len(admission) == 8
        and [row["semantic_branch"] for row in admission]
        == list(ADMISSION_BRANCH_ORDER) * 4,
        "confirmation exact8 projection differs",
    )
    return admission, cells
...
def _expected_shards(
    ...
    for cell in CONFIRMATION_CELL_REGISTRY:
        ...
        require(
            ...
            and len(selected) == 2,
            "confirmation formal shard partition differs",
        )
        rows.append(
            {
                "shard_id": (
...
def _plan_value(
    *, source_specs: Sequence[Mapping[str, Any]], source_tasks: Sequence[Mapping[str, Any]]
    ...
    admission, cells = partition_confirmation_tasks(source_tasks)
    unsigned = {
        "schema_version": PLAN_SCHEMA,
        "plan_id": "BOX-EXP-009-confirmation-action-anchor-exact8-136141-v1",
        ...
        "scientific_target": (
            "test on four held-out confirmation seed cells whether the Frozen "
            "model expresses the requested action and the matched incomplete "
            "endpoint as a distinguishable pair under one identical Gaussian"
        ),
        "learning_target": (
        ...
        "numeric_target": (
            "8/8 exact81 clips, 4/4 byte-identical action/incomplete Gaussian "
            "pairs, 8/8 independent full81 review passes, 4/4 official "
            "same-state materializer gates, 0 diagnostics, 0 optimizer updates"
        ),
        ...
        "core_validation": (
            "independent full81 action/incomplete review plus official same-state "
            "PsiOut threshold gates and fail-closed resource evidence"
        ),
        ...
        "shards": _expected_shards(admission),
        "execution_contract": {
            "formal_dataset": "confirmation_action_anchor_exact8_136141",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "build-plan":
        ...
        summary = {
            ...
            "diagnostic_task_count": value["diagnostic_task_count"],
        }
    else:
