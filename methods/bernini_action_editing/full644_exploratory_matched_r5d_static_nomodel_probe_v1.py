#!/usr/bin/env python3
"""Pure-metadata CPU gate for a fresh r5d case00 package.
...
SCHEMA = "full644-exploratory-matched-r5d-static-nomodel-probe-v1"
...
RELEASE_SCHEMA = "full644-exploratory-matched-root-launch-release-auh-r5d"
ROOT_BOOTSTRAP_SHA256 = (
    "17e63e661faaba23571fa7a38522090776b62e16f4839ce772a576760dfde6cc"
)
...
RELEASE_FILES = {
    ...
    "methods/bernini_action_editing/full644_exploratory_matched_infer_adapter_auh_r5d.py":
...
class R5DStaticProbeError(RuntimeError):
...
def _pairs(items: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in items:
        if key in result:
            raise R5DStaticProbeError("duplicate JSON key")
        result[key] = value
    return result
...
def _read_fd(descriptor: int, size: int) -> bytes:
    if type(size) is not int or size <= 0:
        raise R5DStaticProbeError("metadata file is empty")
    chunks: list[bytes] = []
    offset = 0
    while offset < size:
    ...
    raw = b"".join(chunks)
    if len(raw) != size:
        raise R5DStaticProbeError("metadata read is incomplete")
    return raw
...
def stable_file(
    ...
    if (
        ...
        raise R5DStaticProbeError(f"noncanonical metadata path: {path}")
    descriptor = os.open(
        path,
        os.O_RDONLY
    ...
    if (
        ...
        raise R5DStaticProbeError(f"metadata identity differs: {path}")
    return raw, identity
...
def replay_identity_row(row: Any) -> None:
    if (
        ...
        raise R5DStaticProbeError("launch identity row closure differs")
    ...
    if (
        ...
        raise R5DStaticProbeError("launch identity row replay differs")
...
def strict_json(raw: bytes, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise R5DStaticProbeError(f"{label} is not strict JSON") from error
    if not isinstance(value, dict) or raw != canonical_json_bytes(value) + b"\n":
        raise R5DStaticProbeError(f"{label} is not canonical JSON")
    return value


def _canonical_absolute(raw: Any) -> Path:
    if type(raw) is not str:
        raise R5DStaticProbeError("metadata path is not text")
    path = Path(raw)
    if not path.is_absolute() or os.path.normpath(raw) != raw:
        raise R5DStaticProbeError("metadata path is not canonical")
    return path
...
def validate_release_tree(root: Path) -> None:
    ...
    if not stat.S_ISDIR(root_info.st_mode) or root.is_symlink():
        raise R5DStaticProbeError("release root identity differs")
    files: set[str] = set()
    directories: set[str] = {"."}
    stack = [(root, Path("."))]
    while stack:
        ...
        with os.scandir(current) as entries:
            for entry in entries:
                ...
                if stat.S_ISDIR(info.st_mode):
                    ...
                    raise R5DStaticProbeError("release contains a non-file entry")
    if files != set(RELEASE_FILES) or directories != RELEASE_DIRECTORIES:
        raise R5DStaticProbeError("release physical tree closure differs")


def require_empty_directory(path: Path, *, label: str) -> None:
    info = path.lstat()
    if not stat.S_ISDIR(info.st_mode) or path.is_symlink():
        raise R5DStaticProbeError(f"{label} directory identity differs")
    with os.scandir(path) as entries:
        if next(entries, None) is not None:
            raise R5DStaticProbeError(f"{label} directory is not fresh")
...
def validate_plan(value: Mapping[str, Any], root: Path) -> None:
    if (
        ...
        raise R5DStaticProbeError("plan header differs")
    unsigned = dict(value)
    claimed = unsigned.pop("plan_digest", None)
    if claimed != object_sha256(unsigned):
        raise R5DStaticProbeError("plan digest differs")
    ...
    if not _same_exact_json_value(value.get("authority"), {
        ...
        raise R5DStaticProbeError("plan authority metadata differs")
    if not _same_exact_json_value(value.get("claim_limits"), {
        ...
        raise R5DStaticProbeError("plan claim or execution metadata differs")
    if not _same_exact_json_value(value.get("producer"), {
        ...
        raise R5DStaticProbeError("plan producer metadata differs")
    observed_ids: list[str] = []
    output_root = root / "outputs/media"
    for index, task in enumerate(value["tasks"]):
        if not isinstance(task, dict):
            raise R5DStaticProbeError("plan task type differs")
        ...
        if (
            ...
            raise R5DStaticProbeError("plan task semantics differ")
        ...
        if (
            ...
            raise R5DStaticProbeError("plan output freshness differs")
        observed_ids.append(task_id)
    if tuple(observed_ids) != ALL_TASKS:
        raise R5DStaticProbeError("plan task order differs")
...
def validate_input(value: Mapping[str, Any], root: Path, plan: Path) -> None:
    ...
    if (
        ...
        or value.get("rank_cache_root") != str(root / "runtime/rank-cache")
    ):
        raise R5DStaticProbeError("launch input semantics differ")
    expected_paths = {
        "runner": "full644_exploratory_matched_runner_auh_r5.py",
        "bridge": "full644_exploratory_matched_torchrun_fd_bridge_v2.py",
        "adapter": "full644_exploratory_matched_infer_adapter_auh_r5d.py",
    ...
    for role, relative in expected_paths.items():
        if value.get(role) != str(method_root / relative):
            raise R5DStaticProbeError("launch release path differs")
    if (
        value.get("holder_job_id") != os.environ.get("SLURM_JOB_ID")
        or value.get("expected_node") != os.environ.get("SLURM_JOB_NODELIST")
    ):
        raise R5DStaticProbeError("launch allocation binding differs")
...
def validate_launch_receipt(
    ...
    if (
        ...
        raise R5DStaticProbeError("launch receipt semantics differ")
    ...
    if not isinstance(identities, dict) or set(identities) != expected_roles:
        raise R5DStaticProbeError("launch exact16 identity closure differs")
    local_roles = {
    ...
    for role, relative in local_roles.items():
        ...
        if (
            ...
            raise R5DStaticProbeError("launch local identity row differs")
    for role, digest in EXTERNAL_IDENTITY_PINS.items():
        ...
        if (
            ...
            raise R5DStaticProbeError("launch external identity row differs")
    if (
        ...
        raise R5DStaticProbeError("launch plan or campaign arguments differ")


def _validate_slurm(job_id: str, node: str) -> str:
    if any(field in os.environ for field in SLURM_ABSENT_FIELDS):
        raise R5DStaticProbeError("unsupported Slurm field is present")
    ...
    if (
        ...
        raise R5DStaticProbeError("AUH Slurm source contract differs")
    return step
...
def _publish(path: Path, value: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        while offset < len(raw):
            ...
            if count <= 0:
                raise R5DStaticProbeError("receipt write made no progress")
            offset += count
        os.fsync(descriptor)
        ...
        if (
            ...
            raise R5DStaticProbeError("receipt staging replay differs")
        sentinel = f"R5D_STATIC_NOMODEL_PASS {value['receipt_digest']}\n".encode("ascii")
        if os.write(1, sentinel) != len(sentinel):
            raise R5DStaticProbeError("receipt sentinel write differs")
...
def main(argv: Sequence[str] | None = None) -> int:
    if (
        ...
        raise R5DStaticProbeError("isolated metadata probe differs")
    ...
    if any(
        ...
        raise R5DStaticProbeError("metadata digest syntax differs")
    ...
    if (
        ...
        raise R5DStaticProbeError("package metadata path closure differs")
    ...
    if (
        ...
        raise R5DStaticProbeError("outputs directory closure differs")
    ...
    if (
        ...
        or set(launch_input_row) != {"path", "sha256", "size", "identity"}
        or launch_input_row.get("path") != str(input_path)
        or launch_input_row.get("sha256") != args.launch_input_sha256
        or launch_input_row.get("size") != len(input_raw)
        or not _same_exact_json_value(
        ...
        raise R5DStaticProbeError("launch receipt input or publication binding differs")
    local_roles = {
        "runner": "methods/bernini_action_editing/full644_exploratory_matched_runner_auh_r5.py",
        "bridge": "methods/bernini_action_editing/full644_exploratory_matched_torchrun_fd_bridge_v2.py",
        "adapter": "methods/bernini_action_editing/full644_exploratory_matched_infer_adapter_auh_r5d.py",
    ...
    if any(
        ...
        raise R5DStaticProbeError("launch identity-to-physical-release binding differs")
    ...
    if payload != root / "launch/root_launch_payload_auh_r5d.sh":
        raise R5DStaticProbeError("production payload path differs")
    ...
    if (
        ...
        raise R5DStaticProbeError("production payload size differs")
    ...
    if forbidden_targets or "torch" in sys.modules:
        raise R5DStaticProbeError("GPU/model authority was opened")
    result: dict[str, Any] = {
