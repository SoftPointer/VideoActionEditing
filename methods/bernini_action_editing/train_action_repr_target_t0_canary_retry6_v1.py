...
SCHEMA_VERSION = "bernini-action-repr-target-t0-one-step-retry6-receipt-v1"
STEP_SCHEMA_VERSION = "bernini-action-repr-target-t0-retry6-adapter-state-receipt-v1"
METHOD = "bernini-action-repr-target-t0-one-step-retry6-canary-v1"
ATTEMPT_CLAIM_SCHEMA_VERSION = (
    "bernini-action-repr-target-t0-retry6-preoptimizer-attempt-claim-v1"
)
ATTEMPT_CLAIM_MARKER_NAME = ".single_update.retry6.attempt_claim.json"
...
EXPECTED_ATTEMPT_CLAIM_PATH = (
    "/vast/users/guangyi.chen/causal_group/peiyuan.zhu/work/VideoEditing/"
    "VideoEdit_experiments/action_repr_target_selfgen_middle_g1_20260824_v2/"
    "stage_b_t0_retry6/target_t0/0be6494dfac3/"
...
def _canonical_attempt_output(
    ...
    if candidate.is_symlink():
        fail("retry6 attempt output may not be a symlink")
    if require_fresh and candidate.exists():
        fail("retry6 attempt output must be fresh before the permanent claim")
    try:
        parent = candidate.parent.resolve(strict=True)
    except (FileNotFoundError, OSError) as error:
        raise TargetT0CanaryError(
            "retry6 attempt-claim parent must already be one real directory"
        ) from error
    if not parent.is_dir() or parent.is_symlink() or candidate.name in {"", ".", ".."}:
        fail("retry6 attempt-claim output path is not canonical and safe")
...
def _expected_preoptimizer_attempt_claim(
    *,
    ...
    env = os.environ if environ is None else environ
    g2a_world4.require_sha256(authority_sha256, label="retry6 claim authority")
    g2a_world4.require_sha256(
        source_hash_pins_digest, label="retry6 claim source hash pins"
    )
    ...
    if (
        ...
        fail("retry6 claim requires the numeric Slurm job id")
    if world_size != 4 or sequence_parallel_size != 4:
        fail("retry6 claim requires WORLD4/SP4")
    ...
    if (
        ...
        fail("retry6 authority marker path/output binding differs")
    row: dict[str, Any] = {
...
def validate_preoptimizer_attempt_claim_object(value: Any) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        fail("retry6 preoptimizer attempt claim must be an object")
    row = dict(value)
    declared = row.pop("claim_digest", None)
    g2a_world4.require_sha256(declared, label="retry6 attempt claim object")
    if object_sha256(row) != declared:
        fail("retry6 preoptimizer attempt claim object digest differs")
    if set(row) != {
        "schema_version",
        ...
        fail("retry6 preoptimizer attempt claim field closure differs")
    ...
    if (
        ...
        fail("retry6 preoptimizer attempt claim identity differs")
    g2a_world4.require_sha256(
        row.get("authorization_addendum_sha256"), label="retry6 claim authority"
    )
    g2a_world4.require_sha256(
        row.get("source_hash_pins_digest"), label="retry6 claim source pins"
    )
...
def replay_preoptimizer_attempt_claim(
    ...
    *,
    ...
    if path.name != ATTEMPT_CLAIM_MARKER_NAME:
        fail("retry6 attempt claim marker name differs")
    try:
        before = path.lstat()
    except (FileNotFoundError, OSError) as error:
        raise TargetT0CanaryError("retry6 attempt claim is missing") from error
    if (
        ...
        fail("retry6 attempt claim type, mode, or link count differs")
    nofollow = getattr(os, "O_NOFOLLOW", None)
    if nofollow is None:
        fail("retry6 attempt claim requires O_NOFOLLOW support")
    descriptor = os.open(path, os.O_RDONLY | nofollow)
    try:
        opened = os.fstat(descriptor)
        if (opened.st_dev, opened.st_ino) != (before.st_dev, before.st_ino):
            fail("retry6 attempt claim changed while opening")
        chunks = []
        total = 0
        while True:
            ...
            total += len(chunk)
            if total > 65536:
                fail("retry6 attempt claim is unexpectedly large")
            chunks.append(chunk)
    finally:
        os.close(descriptor)
    ...
    try:
        ...
        raise TargetT0CanaryError(
            "retry6 attempt claim is partial or not canonical JSON"
        ) from error
    if canonical_json_bytes(value) != payload:
        fail("retry6 attempt claim bytes are not canonical JSON")
    ...
    if expected_claim is not None and canonical_json_bytes(value) != canonical_json_bytes(
        ...
        fail("retry6 attempt claim does not match this authorized execution")
    return {
        "path": str(path),
...
def create_preoptimizer_attempt_claim(
    *,
    ...
    if canonical_output.exists() or canonical_output.is_symlink():
        fail("retry6 output ceased to be fresh immediately before atomic claim")
    try:
        descriptor = os.open(marker, flags, 0o600)
    except FileExistsError as error:
        raise TargetT0CanaryError(
            "retry6 permanent attempt claim already exists; use retry7"
        ) from error
    except OSError as error:
        raise TargetT0CanaryError(
            "retry6 permanent attempt claim could not be created"
        ) from error
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                fail("retry6 permanent attempt claim write made no progress")
            offset += written
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def acquire_preoptimizer_attempt_claim_world4(
    *,
    ...
    if not isinstance(result, Mapping) or set(result) != {
        ...
        fail("retry6 rank0 attempt-claim broadcast closure differs")
    if result.get("ok") is not True:
        fail(f"retry6 rank0 attempt claim failed permanently: {result.get('error')}")
    ...
    if (
        ...
        fail("retry6 attempt claim differs from the rank0 broadcast")
    return evidence
...
def validate_preoptimizer_attempt_claim_evidence(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != {
        ...
        fail("retry6 attempt-claim evidence closure differs")
    path = value.get("path")
    claim = value.get("claim")
    if not isinstance(path, str) or not Path(path).is_absolute():
        fail("retry6 attempt-claim evidence path is not absolute")
    if Path(path).name != ATTEMPT_CLAIM_MARKER_NAME:
        fail("retry6 attempt-claim evidence marker name differs")
    g2a_world4.require_sha256(
        value.get("file_sha256"), label="retry6 attempt claim file"
    )
    g2a_world4.require_sha256(
        value.get("object_digest"), label="retry6 attempt claim object"
    )
    validate_preoptimizer_attempt_claim_object(claim)
    if value.get("object_digest") != claim.get("claim_digest"):
        fail("retry6 attempt-claim evidence object digest binding differs")
    ...
    if (
        ...
        fail("retry6 attempt-claim evidence path/output binding differs")
    if expected_output is not None:
        try:
            output = Path(expected_output).expanduser().resolve(strict=True)
        except (FileNotFoundError, OSError) as error:
            raise TargetT0CanaryError(
                "published retry6 output is unavailable for claim replay"
            ) from error
        if output != claim_output:
            fail("published retry6 output differs from its permanent attempt claim")
    return value
...
def validate_authorization_addendum(
    ...
    *,
    ...
    source_hash_pins_digest = object_sha256(dict(pins))
    ...
    if (
        ...
        or runtime_paths.get("fresh_source_root_name") != "source_stage_b_t0_retry6"
        or runtime_paths.get("fresh_stage_root_name") != "stage_b_t0_retry6"
        or runtime_paths.get("fresh_log_root_name") != "logs/stage_b_t0_retry6"
...
def authorize_preoptimizer_inputs(
    *,
    ...
    if str(output_path) != EXPECTED_CANONICAL_OUTPUT_PATH:
        fail("T0 output CLI path differs from the exact retry6 authority")
    ...
    *,
    ...
    for step, state in ((0, result.step0_state), (1, result.step1_state)):
        ...
        payload = save_safetensors(
            dict(state),
            metadata={
                "schema_version": "bernini-action-repr-target-t0-retry6-adapter-state-v1",
...
def validate_t0_receipt(value: Any) -> Mapping[str, Any]:
    ...
    if (
        ...
        fail("retry6 attempt claim/receipt authority or runtime binding differs")
    if (
...
def validate_published_t0_output(output: Path | str) -> Mapping[str, Any]:
    ...
    if canonical_json_bytes(replayed_claim) != canonical_json_bytes(claim_evidence):
        fail("published retry6 attempt claim file/evidence binding differs")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    _consensus(attempt_claim, label="permanent retry6 preoptimizer attempt claim")
