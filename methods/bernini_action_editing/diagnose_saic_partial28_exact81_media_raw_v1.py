#!/usr/bin/env python3
"""CPU-only exact81 camera/technical/temporal diagnostics for sealed SAIC r6.

The only accepted bank index is the already sealed exact28 source-bound input
...
import diagnose_saic_partial28_source_bound_dinov2_raw_v1 as frozen_specialization  # noqa: E402
import saic_exact81_media_diagnostics_v1 as diagnostics  # noqa: E402

# The r6 exact28 specialization configures its pinned exact47 implementation
# and its nested temporal helper to the exact28 executable identity.  Use that
# configured implementation directly so manifest self/source validation stays
# bound to the frozen r6 exact28 validator, while the base algorithm remains
# transitively pinned by the specialization itself.
frozen_input = frozen_specialization.core


SCHEMA_VERSION = "bernini-saic-partial28-exact81-media-raw-v1"
...
def _configure_frozen_input() -> None:
    ...
    expected_path = (METHOD_ROOT / "diagnose_saic_partial28_source_bound_dinov2_raw_v1.py").resolve(
        strict=True
    )
    if (
        ...
        raise Exact47MediaRawError("frozen r6 exact28 input-validator identity differs")
...
def load_and_validate_input_manifest(
    ...
    if expected != EXPECTED_INPUT_MANIFEST_SHA256:
        raise Exact47MediaRawError("only the sealed r6 exact28 input manifest is accepted")
    try:
    ...
    if raw_sha != EXPECTED_INPUT_MANIFEST_SHA256:
        raise Exact47MediaRawError("sealed r6 exact28 input manifest SHA-256 differs")
    if (
        ...
        raise Exact47MediaRawError("sealed r6 exact28 bank contract differs")
    ...
    if type(rows) is not list or len(rows) != EXPECTED_CANDIDATE_COUNT:
        raise Exact47MediaRawError("sealed bank must contain exactly 28 attempts")
    ...
    if len(receipt_paths) != EXPECTED_CANDIDATE_COUNT or receipt_paths != sealed_receipt_paths:
        raise Exact47MediaRawError("attempt root is not the sealed exact28 receipt set")
    ...
    if (
        ...
        raise Exact47MediaRawError("exact28 candidate/MP4/source coverage differs")
...
def run_full(args: Any) -> int:
    ...
    if (
        ...
        raise Exact47MediaRawError("full aggregate exact28/runtime coverage differs")
