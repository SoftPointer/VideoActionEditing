#!/usr/bin/env python3
"""V4E exposed-five-view global codec over frozen V-JEPA2 sequences.
...
Training deliberately exposes all five independently materialized backbone
views for each model-fit IID.  The view axis is exchangeable: reconstruction
is averaged equally over all views and the geometry term averages all ten
unordered view pairs.  No view name, positive/negative role, family, action,
or strict-selection label enters the model or loss.  Checkpoint selection uses
inner-validation originals only and OOF values are inaccessible until the
selected checkpoint is sealed.
...
SCHEMA = "semantic-anchor-vjepa2-multiview-global-codec-exact5-receipt-v4e"
...
class Config:
    ...
    def validate(self) -> None:
        if self != Config():
            raise ValueError("v4-E configuration is immutable")
...
def _binding() -> dict[str, str]:
    ...
    if (
        ...
        raise RuntimeError("v4-E frozen implementation dependency differs")
    return result
...
def _split_fold(
    ...
    if (
        ...
        raise ValueError("v4-E outer population closure differs")
    exploratory_oof = [
    ...
    if not by_family or len(by_family) > 28:
        raise ValueError("v4-E outer-train family closure differs")
    ...
    if (
        ...
        raise ValueError("v4-E model-fit/inner/OOF split closure differs")
    split = {
    ...
    if split["outer_assignment_digest"] == v4c.OUTER_ASSIGNMENT_DIGEST:
        ...
        if literal != FROZEN_INNER_SPLITS[outer_fold]:
            raise ValueError("v4-E frozen real exact644 inner split differs")
...
class ClipPCAInitializedVJepaGlobalCodec(nn.Module):
    ...
    def __init__(self, fitted: ClipPCAFit, fit_only_rms: torch.Tensor) -> None:
        ...
        if count != EXACT_TRAINABLE_PARAMETERS or count >= MAX_TRAINABLE_PARAMETERS:
            raise RuntimeError("v4-E codec parameter closure differs")
...
def _train_fold_model(
    ...
    audit = {
        ...
        "inner_validation_ordered_iids": [row.iid for row in inner_validation],
...
def _evaluate_fold(
    ...
    model: VJepa2GlobalCodec, selected_step: int,
    ...
    if not oof_rows or any(set(row.views) != set(EVAL_VIEWS) for row in oof_rows):
        raise ValueError("v4-E OOF five-view closure differs")
    stacked_cpu = {
    ...
    for index, row in enumerate(oof_rows):
        ...
        output.append({
            ...
            "selected_step0_exact_clip_pca_alias_used": step0_alias_used,
        })
    return output
...
def _paired_lcb(
    ...
    result = v4a._paired_bootstrap_lcbs(values, families, config, f"v4e:{label}")
...
def _paired_ratio_ucb(
    ...
    clip_seed = v4a._bootstrap_seed(config, "v4e", label, "ratio", "clip")
    ...
    family_seed = v4a._bootstrap_seed(config, "v4e", label, "ratio", "family")
...
def _verify_v4c_embedded_teacher_evidence(
    ...
    if len(upstream) != 644 or set(upstream) != {row["iid"] for row in rows}:
        raise ValueError("v4-C/v4-E exact644 IID evidence closure differs")
    max_teacher = 0.0
    for row in rows:
        reference = upstream[row["iid"]]
        if (reference["family"] != row["family"]
                or int(reference["outer_fold"]) != int(row["outer_fold"])):
            raise ValueError("v4-C/v4-E family or fold authority differs")
        for negative in NEGATIVES:
    ...
    if max_teacher > 1.0e-12:
        raise ValueError("v4-C/v4-E teacher evidence differs")
    return {
...
def _run_fold(
    ...
    stage1_request = {
        ...
        **{iid: ("original",) for iid in validation_iids},
    }
    stage1_rows, stage1_audit = _selective_materialize_feature_rows(
        feature_index, stage1_request, stage="stage1_model_fit_and_inner"
    )
    model_fit_rows = [stage1_rows[iid] for iid in fit_iids]
    validation_rows = [stage1_rows[iid] for iid in validation_iids]
    expected_stage1_count = len(fit_iids) * len(EVAL_VIEWS) + len(validation_iids)
    if (
        stage1_audit["semantic_tensor_materialized_count"] != expected_stage1_count
        or stage1_audit["semantic_tensor_materialized_count_by_view"]["original"]
            != len(fit_iids) + len(validation_iids)
        or any(
            stage1_audit["semantic_tensor_materialized_count_by_view"][view]
                != len(fit_iids)
            for view in EVAL_VIEWS if view != "original"
        )
    ):
    ...
    model, selected_step, training_audit = _train_fold_model(
        model_fit_rows, validation_rows, fitted,
    ...
    checkpoint = _save_selected_checkpoint_create_only(
        ...
        config, fold_index, run_binding, validation_rows, device,
    )
    # This is the first semantic materialization of any exploratory OOF tensor.
    # It occurs only after the selected checkpoint was create-only written,
    # chmod 0444, single-FD reloaded, and copied back into the caller model.
    ...
    if (
        ...
        raise RuntimeError("stage2 selective OOF tensor read closure differs")
    oof_rows = [stage2_rows[iid] for iid in oof_iids]
    evaluation = _evaluate_fold(
        oof_rows, model, selected_step, fitted, config, device
    )
    ...
    fold_receipt = {
        ...
        "selective_feature_materialization": {
            ...
            "stage2_only_after_selected_checkpoint_strong_seal_reload": stage2_audit,
            "stage1_model_fit_iid_cluster_count": len(fit_iids),
            "stage1_inner_validation_iid_cluster_count": len(validation_iids),
            "stage1_original_tensor_count": len(fit_iids) + len(validation_iids),
            "stage1_each_derived_view_tensor_count": len(fit_iids),
            "stage1_oof_semantic_tensor_count": 0,
            "stage1_inner_derived_semantic_tensor_count": 0,
            "stage2_oof_iid_cluster_count": len(oof_iids),
            "stage2_each_view_tensor_count": len(oof_iids),
            "stage2_model_fit_or_inner_semantic_tensor_count": 0,
            "oof_first_semantic_materialization_after_checkpoint_seal": True,
        },
...
def _load_selected_checkpoint_sealed(
    ...
    if (
        ...
        != "semantic-anchor-vjepa2-multiview-global-codec-selected-fold-checkpoint-v4e"
...
def _save_selected_checkpoint_create_only(
    path: Path, model: VJepa2GlobalCodec,
    fitted: ClipPCAFit, selected_step: int,
    ...
    metadata: dict[str, Any] = {
        "schema_version": "semantic-anchor-vjepa2-multiview-global-codec-selected-fold-checkpoint-v4e",
...
def _selective_materialize_feature_rows(
    ...
    if stage not in {"stage1_model_fit_and_inner", "stage2_postseal_oof"}:
...
def _prepare_authorities(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("v4-E feature/split/frontier population differs")
    return {
...
def _resolve_fold_root(path_string: str, *, fresh: bool) -> tuple[Path, Path, Path]:
    ...
    if fresh and (
        ...
        raise ValueError("fold root output artifacts must both be fresh")
    return root, fold_path, checkpoint_path


def run_train_fold(args: argparse.Namespace) -> dict[str, Any]:
    """Train exactly one fold, seal its checkpoint, then emit OOF evidence."""

    run_binding = _binding()
    config = Config()
    config.validate()
    if str(torch.__version__) != "2.7.1+rocm6.3":
        raise RuntimeError("v4-E torch runtime differs")
    ...
    fold_root, fold_path, checkpoint_path = _resolve_fold_root(
    ...
    fold, evidence = _run_fold(
        ...
        checkpoint_path, run_binding, authority["feature_index"],
    )
    if (
    ...
    _load_selected_checkpoint_sealed(checkpoint_path, fold["selected_checkpoint_artifact"])
    ...
    return {
        ...
        "selected_checkpoint": str(checkpoint_path.resolve(strict=True)),
...
def _load_fold_receipt_sealed(
    ...
    root, path, checkpoint_path = _resolve_fold_root(fold_root_string, fresh=False)
    ...
    inner_iids = (
        ...
        if type(training) is dict else None
    )
    if (
        ...
        or value.get("upstream_authorities", {}).get("v4d_burned_receipt_digest")
            != V4D_RECEIPT_DIGEST
        or type(fold) is not dict or type(fold.get("fold_index")) is not int
        ...
        or len(evidence) != FROZEN_OOF_COUNTS[fold["fold_index"]]
        ...
        or artifact.get("path") != str(checkpoint_path.resolve(strict=True))
        or artifact.get("outer_fold") != fold["fold_index"]
        or artifact.get("selected_step") != training.get("selected_step")
        or artifact.get("model_state_sha256")
...
def _verify_fold_selective_materialization_ledger(fold: Mapping[str, Any]) -> None:
    """Recompute both selective request ledgers from sealed ordered IID lists."""

    fit_iids = fold.get("model_fit_ordered_iids")
    inner_split = fold.get("inner_split")
    # The fold receipt intentionally does not duplicate inner ordered IIDs in
    # the outer envelope; training audit is the sealed ordering authority.
    ...
    stage1_request = {
        **{iid: list(EVAL_VIEWS) for iid in fit_iids},
        **{iid: ["original"] for iid in inner_iids},
    }
    stage2_request = {iid: list(EVAL_VIEWS) for iid in oof_iids}
    stage1 = ledger.get("stage1_before_checkpoint_seal")
    stage2 = ledger.get("stage2_only_after_selected_checkpoint_strong_seal_reload")
    expected_stage1_counts = {
        view: len(fit_iids) + (len(inner_iids) if view == "original" else 0)
        for view in EVAL_VIEWS
    }
    expected_stage2_counts = {view: len(oof_iids) for view in EVAL_VIEWS}
    if (
        ...
        or type(stage1) is not dict or type(stage2) is not dict
        ...
        or ledger.get("stage1_inner_derived_semantic_tensor_count") != 0
        or ledger.get("stage2_model_fit_or_inner_semantic_tensor_count") != 0
        or ledger.get("oof_first_semantic_materialization_after_checkpoint_seal") is not True
    ):
...
def run_aggregate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        ...
        "training_contract": {
            ...
            "inner_checkpoint_selection_original_raw_mse_only": True,
            "oof_selection": False, "selected_steps_by_fold": selected_steps,
        },
        "evaluation_contract": {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Exact5 exposed-five-view V-JEPA2 global clip-PCA codec"
    )
